...
import { browserDeviceProfile } from "./deviceProfile";
import { useSession } from "./SessionProvider";

/**
 * Pairing entry point shown when this browser has no device token yet. Two
 * paths: redeem a pairing code (primary — produces a TRUSTED device immediately)
 * or request approval (fallback — produces a PENDING device whose token
 * activates once the owner approves). On either success we persist the returned
 * token + device into the session; the request path then waits for the realtime
 * approval to flip the device to trusted (the app re-renders once paired).
 */
export function JoinScreen() {
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-app-background p-4 text-app-foreground">
      <section
        data-testid={selectors.join.screen}
        aria-labelledby="join-heading"
        className="w-full max-w-md rounded-panel border border-app-border bg-app-surface p-6"
      >
        ...
        {requested ? (
          <div data-testid={selectors.join.waiting} className="mt-6 rounded-panel border border-app-border bg-app-surface-muted p-4">
