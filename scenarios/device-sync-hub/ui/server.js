...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start device-sync-hub` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start device-sync-hub` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.device_sync_hub\.v1\./

startScenarioServer({
  uiPort,
  apiPort,
  distDir: './dist',
  serviceName: 'device-sync-hub',
  corsOrigins: '*',
  setupRoutes(app) {
