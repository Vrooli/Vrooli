...
func TestRunHelpListsMigratedDomains(t *testing.T) {
	...
	// subcommand groups. Help is rendered by cli-core; we only verify Run
	// returns without error. The presence of each domain in the help surface
	// is covered by cli-core's own tests.
...
func TestMetadata(t *testing.T) {
	if !strings.EqualFold(appName, "device-sync-hub") {
		t.Fatalf("appName = %q, want device-sync-hub", appName)
	}
