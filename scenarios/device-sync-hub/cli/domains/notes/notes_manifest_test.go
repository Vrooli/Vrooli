...
// the template itself does not build because of device-sync-hub placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_device_sync_hub_v1_notes_notes_proto, "NotesService")
}
