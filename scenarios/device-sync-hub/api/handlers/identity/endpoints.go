...
var Endpoints = []module.EndpointDescriptor{
	{
		...
		Description: "Forwards email + password to scenario-authenticator (resolved by name via api-core/discovery) and returns the issued owner JWT. The hub owns no credential logic — it relays. Called same-origin by the hub UI/CLI so the browser never makes a cross-origin call.",
		Category:    "identity",
		...
		Examples: []module.Example{
			{Name: "Sign in", Curl: "curl http://localhost:${API_PORT}/vrooli.device_sync_hub.v1.identity.IdentityService/Login -H 'Content-Type: application/json' -d '{\"email\":\"you@example.com\",\"password\":\"…\"}'"},
		},
	},
	{
		...
		Description: "Creates an owner account in scenario-authenticator and returns the issued owner JWT (the account is signed in immediately, so a fresh owner can set up their first device in one flow). Relayed via api-core/discovery; same-origin only.",
		Category:    "identity",
		...
		Examples: []module.Example{
			{Name: "Create account", Curl: "curl http://localhost:${API_PORT}/vrooli.device_sync_hub.v1.identity.IdentityService/Register -H 'Content-Type: application/json' -d '{\"email\":\"you@example.com\",\"password\":\"…\"}'"},
		},
	},
}
