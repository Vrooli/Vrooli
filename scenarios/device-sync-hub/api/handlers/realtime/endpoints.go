...
// stream. It is a documented REST exception (event_stream) — a long-lived
// server->client push channel the browser EventSource API consumes, which cannot
// ride Connect framing. Each event's payload is the proto-typed realtime.Event
// marshaled with protojson, so the wire shape stays contract-bound even though
// the streaming envelope is REST.
//
...
var Endpoints = []module.EndpointDescriptor{
	{
		...
		RESTException: &module.RESTException{
			Reason: module.RESTReasonEventStream,
			Note:   "Long-lived SSE stream (text/event-stream) consumed by the browser EventSource API; each event payload is the proto-typed realtime.Event.",
		},
	},
}
