# Troubleshooting — Device Sync Hub
...
make stop
vrooli scenario start device-sync-hub --clean-stale
```
...
device-sync-hub status --auto-start

# Or override the API base for this invocation
device-sync-hub status --api-base "http://localhost:$(vrooli scenario port device-sync-hub API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/device-sync-hub/`.
...
The default route is `${SCENARIO_DATA_DIR}/device-sync-hub.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/device-sync-hub.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/device-sync-hub.db}")"
```
...
`packages/proto/gen/go/device-sync-hub/v1/`, TypeScript under
`packages/proto/gen/typescript/js/device-sync-hub/v1/`, and Python under
`packages/proto/gen/python/device_sync_hub/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/device-sync-hub/v1/<domain>/`.
