...
envelope (`packages/proto/schemas/device-sync-hub/v1/errors/errors.proto`):
...
| **CLI** | `device-sync-hub status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/device-sync-hub/v1/health/health.proto`
...
### `POST /vrooli.device_sync_hub.v1.notes.NotesService/ListNotes`
...
| **CLI** | `device-sync-hub notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.device_sync_hub.v1.notes.NotesService/ListNotes" \
...
### `POST /vrooli.device_sync_hub.v1.notes.NotesService/CreateNote`
...
| **CLI** | `device-sync-hub notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.device_sync_hub.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
### `POST /vrooli.device_sync_hub.v1.notes.NotesService/GetNote`
...
| **CLI** | `device-sync-hub notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.device_sync_hub.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `device-sync-hub notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/device-sync-hub/v1/notes/notes.proto`.
