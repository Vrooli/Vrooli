...
`~/.vrooli/bin/device-sync-hub`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start device-sync-hub` if the API is unreachable |
...
### `device-sync-hub configure <key> <value>`
...
```bash
device-sync-hub configure api_base
```
...
```bash
device-sync-hub notes list
device-sync-hub notes list --json
```

### `device-sync-hub notes create --title <title> [--body <body>]`
...
```bash
device-sync-hub notes create --title "First note" --body "Hello world"
```
...
```bash
device-sync-hub notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start device-sync-hub`" is good.
