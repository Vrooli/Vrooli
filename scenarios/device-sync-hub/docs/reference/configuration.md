...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/device-sync-hub.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `device-sync-hub` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/device-sync-hub/config.json`
3. `~/.vrooli/config/device-sync-hub/config.json`
4. `~/.config/vrooli/device-sync-hub/config.json`
...
```bash
device-sync-hub configure api_base http://localhost:15001/api/v1
device-sync-hub configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port device-sync-hub API_PORT`)
