...
`template-manager orient device-sync-hub`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient device-sync-hub
...
```bash
react-component-library components ingest device-sync-hub <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> device-sync-hub
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario device-sync-hub
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> device-sync-hub <adopted-path>
```
