# Troubleshooting — Backdrop Studio
...
```bash
vrooli scenario status backdrop-studio
make stop
vrooli scenario start backdrop-studio --clean-stale
```
...
```bash
make status
vrooli scenario port backdrop-studio UI_PORT
```
...
backdrop-studio status --auto-start

# Or override the API base for this invocation
backdrop-studio status --api-base "http://localhost:$(vrooli scenario port backdrop-studio API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/backdrop-studio/`.
...
The default route is `${SCENARIO_DATA_DIR}/backdrop-studio.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/backdrop-studio.db"
```
...
`packages/proto/gen/go/backdrop-studio/v1/`, TypeScript under
`packages/proto/gen/typescript/backdrop-studio/v1/`, and Python under
`packages/proto/gen/python/backdrop_studio/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/backdrop-studio/v1/<domain>/`.
