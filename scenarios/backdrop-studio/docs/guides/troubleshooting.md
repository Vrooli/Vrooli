...
```bash
export SQLITE_PATH=/tmp/backdrop-studio.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/backdrop-studio.db}")"
```
...
`packages/proto/gen/typescript/js/backdrop-studio/v1/`, and Python under
