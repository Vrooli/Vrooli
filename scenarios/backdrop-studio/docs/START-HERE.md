...
`template-manager orient backdrop-studio`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient backdrop-studio
...
```bash
cat > /tmp/prd_context_backdrop-studio.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start backdrop-studio --interactive
# …answer the interview, then:
business-health wizard preview backdrop-studio
business-health wizard apply backdrop-studio
```
...
```bash
vrooli scenario requirements validate backdrop-studio --json
```
...
```bash
cat > /tmp/requirements_context_backdrop-studio.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate backdrop-studio --json
business-health fix preview backdrop-studio
business-health fix apply backdrop-studio
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate backdrop-studio --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate backdrop-studio`, the notes page spec
...
validate backdrop-studio --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/backdrop-studio/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest backdrop-studio <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> backdrop-studio
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario backdrop-studio
```
...
- Wire contracts belong in
  `packages/proto/schemas/backdrop-studio/v1/<domain>/`.
