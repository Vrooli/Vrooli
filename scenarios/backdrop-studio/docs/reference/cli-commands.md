...
`~/.vrooli/bin/backdrop-studio`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start backdrop-studio` if the API is unreachable |
...
```bash
backdrop-studio status
backdrop-studio status --json
```

### `backdrop-studio configure <key> <value>`
...
```bash
backdrop-studio configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`backdrop-studio <domain> <verb>`). Every command calls a single API
...
```bash
backdrop-studio notes list
backdrop-studio notes list --json
```

#### `backdrop-studio notes create --title <title> [--body <body>]`
...
```bash
backdrop-studio notes create --title "First note" --body "Hello world"
```
...
```bash
backdrop-studio notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start backdrop-studio`" is good.
