...
#### `POST /vrooli.backdrop_studio.v1.notes.NotesService/ListNotes`
...
| **CLI** | `backdrop-studio notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.backdrop_studio.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.backdrop_studio.v1.notes.NotesService/CreateNote`
...
| **CLI** | `backdrop-studio notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.backdrop_studio.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.backdrop_studio.v1.notes.NotesService/GetNote`
...
| **CLI** | `backdrop-studio notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.backdrop_studio.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `backdrop-studio notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/backdrop-studio/v1/notes/notes.proto`.
