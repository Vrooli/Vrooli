...
# Run standard structure checks
# ...

# Run UI smoke if configured
if [ -f "${TESTING_PHASE_SCENARIO_DIR}/.vrooli/testing.json" ]; then
  ui_smoke_enabled=$(jq -r '.phases.structure.uiSmoke.enabled // false' \
    "${TESTING_PHASE_SCENARIO_DIR}/.vrooli/testing.json")

  if [ "$ui_smoke_enabled" = "true" ]; then
    source "${APP_ROOT}/scripts/scenarios/testing/shell/ui-smoke.sh"

    if testing::ui_smoke::run_all; then
      log::success "UI smoke tests passed"
    else
      testing::phase::add_error "UI smoke tests failed"
    fi
  fi
fi
...
The `ui-smoke.sh` library provides:

```bash
# Run all configured smoke tests
testing::ui_smoke::run_all

# Run smoke test for specific page
testing::ui_smoke::test_page "/dashboard" "Dashboard"

# Check for console errors
testing::ui_smoke::check_console_errors

# Capture screenshot
testing::ui_smoke::capture_screenshot "page-name"
```
