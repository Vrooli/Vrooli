...
| Pixel render-health | `internal/visualcheck` (`RenderHealth`, `Compare`, `Thresholds`/`ThresholdsFromEnv`); applied in `smoke/allpages.go` + `smoke/runner.go` (`applyRenderHealth` → `evidence.Evidence.RenderBroken`) | stdlib `image/png` only; downscales to a luminance grid; a blank/solid render sets `RenderBroken`, which `evidence.Analyze` ranks below network failure, above page exceptions, as a **hard fail** with the page named. Decode/read errors degrade to a skip. | `visualcheck_test.go` (synthetic solid/gradient/shifted PNGs + lever overrides); `allpages_test.go` (`TestBaselineProfile_BlankRenderHardFails`). | **Observability:** the smoke phase is the authority for "the page is visibly broken" (pixels); the playbooks DOM-blank check is the authority where there is no screenshot. One concept, two producers, never double-reported. Thresholds are the `TEST_GENIE_VISUAL_*` levers (see configuration reference). |
| Visual comparison RPC | `internal/app/runs/service.go` (`CompareRunVisuals`) over `internal/visualcheck.Compare` | Per-page pixel comparison of two runs' captures → neutral deltas (`identical`/`changed`/`added`/`removed` + changed fraction). test-genie owns the analyzer; an unreadable capture degrades to `changed` (never a failure). | `compare_visuals_test.go` against `t.TempDir()` (synthetic per-page screenshots). | **Observability:** the single home for visual comparison. git-control-tower's baseline visuals surface consumes these deltas and renders them as its advisory `changed` tier — GCT never re-derives pixel math. |
