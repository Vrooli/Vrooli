...
func BuildPrompt(cfg PromptConfig) string {
	...
	sb.WriteString("   - `__test/` directory for shell-based tests\n")
	...
	sb.WriteString("**When fixing shell scripts (install/lib helpers, bash tests), follow these MANDATORY rules:**\n\n")
	sb.WriteString("### Cleanup Function Safety\n")
	sb.WriteString("Shell cleanup functions can run with unset variables (for example when setup exits early), so an unguarded `rm -f \"$VAR\"*` can expand to `rm -f *` in the current directory.\n\n")
	sb.WriteString("```bash\n")
	...
	sb.WriteString("**ALWAYS** set critical variables before any early-exit conditions:\n\n")
	sb.WriteString("```bash\n")
