...
// controlling how deeply a run captures baseline UI visuals. It is orthogonal
// to the phase set (which phases run) and affects only diagnostic visual
// artifact depth.
//
...
//	default: "" (default depth) — no all-pages baseline visual capture.
//	         This is the FLOOR: routine `comprehensive` runs pay nothing extra.
...
//	effect:  gates all-pages baseline visual capture and whether video is captured.
...
type Profile struct {
	// AllPages requests one visual capture per discovered page.
	AllPages bool
...
const (
	// NameDefault is the empty default profile: no extra visual capture cost.
	NameDefault = ""
