...
func detectRuntimes(env workspace.Environment) []string {
	var runtimes []string
...
func hasGoSources(env workspace.Environment) bool {
	candidates := []string{
...
func hasPythonWorkspace(env workspace.Environment) bool {
	candidates := []string{
...
func detectPackageManagers(env workspace.Environment) []string {
	var managers []string
	lockFiles := []struct {
		Path    string
		Command string
...
func reportResourceExpectations(env workspace.Environment, logWriter io.Writer) ([]string, error) {
