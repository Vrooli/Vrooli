...
func TestLoaderLoadSuccess(t *testing.T) {
	...
	if registry.Playbooks[0].File != "test/playbooks/first.json" {
		t.Errorf("expected first.json first, got %s", registry.Playbooks[0].File)
	}
	if registry.Playbooks[1].File != "test/playbooks/second.json" {
...
func TestLoaderLoadDeprecatedFallback(t *testing.T) {
	...
	content := `{
		...
		"deprecated_playbooks": [
			{"file": "test/playbooks/legacy.json", "description": "Legacy", "order": "1"}
		]
	}`
	...
	if registry.Playbooks[0].File != "test/playbooks/legacy.json" {
...
func TestLoaderRegistryPath(t *testing.T) {
	...
	expected := "/test/dir/playbooks/registry.json"
...
func setupValidRegistry(t *testing.T, testDir string) {
	...
	content := `{
		...
		"playbooks": [
			{"file": "test/playbooks/second.json", "description": "Second test", "order": "2"},
			{"file": "test/playbooks/first.json", "description": "First test", "order": "1"}
		]
	}`
	if err := os.WriteFile(filepath.Join(playbooksDir, RegistryFileName), []byte(content), 0o644); err != nil {
		t.Fatalf("failed to write registry: %v", err)
