...
type ScenarioSummary struct {
	...
	LastExecutionPhases       []PhaseExecutionResult `json:"lastExecutionPhases,omitempty"`
	LastExecutionPhaseSummary *PhaseSummary          `json:"lastExecutionPhaseSummary,omitempty"`
...
func scanScenarioSummary(scanner rowScanner) (ScenarioSummary, error) {
	...
	if len(lastExecutionPhases) > 0 {
		...
		if err := json.Unmarshal(lastExecutionPhases, &phases); err == nil {
			...
			if len(phases) > 0 {
				summaryValue := summarizePhases(phases)
