...
func validateStructureDirs(scenarioDir string, required []string, logWriter io.Writer) PhaseRunReport {
...
func validateStructureFiles(scenarioDir string, required []string, logWriter io.Writer) PhaseRunReport {
	for _, rel := range required {
		abs := resolveStructurePath(scenarioDir, rel)
...
func validateServiceManifest(manifestPath, scenarioName string, logWriter io.Writer, expectations *structureExpectations) PhaseRunReport {
