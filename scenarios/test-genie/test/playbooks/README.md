...
- `__seeds/` includes a seed entrypoint (`seed.go` preferred; `seed.sh` allowed) that writes `test/artifacts/runtime/seed-state.json`.
...
- Playbooks automatically start the scenario against temporary Postgres/Redis for this phase. Seeds run once and write `test/artifacts/runtime/seed-state.json`.
