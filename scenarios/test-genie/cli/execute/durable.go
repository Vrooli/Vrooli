...
func printAgentWaitBlock(w io.Writer, scenario, runID string, eta int, etaKnown bool, waitSeconds int) {
	...
	fmt.Fprintf(w, "    Advisory duration estimate: ~%s; recommended wait timeout: %s.\n", etaStr, humanDuration(waitSeconds))
	fmt.Fprintf(w, "    The estimate is not a failure deadline: do not call the run stalled or block the plan when it is exceeded.\n")
	fmt.Fprintf(w, "    In coding-agent tool execution, give the command at least this timeout and do not poll with short output checks. Only Test Genie terminal evidence can establish the verdict.\n")
...
func toStartRunRequest(req Request) *runspb.StartRunRequest {
	return &runspb.StartRunRequest{
		...
		CaptureProfile:               req.CaptureProfile,
		RetainForEvidence:            req.RetainForEvidence,
		RetentionReason:              req.RetentionReason,
		UiUrl:                        req.UIURL,
		ApiUrl:                       req.APIURL,
		ScenarioPath:                 req.ScenarioPath,
		LogicalRepoRoot:              req.LogicalRepoRoot,
		LogicalScenarioRelPath:       req.LogicalScenarioRelPath,
		ReleaseProfileId:             req.ReleaseProfileID,
		ReleaseCandidateCommit:       req.ReleaseCandidateCommit,
		ReleaseArtifactDigest:        req.ReleaseArtifactDigest,
		ReleaseTargets:               append([]string(nil), req.ReleaseTargets...),
		ReleaseChannel:               req.ReleaseChannel,
		ReleasePolicyVersion:         req.ReleasePolicyVersion,
		ReleaseCandidateId:           req.ReleaseCandidateID,
		ReleaseDestinationRevisionId: req.ReleaseDestinationRevisionID,
		ReleaseAuthorizationEpoch:    req.ReleaseAuthorizationEpoch,
		ReleasePredecessorRunId:      req.ReleasePredecessorRunID,
	}
}
