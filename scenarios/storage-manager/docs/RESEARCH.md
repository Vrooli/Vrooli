# Research — Storage Manager

## Fit Summary

Storage Manager fills the cleanup policy/orchestration gap between
...
- `storage-manager` validates storage architecture and test isolation.
- `workspace-sandbox` owns sandbox-specific lifecycle and garbage
  collection.

Storage Manager should not duplicate those responsibilities. It owns the
...
- Compound-value seam: scenario-owned providers let future scenarios
  expose private cleanup safely while Storage Manager keeps policy and
  audit centralized.
...
- `scenarios/storage-manager/PRD.md`
