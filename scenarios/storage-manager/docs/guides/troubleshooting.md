# Troubleshooting — Storage Manager
...
```bash
vrooli scenario status storage-manager
# Then either:
make stop
# Or, as last resort:
pkill -f 'storage-manager-api'
...
```bash
make status
vrooli scenario port storage-manager UI_PORT
```
...
storage-manager status --auto-start

# Or override the API base for this invocation
storage-manager status --api-base "http://localhost:$(vrooli scenario port storage-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/storage-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/storage-manager.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/storage-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/storage-manager.db}")"
```
...
`packages/proto/gen/go/storage-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/js/storage-manager/v1/`, and Python under
`packages/proto/gen/python/cleanup_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/storage-manager/v1/<domain>/`.
