...
`template-manager orient storage-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient storage-manager
...
```bash
cat > /tmp/prd_context_storage-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start storage-manager --interactive
# …answer the interview, then:
business-health wizard preview storage-manager
business-health wizard apply storage-manager
```
...
```bash
vrooli scenario requirements validate storage-manager --json
```
...
```bash
cat > /tmp/requirements_context_storage-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate storage-manager --json
business-health fix preview storage-manager
business-health fix apply storage-manager
```
...
react-component-library adoptions resolve-path COMPONENT_ID storage-manager
react-component-library adoptions apply COMPONENT_ID storage-manager ADOPTED_PATH
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate storage-manager --json`.
...
validate storage-manager --json` passes.
...
- [x] **Start in proto.** CleanupService lives in `packages/proto/schemas/storage-manager/v1/cleanup/cleanup.proto`.
- [ ] For additional domains, author `packages/proto/schemas/storage-manager/v1/<domain>/<domain>.proto`
...
Storage Manager has removed the template example domain. Keep it that way:

- [x] Notes API, CLI, UI, experience, BAS, and proto artifacts are absent.
- [ ] Run `make test`.
- [ ] Confirm the gate: `template-manager orient storage-manager` should not
      report surviving example-domain markers.

**Exit criteria:** only health plus storage-manager product domains remain.
...
```bash
react-component-library components ingest storage-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> storage-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario storage-manager
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> storage-manager <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/storage-manager/v1/<domain>/`.
