...
Storage Manager owns cleanup policy, planning, approval, replay safety,
...
| `vrooli-autoheal` | Detects failures and offers recovery surfaces. | Broad disk/Docker cleanup actions hand off to `storage-manager cleanup plan`; autoheal does not run prune/vacuum/cache deletion directly. |
...
| `test-genie` and `web-console` | Own run/session retention semantics. | `test-genie-run-retention` and `web-console-sessions` are disabled-by-default owner hooks, not filesystem crawlers in storage-manager. |
...
diagnostic links when storage-manager is unavailable, while storage-manager
...
- **An unknown band is rejected, never defaulted.** `critical` authorises
  deletion, so a value storage-manager does not recognise fails the request.
