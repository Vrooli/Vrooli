...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/storage-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `storage-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/storage-manager/config.json`
3. `~/.vrooli/config/storage-manager/config.json`
4. `~/.config/vrooli/storage-manager/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port storage-manager API_PORT`)
