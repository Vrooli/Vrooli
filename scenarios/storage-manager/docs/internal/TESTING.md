...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/storage-manager/v1/shared/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/storage-manager/v1/shared"`)
...
import (
    ...
    "storage-manager/internal/testutil/assertx"
    "storage-manager/internal/testutil/httpx"
...
The proto schema in `packages/proto/schemas/storage-manager/v1/shared/health.proto`
...
dynamically from `go.mod`, so it works after `storage-manager`
substitution.
...
import (
    ...
    clitest "storage-manager/cli/internal/testutil"
)
