...
The `health` proto in `packages/proto/schemas/storage-manager/v1/shared/`
...
### Cleanup provider contract

| | |
|---|---|
| **Seam** | Provider lifecycle contract |
| **Interface** | `api/internal/cleanup::Provider` (`Metadata`, `Estimate`, `Preview`, `Apply`, `Verify`) |
| **Production wiring** | Future provider registry constructors will validate each provider with `cleanup.ValidateProvider` before registration. Providers without preview-first metadata or safe default gates are rejected. |
| **Test fake** | Domain tests use lightweight provider stubs plus fakes under `api/internal/testutil/cleanup`. |
| **Why it exists** | Cleanup providers can be destructive. The interface forces Estimate and Preview to exist before Apply, while metadata exposes safety tier, approvals, owner, privileges, irreversible effects, provider version, and test substitute. |

### Cleanup FileSystem

| | |
|---|---|
| **Seam** | Filesystem cleanup target inspection and deletion |
| **Interface** | `api/internal/cleanup::FileSystem` (`Stat`, `Walk`, `RemoveAll`) |
| **Production wiring** | Future production constructors are the only allowed place to wrap host filesystem calls. |
| **Test fake** | `api/internal/testutil/cleanup::FileSystem` blocks removal unless explicitly enabled and scoped under its fake root. |
| **Why it exists** | Tests and previews must never delete real host paths. Fake-root enforcement proves provider apply logic can be tested without touching live cleanup targets. |

### Cleanup ProcessRunner

| | |
|---|---|
| **Seam** | Host command execution for privileged cleanup providers |
| **Interface** | `api/internal/cleanup::ProcessRunner` (`Run(ProcessCommand)`) |
| **Production wiring** | Future production constructors will own any `exec.Command` wrapping and command allowlists. |
| **Test fake** | `api/internal/testutil/cleanup::ProcessRunner` records commands and rejects forbidden command strings. |
| **Why it exists** | Journald, apt metadata, and language cache cleanup can require shell commands. Keeping execution behind a typed runner lets tests assert intent without running cleanup commands. |

### Cleanup DockerClient

| | |
|---|---|
| **Seam** | Docker usage inspection and prune operations |
| **Interface** | `api/internal/cleanup::DockerClient` (`SystemUsage`, `Prune`) |
| **Production wiring** | Future production Docker client owns daemon/API integration. No provider should shell out to `docker system prune` directly. |
| **Test fake** | `api/internal/testutil/cleanup::DockerClient` returns canned usage and blocks volume pruning. |
| **Why it exists** | Docker volume cleanup is Conditional and dangerous. The fake makes the default prohibition executable in tests. |

### Cleanup JournalClient
...
| **Why it exists** | Storage Manager orchestrates policy and audit but must not duplicate private deletion logic owned by workspace-sandbox, test-genie, web-console, or future scenarios. |
...
not permission to scan owner-private directories from storage-manager.
...
### CleanupService (orchestration application layer)

| | |
|---|---|
| **Seam** | Cleanup orchestration surface (provider catalog, policy, plan, apply, audit) |
| **Interface** | `api/handlers/cleanup/connect_handler.go::CleanupService` |
| **Production wiring** | `api/handlers/cleanup/module.go::Module(...)` receives an orchestrator service and exposes generated CleanupService Connect handlers. |
| **Test fake** | `api/handlers/cleanup/connect_handler_test.go::fakeCleanupService` records handler inputs and returns canned cleanup-domain values. |
| **Why it exists** | Transport tests should prove proto translation and Connect error mapping without invoking provider previews or apply behavior. The orchestrator owns policy and side effects; the handler stays thin. |

### Cleanup provider side-effect seams

| | |
|---|---|
| **Seam** | Filesystem, process, Docker, journal, clock, and owner-scenario boundaries |
| **Interface** | `api/internal/cleanup/seams.go` |
| **Production wiring** | Built-in providers depend only on the typed interfaces they need; no provider calls `os.Remove`, shell deletion, Docker CLI, or journal commands directly. |
| **Test fake** | `api/internal/testutil/cleanup/fakes.go` supplies fake filesystem, process, Docker, journal, owner-scenario, and clock implementations. |
| **Why it exists** | Cleanup is destructive by nature. Every host-observing or host-mutating capability must be preview-first, policy-gated, and testable without touching real host state. |
