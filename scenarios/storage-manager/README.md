...
Preview-first, policy-gated cleanup orchestration for reclaimable host and scenario storage.

Storage Manager is Vrooli's unified storage census, retention, placement, and cleanup orchestration
capability. It scans reclaimable host and scenario storage, builds
preview-first cleanup plans, enforces safety tiers and operator policy,
and records immutable audit history before any cleanup can be applied.

Ecosystem fit: Storage Manager is a meta-scenario and interface-enabler.
It serves a programmatic CLI/Connect surface for agents and adjacent
scenarios, a direct operator UI, and future action discovery for safe
cleanup workflows. `system-monitor` observes disk pressure,
`vrooli-autoheal` requests remediation, and owner scenarios keep their
...
- Provider registry with Estimate, Preview, Apply, and Verify contracts.
- Safety tiers: Safe, SafeWithOwner, Conditional, and Forbidden.
- Conservative policy profiles with per-provider enablement, approval
  modes, min age, reclaim caps, and privilege handling.
- Replay-safe apply flow requiring exact plan id, policy version,
  provider version, approval mode, and idempotency key.
- Fake-first side-effect seams for filesystem, process, Docker, journald,
  clock, and owner-scenario cleanup hooks.
...
The generated scaffold is intentionally not the product. Storage Manager
...
`test-genie execute storage-manager --preset comprehensive` directly for
...
10. **Never implement cleanup by direct shell deletion.** Route host effects
    through typed seams and keep tests fake/temp-root only.
