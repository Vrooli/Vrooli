...
const storageManagerReadinessPrefix = "STORAGE_MANAGER_DEPLOYMENT_MANAGER_READINESS"
...
func newReadinessReporter(ctx context.Context) ReadinessReporter {
	if strings.TrimSpace(os.Getenv(storageManagerReadinessPrefix+"_TOKEN")) == "" {
		return nil
	}
	baseURL := strings.TrimRight(strings.TrimSpace(os.Getenv(storageManagerReadinessPrefix+"_URL")), "/")
	if baseURL == "" {
	...
	reporter, err := readinessreporter.NewFromEnv(storageManagerReadinessPrefix, baseURL, "storage-architecture-clean", "storage-manager.validate.scenario", "storage-manager")
	if err != nil {
		return readinessConfigurationError{err: err}
	}
	return reporter
...
func (e readinessConfigurationError) Report(context.Context, string, *scenariovalidationv1.ValidateScenarioResponse, time.Time) error {
	if e.err == nil {
		return errors.New("storage-manager readiness reporter is unavailable")
	}
	return e.err
}
