...
// structured queries (not semantic), so storage-manager is NOT a search-hub data
...
// Store seam persists the latest snapshot to storage-manager's own SQLite store
