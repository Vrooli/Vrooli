...
type fileMetadata struct {
	...
	bytes     int64
	allocated int64
	// links is the inode's hard-link count. Only an inode with more than one
	// link can be reached twice by a walk, so the census keys its
	// seen-inode set on this instead of remembering every file on the device.
	links uint64
}
...
func inspectPathHost(path string) (fileMetadata, error) {
	...
	return fileMetadata{identity: fileIdentity{device: uint64(stat.Dev), inode: uint64(stat.Ino), valid: true}, device: uint64(stat.Dev), bytes: bytes, allocated: allocated, links: uint64(stat.Nlink)}, nil
}
