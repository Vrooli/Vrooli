// Package autofix is storage-manager's deterministic remediation registry. It
...
// Candidate aliases the shared auto-fix candidate so storage-manager consumers
// keep a single source of truth with the maturity-go/autofix orchestrator.
type Candidate = autofixcore.Candidate

// registry is the storage-manager fixer set bound to the shared orchestrator.
