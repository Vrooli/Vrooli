...
// until it finds a directory whose scenarios/storage-manager/.vrooli/test-genie.json
...
func findRepoRoot() string {
	...
	for {
		marker := filepath.Join(dir, "scenarios", "storage-manager", ".vrooli", "test-genie.json")
...
// TestHygiene_Dogfood asserts storage-manager's OWN api/ surface trips ZERO
...
func TestHygiene_Dogfood(t *testing.T) {
	...
	if repoRoot == "" {
		t.Skip("repo root not found (storage-manager not on disk); dogfood check skipped")
	}
	...
	if info, err := os.Stat(apiDir); err != nil || !info.IsDir() {
		t.Skip("storage-manager api/ surface not found; dogfood check skipped")
	}

	det := FilesystemDetector{}.Detect(context.Background(), "storage-manager", scenarioDir)
	ac := AnalyzerContext{
	...
	if !ac.IsGo() {
		t.Fatalf("storage-manager api language = %q, want go (dogfood relies on Go detection)", ac.Language)
	}
	...
	for _, a := range hygieneAnalyzers() {
		...
		if len(got) != 0 {
			for _, f := range got {
				t.Errorf("analyzer %q flagged storage-manager: [%s] %s @ %s", a.Name(), f.Code, f.Title, f.Location)
			}
		}
	}
}
