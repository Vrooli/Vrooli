...
// example domains and add their own domains, so pinning the count would
...
// domain's own *_test.go (or provider/orchestrator tests for service domains) for the
...
// api/handlers/<domain>/module_test.go) so the safety net applies
