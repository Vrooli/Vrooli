...
import type { ProviderRenderOptions as BaseRenderOptions, ProviderRenderResult as BaseRenderResult } from "@vrooli/api-base/testing";
export type ProviderRenderOptions = Omit<BaseRenderOptions, "queryClient"> & { queryClient?: QueryClient };
export type ProviderRenderResult = Omit<BaseRenderResult, "queryClient"> & { queryClient: QueryClient };
export const renderWithProviders = (ui: ReactElement, options: ProviderRenderOptions = {}): ProviderRenderResult => {
  // Use this scenario's query context; the shared helper may resolve a separate
  // installed copy after a governed local package refresh.
  const { queryClient: providedClient, ...baseOptions } = options;
  const queryClient = providedClient ?? new QueryClient({ defaultOptions: { queries: { retry: false }, mutations: { retry: false } } });
  return { ...renderWithBaseProviders(createElement(QueryClientProvider, { client: queryClient }, ui), { i18n, ...baseOptions }), queryClient };
};
...
// (e.g. `features/cleanup/mocks/factories.ts`) so deleting a feature
