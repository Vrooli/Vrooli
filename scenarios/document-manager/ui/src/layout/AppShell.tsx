import { BookOpen, FileCheck2, FolderOpen, LayoutDashboard, Settings } from "lucide-react";
...
const NAV_ICONS = {
  ...
  corpus: <FolderOpen aria-hidden className="h-5 w-5" />,
  reader: <BookOpen aria-hidden className="h-5 w-5" />,
  receipt: <FileCheck2 aria-hidden className="h-5 w-5" />,
  settings: <Settings aria-hidden className="h-5 w-5" />,
} as const;

/** Library-owned shell adapter for the Document Manager console. */
export function AppShell() {
  ...
  return (
    <LibraryAppShell
      ...
      sidebarStorageKey="document-manager.sidebar-width"
