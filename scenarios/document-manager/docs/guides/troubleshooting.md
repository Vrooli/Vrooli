...
```bash
vrooli scenario status document-manager
make stop
vrooli scenario start document-manager --clean-stale
```
...
```bash
make status
vrooli scenario port document-manager UI_PORT
```
...
document-manager status --auto-start

# Or override the API base for this invocation
document-manager status --api-base "http://localhost:$(vrooli scenario port document-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/document-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/document-manager.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/document-manager.db"
```
...
`packages/proto/gen/go/document-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/document-manager/v1/`, and Python under
`packages/proto/gen/python/document_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/document-manager/v1/<domain>/`.
