...
```bash
export SQLITE_PATH=/tmp/document-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/document-manager.db}")"
```
...
`packages/proto/gen/typescript/js/document-manager/v1/`, and Python under
