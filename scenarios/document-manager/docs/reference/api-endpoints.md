# API Endpoints — Document Manager
...
envelope (`packages/proto/schemas/document-manager/v1/shared/errors.proto`):
...
| **CLI** | `document-manager status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/document-manager/v1/shared/health.proto`
...
`POST /vrooli.document_manager.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.document_manager.v1.notes.NotesService/ListNotes`
...
| **CLI** | `document-manager notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.document_manager.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.document_manager.v1.notes.NotesService/CreateNote`
...
| **CLI** | `document-manager notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.document_manager.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.document_manager.v1.notes.NotesService/GetNote`
...
| **CLI** | `document-manager notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.document_manager.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `document-manager notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/document-manager/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/document-manager/v1/<domain>/`, then run
   `make generate`.
