...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/document-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `document-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/document-manager/config.json`
3. `~/.vrooli/config/document-manager/config.json`
4. `~/.config/vrooli/document-manager/config.json`
...
```bash
document-manager configure api_base http://localhost:15001/api/v1
document-manager configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port document-manager API_PORT`)
