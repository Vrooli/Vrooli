...
`~/.vrooli/bin/document-manager`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start document-manager` if the API is unreachable |
...
```bash
document-manager status
document-manager status --json
```

### `document-manager configure <key> <value>`
...
```bash
document-manager configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`document-manager <domain> <verb>`). Every command calls a single API
...
```bash
document-manager notes list
document-manager notes list --json
```

#### `document-manager notes create --title <title> [--body <body>]`
...
```bash
document-manager notes create --title "First note" --body "Hello world"
```
...
```bash
document-manager notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start document-manager`" is good.
