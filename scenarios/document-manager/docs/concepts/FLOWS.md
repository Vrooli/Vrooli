...
| Re-derivation | derivation → anchors → handoff | Parser or model upgrade, or an operator correction in the Reader. | A new derivation version; old anchors still resolve; published entries update. | Stateful, bulk, cancellable; must never mutate a prior version. | Target level 4. |
| Ledger publication | handoff | Units are ready and a scope binding exists. | Scoped, idempotent ledger entries carrying provenance. | Queued with retry when the ledger is unreachable; cursor-based. | Target level 4. |
