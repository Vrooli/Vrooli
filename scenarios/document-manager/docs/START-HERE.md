...
`template-manager orient document-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient document-manager
...
```bash
cat > /tmp/prd_context_document-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start document-manager --interactive
# …answer the interview, then:
business-health wizard preview document-manager
business-health wizard apply document-manager
```
...
```bash
vrooli scenario requirements validate document-manager --json
```
...
```bash
cat > /tmp/requirements_context_document-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate document-manager --json
business-health fix preview document-manager
business-health fix apply document-manager
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate document-manager --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate document-manager`, the notes page spec
...
validate document-manager --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/document-manager/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest document-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> document-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario document-manager
```
