...
**Scenario adaptation.** The operational-console language fits Document
Manager well and is adopted without token changes. Its users are
reviewing long documents and audit trails, not browsing marketing
surfaces, so the calm/dense floor is correct: the Reader puts a source
page beside dozens of derived units and needs tight vertical rhythm to
keep both legible at once, and the Receipt is a dense timeline where
tabular alignment matters more than visual flourish. The semantic status
roles carry unusual weight here — they encode **where a step executed**,
which is the product's central claim — so two additional rules bind on
top of the kit rather than replacing it:

1. *Locality is never conveyed by color alone.* Local versus remote
   execution must also differ in label and shape, because a colorblind
   reviewer checking a compliance artifact cannot be asked to
   distinguish green from amber. This is stricter than the kit's
   baseline status contract, not a departure from it.
2. *Confidence is always shown, including when low.* The kit's status
   colors apply to per-unit parse confidence, and a low-confidence unit
   is visibly flagged rather than silently rendered as fact.

Accessibility needs exceed the kit's WCAG 2.1 AA floor in one place: the
Reader's split view requires ARIA wiring that lets a screen-reader user
move between a unit and its source region, since bidirectional anchor
highlighting is a pointer-first interaction by default. Keyboard anchor
traversal is a P0 obligation of `DOC-P0-021`, not a later polish item.
