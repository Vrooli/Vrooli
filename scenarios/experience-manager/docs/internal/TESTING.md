...
import (
    ...
    "experience-manager/internal/testutil/assertx"
...
The proto schema in `packages/proto/schemas/experience-manager/v1/shared/health.proto`
...
dynamically from `go.mod`, so it works after `experience-manager`
substitution.
...
import (
    ...
    clitest "experience-manager/cli/internal/testutil"
)
...
under `packages/proto/schemas/experience-manager/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/experience-manager/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.experience_manager.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/experience-manager/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/experience-manager/v1/<domain>/`,
   `packages/proto/gen/typescript/js/experience-manager/v1/<domain>/`, and
   `packages/proto/gen/python/experience_manager/v1/<domain>/`.
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/experience-manager/v1/<domain>"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/experience-manager/v1/<domain>/<domain>_pb";
