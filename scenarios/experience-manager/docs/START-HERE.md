...
`template-manager orient experience-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient experience-manager
...
```bash
cat > /tmp/prd_context_experience-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start experience-manager --interactive
# …answer the interview, then:
business-health wizard preview experience-manager
business-health wizard apply experience-manager
```
...
```bash
vrooli scenario requirements validate experience-manager --json
```
...
```bash
cat > /tmp/requirements_context_experience-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate experience-manager --json
business-health fix preview experience-manager
business-health fix apply experience-manager
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/experience-manager/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest experience-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> experience-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario experience-manager
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> experience-manager <adopted-path>
```
