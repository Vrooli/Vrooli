# Troubleshooting — Experience Manager
...
```bash
vrooli scenario status experience-manager
make stop
vrooli scenario start experience-manager --clean-stale
```
...
```bash
make status
vrooli scenario port experience-manager UI_PORT
```
...
experience-manager status --auto-start

# Or override the API base for this invocation
experience-manager status --api-base "http://localhost:$(vrooli scenario port experience-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/experience-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/experience-manager.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/experience-manager.db"
```
...
`packages/proto/gen/go/experience-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/experience-manager/v1/`, and Python under
`packages/proto/gen/python/experience_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/experience-manager/v1/<domain>/`.
