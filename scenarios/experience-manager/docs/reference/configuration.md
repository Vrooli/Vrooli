# Configuration — Experience Manager
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/experience-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `experience-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/experience-manager/config.json`
3. `~/.vrooli/config/experience-manager/config.json`
4. `~/.config/vrooli/experience-manager/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port experience-manager API_PORT`)
