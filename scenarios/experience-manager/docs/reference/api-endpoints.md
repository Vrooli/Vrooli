# API Endpoints — Experience Manager
...
envelope (`packages/proto/schemas/experience-manager/v1/shared/errors.proto`):
...
| **CLI** | `experience-manager status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/experience-manager/v1/shared/health.proto`
...
`POST /vrooli.experience_manager.v1.<domain>.<Domain>Service/<Method>`
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/experience-manager/v1/<domain>/`, then run
   `make generate`.
