...
experience-manager <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port experience-manager API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.experience_manager.v1.<domain>.<Service>/<Method>
```
...
CLI (`experience-manager --help`) — see
