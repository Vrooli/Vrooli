...
function nodeSearchText(value: ReactNode) {
  ...
  const filteredRows = useMemo(() => {
    ...
    const searched = rows.filter((row) => {
      ...
      return columns.some((column) => {
        const value = column.searchValue ? column.searchValue(row) : nodeSearchText(column.accessor(row));
  ...
  return (
    <div className={joinClasses("min-w-0 rounded-panel border border-app-border bg-app-surface", className)}>
      <div className="flex flex-col gap-3 border-b border-app-border p-3 md:flex-row md:items-center md:justify-between">
        ...
        {filters.length > 0 && (
          <div className="flex flex-wrap gap-2" role="group" aria-label={filterGroupLabel ?? searchLabel}>
            {filters.map((filter) => (
              <button
                key={filter.id}
                type="button"
              ...
              filteredRows.map((row, index) => (
                <tr key={getRowKey(row, index)} className="border-t border-app-border">
                  {columns.map((column) => (
                    <td
                      key={column.id}
                      className={joinClasses("break-words px-3 py-3 align-middle text-app-foreground", column.className)}
                    >
                      {column.accessor(row)}
                    </td>
                  ))}
