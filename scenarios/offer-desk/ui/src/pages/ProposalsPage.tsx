...
export function ProposalsPage() {
  ...
  const proposalColumns = [
    { id: "proposer", header: t(strings.pages.proposals.proposer), accessor: (proposal: typeof proposalRows[number]) => <span data-testid={selectors.pages.proposalProposer}><span data-testid={`proposal-row-${proposal.id}`}>{proposal.actor} {proposal.declineHistory.map((decline) => decline.reason).join(" ")}</span></span>, searchValue: (proposal: typeof proposalRows[number]) => proposal.actor, className: "break-all" },
    { id: "requested-status", header: t(strings.pages.proposals.requestedStatus), accessor: (proposal: typeof proposalRows[number]) => statusLabel(proposal.requestedStatus), searchValue: (proposal: typeof proposalRows[number]) => statusLabel(proposal.requestedStatus), className: "break-all" },
    { id: "reason", header: t(strings.pages.proposals.reason), accessor: (proposal: typeof proposalRows[number]) => proposal.reason || "—", searchValue: (proposal: typeof proposalRows[number]) => proposal.reason || "", className: "break-all" },
    { id: "evidence", header: t(strings.pages.proposals.evidence), accessor: (proposal: typeof proposalRows[number]) => <span data-testid={selectors.pages.proposalEvidence}>{proposal.evidenceReference || "—"}</span>, searchValue: (proposal: typeof proposalRows[number]) => proposal.evidenceReference || "", className: "break-all" },
    { id: "age", header: t(strings.pages.proposals.age), accessor: (proposal: typeof proposalRows[number]) => timestampLabel(proposal.createdAt), searchValue: (proposal: typeof proposalRows[number]) => timestampLabel(proposal.createdAt), className: "break-all" },
    { id: "declines", header: t(strings.pages.proposals.declineHistory), accessor: (proposal: typeof proposalRows[number]) => proposal.declineHistory.length > 0 ? <ul className="space-y-1">{proposal.declineHistory.map((decline, index) => <li key={`${proposal.id}-decline-${index}`}>{decline.actor}: {decline.reason} ({timestampLabel(decline.createdAt)})</li>)}</ul> : <span>{t(strings.pages.proposals.noDeclines)}</span>, className: "break-all" },
