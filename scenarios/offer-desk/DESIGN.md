...
This scenario is an operator tool read a few times a week, not a monitored dashboard, so the design kit's generous spacing and legible type suit it better than a dense telemetry layout would. Two adaptations follow from what this scenario does: **status is never colour-only** — every node status carries a text label, because an offer's state is the single most consequential thing on any surface — and the **graph view always ships a table equivalent**, because a directed graph is unusable without sight or a pointer and the graph is not the information, only one rendering of it. The token contract is otherwise unmodified.
