...
// envPrefix scopes the shared engine's operational env under OFFER_DESK_*
...
const envPrefix = "OFFER_DESK"

// Start assembles the engine-backed read path over the catalog source from the
...
func Start(ctx context.Context, source *StoreSource, searchJSONPath string, logger *log.Logger) *LiveSearch {
	...
	if !ok {
		logger.Printf("[offer-desk/aisearch] provider %q unavailable in %s — serving lexical search", ProviderID, searchJSONPath)
	...
	cancel()
	if err != nil {
		logger.Printf("[offer-desk/aisearch] embedding policy unresolved (degraded lexical search): %v", err)
		return NewLexicalSearch(lexical)
	}
	deps = resolved

	engine := NewTunedEngine(tuning, deps, NewCatalogSource(source), textFallbackOf(lexical), cfg.ReconcileParallelism, cfg.MaxEmbedsPerTick)
	if err := engine.EnsureCollection(ctx); err != nil {
		logger.Printf("[offer-desk/aisearch] collection ensure failed (degraded lexical search): %v", err)
	} else if !cfg.SyncDisabled {
		loop := pkg.NewSyncLoopFunc(envPrefix, engine.Reconciler, cfg)
		go loop.Start(ctx)
		go func() {
			if _, _, err := loop.RunOnce(ctx); err != nil {
				logger.Printf("[offer-desk/aisearch] initial reconcile failed (continuing degraded): %v", err)
			}
		}()
	}
...
// loadProvider reads the single offer-desk provider block from search.json. A
// missing/malformed file or an absent provider is a degraded lexical boot, never
// a fatal one: search is an enhancement over the authoritative catalog.
func loadProvider(path string) (pkg.ProviderConfig, bool) {
	file, err := pkg.LoadSearchFile(path)
	if err != nil {
		log.Printf("[offer-desk/aisearch] load search.json (%s): %v", path, err)
