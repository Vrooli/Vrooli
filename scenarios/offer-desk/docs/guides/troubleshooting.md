...
offer-desk status --auto-start

# Or override the API base for this invocation
offer-desk status --api-base "http://localhost:$(vrooli scenario port offer-desk API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/offer-desk/`.
...
The default route is `${SCENARIO_DATA_DIR}/offer-desk.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/offer-desk.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/offer-desk.db}")"
```
...
`packages/proto/gen/go/offer-desk/v1/`, TypeScript under
`packages/proto/gen/typescript/js/offer-desk/v1/`, and Python under
`packages/proto/gen/python/offer_desk/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/offer-desk/v1/<domain>/`.
