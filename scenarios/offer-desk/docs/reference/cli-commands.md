...
`~/.vrooli/bin/offer-desk`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start offer-desk` if the API is unreachable |
...
```bash
offer-desk notes list
offer-desk notes list --json
```

#### `offer-desk notes create --title <title> [--body <body>]`
...
```bash
offer-desk notes create --title "First note" --body "Hello world"
```
...
```bash
offer-desk notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start offer-desk`" is good.
