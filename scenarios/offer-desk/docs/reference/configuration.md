...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/offer-desk.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `offer-desk` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/offer-desk/config.json`
3. `~/.vrooli/config/offer-desk/config.json`
4. `~/.config/vrooli/offer-desk/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port offer-desk API_PORT`)
