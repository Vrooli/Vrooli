...
envelope (`packages/proto/schemas/offer-desk/v1/shared/errors.proto`):
...
| **CLI** | `offer-desk status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/offer-desk/v1/shared/health.proto`
...
`POST /vrooli.offer_desk.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.offer_desk.v1.notes.NotesService/ListNotes`
...
| **CLI** | `offer-desk notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.offer_desk.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.offer_desk.v1.notes.NotesService/CreateNote`
...
| **CLI** | `offer-desk notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.offer_desk.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.offer_desk.v1.notes.NotesService/GetNote`
...
| **CLI** | `offer-desk notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.offer_desk.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `offer-desk notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/offer-desk/v1/notes/notes.proto`.
