...
`template-manager orient offer-desk`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient offer-desk
...
```bash
cat > /tmp/prd_context_offer-desk.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start offer-desk --interactive
# …answer the interview, then:
business-health wizard preview offer-desk
business-health wizard apply offer-desk
```
...
```bash
vrooli scenario requirements validate offer-desk --json
```
...
```bash
cat > /tmp/requirements_context_offer-desk.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate offer-desk --json
business-health fix preview offer-desk
business-health fix apply offer-desk
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate offer-desk --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate offer-desk`, the notes page spec
...
validate offer-desk --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/offer-desk/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest offer-desk <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> offer-desk
...
1. Add proto messages and a service under
   `packages/proto/schemas/offer-desk/v1/<domain>/`, then run
