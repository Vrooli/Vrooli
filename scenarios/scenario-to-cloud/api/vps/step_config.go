...
var DefaultStepConfigs = map[string]StepConfig{
	...
	// Release activation may need to compile a cold Go/TypeScript workload on
	// a small VPS before the readiness probe can succeed.
	"release.activate":           {CommandTimeout: 5 * time.Minute},
	...
	"workload.start":             {CommandTimeout: 5 * time.Minute},
