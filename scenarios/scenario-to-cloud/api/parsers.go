...
func parseSystemState(results map[string]sshCommandResult) SystemState {
	state := SystemState{}
	...
	if state.CPU.Cores > 0 && len(state.CPU.LoadAverage) > 0 {
		state.CPU.UsagePercent = (state.CPU.LoadAverage[0] / float64(state.CPU.Cores)) * 100
