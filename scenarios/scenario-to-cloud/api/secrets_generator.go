...
func (g *SecretsGenerator) GenerateSecrets(plans []BundleSecretPlan) ([]GeneratedSecret, error) {
...
func (g *SecretsGenerator) generateValue(plan BundleSecretPlan) (string, error) {
	gen := plan.Generator
	if gen == nil {
...
func (g *SecretsGenerator) generateRandom(length int, charset string) (string, error) {
	if length <= 0 {
		length = 25
	}
...
func (g *SecretsGenerator) generateUUID() (string, error) {
	b := make([]byte, 16)
