...
func (c *SecretsClient) resolveBaseURL(ctx context.Context) (string, error) {
...
func (c *SecretsClient) HealthCheck(ctx context.Context) error {
...
func (c *SecretsClient) FetchBundleSecrets(ctx context.Context, scenario, tier string, resources []string) (*SecretsManagerResponse, error) {
	if tier == "" {
	...
	var result SecretsManagerResponse
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return nil, fmt.Errorf("decode secrets response: %w", err)
	}

	// Transform secrets from secrets-manager format to BundleSecretPlan format
	result.BundleSecrets = transformSecrets(result.Secrets, tier)

	return &result, nil
}

// transformSecrets converts SecretsManagerSecret slice to BundleSecretPlan slice.
// It applies tier-specific strategy resolution and maps the schema.
func transformSecrets(secrets []SecretsManagerSecret, tier string) []BundleSecretPlan {
	if len(secrets) == 0 {
		return nil
	}

	var plans []BundleSecretPlan
	for _, s := range secrets {
		...
		if class == "user_prompt" {
			plan.Prompt = &SecretPromptMetadata{
...
// determineSecretClass maps handling_strategy and classification to a BundleSecretPlan class.
func determineSecretClass(s SecretsManagerSecret, strategy string) string {
...
// BuildManifestSecrets converts a SecretsManagerResponse to ManifestSecrets for the CloudManifest.
func BuildManifestSecrets(resp *SecretsManagerResponse) *ManifestSecrets {
