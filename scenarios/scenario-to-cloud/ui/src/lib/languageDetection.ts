...
const LANGUAGE_PATTERNS: LanguagePattern[] = [
  ...
  },
  {
    ...
    patterns: [
      /^\[\w+\]/m, // Section headers
      /^\w+\s*=\s*"[^"]*"/m, // String values
      /^\w+\s*=\s*\d+/m, // Number values
      /^\[dependencies\]/m, // Common Rust section
    ],
    weight: 8,
  },
  {
  ...
  },
  {
    language: "makefile",
    patterns: [
      /^[\w-]+:\s*$/m, // Target definitions
      /^\t[\w\$@]/m, // Tab-indented commands
      /\$\([\w]+\)/, // Variable references
      /^\.PHONY:/m, // .PHONY directive
    ],
    weight: 8,
  },
];
