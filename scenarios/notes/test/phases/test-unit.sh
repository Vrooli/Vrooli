#!/bin/bash
# Unit tests for SmartNotes - Integrated with centralized testing library
...
# Check if centralized testing helpers exist
...
# Source centralized unit test runner if available
if [ -f "${APP_ROOT}/scripts/scenarios/testing/unit/run-all.sh" ]; then
    ...
    if [ -n "$(type -t testing::phase::end_with_summary)" ]; then
