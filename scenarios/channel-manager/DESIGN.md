...
**Why this design language fits Channel Manager.** The user is an operator working
a list: today's due actions across several identities, each with a window, a
constraint, and evidence to record. That is a work surface, not a dashboard — it is
scanned and acted on repeatedly, often several times a day, and it rewards density
and short pointer travel over composition. The operational console's calm slate
base, blue command colour, and compact panels are the right floor for it.

Three of this scenario's properties bind particularly hard to the design contract:

- **Status must never be colour alone.** An identity is warming, active, paused,
  quarantined, or retired, and a flag may be open against any of them. Several of
  those states are consequential and irreversible in practice, so every one carries
  a label as well as a colour.
- **Inference must read as inference.** A decay flag states its measurement and the
  baseline it was compared against; it never renders as a verdict. Warming programs
  display their provenance and confidence adjacent to their numeric ranges, so a
  speculative default never looks like platform documentation.
- **Disabled controls must explain themselves.** An action that cannot execute
  because a phase forbids it, a cadence ceiling is reached, or an identity is
  paused states that reason inline. A dead control with no explanation is a design
  failure here, not a minor omission, because the operator's next move depends on
  which of those it is.

Mobile matters more than for most scenarios: warming actions happen on a phone, in
sessions, at times the program picked. The due-actions surface must be fully usable
on mobile with the primary action near the bottom edge inside the safe area — not a
read-only view of work that has to be finished at a desk.

The token contract below is inherited from `vrooli-default` unchanged.
