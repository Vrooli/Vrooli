...
- Launch sequencing: identities, platform descriptors, and credential isolation first; then the unified queue with cadence enforcement and the manual executor, which is the point at which the scenario is usable end to end without any automation; then warming programs, sessions, gates, and graduation; then signals and baselines, which need history before they mean anything; then the eligibility query and release handoff that `content-desk` is already written against; then the operator console. The browser executor is deliberately last of the P1 set — it is the only piece carrying terms-of-service exposure, and everything above it must work manually first.
