...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("channel-manager")
	if err != nil {
		return "", fmt.Errorf("resolve channel-manager storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve channel-manager db path: %w", err)
	}
	return sqliteFileDSN(path)
}
...
func scenarioStorageRoots() (storage.Paths, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("channel-manager")
	if err != nil {
		return storage.Paths{}, fmt.Errorf("resolve channel-manager storage namespace: %w", err)
	}
...
func main() {
	...
	if preflight.Run(preflight.Config{ScenarioName: "channel-manager"}) {
		return
	}
