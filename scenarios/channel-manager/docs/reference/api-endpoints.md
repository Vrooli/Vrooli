...
envelope (`packages/proto/schemas/channel-manager/v1/shared/errors.proto`):
...
| **CLI** | `channel-manager status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/channel-manager/v1/shared/health.proto`
...
`POST /vrooli.channel_manager.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.channel_manager.v1.notes.NotesService/ListNotes`
...
| **CLI** | `channel-manager notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.channel_manager.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.channel_manager.v1.notes.NotesService/CreateNote`
...
| **CLI** | `channel-manager notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.channel_manager.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.channel_manager.v1.notes.NotesService/GetNote`
...
| **CLI** | `channel-manager notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.channel_manager.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `channel-manager notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/channel-manager/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/channel-manager/v1/<domain>/`, then run
   `make generate`.
