...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/channel-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `channel-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/channel-manager/config.json`
3. `~/.vrooli/config/channel-manager/config.json`
4. `~/.config/vrooli/channel-manager/config.json`
...
```bash
channel-manager configure api_base http://localhost:15001/api/v1
channel-manager configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port channel-manager API_PORT`)
