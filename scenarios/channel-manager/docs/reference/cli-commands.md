...
`~/.vrooli/bin/channel-manager`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start channel-manager` if the API is unreachable |
...
```bash
channel-manager status
channel-manager status --json
```

### `channel-manager configure <key> <value>`
...
```bash
channel-manager configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`channel-manager <domain> <verb>`). Every command calls a single API
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start channel-manager`" is good.
