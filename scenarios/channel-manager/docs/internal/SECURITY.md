...
| Browser executor exfiltrates session state | A shared browser profile would let one identity's cookies reach another, linking the accounts. | Per-identity BAS session profiles are mandatory, not optional. Two identities never share a profile. Each dispatch also requires an operator-recorded opaque BAS workflow UUID; Channel Manager stores neither session contents nor credentials. | designed (`CHANMGR-P1-001`) |
