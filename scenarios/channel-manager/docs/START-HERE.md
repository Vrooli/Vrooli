...
`template-manager orient channel-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient channel-manager
...
```bash
cat > /tmp/prd_context_channel-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start channel-manager --interactive
# …answer the interview, then:
business-health wizard preview channel-manager
business-health wizard apply channel-manager
```
...
```bash
vrooli scenario requirements validate channel-manager --json
```
...
```bash
cat > /tmp/requirements_context_channel-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate channel-manager --json
business-health fix preview channel-manager
business-health fix apply channel-manager
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate channel-manager --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate channel-manager`, the notes page spec
...
validate channel-manager --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/channel-manager/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest channel-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> channel-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario channel-manager
```
...
- Wire contracts belong in
  `packages/proto/schemas/channel-manager/v1/<domain>/`.
