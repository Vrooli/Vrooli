...
| Contracts (`packages/proto/schemas/channel-manager/`) | Wire shape | Proto messages/services and generated clients | Hand-written route/type mirrors |
...
## Action Layering — the scenario-specific shape

On top of the standard scenario shape, this scenario has one structure that
governs everything: **every platform action for an identity flows through one
queue.**

```
  DESCRIPTORS ── versioned JSON, validated, reviewable.
     │           platforms/*.json declare what a platform allows.
     │           warming-programs/*.json declare how an identity is warmed.
     │           Authored by hand or by an agent; never written by runtime state.
     ▼  (plan generation rolls within declared ranges, and records the roll)
  QUEUE     ── one per identity. Warming steps, maintenance engagement, and
     │         content releases share it, because cadence is counted per
     │         ACCOUNT and not per workflow.
     │         THE ONLY PATH TO A PLATFORM.
     ▼  (executor chosen per platform × action)
  EXECUTION ── manual · browser · api. Same action record from all three.
     │
     ▼
  SIGNALS   ── observations, baselines, flags. Feeds gates and graduation.
```

Five invariants follow, and they are the ones to defend in review:

1. **One queue per identity, across every action kind.** Separate queues for
   warming and publishing would let an identity exceed its daily budget by doing
   both. Platforms count per account.
2. **The action record is identical regardless of executor.** That is what makes
...
4. **Eligibility never fails open.** Unknown is a valid answer; eligible-by-default
   ...
   would post from an unwarmed account.
5. **Signals pause; they never compensate.** A flag stops the queue and reports its
   evidence. Nothing automatic posts more, posts less, or changes strategy, because
   every plausible automatic response either worsens the situation or destroys the
   evidence needed to understand it.

### Why descriptors rather than code

Platform behaviour and warming practice both change faster than a release cycle,
and both are things an agent should be able to revise from evidence. Holding them
as validated JSON follows the idiom the fleet already uses twice — `.vrooli/search.json`
for search providers and the declarative harness adapters in `vrooli-memory`. Adding
a platform or a warming variant is a descriptor and a validation run, not a build.

The corollary matters as much: because a descriptor is data, it can carry its own
**provenance**. Every warming program states its source kind, confidence, capture
date, and revisit trigger, so a future agent editing a threshold can see whether it
is overriding measured data or a guess. The programs shipped at P0 are guesses and
say so.

### Where time lives

Nearly every rule here is temporal — session gaps, active windows, gate wait
intervals, baseline windows, cadence day boundaries. All of it goes through the
`api/internal/clock/` seam. A production path that reads wall-clock time directly is
a defect, and a test that cannot control time is not testing this scenario's actual
behaviour.
...
- scenario proto schemas relocated to
  `packages/proto/schemas/channel-manager/`.
...
For a new platform or warming program, do **not** follow the steps above. Add a
validated descriptor under `data/` and run the validation command. If a descriptor
cannot express what you need, that is a schema change with a decision behind it —
not a reason to write per-platform Go.
...
|---|---|---|---|
| 2026-07-28 | Platform and warming behaviour live in JSON descriptors rather than Go, unlike most policy in the fleet. | Both change faster than a release cycle and both should be revisable by an agent from evidence. Descriptors also carry provenance, which Go constants cannot. | If descriptor validation becomes more complex than the code it replaced. |
| 2026-07-28 | Manual execution is a first-class permanent executor, not a development fallback. | It is the only executor with no terms-of-service exposure, and it works on every platform for every action on day one. | Never — the exposure does not go away. |
| 2026-07-28 | The scenario records environment attestations it cannot itself verify. | Device fingerprint, proxy, and region are provisioned upstream. An unverifiable attestation that blocks program start is still strictly better than no gate. | If an environment provider exposes a programmatic check. |
| 2026-07-28 | No account state, warming detail, or credential is exposed to any caller. | `content-desk`'s contract deliberately restricts the boundary to two questions. A wider surface would let editorial logic depend on account internals. | Never. |
