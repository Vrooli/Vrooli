# Troubleshooting — Channel Manager
...
```bash
vrooli scenario status channel-manager
make stop
vrooli scenario start channel-manager --clean-stale
```
...
channel-manager status --auto-start

# Or override the API base for this invocation
channel-manager status --api-base "http://localhost:$(vrooli scenario port channel-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/channel-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/channel-manager.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/channel-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/channel-manager.db}")"
```
...
`packages/proto/gen/go/channel-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/js/channel-manager/v1/`, and Python under
`packages/proto/gen/python/channel_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/channel-manager/v1/<domain>/`.
