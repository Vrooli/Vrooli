...
// Configuration constants for request validation and processing
const (
	MaxIdeasLimit          = 100  // Maximum number of ideas that can be requested at once
	MaxRefinementLength    = 2000 // Maximum length of refinement text in characters
	MaxTextPreviewLength   = 500  // Maximum length of text preview before truncation
	MaxOllamaInputLength   = 8000 // Maximum text length sent to Ollama to prevent timeouts
	DefaultIdeasQueryLimit = 50   // Default number of ideas returned in queries
)
...
func (ip *IdeaProcessor) GenerateIdeas(ctx context.Context, req GenerateIdeasRequest) GenerateIdeasResponse {
	...
	if req.CampaignID == "" {
		...
		response.Error = "Campaign ID is required"
		return response
	}

	if req.Count < 1 || req.Count > 10 {
		response.Success = false
		response.Error = "Count must be between 1 and 10"
		return response
	}
	...
	if err != nil {
		...
		if err == sql.ErrNoRows {
			response.Error = fmt.Sprintf("Campaign not found: %s", req.CampaignID)
		} else {
	...
	for _, idea := range generatedIdeas {
		...
		if err != nil {
			log.Printf("Failed to store idea: %v\n", err)
			continue
		}
...
func (ip *IdeaProcessor) SemanticSearch(ctx context.Context, req SemanticSearchRequest) ([]SemanticSearchResult, error) {
	...
	if req.Limit > MaxIdeasLimit {
		req.Limit = MaxIdeasLimit
	}
...
func (ip *IdeaProcessor) RefineIdea(ctx context.Context, req RefinementRequest) error {
	...
	if req.Refinement == "" {
		return fmt.Errorf("refinement request cannot be empty")
	}

	if len(req.Refinement) > MaxRefinementLength {
		return fmt.Errorf("refinement request too long (max %d characters)", MaxRefinementLength)
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return fmt.Errorf("idea not found: %s", req.IdeaID)
		}
...
// getCampaignData retrieves campaign information from the database for context enrichment.
// Returns campaign metadata including name, description, and optional context field.
func (ip *IdeaProcessor) getCampaignData(ctx context.Context, campaignID string) (map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve campaign %s: %w", campaignID, err)
	}
...
// getCampaignDocuments retrieves processed documents associated with a campaign.
// Returns up to 5 most recent documents with extracted text for context enrichment.
func (ip *IdeaProcessor) getCampaignDocuments(ctx context.Context, campaignID string) ([]map[string]interface{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve documents for campaign %s: %w", campaignID, err)
	}
	defer rows.Close()
...
// getRecentIdeas retrieves recently refined ideas from a campaign.
// Returns up to 3 most recent refined/finalized ideas to avoid duplication.
func (ip *IdeaProcessor) getRecentIdeas(ctx context.Context, campaignID string) ([]map[string]interface{}, error) {
	...
	rows, err := ip.db.QueryContext(ctx, query, campaignID)
	if err != nil {
		return nil, fmt.Errorf("failed to retrieve recent ideas for campaign %s: %w", campaignID, err)
	}
	defer rows.Close()
...
// buildEnrichedPrompt constructs a comprehensive prompt for AI idea generation.
// Combines campaign context, document extracts, and existing ideas to produce contextually relevant output.
	...
	if len(documents) > 0 {
		...
		for _, doc := range documents {
			...
			if text, ok := doc["extracted_text"].(string); ok && text != "" {
				preview := text
				if len(preview) > MaxTextPreviewLength {
					preview = preview[:MaxTextPreviewLength] + "..."
				}
...
// generateWithOllama generates ideas using Ollama's LLM with structured JSON output.
// Returns parsed JSON response containing generated ideas with titles, descriptions, and tags.
...
func (ip *IdeaProcessor) generateEmbedding(ctx context.Context, text string) ([]float64, error) {
	...
	if len(text) > MaxOllamaInputLength {
...
// storeIdea persists a generated idea to PostgreSQL database.
// Creates new idea record with UUID, timestamps, and campaign association.
...
// storeInVectorDB creates and stores vector embeddings in Qdrant for semantic search.
// Generates embedding from idea title+content and indexes in campaign-specific collection.
func (ip *IdeaProcessor) storeInVectorDB(ctx context.Context, ideaID, campaignID string, idea GeneratedIdea) {
	...
	if err != nil {
		log.Printf("Failed to generate embedding for idea %s: %v\n", ideaID, err)
		return
	}
	...
	if err != nil {
		log.Printf("Failed to store idea in Qdrant: %v\n", err)
		return
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode >= 400 {
		body, _ := io.ReadAll(resp.Body)
		log.Printf("Qdrant returned error status %d: %s\n", resp.StatusCode, string(body))
		return
	}

	log.Printf("✅ Successfully stored idea %s in Qdrant vector database\n", ideaID)
}

// updateVectorDB updates vector embeddings for refined ideas in Qdrant.
// Regenerates embedding from updated content and updates existing vector entry.
