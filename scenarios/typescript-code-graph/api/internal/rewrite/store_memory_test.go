...
func TestMemoryPlanStore_SaveGet(t *testing.T) {
	...
	plan := Plan{ID: "abc", ScenarioPath: "/abs/proj", Operations: []Operation{
	...
	if got.ID != "abc" || got.ScenarioPath != "/abs/proj" || len(got.Operations) != 1 {
...
func TestMemoryPlanStore_ScenarioScope(t *testing.T) {
	s := NewMemoryPlanStore()
	plan := Plan{ID: "abc", ScenarioPath: "/abs/one", Operations: nil}
	if err := s.Save(plan); err != nil {
		t.Fatalf("Save: %v", err)
	}
	// Same PlanID under a different scenario must not collide.
	_, err := s.Get("/abs/two", "abc")
	if !errors.Is(err, ErrPlanNotFound) {
		t.Errorf("scenario scope should isolate plans; got %v", err)
	}
}
