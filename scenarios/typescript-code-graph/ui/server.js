...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start typescript-code-graph` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start typescript-code-graph` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.typescript_code_graph\.v1\./

startScenarioServer({
  uiPort,
  apiPort,
  distDir: './dist',
  serviceName: 'typescript-code-graph',
  corsOrigins: '*',
  setupRoutes(app) {
