...
import { SidecarStatusPanel } from "../features/health/SidecarStatusPanel";
...
import { isWorkspaceUnsupported } from "../features/explorer/lib/extractError";
import { WorkspaceUnsupportedNotice } from "../features/explorer/WorkspaceUnsupportedNotice";
...
/**
 * Single-page operational workbench for typescript-code-graph. The extract bar
 * + stats header persist; Graph / Warnings / Rewrite / Fixtures live as tabs.
 * The Node ts-morph sidecar's lifecycle is surfaced via the SidecarStatusPanel.
 */
export function WorkbenchPage() {
  ...
  // pnpm-workspace projects are explicitly out of scope for v1 (OT-P2-005);
  // the API returns CodeUnimplemented "workspace_unsupported". Present that as
  // information, not a failure.
  const workspaceUnsupported = extract.isError && isWorkspaceUnsupported(extract.error);
      ...
      {submittedPath === null ? (
        ...
        <>
          ...
          >
            ...
            <Stat testId={selectors.workbench.stats.modules} label={t(strings.workbench.stats.modules)} value={summary.packages} />
