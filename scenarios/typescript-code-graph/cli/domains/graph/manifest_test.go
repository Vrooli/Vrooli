...
// TestManifestCoversTypeScriptCodeGraphService asserts that every RPC declared
// on TypeScriptCodeGraphService either has a manifest command binding or is
// documented in the manifest's `omitted` array with a reason. Catches drift
// between proto and CLI: adding a new RPC without binding/omitting it fails
// here.
//
