...
// the template itself does not build because of typescript-code-graph placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_typescript_code_graph_v1_notes_notes_proto, "NotesService")
}
