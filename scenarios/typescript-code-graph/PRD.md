...
Purpose: TypeScript Code Graph extracts a deterministic, reproducible graph of files, modules, top-level declarations, and import edges from any TypeScript project and executes mechanical TypeScript refactors (file moves and import rewrites) on behalf of consumer scenarios. It wraps `ts-morph` so that no other Vrooli scenario has to parse TypeScript source itself. Each declaration node also carries its verbatim leading JSDoc/comment block as structured metadata, so consumers that previously relied on regex tag scraping (most notably `react-component-library`) can migrate onto a typed contract without losing fidelity.
...
- [ ] OT-P0-001 | Deterministic Graph Extraction | Implement `Extract(ExtractRequest{scenario_path}) → Graph` against `ts-morph` using a fixed Project configuration that returns files, modules, top-level declarations (components, hooks, classes, interfaces, types, functions, vars, consts), and import edges plus intra-module symbol references; byte-stable serialization for identical inputs.
...
- [ ] OT-P0-007 | Per-Path Serialization, Parallel Across Paths | Two concurrent `Extract` calls for the same `scenario_path` serialize through an in-process queue (additionally enforced at the Node sidecar layer because `ts-morph` Project state is not safe to share across parallel invocations). Calls for different paths run in parallel.
- [ ] OT-P0-008 | Connect-RPC + CLI + UI Parity | Every capability is exposed through proto + Connect-RPC. The CLI (`typescript-code-graph extract`, `typescript-code-graph rewrite plan`, `typescript-code-graph rewrite apply`) is a translation layer over Connect clients; the UI is the same. No REST/JSON workarounds.
...
- [ ] OT-P0-010 | Golden-Fixture Determinism Gate | Ship at minimum two fixture TS projects under `bas/fixtures/` (`ts-junk-drawer/` and `ts-jsdoc-tags/`) each with hand-curated `expected-graph.json`. CI fails if any fixture's extracted graph diverges byte-for-byte from the expected file. The `ts-jsdoc-tags/` fixture pins the leading-comment contract that `react-component-library` depends on.
- [ ] OT-P0-011 | Graph Explorer + Diagnostics UI | Ship a UI surface that lets an operator paste a scenario path, see the extracted nodes/edges/leading comments/warnings, view server health (including Node sidecar status), and inspect recent extraction calls.
...
- [ ] OT-P2-004 | ParseDiagnostics RPC | `ParseDiagnostics(scenario_path) → []Diagnostic` — structured TypeScript compiler errors and warnings as first-class data.
...
Concurrency model: per-path serialization, parallel across paths. An in-process Go-side mutex keyed by absolute `scenario_path` serializes `Extract` calls for the same path. The Node sidecar also serializes per-Project internally (Project state is not safe across parallel invocations). Apply operations on the same path serialize through the same Go lock.
...
Non-goals: No source-code parsing outside `ts-morph`. No git operations of any kind — never `git mv`, `git commit`, `git status`, or `git diff`. No build invocation — never `tsc`, never `pnpm`. No automatic rollback on partial Rewrite failure; the operator owns rollback via git. No internal extraction cache in v1. No call-site granularity in v1 (top-level declarations + intra-module references only). No support for parsing non-TS languages — that belongs in sibling scenarios.
...
Launch sequencing: (1) Confirm the shared `common/v1/code_graph.proto` envelope (owned jointly with `go-code-graph`) plus author `typescript-code-graph/v1/` service definition with TS node-kind extensions. (2) Stand up the Node sidecar with `ts-morph` and the IPC contract. (3) Implement `Extract` against the sidecar including leading-comment metadata; pass two fixture-validated determinism tests (`ts-junk-drawer/`, `ts-jsdoc-tags/`). (4) Implement the two-step `Rewrite` via `ts-morph` Project APIs; add fixture coverage for FileMove and ImportRewrite. (5) Wire CLI surfaces and UI graph explorer + diagnostics (including sidecar status panel). (6) Stand up per-path serialization mutex and concurrency tests. (7) Ship performance regression suite. (8) P1: Fixture Validator UI, Operation Log persistence, barrel/re-export coverage, JSX/TSX node kinds, content-hash cache hint. (9) Coordinate react-component-library cutover (OT-P1-006) once OT-P0-003 is stable.

## 🎨 UX & Branding

User experience: Two distinct audiences. The CLI is the primary surface for agents and operators — concise, scriptable, default human-friendly output with optional `--json`. Commands map one-to-one to RPCs: `typescript-code-graph extract <path>` prints the graph summary plus warnings; `typescript-code-graph rewrite plan <ops.json>` returns a plan ID and the normalized operation log; `typescript-code-graph rewrite apply <plan_id>` executes and prints the result. The UI is the secondary surface for human debugging — paste a path, see the graph, inspect leading comments and warnings, view server health (API + Node sidecar), and (P1) drop a fixture to diff expected vs. actual. Dense, operational, scan-friendly. Not a marketing surface.
