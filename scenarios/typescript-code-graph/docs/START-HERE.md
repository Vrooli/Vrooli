# Start Here — TypeScript Code Graph
...
`template-manager orient typescript-code-graph`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient typescript-code-graph
...
```bash
cat > /tmp/prd_context_typescript-code-graph.md <<'EOF'
Overview:
- Purpose:
...
```bash
cat > /tmp/requirements_context_typescript-code-graph.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
- [ ] **Start in proto.** Author `packages/proto/schemas/typescript-code-graph/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest typescript-code-graph <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> typescript-code-graph
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario typescript-code-graph
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> typescript-code-graph <adopted-path>
```
...
1. Add proto messages and a service under
   `packages/proto/schemas/typescript-code-graph/v1/<domain>/`, then run
