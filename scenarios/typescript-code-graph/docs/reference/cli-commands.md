# CLI Commands — TypeScript Code Graph
...
`~/.vrooli/bin/typescript-code-graph`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start typescript-code-graph` if the API is unreachable |
...
```bash
typescript-code-graph status
typescript-code-graph status --json
```

### `typescript-code-graph configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
typescript-code-graph configure api_base http://localhost:15001/api/v1
typescript-code-graph configure token <token>
```

Read values back without an argument:

```bash
typescript-code-graph configure api_base
```
...
### `typescript-code-graph notes list`
...
```bash
typescript-code-graph notes list
typescript-code-graph notes list --json
```

### `typescript-code-graph notes create --title <title> [--body <body>]`
...
```bash
typescript-code-graph notes create --title "First note" --body "Hello world"
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start typescript-code-graph`" is good.
