# Configuration — TypeScript Code Graph
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/typescript-code-graph.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `typescript-code-graph` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/typescript-code-graph/config.json`
3. `~/.vrooli/config/typescript-code-graph/config.json`
4. `~/.config/vrooli/typescript-code-graph/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port typescript-code-graph API_PORT`)
