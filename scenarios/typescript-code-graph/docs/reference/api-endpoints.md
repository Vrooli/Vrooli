# API Endpoints — TypeScript Code Graph
...
envelope (`packages/proto/schemas/typescript-code-graph/v1/errors/errors.proto`):
...
| **CLI** | `typescript-code-graph status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/typescript-code-graph/v1/health/health.proto`
...
### `POST /vrooli.typescript_code_graph.v1.notes.NotesService/ListNotes`
...
| **CLI** | `typescript-code-graph notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.typescript_code_graph.v1.notes.NotesService/ListNotes" \
...
### `POST /vrooli.typescript_code_graph.v1.notes.NotesService/CreateNote`
...
| **CLI** | `typescript-code-graph notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.typescript_code_graph.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
### `POST /vrooli.typescript_code_graph.v1.notes.NotesService/GetNote`
...
| **CLI** | `typescript-code-graph notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.typescript_code_graph.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `typescript-code-graph notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/typescript-code-graph/v1/notes/notes.proto`.
