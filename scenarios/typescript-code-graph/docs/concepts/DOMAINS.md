...
- Purpose: produce a deterministic, byte-stable graph snapshot of a TS project by delegating all parsing to `ts-morph` (via the Node sidecar) and normalizing the result into `common.v1.code_graph.Graph` with TS-specific node kinds and leading-comment metadata.
...
| NodeKind / EdgeKind | Extensible enums on the shared envelope. TS-specific kinds (`ts_module`, `ts_component`, `ts_hook`, `ts_class`, `ts_interface`, `ts_type`, `ts_function`, `ts_var`, `ts_const`, `ts_re_export`) live in the typescript-code-graph proto extension. | `common/v1/code_graph.proto` + `typescript-code-graph/v1/` |
...
| Per-path mutex | An in-process lock keyed by absolute `scenario_path`. Serializes extraction and apply for the same path; parallel across paths. **Additionally enforced inside the sidecar** because `ts-morph` Project state is not safe to share. | `graph` (extraction) + `rewrite` (apply) + sidecar |
