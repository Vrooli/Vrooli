...
| SQLite (embedded) | embedded storage | optional (P1 only) | `rewrite` (Operation Log persistence) | resolved by `api-core/storage` from the scenario id; modernc.org/sqlite driver. | If unreachable, Operation Log writes degrade silently; extraction and rewrite continue. v1 ships without persistence. |
...
| Target TS project path is itself a multi-project pnpm workspace root | sidecar detects `pnpm-workspace.yaml` in the selected project directory with packages beyond `.` | Return `ExtractError{kind: workspace_unsupported}` with a hint to point at a specific child project. | Unit test. |
