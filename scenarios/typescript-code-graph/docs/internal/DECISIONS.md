...
| 2026-05-23 | Single-project per `Extract` call; reject selected workspace roots. | Vrooli scenarios are single-project by convention. Multi-project workspace-root extraction adds resolution complexity that has no current consumer, but explicit project roots and `tsconfig.json` files inside the repository workspace are normal Vrooli inputs. | Workspace-root support is deferred to P2 (OT-P2-005). A selected project directory with a multi-project `pnpm-workspace.yaml` is rejected; a local `packages: [.]` boundary and ancestor workspaces are allowed. | Revisit only if a Vrooli scenario actually needs extraction of an entire workspace root as one request. |
