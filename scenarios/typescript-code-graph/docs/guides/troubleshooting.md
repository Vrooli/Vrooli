...
make stop
# Or, as last resort:
pkill -f 'typescript-code-graph-api'
...
```bash
export SQLITE_PATH=/tmp/typescript-code-graph.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/typescript-code-graph.db}")"
```
...
`packages/proto/gen/typescript/js/typescript-code-graph/v1/`, and Python under
