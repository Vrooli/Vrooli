# Troubleshooting — TypeScript Code Graph
...
make stop
vrooli scenario start typescript-code-graph --clean-stale
```
...
```bash
make status
vrooli scenario port typescript-code-graph UI_PORT
```
...
typescript-code-graph status --auto-start

# Or override the API base for this invocation
typescript-code-graph status --api-base "http://localhost:$(vrooli scenario port typescript-code-graph API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/typescript-code-graph/`.
...
The default route is `${SCENARIO_DATA_DIR}/typescript-code-graph.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/typescript-code-graph.db"
```
...
`packages/proto/gen/go/typescript-code-graph/v1/`, TypeScript under
`packages/proto/gen/typescript/typescript-code-graph/v1/`, and Python under
`packages/proto/gen/python/typescript_code_graph/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/typescript-code-graph/v1/<domain>/`.
