...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("ai-gateway")
	if err != nil {
		return "", fmt.Errorf("resolve ai-gateway storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve ai-gateway db path: %w", err)
	}
	return sqliteFileDSN(path)
}
