...
// Endpoints slice.
//
// Deliberately loose on count (not "exactly N"): scenarios add their own
...
//
// Lifted from a per-domain test so the safety net applies
...
func TestProtoConnectParity(t *testing.T) {
	...
	if len(files) == 0 {
		t.Log("no Connect-mounted product domains are registered yet")
		return
	}
