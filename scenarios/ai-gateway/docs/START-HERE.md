...
`vrooli scenario orient ai-gateway`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient ai-gateway
...
run `vrooli scenario detemplate ai-gateway`, the notes page spec
