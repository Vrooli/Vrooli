...
`template-manager orient ai-gateway`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient ai-gateway
...
```bash
cat > /tmp/prd_context_ai-gateway.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start ai-gateway --interactive
# …answer the interview, then:
business-health wizard preview ai-gateway
business-health wizard apply ai-gateway
```
...
```bash
vrooli scenario requirements validate ai-gateway --json
```
...
```bash
cat > /tmp/requirements_context_ai-gateway.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate ai-gateway --json
business-health fix preview ai-gateway
business-health fix apply ai-gateway
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate ai-gateway --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate ai-gateway`, the notes page spec
...
validate ai-gateway --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/ai-gateway/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest ai-gateway <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> ai-gateway
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario ai-gateway
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> ai-gateway <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/ai-gateway/v1/<domain>/`.
