...
```bash
API_PORT=$(vrooli scenario port ai-gateway API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.ai_gateway.v1.<domain>.<Service>/<Method>
```
...
```bash
ai-gateway notes list
ai-gateway notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port ai-gateway API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.ai_gateway.v1.notes.NotesService/ListNotes" \
