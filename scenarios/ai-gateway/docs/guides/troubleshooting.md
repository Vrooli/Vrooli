...
```bash
vrooli scenario status ai-gateway
make stop
vrooli scenario start ai-gateway --clean-stale
```
...
```bash
make status
vrooli scenario port ai-gateway UI_PORT
```
...
ai-gateway status --auto-start

# Or override the API base for this invocation
ai-gateway status --api-base "http://localhost:$(vrooli scenario port ai-gateway API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/ai-gateway/`.
...
The default route is `${SCENARIO_DATA_DIR}/ai-gateway.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/ai-gateway.db"
```
...
`packages/proto/gen/go/ai-gateway/v1/`, TypeScript under
`packages/proto/gen/typescript/js/ai-gateway/v1/`, and Python under
`packages/proto/gen/python/ai_gateway/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/ai-gateway/v1/<domain>/`.
