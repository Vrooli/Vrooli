...
#### `POST /vrooli.ai_gateway.v1.notes.NotesService/ListNotes`
...
| **CLI** | `ai-gateway notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.ai_gateway.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.ai_gateway.v1.notes.NotesService/CreateNote`
...
| **CLI** | `ai-gateway notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.ai_gateway.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.ai_gateway.v1.notes.NotesService/GetNote`
...
| **CLI** | `ai-gateway notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.ai_gateway.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `ai-gateway notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/ai-gateway/v1/notes/notes.proto`.
