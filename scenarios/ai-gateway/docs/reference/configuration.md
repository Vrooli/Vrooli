...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/ai-gateway.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `ai-gateway` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/ai-gateway/config.json`
3. `~/.vrooli/config/ai-gateway/config.json`
4. `~/.config/vrooli/ai-gateway/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port ai-gateway API_PORT`)
