#!/bin/bash
# Dependency validation for smart-file-photo-manager
...
run_cli_check() {
  if [ -f "cli/install.sh" ] && [ -x "cli/install.sh" ]; then
    testing::phase::check "CLI install script parses" bash -c 'cd cli && bash -n install.sh'
  else
    testing::phase::add_warning "CLI install.sh missing or not executable"
    testing::phase::add_test skipped
  fi
}
