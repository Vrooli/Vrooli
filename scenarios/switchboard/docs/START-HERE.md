...
`template-manager orient switchboard`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient switchboard
...
```bash
cat > /tmp/prd_context_switchboard.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start switchboard --interactive
# …answer the interview, then:
business-health wizard preview switchboard
business-health wizard apply switchboard
```
...
```bash
vrooli scenario requirements validate switchboard --json
```
...
```bash
cat > /tmp/requirements_context_switchboard.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate switchboard --json
business-health fix preview switchboard
business-health fix apply switchboard
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate switchboard --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate switchboard`, the notes page spec
...
validate switchboard --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/switchboard/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest switchboard <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> switchboard
