...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/switchboard/v1/shared/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/switchboard/v1/shared"`)
...
import (
    ...
    "switchboard/internal/clock"
    "switchboard/internal/module"
    "switchboard/internal/server"
    "switchboard/internal/testutil/assertx"
    "switchboard/internal/testutil/httpx"
...
The proto schema in `packages/proto/schemas/switchboard/v1/shared/health.proto`
...
|---|---|---|
| Wire contract | `packages/proto/schemas/switchboard/v1/notes/notes.proto` | `Note`, `service NotesService`, `ListNotesResponse`, `CreateNoteRequest`, `CreateNoteResponse`, `GetNoteRequest`, `GetNoteResponse` |
| REST metadata contract | `packages/proto/schemas/switchboard/v1/notes/attachments.proto` | `Attachment` and `UploadAttachmentResponse` for the multipart upload exception |
...
| REST error envelope | `packages/proto/schemas/switchboard/v1/shared/errors.proto` + `internal/httpx/errors.go::WriteError` | Typed body for REST exceptions, with canonical codes (`invalid_request`, `not_found`, `internal`) |
...
dynamically from `go.mod`, so it works after `switchboard`
substitution.
...
import (
    ...
    clitest "switchboard/cli/internal/testutil"
)
