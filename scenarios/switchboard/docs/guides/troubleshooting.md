...
```bash
vrooli scenario status switchboard
make stop
vrooli scenario start switchboard --clean-stale
```
...
```bash
make status
vrooli scenario port switchboard UI_PORT
```
...
switchboard status --auto-start

# Or override the API base for this invocation
switchboard status --api-base "http://localhost:$(vrooli scenario port switchboard API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/switchboard/`.
...
The default route is `${SCENARIO_DATA_DIR}/switchboard.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/switchboard.db"
```
...
`packages/proto/gen/go/switchboard/v1/`, TypeScript under
`packages/proto/gen/typescript/switchboard/v1/`, and Python under
`packages/proto/gen/python/switchboard/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/switchboard/v1/<domain>/`.
