...
```bash
switchboard status
switchboard <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port switchboard API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.switchboard.v1.<domain>.<Service>/<Method>
```
...
```bash
switchboard notes list
switchboard notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port switchboard API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.switchboard.v1.notes.NotesService/ListNotes" \
