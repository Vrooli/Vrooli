...
`~/.vrooli/bin/switchboard`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start switchboard` if the API is unreachable |
...
### `switchboard configure <key> <value>`
...
```bash
switchboard configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`switchboard <domain> <verb>`). Every command calls a single API
...
```bash
switchboard notes list
switchboard notes list --json
```

#### `switchboard notes create --title <title> [--body <body>]`
...
```bash
switchboard notes create --title "First note" --body "Hello world"
```
...
```bash
switchboard notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start switchboard`" is good.

## Planned surface (not yet implemented)

> **Status.** No domain code exists. Everything below is the intended CLI,
> authored documentation-first. The `notes` reference above is the template's
> example domain, real today and removed at `detemplate`. Every command has full
> headless parity with the API; the CLI is a thin typed wrapper over Connect.

```bash
# channels
switchboard channels list [--json]                 # catalogue, ordered by declared setup friction
switchboard channels status [--json]               # per-channel disposition on THIS machine
switchboard channels show <channel-id> [--json]
switchboard channels reload [--json]               # re-read and re-validate data/channels/

# agents
switchboard agents list [--json]
switchboard agents bind --agent <id> --channel <id> --address <addr>
switchboard agents unbind --agent <id> --channel <id> --address <addr>

# conversations
switchboard threads list [--channel <id>] [--agent <id>] [--json]
switchboard threads show <thread-id> [--json]
switchboard threads send <thread-id> --text <body>

# trust
switchboard contacts list [--json]
switchboard contacts tier set --contact <id> --tier owner|trusted|known|stranger
switchboard contacts resolve --contact <id> --agent <id> [--thread <id>] [--json]
switchboard contacts log [--since <ts>] [--json]   # the permission audit record

# turns
switchboard turns show <turn-id> [--json]
switchboard approvals list [--json]
switchboard approvals decide <approval-id> --grant | --deny
switchboard budget show <thread-id> [--json]
switchboard budget cap set <thread-id> --units <n>
```

### Commands deliberately absent

- **No `switchboard send --to <address>` that creates a thread from nothing.**
  Outbound-first contact belongs to `notification-hub`, which owns quiet hours,
  device registry, and retry. Adding it here would create a second owner for
  reaching a human.
- **No command that prints a credential**, in any format, including `--json`.
- **No command that promotes a contact to `owner`.** Owner is established by
  identity and is never reachable by promotion.
- **No `--force` on a tier change.** If a tier assignment needs forcing, the
  resolution is wrong and forcing it hides the defect.

### Cross-node behavior

Every command above runs against the machine it is invoked on. To ask a
different machine — a Mac hosting the iMessage adapter — dispatch through
`vrooli-bridge` rather than a `--node` flag: the flag is advertised widely in
scenario CLIs and read by nothing, and `--node no-such-node` answers about the
local host with exit 0.
