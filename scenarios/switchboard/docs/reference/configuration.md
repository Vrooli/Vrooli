...
For `switchboard` the prefix is the scenario id upper-cased with
...
## Channel descriptors (`data/channels/*.json`)

The descriptor is the extensibility mechanism, not a settings file. Every
capability, limit, and policy question about a channel is answered by reading
it, and no code above the adapter layer branches on a channel identifier
(OT-P0-002). The file is the source of truth; the database table is a cache
rebuilt at boot. An invalid descriptor fails startup naming the file and the
field (OT-P0-001).

Adding a channel is this file plus one adapter. Nothing else changes — not the
core, not the console, not the conformance suite.

| Field | Type | Meaning |
|---|---|---|
| `kind`, `schemaVersion`, `id`, `displayName` | string / int | Identity. `id` binds the descriptor to its adapter and is never branched on |
| `transport.ingress` | enum | `webhook`, `poll`, `socket`, `host-local`, `in-app` — how messages arrive |
| `transport.egress` | enum | How replies leave. Replies always exit through the adapter that ingressed them (OT-P0-013) |
| `transport.auth` | enum | `bot-token`, `oauth-app`, `account-credential`, `none` |
| `supports.*` | bool | `text`, `images`, `files`, `voice_note`, `threads`, `reactions`, `typing`, `edit`, `group`. Asked directly instead of testing the channel id |
| `limits.text_bytes`, `limits.file_mb` | int | Media exceeding these is rejected before egress, not after a transport error |
| `limits.rate_per_min` | int | Drives backoff, exercised by the conformance suite |
| `group.speak_when` | enum | `always` or `mentioned`. Defaults to `mentioned` wherever `supports.group` is true (OT-P1-003) |
| `group.turn_budget_per_hour` | int | Fails closed when exhausted (OT-P1-005) |
| `requires[]` | string[] | Host preconditions — `public_origin`, `mac_fleet_node`, `provisioned_number`. Evaluated against facts `vrooli-bridge` and `tunnel-manager` already hold, so an unavailable channel renders as a prompt rather than a dead option (OT-P1-012) |
| `setup.friction` | int 1–4 | Orders the attach catalogue with no maintained list (OT-P1-013) |
| `setup.steps[]` | string[] | Rendered verbatim during attachment |
| `cost.model` | enum | `free`, `metered`, `gated`. Informational for the catalogue; enforcement is LPBS's, never this file's |

### Proposed, not yet specified

`identity_strength` — how strongly the transport authenticates a sender. SMS
sender identity is spoofable at the network level while a workspace OAuth
identity is not, and the console currently cannot state the difference. Adding
it would let a tier assignment show the strength of the claim it rests on. See
`SWBD-PROB-003`; it is deliberately not in the P0 schema.

### Worked example

```json
{
  "kind": "channel", "schemaVersion": 1, "id": "telegram",
  "displayName": "Telegram",
  "transport": { "ingress": "poll", "egress": "bot-api", "auth": "bot-token" },
  "supports": { "text": true, "images": true, "files": true,
                "voice_note": true, "threads": false, "group": true },
  "limits":   { "text_bytes": 4096, "file_mb": 50, "rate_per_min": 30 },
  "group":    { "speak_when": "mentioned", "turn_budget_per_hour": 40 },
  "requires": [],
  "setup":    { "friction": 1, "steps": ["talk to @BotFather", "paste the bot token"] },
  "cost":     { "model": "free" }
}
```

`requires` is empty and `setup.friction` is `1`, which is why Telegram sorts
first in the attach catalogue and why it is the first adapter built.
...
2. `${XDG_CONFIG_HOME}/vrooli/switchboard/config.json`
3. `~/.vrooli/config/switchboard/config.json`
4. `~/.config/vrooli/switchboard/config.json`
...
```bash
switchboard configure api_base http://localhost:15001/api/v1
switchboard configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port switchboard API_PORT`)
