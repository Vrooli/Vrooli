...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/api-health/v1/shared/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/api-health/v1/shared"`)
...
import (
    ...
    "api-health/internal/clock"
    "api-health/internal/module"
    "api-health/internal/server"
    "api-health/internal/testutil/assertx"
    "api-health/internal/testutil/httpx"
...
The proto schema in `packages/proto/schemas/api-health/v1/shared/health.proto`
...
dynamically from `go.mod`, so it works after `api-health`
substitution.
...
import (
    ...
    clitest "api-health/cli/internal/testutil"
)
...
under `packages/proto/schemas/api-health/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/api-health/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.api_health.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/api-health/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/api-health/v1/<domain>/`,
   `packages/proto/gen/typescript/js/api-health/v1/<domain>/`, and
   `packages/proto/gen/python/api_health/v1/<domain>/`.
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/api-health/v1/<domain>"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/api-health/v1/<domain>/<domain>_pb";
