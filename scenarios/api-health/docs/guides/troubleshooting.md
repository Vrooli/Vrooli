...
```bash
export SQLITE_PATH=/tmp/api-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/api-health.db}")"
```
...
`packages/proto/gen/typescript/js/api-health/v1/`, and Python under
