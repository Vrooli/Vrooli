...
```bash
vrooli scenario status api-health
make stop
vrooli scenario start api-health --clean-stale
```
...
```bash
make status
vrooli scenario port api-health UI_PORT
```
...
api-health status --auto-start

# Or override the API base for this invocation
api-health status --api-base "http://localhost:$(vrooli scenario port api-health API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/api-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/api-health.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/api-health.db"
```
...
`packages/proto/gen/go/api-health/v1/`, TypeScript under
`packages/proto/gen/typescript/api-health/v1/`, and Python under
`packages/proto/gen/python/api_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/api-health/v1/<domain>/`.
