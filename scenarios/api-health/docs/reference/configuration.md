...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/api-health.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `api-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/api-health/config.json`
3. `~/.vrooli/config/api-health/config.json`
4. `~/.config/vrooli/api-health/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port api-health API_PORT`)
