...
envelope (`packages/proto/schemas/api-health/v1/shared/errors.proto`):
...
The proto type lives at `packages/proto/schemas/api-health/v1/shared/health.proto`
...
`POST /vrooli.api_health.v1.<domain>.<Domain>Service/<Method>`
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/api-health/v1/<domain>/`, then run
   `make generate`.
