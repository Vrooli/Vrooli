...
`template-manager orient api-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient api-health
...
```bash
cat > /tmp/prd_context_api-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start api-health --interactive
# …answer the interview, then:
business-health wizard preview api-health
business-health wizard apply api-health
```
...
```bash
vrooli scenario requirements validate api-health --json
```
...
```bash
cat > /tmp/requirements_context_api-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate api-health --json
business-health fix preview api-health
business-health fix apply api-health
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/api-health/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest api-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> api-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario api-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> api-health <adopted-path>
```
