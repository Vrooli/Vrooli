...
import (
	...
	healthH "api-health/handlers/health"
	validationH "api-health/handlers/validation"
)
...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("api-health")
	if err != nil {
		return "", fmt.Errorf("resolve api-health storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve api-health db path: %w", err)
	}
	return sqliteFileDSN(path)
}
...
func main() {
	...
	if preflight.Run(preflight.Config{ScenarioName: "api-health"}) {
		return
	}
	...
	srv := server.New(
		...
		healthH.Module(db, "api-health-api", "1.0.0"),
