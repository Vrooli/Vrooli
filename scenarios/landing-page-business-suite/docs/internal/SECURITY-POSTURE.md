...
- Security Health currently reports residual lockfile advisories from transitive build and test tooling, plus the `x/crypto/openpgp` advisory. OpenTelemetry was upgraded through Scenario Dependency Analyzer to `v1.42.0`, clearing GO-2026-5158. The UI directly pins `picomatch` 4.0.4 through Scenario Dependency Analyzer, which removed the vulnerable 4.x resolver path without weakening coverage policy. Remaining old `minimatch`, `brace-expansion`, `flatted`, and `picomatch` 2.x paths cannot be safely overridden by hand: they require a governed upstream-toolchain update or an approved dependency override. Treat every new production dependency path as a trigger to re-run Security Health.
