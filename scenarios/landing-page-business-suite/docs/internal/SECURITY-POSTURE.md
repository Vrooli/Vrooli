...
- A request whose email could not be delivered is retired and reported as `503 delivery_unavailable`; every well-formed address may sign up, so there is no account-existence oracle to protect. `GET /api/v1/admin/auth/delivery` reports 24-hour outcomes. SendGrid is primary; the site SMTP relay is the fallback.
...
- Stored provider credentials are re-verified hourly and the verdicts are published as `sign_in_email_credentials`, `sign_in_smtp_credentials` and `payments_credentials`. Presence is not usability: a key can be present and revoked, a password present and rotated, a Stripe key present and belonging to the other mode. The probes authenticate and hang up — SendGrid is asked for its key's scopes, the relay is asked to accept a login over TLS and then quits, Stripe is asked for its account — so no mail is sent and no payment object is created. Only an outright rejection degrades health and alerts; "not configured" is a declaration and an unreachable provider is not evidence of a bad credential. A persistent failure alerts once per six-hour window through the operator webhook, recorded in `auth_alert_log`; `GET /api/v1/admin/provider-credentials` serves the cached verdicts.
