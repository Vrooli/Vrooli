...
| landing | Public landing configuration aggregation and resilient fallback projection. | Assemble a renderable public landing response from configuration, pricing, catalog, and display-safe offers without assigning those source domains to the transport. | No independently persisted data; reads configuration, pricing, and delivery state. | aggregation | configuration / settings | landing, landing config, fallback, section, branding | `api/internal/landing/`, `api/handlers/config/`, `api/landing_config_composition.go`, `api/landing_config_connect.go`, `api/internal/landing/presentation_landing_test.go`, `api/handlers/config/landing_config_connect_test.go` |
...
| presentation | Versioned app/bundle/page contracts, validation, eligibility and deterministic page resolution. | Present complete product pages and bundle compositions from configured content without owning transactional state. | No independent persisted data; experimentation ConfigStore owns documents and revisions. | policy / rules | transformation | app profile, bundle, presentation, spotlight, capability, page | `api/internal/presentation/`, `docs/concepts/PRODUCT-PRESENTATION.md`, `ui/src/surfaces/public-landing/presentation/` |

The presentation implementation target is [Product presentation](PRODUCT-PRESENTATION.md).
Experimentation may validate stored presentation documents through the pure
presentation domain. Landing may resolve them and join commerce/delivery state.
Presentation must not import sibling domains, storage, or transports.
