...
| intelligence | Credit-accounted AI gateway requests and upstream provider access. | Provide authenticated AI access with metered usage and provider isolation. | Provider credentials, model usage, AI request records. | Integration / client | temporal workflow | ai, gateway, model, openrouter, usage | `api/handlers/intelligence/` (HTTP transport), `api/internal/intelligence/gateway_service.go`, `api/internal/intelligence/gateway_client.go`, `api/internal/intelligence/openrouter_client.go`, `api/internal/intelligence/` |
