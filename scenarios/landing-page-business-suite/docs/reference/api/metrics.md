...
|--------|------|-------------|
| 400 | `INVALID_EVENT_TYPE` | Event type not in allowed list |
| 400 | `MISSING_VARIANT_ID` | variant_id is required |
| 400 | `MISSING_SESSION_ID` | session_id is required |
| 400 | `EVENT_DATA_REQUIRED` | event_data object required for this event type |
| 400 | `INVALID_JSON` | Malformed JSON in request body |

```json
// Example: Invalid event type error
{
  "error": "Invalid event type",
  "code": "INVALID_EVENT_TYPE",
  "details": {
    "provided": "custom_event",
    "allowed": ["page_view", "scroll_depth", "click", "form_submit", "conversion", "download"]
  }
}

// Example: Missing required data for event type
{
...
|--------|------|-------------|
| 400 | `INVALID_DATE_FORMAT` | Date must be YYYY-MM-DD format |
| 400 | `INVALID_DATE_RANGE` | end_date must be after start_date |
| 401 | `AUTH_REQUIRED` | Admin session not provided |
| 404 | `VARIANT_NOT_FOUND` | Specified variant slug doesn't exist |
...
- [Admin Guide](../../guides/ADMIN_GUIDE.md#analytics-dashboard) - Using the analytics dashboard
