...
func TestRemoteProfileService_RemoteLoginHappyPath(t *testing.T) {
	...
	session, expiresAt, err := svc.remoteLogin(context.Background(), "http://example.com/api/v1", "admin@example.com", "password")
	if err != nil {
...
func TestRemoteProfileService_RemoteLoginMissingCookie(t *testing.T) {
	...
	_, _, err := svc.remoteLogin(context.Background(), "http://example.com/api/v1", "admin@example.com", "password")
	var remoteErr *RemoteProfileError
	if !errors.As(err, &remoteErr) {
		t.Fatalf("expected RemoteProfileError, got %v", err)
	}
