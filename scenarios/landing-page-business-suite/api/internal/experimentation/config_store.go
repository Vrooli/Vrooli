...
// seam: ConfigStorer combines read and write access to configuration.
...
func NewConfigStore(variantsDir, brandingPath string, space *VariantSpace) *ConfigStore {
	return NewConfigStoreWithOptions(ConfigStoreOptions{
...
// ConfigStoreOptions makes filesystem configuration and observability explicit.
...
func (cs *ConfigStore) logEvent(event string, fields map[string]interface{}) {
	if cs.log != nil {
		cs.log(event, fields)
	}
}
...
func (cs *ConfigStore) loadBrandingLocked() error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			cs.branding = defaultBranding()
			cs.logEvent("branding_file_not_found", map[string]interface{}{
...
func (cs *ConfigStore) loadVariantsLocked() error {
	if cs.variantsDir == "" {
		cs.logEvent("variants_dir_not_set", map[string]interface{}{
	...
	if err != nil {
		if os.IsNotExist(err) {
			cs.logEvent("variants_dir_not_found", map[string]interface{}{
				"path":     cs.variantsDir,
				"fallback": "empty variants map",
			})
			return nil
	...
	for _, entry := range entries {
		...
		if err := cs.loadVariantFileLocked(path); err != nil {
			cs.logEvent("variant_load_failed", map[string]interface{}{
				"path":  path,
				"error": err.Error(),
			})
			continue
	...
	cs.logEvent("variants_loaded", map[string]interface{}{
...
func (cs *ConfigStore) loadVariantFileLocked(path string) error {
	...
	snapshot := &VariantSnapshot{
		Variant: VariantSnapshotMeta{
			...
			Weight:       input.Variant.Weight,
			Status:       normalizeVariantStatus(input.Variant.Status),
...
func (cs *ConfigStore) SaveVariant(slug string, snapshot *VariantSnapshot) error {
	...
	cs.logEvent("variant_saved", map[string]interface{}{
		"slug": slug,
		"path": path,
	})
	return nil
...
func (cs *ConfigStore) DeleteVariant(slug string) error {
	...
	cs.logEvent("variant_deleted", map[string]interface{}{
		"slug": slug,
	})
	return nil
}
