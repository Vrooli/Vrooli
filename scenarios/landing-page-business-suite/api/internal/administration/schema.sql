...
CREATE TABLE IF NOT EXISTS remote_profiles (id SERIAL PRIMARY KEY, tag VARCHAR(100) UNIQUE NOT NULL, label TEXT, api_base TEXT NOT NULL, connector_id VARCHAR(64) UNIQUE, remote_session_id TEXT, status VARCHAR(20) NOT NULL DEFAULT 'unknown' CHECK (status IN ('unknown','active','expired','error')), encrypted_session TEXT, encryption_state VARCHAR(16) NOT NULL DEFAULT 'unknown', session_expires_at TIMESTAMP, remote_session_last_synced_at TIMESTAMP, last_login_at TIMESTAMP, last_used_at TIMESTAMP, created_by INTEGER REFERENCES admin_users(id) ON DELETE SET NULL, created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW());
