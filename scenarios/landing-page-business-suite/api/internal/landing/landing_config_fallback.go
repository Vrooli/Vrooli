...
func (s *LandingConfigService) fallbackWithReason(reason string, err error, meta map[string]interface{}) (*LandingConfigResponse, error) {
	...
	if s.eventLogger != nil {
		s.eventLogger("landing_config_fallback", fields)
	}
...
// DefaultFallbackDownloads returns an isolated copy of the startup-safe
// download catalog. API composition uses it only when seeding an empty store.
func DefaultFallbackDownloads() []delivery.App {
	return cloneLandingPayload(defaultFallbackProvider()).Downloads
}
...
func parseFallbackLandingConfig(data []byte) (*LandingConfigPayload, error) {
	...
	payload := &LandingConfigPayload{
		...
		Header:    experimentation.NormalizeLandingHeaderConfig(&raw.Header, raw.Variant.Name),
	}
...
func parseFallbackDownloads(raw json.RawMessage) ([]delivery.App, error) {
	if len(raw) == 0 {
		return []delivery.App{}, nil
	}
	var appsGuess []delivery.App
		...
		if hasStructuredApps {
			for i := range appsGuess {
				if appsGuess[i].Platforms == nil {
					appsGuess[i].Platforms = []delivery.Asset{}
				}
			}
			return appsGuess, nil
		}
	}
	var flatAssets []delivery.Asset
	...
	return []delivery.App{{BundleKey: bundleKey, AppKey: appKey, Name: "Bundle downloads", Tagline: "Installer payload generated from fallback config", Platforms: flatAssets}}, nil
}

func normalizeDownloads(downloads []delivery.App) []delivery.App {
	if downloads == nil {
		return []delivery.App{}
	}
	copied := make([]delivery.App, len(downloads))
...
func cloneDownloads(downloads []delivery.App) []delivery.App {
	if len(downloads) == 0 {
		return []delivery.App{}
	}
	cloned := make([]delivery.App, 0, len(downloads))
	for _, app := range downloads {
		cloned = append(cloned, delivery.App{ID: app.ID, BundleKey: app.BundleKey, AppKey: app.AppKey, Name: app.Name, Tagline: app.Tagline, Description: app.Description, InstallOverview: app.InstallOverview, InstallSteps: append([]string{}, app.InstallSteps...), Storefronts: cloneStorefronts(app.Storefronts), Metadata: cloneContentMap(app.Metadata), DisplayOrder: app.DisplayOrder, Platforms: cloneDownloadAssets(app.Platforms)})
	}
	return cloned
}

func cloneStorefronts(storefronts []delivery.Storefront) []delivery.Storefront {
	if len(storefronts) == 0 {
		return []delivery.Storefront{}
	}
	copied := make([]delivery.Storefront, len(storefronts))
	copy(copied, storefronts)
	return copied
}

func cloneDownloadAssets(assets []delivery.Asset) []delivery.Asset {
	if len(assets) == 0 {
		return []delivery.Asset{}
	}
	copied := make([]delivery.Asset, len(assets))
	for i, asset := range assets {
		copied[i] = delivery.Asset{ID: asset.ID, BundleKey: asset.BundleKey, AppKey: asset.AppKey, Platform: asset.Platform, ArtifactURL: asset.ArtifactURL, ReleaseVersion: asset.ReleaseVersion, ReleaseNotes: asset.ReleaseNotes, Checksum: asset.Checksum, RequiresEntitlement: asset.RequiresEntitlement, Metadata: cloneContentMap(asset.Metadata)}
	}
	return copied
}
...
func cloneHeaderConfig(cfg experimentation.LandingHeaderConfig, variantName string) experimentation.LandingHeaderConfig {
	copy := cfg
	copy.Nav.Links = append([]experimentation.HeaderNavLink{}, cfg.Nav.Links...)
	return experimentation.NormalizeLandingHeaderConfig(&copy, variantName)
}

func clonePricing(pricing *commerce.PricingOverview) *commerce.PricingOverview {
	if pricing == nil {
		return nil
	}
	...
	return cloned.(*commerce.PricingOverview)
}
