...
func TestLoginCreatesASevenDaySecureSession(t *testing.T) {
	...
	_, sessionError := LoginSession(httptest.NewRequest(http.MethodPost, "/", nil), httptest.NewRecorder(), LoginRequest{Email: "admin@example.test", Password: "correct-password"}, deps)
	if sessionError != nil {
		t.Fatal(sessionError)
	}
...
func TestSessionReturnsUnauthenticatedResponseWhenCookieHasNoEmail(t *testing.T) {
	manager := &fakeSessions{session: sessions.NewSession(nil, sessionName)}
	manager.session.Values = map[any]any{}
	response, authenticated := ReadSession(httptest.NewRequest(http.MethodPost, "/", nil), httptest.NewRecorder(), testDependencies(&fakeAuth{}, manager))
