...
- [ ] OT-P0-025 | Stripe credential config | Each landing page resolves Stripe secret and webhook credentials through the Vrooli credential authority; the publishable key may be exposed to the browser through the public runtime configuration
...
- [ ] OT-P0-031 | API-driven landing config and scope-safe failure | Public `/` and eligible app routes MUST render ordered presentation data and owner-supplied commerce from `LandingConfigService.GetLandingConfig` with optional `variant_slug`. When resolution fails, the runtime MUST use only an explicitly configured, validated fallback matching the requested bundle and app scope, or return unavailable when none exists. Any locale fallback MUST identify the resolved locale and reason. A baked generic product narrative MUST NOT replace the requested product on timeout, authorization failure or misconfiguration. This reconciles the legacy fallback target with OT-P0-035 and OT-P0-040; historical BAS fallback bytes remain preserved recovery data.
...
- [x] OT-P0-038 | Product preservation and honest identity | The runtime MUST preserve recovered Browser Automation Studio material in a disabled editable app profile, identify web-console publicly as Aquila, and distinguish available capabilities from coming-soon remote-machine and device control.
- [x] OT-P0-039 | Released visual assets | When presentation assets reference Backdrop Studio, the runtime MUST resolve released surface-specific assets with provenance, dimensions, content hashes, and legibility evidence through public same-origin delivery.
- [x] OT-P0-040 | Scope-safe diagnostics and commerce | When presentation or delivery resolution fails, the runtime MUST report exact revision, variant, scope, and fallback diagnostics and MUST retain commerce-owned prices, entitlements, and installer availability without substituting another product.
...
The following references, decisions, and evolution notes support the implementation and operating posture of this scenario.
