 ...
 * @param context - A descriptive context string for error logging (e.g., "LandingConfigResponse")
 * @returns A discriminated union result with either the validated data or error details
 *
 * @example
 * const result = safeParse(PlanOptionSchema, apiResponse, 'PlanOption');
 * if (result.success) {
 *   // result.data is fully typed
 *   console.log(result.data.plan_name);
 * } else {
 ...
 * @example
 * try {
 *   const config = parseOrThrow(LandingConfigResponseSchema, apiResponse, 'LandingConfig');
 ...
 *
 * @example
 * const plan = parseOrNull(PlanOptionSchema, rawPlan, 'PlanOption');
 * if (plan) {
 *   // plan is fully typed
 * } else {
 *   // Show fallback UI
 ...
 *
 * @example
 * const plans = parseArrayFiltered(PlanOptionSchema, rawPlans, 'PlanOption');
