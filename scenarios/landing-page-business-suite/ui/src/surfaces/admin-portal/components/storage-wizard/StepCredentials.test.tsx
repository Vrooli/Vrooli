...
const existingSettings = { provider: 's3' as const, force_path_style: false, signed_url_ttl_seconds: 900, access_key_id_set: true, secret_access_key_set: true, session_token_set: true, credentials_from_authority: true, settings_row_available: true };

describe('StepCredentials', () => {
  it('shows stored credential guidance and protects configured values behind clear actions', () => {
  ...
  it('forwards newly entered credentials without logging or exposing the secret', () => {
    ...
    expect(screen.getByText(/credential authority, encrypted at rest, and never/)).toBeInTheDocument();
  });
});
