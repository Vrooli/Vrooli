...
export function PresentationPage({ presentation, resolvedActions }: PresentationPageProps) {
  ...
  return <div className={`presentation-page theme-${page.theme.variant} ${presentation.scope === 'bundle' ? 'bundle-page' : 'app-page'}`} lang={page.locale} style={theme} data-presentation-mode={presentation.mode}>
    ...
    <main id={`${id}-main`} tabIndex={-1}>{page.blocks.map(block => <Fragment key={block.id}>{renderBlock(block, { presentation, resources, resolvedActions })}</Fragment>)}</main>
