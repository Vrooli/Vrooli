...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("workflow-health")
	if err != nil {
		return "", fmt.Errorf("resolve workflow-health storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve workflow-health db path: %w", err)
	}
	return sqliteFileDSN(path)
}
