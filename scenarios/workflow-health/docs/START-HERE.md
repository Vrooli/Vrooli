...
`template-manager orient workflow-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient workflow-health
...
```bash
cat > /tmp/prd_context_workflow-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start workflow-health --interactive
# …answer the interview, then:
business-health wizard preview workflow-health
business-health wizard apply workflow-health
```
...
```bash
vrooli scenario requirements validate workflow-health --json
```
...
```bash
cat > /tmp/requirements_context_workflow-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate workflow-health --json
business-health fix preview workflow-health
business-health fix apply workflow-health
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/workflow-health/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest workflow-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> workflow-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario workflow-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> workflow-health <adopted-path>
```
