...
`~/.vrooli/bin/workflow-health`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start workflow-health` if the API is unreachable |
...
```bash
workflow-health status
workflow-health status --json
```

### `workflow-health configure <key> <value>`
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start workflow-health`" is good.
