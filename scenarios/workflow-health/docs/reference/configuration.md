...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/workflow-health.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `workflow-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/workflow-health/config.json`
3. `~/.vrooli/config/workflow-health/config.json`
4. `~/.config/vrooli/workflow-health/config.json`
...
```bash
workflow-health configure api_base http://localhost:15001/api/v1
workflow-health configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port workflow-health API_PORT`)
