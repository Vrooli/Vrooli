# Troubleshooting — Workflow Health
...
```bash
vrooli scenario status workflow-health
# Then either:
make stop
# Or, as last resort:
pkill -f 'workflow-health-api'
...
```bash
make status
vrooli scenario port workflow-health UI_PORT
```
...
workflow-health status --auto-start

# Or override the API base for this invocation
workflow-health status --api-base "http://localhost:$(vrooli scenario port workflow-health API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/workflow-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/workflow-health.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/workflow-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/workflow-health.db}")"
```
...
`packages/proto/gen/go/workflow-health/v1/`, TypeScript under
`packages/proto/gen/typescript/js/workflow-health/v1/`, and Python under
`packages/proto/gen/python/workflow_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/workflow-health/v1/<domain>/`.
