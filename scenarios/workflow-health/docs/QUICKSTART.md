...
workflow-health <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port workflow-health API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.workflow_health.v1.<domain>.<Service>/<Method>
```
...
workflow-health notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port workflow-health API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.workflow_health.v1.notes.NotesService/ListNotes" \
