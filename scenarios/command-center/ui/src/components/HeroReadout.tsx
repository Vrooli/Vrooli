...
import { figureValue, isIllustrative, qualify, resolveReading } from "../lib/provenance";
import { Figure } from "./Figure";
import { FreshnessHairline } from "./FreshnessHairline";
...
export const HeroReadout = forwardRef<HTMLDivElement, HeroReadoutProps>(function HeroReadout({ reading, emptyReason }, ref) {
  if (!reading) {
    return (
      <div ref={ref} className="cc-hero" data-reading data-ink="none" data-provenance="none">
        <Figure value={null} ink="none" scale="wall" placeholder="—" />
  ...
  return (
    <div ref={ref} className="cc-hero" data-reading data-metric-id={reading.id} data-ink={resolution.ink} data-provenance={isIllustrative(resolution) ? "sample" : resolution.figure === "measured" ? "measured" : "absent"} data-coverage={reading.coverage} data-trust={reading.trust} data-empirical={reading.empirical}>
      <Figure value={value} format={reading.format} unit={reading.unit} ink={resolution.ink} scale="wall" placeholder={resolution.ink === "unavailable" ? "––" : "—"} />
      {live ? <FreshnessHairline observedAt={reading.observedAt} ttlSeconds={reading.ttlSeconds} cached={resolution.ink === "dimmed"} /> : null}
      <span className="cc-hero-label">{reading.label}</span>
      <span className="cc-qualifier" data-qualifier data-tone={qualifier.tone}>
        {isIllustrative(resolution) ? <span className="cc-ink-mark" data-ink={resolution.ink}>{resolution.ink === "hollow" ? "in reach" : "missing"}</span> : null}
        {qualifier.text}
      </span>
