...
export function ReadingTile({ reading }: { reading: Reading }) {
  ...
  return (
    <li className="cc-reading" data-reading data-metric-id={reading.id} data-ink={resolution.ink} data-coverage={reading.coverage} data-trust={reading.trust} data-empirical={reading.empirical} data-provenance={illustrative ? "sample" : resolution.figure === "measured" ? "measured" : "absent"} title={reading.description}>
      ...
      <Figure value={value} format={reading.format} unit={reading.unit} ink={resolution.ink} scale="display" placeholder={resolution.ink === "unavailable" ? "––" : "—"} />
      {live ? <FreshnessHairline observedAt={reading.observedAt} ttlSeconds={reading.ttlSeconds} cached={resolution.ink === "dimmed"} /> : null}
      {illustrative && reading.sample ? <Sparkline series={reading.sample.series} illustrative /> : null}
