...
- [ ] OT-P0-007 | Bounded description retrieval | Automatic routing retrieves a bounded provider-description shortlist through the shared embedding contract, persists descriptor vectors with model metadata, and names `routing_index_unavailable` when it falls back to deterministic relevance-scored enumeration.
...
- [ ] OT-P1-002 | Measurement backbone | Per-query telemetry (classified types, providers hit, counts, latency p50/p95, zero-result rate, and degradation reasons); insights surface under-utilized providers and zero-result queries, with declared zero-yield and corpus-validation measures.
