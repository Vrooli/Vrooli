...
const resolveChoice = (choice: ThemeChoice): "light" | "dark" => {
  ...
  // Runtime guard for non-DOM environments (SSR, and jsdom which does not
  // implement matchMedia by default). TS's DOM lib types window.matchMedia as
  // always-present, so the rule flags it as unnecessary — but it is real at
  // runtime; without it `system` theme resolution throws in tests.
...
export function ThemeProvider({ children, initialChoice }: ThemeProviderProps) {
  ...
  useEffect(() => {
    // Same non-DOM runtime guard as resolveChoice — real at runtime, but the
    // DOM lib types make the rule consider it unnecessary.
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
...
// The context hook is intentionally co-located with its provider component;
// splitting it into a separate file to satisfy fast-refresh would fragment the
// context contract for no runtime benefit.
