...
func (b *TelemetryBridge) Record(ctx context.Context, s internalrouting.TelemetrySample) {
	...
	if err := b.store.Record(ctx, internalmetrics.Sample{
		...
		SelectedLeafCount:       s.SelectedLeafCount,
		WidenedLeafCount:        s.WidenedLeafCount,
		FanoutWidthBoundReached: s.FanoutWidthBoundReached,
		...
		ResolverCacheHits:       s.ResolverCacheHits,
		ResolverCacheMisses:     s.ResolverCacheMisses,
