...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/search-hub/v1/health/health.proto`;
...
import (
    ...
    "search-hub/internal/testutil/assertx"
...
The proto schema in `packages/proto/schemas/search-hub/v1/health/health.proto`
...
|---|---|---|
| Wire contract | `packages/proto/schemas/search-hub/v1/notes/notes.proto` | `Note`, `service NotesService`, `ListNotesResponse`, `CreateNoteRequest`, `CreateNoteResponse`, `GetNoteRequest`, `GetNoteResponse` |
| REST metadata contract | `packages/proto/schemas/search-hub/v1/notes/attachments.proto` | `Attachment` and `UploadAttachmentResponse` for the multipart upload exception |
...
| REST error envelope | `packages/proto/schemas/search-hub/v1/errors/errors.proto` + `internal/httpx/errors.go::WriteError` | Typed body for REST exceptions, with canonical codes (`invalid_request`, `not_found`, `internal`) |
...
dynamically from `go.mod`, so it works after `search-hub`
substitution.
...
import (
    ...
    clitest "search-hub/cli/internal/testutil"
)
...
under `packages/proto/schemas/search-hub/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/search-hub/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.search_hub.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/search-hub/v1/<domain>;<domain>_v1";
   ```
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import notesv1 "github.com/vrooli/vrooli/packages/proto/gen/go/search-hub/v1/notes"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/search-hub/v1/notes/notes_pb";
