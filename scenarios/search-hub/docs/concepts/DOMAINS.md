...
| eval | Per-provider search-quality baselines: register provider-owned golden suites, run them against the provider's registered endpoint (reusing `providers.MapResults` — no provider-specific code), and store immutable, **tagged** runs for trend/compare. Soft labels, never a pass/fail gate (opt-in `--assert` per consumer). Sibling of `registry`, not a fork of it. | Telemetry / experimentation | `eval_suites` + `eval_runs` tables (suite definitions + immutable runs — no corpus data). | API, CLI, UI | search-quality-baseline plan | (eval domain) | `api/internal/eval/`, `api/handlers/eval/`, `cli/domains/evals/`, `ui/src/features/evals/`, `packages/proto/schemas/search-hub/v1/eval/` |
