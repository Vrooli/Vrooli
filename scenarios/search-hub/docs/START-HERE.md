...
`vrooli scenario orient search-hub`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient search-hub
...
```bash
prd-control-tower prd generate search-hub \
...
```bash
prd-control-tower prd validate search-hub --json
```
...
```bash
prd-control-tower requirements generate search-hub \
...
```bash
prd-control-tower requirements validate search-hub --json
```
