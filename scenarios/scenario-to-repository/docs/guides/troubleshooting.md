# Troubleshooting — Source repository
...
```bash
vrooli scenario status scenario-to-repository
make stop
vrooli scenario start scenario-to-repository --clean-stale
```
...
```bash
make status
vrooli scenario port scenario-to-repository UI_PORT
```
...
scenario-to-repository status --auto-start

# Or override the API base for this invocation
scenario-to-repository status --api-base "http://localhost:$(vrooli scenario port scenario-to-repository API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/scenario-to-repository/`.
...
The default route is `${SCENARIO_DATA_DIR}/scenario-to-repository.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/scenario-to-repository.db"
```
...
`packages/proto/gen/go/scenario-to-repository/v1/`, TypeScript under
`packages/proto/gen/typescript/scenario-to-repository/v1/`, and Python under
`packages/proto/gen/python/scenario_to_repository/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/scenario-to-repository/v1/<domain>/`.
