...
This is the first document to read when developing or operating the source
distribution ramp. The monorepo remains canonical. The gates below establish
the scenario contract and must not be interpreted as a promise that an export
is independently deployable or already published.
...
`template-manager orient scenario-to-repository`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient scenario-to-repository
--finalize` to remove only that temporary orientation metadata.

The source domain and deterministic archive verifier are durable product
surfaces. The dashboard is an operator preview. In particular:

- Source closure, recipe, policy, archive, verification, and human-publication
  handoff are the real domain. They must remain aligned with the generated
  proto contract and the source-distribution reference.
...
```bash
cat > /tmp/prd_context_scenario-to-repository.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start scenario-to-repository --interactive
# …answer the interview, then:
business-health wizard preview scenario-to-repository
business-health wizard apply scenario-to-repository
```
...
```bash
vrooli scenario requirements validate scenario-to-repository --json
```
...
```bash
cat > /tmp/requirements_context_scenario-to-repository.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate scenario-to-repository --json
business-health fix preview scenario-to-repository
business-health fix apply scenario-to-repository
```
...
```bash
react-component-library adoptions suggest scenario-to-repository --json
react-component-library adoptions link <component-id> scenario-to-repository
react-component-library adoptions obligations scenario-to-repository --json
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate scenario-to-repository --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate scenario-to-repository`, the notes page spec
...
validate scenario-to-repository --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/scenario-to-repository/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest scenario-to-repository <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> scenario-to-repository
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario scenario-to-repository
```
...
- Wire contracts belong in
  `packages/proto/schemas/scenario-to-repository/v1/<domain>/`.
