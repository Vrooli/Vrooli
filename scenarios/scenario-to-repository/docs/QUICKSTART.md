...
```bash
scenario-to-repository status
scenario-to-repository <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port scenario-to-repository API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.scenario_to_repository.v1.<domain>.<Service>/<Method>
```
...
CLI (`scenario-to-repository --help`) — see
