# CLI Commands — Source repository
...
`~/.vrooli/bin/scenario-to-repository`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start scenario-to-repository` if the API is unreachable |
...
```bash
scenario-to-repository status
scenario-to-repository status --json
```

### `scenario-to-repository configure <key> <value>`
...
```bash
scenario-to-repository configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`scenario-to-repository <domain> <verb>`). Every command calls a single API
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start scenario-to-repository`" is good.
