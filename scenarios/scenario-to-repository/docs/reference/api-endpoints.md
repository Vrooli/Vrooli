...
`packages/proto/schemas/scenario-to-repository/v1/<domain>/<file>.proto`.
...
envelope (`packages/proto/schemas/scenario-to-repository/v1/shared/errors.proto`):
...
| **CLI** | `scenario-to-repository status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/scenario-to-repository/v1/shared/health.proto`
...
`POST /vrooli.scenario_to_repository.v1.<domain>.<Domain>Service/<Method>`
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/scenario-to-repository/v1/<domain>/`, then run
   `make generate`.
