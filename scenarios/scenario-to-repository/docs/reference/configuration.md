...
For `scenario-to-repository` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/scenario-to-repository/config.json`
3. `~/.vrooli/config/scenario-to-repository/config.json`
4. `~/.config/vrooli/scenario-to-repository/config.json`
...
```bash
scenario-to-repository configure api_base http://localhost:15001/api/v1
scenario-to-repository configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port scenario-to-repository API_PORT`)
