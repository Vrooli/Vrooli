...
Each requirement carries one or more validation entries. Unit entries point to
deterministic package tests, integration entries identify an owner contract or
fixture, and manual entries identify an operator-reviewable evidence check.
Validation status is earned by Test Genie or the named owner; changing a
requirement status alone is not evidence.
