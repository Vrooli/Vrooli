...
Reproducible one-way source distribution for Vrooli scenarios

This scenario provides a one-way, reproducible source-distribution ramp for
Vrooli scenarios. The monorepo remains canonical; an export is a reviewed
source snapshot and never a bidirectional repository mirror.

The implementation uses the standard full-stack Vrooli scenario shape:
...
An export contains only the SDA-admitted source closure, a canonical
`SOURCE-MANIFEST.json`, deterministic archive metadata, and exact digests.
Private files, Git history, plan artifacts, secret-like content, unresolved
obligations, unsafe paths, and unlicensed binary assets are refused or
excluded according to the versioned policy. Runtime services remain external
requirements unless the closure explicitly records a supported alternative.

See [`docs/reference/source-distribution.md`](docs/reference/source-distribution.md)
for the build, verification, limitation, and human-publication contract.
...
The source domain is the durable product. The dashboard is an operational
preview, not a claim that a source export has been published or that every
runtime dependency is bundled. Host publication requires a human-approved
handoff and destination read-back; no Git repository is created by assembly.

The generated scaffold is not an export promise. Preserve these durable seams:
...
`test-genie execute scenario-to-repository --preset comprehensive` directly for
