...
const connectRpcPath = /^\/vrooli\.scenario_to_repository\.v1\./
...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start scenario-to-repository` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start scenario-to-repository` rather than running server.js directly.',
  )
}
...
startScenarioServer({
  ...
  apiPort,
  distDir: './dist',
  serviceName: 'scenario-to-repository',
  corsOrigins: '*',
  setupRoutes(app) {
