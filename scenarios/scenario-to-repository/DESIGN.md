...
This design language fits source-distribution operators because the primary
work is comparing paths, reasons, digests, gates, and review preconditions in
one dense workspace. The source tree and evidence inspector are the dominant
surfaces; publication is a separate, visibly human-only handoff. The default
tokens remain unchanged so status is legible in light, dark, mobile, and
