...
async function getAppStorageRoot(): Promise<string> {
    ...
    await fs.promises.mkdir(appStorageRoot, { recursive: true });
...
async function initializeTelemetry() {
    try {
        ...
        await fs.promises.appendFile(telemetryFilePath, "");
...
async function recordTelemetry(event: string, details: TelemetryDetails = {}, level: TelemetryLevel = "info"): Promise<void> {
    ...
    try {
        await fs.promises.appendFile(telemetryFilePath, JSON.stringify(payload) + "\n");
    } catch (error) {
...
async function uploadTelemetryFile(filePath: string, uploadURL: string, reason: string, force = false): Promise<void> {
    ...
    try {
        ...
        await fs.promises.writeFile(statePath, JSON.stringify({ lastSignature: signature, reason }, null, 2));
    } catch {
...
async function createMainWindow() {
    ...
    // Attach state manager to track window changes and save on close
...
async function startBundledRuntime(): Promise<string> {
    ...
    try {
        manifestContent = JSON.parse(await fs.promises.readFile(manifestPath, "utf-8"));
    } catch (err) {
...
ipcMain.handle("storage:write-file", async (_event, data: { path: string; data: string | number[]; isBinary: boolean }) => {
    ...
    if (data.isBinary && Array.isArray(data.data)) {
        ...
        await fs.promises.writeFile(fullPath, Buffer.from(data.data));
    } else {
        // String data
        await fs.promises.writeFile(fullPath, data.data as string, "utf-8");
    }
});
...
async function storeAuthTokens(tokens: StoredTokens): Promise<void> {
    ...
    if (safeStorage.isEncryptionAvailable()) {
        ...
        await fs.promises.writeFile(tokensPath, encrypted);
    } else {
        ...
        await fs.promises.writeFile(tokensPath, tokenJson, "utf-8");
    }
}
...
async function getStoredTokens(): Promise<StoredTokens | null> {
    ...
    try {
        const fileContent = await fs.promises.readFile(tokensPath);
...
async function storeUserInfo(user: StoredUser): Promise<void> {
    ...
    await fs.promises.writeFile(userPath, JSON.stringify(user, null, 2), "utf-8");
}
...
async function clearAuthData(): Promise<void> {
    ...
    try {
        if (tokensPath) await fs.promises.unlink(tokensPath);
    } catch (error: any) {
        if (error.code !== "ENOENT") console.error("[Auth] Failed to delete tokens:", error);
    }

    try {
        if (userPath) await fs.promises.unlink(userPath);
    } catch (error: any) {
