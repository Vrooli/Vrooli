...
For Linux, the recommended path is a managed publisher key held by the
credential authority. Name one shared identity to sign every desktop app:
...
The key is generated once and reused for later scenarios that name the same
identity. Detailed custody, rotation, validation, and troubleshooting steps are
in the dedicated signing guide:
