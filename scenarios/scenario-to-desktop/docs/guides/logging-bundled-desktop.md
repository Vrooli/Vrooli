...
3. Regenerate the desktop bundle/installers (example for a profile):
   - `deployment-manager deploy-desktop --profile <profile-id> --platforms linux --timeout 20m`
   - Artifacts land in `scenarios/<scenario>/platforms/electron/dist-electron/`.

   Configure release-authority credentials through the managed credential flow.
   Do not put passphrases in shell history, command-line arguments, or copied
   environment dumps. Local development artifacts remain non-promotable unless
   the release gate says otherwise.
...
ELECTRON_ENABLE_LOGGING=1 ELECTRON_ENABLE_STACK_DUMPING=1 NODE_OPTIONS="" \
  timeout 60s ./"<AppName>-<version>.AppImage" \
  --enable-logging --verbose --disable-gpu 2>&1 | tee ~/bundled-app.log
```
Notes:
