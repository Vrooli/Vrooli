...
| runtime | Supervises bundled services, provider selection, readiness, leases, and safe fallback observations. | Make desktop resource ownership and communication routes explicit at runtime. | Runtime state, provider observations, and lease metadata. | orchestration | service | RuntimeSupervisor, ProviderObservation, SharedLease | `runtime/` |
