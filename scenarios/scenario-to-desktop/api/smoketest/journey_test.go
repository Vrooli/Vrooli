...
func (c *journeyTestCapture) Capture(context.Context, string, string, string, string) (deliveryramp.EvidenceReference, error) {
	c.next++
	return deliveryramp.EvidenceReference{ID: "capture-" + string(rune('a'+c.next)), Kind: "screenshot"}, nil
}
...
func (journeyTestAPI) Probe(_ context.Context, operation string) (JourneyOperationResult, error) {
	if operation == "provider_observation" {
		return JourneyOperationResult{
			...
			Provider: &deliveryramp.ProviderObservation{DeploymentMode: "bundled", ProviderTier: "managed-private", ServiceIdentity: "fixture", Readiness: "ready", SafeRouteClass: "private-bundle"},
		}, nil
	}
...
func (w *journeyTestWaiter) WaitUntil(context.Context, deliveryramp.ReadinessPolicy, func(context.Context) (bool, string, error)) (WaitResult, error) {
	w.waitCount++
	if w.err != nil {
...
func (w *journeyTestWaiter) Settle(context.Context, deliveryramp.SettlePolicy) error {
	w.settleCount++
	return nil
}
...
func TestDesktopJourney_RegisteredFixtureProducesReviewableTimeline(t *testing.T) {
	...
	if result.Disposition != deliveryramp.Disposition(journeyPass) {
		t.Fatalf("disposition = %q, want pass: %+v", result.Disposition, result)
	}
	if result.SchemaVersion != deliveryramp.JourneySchemaVersion || result.PlanID == "" || result.Capability != "hello-desktop" {
	...
	if semantic.Purpose == "" || semantic.AssertionStatus != string(deliveryramp.StepPassed) || semantic.ObservedState != semantic.ExpectedState {
...
func TestWorkflowExecutionReferenceRequiresSharedValidationIdentity(t *testing.T) {
	reference := deliveryramp.WorkflowExecutionReference{
		...
		CellID: "cell-1", Disposition: string(deliveryramp.DispositionPass),
		Artifacts: []deliveryramp.EvidenceReference{{ID: "workflow-trace", Kind: "trace", URI: "evidence/workflow-trace.json", Checksum: "sha256:trace", Redacted: true}},
	}
...
func TestAttachWorkflowReferenceBindsDesktopRunIdentity(t *testing.T) {
	...
	journey := deliveryramp.JourneyResult{SmokeTestID: "desktop-run-1", TargetID: "target-1", CellID: "cell-1", Disposition: deliveryramp.DispositionPass}
...
func TestAppendWorkflowChapterKeepsJourneyTimelineOrdered(t *testing.T) {
	...
	journey := deliveryramp.JourneyResult{
		SchemaVersion: deliveryramp.JourneySchemaVersion, EvidenceVersion: deliveryramp.JourneyEvidenceVersion, SmokeTestID: "run-1",
		ScenarioName: "fixture", Capability: "desktop.launch.baseline", PlanID: "desktop.launch.baseline.v2",
		CreatedAt: start, CompletedAt: start.Add(time.Second), Steps: []deliveryramp.JourneyStep{{ID: "launch", StartedAt: start, CompletedAt: start.Add(time.Second), MonotonicStartMs: 0, MonotonicEndMs: 10}},
	}
	ref := &deliveryramp.WorkflowExecutionReference{Provider: "workflow-provider", AssetID: "asset-1", ExecutionID: "execution-1", RunID: "run-1", ArtifactDigest: "sha256:artifact", TargetID: "target-1", CellID: "cell-1", Disposition: string(deliveryramp.DispositionPass), Artifacts: []deliveryramp.EvidenceReference{{ID: "workflow-trace", Kind: "trace", URI: "evidence/workflow-trace.json", Checksum: "sha256:trace", Redacted: true}}}
	...
	if got := journey.Steps[len(journey.Steps)-1]; got.ID != "workflow" || got.Action != "workflow_execution" || got.Disposition != deliveryramp.StepPassed {
...
func TestDesktopJourney_MissingCapabilityIsUnavailable(t *testing.T) {
	...
	if result.Disposition != deliveryramp.DispositionUnavailable || result.DegradedReason != "capability_not_registered" {
...
func TestDesktopJourney_XdotoolAbsentIsDegradedAndNeverPasses(t *testing.T) {
	...
	if result.Disposition != deliveryramp.Disposition(journeyDegraded) {
...
func TestDesktopJourney_CommunicationFixtureProvesProviderAndRoute(t *testing.T) {
	...
	if result.Steps[1].AssertionStatus != string(deliveryramp.StepPassed) || result.Steps[2].AssertionStatus != string(deliveryramp.StepPassed) {
...
func TestDesktopJourney_CommunicationFixturesHaveDedicatedMachineContracts(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.capability, func(t *testing.T) {
			...
			if len(result.Steps) != 4 || result.Steps[1].AssertionStatus != string(deliveryramp.StepPassed) || result.Steps[2].AssertionStatus != string(deliveryramp.StepPassed) {
...
func TestDesktopJourney_PeerCapabilityIsExplicitlyUnsupported(t *testing.T) {
	...
	if result.Disposition != deliveryramp.DispositionUnsupported || result.DegradedReason != "peer_protocol_not_implemented" {
		t.Fatalf("peer result = %+v", result)
	}
}

func TestJourneyPoliciesSerializeMilliseconds(t *testing.T) {
	data, err := json.Marshal(deliveryramp.ReadinessPolicy{ID: "ready", Timeout: 12 * time.Second, PollInterval: 100 * time.Millisecond, StabilityCount: 2})
	...
	var decoded deliveryramp.ReadinessPolicy
...
func TestDefaultJourneyWaiterRequiresStableReadiness(t *testing.T) {
	...
	policy := deliveryramp.ReadinessPolicy{ID: "stable", Timeout: time.Second, PollInterval: time.Millisecond, StabilityCount: 2}
	attempts := 0
...
func TestDefaultJourneyWaiterRejectsInvalidPolicies(t *testing.T) {
	waiter := defaultJourneyWaiter{clock: RealClock{}}
	if _, err := waiter.WaitUntil(context.Background(), deliveryramp.ReadinessPolicy{ID: "invalid", Timeout: time.Second}, func(context.Context) (bool, string, error) { return true, "", nil }); err == nil {
		t.Fatal("readiness policy without poll interval must be rejected")
	}
	if err := waiter.Settle(context.Background(), deliveryramp.SettlePolicy{ID: "invalid", Minimum: time.Second, Maximum: 500 * time.Millisecond}); err == nil {
...
func TestValidateJourneyTimelineRejectsAmbiguousOrUnsafeRecords(t *testing.T) {
	...
	valid := deliveryramp.JourneyResult{
		SchemaVersion:   deliveryramp.JourneySchemaVersion,
		...
		Steps:           []deliveryramp.JourneyStep{{ID: "launch", Name: "launch", Action: "window_activate", Disposition: deliveryramp.StepPassed, StartedAt: now, CompletedAt: now.Add(500 * time.Millisecond), MonotonicStartMs: 0, MonotonicEndMs: 500}},
		Events:          []deliveryramp.JourneyEvent{{Type: "step_completed", StartedAt: now, CompletedAt: now, MonotonicStartMs: 0, MonotonicEndMs: 0}},
	}
	if err := ValidateJourneyTimeline(valid); err != nil {
		t.Fatalf("valid timeline rejected: %v", err)
	}

	tests := map[string]func(*deliveryramp.JourneyResult){
		"duplicate step IDs": func(value *deliveryramp.JourneyResult) { value.Steps = append(value.Steps, value.Steps[0]) },
		"negative video offset": func(value *deliveryramp.JourneyResult) {
			negative := int64(-1)
			value.Steps[0].VideoStartOffsetMs = &negative
		},
		"redaction leak":          func(value *deliveryramp.JourneyResult) { value.Steps[0].ObservedState = "bearer token" },
		"event timestamp missing": func(value *deliveryramp.JourneyResult) { value.Events[0].StartedAt = time.Time{} },
	}
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			value := valid
			value.Steps = append([]deliveryramp.JourneyStep(nil), valid.Steps...)
			value.Events = append([]deliveryramp.JourneyEvent(nil), valid.Events...)
			mutate(&value)
...
func TestJourneyRequiresScreenshotBeforeAndAfterEveryInteraction(t *testing.T) {
	complete := []deliveryramp.JourneyStep{{Action: "pointer_click", BeforeCaptureID: "before", AfterCaptureID: "after"}}
	if !journeyHasScreenshotPairs(complete) {
		t.Fatal("complete interaction screenshot pair should be accepted")
	}
	incomplete := []deliveryramp.JourneyStep{{Action: "pointer_click", BeforeCaptureID: "before"}}
