...
func (s *DefaultService) RunPreflightJob(jobID string, request Request) {
	...
	manifest, bundleRoot, err := s.loadJobManifest(request)
	if err != nil {
		fail("validation", err)
		return
	}
	...
	handle, cleanup, err := s.acquireJobRuntime(request, manifest, bundleRoot, timeout)
	if err != nil {
		fail("runtime", err)
		return
	}
	...
	s.collectJobRuntimeStatus(jobID, handle.Client, manifest, bundleRoot)

	if err := s.runJobValidationPhase(jobID, handle.Client, request); err != nil {
		fail("secrets", err)
		return
	}

	validation, secrets, err := s.runJobValidateAndSecrets(jobID, handle.Client)
	if err != nil {
		fail("validation", err)
		return
	}

	ready, err := s.runJobReadinessPhase(jobID, handle.Client, request, timeout)
	if err != nil {
		fail("services", err)
		return
	}

	if err := s.runJobDiagnosticsPhase(jobID, handle.Client, manifest, validation, ready, secrets, request); err != nil {
		fail("diagnostics", err)
		return
	}
...
// loadJobManifest loads and validates the manifest and resolves the bundle root.
func (s *DefaultService) loadJobManifest(request Request) (*bundlemanifest.Manifest, string, error) {
	manifestPath, err := filepath.Abs(request.BundleManifestPath)
	if err != nil {
		return nil, "", fmt.Errorf("resolve bundle_manifest_path: %w", err)
	}
	if _, err := os.Stat(manifestPath); err != nil {
		return nil, "", fmt.Errorf("bundle manifest not found: %w", err)
	}
	...
	if err != nil {
		return nil, "", fmt.Errorf("resolve bundle_root: %w", err)
	}
	return manifest, bundleRoot, nil
}

// acquireJobRuntime creates a runtime handle for the async job.
func (s *DefaultService) acquireJobRuntime(request Request, manifest *bundlemanifest.Manifest, bundleRoot string, timeout time.Duration) (*RuntimeHandle, func(), error) {
...
// collectJobRuntimeStatus fetches runtime status and fingerprints, storing results on the job.
func (s *DefaultService) collectJobRuntimeStatus(jobID string, client RuntimeClient, manifest *bundlemanifest.Manifest, bundleRoot string) {
...
// runJobValidationPhase applies secrets.
func (s *DefaultService) runJobValidationPhase(jobID string, client RuntimeClient, request Request) error {
...
// runJobValidateAndSecrets validates the bundle and fetches secrets.
func (s *DefaultService) runJobValidateAndSecrets(jobID string, client RuntimeClient) (*runtimeapi.BundleValidationResult, []Secret, error) {
	s.jobs.SetStep(jobID, "validation", "running", "validating bundle")
	validation, err := client.Validate()
	if err != nil {
		return nil, nil, fmt.Errorf("validate bundle: %w", err)
	}
	s.jobs.SetResult(jobID, func(prev *Response) *Response {
		return updatePreflightResult(prev, func(next *Response) {
...
// runJobReadinessPhase checks runtime readiness.
func (s *DefaultService) runJobReadinessPhase(jobID string, client RuntimeClient, request Request, timeout time.Duration) (*Ready, error) {
...
// runJobDiagnosticsPhase collects ports, telemetry, and logs.
func (s *DefaultService) runJobDiagnosticsPhase(jobID string, client RuntimeClient, manifest *bundlemanifest.Manifest, validation *runtimeapi.BundleValidationResult, ready *Ready, secrets []Secret, request Request) error {
	s.jobs.SetStep(jobID, "diagnostics", "running", "collecting diagnostics")
	ports, err := client.Ports()
	if err != nil {
		return fmt.Errorf("fetch ports: %w", err)
	}
	telemetryResp, err := client.Telemetry()
	if err != nil {
		return fmt.Errorf("fetch telemetry: %w", err)
	}
	logTails := client.LogTails(request)
...
// --- Shared helpers used by both RunBundlePreflight and RunPreflightJob ---

// loadAndValidateManifest loads and validates a bundle manifest from disk.
func loadAndValidateManifest(rawPath string) (*bundlemanifest.Manifest, string, error) {
	manifestPath, err := filepath.Abs(rawPath)
	if err != nil {
...
// resolveBundleRoot resolves the bundle root path, defaulting to the manifest directory.
func resolveBundleRoot(rawRoot, manifestPath string) (string, error) {
	bundleRoot := strings.TrimSpace(rawRoot)
	if bundleRoot == "" {
		bundleRoot = filepath.Dir(manifestPath)
	}
	abs, err := filepath.Abs(bundleRoot)
	if err != nil {
		return "", &StatusError{Status: http.StatusBadRequest, Err: fmt.Errorf("resolve bundle_root: %w", err)}
	}
	return abs, nil
}

// fetchValidationData fetches validation and secrets (skipped in status-only mode).
func fetchValidationData(client RuntimeClient, request Request) (*runtimeapi.BundleValidationResult, []Secret, error) {
...
// fetchDiagnostics fetches ports, telemetry, and log tails from the runtime.
func fetchDiagnostics(client RuntimeClient, request Request) (map[string]map[string]int, *Telemetry, []LogTail, error) {
	ports, err := client.Ports()
	if err != nil {
		return nil, nil, nil, fmt.Errorf("fetch ports: %w", err)
	}
	telemetryResp, err := client.Telemetry()
	if err != nil {
		return nil, nil, nil, fmt.Errorf("fetch telemetry: %w", err)
	}
	logTails := client.LogTails(request)
	return ports, telemetryResp, logTails, nil
}

// statusErrorSlice converts a non-nil error into a string slice for the response.
func statusErrorSlice(err error) []string {
	if err != nil {
		return []string{fmt.Sprintf("runtime status: %v", err)}
	}
	return nil
}

// formatExpiresAt formats an expiry time, returning "" for zero times.
...
func collectAssetChecks(validation *runtimeapi.BundleValidationResult) []Check {
	if validation == nil {
		return nil
	}
	...
	checks := make([]Check, 0, len(assetIssues))
