...
func NewWindowsDetector(fs FileSystem, cmd CommandRunner, env EnvironmentReader) *WindowsDetector {
...
func (d *WindowsDetector) DetectTools(ctx context.Context) []types.ToolDetectionResult {
...
func (d *WindowsDetector) DiscoverCertificates(ctx context.Context) ([]types.DiscoveredCertificate, error) {
	// Only discover on Windows
	if runtime.GOOS != "windows" {
		return nil, nil
	}
...
func (d *WindowsDetector) detectSigntool(ctx context.Context) types.ToolDetectionResult {
...
func (d *WindowsDetector) listStoreCertificates(ctx context.Context) ([]types.DiscoveredCertificate, error) {
	var certs []types.DiscoveredCertificate
	...
	for _, line := range lines {
		...
		certs = append(certs, types.DiscoveredCertificate{
			ID:           thumbprint,
			Name:         name,
			Subject:      subject,
			Issuer:       issuer,
...
func (d *WindowsDetector) ParsePFXCertificate(data []byte, password string) (*types.DiscoveredCertificate, error) {
...
func (d *WindowsDetector) ParsePEMCertificate(data []byte) (*types.DiscoveredCertificate, error) {
...
func (d *WindowsDetector) extractCertificateInfo(cert *x509.Certificate) *types.DiscoveredCertificate {
	now := time.Now()
	...
	return &types.DiscoveredCertificate{
