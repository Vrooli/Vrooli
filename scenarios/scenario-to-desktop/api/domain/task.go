// Package domain defines the core domain types for the scenario-to-desktop scenario.
//
...
const (
	// TaskTypeInvestigate is for diagnosing pipeline failures without making changes.
	TaskTypeInvestigate TaskType = "investigate"
	// TaskTypeFix is for applying fixes through an iterative diagnose-change-rebuild-verify loop.
...
const (
	// EffortChecks performs quick validation (verify outputs exist, check file sizes).
	EffortChecks InvestigationEffort = "checks"
	// EffortLogs performs build log analysis and standard diagnostic procedures.
	EffortLogs InvestigationEffort = "logs"
	// EffortTrace performs deep filesystem/process inspection and dependency analysis.
...
type TaskFocus struct {
	// Harness focuses on scenario-to-desktop pipeline infrastructure.
	Harness bool `json:"harness"`
	// Subject focuses on the target scenario being built.
...
type FixPermissions struct {
	// Immediate allows immediate fixes (rebuild, restart processes, clear caches).
	Immediate bool `json:"immediate"`
	// Permanent allows code/configuration changes in the codebase.
	Permanent bool `json:"permanent"`
	// Prevention allows implementing validation checks or pipeline improvements.
