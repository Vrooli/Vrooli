...
export function findDeploymentOption(mode?: string): DeploymentOption {
  return (
    DEPLOYMENT_OPTIONS.find((option) => option.value === mode) ??
...
export function findServerTypeOption(serverType?: string): ServerTypeOption {
  return (
    SERVER_TYPE_OPTIONS.find((option) => option.value === serverType) ??
    DEFAULT_SERVER_TYPE_OPTION
  );
}
...
export function requiresProxyURL(
  mode: DeploymentMode,
