...
export function ScenarioCard({
  ...
  const uniquePlatforms = Array.from(
    new Set(
      artifacts
        .map((artifact: DesktopBuildArtifact) => artifact.platform)
        .filter(Boolean),
    ),
  ) as string[];
          ...
          <div className="flex min-w-0 gap-3">
            ...
            <div className="min-w-0 space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                <h4 className="break-words text-lg font-semibold text-slate-100">
                  {scenario.name}
                </h4>
                {!scenario.has_desktop && (
                  <Badge
                ...
                {scenario.has_desktop && !scenario.built && (
                  <Badge
                    variant="warning"
                    className="text-xs bg-yellow-900/30 text-yellow-400 border-yellow-700"
                  >
              ...
              <div className="text-xs text-slate-400">
                {scenario.desktop_path
                  ? scenario.desktop_path
                  : "Desktop wrapper not generated yet"}
              </div>
            </div>
          </div>
          <div className="flex shrink-0 flex-col gap-2 text-xs text-slate-400 sm:items-end sm:text-right">
            ...
            {scenario.last_modified && (
              <span>Updated {scenario.last_modified}</span>
            )}
          </div>
        </div>
