...
vi.mock("./ScenarioCard", () => ({
  ScenarioCard: ({
    ...
    onSelect: (scenario: { name: string }) => void;
  }) => (
    <button
      type="button"
      onClick={() => {
...
describe("ScenarioInventory", () => {
  ...
  it("shows a loading state and the server error", async () => {
    mocks.fetchScenarioDesktopStatus.mockRejectedValue(
      new Error("catalog unavailable"),
    );
    render(<ScenarioInventory />);
    expect(screen.getByText("Loading scenarios...")).toBeInTheDocument();
    expect(
      await screen.findByText("Failed to load scenarios"),
    ).toBeInTheDocument();
  ...
  it("reports statistics, filters scenarios, and launches the selected one", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Open canvas-lab" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Open bridge" }),
  ...
  it("searches by display name and explains an empty filtered result", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Open bridge" }),
    ).toBeInTheDocument();
    fireEvent.change(
      screen.getByRole("textbox", { name: "Search scenarios" }),
      {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No scenarios found matching your filters"),
      ).toBeInTheDocument();
    });
  });
});
