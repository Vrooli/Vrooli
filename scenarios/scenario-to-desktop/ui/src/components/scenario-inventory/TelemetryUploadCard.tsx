...
export function TelemetryUploadCard({
  scenarioName,
  appDisplayName,
}: TelemetryUploadCardProps) {
    ...
    return (
      ...
      >
        ...
        <span className="flex-1">
          After testing, help us improve by uploading telemetry
        </span>
    ...
    <div className="rounded-lg border border-slate-700 bg-slate-900/40 p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button
          ...
          size="sm"
          className="h-7 px-2"
          aria-label="Collapse telemetry upload"
          onClick={() => {
            setIsExpanded(false);
          }}
        >
      ...
      <p className="text-xs text-slate-400">
        The desktop app logs startup events and dependency failures to{" "}
        <code className="text-slate-300">deployment-telemetry.jsonl</code>.
        ...
        {insightsError && (
          <p className="text-xs text-red-400">
            {insightsError instanceof Error
              ...
              : "Failed to load telemetry insights"}
          </p>
        )}
          ...
          <div className="grid gap-2 text-xs text-slate-300">
            ...
            {telemetryInsights.last_smoke_test && (
              <div className="flex items-center justify-between">
                ...
                <span className="text-slate-100">
                  {telemetryInsights.last_smoke_test.status} (
                  {formatTimestamp(
                    telemetryInsights.last_smoke_test.completed_at ||
                      telemetryInsights.last_smoke_test.started_at,
                  )}
                  )
                </span>
              </div>
            ...
            {telemetryInsights.last_error && (
              <div className="rounded border border-amber-900/40 bg-amber-950/20 px-2 py-1 text-[11px] text-amber-200">
                Last error: {telemetryInsights.last_error.event} -{" "}
                {telemetryInsights.last_error.message || "No message"} (
            ...
            {telemetrySummary?.exists && (
              <Button
                ...
                onClick={() => {
                  if (
                    !window.confirm(
                      "Delete uploaded telemetry? This cannot be undone.",
                    )
                  ) {
                    return;
                  }
        ...
        {telemetrySummary?.exists && (
          <div className="grid gap-2 text-xs text-slate-300">
            <div className="flex items-center justify-between">
              <span>Events</span>
              <span className="text-slate-100">
                {telemetrySummary.event_count ?? 0}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span>File size</span>
              <span className="text-slate-100">
                {formatBytes(telemetrySummary.file_size_bytes)}
              </span>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {showTail
                  ? "Hide telemetry"
                  : `View last ${String(tailLimit)} events`}
              </Button>
              {showTail && (
                <>
                  <Button
          ...
          <div className="mt-2 max-h-72 space-y-2 overflow-auto rounded border border-slate-800 bg-slate-950/40 p-2">
            <div className="flex items-center justify-between text-[11px] text-slate-400">
              <span>
                ...
                {telemetryTail?.total_lines
                  ? ` (of ${String(telemetryTail.total_lines)})`
                  : ""}
              </span>
            ...
            {tailError && (
              <p className="text-xs text-red-400">
                {tailError instanceof Error
                  ? tailError.message
                  : "Failed to load telemetry entries"}
              </p>
            )}
            {!tailLoading &&
              telemetryTail?.entries &&
            ...
            {filteredEntries.length > 0 && (
              <ul className="space-y-2">
                {filteredEntries.map((entry, index) => (
                  <li
                    key={`${String(index)}-${entry.raw.slice(0, 16)}`}
            ...
            {telemetryPaths.map(({ os, path }) => (
              ...
              >
                ...
                <Button
                  ...
                  aria-label={`Copy ${os} telemetry path`}
                  onClick={() => {
