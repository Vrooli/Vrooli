...
function renderSection(
  overrides: Partial<ComponentProps<typeof ExternalServerSection>> = {},
) {
  const props = {
...
describe("ExternalServerSection", () => {
  it("edits a tunnel URL, chooses detected URLs, and requests a connectivity test", () => {
    ...
    fireEvent.change(screen.getByLabelText("Proxy URL"), {
      target: { value: "https://new.example/proxy/" },
    });
    expect(props.onProxyUrlChange).toHaveBeenCalledWith(
      "https://new.example/proxy/",
    );
    fireEvent.click(screen.getByRole("button", { name: /remote.example/ }));
    expect(props.onProxyUrlChange).toHaveBeenCalledWith(
      "https://remote.example/apps/canvas-lab/proxy/",
    );
  ...
  it("reports connection evidence, errors, pending state, and local CLI management", () => {
    ...
    expect(
      screen.getByText("Tunnel authentication failed"),
    ).toBeInTheDocument();
    ...
    fireEvent.change(screen.getByLabelText("vrooli CLI path"), {
      target: { value: "/usr/local/bin/vrooli" },
    });
    expect(props.onVrooliBinaryPathChange).toHaveBeenCalledWith(
