...
export function BundledRuntimeSection({
  ...
  // Get manifest content from pipeline store (centralized source of truth)
  const bundleResult = usePipelineStore((state) => state.bundleResult);
  const storeManifestContent = bundleResult?.manifest_content;
  ...
  useEffect(() => {
    ...
    // Fall back to fetching from file system for existing manifests not in store
    let isActive = true;
  ...
  const copyManifestJson = async () => {
    ...
    const result = await writeToClipboard(manifestJson);
    if (result.success) {
      setManifestCopyStatus("copied");
      setTimeout(() => setManifestCopyStatus("idle"), 1500);
    } else {
      console.warn("Failed to copy manifest JSON", result.error);
  ...
  const downloadManifestJson = () => {
    ...
    triggerBlobDownload(blob, manifestName);
  };
  ...
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-900/40 p-4 space-y-3">
      ...
      <DeploymentManagerBundleHelper
        ...
        initialBundleDetails={initialBundleResult?.bundleDetails}
