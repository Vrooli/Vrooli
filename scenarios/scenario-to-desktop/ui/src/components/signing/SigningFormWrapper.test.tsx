  ...
  it("keeps disabled signing forms explanatory and toggles configuration accessibly", () => {
    ...
    render(
      <SigningFormWrapper
        ...
        isConfigured={false}
        onToggle={onToggle}
        disabledMessage="Enable Windows signing to add a certificate."
        headerActions={<button type="button">Discover</button>}
        testId="windows-signing"
      >
        <p>Certificate fields</p>
      </SigningFormWrapper>,
    );
    expect(
      screen.getByText("Enable Windows signing to add a certificate."),
    ).toBeInTheDocument();
