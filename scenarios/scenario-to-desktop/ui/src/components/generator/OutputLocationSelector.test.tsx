...
describe("OutputLocationSelector", () => {
  it("presents durable, staging, and custom destinations and reports changes", () => {
    ...
    render(
      <OutputLocationSelector
        locationMode="proper"
        standardPath="/scenarios/canvas-lab/.desktop"
        stagingPreview="/tmp/canvas-lab"
        onChange={onChange}
      />,
    );
  ...
  it("edits an explicit custom output path", () => {
    ...
    fireEvent.change(screen.getByLabelText("Output Directory"), {
      target: { value: "/srv/desktop-apps" },
    });
    expect(onOutputPathChange).toHaveBeenCalledWith("/srv/desktop-apps");
    expect(
      screen.getByText(/Used only when choosing a custom location/),
    ).toBeInTheDocument();
  });
});
