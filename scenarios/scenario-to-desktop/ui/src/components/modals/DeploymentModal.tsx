...
const DEPLOYMENT_VISUALS: Record<
  DeploymentMode,
  { icon: typeof Server; gradient: string; status: DeploymentStatus }
> = {
  "external-server": {
    icon: Server,
...
const SERVER_TYPE_VISUALS: Record<
  ServerType,
  {
        ...
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div className="space-y-1">
            <CardTitle className="text-lg text-slate-100">
              Choose deployment + data source
            </CardTitle>
            <p className="text-sm text-slate-400">
              Pick how the desktop app connects, then decide where its UI and
              APIs will come from.
            </p>
          </div>
          <Button
        ...
        <CardContent className="space-y-6">
          <div className="flex items-start gap-3 rounded-lg border border-slate-800/80 bg-slate-950/60 p-3 text-sm text-slate-200">
            ...
            <div className="space-y-1">
              <p className="font-semibold text-slate-100">
                Start with the deployment intent.
              </p>
              <p className="text-slate-300">
                Thin client is the simplest path today. Bundled is for offline
                experiments and requires a manifest.
              </p>
            </div>
          </div>
          ...
          {showHelper && (
            <div className="rounded-lg border border-blue-800/70 bg-blue-950/30 p-4 space-y-3">
              ...
              <div className="text-sm text-slate-200 space-y-2">
                <p>
                  If your scenario is already running on a server, choose{" "}
                  <strong>Thin client</strong> + <strong>External</strong>.
                </p>
                <p>
                  If the app is UI-only (no API), choose{" "}
                  <strong>Thin client</strong> + <strong>Static files</strong>.
                </p>
                <p>
                  If you want everything inside the installer, choose{" "}
                  <strong>Bundled</strong> (experimental) and prepare a bundle
                  manifest.
                </p>
                <p>
                  <strong>Node</strong> and <strong>Executable</strong> are for
                  advanced local services and need extra setup.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {DEPLOYMENT_OPTIONS.map((option) => {
                ...
                const nextAllowed =
                  option.value === "bundled" || option.value === "cloud-api"
                    ? ["external"]
                ...
                return (
                  <button
                    ...
                    disabled={disabled}
                    onClick={() => {
                      onChange(option.value, nextAllowed[0] as ServerType);
                    }}
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-slate-200">
                Where should this desktop build get its data?
              </p>
              {deploymentMode === "bundled" && (
                <Badge
                  variant="outline"
                  className="text-xs text-amber-200 border-amber-700/60"
                >
              ...
              {SERVER_TYPE_OPTIONS.map((option) => {
                ...
                return (
                  <button
                    ...
                    disabled={!isAllowed}
                    onClick={() => {
                      onChange(deploymentMode, option.value);
                    }}
                    className={`rounded-lg border border-slate-800/80 bg-slate-950/60 p-4 text-left transition-all ${
                      isSelected
                        ? "shadow-[inset_0_0_0_2px_rgba(59,130,246,0.9)]"
