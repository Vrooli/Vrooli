...
export function BundleResultsCard({ result }: BundleResultsCardProps) {
  const runtimeBinaries = result.runtimeBinaries;
  const copiedArtifacts = result.copiedArtifacts;
    ...
    <div className="space-y-3">
      <div className="rounded-md border border-slate-800/70 bg-slate-950/70 p-3 space-y-2">
        {result.bundleDir && (
          <div className="flex items-center gap-2 text-[11px]">
            ...
            <span
              className="text-slate-400 truncate max-w-[300px]"
              title={result.bundleDir}
            >
              {result.bundleDir}
            </span>
          </div>
        ...
        {result.totalSizeHuman && (
          <div className="flex items-center gap-2 text-[11px]">
            ...
            <span className="text-slate-400">{result.totalSizeHuman}</span>
          </div>
        )}
      ...
      {result.sizeWarning && (
        <div className="flex items-start gap-2 text-[11px] text-amber-300 p-2 rounded-md border border-amber-800/40 bg-amber-950/20">
          ...
          <span>{result.sizeWarning.message}</span>
        </div>
      )}
          ...
          <div className="rounded-md border border-slate-800/70 bg-slate-950/70 p-2 space-y-1">
            {Object.entries(runtimeBinaries).map(([platform, binaryPath]) => (
              ...
              >
                ...
                <span
                  className="text-slate-500 truncate ml-auto max-w-[200px]"
                  title={binaryPath}
                >
