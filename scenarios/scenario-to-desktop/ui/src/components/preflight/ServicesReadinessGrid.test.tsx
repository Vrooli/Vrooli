...
describe("ServicesReadinessGrid", () => {
      ...
      <ServicesReadinessGrid
        readinessDetails={[
          [
            "api",
            {
              ...
              ready_at: "2026-07-27T12:00:05Z",
              updated_at: "2026-07-27T12:00:10Z",
              exit_code: 0,
            },
          ],
        ]}
        ...
        bundleManifest={{
          services: [
            {
              id: "api",
              health: { type: "http", path: "/health", port_name: "api" },
            },
          ],
        }}
        snapshotTs={Date.parse("2026-07-27T12:01:00Z")}
        tick={Date.parse("2026-07-27T12:01:00Z")}
      />,
    );
  ...
  it("distinguishes skipped and pending dependencies from a configured health probe", () => {
    renderWithProviders(
      <ServicesReadinessGrid
        readinessDetails={[
          ["cache", { ready: false, skipped: true, message: "not needed" }],
          [
            "worker",
            {
              ready: false,
        ...
        snapshotTs={Date.parse("2026-07-27T12:00:20Z")}
        tick={Date.parse("2026-07-27T12:00:20Z")}
      />,
    );
    expect(screen.getByText("Skipped")).toBeInTheDocument();
    expect(screen.getByText("Waiting")).toBeInTheDocument();
    expect(
      screen.getByText("Not launched yet; waiting on dependencies or secrets."),
    ).toBeInTheDocument();
    expect(
      screen.getAllByText(
