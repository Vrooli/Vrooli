  ...
  return (
    <div className="mt-2 grid gap-2 sm:grid-cols-2">
      {readinessDetails.map(([serviceId, status]) => {
        ...
        const statusAge =
          updatedAt && !isUpdatedAhead
            ? formatDuration(Math.max(0, referenceTs - updatedAt))
            : "";
        const startedAge = startedAt
          ? formatDuration(Math.max(0, referenceTs - startedAt))
          : "";
        const readyAge = readyAt
          ? formatDuration(Math.max(0, referenceTs - readyAt))
          : "";
        ...
        const healthPortInfo = healthPortName
          ...
          : null;
        const healthSupported =
          healthType === "http" &&
          Boolean(healthPath) &&
          Boolean(healthPortInfo);
        ...
        return (
          ...
          >
            ...
            <div className="text-[10px] text-slate-400 space-y-0.5">
              ...
              {readyAt && (
                <p>
                  Ready {new Date(readyAt).toLocaleTimeString()} ({readyAge}{" "}
                  ago)
                </p>
              )}
              {updatedAt && (
              ...
              {typeof status.exit_code === "number" && (
                <p>Exit code {status.exit_code}</p>
              )}
            </div>
            {listenURL && (
