...
interface ValidationIssuesPanelProps {
  validation: NonNullable<PreflightPresentation["validation"]>;
}
...
export function ValidationIssuesPanel({
  ...
  return (
    <details
      className="rounded-md border border-red-900/50 bg-red-950/20 p-3 text-[11px] text-red-200"
      open
    >
      ...
      <div className="mt-2 space-y-3 max-h-64 overflow-y-auto">
        {validation.errors.length > 0 && (
          <div className="space-y-2">
            {validation.errors.map((err, idx) => {
              ...
              return (
                <div
                  key={`${err.code}-${String(idx)}`}
                  className="rounded-md border border-red-900/50 bg-red-950/40 p-2 space-y-1"
                >
                    ...
                    <>
                      ...
                      <ul className="space-y-1 text-[11px] text-red-100">
                        {guidance.remediation.map((step, stepIdx) => (
                          <li key={`${err.code}-remedy-${String(stepIdx)}`}>
                            • {step}
                          </li>
                        ))}
                      </ul>
          ...
          <div className="space-y-1 text-[11px] text-red-100">
            ...
            <ul className="space-y-1">
              {validation.missing_binaries.map((item, idx) => (
                <li key={`${item.service_id}-${item.path}-${String(idx)}`}>
                  {item.service_id}: {item.path} ({item.platform})
                </li>
              ))}
            </ul>
            <p className="text-red-200/80">
              Rebuild the service binaries and re-export the bundle to update
              manifest paths.
            </p>
          </div>
        )}
        {validation.missing_assets.length > 0 && (
          <div className="space-y-1 text-[11px] text-red-100">
            ...
            <p className="text-red-200/80">
              Rebuild UI/assets and re-export the bundle so the staged paths
              exist.
            </p>
          </div>
        )}
        {validation.invalid_checksums.length > 0 && (
          <div className="space-y-1 text-[11px] text-red-100">
    ...
    <details className="rounded-md border border-amber-800/50 bg-amber-950/20 p-3 text-[11px] text-amber-200">
      ...
      <ul className="mt-2 space-y-1">
        {warnings.map((warn, idx) => (
          <li key={`${warn.code}-${String(idx)}`}>{warn.message}</li>
        ))}
      </ul>
    </details>
  );
