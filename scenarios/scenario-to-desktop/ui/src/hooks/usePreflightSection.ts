...
import type { PipelineConfig, BundlePreflightSecret } from "../lib/api";
...
import { buildJobStepMap, resolveAllStepStatuses } from "../controllers/preflightController";
...
export interface UsePreflightSectionReturn {
  ...
  validation: NonNullable<ReturnType<typeof usePipelineStore.getState>["preflightResult"]>["validation"] | undefined;
  readiness: NonNullable<ReturnType<typeof usePipelineStore.getState>["preflightResult"]>["ready"] | undefined;
  ports: NonNullable<ReturnType<typeof usePipelineStore.getState>["preflightResult"]>["ports"] | undefined;
  telemetry: NonNullable<ReturnType<typeof usePipelineStore.getState>["preflightResult"]>["telemetry"] | undefined;
  runtimeInfo: NonNullable<ReturnType<typeof usePipelineStore.getState>["preflightResult"]>["runtime"] | undefined;
  ...
  hasRun: boolean;
  readinessDetails: Array<[string, unknown]>;
  snapshotTs: number;
  tick: number;
...
export function usePreflightSection(props: UsePreflightSectionProps): UsePreflightSectionReturn {
  ...
  const setPreflightSecretAction = usePipelineStore((s) => s.setPreflightSecret);
  const setPreflightOverrideAction = usePipelineStore((s) => s.setPreflightOverride);
  ...
  const latestServiceUpdate = readinessDetails.reduce((latest, [, status]) => {
    const ts = parseTimestamp((status as { updated_at?: string }).updated_at);
  ...
  const diagnosticsAvailable = Boolean(portSummary || telemetry?.path || (logTails && logTails.length > 0));
  const hasRun = Boolean(preflightResult || preflightError || pipelineStatus);

  // Filter checks by step
  const validationChecks = checks.filter((check: { step?: string }) => check.step === "validation");
  const secretChecks = checks.filter((check: { step?: string }) => check.step === "secrets");
  const runtimeChecks = checks.filter((check: { step?: string }) => check.step === "runtime");
  const serviceChecks = checks.filter((check: { step?: string }) => check.step === "services");
  const diagnosticsChecks = checks.filter((check: { step?: string }) => check.step === "diagnostics");
  ...
  const runPreflight = useCallback(
    ...
    [scenarioName, bundleManifestPath, isBundled, preflightSecrets, runPreflightStage, setPreflightOverrideAction]
  );
