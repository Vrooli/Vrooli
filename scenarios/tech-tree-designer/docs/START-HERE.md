...
`vrooli scenario orient tech-tree-designer`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient tech-tree-designer
...
```bash
prd-control-tower prd generate tech-tree-designer \
...
```bash
prd-control-tower prd validate tech-tree-designer --json
```
...
```bash
prd-control-tower requirements generate tech-tree-designer \
...
```bash
prd-control-tower requirements validate tech-tree-designer --json
```
