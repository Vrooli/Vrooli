...
`template-manager orient tech-tree-designer`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient tech-tree-designer
...
and the `notes` example — mixes binding guidance with illustrative examples. Tokens, motion,
...
```bash
cat > /tmp/prd_context_tech-tree-designer.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) tech-tree-designer \
  --context-file /tmp/prd_context_tech-tree-designer.md \
  --publish \
  --json
```
...
```bash
cat > /tmp/requirements_context_tech-tree-designer.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) tech-tree-designer \
  --context-file /tmp/requirements_context_tech-tree-designer.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/tech-tree-designer/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest tech-tree-designer <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> tech-tree-designer
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario tech-tree-designer
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> tech-tree-designer <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/tech-tree-designer/v1/<domain>/`.
...
1. Add proto messages and a service under
   `packages/proto/schemas/tech-tree-designer/v1/<domain>/`, then run
