# Configuration — Tech Tree Designer
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/tech-tree-designer.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `tech-tree-designer` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/tech-tree-designer/config.json`
3. `~/.vrooli/config/tech-tree-designer/config.json`
4. `~/.config/vrooli/tech-tree-designer/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port tech-tree-designer API_PORT`)
