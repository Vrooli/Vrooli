# Troubleshooting — Tech Tree Designer
...
```bash
vrooli scenario status tech-tree-designer
make stop
vrooli scenario start tech-tree-designer --clean-stale
```
...
```bash
make status
vrooli scenario port tech-tree-designer UI_PORT
```
...
tech-tree-designer status --auto-start

# Or override the API base for this invocation
tech-tree-designer status --api-base "http://localhost:$(vrooli scenario port tech-tree-designer API_PORT)/api/v1"
```
...
`vite build` processes a large production bundle. This
...
SQLite single-writer behaviour. If concurrent processes hold the file,
...
The generated package paths follow `packages/proto/gen/go/tech-tree-designer/v1/<domain>/`.
