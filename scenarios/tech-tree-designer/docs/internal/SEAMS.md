...
| GraphSource | `api/internal/graph.GraphSource.Graph(ctx, SourceRequest)` returning `TechTreeGraph` | `SDASource` backed by `scenario-dependency-analyzer` `DescribeInterfaceGraph`; `graph.Service` consumes it for Describe/Neighborhood/Path/Ancestors/Export | fake `SDAInterfaceGraphClient` fixtures and fake `GraphSource` service tests | Keeps graph logic independent of upstream provider shape while consuming the canonical fleet interface-graph contract. |
