...
// SectorService encapsulates business logic for sector operations.
...
// FetchSectorsWithStages retrieves all sectors for a tree.
// Note: This method fetches stages via the stageService, so it requires stageService to be initialized.
func (s *SectorService) FetchSectorsWithStages(ctx context.Context, treeID string, stageService *StageService) ([]Sector, error) {
	if s.db == nil {
		return nil, errors.New("database connection is not initialized")
	}
	...
	for rows.Next() {
		...
		// Optionally load stages if stageService is provided
		if stageService != nil {
			stages, stageErr := stageService.GetStagesForSector(ctx, sector.ID)
			if stageErr == nil {
