...
```bash
vrooli scenario status signal-inbox
make stop
vrooli scenario start signal-inbox --clean-stale
```
...
signal-inbox status --auto-start

# Or override the API base for this invocation
signal-inbox status --api-base "http://localhost:$(vrooli scenario port signal-inbox API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/signal-inbox/`.
...
The default route is `${SCENARIO_DATA_DIR}/signal-inbox.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/signal-inbox.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/signal-inbox.db}")"
```
...
`packages/proto/gen/go/signal-inbox/v1/`, TypeScript under
`packages/proto/gen/typescript/js/signal-inbox/v1/`, and Python under
`packages/proto/gen/python/signal_inbox/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/signal-inbox/v1/<domain>/`.
