...
envelope (`packages/proto/schemas/signal-inbox/v1/shared/errors.proto`):
...
| **CLI** | `signal-inbox status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/signal-inbox/v1/shared/health.proto`
...
`POST /vrooli.signal_inbox.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.signal_inbox.v1.notes.NotesService/ListNotes`
...
| **CLI** | `signal-inbox notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.signal_inbox.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.signal_inbox.v1.notes.NotesService/CreateNote`
...
| **CLI** | `signal-inbox notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.signal_inbox.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.signal_inbox.v1.notes.NotesService/GetNote`
...
| **CLI** | `signal-inbox notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.signal_inbox.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `signal-inbox notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/signal-inbox/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/signal-inbox/v1/<domain>/`, then run
   `make generate`.
