...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/signal-inbox.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `signal-inbox` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/signal-inbox/config.json`
3. `~/.vrooli/config/signal-inbox/config.json`
4. `~/.config/vrooli/signal-inbox/config.json`
...
```bash
signal-inbox configure api_base http://localhost:15001/api/v1
signal-inbox configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port signal-inbox API_PORT`)
