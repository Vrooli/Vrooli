...
`~/.vrooli/bin/signal-inbox`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start signal-inbox` if the API is unreachable |
...
```bash
signal-inbox status
signal-inbox status --json
```

### `signal-inbox configure <key> <value>`
...
```bash
signal-inbox configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`signal-inbox <domain> <verb>`). Every command calls a single API
...
```bash
signal-inbox notes list
signal-inbox notes list --json
```

#### `signal-inbox notes create --title <title> [--body <body>]`
...
```bash
signal-inbox notes create --title "First note" --body "Hello world"
```
...
```bash
signal-inbox notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start signal-inbox`" is good.
