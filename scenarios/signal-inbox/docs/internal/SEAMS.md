...
`packages/proto/schemas/signal-inbox/v1/<domain>/<file>.proto` —
...
The `health` proto in `packages/proto/schemas/signal-inbox/v1/shared/`
...
| | |
|---|---|
| **Seam** | Immutable signal-journal persistence |
| **Interface** | `internal/signals/repository.go::Repository` (`Append`, `Get`, `List`) — deliberately no update or delete operation. |
| **Production wiring** | `handlers/signals/module.go` wires `signals.NewSQLiteRepository` into `signals.NewService`. |
| **Test fake** | Repository integration tests use a fresh SQLite database with the production signals schema; future handler tests may add a domain-local fake without expanding the production interface. |
| **Why it exists** | Append-only storage is an invariant, not a convention. The narrow interface prevents later domains from adding a convenient mutation path to the journal. |

### Signals image BlobStore

| | |
|---|---|
| **Seam** | Opaque image-byte storage before journal capture |
| **Interface** | `api-core/blobstore::BlobStore` (`Put`, `Get`, `Delete`) |
| **Production wiring** | `handlers/signals/module.go::defaultBlobStore` resolves scenario-local filesystem storage; `ModuleWithBlobStore` accepts an injected implementation. |
| **Test fake** | `blobstore.NewMemoryBlobStore()` in `handlers/signals/image_upload_test.go`. |
| **Why it exists** | Multipart image bytes must not enter Connect payloads. The upload returns only a typed payload reference; the capture request then appends an image signal. Content-addressed keys make identical bytes deduplicate regardless of filename. |

### Enrichment extraction and projection

| | |
|---|---|
| **Seam** | Post-capture extraction plus immutable read projection |
| **Interface** | `internal/signals/types.go::PostCapture` and `ReadProjection`; `internal/enrichment/types.go::Extractor` and `Repository` |
| **Production wiring** | `handlers/signals/module.go` constructs `enrichment.Service` once with the bounded HTML client and the `image-tools analyze ocr` delegate, then passes it into `signals.NewService`. |
| **Test fake** | `internal/enrichment/service_test.go` supplies an `HTTPDoer` function and `CommandRunner` function; SQLite uses the real append-only sidecar schema. |
| **Why it exists** | Extraction happens after capture and must never block it. The sidecar projects readable text or `needs_attention` over a signal read, so no derived result issues an UPDATE against the journal row (D-006, D-019, D-027). |

### Inference client

| | |
|---|---|
| **Seam** | Gateway-routed embeddings and classification |
| **Interface** | `internal/inference/client.go::Client` (`Embed`, `Classify`) and its `Embedder` adapter for `ai-go/search`. |
| **Production wiring** | The composition root resolves `ai-gateway` through scenario discovery and constructs `inference.NewGatewayClient(...)` once before passing it to domains that need classification or retrieval. |
| **Test fake** | `internal/testutil/mocks::FakeInference` supplies deterministic vectors, classifications, errors, and call counts. |
| **Why it exists** | The gateway is the policy boundary for provider choice and failure handling. This seam prevents a later domain from silently shelling out to a provider command, while preserving distinct document and query embedding instructions needed for retrieval quality. |
