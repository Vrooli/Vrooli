...
| X archive format is unverified | Platform formats are owned by the platforms and drift without notice. Chrome bookmark HTML was measured on 2026-07-27 and a Reddit GDPR ZIP was measured and selectively imported on 2026-07-28. The X archive is still unavailable, so its fields must not be inferred. | `SIG-P0-008` | Provide one real X archive. Record only its ZIP layout and URL/timestamp/identity field names, then validate the parser before treating the multi-format import requirement as done. Listed as a `manual` validation on the requirement. |
