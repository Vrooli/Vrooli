...
```bash
vrooli scenario port signal-inbox UI_PORT
```
...
```bash
signal-inbox status
signal-inbox <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port signal-inbox API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.signal_inbox.v1.<domain>.<Service>/<Method>
```
...
```bash
signal-inbox notes list
signal-inbox notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port signal-inbox API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.signal_inbox.v1.notes.NotesService/ListNotes" \
