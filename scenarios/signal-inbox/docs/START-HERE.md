...
`template-manager orient signal-inbox`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient signal-inbox
...
```bash
cat > /tmp/prd_context_signal-inbox.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start signal-inbox --interactive
# …answer the interview, then:
business-health wizard preview signal-inbox
business-health wizard apply signal-inbox
```
...
```bash
vrooli scenario requirements validate signal-inbox --json
```
...
```bash
cat > /tmp/requirements_context_signal-inbox.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate signal-inbox --json
business-health fix preview signal-inbox
business-health fix apply signal-inbox
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate signal-inbox --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate signal-inbox`, the notes page spec
...
validate signal-inbox --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/signal-inbox/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest signal-inbox <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> signal-inbox
