...
|---|---|---|---|---|---|
| Capture | signals → enrichment → categories | Operator submits a URL, text, or image | A journal entry with extracted content and a proposed category | Deduplicates by content hash; extraction and classification each degrade independently without failing the capture | `SIG-P0-001`, `SIG-P0-002`, `SIG-P0-003`, `SIG-P0-005` |
| Import run | sources → signals | Operator supplies a platform export | Many journal entries, idempotent on re-run | Per-run counts; a shape-validation failure fails the whole run rather than importing partial garbage | `SIG-P0-008` |
| Triage | triage | Operator opens the review queue | Confirmed category, disposition advanced, optional annotation | One signal at a time; every action is a separate durable write, so an abandoned session loses nothing | `SIG-P0-007`, `SIG-P0-013` |
| Route | triage → prompt-manager | Operator routes a triaged signal | A team knowledge entry plus an outcome-link annotation | Fails closed: if the knowledge write fails, no outcome link is recorded and the signal stays `triaged`, so the routing is retryable and never half-done | `SIG-P1-002` |
| Ambient read | retrieval | An agent requests the ambient view | A budgeted set of unresolved signals | Read-only; consumes no state and changes no disposition | `SIG-P0-012` |
| Adapter run | sources | Schedule or manual sync on an enabled adapter | New signals, or an auto-disabled adapter | **Auto-disable on anomaly, never retry.** Disabled state persists across restart until an operator re-enables | `SIG-P0-014` |
