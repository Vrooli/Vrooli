...
// the template itself does not build because of signal-inbox placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_signal_inbox_v1_notes_notes_proto, "NotesService")
}
