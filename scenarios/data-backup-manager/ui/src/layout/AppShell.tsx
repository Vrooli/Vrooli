...
const NAV_ICONS = {
  overview: <LayoutDashboard aria-hidden className="h-5 w-5" />,
  targets: <Archive aria-hidden className="h-5 w-5" />,
  destinations: <HardDrive aria-hidden className="h-5 w-5" />,
  plans: <CalendarClock aria-hidden className="h-5 w-5" />,
  runs: <History aria-hidden className="h-5 w-5" />,
  restores: <RotateCcw aria-hidden className="h-5 w-5" />,
  drills: <ShieldCheck aria-hidden className="h-5 w-5" />,
  settings: <Settings aria-hidden className="h-5 w-5" />,
} as const;

/** Library-owned shell adapter for the Data Backup Manager console. */
export function AppShell() {
  ...
  return (
    <>
      <a className="data-backup-manager-skip-link" href={`#${selectors.layout.shell}-main`}>
        {t(strings.layout.mainLabel)}
      </a>
      <LibraryAppShell
        density="sidebar"
        mobileNav="tabs"
        ...
        sidebarStorageKey="data-backup-manager.sidebar-width"
