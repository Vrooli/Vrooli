...
| 2026-05-26 | Encryption ON by default for every destination; secrets only from `vault`. | Backups frequently contain secrets-bearing source data, and a leaked unencrypted repository is a full breach. | Every destination is an encrypted kopia repository. Repo passphrases and backend access keys come from the `vault` resource at runtime, never from config files or process arguments. See `SECURITY.md`. | Revisit only with a vetted reason to allow an unencrypted destination (none expected). |
