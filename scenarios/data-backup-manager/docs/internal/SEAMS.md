...
| **Why it exists** | Backup/restore/verify must be substitutable so domain tests stay hermetic — they assert "the run snapshotted into the right repo and applied retention", not real kopia behavior. Encryption-on-by-default and credential-authority-owned repository passphrases mean no secret ever crosses this interface; the production impl never puts a passphrase in argv. Real-engine behavior is covered by integration tests gated behind `KOPIA_INTEGRATION`. |
...
| **Why it exists** | A detached external drive must explain itself: the operator-facing bundle root carries human-readable README/RECOVERY plus a non-secret JSON manifest, while the vanilla kopia repository stays nested under `repositories/<slug>.kopia` (never wrapped or made DBM-proprietary). The seam keeps that filesystem materialization out of the service's decision logic and out of test paths. The writer is idempotent on identical content and fails closed on conflicting pre-existing files; it never writes a secret value (only a credential identity/field reference and import command). |

### Executor (background run execution)

| | |
|---|---|
| **Seam** | The boundary that decouples a run's execution from the request that triggered it. `TriggerRun` creates the run row and enqueues a `RunJob`; the executor runs it to a terminal state on a worker bound to a server-lifetime context, so a client/proxy disconnect cannot cancel an in-flight backup. |
| **Interface** | `internal/runs/executor.go::Executor` (`Bind(baseCtx, RunFunc)`, `Submit(RunJob)`, `Shutdown(ctx)`). The service binds it to `executeRun`. |
| **Production wiring** | `runs.NewAsyncExecutor(workers)` — a bounded worker pool over a buffered queue, bound in `main.go` to `execCtx` (cancelled on graceful shutdown; drained in the server `Cleanup`). |
| **Test fake** | `internal/runs/mocks::SyncExecutor` — runs each submitted job inline to completion during `Submit`, so a test can `TriggerRun` then immediately `GetRun` the terminal run without polling. The async-contract test (`async_test.go`) uses the real `AsyncExecutor` with channel-gated capture/snapshot hooks to assert the persisted `capturing`→`snapshotting`→`completed` transitions mid-flight. |
| **Why it exists** | The synchronous, request-bound execution it replaces stranded runs in `pending` whenever a connection dropped mid-snapshot. There is no synchronous production path — the async contract is the only contract; startup `Reconcile` closes any run left non-terminal by a crash/restart. |
...
| **Production wiring** | `discovery.NewWellKnownScanner()` reads the runtime-home contract scoped to the resolved `~/.vrooli` root: `plans`, `state`, `config`, `data`, encrypted `secrets.enc.json` and `runtime.db`; legacy plaintext `secrets.json` is deliberately excluded. Strictly read-only — stats paths and bounded-walks directory sizes; never reads file contents. Resource, scenario, tool, and safeguard storage suggestions come from `api-core/storage.LoadOwnerInventory`. |
