...
|---|---|---|---|
| Local Vrooli stack | active | Vrooli lifecycle, Go, Node/pnpm, SQLite path, `kopia` resource, and credential authority | Validate a real destination, scheduled run, and restore verification before treating an install as protected. Companion `kopia` resource must be ready (`plan-manager plans get kopia-resource-implementation-validation-plan`). |
