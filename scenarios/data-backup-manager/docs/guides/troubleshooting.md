...
make stop
# Or, as last resort:
pkill -f 'data-backup-manager-api'
...
```bash
export SQLITE_PATH=/tmp/data-backup-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/data-backup-manager.db}")"
```
...
`packages/proto/gen/typescript/js/data-backup-manager/v1/`, and Python under
