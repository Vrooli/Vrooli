# Troubleshooting — Data Backup Manager
...
```bash
vrooli scenario status data-backup-manager
make stop
vrooli scenario start data-backup-manager --clean-stale
```
...
data-backup-manager status --auto-start

# Or override the API base for this invocation
data-backup-manager status --api-base "http://localhost:$(vrooli scenario port data-backup-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/data-backup-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/data-backup-manager.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/data-backup-manager.db"
```
...
`packages/proto/gen/go/data-backup-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/data-backup-manager/v1/`, and Python under
`packages/proto/gen/python/data_backup_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/data-backup-manager/v1/<domain>/`.
