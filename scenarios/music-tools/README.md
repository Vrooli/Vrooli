...
- The former generated example domain — removed when composition became the
  first scenario-owned vertical slice.
...
`api/internal/module/module.go`. Audio retrieval is the current REST exception
because bytes do not ride proto.
...
`test-genie execute music-tools --preset comprehensive` directly for
