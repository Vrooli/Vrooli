...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/music-tools/v1/shared/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   `packages/proto/schemas/music-tools/v1/<domain>/<file>.proto`.
   Tests import the generated Go type directly
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/music-tools/v1/shared"`)
...
import (
    ...
    "music-tools/handlers/health"
    "music-tools/internal/clock"
    "music-tools/internal/module"
    "music-tools/internal/server"
    "music-tools/internal/testutil/assertx"
    "music-tools/internal/testutil/httpx"
    "music-tools/internal/testutil/mocks"
)
...
The proto schema in `packages/proto/schemas/music-tools/v1/shared/health.proto`
...
|---|---|---|
| Wire contract | `packages/proto/schemas/music-tools/v1/notes/notes.proto` | `Note`, `service NotesService`, `ListNotesResponse`, `CreateNoteRequest`, `CreateNoteResponse`, `GetNoteRequest`, `GetNoteResponse` |
| REST metadata contract | `packages/proto/schemas/music-tools/v1/notes/attachments.proto` | `Attachment` and `UploadAttachmentResponse` for the multipart upload exception |
...
| REST error envelope | `packages/proto/schemas/music-tools/v1/shared/errors.proto` + `internal/httpx/errors.go::WriteError` | Typed body for REST exceptions, with canonical codes (`invalid_request`, `not_found`, `internal`) |
...
dynamically from `go.mod`, so it works after `music-tools`
substitution.
...
import (
    ...
    clitest "music-tools/cli/internal/testutil"
)
...
under `packages/proto/schemas/music-tools/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/music-tools/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.music_tools.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/music-tools/v1/<domain>;<domain>_v1";
   ```
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/music-tools/v1/<domain>"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/music-tools/v1/<domain>/<domain>_pb";
