...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/music-tools.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `music-tools` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/music-tools/config.json`
3. `~/.vrooli/config/music-tools/config.json`
4. `~/.config/vrooli/music-tools/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port music-tools API_PORT`)
