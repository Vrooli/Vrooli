...
`template-manager orient music-tools`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient music-tools
...
```bash
cat > /tmp/prd_context_music-tools.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start music-tools --interactive
# …answer the interview, then:
business-health wizard preview music-tools
business-health wizard apply music-tools
```
...
```bash
vrooli scenario requirements validate music-tools --json
```
...
```bash
cat > /tmp/requirements_context_music-tools.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate music-tools --json
business-health fix preview music-tools
business-health fix apply music-tools
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate music-tools --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate music-tools`, the notes page spec
...
validate music-tools --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/music-tools/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest music-tools <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> music-tools
...
- Wire contracts belong in
  `packages/proto/schemas/music-tools/v1/<domain>/`.
