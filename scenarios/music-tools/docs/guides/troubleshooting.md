...
```bash
export SQLITE_PATH=/tmp/music-tools.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/music-tools.db}")"
```
...
`packages/proto/gen/typescript/js/music-tools/v1/`, and Python under
