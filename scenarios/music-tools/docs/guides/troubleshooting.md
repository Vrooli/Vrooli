...
```bash
vrooli scenario status music-tools
make stop
vrooli scenario start music-tools --clean-stale
```
...
```bash
make status
vrooli scenario port music-tools UI_PORT
```
...
music-tools status --auto-start

# Or override the API base for this invocation
music-tools status --api-base "http://localhost:$(vrooli scenario port music-tools API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/music-tools/`.
...
The default route is `${SCENARIO_DATA_DIR}/music-tools.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/music-tools.db"
```
...
`packages/proto/gen/go/music-tools/v1/`, TypeScript under
`packages/proto/gen/typescript/music-tools/v1/`, and Python under
`packages/proto/gen/python/music_tools/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/music-tools/v1/<domain>/`.
