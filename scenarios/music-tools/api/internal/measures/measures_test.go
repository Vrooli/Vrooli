...
func TestFallbackCounted(t *testing.T) {
	ctx := context.Background()
	r := newRecorder(t)
	_ = r.Record(ctx, Sample{Operation: "text_to_audio", State: "succeeded", DurationMS: 9000, Tier: "local-cpu", FallbackUsed: true})
	_ = r.Record(ctx, Sample{Operation: "text_to_audio", State: "succeeded", DurationMS: 800, Tier: "local-gpu"})

	stats, err := r.Stats(ctx, "text_to_audio")
	if err != nil {
		t.Fatalf("stats: %v", err)
	}
