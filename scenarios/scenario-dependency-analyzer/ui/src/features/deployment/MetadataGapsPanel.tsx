...
function GapItem({ gap, onFix }: { gap: ScenarioGapInfo; onFix?: (scenarioName: string, gapType: string) => void }) {
  ...
  return (
    <div className={`space-y-3 rounded-lg border p-4 ${statusTone("warning").panel}`}>
      ...
      {!gap.has_deployment_block && (
        <div className={`space-y-2 rounded border p-3 ${statusTone("danger").panel}`}>
          <div className="flex items-center gap-2 text-sm">
            <AlertTriangle className={`h-4 w-4 ${statusTone("danger").text}`} />
            <span className="font-medium">No deployment block</span>
          </div>
          ...
          <Button
            size="sm"
            variant="outline"
            className={`h-7 text-xs ${statusTone("danger").badge} hover:bg-destructive/20`}
      ...
      {gap.missing_dependency_catalog && (
        <div className={`space-y-2 rounded border p-3 ${statusTone("warning").panel}`}>
          <div className="flex items-center gap-2 text-sm">
            <Package className={`h-4 w-4 ${statusTone("warning").text}`} />
            <span className="font-medium">Missing dependency catalog</span>
          </div>
          <p className="text-xs opacity-80">
          ...
          <Button
            size="sm"
            variant="outline"
            className={`h-7 text-xs ${statusTone("warning").badge} hover:bg-warning/20`}
      ...
      {gap.missing_tier_definitions && gap.missing_tier_definitions.length > 0 && (
        <div className={`space-y-2 rounded border p-3 ${statusTone("warning").panel}`}>
          <div className="flex items-center gap-2 text-sm">
            <Layers className={`h-4 w-4 ${statusTone("warning").text}`} />
            <span className="font-medium">Missing tier definitions</span>
          </div>
          <div className="flex flex-wrap gap-1">
            {gap.missing_tier_definitions.map((tier) => (
              <Badge key={tier} variant="outline" className={`${statusTone("warning").badge} text-[10px]`}>
                {tier}
              </Badge>
            ))}
          </div>
        ...
        <div className={`space-y-2 rounded border p-3 ${statusTone("warning").panel}`}>
          ...
          <div className="flex flex-wrap gap-1">
            {gap.missing_resource_metadata.slice(0, 5).map((resource) => (
              <Badge key={resource} variant="outline" className={`${statusTone("warning").badge} text-[10px]`}>
                {resource}
              </Badge>
            ))}
            {gap.missing_resource_metadata.length > 5 && (
              <Badge variant="outline" className={`${statusTone("warning").badge} text-[10px]`}>
      ...
      {gap.missing_scenario_metadata && gap.missing_scenario_metadata.length > 0 && (
        <div className={`space-y-2 rounded border p-3 ${statusTone("warning").panel}`}>
          <div className="flex items-center gap-2 text-sm">
            <FileText className={`h-4 w-4 ${statusTone("warning").text}`} />
            <span className="font-medium">
          ...
          <div className="flex flex-wrap gap-1">
            {gap.missing_scenario_metadata.slice(0, 5).map((scenario) => (
              <Badge key={scenario} variant="outline" className={`${statusTone("warning").badge} text-[10px]`}>
                {scenario}
              </Badge>
            ))}
...
export function MetadataGapsPanel({ gaps, onFixGap }: MetadataGapsPanelProps) {
    ...
    return (
      <Card className="border border-border/40 bg-background/40">
        ...
        <CardContent className="flex items-center gap-2 text-sm text-muted-foreground">
          <CheckCircle2 className={`h-4 w-4 ${statusTone("success").text}`} />
  ...
  return (
    <Card className={`border bg-background/40 ${statusTone("warning").metric}`}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2 text-sm">
              <AlertTriangle className={`h-4 w-4 ${statusTone("warning").text}`} />
      ...
      <CardContent className="space-y-4">
        ...
        {gaps.recommendations && gaps.recommendations.length > 0 && (
          <div className={`rounded-lg border p-3 ${statusTone("warning").panel}`}>
            <p className="mb-2 text-xs font-medium uppercase tracking-wide">Recommendations</p>
