...
// validation call so ValidateDependencyHealth can open profiling stages
// without changing its signature (and the handler can thread the collector
// via context). When no collector is present, every stage call is a nil no-op.
