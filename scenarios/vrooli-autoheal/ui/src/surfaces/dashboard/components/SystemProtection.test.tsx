...
const mockWatchdogStatusPartial: api.WatchdogStatus = createWatchdogStatus({
  watchdogType: '',
...
const mockWatchdogStatusNone: api.WatchdogStatus = createWatchdogStatus({
  loopRunning: false,
  watchdogType: '',
  watchdogInstalled: false,
  watchdogEnabled: false,
...
describe('SystemProtection - compact mode', () => {
  ...
  it('shows green indicator for full protection', async () => {
    ...
    await waitFor(() => {
      ...
      expect(compactElement).toHaveClass('bg-accent-success/20');
    });
  });
  ...
  it('shows amber indicator for partial protection', async () => {
    ...
    await waitFor(() => {
      const compactElement = container.querySelector('[data-testid="autoheal-system-protection-compact"]');
      expect(compactElement).toHaveClass('bg-accent-warning/20');
    });
  });
