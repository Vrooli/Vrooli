...
import { fetchUptimeStats, fetchTimeline, HealthStatus, normalizeHealthStatus } from "../../../lib/api";
...
function StatusSparkline({ statuses }: { statuses: HealthStatus[] }) {
  ...
  if (displayStatuses.length === 0) {
    return <span className="text-xs text-text-muted/80">No data</span>;
  }
...
export function UptimeStats({ onShowTrends }: UptimeStatsProps) {
  ...
  const recentStatuses: HealthStatus[] = (timelineData?.events ?? [])
    .slice(0, 20)
    .map((e) => normalizeHealthStatus(e.status, "ok"));
    ...
    return (
      <Panel>
        <PanelHeader className="justify-start">
          ...
          <PanelTitle>Uptime (24h)</PanelTitle>
        </PanelHeader>
        <p className="text-sm text-text-muted/80">Loading...</p>
      </Panel>
    );
  }
    ...
    >
      ...
      <PanelHeader>
        <div className="flex items-center gap-2">
          <TrendingUp size={18} className="text-accent-primary" />
          <PanelTitle>Uptime ({data.windowHours}h)</PanelTitle>
        </div>
        {isClickable && (
          <ChevronRight size={16} className="text-text-muted/80" />
        )}
      </PanelHeader>
      ...
      <div className="mt-3 flex items-center justify-between border-t border-border-default/60 pt-3 text-xs text-text-muted/80">
        <div className="flex items-center gap-1.5">
          <Clock size={12} />
          <span>{data.totalEvents} checks in {data.windowHours}h</span>
        </div>
        {isClickable && (
          <span className="text-accent-primary">View trends →</span>
        )}
      </div>
    </Panel>
  );
