...
export function DocsPage() {
  ...
  if (manifestError) {
    return (
      <div className="space-y-6">
        <section className="rounded-xl border border-border-default/70 bg-surface-elevated/40 p-6">
          <p className="text-xs uppercase tracking-[0.25em] text-text-muted">Documentation</p>
          <h1 className="mt-2 text-2xl font-semibold">Autoheal Docs</h1>
          <p className="mt-4 text-text-primary">
            Documentation is available in the <code className="rounded bg-surface-overlay/70 px-1.5 py-0.5 text-accent-primary">scenarios/vrooli-autoheal/docs/</code> directory.
          </p>
          <p className="mt-4 text-sm text-text-muted">
            The docs API is not currently available. Ensure the autoheal scenario is running with the latest API.
          </p>
          <div className="mt-6">
            <p className="mb-2 text-sm text-text-primary">Available documents:</p>
  ...
  return (
    <div className="space-y-6" data-testid="docs-page">
      {/* Header */}
      <section className="rounded-xl border border-border-default/70 bg-gradient-to-r from-accent-primary/15 via-transparent to-surface-overlay/50 p-6">
        <p className="text-xs uppercase tracking-[0.25em] text-text-muted">Documentation</p>
        <h1 className="mt-2 text-2xl font-semibold">Autoheal Docs</h1>
