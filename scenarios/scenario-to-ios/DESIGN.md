...
### Why this design language fits Scenario to iOS

This is an operator's instrument, not a consumer product. The person using
it is deciding whether something may ship, and the primary surface — the
target matrix — is scanned rather than read. The Operational Console's
density, tabular emphasis, and information-first posture suit that.

Three adaptations matter for this scenario specifically:

- **`unsupported` is permanent here, and must look it.** Every other ramp's
  matrix eventually goes green. This one has rows that never will: a Linux
  host cannot build for iOS, and no amount of work changes that. The
  status-colour roles are the binding contract, but `unsupported` must be
  visually distinct from `unavailable` in shape as well as hue — one is a
  fact about the world, the other is a task with a next action. Collapsing
  them visually would undo the honesty the whole scenario is built on.
  Colour never carries disposition alone.

- **Apple's path is the most opaque part of shipping, so the readiness
  ladder is a product surface rather than documentation.** It should read
  as a walkthrough: each rung's state, its cost, whether the ramp can
  complete it, and what the owner must do personally. Enrollment, identity
  verification, and payment are irreducibly human, and the surface should
  say so plainly rather than implying automation that cannot exist.

- **Journey video is evidence, not interface.** Every verdict, chapter
  assertion, strategy attribution, and readiness rung must be fully
  perceivable and operable without playing a recording — full keyboard
  operation, visible focus, semantic roles and names, stable
  `data-testid` selectors. This matters more here than elsewhere because
  one strategy produces pixel-grade evidence, and a reviewer must be able
  to tell that apart from a semantic assertion *in text*.

The token contract is otherwise adopted unchanged. Generated iOS
applications carry the target scenario's branding, not this ramp's.
