...
```bash
Run the registry build through the CLI before capturing a workflow: `cli[future]:test-genie registry build`.
```
...
performance-health audit run "scenario-to-ios" --workflow "perf-example-scroll"
# 3. Analyze the returned trace, then optionally set a per-flow budget:
performance-health analysis analyze "scenario-to-ios" --trace "$TRACE_KEY"
performance-health budget set "scenario-to-ios" --flow "perf-example-scroll" --lcp-max-ms 2500 --ratchet
```
