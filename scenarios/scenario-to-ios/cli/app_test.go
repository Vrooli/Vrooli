...
func TestRunVersion(t *testing.T) {
	...
	// --version is handled by cli-core; it must not return an error and must
	// not touch the network (no NeedsAPI path triggered).
...
func TestMetadata(t *testing.T) {
	if !strings.EqualFold(appName, "scenario-to-ios") {
		t.Fatalf("appName = %q, want scenario-to-ios", appName)
	}
