...
`template-manager orient scenario-to-ios`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient scenario-to-ios
...
```bash
cat > /tmp/prd_context_scenario-to-ios.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start scenario-to-ios --interactive
# …answer the interview, then:
business-health wizard preview scenario-to-ios
business-health wizard apply scenario-to-ios
```
...
```bash
vrooli scenario requirements validate scenario-to-ios --json
```
...
```bash
cat > /tmp/requirements_context_scenario-to-ios.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate scenario-to-ios --json
business-health fix preview scenario-to-ios
business-health fix apply scenario-to-ios
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate scenario-to-ios --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate scenario-to-ios`, the notes page spec
...
validate scenario-to-ios --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/scenario-to-ios/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest scenario-to-ios <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> scenario-to-ios
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario scenario-to-ios
```
...
1. Add proto messages and a service under
   `packages/proto/schemas/scenario-to-ios/v1/<domain>/`, then run
