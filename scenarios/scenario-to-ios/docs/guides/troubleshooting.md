# Troubleshooting — Scenario to iOS
...
```bash
vrooli scenario status scenario-to-ios
make stop
vrooli scenario start scenario-to-ios --clean-stale
```
...
```bash
make status
vrooli scenario port scenario-to-ios UI_PORT
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/scenario-to-ios/`.
...
The default route is `${SCENARIO_DATA_DIR}/scenario-to-ios.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/scenario-to-ios.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/scenario-to-ios.db}")"
```
...
`packages/proto/gen/go/scenario-to-ios/v1/`, TypeScript under
`packages/proto/gen/typescript/js/scenario-to-ios/v1/`, and Python under
`packages/proto/gen/python/scenario_to_ios/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/scenario-to-ios/v1/<domain>/`.
