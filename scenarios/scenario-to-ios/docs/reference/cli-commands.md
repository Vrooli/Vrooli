...
`~/.vrooli/bin/scenario-to-ios`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start scenario-to-ios` if the API is unreachable |
...
(`scenario-to-ios <domain> <verb>`). Every command calls a single API
...
```bash
scenario-to-ios notes list
scenario-to-ios notes list --json
```

#### `scenario-to-ios notes create --title <title> [--body <body>]`
...
```bash
scenario-to-ios notes create --title "First note" --body "Hello world"
```
...
```bash
scenario-to-ios notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start scenario-to-ios`" is good.
