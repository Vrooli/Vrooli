...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/scenario-to-ios.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `scenario-to-ios` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/scenario-to-ios/config.json`
3. `~/.vrooli/config/scenario-to-ios/config.json`
4. `~/.config/vrooli/scenario-to-ios/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port scenario-to-ios API_PORT`)
