...
envelope (`packages/proto/schemas/scenario-to-ios/v1/shared/errors.proto`):
...
| **CLI** | `scenario-to-ios status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/scenario-to-ios/v1/shared/health.proto`
...
`POST /vrooli.scenario_to_ios.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.scenario_to_ios.v1.notes.NotesService/ListNotes`
...
| **CLI** | `scenario-to-ios notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.scenario_to_ios.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.scenario_to_ios.v1.notes.NotesService/CreateNote`
...
| **CLI** | `scenario-to-ios notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.scenario_to_ios.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.scenario_to_ios.v1.notes.NotesService/GetNote`
...
| **CLI** | `scenario-to-ios notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.scenario_to_ios.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `scenario-to-ios notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/scenario-to-ios/v1/notes/notes.proto`.
