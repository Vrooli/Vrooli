...
func (c *Client) CreateProfile(ctx context.Context, profile *domainpb.AgentProfile) (*domainpb.AgentProfile, error) {
	...
	if err := c.createResource(ctx, "/api/v1/profiles", &apipb.CreateProfileRequest{Profile: profile}, &result); err != nil {
		return nil, err
	}
...
func (c *Client) GetProfile(ctx context.Context, profileID string) (*domainpb.AgentProfile, error) {
	...
	found, err := c.getResource(ctx, fmt.Sprintf("/api/v1/profiles/%s", profileID), &result)
	if err != nil || !found {
		return nil, err
	}
	return result.Profile, nil
