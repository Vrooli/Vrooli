...
## Why This Design Language Fits Compute Manager

Compute Manager keeps `vrooli-default` unchanged. The token contract, the
colour roles, the typography scale and the accessibility floors below are
all binding here, and nothing in this scenario earns a separate visual
identity: this is operator infrastructure, not a marketed surface, and a
shared kit means the operator carries one set of habits across the whole
fleet. The reasoning behind the choice is recorded in
[`docs/concepts/EXPERIENCE.md`](docs/concepts/EXPERIENCE.md); this note
records what the choice obliges the UI to do.

The users are operators and agents buying capacity, and the primary
surface is an inventory list they open and stay on. That is exactly the
dense, tabular, repeated-work shape this design is built for, so the
layout guidance below applies without adaptation: full-width application
surfaces, a table rather than a grid of cards, compact page titles, and a
single dominant action.

Three obligations from
[`PRD.md`](PRD.md) section "UX and Branding" bind harder here than the
generic guidance below, because this scenario renders money and time:

1. **Tabular numerals wherever cost or elapsed time is rendered.** Use
   the monospace stack, or a proportional face with `font-variant-numeric:
   tabular-nums`, for every currency amount, elapsed duration and
   remaining lifetime, in tables and in cards alike. The Typography
   section below already reserves monospace for tabular metrics; this
   scenario reads that as a requirement rather than a preference. The
   point is that a column of figures compares by eye, without anyone
   reading a single number. Digits that change width turn a scan into a
   calculation, and the whole reason cost sits beside inventory is that
   nobody should have to calculate.

2. **Expiring-soon and over-budget must be distinguishable without
   colour, and from each other.** The Color section below already forbids
   colour as the only status carrier. This scenario has a sharper version
   of that problem: its two most urgent states are conventionally red, so
   red alone makes them indistinguishable even for a viewer who sees
   colour perfectly. Give each one a distinct non-colour carrier. Expiry
   is a countdown, so it reads as a duration with a shape that shortens.
   Budget is a threshold, so it reads as a labelled boundary that has
   been crossed. Never let the two share a carrier, and never let hue be
   the only difference between them.

3. **Live regions announce a provisioning result.** A row that silently
   changes is a change nobody was told about. Provisioning, extension and
   destruction each finish asynchronously and each spend or stop spending
   money, so every one of them announces its outcome through the
   accessibility tree with the amount and the unit named, rather than
   swapping a row underneath the reader. This is the UX-State Contract
   below applied to the one surface where a missed state costs money:
   `pending` while the provider is called, then `success` or
   `request-error` announced, never an unannounced replacement.

One more consequence of the product, rather than of the kit. The
Components section below lists destructive actions as separated,
confirmed and visually distinct. Here destruction is the only stop, so
the destructive control is not an edge case tucked away from the main
flow; it is a routine action that still has to be typed to confirm.
Design it as a normal part of the instance surface that happens to
demand a deliberate keystroke, not as a hidden danger zone.
