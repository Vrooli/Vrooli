...
func TestProtoConnectParity(t *testing.T) {
	...
	// A Connect procedure path is "/<proto package>.<Service>/<Method>",
	// so every Connect-mounted endpoint starts with the proto namespace.
	// Anything else is a declared REST exception and has no proto file.
	connectMounted := 0
	for _, ep := range endpoints {
		if strings.HasPrefix(ep.Path, "/vrooli.") {
			connectMounted++
		}
	}
	...
	// The invariant is parity in both directions, not that a domain
	// exists. A scenario whose only routes are declared REST exceptions
	// registers no proto file and has nothing to check; a scenario that
	// mounts Connect procedures must register the file they come from.
	if connectMounted == 0 {
		require.Empty(t, files,
			"AllProtoFiles() registered %d entries while no endpoint mounts a "+
				"Connect procedure; a registered proto file with no mounted "+
				"service is an unreachable contract", len(files))
		t.Skip("no Connect-mounted domain module; parity is vacuous")
	}
