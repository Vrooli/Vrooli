...
`vrooli scenario orient code-facts`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient code-facts
...
```bash
prd-control-tower prd generate code-facts \
...
```bash
prd-control-tower prd validate code-facts --json
```
...
```bash
prd-control-tower requirements generate code-facts \
...
```bash
prd-control-tower requirements validate code-facts --json
```
