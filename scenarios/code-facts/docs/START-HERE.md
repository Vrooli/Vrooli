...
`template-manager orient code-facts`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient code-facts
...
```bash
cat > /tmp/prd_context_code-facts.md <<'EOF'
Overview:
- Purpose:
...
```bash
vrooli scenario requirements validate code-facts --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/code-facts/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest code-facts <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> code-facts
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario code-facts
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> code-facts <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/code-facts/v1/<domain>/`.
...
1. Add proto messages and a service under
   `packages/proto/schemas/code-facts/v1/<domain>/`, then run
