...
## 2026-09-15 — 3.9 GB heap and unhealthy for 32 days on minimouse

- Rung: W3 (runtime environment defect, not a code-facts contract gap)
- Evidence: On minimouse, code-facts-api (pid 80100) had run for 32 days with `/health` 503 `unable to open database file (14)`, a 3,930 MB Go heap and a 4,292 MB footprint on an 8 GB host. Its cwd was `~/.vrooli.bridge-old-18984/...`: a Bridge working-tree ship had swapped `~/vrooli` for a new snapshot and deleted the old tree. That also deleted `scenarios/code-facts/data/code-facts.db`, and nothing restarted the process. After a lifecycle restart the footprint is 29 MB and health is 200. Locally the same build holds 260 MB RSS after 30 hours with a full index.
- Cause of the heap growth: not isolated. The orphaned process exposed no profile, and it was restarted to restore service. The growth happened only while every database operation failed. A fresh process does not reproduce it.
- Owner of the real fix: vrooli-bridge working-tree ship. Gitignored in-tree data must survive a ship, and stale processes must be restarted.
- Also fixed here: the duplicate local request logger was removed. api-core's access log is the single request log and no longer logs repeated `/health` probes.
- Measured: 2026-09-15
