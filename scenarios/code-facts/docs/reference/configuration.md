...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/code-facts.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `code-facts` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/code-facts/config.json`
3. `~/.vrooli/config/code-facts/config.json`
4. `~/.config/vrooli/code-facts/config.json`
...
```bash
code-facts configure api_base http://localhost:15001/api/v1
code-facts configure token "<token>"
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port code-facts API_PORT`)
