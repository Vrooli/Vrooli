...
```bash
vrooli scenario status code-facts
make stop
vrooli scenario start code-facts --clean-stale
```
...
```bash
make status
vrooli scenario port code-facts UI_PORT
```
...
code-facts status --auto-start

# Or override the API base for this invocation
code-facts status --api-base "http://localhost:$(vrooli scenario port code-facts API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/code-facts/`.
...
The default route is `${SCENARIO_DATA_DIR}/code-facts.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/code-facts.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/code-facts.db}")"
```
...
`packages/proto/gen/go/code-facts/v1/`, TypeScript under
`packages/proto/gen/typescript/js/code-facts/v1/`, and Python under
`packages/proto/gen/python/code_facts/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/code-facts/v1/<domain>/`.
