...
// the template itself does not build because of code-facts placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_code_facts_v1_notes_notes_proto, "NotesService")
}
