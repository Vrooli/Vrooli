...
func TestHealthResponse_TypeAlias(t *testing.T) {
	...
	if got, want := string(response.ProtoReflect().Descriptor().FullName()), "vrooli.code_facts.v1.health.Response"; got != want {
		t.Errorf("HealthResponse descriptor = %q, want %q", got, want)
	}
	if got, want := string(dependency.ProtoReflect().Descriptor().FullName()), "vrooli.code_facts.v1.health.DependencyStatus"; got != want {
