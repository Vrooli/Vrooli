...
func TestSearchFailsClosedWithoutIndexedBackend(t *testing.T) {
	t.Parallel()
	_, err := NewService().Search(context.Background(), &factsv1.SearchRequest{Query: "provider demotion"})
	if err == nil || !strings.Contains(err.Error(), "indexed search backend") {
		t.Fatalf("Search() error = %v, want missing indexed backend", err)
	}
}
