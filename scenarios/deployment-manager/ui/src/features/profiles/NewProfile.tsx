...
export function NewProfile() {
  ...
  const scenarioOptions = useMemo(() => {
    ...
    const typed = formData.scenario.trim().toLowerCase();
    ...
    if (typed && !combined.some((name) => name.toLowerCase() === typed)) {
      filtered.unshift(formData.scenario);
    }
    return filtered.slice(0, 8);
  }, [profiles, formData.scenario]);
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {scenarioOptions.map((name) => (
                    <button
                      key={name}
                      type="button"
                      onClick={() => setFormData((prev) => ({ ...prev, scenario: name }))}
                      className={`rounded-lg border px-3 py-2 text-left transition ${
                        formData.scenario === name
