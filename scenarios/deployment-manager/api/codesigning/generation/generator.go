...
//
// This package transforms codesigning.SigningConfig into platform-specific
...
type Generator interface {
	...
	GenerateElectronBuilder(config *codesigning.SigningConfig) (*codesigning.ElectronBuilderSigningConfig, error)
	...
	GenerateEntitlements(config *codesigning.MacOSSigningConfig, capabilities []string) ([]byte, error)
	...
	GenerateNotarizeScript(config *codesigning.MacOSSigningConfig) ([]byte, error)
	...
	GenerateAll(config *codesigning.SigningConfig) (map[string][]byte, error)
}
...
type Options struct {
	...
	EnvironmentResolver codesigning.EnvironmentReader
}
...
// GenerateEntitlements creates macOS entitlements.plist content.
func (g *DefaultGenerator) GenerateEntitlements(config *codesigning.MacOSSigningConfig, capabilities []string) ([]byte, error) {
	if config == nil {
		return nil, nil
	}
...
// GenerateNotarizeScript creates the afterSign JavaScript for electron-builder.
func (g *DefaultGenerator) GenerateNotarizeScript(config *codesigning.MacOSSigningConfig) ([]byte, error) {
...
// GenerateAll creates all signing-related files and returns them as a map.
func (g *DefaultGenerator) GenerateAll(config *codesigning.SigningConfig) (map[string][]byte, error) {
	if config == nil || !config.Enabled {
		return nil, nil
	}
