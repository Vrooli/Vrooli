...
func publishedVersionRows() *sqlmock.Rows {
	now := time.Now()
	return sqlmock.NewRows([]string{"id", "profile_id", "platform", "version", "sha512", "git_commit_hash", "artifact_id", "deployment_id", "release_id", "published_at"}).
		AddRow(1, "p1", "linux-x64", "1.0.0", "digest", "abc", int64(7), "d1", "r1", now)
}
...
func TestSQLPublishedVersionsRepositoryPersistsAndQueriesHistory(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("INSERT INTO published_versions")).WithArgs("p1", "linux-x64", "1.0.0", sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg(), sqlmock.AnyArg()).WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(1))
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, profile_id, platform, version, sha512, git_commit_hash, artifact_id, deployment_id, release_id, published_at")).WithArgs("p1", "linux-x64", 50).WillReturnRows(publishedVersionRows())
	history, err := repo.GetHistory(context.Background(), "p1", "linux-x64", 0)
	if err != nil || len(history) != 1 || history[0].GitCommitHash != "abc" {
		t.Fatalf("history = %#v, %v", history, err)
	}
	mock.ExpectQuery(regexp.QuoteMeta("SELECT id, profile_id, platform, version, sha512, git_commit_hash, artifact_id, deployment_id, release_id, published_at")).WithArgs("p1", 10).WillReturnRows(publishedVersionRows())
