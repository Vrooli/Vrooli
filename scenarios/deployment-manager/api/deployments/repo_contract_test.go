...
func TestNewOrchestratorFullCanonicalizesContractDescendant(t *testing.T) {
	root := newDeploymentsContractFixtureRepo(t)
...
func TestResolveTopLevelScenarioDirUsesContractLayout(t *testing.T) {
	root := newDeploymentsContractFixtureRepo(t)
	got := resolveTopLevelScenarioDir(root)
	want := filepath.Join(root, "scenarios")
	if got != want {
		t.Fatalf("resolveTopLevelScenarioDir() = %q, want %q", got, want)
	}
}
...
func newDeploymentsContractFixtureRepo(t *testing.T) string {
	t.Helper()

	root := t.TempDir()
	repoRoot := deploymentsRepoRoot(t)
