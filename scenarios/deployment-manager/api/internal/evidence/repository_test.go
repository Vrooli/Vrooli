...
func evidenceVerdict() *commonv1.TargetVerdict {
	return &commonv1.TargetVerdict{
		...
		Disposition: commonv1.Disposition_DISPOSITION_PASSED, EvidenceClass: "release-grade",
		RunId:  "run-1",
...
func TestSQLRepositorySaveAndListRoundTrip(t *testing.T) {
	...
	if len(got) != 1 || len(got[0].Refs) != 1 || got[0].Refs[0].ArtifactId != "capture-1" || got[0].EvidenceClass != "release-grade" {
...
func TestEvidenceClassIsReconciledForAnExistingLedger(t *testing.T) {
	db := testutil.OpenSQLite(t)
	legacy := `CREATE TABLE deployment_evidence_verdicts (
		id TEXT PRIMARY KEY, profile_id TEXT NOT NULL, git_commit_hash TEXT NOT NULL,
		target_ramp TEXT NOT NULL, target_platform TEXT NOT NULL, target_os TEXT NOT NULL,
		device_kind INTEGER NOT NULL, bridge_node_id TEXT, bridge_job_id TEXT,
		disposition INTEGER NOT NULL, run_id TEXT NOT NULL, detail TEXT NOT NULL,
	...
	if err := database.EnsureSchemas(context.Background(), db, database.SchemaProviderFunc(Schema)); err != nil {
		t.Fatalf("reconcile evidence schema: %v", err)
	}
	repo := NewSQLRepository(db, "sqlite")
	if err := repo.Save(context.Background(), "profile-1", "commit-1", evidenceVerdict()); err != nil {
		t.Fatalf("save after schema reconciliation: %v", err)
	}
	got, err := repo.List(context.Background(), "profile-1", "commit-1", 10)
	if err != nil || len(got) != 1 || got[0].EvidenceClass != "release-grade" {
		t.Fatalf("reconciled evidence round trip = %+v, err=%v", got, err)
	}
}
