...
testing::runner::init \
  --scenario-name "prompt-injection-arena" \
...
testing::runner::register_phase --name business --script "$PHASES_DIR/test-business.sh" --timeout 240
testing::runner::register_phase --name performance --script "$PHASES_DIR/test-performance.sh" --timeout 90 --requires-runtime true
