	...
	endpoints := []module.EndpointDescriptor{
		{
			...
			CLIMapping: &module.CLIMapping{
				Command: "go-code-graph lonely subcommand",
			},
		},
	}
...
func TestCrossCheck_PassesWhenSeeded(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "x", CLIMapping: &module.CLIMapping{Command: "go-code-graph x"}},
...
// normalisation step: the endpoint's "go-code-graph notes list"
...
func TestStripBinaryPrefix(t *testing.T) {
	cases := []struct {
		...
		{in: "go-code-graph status", want: "status"},
		{in: "go-code-graph notes list", want: "notes list"},
		{in: "already-stripped", want: "already-stripped"},
		{in: "go-code-graph", want: "go-code-graph"}, // no trailing space → preserved
	}
