 ...
 */
export function RewriteTab({ modulePath }: RewriteTabProps) {
  ...
  const runPlan = () => {
    apply.reset();
    setConfirming(false);
    plan.mutate({ modulePath, operations });
  };
  const runApply = () => {
    const planId = plan.data?.planId;
    if (planId === undefined) return;
    setConfirming(false);
    apply.mutate({ modulePath, planId });
  };

  const planId = plan.data?.planId;
  const canPlan = modulePath.length > 0 && operations.length > 0 && !plan.isPending;
      ...
      {confirming ? (
        ...
        >
          ...
          <p className="text-sm text-app-foreground">
            {t(strings.rewrite.apply.confirmMessage, { target: modulePath })}
          </p>
