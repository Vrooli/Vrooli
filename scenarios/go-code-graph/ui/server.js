...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start go-code-graph` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start go-code-graph` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.go_code_graph\.v1\./

startScenarioServer({
  uiPort,
  apiPort,
  distDir: './dist',
  serviceName: 'go-code-graph',
  corsOrigins: '*',
  setupRoutes(app) {
