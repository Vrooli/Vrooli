...
make stop
vrooli scenario start go-code-graph --clean-stale
```
...
```bash
make status
vrooli scenario port go-code-graph UI_PORT
```
...
go-code-graph status --auto-start

# Or override the API base for this invocation
go-code-graph status --api-base "http://localhost:$(vrooli scenario port go-code-graph API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/go-code-graph/`.
...
The default route is `${SCENARIO_DATA_DIR}/go-code-graph.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/go-code-graph.db"
```
...
`packages/proto/gen/go/go-code-graph/v1/`, TypeScript under
`packages/proto/gen/typescript/go-code-graph/v1/`, and Python under
`packages/proto/gen/python/go_code_graph/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/go-code-graph/v1/<domain>/`.
