...
```bash
export SQLITE_PATH=/tmp/go-code-graph.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/go-code-graph.db}")"
```
...
`packages/proto/gen/typescript/js/go-code-graph/v1/`, and Python under
