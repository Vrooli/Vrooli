...
| 2026-05-23 | No internal extraction cache in v1. | Initial scope favored cartographer-owned snapshots over cache invalidation in the graph service. | Superseded after live profiling showed repeated identical calls were a dominant CPU source. | Superseded 2026-08-20 by the bounded content-fingerprint cache. |
| 2026-08-20 | Add opt-in profiles, scoped package patterns, and a bounded content-fingerprint cache. | Live calls repeatedly paid the full `go/packages` type-check cost. Consumers differ in whether they need tests and semantic usage facts. | Full remains the compatibility default. Structural extraction avoids type checking; semantic excludes tests; patterns narrow scope; cache reuse is correctness-keyed and best effort. | Revisit when profiling shows fingerprinting itself is material; then add package-aware or incremental fingerprinting. |
| 2026-08-20 | Make profile omissions explicit in the response and downstream snapshots. | Lighter profiles intentionally leave some fields empty; consumers need to distinguish an omitted capability from a parser failure or missing data. | `ExtractResponse` returns the effective profile plus typed omission metadata. Architecture-cartographer requests structural Go extraction in production and persists the same explainability metadata; snapshot identity includes the profile contract. | Revisit if a consumer needs a richer capability negotiation model than the current profile/omission pair. |
| 2026-05-23 | Per-path serialization, parallel across paths. | `go/packages.Load` is CPU-heavy. Two concurrent loads on the same path waste work; concurrent loads on different paths are safe and useful. | A per-path mutex serializes per `filepath.Abs(module_path)`. Apply also takes the mutex, so a Rewrite cannot race an Extract on the same path. | Revisit if concurrency contention becomes a measured bottleneck. |
