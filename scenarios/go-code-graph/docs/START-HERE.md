...
`template-manager orient go-code-graph`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient go-code-graph
...
```bash
cat > /tmp/prd_context_go-code-graph.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) go-code-graph \
  --context-file /tmp/prd_context_go-code-graph.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate go-code-graph --json
```
...
```bash
cat > /tmp/requirements_context_go-code-graph.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) go-code-graph \
  --context-file /tmp/requirements_context_go-code-graph.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/go-code-graph/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest go-code-graph <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> go-code-graph
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario go-code-graph
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> go-code-graph <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/go-code-graph/v1/<domain>/`.
