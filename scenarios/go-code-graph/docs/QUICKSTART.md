...
go-code-graph notes create --title "First note" --body "Hello"
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port go-code-graph API_PORT)
curl -s "http://localhost:${API_PORT}/health"
curl -s -X POST "http://localhost:${API_PORT}/vrooli.go_code_graph.v1.notes.NotesService/ListNotes" \
