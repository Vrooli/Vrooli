...
envelope (`packages/proto/schemas/go-code-graph/v1/errors/errors.proto`):
...
| **CLI** | `go-code-graph status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/go-code-graph/v1/health/health.proto`
...
### `POST /vrooli.go_code_graph.v1.notes.NotesService/ListNotes`
...
| **CLI** | `go-code-graph notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.go_code_graph.v1.notes.NotesService/ListNotes" \
...
### `POST /vrooli.go_code_graph.v1.notes.NotesService/CreateNote`
...
| **CLI** | `go-code-graph notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.go_code_graph.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
### `POST /vrooli.go_code_graph.v1.notes.NotesService/GetNote`
...
| **CLI** | `go-code-graph notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.go_code_graph.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `go-code-graph notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/go-code-graph/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/go-code-graph/v1/<domain>/`, then run
   `make generate`.
