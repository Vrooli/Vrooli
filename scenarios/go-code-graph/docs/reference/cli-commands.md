...
`~/.vrooli/bin/go-code-graph`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start go-code-graph` if the API is unreachable |
...
```bash
go-code-graph status
go-code-graph status --json
```

### `go-code-graph configure <key> <value>`
...
```bash
go-code-graph configure api_base
```
...
```bash
go-code-graph notes list
go-code-graph notes list --json
```

### `go-code-graph notes create --title <title> [--body <body>]`
...
```bash
go-code-graph notes create --title "First note" --body "Hello world"
```
...
```bash
go-code-graph notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start go-code-graph`" is good.
