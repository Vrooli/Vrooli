...
Parser dependency: `golang.org/x/tools/go/packages` with load mode `NeedFiles | NeedImports | NeedTypes | NeedSyntax | NeedTypesInfo | NeedName | NeedDeps`. This is the canonical Go AST + import resolver; it loads packages with full type information, which is required for accurate symbol references and import resolution. No alternative parser is supported.
