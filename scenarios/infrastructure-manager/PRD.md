...
- **Operational risks**: `vrooli-autoheal` has no proto schemas and serves a plain HTTP router, so the highest-value sensor source needs a typed surface built before it can be joined properly. Two of four trust rules are blocked on roadmap Gap 10 (check reconcile, shelve-with-expiry), so the condition axis is structurally incomplete until that ships. No control layer has authored a space doc yet, so every denominator starts at `SKETCH` confidence. The scenario template reports `quarantined` in the registry, which is a generation-path risk rather than a runtime one. The team loop has been paused since 2026-07-24, so no current-state value is a live baseline until it resumes.
...
- `docs/infra-health/strategy/RELIABILITY_TARGETS.md` — the setpoint in its pre-migration prose form; migrated into this scenario as checked-in data.
- `docs/infra-health/evidence/INSTRUMENTATION_ROADMAP.md` — the capability gaps behind every open-loop cell; migrated here and replaced by the computed open-loop set.
