...
| `vrooli-autoheal` | scenario (read-only) | no | `coverage`, `condition` | Typed Connect over checks, actions, incidents, healing, and readiness/episode evidence; `space --projection supervision\|availability\|recovery --json`. | Per-source `UNAVAILABLE` with stated reason. Readiness and episode readings disappear independently when the source is unavailable. |
...
| `scenario-dependency-analyzer` | scenario (read-only) | no | `condition` (`supervision` projection), `portability` | Typed Connect core-set closure and fleet platform-verdict reads | The supervision and portability projections degrade to `UNAVAILABLE` — neither falls back to an enumerated roster or a second local resolver. |
