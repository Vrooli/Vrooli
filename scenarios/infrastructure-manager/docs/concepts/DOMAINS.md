...
|---|---|---|
| `watchdog-supervision` | The plant does not exist yet. A watchdog tier — liveness of autoheal and the core set, on a seconds clock, with an enumerated action set — is a separate operator decision precisely because it is the only piece that would touch a running system. Modelling its supervision now would inflate the open-loop count with a hole nobody can close. This plan adds host-pressure substrate cells and a report-only watchdog; it does not authorize the deferred supervision projection. | The operator authorizes a watchdog tier. It then becomes an eleventh **projection**, not a domain, supervising liveness, enumerated-action counts, and claim-suppression rate. |
