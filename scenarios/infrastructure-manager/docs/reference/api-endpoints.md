# API Endpoints — Infrastructure Manager
...
envelope (`packages/proto/schemas/infrastructure-manager/v1/shared/errors.proto`):
...
| **CLI** | `infrastructure-manager status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/infrastructure-manager/v1/shared/health.proto`
...
#### `POST /vrooli.infrastructure_manager.v1.notes.NotesService/ListNotes`
...
| **CLI** | `infrastructure-manager notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.infrastructure_manager.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.infrastructure_manager.v1.notes.NotesService/CreateNote`
...
| **CLI** | `infrastructure-manager notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.infrastructure_manager.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.infrastructure_manager.v1.notes.NotesService/GetNote`
...
| **CLI** | `infrastructure-manager notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.infrastructure_manager.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `infrastructure-manager notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/infrastructure-manager/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/infrastructure-manager/v1/<domain>/`, then run
   `make generate`.
