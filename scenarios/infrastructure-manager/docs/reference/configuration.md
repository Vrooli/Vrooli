# Configuration — Infrastructure Manager
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/infrastructure-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `infrastructure-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/infrastructure-manager/config.json`
3. `~/.vrooli/config/infrastructure-manager/config.json`
4. `~/.config/vrooli/infrastructure-manager/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port infrastructure-manager API_PORT`)
