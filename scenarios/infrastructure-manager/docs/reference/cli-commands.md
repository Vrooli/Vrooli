# CLI Commands — Infrastructure Manager
...
`~/.vrooli/bin/infrastructure-manager`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start infrastructure-manager` if the API is unreachable |
...
```bash
infrastructure-manager status
infrastructure-manager status --json
```

### `infrastructure-manager configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
infrastructure-manager configure api_base http://localhost:15001/api/v1
infrastructure-manager configure token <token>
```

Read values back without an argument:

```bash
infrastructure-manager configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`infrastructure-manager <domain> <verb>`). Every command calls a single API
...
```bash
infrastructure-manager notes list
infrastructure-manager notes list --json
```

#### `infrastructure-manager notes create --title <title> [--body <body>]`
...
```bash
infrastructure-manager notes create --title "First note" --body "Hello world"
```
...
```bash
infrastructure-manager notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start infrastructure-manager`" is good.
