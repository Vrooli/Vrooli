...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/infrastructure-manager/v1/shared/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/infrastructure-manager/v1/shared"`)
...
import (
    ...
    "infrastructure-manager/internal/clock"
    "infrastructure-manager/internal/module"
    "infrastructure-manager/internal/server"
    "infrastructure-manager/internal/testutil/assertx"
    "infrastructure-manager/internal/testutil/httpx"
...
The proto schema in `packages/proto/schemas/infrastructure-manager/v1/shared/health.proto`
...
|---|---|---|
| Wire contract | `packages/proto/schemas/infrastructure-manager/v1/notes/notes.proto` | `Note`, `service NotesService`, `ListNotesResponse`, `CreateNoteRequest`, `CreateNoteResponse`, `GetNoteRequest`, `GetNoteResponse` |
| REST metadata contract | `packages/proto/schemas/infrastructure-manager/v1/notes/attachments.proto` | `Attachment` and `UploadAttachmentResponse` for the multipart upload exception |
...
| REST error envelope | `packages/proto/schemas/infrastructure-manager/v1/shared/errors.proto` + `internal/httpx/errors.go::WriteError` | Typed body for REST exceptions, with canonical codes (`invalid_request`, `not_found`, `internal`) |
...
dynamically from `go.mod`, so it works after `infrastructure-manager`
substitution.
...
import (
    ...
    clitest "infrastructure-manager/cli/internal/testutil"
)
...
under `packages/proto/schemas/infrastructure-manager/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/infrastructure-manager/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.infrastructure_manager.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/infrastructure-manager/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/infrastructure-manager/v1/<domain>/`,
   `packages/proto/gen/typescript/js/infrastructure-manager/v1/<domain>/`, and
   `packages/proto/gen/python/infrastructure_manager/v1/<domain>/`.
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/infrastructure-manager/v1/<domain>"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/infrastructure-manager/v1/<domain>/<domain>_pb";
