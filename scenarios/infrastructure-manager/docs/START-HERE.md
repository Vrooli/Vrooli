# Start Here — Infrastructure Manager
...
`template-manager orient infrastructure-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient infrastructure-manager
...
```bash
cat > /tmp/prd_context_infrastructure-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start infrastructure-manager --interactive
# …answer the interview, then:
business-health wizard preview infrastructure-manager
business-health wizard apply infrastructure-manager
```
...
```bash
vrooli scenario requirements validate infrastructure-manager --json
```
...
```bash
cat > /tmp/requirements_context_infrastructure-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate infrastructure-manager --json
business-health fix preview infrastructure-manager
business-health fix apply infrastructure-manager
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate infrastructure-manager --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate infrastructure-manager`, the notes page spec
...
validate infrastructure-manager --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/infrastructure-manager/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest infrastructure-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> infrastructure-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario infrastructure-manager
```
