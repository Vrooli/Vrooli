...
```bash
vrooli scenario status infrastructure-manager
make stop
vrooli scenario start infrastructure-manager --clean-stale
```
...
infrastructure-manager status --auto-start

# Or override the API base for this invocation
infrastructure-manager status --api-base "http://localhost:$(vrooli scenario port infrastructure-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/infrastructure-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/infrastructure-manager.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/infrastructure-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/infrastructure-manager.db}")"
```
...
The generated package paths follow `packages/proto/gen/go/infrastructure-manager/v1/<domain>/`.
