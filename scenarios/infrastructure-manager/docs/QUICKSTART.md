# Quickstart — Infrastructure Manager
...
```bash
vrooli scenario port infrastructure-manager UI_PORT
```
...
```bash
infrastructure-manager status
infrastructure-manager <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port infrastructure-manager API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.infrastructure_manager.v1.<domain>.<Service>/<Method>
```
...
```bash
infrastructure-manager notes list
infrastructure-manager notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port infrastructure-manager API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.infrastructure_manager.v1.notes.NotesService/ListNotes" \
...
CLI (`infrastructure-manager --help`) — see
