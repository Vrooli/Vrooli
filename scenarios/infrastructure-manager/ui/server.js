...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start infrastructure-manager` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start infrastructure-manager` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.infrastructure_manager\.v1\./

startScenarioServer({
  uiPort,
  apiPort,
  distDir: './dist',
  serviceName: 'infrastructure-manager',
  corsOrigins: '*',
  setupRoutes(app) {
