...
func sqliteDSN() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("resolve infrastructure-manager storage namespace: %w", err)
	}
	path, err := resolver.Path(
		storage.Options{ScenarioID: scenarioID},
		storage.ClassData,
		"infrastructure-manager.db",
	)
	if err != nil {
		return "", fmt.Errorf("resolve infrastructure-manager db path: %w", err)
	}
	return sqliteFileDSN(path)
}
