...
type TwitterConfig struct {
	...
	// BrowserEscalation escalates JavaScript-heavy pages (X/Twitter renders its
	// timeline client-side) to browser-automation-studio's CaptureService with
	// inline_dom=true, which navigates the page in a real browser and returns
	// the rendered DOM. This replaces the former direct-to-browserless fallback.
	// It is optional and degrades gracefully: when BAS is not configured or is
	// unreachable, the escalation is skipped and the static scrape stands — the
	// same optionality the removed browserless_url field provided.
	BrowserEscalation bool   `json:"browser_escalation"`
...
func (t *TwitterIntegration) GetBookmarks(limit int) ([]StandardBookmark, error) {
	...
	// 1. Web scraping with session cookies + BAS capture escalation
	...
	// 2. Escalate to browser-automation-studio CaptureService (inline_dom=true)
	//    for JavaScript-heavy content, reading the rendered DOM it returns
...
func (t *TwitterIntegration) ValidateConfig() error {
	...
	// browser_escalation (BAS CaptureService) is optional: it degrades to
	// static-only scraping when browser-automation-studio is unreachable, so it
	// is not a hard configuration requirement.
...
func (t *TwitterIntegration) GetPlatformInfo() PlatformInfo {
	return PlatformInfo{
		...
		RequiredCredentials: []string{"session_cookie"},
		...
		Notes:               "Uses web scraping with browser-automation-studio (BAS) capture escalation for JavaScript-heavy content, due to API limitations",
	}
}
