...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("template-manager")
	if err != nil {
		return "", fmt.Errorf("resolve template-manager storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve template-manager db path: %w", err)
	}
	return sqliteFileDSN(path)
}
