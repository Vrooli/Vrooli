...
progress-handoff, and — where the template ships the file — others) while
leaving `design-language` marker-only (the template does not ship
`DESIGN.md`; it is generated by the design adapter and legitimately stays at
the adapter baseline across the fleet).
