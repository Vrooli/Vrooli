...
Generated `docs/internal/TESTING.md` files link to
[shared harness recipes](../internal/TESTING-RECIPES.md) and retain scenario-local
strategy and examples. Generated `cli/domains/README.md` files link to
[CLI Core's domain guide](/packages/cli-core/README.md#scenario-cli-domains).
Keep shared manuals with these owners; update incoming heading links and manifest
requirements when consolidating older scenario copies. Preserve local obligations
and exceptions. The recipe file is not part of the generated document set.
...
## Orientation Content-Quality Gates

Green orientation must certify real adaptation work, not merely that the
template shipped a file. A freshly generated, unedited scenario is
byte-identical (after placeholder substitution) to the template scaffold,
so gates built only from `file_exists` plus a `text_absent` placeholder
marker are gameable: deleting the marker line flips the gate green without
any scenario-specific content. The orientation engine closes this loophole
with a content-quality check that measures divergence from the template
source.

**`content_adapted` check kind.** A declarable orientation check that
passes only when the target file carries scenario-specific content beyond
the template scaffold. It resolves the scenario's originating template from
`.vrooli/service.json::generation.template.id`, reads the template's source
version of the same relative path, and counts the scenario's **net-new
content lines** — lines that are not derived from the template. Matching is
placeholder-aware: a scenario line is template-derived when it equals a
plain template line, or contains a placeholder line's literal segments in
order, so generator substitution never registers as adaptation.
...
- Default threshold: `1` net-new line for JSON files (thin registries are
  oriented by re-pointing a small number of values), `3` for prose/markdown
  (meant to be rewritten). A declared `minCount` overrides the default.
- The check **skips** (passes as not-applicable) when the scenario has no
  generation metadata or the template does not ship the file, so hand-built
  scenarios, finalized scenarios, and design-adapter-owned artifacts such as
  `DESIGN.md` are never falsely failed.

**Automatic strengthening of existing gates.** The engine derives a
`content_adapted` companion for every declared `text_absent` check whose
target file the template also ships. This upgrades the *meaning* of the
existing declarative gates for **all** templates without editing any
`template.json`: a placeholder-removal marker now also requires the file to
carry real adapted content. Because derivation keys off `text_absent` and
skips template-less files, it hardens the six previously-vacuous react-vite
gates (charter/PRD, requirements-registry, domain-map, experience-contract,
progress-handoff, and — where the template ships the file — others).

The `design-language` step also has engine-owned source checks, including for
older generated manifests that only require `DESIGN.md`. For a UI scenario,
Template Manager requires the home placeholder marker to be absent, the shell
configuration to differ from the template, and the design kit color or font
values to differ from stock. Missing source or stock comparison evidence fails
with the condition named. CSS comments, formatting, and unrelated added tokens
do not count as a chosen kit. `DESIGN.md` remains an adapter-owned artifact; the
source checks establish that the product decision reached the implementation.

New template orientation blocks may also declare `content_adapted` directly
(for example on `README.md` or `CHANGELOG.md`) to certify those artifacts
were adapted.
