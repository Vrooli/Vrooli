...
itself. Template Manager owns this factory-audience guide; generated
scenarios should not carry template-maintainer instructions.
...
5. For the `notes` example domain specifically, the "do not interleave"
   ...
   `EXAMPLE-DOMAIN:<marker>` marker (see the next section), and a generated
...
## Example Domain Markers (`EXAMPLE-DOMAIN:<marker>`)
...
2. **Trailing line marker** (single line) — a registration line (import,
   ...
   ```go
   notesH "{{SCENARIO_ID}}/handlers/notes" // EXAMPLE-DOMAIN:<marker>
   ```

   ```ts
   { path: "notes", element: <NotesPage /> }, // EXAMPLE-DOMAIN:<marker>
   ```
...
Use `template.json::copyExcludes` for files that must remain in the template
source tree but should never appear in generated scenarios. The current
template-only file is `CHANGELOG.md`.

Factory-audience guides now live under `scenarios/template-manager/docs/`.
Keep maintainer notes there instead of adding more generated-scenario excludes
for prose that Template Manager should own and serve.

Do not rely on post-generation shell hooks to remove template-only files.
Excluding them during copy is simpler and avoids briefly writing files that
should never be part of a generated scenario.
...
When a template change alters the domain workflow, keep generated docs short
and generated-scenario-facing. Put factory-facing scratch or maintenance
guidance under `scenarios/template-manager/docs/factory/`.
