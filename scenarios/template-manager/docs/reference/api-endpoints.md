# API Endpoints — Template Manager
...
`packages/proto/schemas/template-manager/v1/<domain>/<file>.proto`.
...
envelope (`packages/proto/schemas/template-manager/v1/shared/errors.proto`):
...
The proto type lives at `packages/proto/schemas/template-manager/v1/shared/health.proto`
...
#### `POST /vrooli.template_manager.v1.notes.NotesService/ListNotes`
...
| **CLI** | `template-manager notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.template_manager.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.template_manager.v1.notes.NotesService/CreateNote`
...
| **CLI** | `template-manager notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.template_manager.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.template_manager.v1.notes.NotesService/GetNote`
...
| **CLI** | `template-manager notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.template_manager.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `template-manager notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/template-manager/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/template-manager/v1/<domain>/`, then run
   `make generate`.
