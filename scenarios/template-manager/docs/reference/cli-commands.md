...
`~/.vrooli/bin/template-manager`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start template-manager` if the API is unreachable |
...
```bash
template-manager status
template-manager status --json
```

### `template-manager configure <key> <value>`
...
```bash
template-manager notes list
template-manager notes list --json
```

#### `template-manager notes create --title <title> [--body <body>]`
...
```bash
template-manager notes create --title "First note" --body "Hello world"
```
...
```bash
template-manager notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start template-manager`" is good.
