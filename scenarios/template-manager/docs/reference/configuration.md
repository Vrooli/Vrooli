...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/template-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `template-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/template-manager/config.json`
3. `~/.vrooli/config/template-manager/config.json`
4. `~/.config/vrooli/template-manager/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port template-manager API_PORT`)
