# Shared testing harness recipes

Template Manager owns these shared React/Vite scenario harness recipes. Scenario
`docs/internal/TESTING.md` files own their local test strategy, examples, fixtures,
and exceptions. Keep shared recipe changes here instead of copying this manual.

Read the [test authoring standard](/docs/testing/UNIT-TEST-AUTHORING.md) to choose
boundaries and independent expectations, and [Testing](/docs/TESTING.md) to choose
execution scope. Recipes illustrate harness mechanics; they do not establish
behavioral adequacy or native conformance.

The source examples live in [the React/Vite template](/templates/scenarios/react-vite/).
Paths beginning with `api/`, `ui/`, or `cli/` refer to that template unless a
scenario is named. Bare `internal/` and `handlers/` paths are relative to its
`api/` directory. `example-scenario` and `example_scenario` stand for the generated
scenario's module and proto namespace; substitute its actual names. Read the
scenario's local test guide and configuration before copying a recipe: older
scaffolds may use different helper APIs, proto package paths, or coverage floors.
Example domains and proposed layouts are illustrative, not required product scope.

Navigate to [API testing](#api-testing), [UI testing](#ui-testing),
[CLI testing](#cli-testing), [coverage configuration](#coverage-thresholds),
or [common harness choices](#common-patterns-and-anti-patterns).
...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  `apihttptest.MustUnmarshalProto[healthv1.Response]` (the wire shape lives
  in `packages/proto/schemas/example-scenario/v1/shared/health.proto`;
...
- **CLI**: `cli/app_test.go` — smoke gate (NewApp, --version, --help).
  ...
  `clitest.CaptureStdout` from the shared `packages/cli-core/cliapptest/` package.

Use the boundary decision table in the canonical authoring guide for other
test kinds, including pure policy, repository, property, and compile checks.

## API testing

### Layout

Keep production domain code under `api/internal/<domain>/` and transport tests
under `api/handlers/<domain>/`. Domain fakes remain beside their domain. Reuse
these shared harness owners instead of generating private copies:

| Need | Shared owner | Template example |
|---|---|---|
| Clock | [schedule](/packages/api-core/schedule/) and [scheduletest](/packages/api-core/scheduletest/) | [Repository test](/templates/scenarios/react-vite/api/internal/notes/sqlite_test.go) |
| Typed HTTP assertions | [apihttptest](/packages/api-core/apihttptest/) | [Health handler test](/templates/scenarios/react-vite/api/handlers/health/handler_test.go) |
| Live production server | [servertest](/packages/api-core/servertest/) | [Health handler test](/templates/scenarios/react-vite/api/handlers/health/handler_test.go) |
| Test database | [databasetest](/packages/api-core/databasetest/) | [Repository test](/templates/scenarios/react-vite/api/internal/notes/sqlite_test.go) |
| Formal replay | [modeltest](/packages/proto/modeltest/) | [Flow replay test](/templates/scenarios/react-vite/api/internal/notes/flow/flow_test.go) |

### API harness primitives

1. **`scheduletest.FakeClock`** — substitutes `schedule.Clock`. Construct with
   `scheduletest.New(time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC))`,
...
3. **`httpx.NewLiveServer(t, srv)`** — wraps your `*server.Server` in a
   ...
   Use a real socket when flushing, streaming, hijacking, or connection
   closure is the claim. A recorder is valid for pure decoding/status mapping
   when transport capabilities do not affect the behavior. The real-server
   recipe protects transport-dependent middleware from recorder blind spots.
4. **`apihttptest`** — `AssertStatus(t, resp, want)` for status code
...
5. **Generated proto types** — every endpoint's wire shape lives in
   `packages/proto/schemas/example-scenario/v1/<domain>/<file>.proto`.
   Tests import the generated Go type directly
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/example-scenario/v1/shared"`)
   and decode wire bodies into it via `apihttptest.MustUnmarshalProto`. Create
...
import (
    ...
    "example-scenario/handlers/health"
    "github.com/vrooli/api-core/schedule"
    "example-scenario/internal/module"
    "example-scenario/internal/server"
    "github.com/vrooli/api-core/apihttptest"
    httpx "github.com/vrooli/api-core/servertest"
    "example-scenario/internal/testutil/mocks"
)
...
func TestHealthHandler(t *testing.T) {
    ...
    for _, tc := range cases {
        t.Run(tc.name, func(t *testing.T) {
            ...
            apihttptest.AssertStatus(t, resp, tc.wantCode)
...
The proto schema in `packages/proto/schemas/example-scenario/v1/shared/health.proto`
...
`databasetest.NewSQLite(t)` (imported as `db` in the example) returns a blank handle. Repository tests apply the
...
|---|---|---|
| Wire contract | `packages/proto/schemas/example-scenario/v1/notes/notes.proto` | `Note`, `service NotesService`, `ListNotesResponse`, `CreateNoteRequest`, `CreateNoteResponse`, `GetNoteRequest`, `GetNoteResponse` |
| REST metadata contract | `packages/proto/schemas/example-scenario/v1/notes/attachments.proto` | `Attachment` and `UploadAttachmentResponse` for the multipart upload exception |
...
| REST error envelope | `packages/proto/schemas/example-scenario/v1/shared/errors.proto` + `internal/httpx/errors.go::WriteError` | Typed body for REST exceptions, with canonical codes (`invalid_request`, `not_found`, `internal`) |
...
| CLI test | `cli/domains/notes/handlers_test.go` | Spins a real `httptest.Server` via `cliapptest.NewAPIServer`, captures stdout via `cliapptest.CaptureStdout` |
...
scenario-local `docs/concepts/FLOWS.md`. Tests prove the state/event
...
the shared `packages/proto/modeltest` package to prove:
...
dynamically from `go.mod`, so it works after `example-scenario`
substitution.
...
│   └── mocks/
│       └── api.ts               # builders for the scenario API
├── layout/
│   ├── AppShell.test.tsx
...
The `mocks/` directory holds scenario API mock builders. Shared gamepad
and React adapter behavior is tested in `packages/iframe-bridge`. Scenario
tests should exercise their own controls and modal behavior using the shared
controller, rather than maintaining copies of SDK mocks.

### UI harness primitives
...
1. **`renderWithProviders(<Component />, opts?)`** — wraps the tree in
   ...
   shared `packages/api-core/servertest/server_test.go` pattern.
...
returns the literal key `"app.title"`. Prefer role/name queries with the typed
`strings.*` registry when the user-facing behavior is semantic. Use
`selectors.*` test IDs where semantic queries cannot express the target.
Cimode verifies copy-independent wiring, not real translation behavior.

A second `describe` block opts into real locales with
`beforeEach(async () => { await setLocale("en"); })` and asserts on the
English catalog wiring via `en.json` references. This verifies display wiring;
independently pin copy only when exact wording is a product requirement.
...
- **Shell**: `layout/AppShell.a11y.test.tsx` renders
  `<AppShell>` with stable placeholder children. It covers document structure,
...
The shared axe helper in jsdom checks supported DOM rules only. It does not
prove color contrast, browser layout, visual focus visibility, or complete
accessibility. Keep those claims in browser and manual validation. A delegated
`expectNoA11yViolations` assertion is valid test evidence; do not inline it just
to satisfy a static assertion counter.

`test-setup.ts` installs the tested `test-utils/console.ts` guard for unexpected
`console.error` and `console.warn` output. Intentional synchronous React errors
use `withExpectedConsoleErrors` around only the throwing render, with a precise
diagnostic predicate. Unmatched errors still reach the guard; methods are
restored even when rendering throws. `ErrorBoundary.test.tsx` demonstrates this
while asserting `onError`, fallback UI and recovery. Do not suppress an entire
file or clear a global spy's history to make an unexpected diagnostic disappear.

`test-utils/renderWithProviders.test.tsx` exercises this scenario's wrapper,
not just the shared api-base helper. It verifies fresh query caches, cancellation
on unmount, real-locale provider wiring and per-test language/storage/direction
reset. `i18n/format.test.ts` separately pins selected editorial literals, fallback
and missing-key policy; catalog-based component expectations only verify wiring.
...
- **Expected-error capture** around the throwing render with
  `withExpectedConsoleErrors` and a precise diagnostic predicate. React
  intentionally logs the caught error; unmatched diagnostics must still fail.
  Assert `onError`, the fallback, and recovery rather than suppressing a suite's
...
`cli/app_test.go` exercises construction, help, and version. Domain command tests
live in `cli/domains/<domain>/`. Import [CLI Core's cliapptest package](/packages/cli-core/cliapptest/)
for test servers, API-base overrides, and stdout capture; do not copy that harness
into `cli/internal/testutil/`. The local quarantine test still guards imports of
scenario-local test-only helpers.
...
When a scenario adds an API-backed command, exercise its request and output
contract through the command entry point.

Read the [notes command tests](/templates/scenarios/react-vite/cli/domains/notes/handlers_test.go)
for a complete example using generated Connect handlers, the scenario's actual
command registration, and `cliapptest` helpers. Use the test's current imports and
construction path when adapting it to another command.
...
2. **Real httptest.Server**: the CLI makes actual HTTP requests, so its
   client boundary needs a server. This does not prohibit recorders in
   separate tests of pure API decoding or response mapping.
...
under `packages/proto/schemas/example-scenario/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/example-scenario/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.example_scenario.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/example-scenario/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/example-scenario/v1/<domain>/`,
   `packages/proto/gen/typescript/example-scenario/v1/<domain>/`, and
   `packages/proto/gen/python/example_scenario/v1/<domain>/`.
...
3. **Wire it on the API side.** Import the generated Go type in your
   handler test and decode via `apihttptest.MustUnmarshalProto`:

   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/example-scenario/v1/<domain>"

   got := apihttptest.MustUnmarshalProto[domainv1.ListResponse](t, body)
   ```
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/example-scenario/v1/<domain>/<domain>_pb";
...
`api/main_e2e_test.go` retains the `e2e` build tag and declares the expected
service identity with `boottest.Run(t, boottest.Config{Service: "<scenario>-api"})`.
The shared `packages/api-core/boottest` harness:

1. Builds the API entry point with a two-minute overall deadline.
2. Starts it on a loopback port with an isolated `VROOLI_STORAGE_ROOT` and a
   simulated lifecycle-managed environment.
3. Checks decoded `/health` JSON for `status == "healthy"` and the exact service.
4. Requires successful process exit after SIGTERM within fifteen seconds on Unix.
5. Reaps failed children and reports the last 32 KiB of process output.

Startup defaults to ten seconds; `Config.StartupTimeout` supports a measured
slower scenario such as Bridge. `Config.Env` supplies additional startup inputs
without replacing harness-owned listener or storage settings. Unit Health opts
into `WindowsTermination`: boot is checked, then the process is forcibly
terminated. That Windows branch does not prove graceful shutdown.

Default `go test ./...` skips the scenario check. Existing CI E2E steps continue
to run it with `go test -tags=e2e -run TestE2E .`. Use the validation scope and
Test Genie protocol in `docs/TESTING.md` for scenario suites.

This tests real API entry-point wiring. It does not establish control-plane
supervision, stale-source rebuild behavior, the default host storage location,
domain schema contents, or cleanup effects beyond exit success. Add focused
integration assertions for those promises when required. Maintain process
machinery and failure-path regressions in the shared harness.
...
intentionally lower than the UI's 85% because the template sets different
floors for the Go and UI modules. The percentages do not establish equivalent
behavioral coverage across languages.
...
the shared `packages/api-core/databasetest/` tests and
`packages/api-core/servertest/server_test.go`) so the substrate
...
| ✅ DO | ❌ DON'T |
|---|---|
| `scheduletest.FakeClock` for time-dependent assertions | `time.Sleep(150 * time.Millisecond)` then assert on a fuzzy match |
| Real server for streaming/connection claims; recorder for pure mapping | Claiming a recorder proves socket capabilities |
| Role/name queries for semantic behavior; registry test IDs for other targets | Treating either test IDs or literal copy as a universal query rule |
...
| Adding new seams to scenario-local `docs/internal/SEAMS.md` at the same commit | "We'll document it later" |

## Cross-references

### Native test-syntax checks

Quality Health owns the reviewed test-syntax lint profile. Unit Health consumes
its source-bound observations instead of running another ESLint process. The
profile selects only `vitest/no-focused-tests` and `vitest/valid-expect`, with
warnings rather than an unreviewed upstream preset. Returned assertion promises
are valid. File-level syntax checks do not count as executed test cases.

The calibrated tuple is plugin 1.6.9, ESLint 9.39.4, and typescript-eslint 8.59.2.
The owner reports its actual configuration digest; Unit Health rejects missing
or changed configurations. Focus aliases, local lookalikes, computed access and
extended/chained test forms outside this profile remain unknown. No broad helper
name exemption proves that assertions occurred.

Do not add a second target-local copy of this selected profile merely to obtain
the same observation. Ordinary project linting remains available for its other
rules. Missing owner execution, unsupported tooling and stale source evidence
are unknown, not a successful syntax gate. See [Unit Health's test-quality rules](/scenarios/unit-health/docs/reference/test-quality-rules.md) for rule identities and promotion limits.

- **Seams definition + adding new seams**: scenario-local `docs/internal/SEAMS.md`.
- **Skill bundle for testing-related work** (load before substantial test changes):
  ...
  ```
- **Execution and validation scope**: [Testing](/docs/TESTING.md).
  Use the scenario's current lifecycle and CI declarations for its enabled gates.
- **Helper placement**: share repeated cross-domain setup in testutil;
  keep domain fakes local. A small one-use fake may stay beside its test.
  Extract it when reuse or harness behavior justifies a shared owner.
