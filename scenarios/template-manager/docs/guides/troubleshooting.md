# Troubleshooting — Template Manager
...
```bash
vrooli scenario status template-manager
make stop
vrooli scenario start template-manager --clean-stale
```
...
template-manager status --auto-start

# Or override the API base for this invocation
template-manager status --api-base "http://localhost:$(vrooli scenario port template-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/template-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/template-manager.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/template-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/template-manager.db}")"
```
...
`packages/proto/gen/go/template-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/js/template-manager/v1/`, and Python under
`packages/proto/gen/python/template_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/template-manager/v1/<domain>/`.
