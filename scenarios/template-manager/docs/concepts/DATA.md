...
| Debt entries | debt | SQLite | Seed data plus mapped findings | Kept until resolved/archived; never silently deleted by reruns. | Stable keys deduplicate repeated findings. |
...
| Debt entries | Manual/archive transition, not validation rerun absence. | Preserve resolved history for learning loop. | Status model lands with debt service. |
