...
`vrooli scenario orient template-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient template-manager
...
run `vrooli scenario detemplate template-manager`, the notes page spec
