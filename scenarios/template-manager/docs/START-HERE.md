...
`template-manager lifecycle orient template-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager lifecycle orient template-manager
...
```bash
cat > /tmp/prd_context_template-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start template-manager --interactive
# …answer the interview, then:
business-health wizard preview template-manager
business-health wizard apply template-manager
```
...
```bash
vrooli scenario requirements validate template-manager --json
```
...
```bash
cat > /tmp/requirements_context_template-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate template-manager --json
business-health fix preview template-manager
business-health fix apply template-manager
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate template-manager --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager lifecycle detemplate template-manager`, the notes page spec
...
validate template-manager --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/template-manager/v1/<domain>/<domain>.proto`
...
- [ ] Preview first: `template-manager lifecycle detemplate SCENARIO_NAME --dry-run`.
- [ ] Remove it: `template-manager lifecycle detemplate SCENARIO_NAME`. If it refuses
...
- [ ] Run `make test`.
- [ ] Confirm the gate: `template-manager lifecycle orient SCENARIO_NAME` — the
...
```bash
react-component-library components ingest template-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> template-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario template-manager
```
...
remove the example with `template-manager lifecycle detemplate SCENARIO_NAME`. Use
