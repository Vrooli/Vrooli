    ...
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex gap-2">
        <Tooltip content={!hasUnsavedChanges ? 'No changes to save' : 'Save your changes (⌘S / Ctrl+S)'}>
          <Button
            data-testid={selectors.drafts.editor.saveButton}
            onClick={onSave}
        ...
        {onPublish && (
          <Tooltip content="Publish this draft to PRD.md in the repository">
            <Button
              data-testid={selectors.drafts.editor.publishButton}
              variant="default"
              onClick={onPublish}
              size="lg"
      ...
      <div className="flex gap-2">
        <Tooltip content="Get AI help with generating or improving sections (⌘K / Ctrl+K)">
          <Button data-testid={selectors.drafts.editor.aiAssistButton} variant="outline" onClick={onOpenAI} className="gap-2 border-violet-200 hover:bg-violet-50 hover:border-violet-300 transition-colors">
        ...
        <Tooltip content={!hasUnsavedChanges ? 'No changes to discard' : 'Discard unsaved changes'}>
          <Button data-testid={selectors.drafts.editor.deleteButton} variant="outline" onClick={onDiscard} disabled={!hasUnsavedChanges} className="gap-2">
            <RotateCcw size={16} />
