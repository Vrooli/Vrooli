...
### 1. Agent Execution via Agent-Manager (CLEANED)
**Update**: app-issue-tracker now runs investigations exclusively through **agent-manager**. Local CLI execution, exit-code handling, and transcript file parsing have been removed to keep a clean, centralized lifecycle.
...
### 4. Investigation Script Integration (Resolved)
**Update**: Investigations now run exclusively through agent-manager. Script-based execution has been removed in favor of centralized run management and event capture.
