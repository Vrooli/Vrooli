...
func ResetInvestigationForReopen(issue *issuespkg.Issue) {
	...
	if issue.Metadata.Extra != nil {
		...
		delete(issue.Metadata.Extra, metadata.AgentRunIDKey)
		delete(issue.Metadata.Extra, metadata.AgentProfileKey)
		delete(issue.Metadata.Extra, metadata.AgentRunnerTypeKey)
...
func MarkInvestigationStarted(issue *issuespkg.Issue, agentID, runID, profileKey, runnerType, sessionID string, startedAt time.Time) {
	...
	if trimmed := strings.TrimSpace(runID); trimmed != "" {
		extras[metadata.AgentRunIDKey] = trimmed
	}
	if trimmed := strings.TrimSpace(profileKey); trimmed != "" {
		extras[metadata.AgentProfileKey] = trimmed
	}
	if trimmed := strings.TrimSpace(runnerType); trimmed != "" {
		extras[metadata.AgentRunnerTypeKey] = trimmed
	}
	if trimmed := strings.TrimSpace(sessionID); trimmed != "" {
		extras[metadata.AgentSessionIDKey] = trimmed
	} else {
		delete(extras, metadata.AgentSessionIDKey)
	}
}
...
func RecordAgentExecutionFailure(issue *issuespkg.Issue, errMsg, runID, profileKey, runnerType, sessionID string, timestamp time.Time, maxTurnsExceeded bool) {
	extras := ensureMetadataExtra(issue)
...
func MarkInvestigationSuccess(issue *issuespkg.Issue, report, runID, profileKey, runnerType, sessionID string, completedAt time.Time) {
