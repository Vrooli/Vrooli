...
interface StreamSettingsProps {
  ...
  /** Whether to show performance stats (controlled) - also enables server-side debug timing */
  showStats?: boolean;
...
export function StreamSettings({
  ...
  // Sync perfMode with server when session becomes active
  // This ensures server-side timing instrumentation is enabled when showStats was already on from localStorage
  useEffect(() => {
    if (!sessionId || !showStats) return;

    // Send perfMode to server to enable timing headers
    const syncPerfMode = async () => {
      try {
        const response = await fetch(`/api/v1/recordings/live/${sessionId}/stream-settings`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ perfMode: true }),
        });
        if (!response.ok) {
          console.warn('Failed to sync perfMode:', await response.text());
        }
      } catch (err) {
        console.warn('Failed to sync perfMode:', err);
      }
    };
  ...
  // Handle show stats toggle - also enables/disables server-side perf mode
  const handleShowStatsToggle = useCallback(async () => {
    const newValue = !showStats;
    setShowStats(newValue);
    saveShowStats(newValue);

    // Send perfMode to API if we have an active session
    // This enables server-side timing instrumentation when stats are shown
    if (sessionId) {
      try {
        const response = await fetch(`/api/v1/recordings/live/${sessionId}/stream-settings`, {
          method: 'POST',
  ...
  }, [showStats, setShowStats, sessionId]);
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Show stats toggle - also enables server-side timing instrumentation */}
          ...
          {/* Application hint - only show for scale limitation */}
          {effectiveHasActiveSession && sessionId && (
            <div className="px-3 py-2 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
              <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
                ...
                Resolution (Standard/HiDPI) requires new session
              </p>
            </div>
          )}
        </div>
