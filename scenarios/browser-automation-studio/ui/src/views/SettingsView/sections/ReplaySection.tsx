...
export function ReplaySection({ onRandomize, onSavePreset }: ReplaySectionProps) {
  ...
  const handleViewportChange = useCallback(
    ({ width, height }: { width: number; height: number }) => {
      setReplaySetting('presentationWidth', width);
      setReplaySetting('presentationHeight', height);
    },
    [setReplaySetting]
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <SettingSection title="Replay Dimensions" tooltip="Set the default canvas size for styled replays.">
        <ViewportPicker
          value={{ width: replay.presentationWidth, height: replay.presentationHeight }}
          onChange={handleViewportChange}
          useCustom={replay.useCustomDimensions}
          onUseCustomChange={(v) => setReplaySetting('useCustomDimensions', v)}
