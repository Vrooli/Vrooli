...
2. The Playwright driver evaluates the condition: selectors run inside the active frame, visibility checks rely on computed bounding boxes, and expression mode executes arbitrary JS with access to `document`.
