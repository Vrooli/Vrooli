...
- `POST /api/v1/projects/{id}/files/write` — currently JSON body; not multipart.
- `GET  /api/v1/projects/{id}/files/*` — file content stream; likely
  `third_party_shape` (browser <img>/<a download>); to be confirmed when the
  `project_files` domain migrates (Phase 5).
