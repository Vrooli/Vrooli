...
## Last Updated
2026-07-26 — updated after the `_time_format=sqlite` DSN fix and the per-domain SQLite schema registry extraction.
...
- [x] No sql migrations directory. Domain sources are applied through one ordered registry.
...
## Per-domain schema ownership

BAS uses per-domain schema ownership through `database.SchemaRegistry`. The registry applies `core.sql`, `recording.sql`, `billing.sql`, `uxmetrics.sql`, and `triggers.sql` in dependency order. Production startup and focused repository tests use the same `database.ApplySchemaRegistry` seam.
...
The registry sources declare tables across these domain boundaries (mapping each to the existing code package that owns its read/write logic):
