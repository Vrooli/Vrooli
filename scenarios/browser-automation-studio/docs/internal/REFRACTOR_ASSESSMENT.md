...
| BAS-RF-029 | `useInputForwarding.ts:262` forwards Ctrl+A, Command+C and Alt+F as plain text a/c/f. Shift-pointer-down omits modifier state. An `isComposing=true` Process keydown becomes a standalone keyboard command. Plain x and Shift-Tab are successful controls. | The UI hook distinguishes text from key chords, includes active pointer modifiers and leaves IME composition to the browser. The driver `recording-input.ts` owns modifier lifetime across pointer down/up, releases state after completion/failure, and forwards keyboard chords. Preserve platform-specific shortcuts and test browser/OS effects; hook/route tests prove payload/effect intent, not native OS behavior. |
