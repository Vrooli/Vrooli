...
// ============================================================================
// InspectProjectFolder Tests
// ============================================================================

func TestInspectProjectFolder_NonExistent(t *testing.T) {
	handler, _, _, _, _, _ := createTestHandler()

	body := `{"folder_path": "/nonexistent/path/that/does/not/exist"}`
	req := httptest.NewRequest(http.MethodPost, "/api/v1/projects/inspect-folder", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()

	handler.InspectProjectFolder(rr, req)

	if rr.Code != http.StatusOK {
		t.Fatalf("expected status 200, got %d: %s", rr.Code, rr.Body.String())
	}

	var response InspectProjectFolderResponse
	if err := json.Unmarshal(rr.Body.Bytes(), &response); err != nil {
		t.Fatalf("failed to parse response: %v", err)
	}
...
func TestInspectProjectFolder_ExistingDir(t *testing.T) {
	handler, _, _, _, _, _ := createTestHandler()

	tmpDir := t.TempDir()

	body := `{"folder_path": "` + tmpDir + `"}`
	req := httptest.NewRequest(http.MethodPost, "/api/v1/projects/inspect-folder", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
	...
	if !response.Exists {
		t.Error("expected Exists to be true for existing path")
	}
	if !response.IsDir {
...
func TestInspectProjectFolder_MissingPath(t *testing.T) {
	handler, _, _, _, _, _ := createTestHandler()
...
func TestImportProject_Success(t *testing.T) {
	handler, _, _, _, _, _ := createTestHandler()

	tmpDir := t.TempDir()

	body := `{"folder_path": "` + tmpDir + `"}`
	req := httptest.NewRequest(http.MethodPost, "/api/v1/projects/import", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()

	handler.ImportProject(rr, req)
...
func TestImportProject_NonExistentPath(t *testing.T) {
	handler, _, _, _, _, _ := createTestHandler()

	body := `{"folder_path": "/nonexistent/path"}`
	req := httptest.NewRequest(http.MethodPost, "/api/v1/projects/import", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
...
func TestImportProject_AlreadyIndexed(t *testing.T) {
	handler, catalogSvc, _, _, _, _ := createTestHandler()
	...
	// Should return 200 (idempotent) with existing project
	if rr.Code != http.StatusOK {
		t.Fatalf("expected status 200 for already indexed project, got %d: %s", rr.Code, rr.Body.String())
	}
}

func TestImportProject_MissingPath(t *testing.T) {
	handler, _, _, _, _, _ := createTestHandler()
