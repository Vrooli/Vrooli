// Package uxmetrics wires the BAS `uxmetrics` CLI group to the proto
// UXMetricsService via the generic protodispatch dispatcher. The manifest
...
const GroupName = "uxmetrics"

// Register builds the uxmetrics subcommand group by binding every manifest
// command to a generic protodispatch handler resolved against the
// generated UXMetricsService descriptor.
func Register(core *cliapp.ScenarioApp, manifest []byte) (cliapp.SubcommandGroup, error) {
	svc := pbsvc.File_browser_automation_studio_v1_uxmetrics_uxmetrics_proto.Services().ByName("UXMetricsService")
	if svc == nil {
		return cliapp.SubcommandGroup{}, fmt.Errorf("%s: UXMetricsService descriptor not found", GroupName)
	}
