// Package tools wires the BAS `tools` CLI group to the proto
// ToolsService via the generic protodispatch dispatcher. The manifest
...
import (
	...
	pbsvc "github.com/vrooli/vrooli/packages/proto/gen/go/browser-automation-studio/v1/tools"

	"github.com/vrooli/cli-core/cliapp"
)

// GroupName is the manifest group this register package owns.
const GroupName = "tools"

// Register builds the tools subcommand group by binding every manifest
// command to a generic protodispatch handler resolved against the
// generated ToolsService descriptor.
func Register(core *cliapp.ScenarioApp, manifest []byte) (cliapp.SubcommandGroup, error) {
	svc := pbsvc.File_browser_automation_studio_v1_tools_tools_proto.Services().ByName("ToolsService")
	if svc == nil {
		return cliapp.SubcommandGroup{}, fmt.Errorf("%s: ToolsService descriptor not found", GroupName)
	}
