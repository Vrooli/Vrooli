...
- `__seeds/` – deterministic seed entrypoint (seed.go or seed.sh) supporting flows
...
- `@seed/<key>` tokens (e.g., `@seed/projectName`) are resolved by the workflow resolver *before* execution using the JSON artifact generated by `test/playbooks/__seeds/seed.go|seed.sh` (`coverage/runtime/seed-state.json`). Use these when you need literal strings (selectors, labels, etc.) derived from the dynamic seed state.
