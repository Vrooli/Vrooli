...
envelope = {"program": "program-runtime.typed-inference", "version": "2", "status": "failed",
            "phase": "validate", "inputs": {}, "signals": {}, "errors": [], "evidence": []}
try:
    corpus = inputs.get("corpus", ['The provider timed out during a retry.', 'The user supplied an invalid request field.'])
    labels = inputs.get("labels", ["infra", "user"])
    if not isinstance(corpus, list) or not 1 <= len(corpus) <= 8:
        raise ValueError("corpus must contain 1 to 8 texts")
