...
// domain reference and add their own domains, so pinning the count would
...
// domain's own *_test.go for the canonical apply-and-query coverage.
...
// api/handlers/*/module_test.go) so the safety net applies
