...
func (r *invocationRepository) RecordInvocation(ctx context.Context, in Invocation) error {
 ...
 (invocation_id, binding_id, target_scenario, session_id, program_id, provenance, outcome, reason, latency_ms, usage_input_tokens, usage_output_tokens, usage_cost_micros, occurred_at)
 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		in.InvocationID, in.BindingID, in.TargetScenario, in.SessionID, in.ProgramID, in.Provenance, in.Outcome, in.Reason, in.LatencyMS, in.UsageInputTokens, in.UsageOutputTokens, in.UsageCostMicros, in.OccurredAt.UTC().Format(time.RFC3339Nano))
	if err != nil {
...
func (r *invocationRepository) ListInvocations(ctx context.Context, since time.Time, bindingID, scenario string) ([]Invocation, error) {
	query := `SELECT invocation_id, binding_id, target_scenario, session_id, program_id, provenance, outcome, reason, latency_ms, usage_input_tokens, usage_output_tokens, usage_cost_micros, occurred_at FROM binding_invocations WHERE occurred_at >= ?`
	...
	for rows.Next() {
		var in Invocation
		var occurred string
		if err := rows.Scan(&in.InvocationID, &in.BindingID, &in.TargetScenario, &in.SessionID, &in.ProgramID, &in.Provenance, &in.Outcome, &in.Reason, &in.LatencyMS, &in.UsageInputTokens, &in.UsageOutputTokens, &in.UsageCostMicros, &occurred); err != nil {
