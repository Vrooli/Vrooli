...
### Sessions, ceilings, and recurring shapes
...
A program may change only session, program, telemetry, and declared library state owned by program-runtime, and files inside a declared sandbox workspace. `bindings sweep --execute` and `library promote` are the two writes this skill's tree reaches; both need a stated reason. Never edit a library row; edit the scenario file and let the projection follow.
...
| A recurring shape has no declared program | The work recurs across non-test sessions | `program-runtime shapes list --uncovered --min-occurrences 3` | File the W1 obligation against the dominant scenario |
