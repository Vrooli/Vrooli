...
- [ ] OT-P0-010 | Bounded asynchronous execution | When work cannot complete within the synchronous response budget, the Program Runtime MUST return a durable program identifier and provide one bounded wait operation that reports either terminal completion or the current non-terminal state without client polling.
