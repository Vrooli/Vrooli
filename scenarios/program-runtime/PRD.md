...
| Scenario | Why |
|---|---|
| `ai-gateway` | Typed inference inside programs. Depends on promoting the structured-extract pipeline out of agent-manager, where it is currently run-attached. |
| `agent-manager` | Delegated agent runs spawned from a program. |
| `vrooli-events` | Telemetry through the shared event bus. Agent-manager must subscribe to this scenario's events for friction analysis to see them. |
...
2. *Manifest coverage bounds the Act surface.* 58 of 128 scenarios ship a CLI manifest. The callable surface cannot exceed manifest coverage, and the Act numerator reports that honestly rather than concealing it.
...
1. Binding generation and typed validation, proven against the live descriptor with no kernel present.
2. Handles and bounded materialization, with context bytes per query as the measured acceptance signal.
3. Session lifecycle, grants, and governance refusal.
4. Telemetry and the event contract.
5. Act denominator relocation, the space verb, and the numerator RPC.
...
7. Inference and delegation bindings.
8. The binding registry inspection surface, which is useful before any program exists.
...
**Experience contract.** Authored in `experience/`: six product pages, two journeys, and per-state behavior for the three failure classes. The triage journey — a failed program read, understood, forked, and resubmitted — is the spine that the other surfaces branch from.
