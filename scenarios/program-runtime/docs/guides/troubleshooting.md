# Troubleshooting — Program Runtime
...
```bash
vrooli scenario status program-runtime
make stop
vrooli scenario start program-runtime --clean-stale
```
...
```bash
make status
vrooli scenario port program-runtime UI_PORT
```
...
program-runtime status --auto-start

# Or override the API base for this invocation
program-runtime status --api-base "http://localhost:$(vrooli scenario port program-runtime API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/program-runtime/`.
...
The default route is `${SCENARIO_DATA_DIR}/program-runtime.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/program-runtime.db"
```
...
`packages/proto/gen/go/program-runtime/v1/`, TypeScript under
`packages/proto/gen/typescript/program-runtime/v1/`, and Python under
`packages/proto/gen/python/program_runtime/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/program-runtime/v1/<domain>/`.
