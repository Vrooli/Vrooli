...
```bash
export SQLITE_PATH=/tmp/program-runtime.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/program-runtime.db}")"
```
...
`packages/proto/gen/typescript/js/program-runtime/v1/`, and Python under
