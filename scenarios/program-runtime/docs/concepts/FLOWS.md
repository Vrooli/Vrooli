...
|---|---|---|---|---|---|
| Binding generation | bindings | Session creation, or descriptor/manifest change. | A resolved callable surface plus a refusal list explaining every withheld command. | Single-shot, idempotent for a given descriptor digest. **Fails closed** — no partial surface is exposed. Not cancellable; it is fast and has no side effects. | Unit. |
| Session lifecycle | sessions | `session create`; ends on explicit close, idle ceiling, wall-clock ceiling, memory ceiling, or kernel crash. | A live kernel bound to grants and optionally a sandbox workspace; on exit, a reclamation reason that outlives the session. | States: `pending → active → (idle) → reclaimed\|closed\|crashed`. Kernel crash is terminal and is never silently restarted — a restarted kernel would present an empty namespace as if it were the caller's state. | Unit. |
| Program submission | programs | `program exec` against a live session. | Materialized output plus a durable record of source, result, and failure detail. | States: `submitted → admitted → running → completed\|failed\|aborted`. Serialized per session — one program at a time, because the kernel namespace is shared mutable state. Cancellable mid-run; a cancelled program leaves its partial mutations in the namespace and says so. | Unit. |
| Telemetry emission | telemetry | Any submission, invocation, or failure. | A typed platform event on the bus. | Outbox with retry; dead-letter after 7 days. **Fire-and-forget** — program execution never blocks on emission, and a telemetry outage never fails a program. | Unit. |

Two deliberate non-flows: the Act numerator join (`actspace`) is a
stateless read computed live per request, and `health` is stateless
reporting. Neither has states, retries, or cancellation, so neither
belongs in this inventory.
...
| Flow | Risk | Next Step |
|---|---|---|
| Concurrent submissions within one session | Serializing per session is the safe default, but a program that blocks on a long delegated run holds its session idle. Parallelism inside one namespace risks non-deterministic mutation. | Measure how often it actually blocks before designing around it. If it matters, prefer additional sessions over concurrency inside one namespace. |
| Kernel state recovery after crash | A crashed kernel loses its namespace. Recovery is deliberately not attempted; the risk is that callers assume durability the design never offered. | None planned. Documented in `DATA.md` §"Why kernel state is not persisted". Revisit only with a measured need, and as a distinct capability. |
| Identity propagation across the sidecar | In-program inference calls reach ai-gateway without agent identity, so those events cannot be attributed to the originating agent. | Deferred by decision. See `INTEGRATIONS.md` §"Identity Propagation — Known Gap". Not a launch blocker; blocks treating in-program inference telemetry as evidence. |
