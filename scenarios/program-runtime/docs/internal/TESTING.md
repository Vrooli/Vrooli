...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/program-runtime/v1/shared/health.proto`;
...
import (
    ...
    "program-runtime/internal/testutil/assertx"
...
The proto schema in `packages/proto/schemas/program-runtime/v1/shared/health.proto`
...
### Agent-facing scaling evidence

`api/internal/programs/scaling_test.go` starts the real Python kernel through
the supervisor and executes one bounded query shape over 10, 1,000, 100,000,
and 10,000,000 synthetic rows. It asserts the returned `agent_bytes` values
remain in one constant band, then runs the same result at full materialization
and asserts that the band is exceeded. The test logs all four measurements;
the current evidence is 16, 16, 16, 16 bytes and 65,536 bytes for the fully
materialized response. Do not replace this with a fake runner or a constant
returned by the test fixture.
...
dynamically from `go.mod`, so it works after `program-runtime`
substitution.
...
under `packages/proto/schemas/program-runtime/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/program-runtime/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.program_runtime.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/program-runtime/v1/<domain>;<domain>_v1";
   ```
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/program-runtime/v1/<domain>"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/program-runtime/v1/<domain>/<domain>_pb";
