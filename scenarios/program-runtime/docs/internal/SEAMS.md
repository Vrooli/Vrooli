...
The `health` proto in `packages/proto/schemas/program-runtime/v1/shared/`
...
### Kernel supervisor boundary

`internal/programs.SubprocessRunner` owns the cross-platform JSON-lines
supervisor boundary: it creates the mode-0700 workspace, allowlists child
environment, applies host-specific resource limits when available, kills the
process group on deadline, and records `context_bytes` versus `agent_bytes`.
Tests use the real Python kernel for scaling and the runner seam for deadline
and environment-failure cases.

### Workspace resolver

`internal/sessions.WorkspaceResolver` is the consumer-owned seam for resolving
a validated workspace-sandbox workspace. The resolver is discovery-backed in
production and a typed fake in tests; program-runtime does not repair host
state or implement a second workspace manager.

### Program-runtime friction reader

`meta-optimization-manager/internal/focus.ProgramFrictionReader` is the
consumer-owned read seam for failure shapes, refusal shapes, and unresolved
binding attempts. Production uses a typed Connect client through discovery;
focus tests substitute an in-memory report, and source failure becomes a
named availability gap without suppressing other sources.
