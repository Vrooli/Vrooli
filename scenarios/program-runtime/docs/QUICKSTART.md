...
program-runtime <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port program-runtime API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.program_runtime.v1.<domain>.<Service>/<Method>
```
...
```bash
program-runtime notes list
program-runtime notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port program-runtime API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.program_runtime.v1.notes.NotesService/ListNotes" \
