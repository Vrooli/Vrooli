...
`~/.vrooli/bin/program-runtime`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start program-runtime` if the API is unreachable |
...
```bash
program-runtime status
program-runtime status --json
```

### `program-runtime configure <key> <value>`
...
(`program-runtime <domain> <verb>`). Every command calls a single API
...
```bash
program-runtime notes list
program-runtime notes list --json
```

#### `program-runtime notes create --title <title> [--body <body>]`
...
```bash
program-runtime notes create --title "First note" --body "Hello world"
```
...
```bash
program-runtime notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start program-runtime`" is good.
