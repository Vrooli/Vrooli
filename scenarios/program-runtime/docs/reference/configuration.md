...
ports as outbound source ports. See the project-level port allocation policy
for the full policy.
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/program-runtime.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `program-runtime` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/program-runtime/config.json`
3. `~/.vrooli/config/program-runtime/config.json`
4. `~/.config/vrooli/program-runtime/config.json`
...
```bash
program-runtime configure api_base http://localhost:15001/api/v1
program-runtime configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port program-runtime API_PORT`)
