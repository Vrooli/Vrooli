...
`packages/proto/schemas/program-runtime/v1/<domain>/<file>.proto`.
...
envelope (`packages/proto/schemas/program-runtime/v1/shared/errors.proto`):
...
| **CLI** | `program-runtime status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/program-runtime/v1/shared/health.proto`
...
`POST /vrooli.program_runtime.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.program_runtime.v1.notes.NotesService/ListNotes`
...
| **CLI** | `program-runtime notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.program_runtime.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.program_runtime.v1.notes.NotesService/CreateNote`
...
| **CLI** | `program-runtime notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.program_runtime.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.program_runtime.v1.notes.NotesService/GetNote`
...
| **CLI** | `program-runtime notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.program_runtime.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `program-runtime notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/program-runtime/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/program-runtime/v1/<domain>/`, then run
   `make generate`.
