...
`template-manager orient program-runtime`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient program-runtime
...
```bash
cat > /tmp/prd_context_program-runtime.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start program-runtime --interactive
# …answer the interview, then:
business-health wizard preview program-runtime
business-health wizard apply program-runtime
```
...
```bash
vrooli scenario requirements validate program-runtime --json
```
...
```bash
cat > /tmp/requirements_context_program-runtime.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate program-runtime --json
business-health fix preview program-runtime
business-health fix apply program-runtime
```
...
validate program-runtime --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/program-runtime/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest program-runtime <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> program-runtime
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario program-runtime
```
