...
type DynamicSelectorBranch = {
  readonly [key: string]: DynamicSelectorBranch | DynamicSelectorDefinition<ParamSchema>;
};
...
const isDynamicDefinition = (value: unknown): value is DynamicSelectorDefinition<ParamSchema> =>
  Boolean(value && typeof value === "object" && "kind" in value && value.kind === "dynamic-selector");

const normalizeParams = (
  definition: DynamicSelectorDefinition<ParamSchema>,
  ...
  for (const key of Object.keys(schema)) {
    ...
    const definitionEntry = schema[key] ?? { type: "string" satisfies ParamType };
    const value = raw[key] ?? "";
...
const mergeLiteralAndDynamicNodes = (
  ...
  keys.forEach((key) => {
    ...
    if (literalValue && typeof literalValue === "object") {
      const nestedDynamic = isDynamicDefinition(dynamicValue) ? undefined
        : (dynamicValue && typeof dynamicValue === "object" ? dynamicValue : undefined);
...
// Helper: assigns an untyped value into a typed variable without `as` casts.
// The merged selector tree is structurally correct at runtime (literal keys →
// strings, dynamic definitions → callable functions) but TypeScript cannot
// infer the recursive mapped type from the imperative merge logic.
function coerceSelectorTree<T>(value: unknown): T {
...
>(literalTree: L, dynamicTree: D) => {
  const selectors: SelectorTreeResult<L, D> = coerceSelectorTree(
    mergeLiteralAndDynamicNodes(literalTree, dynamicTree),
  );
  const manifest = {
