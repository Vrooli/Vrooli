...
if [ -f "${SCENARIO_ROOT}/api/go.mod" ]; then
    go_api_count=$(cd "${SCENARIO_ROOT}/api" && go test ./... -v -count=1 -timeout 60s 2>&1 | grep -c '^--- PASS' || true)
...
if [ -n "${cli_module_dir}" ] && [ -f "${SCENARIO_ROOT}/${cli_module_dir}/go.mod" ]; then
    go_cli_count=$(cd "${SCENARIO_ROOT}/${cli_module_dir}" && go test ./... -v -count=1 -timeout 60s 2>&1 | grep -c '^--- PASS' || true)
...
jq -n \
    --arg scenario "stream-of-consciousness-analyzer" \
