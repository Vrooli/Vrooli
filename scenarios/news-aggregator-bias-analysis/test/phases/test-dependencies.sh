#!/bin/bash
# Dependencies tests for news-aggregator-bias-analysis scenario
...
if command -v go >/dev/null 2>&1; then
    ...
    # Check Go version is at least 1.21
    go_minor=$(echo "$go_version" | sed 's/go1\.\([0-9]*\).*/\1/')
    if [ "$go_minor" -ge 21 ]; then
        log::success "✅ Go version is compatible (>= 1.21)"
    else
        testing::phase::add_warning "⚠️  Go version may be too old (found 1.$go_minor, need >= 1.21)"
    fi
else
...
if [ -f "$TESTING_PHASE_SCENARIO_DIR/api/go.mod" ]; then
    ...
    testing::phase::add_warning "⚠️  No go.mod found, skipping Go dependency check"
fi

echo "🔍 Validating Node.js dependencies..."
if [ -f "$TESTING_PHASE_SCENARIO_DIR/ui/package.json" ]; then
    cd "$TESTING_PHASE_SCENARIO_DIR/ui" || exit 1
    if [ -d "node_modules" ]; then
        log::success "✅ Node.js dependencies installed"
    else
        testing::phase::add_warning "⚠️  node_modules not found (run npm install)"
    fi
else
    testing::phase::add_warning "⚠️  No package.json found in ui/, skipping Node dependency check"
fi
