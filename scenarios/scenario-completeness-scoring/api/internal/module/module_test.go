...
func TestEndpointDescriptorJSONShape(t *testing.T) {
	descriptor := module.EndpointDescriptor{
		...
		Examples: []module.Example{{
			Name: "Create",
			Curl: "curl http://localhost:${API_PORT}/api/v1/tasks",
		}},
		CLIMapping: &module.CLIMapping{
			Command: "scenario-completeness-scoring tasks create",
	...
	require.Equal(t, "tasks_create", got["id"])
	require.Equal(t, "/api/v1/tasks", got["path"])
