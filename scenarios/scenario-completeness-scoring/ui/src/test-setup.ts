...
// jsdom does not implement window.matchMedia; production code (e.g. the
// ThemeProvider's `prefers-color-scheme` resolution) calls it directly per
// the DOM types. Provide an environment-level stub that reports "no match"
// so theme resolution defaults to light. Tests that need a specific media
// state should override window.matchMedia in their own beforeEach.
