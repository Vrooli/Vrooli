...
func WriteOutput(path string, data []byte) error {
	...
	if err := os.WriteFile(path, data, 0o644); err != nil { //nolint:gosec // caller-chosen path
...
// JSONLines returns pretty-printed JSON split into lines, suitable for ListReport.Results.
func JSONLines(body []byte) []string {
	...
	trimmed := strings.TrimRight(string(pretty), "\n")
	if trimmed == "" {
		return []string{"(empty payload)"}
	}
