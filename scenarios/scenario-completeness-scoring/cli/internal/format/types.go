// Package format renders the rich, multi-section completeness-score report
// for the human-facing `score get` command. JSON output bypasses this package.
//
// The types mirror the server's response shape at
// GET /api/v1/scores/{scenario}; optional per-issue fields stay as zero values
// when the API omits them, and every renderer guards them with len/nil checks.
package format
