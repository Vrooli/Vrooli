# CLI Commands — Scenario Completeness Scoring
...
`~/.vrooli/bin/scenario-completeness-scoring`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start scenario-completeness-scoring` if the API is unreachable |
...
```bash
scenario-completeness-scoring status
scenario-completeness-scoring status --json
```

### `scenario-completeness-scoring configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
scenario-completeness-scoring configure api_base http://localhost:15001/api/v1
scenario-completeness-scoring configure token <token>
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start scenario-completeness-scoring`" is good.
