# Configuration — Scenario Completeness Scoring
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/scenario-completeness-scoring.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `scenario-completeness-scoring` the prefix is the scenario id upper-cased with
...
for the per-seam table including the per-domain schema providers and
...
2. `${XDG_CONFIG_HOME}/vrooli/scenario-completeness-scoring/config.json`
3. `~/.vrooli/config/scenario-completeness-scoring/config.json`
4. `~/.config/vrooli/scenario-completeness-scoring/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port scenario-completeness-scoring API_PORT`)
