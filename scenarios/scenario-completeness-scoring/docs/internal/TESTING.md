    ...
    for _, tc := range cases {
        t.Run(tc.name, func(t *testing.T) {
            ...
            h := health.NewHandler(health.Deps{
                Pinger:  pinger,
                Service: "scenario-completeness-scoring",
                Version: "1.0.0",
            })
            mod := module.Module{
                Name: "health",
...
The proto schema in `packages/proto/schemas/scenario-completeness-scoring/v1/health/health.proto`
...
|---|---|---|
| Wire contract | `packages/proto/schemas/scenario-completeness-scoring/v1/notes/notes.proto` | `Note`, `service NotesService`, `ListNotesResponse`, `CreateNoteRequest`, `CreateNoteResponse`, `GetNoteRequest`, `GetNoteResponse` |
| REST metadata contract | `packages/proto/schemas/scenario-completeness-scoring/v1/notes/attachments.proto` | `Attachment` and `UploadAttachmentResponse` for the multipart upload exception |
...
| REST error envelope | `packages/proto/schemas/scenario-completeness-scoring/v1/errors/errors.proto` + `internal/httpx/errors.go::WriteError` | Typed body for REST exceptions, with canonical codes (`invalid_request`, `not_found`, `internal`) |
...
dynamically from `go.mod`, so it works after `scenario-completeness-scoring`
substitution.
...
import (
    ...
    clitest "scenario-completeness-scoring/cli/internal/testutil"
)
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import notesv1 "github.com/vrooli/vrooli/packages/proto/gen/go/scenario-completeness-scoring/v1/notes"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/scenario-completeness-scoring/v1/notes/notes_pb";
