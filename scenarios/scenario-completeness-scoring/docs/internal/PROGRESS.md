...
| 2026-06-10 | codex | done | Added the optional importance enrichment line now that dep-analyzer centrality exists: proto `ImportanceSummary`, `api/internal/importance` reader (dep-analyzer centrality + swarm-manager operations under 1s, neutral partial defaults, omitted when all sources miss), scoring/Connect wiring, CLI report section, UI `ImportanceCard`, i18n/selectors/factories/tests, and docs drift cleanup. |
| 2026-06-10 | claude | done | test-genie report supplement (Phase 2) and ecosystem-manager importance scheduling (Phases 3-4) shipped in later plan passes; see work records `rec-1ccdc0b08ea21ad1`, `rec-2ea173bf4a8abda5`, and `rec-ea020f3644330a61`. |
