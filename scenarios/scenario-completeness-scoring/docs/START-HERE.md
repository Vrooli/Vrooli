# Start Here — Scenario Completeness Scoring
...
`template-manager orient scenario-completeness-scoring`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient scenario-completeness-scoring
...
```bash
cat > /tmp/prd_context_scenario-completeness-scoring.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) scenario-completeness-scoring \
  --context-file /tmp/prd_context_scenario-completeness-scoring.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate scenario-completeness-scoring --json
```
...
```bash
cat > /tmp/requirements_context_scenario-completeness-scoring.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) scenario-completeness-scoring \
  --context-file /tmp/requirements_context_scenario-completeness-scoring.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/scenario-completeness-scoring/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest scenario-completeness-scoring <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> scenario-completeness-scoring
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario scenario-completeness-scoring
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> scenario-completeness-scoring <adopted-path>
```
