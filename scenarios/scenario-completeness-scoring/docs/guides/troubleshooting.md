...
```bash
export SQLITE_PATH=/tmp/scenario-completeness-scoring.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/scenario-completeness-scoring.db}")"
```
...
`packages/proto/gen/typescript/js/scenario-completeness-scoring/v1/`, and Python under
