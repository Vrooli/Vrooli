# Troubleshooting — Scenario Completeness Scoring
...
make stop
vrooli scenario start scenario-completeness-scoring --clean-stale
```
...
scenario-completeness-scoring status --auto-start

# Or override the API base for this invocation
scenario-completeness-scoring status --api-base "http://localhost:$(vrooli scenario port scenario-completeness-scoring API_PORT)/api/v1"
```
...
### `scenario-completeness-scoring configure` doesn't persist

Check which config-file path resolved (precedence in
[`../reference/configuration.md`](../reference/configuration.md#cli-config-file)).
Most commonly `~/.vrooli/config/scenario-completeness-scoring/config.json`. The
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/scenario-completeness-scoring/`.
...
The default route is `${SCENARIO_DATA_DIR}/scenario-completeness-scoring.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/scenario-completeness-scoring.db"
```
...
`packages/proto/gen/go/scenario-completeness-scoring/v1/`, TypeScript under
`packages/proto/gen/typescript/scenario-completeness-scoring/v1/`, and Python under
`packages/proto/gen/python/scenario_completeness_scoring/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/scenario-completeness-scoring/v1/<domain>/`.
