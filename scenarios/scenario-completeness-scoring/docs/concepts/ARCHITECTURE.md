# Architecture — Scenario Completeness Scoring
...
```
                       ...
                       │   scenario-completeness-scoring/v1/...    │
                       └──────────────┬──────────────┘
...
| Contracts (`packages/proto/schemas/scenario-completeness-scoring/`) | Wire shape | Proto messages/services and generated clients | Hand-written route/type mirrors |
...
- scenario proto schemas relocated to
  `packages/proto/schemas/scenario-completeness-scoring/`.
...
|---|---|
| `RESTReasonMultipartUpload` | Opaque file bytes via `multipart/form-data` (proto cannot express multipart uploads). |
...
proto-typed wherever possible — only the request transport differs.
Drift between API/UI/CLI is eliminated as long as the wire payload
type is shared.
...
1. Add proto messages and service methods under
   `packages/proto/schemas/scenario-completeness-scoring/v1/<domain>/`.
...
|---|---|---|---|
| API | Reference-ready | Domain-owned scoring/signals/freshness/importance stack, routed snapshot store/sweeper, measures federation surface, module registry, per-domain schema, documented seams. | Full scenario/GCT validation still pending. |
| UI | Reference-ready | Feature folders, typed API clients, selector/i18n registries, modeltest helpers, dashboard trend card, persisted fleet table, BAS routed-database proof against the fleet table. | Full scenario/GCT validation still pending. |
...
|---|---|---|---|
| 2026-06-10 | None yet. | Generated from `react-vite`. | Update when the scenario intentionally diverges. |
