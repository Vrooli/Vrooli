# Flows — Scenario Completeness Scoring
...
- Owner domain: scoring; signals and freshness are invoked as pure
  in-process libraries.
- Trigger: `GetScore` Connect RPC (CLI `score get`, UI dashboard, or the
  test-genie report supplement shelling the CLI).
- Inputs: target scenario name (resolved to a directory under the
  scenarios root).
- Steps:
  1. Resolve the target scenario directory; reject unknown scenarios.
  2. Run registered collectors (requirements, phase results, service
     manifest, UI heuristics) — each behind its circuit breaker; failures
     disable that collector and surface as degradation notices.
  3. Compute the current tree digest and per-phase fresh/stale/unknown
     verdicts from `coverage/runs.index.json` (freshness-go).
  4. Map findings onto maturity dimensions; evaluate ladder gates for the
     rung headline.
  5. Compute composite + classification + breakdown + recommendations +
     action plan.
  6. Assemble the response, labeling every number with the digest.
- Outputs: score payload, or a typed error (unknown scenario).
- Failure modes: missing/malformed artifacts (collector degrades, never
  crashes), never-tested scenario (verdicts "unknown"), digest computation
  failure (freshness block marked unknown with reason).
- Statefulness: none — no caching across requests in v1 (each call re-reads;
