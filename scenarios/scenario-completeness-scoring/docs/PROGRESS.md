...
| 2025-12-04 | Codex Agent | Go CLI restored | Added a Go-based CLI with config-aware API detection, endpoint coverage, and local configuration helpers. Updated `cli/install.sh` to compile the binary before installation so `vrooli scenario completeness` callers can use the new CLI instead of the deleted legacy scripts. |
