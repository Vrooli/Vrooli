...
CREATE INDEX IF NOT EXISTS idx_api_endpoints_scenario ON api_endpoints(scenario_id);
CREATE INDEX IF NOT EXISTS idx_api_endpoints_method_path ON api_endpoints(method, path);
CREATE INDEX IF NOT EXISTS idx_vulnerability_scans_scenario ON vulnerability_scans(scenario_id);
...
CREATE INDEX IF NOT EXISTS idx_performance_metrics_scenario ON performance_metrics(scenario_id);
CREATE INDEX IF NOT EXISTS idx_performance_metrics_endpoint ON performance_metrics(endpoint_id);
CREATE INDEX IF NOT EXISTS idx_improvements_scenario ON improvements(scenario_id);
CREATE INDEX IF NOT EXISTS idx_improvements_status ON improvements(status);
CREATE INDEX IF NOT EXISTS idx_scan_history_scenario ON scan_history(scenario_id);
CREATE INDEX IF NOT EXISTS idx_scan_history_type ON scan_history(scan_type);

-- Full-text search indexes
CREATE INDEX IF NOT EXISTS idx_vulnerability_scans_search ON vulnerability_scans USING gin(to_tsvector('english', title || ' ' || description));
CREATE INDEX IF NOT EXISTS idx_improvements_search ON improvements USING gin(to_tsvector('english', title || ' ' || description));
...
DROP TRIGGER IF EXISTS update_api_documentation_updated_at ON api_documentation;
...
CREATE OR REPLACE VIEW v_scenario_health AS
SELECT 
    s.name,
    s.status,
    s.last_scanned,
