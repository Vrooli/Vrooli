...
concurrent_test() {
    curl -s "http://localhost:$API_PORT/api/v1/health" >/dev/null 2>&1 &
}
...
if echo "$scan_response" | jq -e '.summary' >/dev/null 2>&1; then
    ...
    # Check if scan time is within acceptable range (< 30 seconds)
    if [ $scan_time -lt 30000 ]; then
        echo "✅ Scan performance acceptable"
    else
        echo "⚠️  Scan performance slow: ${scan_time}ms"
    fi
else
    echo "❌ Scan failed during performance test"
    exit 1
fi
