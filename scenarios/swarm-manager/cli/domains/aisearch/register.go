...
// `backlog search-ai` and `goals search-ai` are registered by the
...
func Register(deps support.Dependencies) cliapp.SubcommandGroup {
	return cliapp.SubcommandGroup{
		Name:        "ai-search",
		Description: "AI (semantic) search over backlog items and goals",
		Subcommands: []cliapp.Command{
			support.APICommand("status", "Show AI search availability and index coverage", deps.AISearchStatus),
			support.APICommand("query", "Search both collections (use backlog/goals search-ai to scope)", deps.AISearchQuery),
