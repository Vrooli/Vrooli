...
| **Target** | The mode's **unit of work** — the thing a run operates on. Declared in mode data as one of: `plan-execution` (a canonical, provider-neutral plan execution), `initiative` (a swarm-manager initiative with member items), or `backlog-item` (a single backlog item — vocabulary is pinned; its adapter arrives in a later phase). Each target kind has an **adapter** that supplies target-specific reads, artifact roots, and locking identity. Initiative is one adapter among several, not the base case. (The pre-cutover unmanaged `plan-ref` target kind was removed in the declarative-operations rebuild; the domain `plan_ref` *field* on items/initiatives is unrelated — see [AGENT-OPERATIONS.md](./AGENT-OPERATIONS.md).) |
...
| **Input contract / Reads** | The mode declares logical input specs independently of retrieval, one explicit source per spec, and prompt aliases. A phase's `reads` selects aliases from the compiled contract. Generic and target providers expose typed capability descriptors; caller, derived, and default inputs use the same compiler. |
...
|-------------|--------------|------------------|
| `plan-execution` | A canonical, provider-neutral plan execution (execution id / slug; resolved instance carries plan id + execution id) | Plan phase context, plan log access, plan-derived artifact root, plan-scoped lock identity |
...
| `backlog-item` | A single swarm-manager backlog item | Vocabulary is pinned (schema enum + engine constants); the runtime adapter is a later phase, so no mode declares this kind yet and `AdapterFor` fails closed for it |
...
## Compiled inputs: the read-side SSOT

**The v2 symmetry (D1b, completed by the P0 input compiler).** Emits have been declarative from the start: `declared_output.fields` drives extraction, validation, guards, and the UI Emits tab. The compiled input contract is the matching artifact on the input side.

- **Identity is not retrieval.** `input_contract.specs` gives every logical value a stable dotted id, JSON type/format/bounds, requiredness, sensitivity, retention policy, and operator-facing description. `sources` separately binds that id to a generic-provider capability, target-adapter capability, caller input, derived capability, or explicit default. `aliases` maps prompt-facing names such as `PRIOR_ROUNDS_JSON` to logical ids; each phase selects aliases with `reads`.
- **Typed capability registry.** Provider descriptors expose capability id, supported target kinds, value type, freshness, and failure policy to the compiler. Retrieval and authorization remain code only where behavior is unavoidable. A target-specific input is therefore data-bound to a capability rather than inferred from a switch over read names.
- **Transitive compilation.** The parent and every reachable delegated mode compile into one deterministic artifact. Compilation rejects duplicate or unnamespaced ids, missing or competing sources, unavailable capabilities, target/type mismatches, derived cycles, unsafe sensitivity/retention combinations, unknown aliases, and declared-but-unused aliases. Runtime prompt assembly consumes the compiled phase bindings.
- **Immutable executions.** Execution creation pins the compiled artifact and its SHA-256 digest beside the definition bundle before writing the manifest. Existing executions decode that pinned contract; live registry edits only affect new executions.
...
The symmetry is the point: **a phase is fully specified by its compiled Inputs, its pinned prompt, and its Emits — all three declared, validated, and attributable.**
...
- **Target:** `plan-execution` — a plan that already says what to do.
...
|------|--------|-------------------|--------------------|
| **member-item workflow strategy** (default for initiatives; persisted `mode` sentinel `item-level` or blank — NOT a mode, no mode.json; see [AGENT-OPERATIONS.md](./AGENT-OPERATIONS.md)) | initiative | One backlog item per agent run (each item runs its own operation) | Items are right-sized, independent, reviewable in isolation, and stable through execution |
...
- **member-item workflow strategy** (`item-level` wire value) — one backlog item per agent run through the item pipeline (`backlog → researching → ready → queued → in_progress → completed/failed`); refinement via the workshop loop converging to a `plan_ref`. It is initiative strategy configuration (`agentops.MemberItemStrategy`), not a methodology loop: no phase graph, no mode rounds, no catalog entry. Strengths: parallelism, per-item provenance, bounded blast radius. Failure mode: items coupled by a shared substrate produce broken intermediate states.
...
- **Use the member-item workflow strategy** (`item-level`) when items are right-sized for one run, loosely coupled, reviewable in isolation, stable, and parallelism is valuable.
...
- **D1 — Unit-of-work decoupling (targets).** A mode declares its `target` (`backlog-item` | `initiative` | `plan-execution`); the engine runs on a generic run context and target adapters supply the specifics; the generic plan drain runs plan-first with no initiative. (The declarative-operations rebuild removed the unmanaged `plan-ref` target kind and renamed `plan-manager-plan` → `plan-execution`; see [AGENT-OPERATIONS.md](./AGENT-OPERATIONS.md), D6.) *Rejected:* keeping initiative as the universal substrate and importing every plan into an initiative first — that taxes the simplest case with ceremony it doesn't need and welds a generic engine to one product concept.
...
- **Target-context flow.** The delegating mode supplies the sub-mode's target from its own resolved target instance: same target kind passes through unchanged; an initiative-target mode may delegate to a `plan-execution`-target sub-mode **only** when it declares `target.plan_ref.required` — the initiative's bound plan becomes the sub-mode's unit of work. Every other combination is rejected at load. Correspondingly, a missing bound plan is tolerated (surfaced as missing context, not an error) for the start phase *and* for any phase whose own output contract emits `plan_ref` — the phase that authors the plan must be able to run before the binding exists.
...
- **Plan-first start surface.** `OperatingModeService.StartTargetPhase` (CLI: `swarm-manager operating-mode start --mode <id> --target <plan-id|slug|path>`) starts a round of a non-initiative-target mode directly on its target. Plan-target rounds store under `<dataRoot>/mode-targets/<kind>/<sanitized-id>/` and lock on the plan ownership key (`plan--<id>`); round follow-up reuses the ordinary round actions with the resolved scope id plus an explicit `--mode`. Initiative-target modes are rejected on this surface — initiatives keep the initiative-keyed `StartPhase`.
...
- [`docs/concepts/AGENT-OPERATIONS.md`](./AGENT-OPERATIONS.md) — the declarative agent-operations layer that sits *above* modes: operation contracts, target capabilities, layered bindings, execution provenance, workflow instances, and the closed domain-action registry. Modes are the *how*; that layer is the *what* and *which mode implements it*.
...
- [`docs/guides/phased-plan-drain-mode.md`](../guides/phased-plan-drain-mode.md) — operator workflow for the phased-plan-drain mode.
...
- **2026-07-14** — **Target vocabulary normalized for the declarative-operations rebuild** (Phase 2 contracts): the target enum is now `backlog-item` | `initiative` | `plan-execution`. `plan-manager-plan` was renamed to the provider-neutral `plan-execution` (wire number unchanged); the unmanaged `plan-ref` target kind and its workspace-file adapter were removed (0 live instances); `backlog-item` was added as pinned vocabulary with no runtime adapter yet (`AdapterFor` fails closed). The domain `plan_ref` field on items/initiatives is unchanged. Both operating-mode schema copies, the proto `OperatingModeTargetKind` enum (reserved 2), and the engine constants moved together. The new declarative-operations layer is documented in [AGENT-OPERATIONS.md](./AGENT-OPERATIONS.md).
