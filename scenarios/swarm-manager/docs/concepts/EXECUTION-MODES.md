...
What this replaces: today the run substrate is initiative-shaped end to end — phase resolution keys off the initiative name, the run lock owner is the initiative, and every prompt is built from initiative data. In v2 those become adapter concerns behind the generic run context. **Initiative is one adapter among several**; nothing in the shared engine assumes an initiative exists.
...
**The v2 generalization (D2).** Guards route on fields of the completed round's resolved output. But the most natural agent output for an execute round is a *handoff* — prose-plus-structure about what happened — not a routing enum. Today that gap is bridged by a dedicated classifier **phase** (`classify_progress`): a whole extra agent round whose only job is to read the previous handoff and emit `progress.decision`. That is a phase doing a transition's job.
