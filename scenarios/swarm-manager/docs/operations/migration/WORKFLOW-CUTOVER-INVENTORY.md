...
| Execution retry | `work.retry` | New `swarm-manager/phased-plan-drain` or `swarm-manager/research-conclude` workflow execution with a fresh immutable intent and parent correlation. | Historical execution remains immutable; no operation-runner retry correlation or in-place mutation. | E in progress |
...
| Cancellation, resume, approval | `work.control` | Deterministic Swarm authorization calling Agent Manager workflow control/signal APIs. | Keep control audit; remove Swarm orchestration state. | D → E |
