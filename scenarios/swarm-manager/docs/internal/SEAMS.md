...
apply a proposal — proposal sessions, operating-mode reconciliation, future
...
- **StateBuilder seam**: `proposals.StateBuilder` is `func(initiativeName string) (CurrentState, error)`. Each surface owns its own state-loading closure (e.g., the route-wired `newProposalStateBuilder`); the proposals package never imports the graph projection or initiative store directly.
...
- **Validate-only carve-out**: Proposal-session ingestion calls `Normalize` directly without `Apply`. This explicit one-shot surfaces validation errors back to the agent and is out of scope for ApplyFlow.
...
| Boundary | Location | Behavior | Test |
|----------|----------|----------|------|
| Session store | `api/internal/agentsessions/store.go` | Persists `agent-sessions/sess_*/session.json` snapshots, append-only `messages.jsonl`, and proposal JSON files under `proposals/`. Historic `artifacts.jsonl` is read only for deterministic migration/recovery; canonical artifact links live in the evidence ledger. | `agentsessions/store_test.go`, `agentsessions/service_artifacts.go` |
| Session service | `api/internal/agentsessions/service.go` | Owns create/list/get/start/continue/events/refresh/cancel/proposal/artifact operations. Create persists a draft; start spawns through Agent Activity with `owner_type=session`, injects session environment variables, maps Agent Manager run status to session lifecycle state, persists run summaries as assistant transcript messages, bounds run-event payloads, resolves `run_id -> session` for provenance enrichment, and projects artifact links from canonical evidence. | `agentsessions/service_test.go`, `evidence/service_test.go` |
