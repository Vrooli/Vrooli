...
- **Execution manifest contract**: `execution.go` deep-copies and hashes the parent plus every reachable delegated definition and rejects a manifest whose stored digest no longer matches that bundle. It also owns explicit empty slots for the Phase 4 compiled input contract, validated input snapshot, retention metadata, and reachable prompt-source bundle. Existing execution interpretation reads this bundle; `DefinitionFor` is only the source for creating a new execution or reading legacy data. The workspace Connect projection exposes the bundle and its non-sensitive digest/revision metadata to CLI and UI consumers.
