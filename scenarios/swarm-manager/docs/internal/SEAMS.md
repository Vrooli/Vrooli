...
**Testing at the seam**: Go handler/projection tests validate proto JSON shape and invalid-lens rejection. UI service/store/presentation tests validate typed graph mapping and canvas rendering against the shared graph contract.
...
- **Execution, artifact, and round persistence**: `operatingmode.Store` owns mode-scoped paths under each target (`modes/<mode>/...`), immutable manifests at `executions/<execution-id>/manifest.json`, execution-owned rounds at `executions/<execution-id>/rounds/round-NNN.json`, the run-owner index, and current-state artifact reads/writes. Unambiguous flat histories are staged, validated, assigned a deterministic `legacy-*` execution id, and indexed before their original bytes move under `legacy-rounds/<execution-id>/`; ambiguous histories remain readable but are never resumed. New service-dispatched rounds use the execution layout. Runner and handler code should use this store rather than constructing paths directly. Derived artifact writes belong in phase `ResultBindings`, not in mode-specific artifact applier branches.
- **Execution manifest contract**: `execution.go` deep-copies and hashes the parent plus every reachable delegated definition and rejects a manifest whose stored digest no longer matches that bundle. Creation also transitively compiles and pins the canonical input contract plus SHA-256 digest before any execution directory is written; invalid capabilities, types, aliases, target support, or retention policy leave no manifest. Validated caller-input snapshots, retention metadata, and reachable prompt-source bundles are the remaining Phase 4 slots. Existing execution prompt assembly reads the pinned definition and compiled-input artifacts; `DefinitionFor` supplies only new executions or legacy adoption. The workspace Connect projection exposes non-sensitive digest/revision metadata to CLI and UI consumers.
- **Input compiler and provider seam**: `api/internal/operatingmode/input_contract.go` owns logical `InputSpec`, source, alias, and transitive phase-binding compilation. `InputProviderCapabilities` is the descriptor SSOT (type, targets, freshness, failure policy); `RunContext.resolveProviderCapability` is the behavioral retrieval edge. `DeclaredReads` renders only compiled phase bindings and prefers the execution-pinned contract over mutable mode data. Do not add a parallel read-name validator or assemble prompt variables outside this seam.
- **Round envelope contract**: Round JSON records `execution_id`, `definition_digest`, mode, phase, scope, run strategy, selected `agent_profile_key`, run ID, status, readiness, artifact updates, handoffs, and phase payload. Sequential modes preserve handoffs in round JSON so future runs have durable context.
...
- **Unified phase viewer rule**: `ui/src/components/initiative/operating-mode/phase-viewer.tsx` is the *one* surface that renders a phase, driven by a source-agnostic `PhaseView` view-model (`phase-view.ts`: `contractPhaseView` / `simulationPhaseView` / `livePhaseView`). It has four concern tabs — **Instructions / Reads / Emits / Transition** — and both `PhaseCard` (Phases tab, Contract source) and the Flow tab compose it; the Flow tab's data-source control swaps Contract/Simulation/Live for the *same* viewer instead of stacking panels. The Instructions tab renders the literal prompt lazily via `use-phase-prompt.ts` (Contract → `promptService.getSkill` template slots; Simulation/Live → the render endpoints), degrading to the variable map. Reads cards are variable-anchored (`MEMBER_ITEMS_JSON`, …) so the vocabulary is identical across sources. Do not reintroduce a separate simulation/live/contract phase component.
...
| [CODE: api/internal/operatingmode/execution.go] | Immutable definition bundles and digests, execution manifests/status, run-owner index |
...
- **Column semantics are actionability** (D2): Now = in flight (header counts only — cards come from the operations aggregate); Next = gate cards (decide/review/classify, answerable regardless of wave) + wave-0 run/workshop item cards; Later = wave ≥ 1 grouped by nearest blocker (gate-blocked groups sort above item-blocked; cycle diagnostics last); Done = window-capped outcomes (`window_seconds` clamped to [60, 86400], cap 50).
- **One placement per item**: an item with its own decide gate appears only as the Next gate card (wave badge attached), never duplicated into Later. The accepted double-count is `needs_review` runs appearing in both Done (run outcome) and Next (pending human gate).
- **Invalidation rides the graph dispatch**: mutating services include the dispatch-only `"plan"` lens in `DispatchInvalidate` payloads (backlog/captures/execution/initiatives dispatch `"topology", "plan"`; agentactivity dispatches `"plan"`); `graph.dispatchableLens` accepts it while `ValidateLens` keeps it off the graph HTTP endpoint. No server-side plan cache exists — the projection rebuilds per request (measured cheap; revisit only with profiling evidence).
- **Snooze never reaches the server.** Snooze lives in the client `snooze-store` (localStorage); the board filters/dims snoozed cards client-side (`applySnoozeFilter`), so `/api/v1/plan` deliberately has no `show_snoozed` parameter.
- **Measures posture**: the `plan` proto domain is declared `stateful: false` in `cli/manifest.json` `measures.domains[]` — it is a computed projection with no rows of its own.
