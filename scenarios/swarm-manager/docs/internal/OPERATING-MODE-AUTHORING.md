...
- **Reads** — each phase declares `reads`: the named variables its prompt template may reference, composed as generic-base provider ∪ target-adapter provider. The loader rejects a read the mode's target does not provide and any read no provider supplies; the renderer substitutes exactly the declared reads.
