...
- 2026-05-01: Started UI hot-spot warning cleanup. `SettingsDrawer` no longer emits missing-key or nested-button React warnings, and its test asserts a quiet `console.error`. `BacklogDetailContext.test.tsx` now uses a shared `withExpectedReactHookError` helper so the intentional provider-invariant throw does not print jsdom/React stacks. `ExecutionPage.test.tsx` now uses the shared render/query harness and no longer emits React Router future-flag warnings, though its polling-driven `act(...)` warning remains. `setupTests.ts` also filters the known `[api-base]` startup stdout prefix from the test environment.
