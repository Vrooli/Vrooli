 ...
 * workshop behavior, agent settings, and UI behavior.
 *
    ...
    <div className="space-y-6" data-testid={selectors.settings.page}>
      ...
      <Card data-testid={selectors.settings.workshopSettings}>
        <div className="flex items-center justify-between">
          ...
          <button className="text-xs text-slate-500 hover:text-slate-300" onClick={() => patch({ maxAutoRounds: DEFAULT_SETTINGS.maxAutoRounds })}>Reset</button>
        </div>
        <div className="mt-4">
          <label className="block text-sm font-medium text-slate-300">Max Auto Rounds</label>
          <p className="mt-1 text-xs text-slate-400">Maximum workshop rounds before auto-advancement stops (1-50).</p>
          <Input
            type="number"
            min={1}
            max={50}
            ...
            onChange={(e) => patch({ maxAutoRounds: Math.max(1, Math.min(50, Number(e.target.value || 1))) })}
          />
        </div>
      </Card>
