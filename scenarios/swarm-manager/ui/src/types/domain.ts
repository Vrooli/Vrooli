 ...
 */
export type IdeaAgentMode = "clarify" | "suggest" | "enhance" | "initialize";

/**
 * Snapshot of a question's answer at the time of the last enhance (synthesis) run.
 */
export interface QuestionLastSynthesis {
  answer: string;
  round: number;
}

/**
 * Snapshot of a suggestion's decision at the time of the last enhance (synthesis) run.
 */
export interface SuggestionLastSynthesis {
...
export interface IdeaClarificationQuestion {
  ...
  options?: string[];
  answer?: string;
  lastSynthesis?: QuestionLastSynthesis;
}
