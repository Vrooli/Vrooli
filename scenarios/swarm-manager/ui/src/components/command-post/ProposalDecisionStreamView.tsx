...
export function ProposalDecisionStreamView({ proposals, onComplete, onBack, onSnooze }: Props) {
  ...
  return <div className="flex h-full flex-col" data-testid="proposal-decision-stream">
    <div className="flex shrink-0 items-center gap-2 border-b border-slate-700/50 px-3"><GitPullRequestArrow className="h-4 w-4 shrink-0 text-violet-300" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-slate-200">{targetName}</p>{subtitle ? <p className="truncate text-xs text-slate-500">{subtitle}</p> : null}</div><span className="text-xs tabular-nums text-slate-500">{safeIndex + 1}/{active.length}</span></div>
