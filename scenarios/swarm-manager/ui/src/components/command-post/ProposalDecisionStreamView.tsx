...
interface Props { proposals: ProposalDecisionStreamItem[]; onComplete: () => void; onBack: () => void; onSnooze: (id: string) => void; /** Navigate to the attached backlog item / goal detail view. */ onOpenItem?: () => void; }

/** First-class proposal cards for the decision stream, not a panel below it. */
export function ProposalDecisionStreamView({ proposals, onComplete, onBack, onSnooze, onOpenItem }: Props) {
  ...
  return <div className="flex h-full flex-col" data-testid="proposal-decision-stream">
    <div className="flex shrink-0 items-center gap-2 border-b border-slate-800 px-3"><GitPullRequestArrow className="h-4 w-4 shrink-0 text-violet-300" /><div className="min-w-0 flex-1">{onOpenItem ? <button type="button" onClick={onOpenItem} className="block max-w-full cursor-pointer truncate text-left text-sm font-medium text-cyan-300 hover:text-cyan-200 hover:underline">{targetName}</button> : <p className="truncate text-sm font-medium text-slate-200">{targetName}</p>}{subtitle ? <p className="truncate text-xs text-slate-500">{subtitle}</p> : null}</div><span className="text-xs tabular-nums text-slate-500">{safeIndex + 1}/{active.length}</span></div>
