...
function DetailSummary({ entry, detail }: { entry: WorkFeedEntry; detail: EpisodeDetail }) {
  ...
  return <div className="rounded-lg border border-white/10 bg-slate-950/30 p-3"><p className="text-xs font-medium text-slate-300">Source detail</p><dl className="mt-2 grid gap-2 text-xs text-slate-400">{status ? <div><dt className="text-slate-500">Status</dt><dd className="mt-0.5 capitalize text-slate-200">{status}</dd></div> : null}{entry.kind === "execution" && ["completed", "needs_fixup"].includes(status ?? "") ? <div><dt className="text-slate-500">Meaning</dt><dd className="mt-0.5 text-slate-200">Agent session ended; product acceptance is separate.</dd></div> : null}{entry.kind === "review" && typeof detail.agent_assessment === "string" ? <div><dt className="text-slate-500">Assessment</dt><dd className="mt-0.5 text-slate-200">{detail.agent_assessment}</dd></div> : null}{entry.kind === "execution" && typeof detail.strategy === "string" ? <div><dt className="text-slate-500">Strategy</dt><dd className="mt-0.5 text-slate-200">{detail.strategy}</dd></div> : null}</dl></div>;
}

export function WorkFeedList({ entries, loading, error, agentManagerUrl }: { entries: WorkFeedEntry[]; loading: boolean; error: Error | null; agentManagerUrl?: string | null }) {
