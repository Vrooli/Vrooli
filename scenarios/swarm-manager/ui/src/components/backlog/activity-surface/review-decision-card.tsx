...
export function ReviewDecisionCard({
  ...
  return <section className="rounded-xl border border-violet-400/25 bg-violet-400/[0.06] p-4" aria-label="Review decision">
    ...
    {criteria.length === 0 ? <p className="mt-3 rounded border border-amber-300/25 bg-amber-300/[0.08] p-2 text-xs text-amber-100">This item has no typed criteria. Define criteria before relying on this review as a definition of done.</p> : <div className="mt-3 overflow-hidden rounded border border-slate-700"><table className="w-full text-left text-xs"><thead className="bg-slate-900 text-slate-300"><tr><th className="p-2">Criterion</th><th className="p-2">Settlement</th><th className="p-2">Evidence</th></tr></thead><tbody>{criteria.map((criterion) => { const related = evidence.filter((item) => item.criterion_id === criterion.id); const settlement = related.length === 0 ? "unsettled" : related.map((item) => item.settlement || "pending").join(", "); return <tr key={criterion.id} className="border-t border-slate-700 align-top"><td className="p-2 text-slate-100">{criterion.gherkin}</td><td className="p-2 capitalize text-slate-300">{settlement}</td><td className="p-2 text-slate-300">{related.length}</td></tr>; })}</tbody></table></div>}
