...
export function RunSheet({ isOpen, onClose, target, targets, onSuccess }: RunSheetProps) {
  ...
  const [blockerRepairPolicy, setBlockerRepairPolicy] = useState<"in_scope_only" | "investigate" | "repair_and_continue">("in_scope_only");
  ...
  useEffect(() => {
    if (!isOpen) return;
    let active = true;
    setError(null); setForce(false); setBlockerRepairPolicy("in_scope_only"); setStalePlanFor(null); setPreflight(null); setPreviews([]); setStrategies([]); setExecutionOptions([]); setStrategy(""); setMaxSlices(6); setPreferredRunner(""); setModel(""); setEffort(""); setLoading(true);
    ...
    void Promise.all([
      ...
      const savedStrategy = preview?.item?.executionMode;
  ...
  const isGoalSession = strategy === "goal";
  ...
  const queue = async () => {
    ...
    try {
      ...
      for (const item of effectiveTargets) {
        last = await backlogService.queue(item.kind, item.name, { mode: "yolo", startedBy: "swarm-manager-ui", confirm: true, force, ...(!isBulk ? { strategy, maxSlices, blockerRepairPolicy, ...((preferredRunner || model || effort) ? { preferredRunner, model, effort } : {}) } : {}) });
      }
  ...
  return <Drawer isOpen={isOpen} onClose={onClose} title={title} description="Review the approach and guardrails before starting work." className="md:w-[520px]" testId="run-sheet" footer={<div className="flex items-center justify-between gap-3"><p className="hidden text-xs text-slate-500 sm:block">Starts immediately after you confirm.</p><div className="ml-auto flex justify-end gap-2"><Button variant="outline" onClick={onClose} disabled={submitting}>Cancel</Button><Button onClick={() => void queue()} disabled={loading || submitting || effectiveBlocked || (!isBulk && !strategy) || previews.length !== effectiveTargets.length || effectiveTargets.length === 0}>{submitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Starting…</> : <><Play className="h-4 w-4 fill-current" />Start run</>}</Button></div></div>}>
    <div className="space-y-5 p-4">
      ...
      {!isBulk ? <section className="rounded-xl border border-amber-300/20 bg-amber-300/[0.05] p-4" aria-label="External blocker response"><h3 className="text-sm font-semibold text-amber-100">External blocker response</h3><p className="mt-1 text-xs leading-5 text-slate-300">Choose what the agent may do if a required producer or infrastructure dependency—such as Browser Automation Studio—fails.</p><div className="mt-3 space-y-2">{([[
        "in_scope_only", "Original scope only", "Stop and report when repair requires an unapproved boundary change."
      ], ["investigate", "Investigate, then ask", "Diagnose the external cause and preserve evidence, but do not edit it without an approved extension."], ["repair_and_continue", "Authorize repair and continue", "Expected to root-cause, safely repair, validate, record the boundary extension, and continue when needed."]] as const).map(([value, label, description]) => <label key={value} className={`block cursor-pointer rounded-lg border p-3 ${blockerRepairPolicy === value ? "border-amber-200/70 bg-amber-200/[0.10]" : "border-white/10 bg-slate-950/20"}`}><input className="sr-only" type="radio" name="blocker-repair-policy" value={value} checked={blockerRepairPolicy === value} onChange={() => setBlockerRepairPolicy(value)} /><span className="font-medium text-slate-100">{label}</span><span className="mt-1 block text-xs leading-5 text-slate-400">{description}</span></label>)}</div><p className="mt-3 text-[11px] leading-4 text-slate-500">The repair option does not override explicit denies or authorize destructive, paid, private, credential-gated, or ambiguous actions. Its decision and instructions are saved with the run.</p></section> : null}
      {!isBulk && (strategy === "sliced" || strategy === "goal") ? <section className="rounded-xl border border-amber-300/15 bg-amber-300/[0.04] p-4" aria-label="Unattended operation"><h3 className="text-sm font-semibold text-amber-100">Unattended operation</h3><p className="mt-1 text-xs leading-5 text-slate-300">These reviewed item settings control continuation after this run reaches its cap. The dialog displays them; it does not change them.</p><dl className="mt-3 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-slate-500">Continuation</dt><dd className="mt-1 font-medium text-slate-200">{preflight?.item?.continuation || "manual"}</dd></div><div><dt className="text-slate-500">Scope policy</dt><dd className="mt-1 font-medium text-slate-200">{preflight?.item?.scopePolicy || "fixed"}</dd></div></dl><p className="mt-3 text-xs text-slate-400">A budget-exhausted run records its handoff; “until allowance” may start a fresh child run while the allowance and scope remain valid.</p>{itemHref ? <a className="mt-3 inline-flex text-xs font-medium text-cyan-300 hover:text-cyan-200" href={itemHref} onClick={onClose}>Edit on the item <span aria-hidden="true">→</span></a> : null}</section> : null}
