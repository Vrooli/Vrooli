...
describe("BacklogFormDetailsSection execution contract", () => {
  beforeEach(() => {
    mockGet.mockResolvedValue({ items: [
      { id: "sliced", display_name: "Phased plan drain" },
      { id: "sliced", display_name: "Adaptive improvement" },
      { id: "goal", display_name: "Goal session" },
    ] });
  });
  ...
  it("renders reviewed strategy, limits, continuation, and scope fields", async () => {
    ...
    expect(screen.getByRole("combobox", { name: "Execution strategy" })).toHaveValue("sliced");
    ...
    fireEvent.change(screen.getByRole("combobox", { name: "Execution strategy" }), { target: { value: "goal" } });
    expect(onFieldChange).toHaveBeenCalledWith("executionMode", "goal");
