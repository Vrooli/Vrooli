...
function RequirementGroupNode({ group, depth = 0 }: { group: ArchiveRequirementGroup; depth?: number }) {
      ...
      {expanded && (
        <div className="mt-1 space-y-1">
          {group.requirements.map((req) => (
            <div
              key={req.id}
              className="flex items-start gap-2 rounded-md px-2 py-1.5 text-sm hover:bg-slate-800/30"
            >
          ...
          {group.children.map((child) => (
            <RequirementGroupNode key={child.id} group={child} depth={depth + 1} />
          ))}
        </div>
      )}
    </div>
...
export function OperationalTargetsPanel({ targets, requirements }: OperationalTargetsPanelProps) {
