...
7. Add the typed `Source` enum (per D8) to `scenarios/workspace-sandbox/api/internal/types/types.go`: ∈ {`agent-manager-auto-apply`, `git-control-tower`, `agent-manager-ui`, `workspace-sandbox-ui`, `cli`, `workspace-sandbox-gc`}. The `workspace-sandbox-gc` value is system-initiated only — emitted by the GC auto-deny path (Phase 4) and rejected if it appears on an inbound `ApprovalRequest` or `ApplyAtRunEndRequest`. Add the `Source` field to existing `ApprovalRequest` and audit-event types so audit queries don't fork; `Actor` continues to carry user/system identity classification (unchanged).
...
14. Add a `pending-review` branch to `LifecycleReconciler.ReconcileLifecycle` (lifecycle.go:76). At each tick (default 15 min, `WORKSPACE_SANDBOX_GC_INTERVAL`), find sandboxes whose status is `pending-review` and where `now - run_end_at > LifecycleConfig.ManualReviewTTL`. For each:
    ...
    c. Emit an audit event with `source=workspace-sandbox-gc` (the system-initiated value of the typed `Source` enum, per D8 + Phase 2 step 7) and `actor=manual-review-ttl-expiry`.
...
| D8 | Originating-approval-surface field shape (typed `Source` enum vs Actor prefix vs ActorType reuse) | **Resolved (R2) — A: typed `Source` enum on `ApprovalRequest`, `ApplyAtRunEndRequest`, and audit events ∈ {`agent-manager-auto-apply`, `git-control-tower`, `agent-manager-ui`, `workspace-sandbox-ui`, `cli`, `workspace-sandbox-gc`} (last value is system-initiated only)** |
...
| 13 | Typed `Source` enum round-trips through `ApprovalRequest`, `ApplyAtRunEndRequest`, and audit events; unknown values rejected at decode; inbound requests carrying the system-initiated `workspace-sandbox-gc` value are rejected | `types_test.go` + `handlers_test.go` | Unit |
...
2. `POST /api/v1/sandboxes/{id}/apply-at-run-end` (per D6) accepts and persists `agent_manager_run_id`, `conversation_id`, `cost`, `runOutcome` (run group) and per-file `state`. Typed `Source` enum (per D8) carries the originating surface on `ApprovalRequest`, `ApplyAtRunEndRequest`, and audit events; the system-initiated `workspace-sandbox-gc` value is rejected on inbound requests.
