...
func (h *Handler) RegisterRoutes(r *mux.Router) {
	...
	r.HandleFunc("/api/v1/goal-candidates", h.GoalCandidates).Methods(http.MethodGet)
}
...
func (h *Handler) GoalCandidates(w http.ResponseWriter, r *http.Request) {
	purpose := strings.TrimSpace(r.URL.Query().Get("purpose"))
	if purpose != "" && purpose != "next-action" {
		apierr.MapError(w, "[session-context] goal candidates", apierr.BadRequest("unsupported purpose %q", purpose))
		return
	}
	h.writeStartupBrief(w, r, agentsessions.KindMetaOrchestration, "[session-context] goal candidates")
}
