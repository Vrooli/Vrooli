...
func (h *Handler) ListRounds(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputil.InternalError(w, "[review]", fmt.Sprintf("list review rounds: %v", err))
		return
	}
	if rounds == nil {
		rounds = []Round{}
	...
	httputil.JSONWithStatus(w, http.StatusOK, map[string]any{"rounds": rounds})
}
...
func (h *Handler) GetCapture(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputil.NotFound(w, "[review]", fmt.Sprintf("capture not found: %v", err))
		return
	}
...
func (h *Handler) VerifyEvidence(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputil.BadRequest(w, "[review]", "invalid round number")
		return
	}
	...
	if err := readJSON(r, &body); err != nil {
		httputil.BadRequest(w, "[review]", fmt.Sprintf("invalid request body: %v", err))
		return
	}

	if err := h.service.VerifyEvidence(kind, name, roundNum, evidenceID, body.Verified, body.ExecutionID); err != nil {
		httputil.InternalError(w, "[review]", fmt.Sprintf("verify evidence: %v", err))
		return
	}

	httputil.JSONWithStatus(w, http.StatusOK, map[string]any{"ok": true})
}
...
func (h *Handler) RequestMoreEvidence(w http.ResponseWriter, r *http.Request) {
	...
	if body.Message == "" {
		httputil.BadRequest(w, "[review]", "message is required")
		return
	}

	threadID, err := h.service.RequestMoreEvidence(r.Context(), kind, name, roundNum, body.Message, body.EvidenceID)
	if err != nil {
		httputil.InternalError(w, "[review]", fmt.Sprintf("request more evidence: %v", err))
		return
	}

	httputil.JSONWithStatus(w, http.StatusCreated, map[string]any{"thread_id": threadID})
}
...
func (h *Handler) ContinueRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.service.ContinueRequest(kind, name, roundNum, threadID, body.Message); err != nil {
		httputil.InternalError(w, "[review]", fmt.Sprintf("continue request: %v", err))
		return
	}
...
func (h *Handler) DismissRequest(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.service.DismissRequest(kind, name, roundNum, threadID); err != nil {
		httputil.InternalError(w, "[review]", fmt.Sprintf("dismiss request: %v", err))
		return
	}
...
func (h *Handler) TriggerReviewAgent(w http.ResponseWriter, r *http.Request) {
	...
	if body.BacklogKind == "" || body.BacklogName == "" {
		httputil.BadRequest(w, "[review]", "backlog_kind and backlog_name are required")
		return
	}
	...
	if err := h.service.TriggerReviewAgent(r.Context(), body.BacklogKind, body.BacklogName, executionID, body.AffectedScenarios); err != nil {
		httputil.InternalError(w, "[review]", fmt.Sprintf("trigger review agent: %v", err))
		return
	}

	httputil.JSONWithStatus(w, http.StatusAccepted, map[string]any{"ok": true})
}
