...
func TestCompositeMutationProcessorDerivesMilestoneFromGoalMembership(t *testing.T) {
	root := t.TempDir()
	store := backlog.NewFileStore(root)
	goalService := goals.NewService(goals.NewStore(root), store)
	if _, err := goalService.Create(goals.CreateRequest{Name: "release", Title: "Release", Priority: 5}); err != nil {
	...
	item := backlog.BacklogItem{Name: "compile", Title: "Compile", Kind: backlog.KindExecute, Status: backlog.StatusBacklog, Priority: 3, Created: "2026-07-24T00:00:00Z", Updated: "2026-07-24T00:00:00Z"}
	if err := os.MkdirAll(store.ItemDir(item.Kind, item.Name), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := store.SaveItem(item); err != nil {
	...
	item, err := store.LoadItem(backlog.KindExecute, "compile")
	if err != nil {
		t.Fatal(err)
	}
	item.Milestone = ""
