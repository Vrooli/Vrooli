...
func (db *DB) initSchema() error {
	...
	if err := db.migrateColumns(ctx, "workflow_executions", []columnMigration{{column: "parent_attempt_id", ddl: "ALTER TABLE workflow_executions ADD COLUMN parent_attempt_id TEXT"}, {column: "depth", ddl: "ALTER TABLE workflow_executions ADD COLUMN depth INTEGER NOT NULL DEFAULT 0"}, {column: "engagement_grant_json", ddl: "ALTER TABLE workflow_executions ADD COLUMN engagement_grant_json TEXT"}, {column: "approval_digest", ddl: "ALTER TABLE workflow_executions ADD COLUMN approval_digest TEXT NOT NULL DEFAULT ''"}, {column: "grant_digest", ddl: "ALTER TABLE workflow_executions ADD COLUMN grant_digest TEXT NOT NULL DEFAULT ''"}}); err != nil {
		return err
	}
