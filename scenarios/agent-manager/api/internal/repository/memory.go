...
// MemoryInvestigationRepository is an in-memory implementation of InvestigationRepository.
type MemoryInvestigationRepository struct {
	mu             sync.RWMutex
	investigations map[uuid.UUID]*domain.Investigation
}

// NewMemoryInvestigationRepository creates a new in-memory investigation repository.
func NewMemoryInvestigationRepository() *MemoryInvestigationRepository {
	return &MemoryInvestigationRepository{
		investigations: make(map[uuid.UUID]*domain.Investigation),
	}
}

func (r *MemoryInvestigationRepository) Create(ctx context.Context, investigation *domain.Investigation) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	if _, exists := r.investigations[investigation.ID]; exists {
		return domain.NewValidationErrorWithCode("investigation.id", "already exists", domain.ErrCodeValidationConflict)
	}

	copy := *investigation
	r.investigations[investigation.ID] = &copy
	return nil
}

func (r *MemoryInvestigationRepository) Get(ctx context.Context, id uuid.UUID) (*domain.Investigation, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()

	inv, exists := r.investigations[id]
	if !exists {
...
func (r *MemoryInvestigationRepository) List(ctx context.Context, filter InvestigationListFilter) ([]*domain.Investigation, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	for _, inv := range r.investigations {
		...
		if filter.SourceInvestigationID != nil {
			if inv.SourceInvestigationID == nil || *inv.SourceInvestigationID != *filter.SourceInvestigationID {
				continue
			}
		}
...
func (r *MemoryInvestigationRepository) GetActive(ctx context.Context) (*domain.Investigation, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *MemoryInvestigationRepository) Update(ctx context.Context, investigation *domain.Investigation) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *MemoryInvestigationRepository) UpdateStatus(ctx context.Context, id uuid.UUID, status domain.InvestigationStatus, errorMsg string) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *MemoryInvestigationRepository) UpdateProgress(ctx context.Context, id uuid.UUID, progress int) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *MemoryInvestigationRepository) UpdateFindings(ctx context.Context, id uuid.UUID, findings *domain.InvestigationReport, metrics *domain.MetricsData) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *MemoryInvestigationRepository) Delete(ctx context.Context, id uuid.UUID) error {
	r.mu.Lock()
	defer r.mu.Unlock()

	if _, exists := r.investigations[id]; !exists {
		return domain.NewNotFoundError("Investigation", id)
	}

	delete(r.investigations, id)
	return nil
}

// Verify interface compliance
var _ InvestigationRepository = (*MemoryInvestigationRepository)(nil)
