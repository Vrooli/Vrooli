...
const DiagnosticCodeCatalogInvalid = "PERMISSION_POLICY_CATALOG_INVALID"
...
// State atomically activates complete desired-permission revisions. A failed
// reload leaves the last known-good revision active and exposes the failure.
type State struct {
	mu          sync.RWMutex
	path        string
...
func (s *State) Validate() (*Revision, error) {
	if s == nil {
		return nil, fmt.Errorf("permission policy state is not configured")
	}
	return Load(s.path)
}
...
func (s *State) ReadinessError() error {
	...
	if reason == "" {
		reason = "desired coding-agent permissions are required"
	}
	...
	return fmt.Errorf("required permission policy catalog at %s is not ready (%s): %s", status.Path, reason, message)
}
