...
func TestResolvePathCanonicalizesContractDescendant(t *testing.T) {
	root := newModelPolicyContractFixtureRepo(t)
	nested := filepath.Join(root, "scenarios", "agent-manager", "api")
	if err := os.MkdirAll(nested, 0o755); err != nil {
		t.Fatalf("mkdir nested: %v", err)
	}

	t.Setenv("AGENT_MANAGER_MODEL_POLICY_CATALOG_PATH", "")
	t.Setenv("VROOLI_SOURCE_ROOT", nested)
	t.Setenv("VROOLI_ROOT", "")

	got := ResolvePath()
	want := filepath.Join(root, "scenarios", "agent-manager", "config", "model-policy-catalog.json")
	if got != want {
		t.Fatalf("ResolvePath() = %q, want %q", got, want)
	}
}

func newModelPolicyContractFixtureRepo(t *testing.T) string {
	t.Helper()

	root := t.TempDir()
	repoRoot := modelPolicyRepoRoot(t)
	...
	if err := os.WriteFile(filepath.Join(root, "go.mod"), []byte("module example.com/agent-manager-modelpolicy-test\n\ngo 1.24.0\n"), 0o644); err != nil {
...
func modelPolicyRepoRoot(t *testing.T) string {
	t.Helper()
