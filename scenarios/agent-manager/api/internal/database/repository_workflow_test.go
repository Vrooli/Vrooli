...
func TestWorkflowExecutionRepositoryCASAndJournalSurviveReload(t *testing.T) {
	...
	attempt := &domain.WorkflowNodeAttempt{ID: uuid.New(), ExecutionID: execution.ID, NodeID: "start", Ordinal: 1, Strategy: domain.WorkflowAttemptChild, Status: domain.WorkflowAttemptDispatchPending, IdempotencyKey: "attempt-once", InputSnapshot: json.RawMessage(`{}`), PromptSnapshot: "secret prompt persisted, never logged", ChildExecutionID: &childExecutionID, Version: 1, CreatedAt: now, UpdatedAt: now}
	execution.Version = 2
	...
	if err != nil || len(attempts) != 1 || attempts[0].PromptSnapshot != attempt.PromptSnapshot || attempts[0].ChildExecutionID == nil || *attempts[0].ChildExecutionID != childExecutionID {
