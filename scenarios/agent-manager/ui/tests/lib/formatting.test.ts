...
test("formatRelativeTimeShort uses relative labels for recent timestamps", () => {
  ...
  assert.equal(
    formatRelativeTimeShort("2026-02-07T19:59:30.000Z", { now }),
    "just now"
  );
  assert.equal(
    formatRelativeTimeShort("2026-02-07T19:58:00.000Z", { now }),
    "2m ago"
  );
  assert.equal(
    formatRelativeTimeShort("2026-02-07T18:00:00.000Z", { now }),
    "2h ago"
  );
  assert.equal(
    formatRelativeTimeShort("2026-02-05T20:00:00.000Z", { now }),
    "2d ago"
  );
});
