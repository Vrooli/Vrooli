...
test("run controls encode inspection, investigation, review, continuation, and lifecycle requests", async () => {
  const fetch = vi.fn(async (url: string) => {
    ...
    if (url.endsWith("/investigations")) return json({ investigation: { investigationId: "investigation-a", operationStatus: "accepted" } });
  ...
  await act(async () => {
    ...
    await runs.result.current.startTypedInvestigation(["run-a"], "focus failures", "deep");
  ...
  assert.equal(request("/investigations")?.[1]?.method, "POST");
  assert.match(String(request("/investigations")?.[1]?.body), /"schemaVersion":"investigation-request\/v1"/);
  assert.match(String(request("/investigations")?.[1]?.body), /"allowSubjectMutation":false/);
