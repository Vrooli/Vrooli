...
- Event retention selects expired candidates through the event owner's timestamp index before acquiring SQLite's writer. The short deletion transaction rechecks each stable event ID, cutoff, completed projection and non-imported execution mode; only actual deletions advance the retention generation. A row limit alone does not bound a scan inside a write transaction. An event whose run row no longer exists is deleted at the cutoff without a completed projection, because nothing can project a deleted run.
- Imported runs are the conversation-recall corpus and are never age-deleted. `SQLiteStore.CompactImportedToolPayloads` (the `event.PayloadCompactor` seam, driven by the reconciler) instead rewrites their `tool_call`/`tool_result` payloads older than `ImportedToolCompactionDays` and larger than `ImportedToolCompactionMinBytes` to a head and tail excerpt with the original byte count and sha256. It scans a bounded rowid window per tick, rechecks the selected bytes in short write transactions, and queues an `upsert_run` conversation-search change for each rewritten event in the same transaction. Prose is never compacted.
