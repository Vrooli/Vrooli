...
INSERT INTO {{ scenario_id }}.app_metadata (
...
INSERT INTO {{ scenario_id }}.app_config (config_key, config_value, config_type, description) VALUES 
    ...
    ('feature_analytics_dashboard', '{{ "true" if "questdb" in resources.required else "false" }}'::jsonb, 'feature_flag', 'Enable analytics dashboard UI'),
    ...
    ('app_name', '"{{ scenario.name }}"'::jsonb, 'setting', 'Application display name'),
...
INSERT INTO {{ scenario_id }}.activity_log (event_type, event_data, session_id, user_identifier, status) VALUES
    ('ai_query_processed', 
...
SELECT 'Application metadata:' as info, COUNT(*) as count FROM {{ scenario_id }}.app_metadata
UNION ALL
SELECT 'Configuration entries:', COUNT(*) FROM {{ scenario_id }}.app_config
UNION ALL
SELECT 'Sample sessions:', COUNT(*) FROM {{ scenario_id }}.user_sessions
UNION ALL
SELECT 'Activity log entries:', COUNT(*) FROM {{ scenario_id }}.activity_log
UNION ALL
SELECT 'Resource metrics:', COUNT(*) FROM {{ scenario_id }}.resource_metrics;
