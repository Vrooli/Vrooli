...
if command -v node >/dev/null 2>&1; then
    ...
    testing::phase::add_warning "⚠️  Node.js runtime not found (UI tests will be skipped)"
fi
...
if command -v resource-postgres >/dev/null 2>&1; then
    if resource-postgres status >/dev/null 2>&1; then
        log::success "✅ PostgreSQL resource available"
    else
        testing::phase::add_error "❌ PostgreSQL resource not running"
    fi
else
    testing::phase::add_warning "⚠️  PostgreSQL resource CLI not available"
fi
