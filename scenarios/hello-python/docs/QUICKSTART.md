...
```bash
hello-python status
hello-python <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port hello-python API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.hello_python.v1.<domain>.<Service>/<Method>
```
...
hello-python notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port hello-python API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.hello_python.v1.notes.NotesService/ListNotes" \
