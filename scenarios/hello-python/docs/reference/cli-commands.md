...
`~/.vrooli/bin/hello-python`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start hello-python` if the API is unreachable |
...
```bash
hello-python status
hello-python status --json
```

### `hello-python configure <key> <value>`
...
```bash
hello-python configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`hello-python <domain> <verb>`). Every command calls a single API
...
```bash
hello-python notes list
hello-python notes list --json
```

#### `hello-python notes create --title <title> [--body <body>]`
...
```bash
hello-python notes create --title "First note" --body "Hello world"
```
...
```bash
hello-python notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start hello-python`" is good.
