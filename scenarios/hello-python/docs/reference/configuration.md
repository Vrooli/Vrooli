...
For `hello-python` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/hello-python/config.json`
3. `~/.vrooli/config/hello-python/config.json`
4. `~/.config/vrooli/hello-python/config.json`
...
```bash
hello-python configure api_base http://localhost:15001/api/v1
hello-python configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port hello-python API_PORT`)
