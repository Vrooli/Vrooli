...
envelope (`packages/proto/schemas/hello-python/v1/shared/errors.proto`):
...
| **CLI** | `hello-python status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/hello-python/v1/shared/health.proto`
...
`POST /vrooli.hello_python.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.hello_python.v1.notes.NotesService/ListNotes`
...
| **CLI** | `hello-python notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.hello_python.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.hello_python.v1.notes.NotesService/CreateNote`
...
| **CLI** | `hello-python notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.hello_python.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.hello_python.v1.notes.NotesService/GetNote`
...
| **CLI** | `hello-python notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.hello_python.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `hello-python notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/hello-python/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/hello-python/v1/<domain>/`, then run
   `make generate`.
