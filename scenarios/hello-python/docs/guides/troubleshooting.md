...
```bash
vrooli scenario status hello-python
make stop
vrooli scenario start hello-python --clean-stale
```
...
```bash
make status
vrooli scenario port hello-python UI_PORT
```
...
hello-python status --auto-start

# Or override the API base for this invocation
hello-python status --api-base "http://localhost:$(vrooli scenario port hello-python API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/hello-python/`.
...
The default route is `${SCENARIO_DATA_DIR}/hello-python.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/hello-python.db"
```
...
`packages/proto/gen/go/hello-python/v1/`, TypeScript under
`packages/proto/gen/typescript/hello-python/v1/`, and Python under
`packages/proto/gen/python/hello_python/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/hello-python/v1/<domain>/`.
