...
`template-manager orient hello-python`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient hello-python
...
```bash
cat > /tmp/prd_context_hello-python.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start hello-python --interactive
# …answer the interview, then:
business-health wizard preview hello-python
business-health wizard apply hello-python
```
...
```bash
vrooli scenario requirements validate hello-python --json
```
...
```bash
cat > /tmp/requirements_context_hello-python.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate hello-python --json
business-health fix preview hello-python
business-health fix apply hello-python
```
...
```bash
react-component-library adoptions suggest hello-python --json
react-component-library adoptions link <component-id> hello-python
react-component-library adoptions obligations hello-python --json
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate hello-python --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate hello-python`, the notes page spec
...
validate hello-python --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/hello-python/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest hello-python <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> hello-python
