...
```bash
vrooli scenario status image-tools
make stop
vrooli scenario start image-tools --clean-stale
```
...
image-tools status --auto-start

# Or override the API base for this invocation
image-tools status --api-base "http://localhost:$(vrooli scenario port image-tools API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/image-tools/`.
...
The default route is `${SCENARIO_DATA_DIR}/image-tools.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/image-tools.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/image-tools.db}")"
```
...
`packages/proto/gen/go/image-tools/v1/`, TypeScript under
`packages/proto/gen/typescript/js/image-tools/v1/`, and Python under
`packages/proto/gen/python/image_tools/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/image-tools/v1/<domain>/`.
