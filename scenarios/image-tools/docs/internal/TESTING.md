...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/image-tools/v1/health/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/image-tools/v1/health"`)
...
import (
    ...
    "image-tools/handlers/health"
    "image-tools/internal/clock"
    "image-tools/internal/module"
    "image-tools/internal/server"
    "image-tools/internal/testutil/assertx"
    "image-tools/internal/testutil/httpx"
    "image-tools/internal/testutil/mocks"
)
...
The proto schema in `packages/proto/schemas/image-tools/v1/health/health.proto`
...
dynamically from `go.mod`, so it works after `image-tools`
substitution.
...
import (
    ...
    clitest "image-tools/cli/internal/testutil"
)
...
1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/image-tools/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.image_tools.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/image-tools/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/image-tools/v1/<domain>/`,
   `packages/proto/gen/typescript/js/image-tools/v1/<domain>/`, and
   `packages/proto/gen/python/image_tools/v1/<domain>/`.
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import notesv1 "github.com/vrooli/vrooli/packages/proto/gen/go/image-tools/v1/notes"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/image-tools/v1/notes/notes_pb";
