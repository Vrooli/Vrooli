...
| GPU not detected | system-monitor probe output, API logs for fallback messages | Expected to fall back to the CPU-capable default with a visible warning; verify NVIDIA drivers if GPU is expected. Op still completes on CPU (slower). |
