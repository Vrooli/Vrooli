...
`template-manager orient image-tools`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient image-tools
...
```bash
cat > /tmp/prd_context_image-tools.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) image-tools \
  --context-file /tmp/prd_context_image-tools.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate image-tools --json
```
...
```bash
cat > /tmp/requirements_context_image-tools.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) image-tools \
  --context-file /tmp/requirements_context_image-tools.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/image-tools/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest image-tools <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> image-tools
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario image-tools
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> image-tools <adopted-path>
```
