...
`vrooli scenario orient image-tools`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient image-tools
...
```bash
prd-control-tower prd generate image-tools \
...
```bash
prd-control-tower prd validate image-tools --json
```
...
```bash
prd-control-tower requirements generate image-tools \
...
```bash
prd-control-tower requirements validate image-tools --json
```
