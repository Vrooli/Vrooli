...
func (h *handlers) runCommands() []cliapp.Command {
	...
	commands := []cliapp.Command{
		...
		cmd("rasterize", "Rasterize an SVG at an exact pixel size", h.rasterizeParams,
			cliapp.Flag{Name: "width", Description: "Target width in px"},
			cliapp.Flag{Name: "height", Description: "Target height in px"},
			cliapp.Flag{Name: "background", Description: "Flatten onto this hex colour"},
		),
		cmd("vectorize", "Trace a flat raster mark into a clean SVG", h.vectorizeParams,
			cliapp.Flag{Name: "colors", Description: "Palette entries for k-means (default 4)"},
			cliapp.Flag{Name: "keep-color", Repeated: true, Description: "Pin a layer to this hex colour (repeatable); assigns within delta-E 12"},
			cliapp.Flag{Name: "drop-background-layers", Bool: true, Description: "Drop the layer touching the image border"},
			cliapp.Flag{Name: "clip-to-largest-rounded-region", Bool: true, Description: "Clip to the enclosing tile hull"},
			cliapp.Flag{Name: "inset-px", Description: "Clip erosion in px (default 2% of the short edge)"},
			cliapp.Flag{Name: "tolerance-px", Description: "Simplification tolerance in px (default 0.8)"},
			cliapp.Flag{Name: "smoothing", Bool: true, Description: "Fit cubic Beziers to the simplified contours"},
			cliapp.Flag{Name: "min-area-px", Description: "Drop contours smaller than this area (default 40)"},
		),
		cmd("icon_container", "Pack PNG renders into an ICO or ICNS container", h.iconContainerParams,
			cliapp.Flag{Name: "format", Description: "ico|icns", Values: []string{"ico", "icns"}},
			cliapp.Flag{Name: "size", Repeated: true, Description: "Exact pixel size (repeatable)"},
		),
	}
