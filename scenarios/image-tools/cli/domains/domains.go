...
import (
	"image-tools/cli/domains/adapters"
	...
	"image-tools/cli/domains/safety"
	"image-tools/cli/domains/selection"
...
func SubcommandGroups(core *cliapp.ScenarioApp, manifest []byte) ([]cliapp.SubcommandGroup, error) {
	...
	adaptersGroup, err := adapters.Register(core, manifest)
	if err != nil {
		return nil, err
	}
	...
	analyzeGroup, err := analyze.Register(core, manifest)
	if err != nil {
		return nil, err
	}
	...
	selectGroup, err := selection.Register(core, manifest)
	if err != nil {
		return nil, err
	}
	diffGroup, err := diff.Register(core, manifest)
	if err != nil {
		return nil, err
	}
	...
	return []cliapp.SubcommandGroup{jobsGroup, modelsGroup, adaptersGroup, backendsGroup, opsGroup, aiGroup, analyzeGroup, settingsGroup, looksGroup, selectGroup, diffGroup, safetyGroup}, nil
}
