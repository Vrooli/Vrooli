...
|---|---|---|---|
| **Local single-user** | **active / primary** | Vrooli lifecycle, Go toolchain, Node/pnpm for the UI build, writable SQLite path | A real domain, not the scaffold example, must be implemented and validated first. |
...
| External services | **None required.** SQLite is in-process. A configured AI/model provider is optional and only relevant at R2. |
...
| Printable output | Vector recipe and weekly PDFs plus shopping CSV (`UX-DAT-04`, `UX-REC-07`). PDFs are generated server-side with the real page size and content. |
...
- [ ] Provider-optional release: the app starts and all core flows work
      with no provider, no model, and no network.
...
- [`../reference/product-specification.md`](../reference/product-specification.md) — sections 11, 18, 19, and 22
