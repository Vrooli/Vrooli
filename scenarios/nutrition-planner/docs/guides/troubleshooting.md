# Troubleshooting — Nutrition Planner
...
make stop
vrooli scenario start nutrition-planner --clean-stale
```
...
nutrition-planner status --auto-start

# Or override the API base for this invocation
nutrition-planner status --api-base "http://localhost:$(vrooli scenario port nutrition-planner API_PORT)/api/v1"
```
...
Most commonly `~/.vrooli/config/nutrition-planner/config.json`. The
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/nutrition-planner/`.
...
The default route is `${SCENARIO_DATA_DIR}/nutrition-planner.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/nutrition-planner.db"
```
...
`packages/proto/gen/go/nutrition-planner/v1/`, TypeScript under
`packages/proto/gen/typescript/nutrition-planner/v1/`, and Python under
`packages/proto/gen/python/nutrition_planner/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/nutrition-planner/v1/<domain>/`.
