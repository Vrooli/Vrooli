# Configuration — Nutrition Planner
...
For `nutrition-planner` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/nutrition-planner/config.json`
3. `~/.vrooli/config/nutrition-planner/config.json`
4. `~/.config/vrooli/nutrition-planner/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port nutrition-planner API_PORT`)
