# API Endpoints — Nutrition Planner
...
envelope (`packages/proto/schemas/nutrition-planner/v1/shared/errors.proto`):
...
| **CLI** | `nutrition-planner status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/nutrition-planner/v1/shared/health.proto`
...
`POST /vrooli.nutrition_planner.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.nutrition_planner.v1.notes.NotesService/ListNotes`
...
| **CLI** | `nutrition-planner notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.nutrition_planner.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.nutrition_planner.v1.notes.NotesService/CreateNote`
...
| **CLI** | `nutrition-planner notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.nutrition_planner.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.nutrition_planner.v1.notes.NotesService/GetNote`
...
| **CLI** | `nutrition-planner notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.nutrition_planner.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `nutrition-planner notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/nutrition-planner/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/nutrition-planner/v1/<domain>/`, then run
   `make generate`.
