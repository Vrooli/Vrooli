# CLI Commands — Nutrition Planner
...
`~/.vrooli/bin/nutrition-planner`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start nutrition-planner` if the API is unreachable |
...
```bash
nutrition-planner status
nutrition-planner status --json
```

### `nutrition-planner configure <key> <value>`
...
```bash
nutrition-planner configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`nutrition-planner <domain> <verb>`). Every command calls a single API
...
```bash
nutrition-planner notes list
nutrition-planner notes list --json
```

#### `nutrition-planner notes create --title <title> [--body <body>]`
...
```bash
nutrition-planner notes create --title "First note" --body "Hello world"
```
...
```bash
nutrition-planner notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start nutrition-planner`" is good.
