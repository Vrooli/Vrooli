...
`template-manager orient nutrition-planner`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient nutrition-planner
...
```bash
cat > /tmp/prd_context_nutrition-planner.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start nutrition-planner --interactive
# …answer the interview, then:
business-health wizard preview nutrition-planner
business-health wizard apply nutrition-planner
```
...
```bash
cat > /tmp/requirements_context_nutrition-planner.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate nutrition-planner --json
business-health fix preview nutrition-planner
business-health fix apply nutrition-planner
```
...
run `template-manager detemplate nutrition-planner`, the notes page spec
...
validate nutrition-planner --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/nutrition-planner/v1/<domain>/<domain>.proto`
