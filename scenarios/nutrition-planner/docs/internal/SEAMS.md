...
The `health` proto in `packages/proto/schemas/nutrition-planner/v1/shared/`
...
|---|---|---|---|---|
| Provider HTTP (generic) | Reuse `internal/httpc/doer.go::Doer` for all outbound provider calls | `main.go` wires `&http.Client{Timeout: …}` into `server.Deps` | `internal/testutil/mocks::FakeDoer` (already exists) | One outbound boundary; providers never call `http.Get` directly |
| Nutrition provider (USDA FoodData Central) | `internal/providers/nutrition::Provider` (search, detail, source identity/version, mapping) | Provider module constructs the adapter over `Doer` | Recorded-response fake (contract tests never call the live API) | Provider absence must not block manual entry (`DATA-01`, `ACT-058`) |
| Price / receipt providers | `internal/providers/prices::Source`, `internal/providers/receipts::Parser` returning structured observations/proposals | Each source is an adapter behind one interface; credentials via the secret seam | Recorded-response + duplicate-transaction fake | Store-specific, opt-in, never load-bearing (`DATA-02`, `ACT-059`) |
| SSRF-safe URL fetcher | `internal/fetch::Fetcher` with scheme/redirect/private-address validation and size/time bounds | Controlled adapter, isolated outbound credentials | Fake returning canned pages; adversarial-URL tests | User-supplied URLs are untrusted and must not reach internal hosts (`AI-03`, `ACT-057`) |
...
- A boundary belongs here only if production-vs-test substitution matters. Pure
  calculation (nutrition arithmetic, scoring, unit conversion, graph
  validation) is **not** a seam and stays directly callable.
- The deterministic core must receive its inputs as snapshots (profile, prices,
  stock, feedback, seed) and return values, provenance, and structured reasons —
  never reach for a repository, clock, or network itself (`ARCH-02`, `PLN-01`).
