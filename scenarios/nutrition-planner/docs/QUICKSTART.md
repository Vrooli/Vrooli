...
```bash
nutrition-planner notes list
nutrition-planner notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port nutrition-planner API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.nutrition_planner.v1.notes.NotesService/ListNotes" \
