...
export function WeekPage() {
  ...
  const closeSwap = useCallback(() => { setSwapDate(""); setSwapPreview(undefined); }, []);
  ...
  function updateOccurrence(date: string, update: (occurrence: PlanDraft["occurrences"][number]) => PlanDraft["occurrences"][number]) {
    setDraft((current) => current ? { ...current, occurrences: current.occurrences.map((occurrence) => occurrence.date === date ? update(occurrence) : occurrence) } : current);
  }

  function toggleLock(date: string) {
    updateOccurrence(date, (occurrence) => ({ ...occurrence, locked: !occurrence.locked }));
  }

  function skipOccurrence(date: string) {
    updateOccurrence(date, (occurrence) => ({ ...occurrence, recipeId: "", recipeName: "", mode: "open", locked: false, reason: "Skipped by the user; this slot remains open." }));
  }
  ...
  async function saveWeek() {
    ...
    try {
      ...
      setDraft((current) => current ? { ...current, currentRevision: applied.revision } : current);
    } catch (err) {
  ...
  return <section data-testid={selectors.pages.week} aria-labelledby="week-heading" className="flex flex-col gap-6">
    ...
    {state === "ready" && <ol className="divide-y rounded-lg border border-slate-200 bg-white">{dates.map((date) => { const item = draft?.occurrences.find((occurrence) => occurrence.date === date); return <li key={date} className="grid gap-2 p-4 sm:grid-cols-[10rem_1fr_auto_auto_auto]"><time className="font-medium text-slate-700" dateTime={date}>{date}</time><span className="text-slate-900">{item?.recipeName ?? "Open dinner slot — unknown"}</span><span className="text-sm text-slate-600">{item?.locked ? "Locked" : item?.mode === "open" ? "Skipped / open" : item ? "Planned" : "Unresolved"}</span>{item && item.recipeId && <button type="button" className="min-h-10 rounded border px-3 text-sm" onClick={() => { setSwapDate(date); setReplacementId(""); setSwapPreview(undefined); }}>Swap meal</button>}{item && <><button type="button" className="min-h-10 rounded border px-3 text-sm" onClick={() => toggleLock(date)}>{item.locked ? "Unlock" : "Lock"}</button><button type="button" className="min-h-10 rounded border px-3 text-sm" onClick={() => skipOccurrence(date)} disabled={item.mode === "open"}>Skip</button></>}</li>; })}</ol>}
