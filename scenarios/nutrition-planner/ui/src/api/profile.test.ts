...
describe("profile API", () => {
  ...
  it("applies a profile and preserves the eligibility counts", async () => { const profile = create(ProfileSchema, { workspaceId: "w1" }); applyProfile.mockResolvedValue({ profile, matchingMeals: 3n, needsReviewMeals: 2n, excludedMeals: 4n }); const input = { workspaceId: "w1", preset: "vegan", excludedGroups: [], allergies: [], appliances: [], costWeight: .34, effortWeight: .33, varietyWeight: .33 }; await expect(apply(input)).resolves.toEqual({ profile, matchingMeals: 3n, needsReviewMeals: 2n, excludedMeals: 4n }); applyProfile.mockResolvedValue({}); await expect(apply(input)).rejects.toThrow("no applied profile"); });
});
