...
func Module(db *database.RoutedDB, clock schedule.Clock, logger *log.Logger) module.Module {
	return ModuleWithRepository(internal.NewSQLiteRepository(db), workspace.NewService(workspace.NewSQLiteRepository(db, clock)), logger)
}

func ModuleWithRepository(repo internal.Repository, ws workspace.Service, logger *log.Logger) module.Module {
	path, handler := connect.NewInventoryServiceHandler(NewConnectHandler(repo, ws, logger))
	return module.Module{Name: "inventory", Mount: func(r *mux.Router) { connectx.RegisterServices(r, connectx.ServiceMount{Path: path, Handler: handler}) }, Endpoints: Endpoints}
}
