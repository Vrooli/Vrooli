...
func Module(db *database.RoutedDB, clock schedule.Clock, logger *log.Logger) module.Module {
	return ModuleWithRepositories(internal.NewSQLiteTargetRepository(db, clock), internal.NewSQLiteIntakeRepository(db, clock), workspace.NewService(workspace.NewSQLiteRepository(db, clock)), logger)
}
