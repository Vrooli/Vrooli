...
func ModuleWithRepository(repo internal.Repository, ws workspace.Service, logger *log.Logger) module.Module {
	path, handler := connect.NewCostServiceHandler(NewConnectHandler(repo, ws, logger))
	return module.Module{Name: "cost", Mount: func(r *mux.Router) { connectx.RegisterServices(r, connectx.ServiceMount{Path: path, Handler: handler}) }, Endpoints: Endpoints}
}
