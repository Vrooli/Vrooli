...
|---|---|---|---|
| 2026-06-24 | Matthew Halloran | done | Residual doc/requirements reconciliation after the greedy-controller simplification (no code-behavior change). Deleted the 7 stale autosteer requirements describing removed subsystems (cost capture, effectiveness ledger, credit assignment, bandit selection, Layer-2 thrashing defense, DTV priors + eligibility gate — EM-P1-001..005, EM-P2-001/002) and the dead `pkg/effectiveness`/`pkg/dtv`/`*_bandit`/`*_thrashing`/`run_cost` test refs; added **EM-CTRL-003** (greedy selection) + **EM-CTRL-004** (gameguard promote-safety gate) for the as-built behavior; rewrote **EM-P1-006** to the real decision-trace surface (`OT-P1-003` retitled *Decision-trace transparency*). Removed **OT-P1-005** (DTV trust/cost priors) from the PRD and the *Auto-Steer Controller* section now reads as greedy. Re-pointed two dangling `[REQ:]` tags (`run_cost_test`→untagged, `diff_test`→EM-CTRL-002) and tagged the new reqs' tests. Docs: `manifest.json` `requiredHeadings`/`canonicalFor` re-synced to the already-rewritten CONTROL-MODEL + the `Settings & Discovery` api-endpoints heading; FLOWS/SEAMS/TESTING rewritten off the deleted phase-architecture (`phase_coordinator`/`evaluator`/`iteration_evaluator`/`MetricsProvider`/`MockConditionEvaluator`) onto the real seams (completeness `Provider`, findings `AuditRunner`, `Selector`, `Terminator`, `PromptEnhancerAPI`); DOMAINS/ARCHITECTURE/OBSERVABILITY metric-collector + decision-trace-gap references reconciled. |
