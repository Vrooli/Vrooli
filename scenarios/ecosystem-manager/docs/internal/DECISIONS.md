...
| 2026-01 | Store auto-steer profiles on the filesystem (`profiles/*/profile.json` + `metadata.json`); keep only execution state and history in Postgres. | Profiles are human-authored, version-controlled configuration (objective functions). They belong in git alongside code, not in a database. Postgres holds only the mutable runtime state. `[CODE: profiles/]` | ACCEPTED |
