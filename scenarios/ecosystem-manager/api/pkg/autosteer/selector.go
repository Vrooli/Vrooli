...
// Selector implements the controller's greedy SELECT stage: pick the
// highest-weighted open dimension that has at least one eligible skill in the
// profile's allow-set. Deterministic and fully explainable — no effectiveness
// weighting in v0 (see CONTROL-MODEL.md "Selection Policy").
type Selector struct {
	resolver SkillResolver
}

// NewSelector creates a Selector over a skill resolver.
...
func (s *Selector) SelectNextSkill(state findings.FindingsState, profile *AutoSteerProfile) Selection {
	...
	for _, wd := range ranked {
		...
		rationale := fmt.Sprintf(
			"dimension %q is the heaviest open cluster with an eligible skill (weighted score %.2f, %d findings, weight %.2f); selected %q from %v",
			wd.dim, wd.score, wd.count, wd.weight, chosen, eligible,
		)
		if len(skipped) > 0 {
