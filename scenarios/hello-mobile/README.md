...
Deterministic mobile conformance fixture for Android and iOS delivery ramps.

This scenario started from the `react-vite` template and is now the shared
deterministic mobile conformance fixture for Android and iOS delivery ramps.
Its UI intentionally has no backend dependency: it exposes stable selectors
for state persistence, response submission, route, connectivity, and
notification transitions so BAS and native-device chapters can reuse one
contract.

It still packages the standard full-stack Vrooli scenario shape:
...
`test-genie execute hello-mobile --preset comprehensive` directly for
