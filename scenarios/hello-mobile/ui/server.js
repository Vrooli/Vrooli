...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start hello-mobile` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start hello-mobile` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.hello_mobile\.v1\./

startScenarioServer({
  uiPort,
  apiPort,
  distDir: './dist',
  serviceName: 'hello-mobile',
  corsOrigins: '*',
  setupRoutes(app) {
