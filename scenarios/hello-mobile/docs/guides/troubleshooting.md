...
```bash
vrooli scenario status hello-mobile
make stop
vrooli scenario start hello-mobile --clean-stale
```
...
```bash
make status
vrooli scenario port hello-mobile UI_PORT
```
...
hello-mobile status --auto-start

# Or override the API base for this invocation
hello-mobile status --api-base "http://localhost:$(vrooli scenario port hello-mobile API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/hello-mobile/`.
...
The default route is `${SCENARIO_DATA_DIR}/hello-mobile.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/hello-mobile.db"
```
...
`packages/proto/gen/go/hello-mobile/v1/`, TypeScript under
`packages/proto/gen/typescript/hello-mobile/v1/`, and Python under
`packages/proto/gen/python/hello_mobile/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/hello-mobile/v1/<domain>/`.
