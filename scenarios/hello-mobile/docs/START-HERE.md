...
`template-manager orient hello-mobile`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient hello-mobile
...
```bash
cat > /tmp/prd_context_hello-mobile.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start hello-mobile --interactive
# …answer the interview, then:
business-health wizard preview hello-mobile
business-health wizard apply hello-mobile
```
...
```bash
vrooli scenario requirements validate hello-mobile --json
```
...
```bash
cat > /tmp/requirements_context_hello-mobile.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate hello-mobile --json
business-health fix preview hello-mobile
business-health fix apply hello-mobile
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate hello-mobile --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate hello-mobile`, the notes page spec
...
validate hello-mobile --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/hello-mobile/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest hello-mobile <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> hello-mobile
