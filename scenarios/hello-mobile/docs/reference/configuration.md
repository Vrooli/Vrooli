...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/hello-mobile.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `hello-mobile` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/hello-mobile/config.json`
3. `~/.vrooli/config/hello-mobile/config.json`
4. `~/.config/vrooli/hello-mobile/config.json`
...
```bash
hello-mobile configure api_base http://localhost:15001/api/v1
hello-mobile configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port hello-mobile API_PORT`)
