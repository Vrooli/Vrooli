...
`packages/proto/schemas/hello-mobile/v1/<domain>/<file>.proto`.
...
envelope (`packages/proto/schemas/hello-mobile/v1/shared/errors.proto`):
...
| **CLI** | `hello-mobile status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/hello-mobile/v1/shared/health.proto`
...
`POST /vrooli.hello_mobile.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.hello_mobile.v1.notes.NotesService/ListNotes`
...
| **CLI** | `hello-mobile notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.hello_mobile.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.hello_mobile.v1.notes.NotesService/CreateNote`
...
| **CLI** | `hello-mobile notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.hello_mobile.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.hello_mobile.v1.notes.NotesService/GetNote`
...
| **CLI** | `hello-mobile notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.hello_mobile.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `hello-mobile notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/hello-mobile/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/hello-mobile/v1/<domain>/`, then run
   `make generate`.
