...
`~/.vrooli/bin/hello-mobile`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start hello-mobile` if the API is unreachable |
...
```bash
hello-mobile status
hello-mobile status --json
```

### `hello-mobile configure <key> <value>`
...
```bash
hello-mobile configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`hello-mobile <domain> <verb>`). Every command calls a single API
...
```bash
hello-mobile notes list
hello-mobile notes list --json
```

#### `hello-mobile notes create --title <title> [--body <body>]`
...
```bash
hello-mobile notes create --title "First note" --body "Hello world"
```
...
```bash
hello-mobile notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start hello-mobile`" is good.
