...
import (
	...
	"hello-mobile/internal/capabilities"
	"hello-mobile/internal/modules"
	"hello-mobile/internal/server"
	...
	capsH "hello-mobile/handlers/capabilities"
	healthH "hello-mobile/handlers/health"
	notesH "hello-mobile/handlers/notes" // EXAMPLE-DOMAIN:notes
)
...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("hello-mobile")
	if err != nil {
		return "", fmt.Errorf("resolve hello-mobile storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve hello-mobile db path: %w", err)
	}
	return sqliteFileDSN(path)
}
...
func scenarioStorageRoots() (storage.Paths, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("hello-mobile")
	if err != nil {
		return storage.Paths{}, fmt.Errorf("resolve hello-mobile storage namespace: %w", err)
	}
...
func main() {
	...
	if preflight.Run(preflight.Config{ScenarioName: "hello-mobile"}) {
		return
	}
	...
	srv := server.New(
		...
		healthH.Module(db, "hello-mobile-api", "1.0.0"),
