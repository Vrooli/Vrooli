...
> Source Ledger uses this design language because operators compare dense
> corpus timelines, scope policies, frontier scores, and maintenance outcomes.
> Calm operational color, compact panels, keyboard-safe navigation, and explicit
> status semantics keep authoritative source data distinct from derived cache
> state while preserving the token and accessibility contract.
