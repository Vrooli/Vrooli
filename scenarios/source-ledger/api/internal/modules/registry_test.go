...
func TestProtoConnectParity(t *testing.T) {
	...
	// A newly generated scenario can have no Connect-mounted domain yet.
	// In that state the parity set is empty by design; once a domain module is
	// registered, the loop below enforces every RPC-to-endpoint mapping.
