...
func (h *handlers) createCall(ctx cliapp.OperationContext) (*scopesv1.CreateScopeResponse, error) {
	...
	response, err := h.client.CreateScope(context.Background(), connect.NewRequest(&scopesv1.CreateScopeRequest{Scope: &scopesv1.Scope{Id: ctx.Positional("id"), Label: ctx.Flag("label"), FrontierTarget: int32(frontier), WakeBudget: int32(wake), MaxEntryLines: int32(maxLines), Facets: facets}}))
	if err != nil {
		return nil, cliapp.WrapAPIError("create source-ledger scope", err, nil)
	}
...
func (h *handlers) createReport(_ cliapp.OperationContext, msg *scopesv1.CreateScopeResponse) cliapp.MutationReport {
	scope := msg.GetScope()
	return cliapp.MutationReport{Result: []string{fmt.Sprintf("Registered scope %s with frontier target %d and wake budget %d.", scope.GetId(), scope.GetFrontierTarget(), scope.GetWakeBudget())}}
}
...
func (h *handlers) listCall(_ cliapp.OperationContext) (*scopesv1.ListScopesResponse, error) {
	...
	if err != nil {
		return nil, cliapp.WrapAPIError("list source-ledger scopes", err, nil)
	}
	return response.Msg, nil
}
...
func (h *handlers) listReport(_ cliapp.OperationContext, msg *scopesv1.ListScopesResponse) cliapp.ListReport {
	...
	return cliapp.ListReport{Summary: []string{fmt.Sprintf("%d source-ledger scope(s).", len(results))}, Results: results}
}
