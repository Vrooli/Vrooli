...
type Facet = { id: string; label: string; guidance?: string; retentionPolicy: string; compactionEligible: boolean; residentBudget: number };
...
export function VocabularyPage() {
  ...
  const [draft, setDraft] = useState<Record<string, Pick<Facet, "retentionPolicy" | "compactionEligible" | "residentBudget">>>({});
  useEffect(() => {
    if (!facets.data) return;
    setDraft(Object.fromEntries(facets.data.facets.map((facet) => [facet.id, { retentionPolicy: facet.retentionPolicy, compactionEligible: facet.compactionEligible, residentBudget: facet.residentBudget }])));
  }, [facets.data]);
  const save = async () => {
    if (!scope || !facets.data) return;
    await Promise.all((facets.data.facets ?? []).map((facet) => post("/vrooli.source_ledger.v1.facets.FacetsService/SetFacetPolicy", { scope, facetId: facet.id, ...draft[facet.id] })));
    setSaved(true);
  };
  if (!scope) return <UnavailableScopePage />;
  return <section className="flex min-w-0 flex-col gap-5" aria-labelledby="vocabulary-heading"><div className="min-w-0"><h2 id="vocabulary-heading" className="break-words text-2xl font-semibold">Vocabulary · {scope}</h2><p className="text-app-muted-foreground">Update the scoped retention, compaction, and resident-memory policy. Changes apply to future recall and compaction without rewriting journal rows.</p></div><Card className="min-w-0"><CardContent className="space-y-4">{facets.data?.facets?.map((facet) => { const value = draft[facet.id] || facet; return <div key={facet.id} className="grid min-w-0 gap-2 border-b border-app-border pb-4"><span className="break-words text-sm font-medium">{facet.label || facet.id}</span><p className="break-words text-xs text-app-muted-foreground">{facet.guidance}</p><label className="grid min-w-0 gap-1"><span className="text-xs text-app-muted-foreground">Retention policy</span><select value={value.retentionPolicy} aria-label={`${facet.id} retention policy`} onChange={(event) => setDraft({ ...draft, [facet.id]: { ...value, retentionPolicy: event.target.value } })} className="min-w-0 rounded-control border border-app-border bg-app-background px-3 py-2"><option value="retain">Retain</option><option value="compact">Compact</option><option value="expire-on-resolution">Expire on resolution</option><option value="pinned-or-review">Pinned or review</option></select></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={value.compactionEligible} onChange={(event) => setDraft({ ...draft, [facet.id]: { ...value, compactionEligible: event.target.checked } })} />Eligible for compaction</label><label className="grid min-w-0 gap-1"><span className="text-xs text-app-muted-foreground">Resident budget (entries)</span><input type="number" min={0} value={value.residentBudget} aria-label={`${facet.id} resident budget`} onChange={(event) => setDraft({ ...draft, [facet.id]: { ...value, residentBudget: Number(event.target.value) } })} className="min-w-0 rounded-control border border-app-border bg-app-background px-3 py-2" /></label></div>; })}<Button disabled={!facets.data} onClick={() => void save()}>{saved ? "Policy saved" : "Save vocabulary policy"}</Button></CardContent></Card></section>;
}
