# Quickstart — Source Ledger

Get this scenario running locally quickly. The lifecycle
...
```bash
API_PORT=$(vrooli scenario port source-ledger API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.source_ledger.v1.<domain>.<Service>/<Method>
```
...
- Read [`concepts/ARCHITECTURE.md`](concepts/ARCHITECTURE.md) for the
  mental model: several surfaces, proto bridge, layered API, where to
  add code.
