...
`template-manager orient source-ledger`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient source-ledger
...
and the fenced example domain — mixes different kinds of guidance. Tokens, motion,
...
```bash
cat > /tmp/prd_context_source-ledger.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start source-ledger --interactive
# …answer the interview, then:
business-health wizard preview source-ledger
business-health wizard apply source-ledger
```
...
```bash
vrooli scenario requirements validate source-ledger --json
```
...
```bash
cat > /tmp/requirements_context_source-ledger.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate source-ledger --json
business-health fix preview source-ledger
business-health fix apply source-ledger
```
...
Concretely: if `DESIGN.md` shows a settings page with several example
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate source-ledger --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate source-ledger`, the notes page spec
...
validate source-ledger --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/source-ledger/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest source-ledger <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> source-ledger
...
- Wire contracts belong in
  `packages/proto/schemas/source-ledger/v1/<domain>/`.
