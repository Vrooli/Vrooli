...
```bash
vrooli scenario status source-ledger
make stop
vrooli scenario start source-ledger --clean-stale
```
...
```bash
make status
vrooli scenario port source-ledger UI_PORT
```
...
source-ledger status --auto-start

# Or override the API base for this invocation
source-ledger status --api-base "http://localhost:$(vrooli scenario port source-ledger API_PORT)/api/v1"
```
...
`vite build` processes the generated production bundle. This
...
together — never leave any of the generated files behind.
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/source-ledger/`.
...
The default route is `${SCENARIO_DATA_DIR}/source-ledger.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/source-ledger.db"
```
...
`packages/proto/gen/go/source-ledger/v1/`, TypeScript under
`packages/proto/gen/typescript/source-ledger/v1/`, and Python under
`packages/proto/gen/python/source_ledger/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/source-ledger/v1/<domain>/`.
