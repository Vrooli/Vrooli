...
```bash
export SQLITE_PATH=/tmp/source-ledger.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/source-ledger.db}")"
```
...
`packages/proto/gen/typescript/js/source-ledger/v1/`, and Python under
