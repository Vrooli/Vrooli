...
`~/.vrooli/bin/source-ledger`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start source-ledger` if the API is unreachable |
...
```bash
source-ledger status
source-ledger status --json
```

### `source-ledger configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
source-ledger configure api_base http://localhost:15001/api/v1
source-ledger configure token "<token>"
```

Read values back without an argument:

```bash
source-ledger configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`source-ledger "<domain>" "<verb>"`). Every command calls a single API
endpoint and renders the result through one of the supported output
...
- **One command per API endpoint.** If you find yourself making multiple
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start source-ledger`" is good.
