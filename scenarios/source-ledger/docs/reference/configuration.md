...
reference](../../../docs/reference/port-allocation.md) for the full policy.
...
For `source-ledger` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/source-ledger/config.json`
3. `~/.vrooli/config/source-ledger/config.json`
4. `~/.config/vrooli/source-ledger/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port source-ledger API_PORT`)
...
The React/Vite template requires several roles:
