...
react-component-library adoptions resolve-path "<component-id>" source-ledger
react-component-library adoptions apply "<component-id>" source-ledger "<adopted-path>"
```
...
table in `ui/src/app/routes.tsx`. Keep the surfaces aligned:
...
Run `experience-manager spec validate source-ledger --json` after route or
selector changes. Source Ledger page specs describe the corpus-first operator
surfaces and must evolve with the route table and selector registry.
