...
The `health` proto in `packages/proto/schemas/source-ledger/v1/shared/`
...
between the alternatives is impossible because both consume one source of
truth.
...
the network without going through a listed entry below, that's a
...
| **Why it exists** | Middleware computes request-duration log lines from a pair of `Now()` calls. With `time.Now()` direct, duration assertions are flaky on loaded CI and undefined on fast hardware. With `FakeClock.Advance(a short test interval)` inside the inner handler, the duration string is bit-for-bit deterministic. See `internal/middleware/logging_test.go::TestLoggingMiddleware_LogsDuration`. |
...
| **Test fake** | A literal `module.Module{Name: "stub", Mount: func(r){...}}` in `internal/server/server_test.go` proves the iteration; each domain `module_test.go` exercises the real constructors against in-memory fixtures. |
| **Why it exists** | Eliminates the central registry that would otherwise grow per-domain fields on `server.Deps` and per-domain wiring lines in `routes.go`. Adding a domain means creating files; deleting it means removing files. The endpoint descriptors travel with the module, so `.vrooli/endpoints.json` codegen has a shared source per domain (no manual JSON editing). |
...
| **Why it exists** | Hand-edited endpoints manifests drift from real handlers. The shared `modules` registry means runtime (`main.go`) and codegen (`gen-endpoints`) read endpoints + schema from one place — adding a domain is a small registry change, not separate edits in `main.go` and `gen-endpoints/main.go`. The CI drift check makes "I forgot to regenerate" a build failure, not a stale-doc bug. |
...
5. **Update this document.** A row in the table above with the same
   the required columns. If you skip this step, the seam exists but isn't
