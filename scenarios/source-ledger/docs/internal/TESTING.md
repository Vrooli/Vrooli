...
The proto schema in `packages/proto/schemas/source-ledger/v1/shared/health.proto`
...
dynamically from `go.mod`, so it works after `source-ledger`
substitution.
...
import (
    ...
    clitest "source-ledger/cli/internal/testutil"
)
...
under `packages/proto/schemas/source-ledger/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/source-ledger/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.source_ledger.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/source-ledger/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/source-ledger/v1/<domain>/`,
   `packages/proto/gen/typescript/js/source-ledger/v1/<domain>/`, and
   `packages/proto/gen/python/source_ledger/v1/<domain>/`.
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/source-ledger/v1/<domain>"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/source-ledger/v1/<domain>/<domain>_pb";
