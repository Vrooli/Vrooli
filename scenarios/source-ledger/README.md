...
Source Ledger provides an append-only corpus authority, scope-aware recall,
rebuildable derived views, and operator-facing health evidence. The current
foundation is intentionally contract-first; later phases add the ledger
engine behind these declared seams.

## Documentation Map

Start with `docs/START-HERE.md`, then read the PRD, requirements registry,
domain map, architecture, data, flow, integration, and experience contracts.

## Customize Safely

Keep the journal authority append-only, keep derived data rebuildable, and
route dependencies through the scenario lifecycle and governed dependency
analyzer. Update the relevant contract before changing a public surface.
...
- Root-level `DESIGN.md` plus generated UI token assets from the
  selected design kit.
- Generated `experience/` specs for the Source Ledger operator routes. These
  are contract-first claims, not finished product implementation.
...
`test-genie execute source-ledger --preset comprehensive` directly for
