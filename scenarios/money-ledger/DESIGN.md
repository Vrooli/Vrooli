...
Financial figures on this scenario's surfaces are read *in comparison* — a row against the row above it, this month against last — so two adaptations of the design kit are binding here beyond its defaults: **tabular numerals and consistent decimal alignment everywhere a figure appears**, and **direction is never colour-only** — an outflow carries a sign and a label, because a red tint is invisible to a substantial fraction of readers and meaningless in a screen reader. A third follows from the honesty rules in the PRD: **every figure has room beside it for its basis**, so a layout must not be designed so tightly that provenance has to be hidden behind a hover. The token contract is otherwise unmodified.
