...
export function AccountsPage() {
  ...
  return (
    <ExperienceSurface surfaceId="accounts" state={empty ? "empty" : "ready"} data-testid={selectors.pages.accounts} aria-labelledby="accounts-heading" className="flex flex-col gap-4">
      ...
      <Card data-testid={selectors.pages.bookList} role="list">
        ...
        <CardContent>
          ...
          <div data-testid={selectors.pages.bookLabel} role="status" className="mt-4 rounded-md border p-3">{books.data?.books[0]?.name ?? t(strings.pages.accounts.title)}</div>
        </CardContent>
      </Card>
      <Card data-testid={selectors.pages.accountTable} role="table">
        <CardContent className="pt-6">
          ...
          <p data-testid={selectors.pages.transferPair} role="link" className="flex min-h-11 items-center text-sm text-app-muted-foreground">{t(strings.pages.accounts.transferPair)}</p>
          {accounts.data?.accounts.map((account) => <p key={account.id} className="mt-2 text-sm">{account.name} · {account.kind}</p>)}
          {empty && <p data-testid={selectors.pages.accountsEmptyGuidance} role="note" className="mt-3 rounded-md border border-dashed p-3 text-app-muted-foreground">{t(strings.pages.accounts.emptyGuidance)}</p>}
        </CardContent>
      </Card>
    </ExperienceSurface>
  );
