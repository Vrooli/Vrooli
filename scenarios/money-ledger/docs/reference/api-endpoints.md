...
envelope (`packages/proto/schemas/money-ledger/v1/shared/errors.proto`):
...
| **CLI** | `money-ledger status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/money-ledger/v1/shared/health.proto`
...
`POST /vrooli.money_ledger.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.money_ledger.v1.notes.NotesService/ListNotes`
...
| **CLI** | `money-ledger notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.money_ledger.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.money_ledger.v1.notes.NotesService/CreateNote`
...
| **CLI** | `money-ledger notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.money_ledger.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.money_ledger.v1.notes.NotesService/GetNote`
...
| **CLI** | `money-ledger notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.money_ledger.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `money-ledger notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/money-ledger/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/money-ledger/v1/<domain>/`, then run
   `make generate`.
