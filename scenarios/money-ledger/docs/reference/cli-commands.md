...
`~/.vrooli/bin/money-ledger`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start money-ledger` if the API is unreachable |
...
```bash
money-ledger status
money-ledger status --json
```

### `money-ledger configure <key> <value>`
...
```bash
money-ledger configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`money-ledger <domain> <verb>`). Every command calls a single API
...
```bash
money-ledger notes list
money-ledger notes list --json
```

#### `money-ledger notes create --title <title> [--body <body>]`
...
```bash
money-ledger notes create --title "First note" --body "Hello world"
```
...
```bash
money-ledger notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start money-ledger`" is good.
