...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/money-ledger.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `money-ledger` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/money-ledger/config.json`
3. `~/.vrooli/config/money-ledger/config.json`
4. `~/.config/vrooli/money-ledger/config.json`
...
```bash
money-ledger configure api_base http://localhost:15001/api/v1
money-ledger configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port money-ledger API_PORT`)
