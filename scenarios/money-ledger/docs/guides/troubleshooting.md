...
money-ledger status --auto-start

# Or override the API base for this invocation
money-ledger status --api-base "http://localhost:$(vrooli scenario port money-ledger API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/money-ledger/`.
...
The default route is `${SCENARIO_DATA_DIR}/money-ledger.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/money-ledger.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/money-ledger.db}")"
```
...
`packages/proto/gen/go/money-ledger/v1/`, TypeScript under
`packages/proto/gen/typescript/js/money-ledger/v1/`, and Python under
`packages/proto/gen/python/money_ledger/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/money-ledger/v1/<domain>/`.
