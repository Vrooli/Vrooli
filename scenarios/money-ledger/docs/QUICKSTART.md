...
```bash
money-ledger notes list
money-ledger notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port money-ledger API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.money_ledger.v1.notes.NotesService/ListNotes" \
