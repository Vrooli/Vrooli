...
| Offer Desk | inbound only | Offer Desk reads this scenario for its actuals join. **This scenario has no knowledge of offers** and must never gain any — the direction is one-way by design, so the ledger stays usable by an operator who sells nothing. | Offer Desk calls this scenario's read API. Nothing here calls Offer Desk. |
