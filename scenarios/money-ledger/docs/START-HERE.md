...
`template-manager orient money-ledger`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient money-ledger
...
```bash
cat > /tmp/prd_context_money-ledger.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start money-ledger --interactive
# …answer the interview, then:
business-health wizard preview money-ledger
business-health wizard apply money-ledger
```
...
```bash
vrooli scenario requirements validate money-ledger --json
```
...
```bash
cat > /tmp/requirements_context_money-ledger.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate money-ledger --json
business-health fix preview money-ledger
business-health fix apply money-ledger
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate money-ledger --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate money-ledger`, the notes page spec
...
validate money-ledger --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/money-ledger/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest money-ledger <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> money-ledger
