...
`template-manager orient web-search`, which reads the temporary
...
```bash
cat > /tmp/prd_context_web-search.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) web-search \
  --context-file /tmp/prd_context_web-search.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate web-search --json
```
...
```bash
cat > /tmp/requirements_context_web-search.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) web-search \
  --context-file /tmp/requirements_context_web-search.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/web-search/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest web-search <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> web-search
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario web-search
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> web-search <adopted-path>
```
