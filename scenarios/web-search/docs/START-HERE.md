...
`vrooli scenario orient web-search`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient web-search
...
```bash
prd-control-tower prd generate web-search \
...
```bash
prd-control-tower prd validate web-search --json
```
...
```bash
prd-control-tower requirements generate web-search \
...
```bash
prd-control-tower requirements validate web-search --json
```
