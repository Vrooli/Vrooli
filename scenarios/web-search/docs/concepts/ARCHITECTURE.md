...
web-search is a search-hub provider with **two faces**: a **live** face
(an external passthrough that queries the local SearXNG resource — L0
raw results, optional L1 cited snippet synthesis, guarded by a cache +
budget governor) and a **learnings** face (a local, self-curating store
of cited **findings** distilled from research runs and semantically
indexed via aisearch-go). Its product domains are `livesearch`,
`findings`, `research`, `federation`, plus a deferred-P2 `curation`
domain and the infra `health` domain — see [`DOMAINS.md`](DOMAINS.md).
The same proto-first Go-API + Connect-RPC + react-vite shape below
applies; the two-faced live/learnings split shows up as two registered
search-hub providers (`web-search.live` / `web-search.learnings`).
...
The local SQLite holds findings/briefs metadata and the audit log. The
`findings` domain additionally reads/writes a Qdrant semantic index
(via aisearch-go) and the `livesearch` domain talks to the SearXNG
resource (with a TTL cache); those are infrastructure adapters, not
extra surfaces — see [`DATA.md`](DATA.md) and
...
| Contracts (`packages/proto/schemas/web-search/`) | Wire shape | Proto messages/services and generated clients | Hand-written route/type mirrors |
...
- scenario proto schemas relocated to
  `packages/proto/schemas/web-search/`.
...
```
packages/proto/schemas/web-search/v1/<domain>/<file>.proto
       │
       ▼
       make generate
       │
...
1. Add proto messages and service methods under
   `packages/proto/schemas/web-search/v1/<domain>/`.
...
|---|---|---|---|
| 2026-06-09 | Owns external storage resources (Qdrant) and external resource clients (SearXNG, browserless, Ollama, reranker), not just in-process SQLite. | The `findings` domain genuinely needs semantic recall, and the `livesearch`/`research` domains are inherently external-facing. | Documented in [`DATA.md`](DATA.md) / [`INTEGRATIONS.md`](INTEGRATIONS.md); revisit if a domain stops needing its adapter. |
| 2026-06-09 | Registers **two** search-hub providers from one scenario (`web-search.live` SCOPE_EXTERNAL + `web-search.learnings` SCOPE_PROJECT). | The two faces must be routed under different scopes so live web stays off the default federated path. | Revisit only if the scope model in search-hub changes. |
