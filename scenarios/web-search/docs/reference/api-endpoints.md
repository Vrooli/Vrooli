...
the high-level contract derived from `PRD.md`. All proto-typed calls use
generated Connect-RPC procedures under
`/vrooli.web_search.v1.<domain>.<Service>/<Method>`.

| Domain | Planned RPC / endpoint | Purpose | Requirements |
|---|---|---|---|
| livesearch | `LiveSearchService/Search` | L0 live web search — query SearXNG, normalize to `SearchHit`, served via cache + budget governor. Backs the `web-search.live` provider. | OT-P0-001, OT-P0-007 |
| livesearch | `LiveSearchService/Synthesize` | L1 optional cited snippet synthesis over returned snippets; additive, abstains on conflict, never blocks raw hits. | OT-P0-002 |
| findings | `FindingsService/SearchFindings` | Semantic search of the learnings corpus (active + disputed, excludes superseded). Backs the `web-search.learnings` provider. | OT-P0-005 |
| findings | `FindingsService/ListFindings` / `GetFinding` | Browse/read findings with citations, confidence, age, status. | OT-P0-005, OT-P0-006 |
| findings | `FindingsService/AddFinding` | Manual operator-authored finding + citation. | OT-P0-006 |
| findings | `FindingsService/EditFinding` / `SupersedeFinding` / `FlagFinding` / `PruneFinding` | Mutate finding lifecycle (never hard-delete; supersede/archive only; every mutation audited). | OT-P0-006, OT-P1-005 |
| research | `ResearchService/StartRun` | Start an L2 (fetch+synthesize) or L3 (agent-manager iterative) research run; emits a cited brief. | OT-P1-001, OT-P1-002 |
| research | `ResearchService/GetRunStatus` | Poll long-running research run state (planning → gather → synthesize → distill → reconcile → done/failed). | OT-P1-002, OT-P1-003 |
| research | `ResearchService/ListDisputes` / `ResolveDispute` | Dispute review queue: list flagged contradictions; resolve / re-research / dismiss (explicit, audited — never silent). | OT-P1-005, OT-P1-007 |
| federation | livesearch + findings Search procedures (above) are the provider-facing endpoints; self-registration runs at boot from `.vrooli/search.json`. Mutating verbs (reindex / config / query-overrides) are gated by the search-hub control token. | OT-P0-003, OT-P0-004 |
| curation *(P2)* | TBD | Usage telemetry, classifier auto-routing, GC. Deferred. | OT-P2-001..003 |

The real surface (paths, request/response messages, error codes) is
generated and pinned in [`.vrooli/endpoints.json`](../../.vrooli/endpoints.json)
when each domain's proto and handlers are implemented.
...
envelope (`packages/proto/schemas/web-search/v1/errors/errors.proto`):
...
| **CLI** | `web-search status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/web-search/v1/health/health.proto`
...
### `POST /vrooli.web_search.v1.notes.NotesService/ListNotes`
...
| **CLI** | `web-search notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.web_search.v1.notes.NotesService/ListNotes" \
...
### `POST /vrooli.web_search.v1.notes.NotesService/CreateNote`
...
| **CLI** | `web-search notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.web_search.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
### `POST /vrooli.web_search.v1.notes.NotesService/GetNote`
...
| **CLI** | `web-search notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.web_search.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `web-search notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/web-search/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/web-search/v1/<domain>/`, then run
   `make generate`.
