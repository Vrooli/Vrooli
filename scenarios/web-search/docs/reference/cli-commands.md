...
`~/.vrooli/bin/web-search`, and rebuilt automatically when its
...
| Planned command | Mirrors | Purpose |
|---|---|---|
| `findings list` | `FindingsService/ListFindings` | Browse findings (status/confidence/age/citations); `--include-archived` to retrieve superseded. |
| `findings search <query>` | `FindingsService/SearchFindings` | Semantic search the corpus. |
| `findings get <id>` | `FindingsService/GetFinding` | Read one finding with full provenance. |
| `findings add` | `FindingsService/AddFinding` | Manually author a finding + citation. |
| `findings edit <id>` | `FindingsService/EditFinding` | Edit an existing finding. |
| `findings supersede <id>` | `FindingsService/SupersedeFinding` | Archive in place (never hard-delete; audited). |
| `findings flag <id>` | `FindingsService/FlagFinding` | Mark disputed with a dispute note. |
| `findings prune` | `FindingsService/PruneFinding` | Confidence-gated cleanup (still archive, not delete). |

`web-search research ...` — drive research runs and disputes:

| Planned command | Mirrors | Purpose |
|---|---|---|
| `research run "<question>"` | `ResearchService/StartRun` | Start an L2/L3 run; emits a cited brief. |
| `research status <run-id>` | `ResearchService/GetRunStatus` | Poll run phase/progress. |
| `research disputes` | `ResearchService/ListDisputes` | List flagged contradictions (dispute review queue). |
| `research resolve <id>` | `ResearchService/ResolveDispute` | Resolve / re-research / dismiss a dispute (explicit + audited). |

Live search (L0/L1) is reached through the federated search-hub query
path rather than a dedicated `web-search` CLI verb; the providers
register at boot. Governance metadata (`effect`, `run_eligible`,
`permissions`, `requires_confirmation`) is declared per command in
`cli/manifest.json` when these land.
...
| `--auto-start` | Run `vrooli scenario start web-search` if the API is unreachable |
...
```bash
web-search status
web-search status --json
```

### `web-search configure <key> <value>`
...
```bash
web-search notes list
web-search notes list --json
```

### `web-search notes create --title <title> [--body <body>]`
...
```bash
web-search notes create --title "First note" --body "Hello world"
```
...
```bash
web-search notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start web-search`" is good.
