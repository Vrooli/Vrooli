...
| 2026-06-10 | hardening plan | done | All 8 pending validations cleared → 18/18 requirement modules complete: attended runbook `scripts/live-validate.sh` + 30-day manual evidence (REQ-P1-002/P1-004/P0-004); prune `--force` gate; capture→reconcile-kick index freshness (aisearch-go `SyncLoop.Kick` + `WithMutationNotify`); L2 abstain reasons + `excerpts` proto + relevance-aware excerpting + synthesis model llama3.2:3b→qwen3:4b (eval 2/10→9/10 correct); search-hub learnings routing (description+prompt rule, recall gates) + weak-hit escalation (flag-gated) + classifier warm p95 1.2s≤2s. Records rec-0604767cd902eb06/rec-673eaa1ed70cad2a/rec-fae576cd68d59c3a/rec-afc6c263a9f645f6. |
