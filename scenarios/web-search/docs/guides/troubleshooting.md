...
make stop
vrooli scenario start web-search --clean-stale
```
...
```bash
make status
vrooli scenario port web-search UI_PORT
```
...
web-search status --auto-start

# Or override the API base for this invocation
web-search status --api-base "http://localhost:$(vrooli scenario port web-search API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/web-search/`.
...
The default route is `${SCENARIO_DATA_DIR}/web-search.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/web-search.db"
```
...
`packages/proto/gen/go/web-search/v1/`, TypeScript under
`packages/proto/gen/typescript/web-search/v1/`, and Python under
`packages/proto/gen/python/web_search/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/web-search/v1/<domain>/`.
