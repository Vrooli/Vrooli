...
- [x] OT-P0-004 | Scope-Aware Federated Blending | Ensure a default federated query routes to `web-search.learnings` and surfaces internalized findings without firing any live web request; live web joins only on explicit request or fallback escalation when project results are empty or weak.
...
- [x] OT-P0-006 | Findings Management CLI Commands | Expose CLI commands for browse/list, manual add, edit, supersede, flag, and prune of findings, usable by both human operators and (later) the L3 agent.
...
- [x] OT-P0-009 | Evidence-Aware Research | When research is requested, web-search shall enforce explicit freshness and source requirements, reuse sufficient stored evidence before live work, and return a complete cited answer or an explicit gap without conflating unavailable evidence with success.
- [x] OT-P0-010 | Governed Research Lifecycle | When iterative research is requested, web-search shall delegate to Agent Manager with bounded execution and provide governed start, wait, and result operations that preserve durable run identity and distinguish task completion from research quality.
- [x] OT-P0-011 | Outcome-Linked Learning | For every research attempt, the usage workflow shall record provenance, context, effort, evidence, unresolved outcomes, and contribution feedback through Vrooli Memory; improvement shall compare equivalent cohorts without inventing baselines.
- [x] OT-P0-012 | Validated Research Method Reuse | When a research method recurs, web-search shall offer a versioned governed program with evidence requirements and regression fixtures, preserve failed attempts, and require quality evidence before claiming efficiency improvements.

### 🟠 P1 – Should have post-launch
- [x] OT-P1-001 | L2 Fetch, Read, and Synthesize | Fetch top-N result pages through HTTP with browser-automation-studio escalation, extract readable text, and produce a single-pass cited synthesis, exposed as discrete API endpoints and agent tools.
- [x] OT-P1-002 | L3 Iterative Research Agent | Spawn an agent-manager–managed research run that uses L2 endpoints and web-search CLI commands as tools, plans and decomposes the query, loops (search → read → find gaps → re-search), verifies across sources, and emits a cited brief, reusing existing agentic infrastructure with no hand-rolled loop plumbing.
...
- [x] OT-P1-004 | Finding Auto-Capture Policy | Auto-capture findings by default in L3 runs, offer opt-in capture via flag for L2, and never persist findings for L0/L1; an LLM distillation pass at run end emits structured findings (claim + citations + confidence).
...
- [x] OT-P2-002 | Classifier Auto-Routing to Live Web | Train the search-hub routing classifier to detect "web-shaped" queries and automatically include the live web provider, gated behind the proven cache and budget governor to manage rate-limit risk.
