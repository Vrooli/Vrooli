...
Evidence policy (mandatory): {{.evidence_policy}}
Requested questions and required coverage: {{.questions}}
...
2. RESEARCH what existing findings do not already cover. Use `web-search research answer "<focused sub-query>" --effort l2 --parent-run-id "{{.parent_run_id}}"` for fresh evidence. `web-search research l2` is the underlying page-reading tool; when using it directly, pass `--parent-run-id "{{.parent_run_id}}"` as well. The parent execution identity groups every child L2 attempt under this investigation for durable recovery and measurement. Use `web-search search search` for candidate URLs. Verify current claims against primary sources. Identify gaps and pursue each focused sub-query within the budget.
3. Produce the answer first. Ground each claim in source URLs and retrieved_at dates; list unresolved gaps and contradictions. Return abstained when evidence cannot support an answer, partial when coverage is incomplete or any gap/contradiction remains, answered only when all requested parts are supported and both gaps and contradictions are empty. Never equate agent completion with evidence quality.
