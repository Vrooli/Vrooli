...
// the template itself does not build because of web-search placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_web_search_v1_notes_notes_proto, "NotesService")
}
