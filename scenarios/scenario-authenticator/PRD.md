...
### Documentation contract for deployment and account ownership

The project-level contract in
[`../../docs/concepts/IDENTITY-AND-AUTHENTICATION.md`](../../docs/concepts/IDENTITY-AND-AUTHENTICATION.md)
is normative for this PRD. The authenticator owns person/service identity,
sessions, MFA, machine bindings, and coarse capabilities. It does not own
scenario domain authorization, LPBS customer accounts, subscriptions,
downloads, or commercial entitlements.

Tier 2 bundles declare `personal_local`, `local_multi_user`,
`remote_vrooli`, or `shared_provider`. A bundled personal application does
not require a human sign-in by default. A local multi-user application
explicitly provisions a private local realm. An LPBS website session is not a
local Vrooli session; linking requires an explicit, scoped, auditable exchange.

The PRD distinguishes shipped compatibility behavior from the target migration:
the default realm and `scenario-authenticator:default` audience remain for
existing consumers during migration, while new integrations target
resource-specific audiences and the shared `api-core/identity`/`api-core/authn`
contract.
...
- [ ] OT-P0-005 | Sessions + Revocation | Server-tracked sessions support list and per-session revoke (and "log out everywhere"). A single local/desktop replica may use the durable local hot-state seam; shared Redis is required above one replica. The `/api/v1/sessions/{id}` revoke contract device-sync-hub calls is preserved or delivered as a Connect equivalent in lockstep.
...
- [ ] OT-P0-011 | SQLite Persistence via Storage Seam + Tiered Hot State | All persistence is SQLite through the `api-core/storage` seam (no shared Postgres); durable local hot state is valid for one replica and shared Redis is required above one replica for sessions, revocation, OAuth CSRF state, and rate limiting; schema changes are additive migrations, never recreation.
...
- [ ] OT-P1-002 | MFA Policy and Recovery Hardening | The shipped MFA primitives (enrollment, challenge, and recovery-code paths) are hardened and completed with per-realm enforcement policy, recovery UX, and cross-replica correctness.
...
Preferred storage: SQLite is the local and default substrate via the `api-core/storage` seam — this is the mechanism that removes the shared-Postgres blast radius. The seam keeps the database swappable to a managed server DB for cloud scale, so SQLite is a default, not a lock-in. Redis is retained as the shared hot-state store above one replica. A single local or desktop replica may use durable local state behind the same store seam; multi-replica session revocation, refresh-family reuse detection, and rate limiting require shared Redis. Only token and credential hashes plus signed material are stored at rest — never plaintext secrets. SQLite schema changes are always additive migrations, never database recreation. Rate limiting uses in-memory state as the primary layer with shared state when replicas require coordination.

Integration strategy: API-to-API only. Consumers resolve scenario-authenticator by slug via `api-core/discovery` and verify tokens locally against JWKS. There are no cross-origin browser calls anywhere in the model — a browser talks only to its own scenario's API, which forwards same-origin if it must reach the authenticator. Shared workflows are preferred over resource CLI over direct API where a choice exists. The shared `api-core/identity` and `api-core/authn` packages become the canonical relying-party adapter; existing `owneridentity` and scenario-specific verifiers are compatibility seams to retire. LPBS may migrate its person identity to this provider while retaining customer accounts and entitlements. Device-sync-hub's live flow remains the first integration gate.
...
Required resources: SQLite via the `api-core/storage` seam (no shared Postgres) and the signing keypair persisted to the storage root (private.pem / public.pem, load-or-generate pattern carried over verbatim). Redis is optional for a single local/desktop replica and required above one replica for shared sessions, revocation, refresh-family reuse detection, OAuth CSRF state, and distributed rate limiting.
...
Operational risks: Auth is a security boundary — the single biggest risk is re-deriving crypto incorrectly during the rewrite; mitigated by porting RS256/JWKS/claims/Argon2id hashing verbatim, locking the algorithm to exactly RS256, and explicitly rejecting `none` and HS-family confusion at the verification layer. Signing-key persistence and rotation must be deliberate or every restart invalidates live tokens; the load-or-generate pattern with file-backed keypairs directly addresses this. SQLite is a single-writer store — that is the authenticator's own write-throughput ceiling; mitigated because the hot path (token verification) is stateless and never touches SQLite, and the storage seam allows a managed DB at scale when throughput demands it. Durable local hot state is valid for one replica; shared Redis is required above one replica so revocation and rate-limit decisions do not diverge. Realm isolation and future resource-audience correctness must be enforced at issuance and verification time and covered by integration tests — a misconfiguration here constitutes a cross-tenant or cross-resource token leak.

Launch sequencing: (1) **P0 core** — accounts + Argon2id + RS256/JWKS/keypair persistence + refresh-token rotation with reuse detection + tier-appropriate hot state (durable local state for one replica, shared Redis above one replica) + rate limiting + account lockout + auth-event audit log + single default realm with aud-scoped tokens + Connect-RPC surface (Login, Register, Refresh, Logout, Validate, session management) + CLI parity + SQLite via storage seam. (2) **Live consumer migration** — migrate device-sync-hub's forwarder from REST to the typed Connect client in lockstep and prove the live first-run flow end-to-end; no P1 work begins until this is green. (3) **P1** — realms as a real tenant boundary, MFA policy/recovery hardening, OAuth social federation with account linking, API keys and client-credentials grant, scopes RBAC, WebAuthn/passkeys, admin console, end-user self-service UI, account recovery flows. (4) **P2** — SAML/enterprise SSO, OIDC-provider mode ("Login with Vrooli"), groups/orgs hierarchy, delegated policy engine, per-realm key isolation and automated rotation, managed-DB/HA via storage seam, SCIM provisioning.
