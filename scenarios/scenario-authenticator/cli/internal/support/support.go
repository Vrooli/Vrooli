...
const CLIName = "scenario-authenticator"
...
// EnvelopeMessage returns the "message" string if body is a {success, message}
// or {success, data: {message}} envelope, else empty.
func EnvelopeMessage(body []byte) string {
	trimmed := strings.TrimSpace(string(body))
	if !strings.HasPrefix(trimmed, "{") {
		return ""
	}
	// Top-level message takes precedence (this API returns {"success":true,"message":"..."}).
	var top struct {
		Message string `json:"message"`
	}
	if err := json.Unmarshal(body, &top); err == nil {
		if msg := strings.TrimSpace(top.Message); msg != "" {
			return msg
		}
	}
...
// FormatTimePtr renders a nullable time; nil or zero renders as "never".
...
// JoinStrings joins a slice with ", " and returns fallback when empty.
