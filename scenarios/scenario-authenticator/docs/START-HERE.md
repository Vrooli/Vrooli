...
`template-manager orient scenario-authenticator`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient scenario-authenticator
...
```bash
cat > /tmp/prd_context_scenario-authenticator.md <<'EOF'
Overview:
- Purpose:
...
```bash
cat > /tmp/requirements_context_scenario-authenticator.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
- [ ] **Start in proto.** Author `packages/proto/schemas/scenario-authenticator/v1/<domain>/<domain>.proto`
...
1. Add proto messages and a service under
   `packages/proto/schemas/scenario-authenticator/v1/<domain>/`, then run
