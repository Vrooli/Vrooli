...
| 2026-06-18 | **Keep Redis as the shared hot-state resource.** | Redis backs sessions, token-family revocation, OAuth CSRF state, and distributed rate limiting; these are security controls, not a cache. | Redis remains the horizontal-scale enabler for shared session/rate-limit state across replicas. Superseded for single-replica deployments by the durable-local decision recorded on 2026-09-07. | See the 2026-09-07 tier-dependent hot-state decision. |
| 2026-09-07 | **Use a tier-dependent hot-state store.** | Private single-replica installations need no separate Redis process; hosted and multi-replica installations need shared coordination. | The API selects Redis when configured and otherwise uses durable local hot state. It never silently falls back when shared Redis was explicitly configured. | Revisit if a different shared store becomes the supported hosted implementation. |
