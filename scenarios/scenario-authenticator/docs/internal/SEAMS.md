...
The `health` proto in `packages/proto/schemas/scenario-authenticator/v1/health/`
...
| | |
|---|---|
| **Seam** | The RS256 signing/verification keypair — the single most security-critical seam in the scenario. |
| **Interface** | `internal/crypto/keys.go::SigningKeyProvider` (e.g. `Private() *rsa.PrivateKey`, `Public() *rsa.PublicKey`, `KeyID() string`). |
| **Production wiring** | `handlers/tokens/module.go` constructs a file-backed provider that load-or-generates `private.pem`/`public.pem` in the `api-core/storage` root (ported verbatim from the old scenario), perms `0600` on the private key. Stable across restarts. |
| **Test fake** | A fake provider seeded with a fixed, test-only keypair (the old scenario exposes `SetTestKeys`; the port replaces that global with an injected fake). Lets sign/verify, JWKS round-trip, `alg=none`/`alg=HS256` rejection, and `kid` tests run with a deterministic key and no disk I/O. |
| **Why it exists** | Tokens are signed and verified everywhere; the key must be a substitutable dependency, not a package global. The fake makes the algorithm-confusion regression tests and the JWKS-publication test fast and deterministic. The persisted load-or-generate behavior is itself a tested production contract (a fresh key per boot would invalidate every live token — the bug this seam's persistence path prevents). See [`SECURITY.md`](SECURITY.md) "Signing-key persistence and rotation". |

### RealmResolver (tenant boundary)

| | |
|---|---|
| **Seam** | Resolve the realm for a request and supply its policy (token TTLs, allowed methods, redirect URIs, branding) and `aud` value. |
| **Interface** | `internal/realms/resolver.go::RealmResolver` (e.g. `Resolve(ctx, key) (Realm, error)`, `Default(ctx) (Realm, error)`). |
| **Production wiring** | `handlers/realms/module.go` constructs a SQLite-backed resolver over the `api-core/storage` seam; P0 ships a single default realm, P1 makes it true multi-tenant CRUD. |
...
| | |
|---|---|
| **Seam** | Resolve another scenario's base URL by slug (used by the live-consumer migration path and any future outbound federation to a Vrooli scenario). |
| **Interface** | `api-core/discovery::Resolver` (resolve a scenario slug → base URL); consumed wherever this scenario must reach a sibling API-to-API. |
| **Production wiring** | `main.go` wires the real `api-core/discovery` resolver. **This scenario is bottom-of-the-stack** — it has no upstream scenario dependencies, so the production consumer is minimal today; the seam exists for symmetry with the RP side (device-sync-hub resolves *this* scenario by slug) and for future federation. |
| **Test fake** | A static resolver mapping a slug to a test `httptest.Server` URL, so any cross-scenario call is exercised against an in-process stub — no real discovery, no network. The integration tests use this to stand up an **in-process stub Relying Party** that fetches JWKS and verifies tokens locally. |
| **Why it exists** | The integration contract that must never break is "an RP resolves scenario-authenticator by slug and verifies its tokens locally." A static resolver lets a test play the RP end-to-end (resolve → fetch JWKS → local verify) without the discovery service or a second running scenario. Resolving by slug (never a hard-coded cross-origin URL) is the rule from PRD Appendix A. |

### api-core/storage resolver (SQLite default, swappable at scale)

| | |
|---|---|
| **Seam** | The persistence root — SQLite by default, swappable to a managed DB at scale — resolved through `api-core/storage`. |
| **Interface** | `api-core/storage::NewResolver(ProfileAuto)` → a storage root; each domain's repository opens its `*sql.DB` against the resolved SQLite path. Schema is applied additively via `api-core/database.EnsureSchemas` (see the `database.SchemaExecer` and `<domain>.Schema` seams above). |
| **Production wiring** | `main.go` resolves the storage root via `storage.NewResolver(ProfileAuto)`; domains open SQLite handles under it. **No shared Postgres** — this is the mechanism that removes the shared-DB blast radius that motivated the rewrite ([`PROBLEMS.md`](PROBLEMS.md)). Migrations are additive only, never recreation. |
| **Test fake** | `internal/testutil/db::NewSQLite(t)` returns a fresh in-memory/throwaway SQLite handle; repository tests apply the production schema providers via `EnsureSchemas` (the `newSchemaDB` compose pattern in [`TESTING.md`](TESTING.md)). The seam means tests never touch a shared database, and the at-scale managed-DB swap (OT-P2-006) changes one wiring line, not domain code. |
| **Why it exists** | The storage seam keeps SQLite a *default*, not a lock-in: the hot path (token verification) never touches it, so SQLite's single-writer ceiling is the authenticator's own write ceiling and is mitigated by design ([`PERFORMANCE.md`](PERFORMANCE.md)). The seam is also the honest boundary for the eventual managed-DB/HA backing. |
