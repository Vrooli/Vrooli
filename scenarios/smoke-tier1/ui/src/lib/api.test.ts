 ...
 *
 * `fetchHealth` is a thin wrapper around `fetch`. We stub the global
 * `fetch` rather than reaching for MSW: a 4-line wrapper doesn't
 * justify the heavy dep, and `vi.stubGlobal` + `vi.unstubAllGlobals`
 * gives us the same isolation per test without the install footprint.
 *
 * What these tests pin:
 *   - the happy path returns the parsed JSON
...
describe("lib/api.fetchHealth", () => {
  ...
  it("requests /health with cache: 'no-store' and JSON content-type", async () => {
    fetchSpy.mockResolvedValueOnce(
      new Response('{"status":"ok","service":"x","timestamp":"t"}', { status: 200 }),
    );
