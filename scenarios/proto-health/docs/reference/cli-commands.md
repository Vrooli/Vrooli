...
| `--auto-start` | Run `vrooli scenario start proto-health` if the API is unreachable |
...
```bash
proto-health status
proto-health status --json
```

### `proto-health configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
proto-health configure api_base http://localhost:15001/api/v1
proto-health configure token <token>
```
...
```bash
proto-health notes list
proto-health notes list --json
```

### `proto-health notes create --title <title> [--body <body>]`
...
```bash
proto-health notes create --title "First note" --body "Hello world"
```
...
```bash
proto-health notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start proto-health`" is good.
