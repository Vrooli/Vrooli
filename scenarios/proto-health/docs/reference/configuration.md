...
| `PROTO_HEALTH_SKIP_GEN_SYNC` | unset / false | Skip generated-artifact manifest verification. Retained as an emergency escape hatch; normal validation is file-hash based and should be fast. |
...
For `proto-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/proto-health/config.json`
3. `~/.vrooli/config/proto-health/config.json`
4. `~/.config/vrooli/proto-health/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port proto-health API_PORT`)
