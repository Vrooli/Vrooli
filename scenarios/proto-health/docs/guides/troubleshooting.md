...
```bash
vrooli scenario status proto-health
make stop
vrooli scenario start proto-health --clean-stale
```
...
```bash
make status
vrooli scenario port proto-health UI_PORT
```
...
proto-health status --auto-start

# Or override the API base for this invocation
proto-health status --api-base "http://localhost:$(vrooli scenario port proto-health API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/proto-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/proto-health.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/proto-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/proto-health.db}")"
```
...
`packages/proto/gen/go/proto-health/v1/`, TypeScript under
`packages/proto/gen/typescript/js/proto-health/v1/`, and Python under
`packages/proto/gen/python/proto_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/proto-health/v1/<domain>/`.
