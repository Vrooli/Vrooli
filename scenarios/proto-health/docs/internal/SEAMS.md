...
### DescriptorLoader (proto descriptor image)

| | |
|---|---|
| **Seam** | Fleet proto descriptor loading |
| **Interface** | `internal/protosurface::Loader`, implemented by `DescriptorLoader`, returning a typed `Surface` over `packages/proto/gen/descriptor/image.binpb`. |
...
| | |
|---|---|
| **Seam** | Scenario and proto source discovery |
| **Interface** | Planned `internal/protosurface::ScenarioLocator`, resolving a scenario slug to scenario root, schema root, generated output paths, and implementation hints. |
| **Production wiring** | Production uses repo-root-relative paths and existing repo/scenario contract helpers where available. |
