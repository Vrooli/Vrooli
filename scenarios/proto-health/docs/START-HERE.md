...
`vrooli scenario orient proto-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient proto-health
...
```bash
prd-control-tower prd generate proto-health \
...
```bash
prd-control-tower prd validate proto-health --json
```
...
```bash
prd-control-tower requirements generate proto-health \
...
```bash
prd-control-tower requirements validate proto-health --json
```
