...
`template-manager orient proto-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient proto-health
...
```bash
cat > /tmp/prd_context_proto-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) proto-health \
  --context-file /tmp/prd_context_proto-health.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate proto-health --json
```
...
```bash
cat > /tmp/requirements_context_proto-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) proto-health \
  --context-file /tmp/requirements_context_proto-health.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/proto-health/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest proto-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> proto-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario proto-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> proto-health <adopted-path>
```
