...
// the template itself does not build because of proto-health placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_proto_health_v1_notes_notes_proto, "NotesService")
}
