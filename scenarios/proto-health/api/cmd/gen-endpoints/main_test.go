...
func TestCrossCheck_PassesWhenRegistered(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		...
		{ID: "y_alias", CLIMapping: &module.CLIMapping{Command: "proto-health notes ll"}},
...
func TestBuildCLICommands_ResolvesEndpointIDs(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "health", CLIMapping: &module.CLIMapping{Command: "proto-health status"}},
		{ID: "notes_list", CLIMapping: &module.CLIMapping{Command: "proto-health notes list"}},
	}
	registered := []registeredCommand{
		{Name: "configure"},
		{Name: "notes list"},
