...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("proto-health")
	if err != nil {
		return "", fmt.Errorf("resolve proto-health storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve proto-health db path: %w", err)
	}
	return sqliteFileDSN(path)
}
