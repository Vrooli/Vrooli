...
export const LayoutTuningSchema = z.object({
  ...
  surfaces: z.object({
    ...
    floorThickness: metres(0.001, 0.4, 'Corridor floor slab thickness'),
...
export const CameraTuningSchema = z.object({
  ...
  initialPosition: z.tuple([metres(-1000, 1000, 'Initial camera x'), metres(-1000, 1000, 'Initial camera y'), metres(-1000, 1000, 'Initial camera z')]).describe('Initialization-only camera position before the framing rig is ready (metres, x/y/z)').meta({ readOnly: true }),
  ...
  far: metres(10, 2000, 'Minimum far clip distance; expands with scene extent and available dolly range'),
  polarMinDeg: degrees(0, 89, 'Steepest allowed camera angle from straight above'),
  polarMaxDeg: degrees(1, 179, 'Maximum polar angle; values above 90 permit upward sky viewing with terrain clearance'),
  azimuthRangeDeg: degrees(0, 180, 'Orbit allowed either side of the hero azimuth; 180 enables continuous full yaw'),
  minDistance: metres(1, 100, 'Closest dolly distance'),
  maxDistance: metres(2, 500, 'Minimum available farthest dolly distance; expands to frame generated scene bounds'),
  introSeconds: seconds(0, 10, 'Length of the establishing-to-hero dolly on load').meta({ readOnly: true }),
  ...
  focusVisibilityTurnsDeg: z.array(z.number().min(-180).max(180)).min(1).max(8).describe('Bounded azimuth offsets searched for a clear actor focus (degrees)'),
  focusVisibilityHeights: z.array(z.number().min(0).max(1)).min(1).max(8).describe('Actor height fractions tested for focus visibility'),
...
export const LightingTuningSchema = z.object({
  ...
  clockPollSeconds: seconds(1, 3600, 'How often live lighting samples the shared civil-time clock'),
  keyLight: z.object({
...
export const QualityProfileSchema = z.object({
  ...
  ao: z.boolean().describe('Derived compatibility flag: true when aoQuality is not off; edit aoQuality instead').meta({ readOnly: true }),
...
export const QualityTuningSchema = z.object({
  diagnostics: z.object({
    ...
    interactionSampleLimit: count(60, 60000, 'Maximum samples retained in an explicitly requested interaction interval'),
    interactionLongFrameMs: z.number().positive().describe('Frame duration counted as a long interaction frame (milliseconds)'),
...
export const PostTuningSchema = z.object({
  aoMotionPauseMs: z.number().min(0).max(2000).describe('Pause screen-space AO during camera movement and resume after this settle interval (milliseconds)'),
  aoMotionEpsilon: z.number().positive().max(0.1).describe('Camera movement tolerance for AO adaptation (metres/radians)'),
