...
it('refreshes at the configured cadence and replaces and clears its timer', () => {
  vi.useFakeTimers()
  resetDiagnostics()
  const view = render(<DiagnosticsOverlay seed={1} seedDigest="test" refreshMs={250} scene="park" />)
  act(() => { updateDiagnostics({ drawCalls: 42 }); vi.advanceTimersByTime(249) })
  expect(screen.queryByText('42')).toBeNull()
  act(() => { vi.advanceTimersByTime(1) })
  expect(screen.getByText('42')).toBeTruthy()
  view.rerender(<DiagnosticsOverlay seed={1} seedDigest="test" refreshMs={500} scene="park" />)
