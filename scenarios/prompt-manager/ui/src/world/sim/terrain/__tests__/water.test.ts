...
import { buildTerrain, isWater, shoreDistance, waterCells } from '..'

describe('terrain water', () => {
  it('forms optional deterministic ponds across the seed matrix', () => {
    const counts = [1, 7, 99, 12345].map((seed) => waterCells(buildTerrain({ seed, tuning: tuning.terrain }), tuning.terrain))
    expect(counts.filter((entry) => entry.count > 0).length).toBeGreaterThanOrEqual(2)
    const dry = { ...tuning.terrain, waterLevel: -20 }
  ...
  it('marks water and its dry-side shore margin unwalkable', () => {
    ...
    const grid = buildNavGrid(bounds, [], [], 1, 0.5, 0.4, field, tuning.terrain)
    let sawWater = false
    let sawShore = false
    for (let row = 0; row < grid.rows; row += 1) {
      for (let col = 0; col < grid.cols; col += 1) {
        const point: [number, number] = [grid.originX + (col + 0.5) * grid.cellSize, grid.originZ + (row + 0.5) * grid.cellSize]
        const distance = shoreDistance(field, tuning.terrain, point[0], point[1])
        if (isWater(field, tuning.terrain, point[0], point[1])) sawWater = true
  ...
  it('routes between dry points around a pond', () => {
    ...
    const wetIndex = field.height.findIndex((_, index) => {
      ...
      return Math.abs(x) < 25 && Math.abs(z) < 25 && isWater(field, tuning.terrain, x, z)
    })
