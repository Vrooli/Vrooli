...
describe('site selection', () => {
  it('is deterministic and seed-sensitive', () => {
    ...
    const first = selectSites(field, tuning, sizes, 1)
    const again = selectSites(field, tuning, sizes, 1)
    expect(first).toEqual(again)
    const otherField = buildTerrain({ seed: 99, tuning: tuning.terrain })
    expect(selectSites(otherField, tuning, sizes, 99).sites.map((site) => site.position)).not.toEqual(first.sites.map((site) => site.position))
  })

  it('selects dry, flat, disjoint sites with 15-degree rotations', () => {
    const field = buildTerrain({ seed: 7, tuning: tuning.terrain })
    const result = selectSites(field, tuning, sizes, 7)
    result.sites.forEach((site, index) => {
      expect(isWater(field, tuning.terrain, site.position[0], site.position[1])).toBe(false)
