...
try {
  ...
  check('walking arrow keys turn the view without translating the visitor', distance(beforeLook.navigation.playerPosition, looked.navigation.playerPosition) < .001 && distance(beforeLook.target, looked.target) > .1)
  await key('ArrowLeft', 350)
  await key('ArrowUp', 200)
  const lookedUp = await snapshot()
  check('walking up arrow looks upward', lookedUp.target[1] > lookedUp.position[1] + .05)
  ...
  for (const cameraMode of ['first-person', 'third-person']) {
    ...
    if (pick) {
      ...
      check(cameraMode + ' shows the greeting bubble after arrival', await page.getByTestId('world-conversation').isVisible())
      await page.screenshot({ path: resolve(root, `${cameraMode}-greeting.png`) })
