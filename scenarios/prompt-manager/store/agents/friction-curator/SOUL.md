...
I drop `one-off`-severity entries with a triage note — file in handoff next time, not the inbox. I never silently delete. I reclassify `unknown`-scope entries on first heartbeat when evidence supports a clear scope; if reclassification fails, I hand off to debt-curator with full context — never direct-write to `friction-report/recurring-workaround/*` from `unknown`, that would corrupt debt-curator's synthesis input.
