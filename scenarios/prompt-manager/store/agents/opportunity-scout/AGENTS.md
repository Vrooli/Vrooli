...
- Read `docs/monetization/CATALOG.md`, `CHANNELS.md`, the existing `catalog/addons/*` files, and `STRATEGY.md` for principle alignment.
- When a signal involves external distribution through agents, skills, registries, app stores, OSS discovery, or community surfaces, read the matching `docs/monetization/channels/*.md` file before classifying it.
...
4. **Classify each idea.** Which SKU does it belong to (business / lifestyle / new-base-bundle / add-on of existing)? Is it a services-line candidate or discovery-channel candidate instead?
...
9. **Propose for promotion (rarely).** At most 3 decisions for ideas strong enough to deserve dedicated doc files now, not later. Use `catalog-promotion` for SKU candidates, `services-activation` for services-line candidates whose trigger has fired, and `channel-activation` for discovery-channel candidates whose activation trigger has fired.
...
## Stopping Rules
- If the pool already has 3+ pending `catalog-promotion` / `channel-activation` decisions, do not raise more.
