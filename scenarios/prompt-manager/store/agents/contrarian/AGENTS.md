...
- Read `docs/monetization/STRATEGY.md` (principles), `FINANCIAL_MODEL.md` (guardrails), `REVENUE_LINES.md` (services discipline), `CATALOG.md` (catalog discipline), `TIERS.md` (tier-activation prereqs).
...
2. **Score each proposal.** Walk the seven failure modes in order for each pending decision or fresh proposal. For each hit, note the specific missing element.
...
5. **Flag framework gaps.** If a proposal has a real flaw not covered by the seven modes, capture a knowledge entry and propose a `framework-update` decision separately. Do not invent an eighth failure mode on the fly.
