...
- When recent activity includes a published external skill or skill-registry milestone, read `docs/monetization/channels/skill-registries.md` and `skills/README.md` before drafting.
...
6. **Check skill-publication events.** If a Vrooli skill was signed, scanned, published, accepted into a curated registry, or entered a measurement window, treat it as a possible dev-log / OSS narrative event. Keep claims factual: what capability it wraps, what registry/trust milestone happened, what remains unproven.
7. **Draft artifacts.** Produce 0-2 new drafts per heartbeat, prioritized by:
   - High-score dev-log story arcs from the mining phase.
   - Skill-publication milestones that have concrete trust or distribution evidence.
...
8. **Raise coverage-gap for OSS narrative.** If `oss-platform.json` shows `status: stale` or `status: missing` AND no in-flight draft, raise a `coverage-gap` decision. Cap: 1 per heartbeat.
9. **Raise capability-gap when tooling blocks.** When drafting needed a missing capability (e.g., video production for a feature demo), raise `capability-gap` AND append a workaround note to the relevant notebook file. Cap: 1 per heartbeat.
10. **Supersession check.** For each pending decision in your owned contexts (`content-publish-proposal` OSS variants, `coverage-gap` OSS narrative, `capability-gap`), check for fresher takes. Mark `superseded`; include `supersedes: <prior-id>` on replacements.
11. **Write ad-run knowledge entry.** Topic `oss-ad-run-YYYY-MM-DD` (append-only). Summarize: activity period mined, story arcs selected, drafts produced, coverage/capability gaps raised.
12. **Handoff.** End with `## HANDOFF` per HEARTBEAT.md.
...
## Stopping Rules
- Team ceiling ≥12 pending → read-only (skip steps 7-9; supersession in step 10 still runs).
