...
- Read `docs/monetization/FINANCIAL_MODEL.md` (framework + assumptions), `PRICING.md`, `REVENUE_LINES.md`, `CHANNELS.md`, `TELEMETRY_ROADMAP.md`.
...
2. **Compute.** Cash, per-category burn, per-tier/per-bundle/per-revenue-line revenue, channel attribution where telemetry exists, runway, default-alive gap, time allocation, LTV (if data permits).
...
5. **Flag.** Name flags explicitly — `services-trap-warning`, `runway-warning`, `assumption-drift`, `channel-attribution-gap`.
6. **Raise decisions.** At most 2 per heartbeat, by priority: `services-trap-warning` > `runway-warning` > `channel-attribution-gap` > `financial-model-assumption-update` > `pricing-decision`.
