...
- **No controlled CLI.** Do not propose an Action until one Vrooli-controlled CLI command owns the deterministic operation; route missing command work to CLI-backlog or capability work item instead.
- **No phantom graduation.** Do not raise a `skill-graduation` work item until the programmatic home *actually exists and runs* (e.g. the test-genie phase ships and detects the lens). `programmaticHome` is a record-of-fact, not a forecast — setting it prematurely makes both the quality-auditor and automation stop watching that lens, opening a coverage gap. The graduation work item proposes the `prompt-manager skill update --programmatic-home <engine:id>` write; the write itself happens when the work item is applied (you have no skill-write tool).
