...
3. Drain cap: track today's total routings (from this morning's `friction-triage/<today>`). If today's total + this heartbeat's queue would exceed `dailyInboxDrainCap` (default 25), emit `inbox-overflow` triage entry, hand off to debt-curator, and stop routing this heartbeat.
4. For each entry:
   ...
   5. Otherwise (validated scope and severity), action: `route` — write the entry to `friction/<scope>/<date>/<slug>` on the destination sub-member's behalf, then close the original `friction-inbox` entry.
5. Update `friction-triage/<YYYY-MM-DD>` (supersedesPrevious=true within a day): increment counts, append to Routings/Drops/Blocked sections.
...
- friction-inbox/<scope>/<slug> → friction/<scope>/<date>/<slug> (reporter: <team>/<agent>, severity: <sev>)
- ...
...
### Triage snapshot
- friction-triage/<YYYY-MM-DD> updated as `<knw-id>`
