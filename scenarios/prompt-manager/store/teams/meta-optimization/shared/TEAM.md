...
- **Convert** — push deterministic prose into Vrooli-controlled CLIs and Action contracts (cheaper tokens, reproducible, testable)
...
- **skill-optimizer** — identifies high-usage skills, pushes deterministic execution toward Actions, audits drift, proposes deprecation of unused skills and obsolete Actions.
...
Each member has a `RESPONSIBILITIES.md` + `HEARTBEAT.md` under `store/teams/meta-optimization/members/<member>/`. The structured operating contract in `team.json` owns lanes, contexts, write rules, caps, shared artifacts, and task parameters.
...
2. **Programmatic conversion is first-class, not a side effect.** The single highest-leverage thing the team does is move deterministic execution out of prose. When skill-optimizer reviews a skill, the first question is whether a Vrooli-controlled CLI already owns the operation, whether an Action should expose it, or whether the missing CLI belongs in backlog before any Action exists.
...
7. **Conversion-without-measurement** — proposing a programmatic conversion or Action change without a baseline for tokens / cost / effectiveness, so "did this help" can't be answered post-hoc. Conversion proposals must include the baseline measurement.
