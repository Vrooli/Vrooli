...
|---|---|---|---|
| 002 | A — binary missing | `env: '/home/matthalloran8/.local/bin/claude': No such file or directory` | 0.14s |
...
- **Triple-firing escalation watch.** Run-introspector itself is now triple-fired this heartbeat (3 instances 4ms apart). If the next 3 heartbeats also show 2+ run-introspector instances, escalate the scheduler issue from "out of lane, observed" to a `capability-gap` for director-swarm to consume (deduplication primitive in agent-manager). One of the three could pick that up next heartbeat — explicit handoff to the next run-introspector iteration.
