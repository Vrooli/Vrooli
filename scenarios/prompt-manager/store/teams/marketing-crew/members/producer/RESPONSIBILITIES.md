...
## Pending
Campaign work, drafts, claims, coverage, and publish history move to the `content-desk` scenario. Until its CLI surface is settled and its paired skill exists, the drafting mechanics in `# Task Loop` are intentionally unversed in specific commands.
