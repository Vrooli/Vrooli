...
- Do not write to `path:docs/monetization/` — propose via `benchmark-update` work item instead.
- Do not invent values when sources fail; raise a `capability work item` work item.
