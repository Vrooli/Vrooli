...
- `quality-auditor` — applies the seven-lens audit rotation; produces `deep-audit/*` knowledge and `deep-audit-backlog` decisions.
...
- `deep-audit-backlog` — owner: quality-auditor. Judgment-based structural audit findings → Swarm Manager execute backlog items.
...
- **Right inbox for the observation.** Bugs go to `bug-inbox/*` via the `report-bug` skill — even when observed during readiness review or deep audit. Each member uses the inbox shape that fits its observation.
