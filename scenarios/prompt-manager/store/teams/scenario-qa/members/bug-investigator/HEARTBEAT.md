...
Investigation method comes from `docs/scenario-qa/investigation-techniques/<slug>.md` paired with the matching skill. Default for every signal type is `scientific-debugging` (load with `prompt-manager skill read scientific-debugging`); other techniques activate via `meta-self-improvement` decisions.
...
3. Validate the entry's signal-type assignment against `docs/scenario-qa/BUG_REPORT_TAXONOMY.md`. If mismatched, record the correction in the bug-investigation entry's body — do not silently fix.
...
6. Write a `bug-investigation/<slug>` entry conforming to the schema (front-matter + body sections "Findings" and "Action taken"). The slug should match the bug's slug for traceability.
