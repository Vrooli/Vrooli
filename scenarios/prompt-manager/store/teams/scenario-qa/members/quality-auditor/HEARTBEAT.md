...
2. **Gated one-time adoption.** For each eligible lens NOT already in my `audit-lens-adoption/*` knowledge log (i.e. new since a prior heartbeat), do a one-time assessment *before* auditing with it:
   ...
   - If it does NOT qualify (mis-tagged, no paired doc, not actually an audit lens — e.g. a campaign/tooling skill that happens to carry the `audit-technique` tag) → write `audit-lens-adoption/<slug>` knowledge (verdict: NEEDS-REVIEW) and a handoff note, and do NOT audit with it this cycle. Do not edit the skill, do not create a new skill, do not change its tags.
...
## Handoff Shape
### Rotation query result (eligible lenses this heartbeat)
### Adoption assessments (new lenses gated this heartbeat: ADOPTED / NEEDS-REVIEW)
### Scenario audited
### Skill applied
### Findings
