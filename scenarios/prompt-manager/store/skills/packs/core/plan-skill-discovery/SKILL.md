...
**Actions:**
1. Determine the type of work using this decision tree:

```
What kind of work is this?
├─ Bug/debugging        → search "debugging <domain>"
├─ New feature/scenario → search "<scenario-name> OR <domain>"
├─ Refactor/cleanup     → search "refactor <area>"
├─ Deployment/infra     → search "deploy OR infrastructure <target>"
└─ Unsure               → search "<describe the goal>"
```

**Exit criteria:**
- [ ] Work type identified
- [ ] Search query formulated

---

### **Phase 2: Search**

**Entry criteria:** Search query is ready.

**Actions:**
1. **Run the search:**
   ```bash
   prompt-manager search "<query>" -limit 5
   ```
2. **If too many results, narrow with tags:**
   ```bash
   prompt-manager search "<query>" -tag testing
   prompt-manager search "<query>" -tag infrastructure
   ```
3. **If results seem off, try alternative queries** — rephrase using different domain terms.

**Exit criteria:**
- [ ] Candidate skills identified (0-5 results)
...
3. **Discard irrelevant results** — Only keep skills that add real value.
...
| Potentially useful but not essential | Include as optional reading |
...
**Actions:**
1. **Add a Required Reading section** at the top of the plan:
   ```markdown
   ## Required Reading
   prompt-manager skill read <skill-1> <skill-2> <skill-3>
   ```
2. **Add Optional Reading** if there are nice-to-have skills:
   ```markdown
   ## Optional Reading
   prompt-manager skill read <skill-4>
   ```
3. This ensures any agent executing the plan loads the same context.
...
| **Vague search queries** | Poor results | Use the classification tree for targeted queries |
...
1. **A search query** based on the work classification
2. **A relevance assessment** of found skills
3. **A Required Reading section** in the plan (even if empty — state "no relevant skills found")
4. **A greenfield statement** unless the user explicitly requires backwards compatibility
5. **A cleanup & verification step** if the plan modifies a scenario

You **should** also:
- Try at least 2 search queries if the first yields poor results
