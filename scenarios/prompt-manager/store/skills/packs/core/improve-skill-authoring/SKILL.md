...
metadata:
  ...
  requires:
    ...
    commands: ["prompt-manager skill read", "measures-health validate scenario", "program-runtime bindings describe", "program-runtime sessions create", "program-runtime programs submit", "program-runtime sessions delete", "prompt-manager skill-set validate", "vrooli scenario test"]
  origin:
    kind: "authored"
---
...
1. `collect`: use `gather` for independent governed reads, including existing sensor programs called as `lib.<scenario>.<name>(input=value)`. Compose a child when its declared effects and budget fit the board and its envelope supplies the row's evidence. Check child status and validity conditions (for example, truncation, sample count, and unknown attribution) before using the reading. Use `read_elsewhere:<program>` only when the child needs a separate budget, effects, or human interpretation; state that reason in the contract. Emit other unavailable rows using the vocabulary in `program-contracts.md`: `no_governed_binding`, `pending_telemetry`, or `kernel_invoke_budget`. Classify bridge exceptions with that guide's `classify_transport`; propagate a child's typed errors instead of converting an unsuccessful child into a healthy row.
