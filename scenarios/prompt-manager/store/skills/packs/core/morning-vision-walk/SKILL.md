...
Validation-queue entries are the same pattern for the market-validator's `validation-queue/<request-type>/<slug>` topic. The `market-validation-triage` triages and converts to `monetization/market-scan/<slug>` or raises a benchmark/pricing/financial-model-assumption decision. The queue view (`team knowledge-list monetization --topic-prefix=validation-queue/`) is always the unrouted set.
