...
The shape of a backlog item, goal, and milestone — what `title`,
`description`, and acceptance criteria must say — lives in
`swarm-manager-work-authoring`. Read it before you create or edit work.
...
swarm-manager goals milestone-create --goal "my-goal" --name "build" --title "Build" \
  --acceptance "Given the sources, when the build runs, then it produces a signed artifact."
swarm-manager goals milestone-update --goal "my-goal" --name "build" --title "Build" \
  --acceptance "Given the sources, when the build runs, then it produces a signed artifact."
...
`--acceptance` is repeatable and required: a milestone without acceptance criteria can never be reviewed, so its goal can never be closed out. `milestone-update` replaces the whole definition, so restate every criterion you intend to keep.

Use `goals context` as the single-call loader before proposing a goal or backlog change. It returns target roots, dependency-derived scope, milestone rollups, and unassigned scope items. The derived scope is authoritative; do not reconstruct membership client-side.
