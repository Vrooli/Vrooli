...
func fakeEmbedderOK() Embedder {
	return newEmbedderWithRunner("", func(_ context.Context, _ []string, _ []byte) ([]byte, error) {
...
func TestEmbedder_Embed_RunnerError(t *testing.T) {
	...
	e := newEmbedderWithRunner("custom.embedding", r.run)
	...
	for i, a := range r.gotArgs {
		if a == "--role" && i+1 < len(r.gotArgs) {
			if r.gotArgs[i+1] != "embedding.default" {
				t.Fatalf("default role = %q", r.gotArgs[i+1])
			}
			return
		}
	}
