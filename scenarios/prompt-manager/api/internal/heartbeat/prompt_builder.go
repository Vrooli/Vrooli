...
func buildSharedDoctrineSection(includeHeartbeat bool) string {
	...
	section.WriteString("1. A broken behavior or defect: read `prompt-manager skill read report-bug` and file the typed bug report.\n2. Missing, confusing, or repeated process friction: read `prompt-manager skill read report-friction`.\n3. A raw observation whose outcome is not shaped: use `swarm-manager captures create`; do not turn every observation into implementation work.\n4. A bounded, evidenced outcome: create or update one Swarm backlog item with the outcome, acceptance criteria, allowed paths, evidence source, and suggested owner skills.\n5. Material scenario development: shape the backlog item through `implementation-plan-authoring`/Plan Manager, bind its canonical `plan_ref`, and accept the current plan revision before asking for an adaptive grant. Include the target scenario's usage and improve skills, validation evidence, special instructions, and any optional Tech Tree Designer revision in the plan context.\n6. Successive repairs under one approved plan: propose the `adaptive-improvement` execution strategy. Swarm retains the reviewed grant and checkpoint; Agent Manager runs `scenario-improvement-campaign` only after operator approval. A goal states the outcome and points to the work package; it does not replace the plan.\n\nApproval retains authority and does not launch work. Never approve, queue, or execute a plan from a heartbeat.\n\n")
