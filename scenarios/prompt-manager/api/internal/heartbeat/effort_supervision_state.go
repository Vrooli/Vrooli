...
type SupervisionState struct {
	...
	// WakeHints are owner-change notifications. They are hints only: Tick still
	// rereads Agent Manager and admission remains owner-authoritative.
	WakeHints map[string]string `json:"wakeHints,omitempty"`
...
type SupervisedCut struct {
	...
	LastObservedKey     string    `json:"lastObservedKey,omitempty"`
}
...
func (s FileSupervisionStateStore) Load(teamID, agentID string) (*SupervisionState, error) {
	...
	if os.IsNotExist(err) {
		return &SupervisionState{Version: 1, Status: "idle", Efforts: map[string]SupervisedCut{}, WakeHints: map[string]string{}}, nil
	}
	if err != nil {
		return nil, err
	}
	...
	if state.Version == 0 {
		// Older runtime state had no explicit version but is structurally
		// compatible. Preserve pending/unresolved dispatch fences and let the
		// next owner observation populate the new admission fields.
		state.Version = 1
	}
