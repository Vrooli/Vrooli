...
func TestToPMTeam_Basic(t *testing.T) {
	...
	if imp.Team.OperatingContract == nil {
		t.Fatal("Team.OperatingContract must be initialized for persistent-store validation")
	}
	if _, ok := imp.Team.OperatingContract.Members["alice"]; !ok {
		t.Errorf("operating contract missing imported member alice: %+v", imp.Team.OperatingContract.Members)
	}
