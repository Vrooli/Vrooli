...
func (h *Handlers) Update(w http.ResponseWriter, r *http.Request) {
	...
	updates, err := h.agentStore.Get(ctx, id)
	if err != nil {
		http.Error(w, "Agent not found", http.StatusNotFound)
		return
	}

	// Merge the sparse request into the current entity. AgentStore.Update has
	// replacement semantics for mutable fields so explicit empty strings and
	// empty lists remain distinguishable from omitted fields here.
