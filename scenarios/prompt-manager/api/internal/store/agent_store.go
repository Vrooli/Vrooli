...
func (s *FileAgentStore) Update(ctx context.Context, id string, updates *Agent) error {
	...
	// Replace every mutable field. Transport/domain adapters merge sparse
	// patches before calling Update, which preserves intentional clears without
	// relying on zero-value heuristics in the persistence layer.
	...
	agent.Connectors = updates.Connectors
	agent.DefaultProfileRef = updates.DefaultProfileRef
	agent.Heartbeat = updates.Heartbeat
