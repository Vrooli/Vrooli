...
const (
	...
	// OperatingGraphNodeKindInstrument is a scenario that COMPUTES what a plan
	// of record would otherwise restate — a team's setpoint, its cell grid, its
	// ranked findings. It is distinct from `external`: an external node is
	// something the team merely reads, while an instrument is the authority the
	// team deliberately moved its own durable numbers into. Modelling one as
	// external would erase exactly the relationship the migration created.
	OperatingGraphNodeKindInstrument OperatingGraphNodeKind = "instrument"
)
