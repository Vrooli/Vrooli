...
- [x] prompt-manager-nice-to-have-3d-world-visualization-for-agents | Agent world | Visualize agents and coordination state on a seeded terrain world where terrain and place are state: each agent stands at its desk when running, at the team table when a heartbeat is due, or in the commons when idle; weather reflects swarm health, and the actionable HUD also works without WebGL.
