...
CREATE INDEX IF NOT EXISTS idx_scheduled_events_start ON scheduled_events(start_time) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_scheduled_events_end ON scheduled_events(end_time) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_scheduled_events_organizer ON scheduled_events(organizer_id) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_scheduled_events_status ON scheduled_events(status);
CREATE INDEX IF NOT EXISTS idx_scheduled_events_recurring ON scheduled_events(recurrence_pattern_id) WHERE is_recurring = true;
CREATE INDEX IF NOT EXISTS idx_scheduled_events_external ON scheduled_events(external_id, external_source);
CREATE INDEX IF NOT EXISTS idx_scheduled_events_timezone ON scheduled_events(timezone);
CREATE INDEX IF NOT EXISTS idx_scheduled_events_daterange ON scheduled_events USING gist(tstzrange(start_time, end_time));

CREATE INDEX IF NOT EXISTS idx_business_hours_entity ON business_hours(entity_id, entity_type);
...
CREATE INDEX IF NOT EXISTS idx_time_analytics_entity ON time_analytics(entity_id, entity_type);
CREATE INDEX IF NOT EXISTS idx_time_analytics_period ON time_analytics(period_start, period_end);

CREATE INDEX IF NOT EXISTS idx_conflicts_unresolved ON scheduling_conflicts(resolution_status) WHERE resolution_status = 'unresolved';

CREATE INDEX IF NOT EXISTS idx_available_slots_entity ON available_slots(entity_id, entity_type);
CREATE INDEX IF NOT EXISTS idx_available_slots_time ON available_slots(start_time, end_time);
CREATE INDEX IF NOT EXISTS idx_available_slots_can_schedule ON available_slots(can_schedule) WHERE can_schedule = true;
...
DROP TRIGGER IF EXISTS update_timezone_definitions_updated_at ON timezone_definitions;
CREATE TRIGGER update_timezone_definitions_updated_at BEFORE UPDATE ON timezone_definitions
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_recurrence_patterns_updated_at ON recurrence_patterns;
...
DROP TRIGGER IF EXISTS detect_conflicts_on_event_insert ON scheduled_events;
...
BEGIN
    work_date := DATE(start_ts AT TIME ZONE tz);
    
    WHILE work_date <= DATE(end_ts AT TIME ZONE tz) LOOP
        -- Get business hours for this day
        SELECT start_time, end_time INTO day_start, day_end
        FROM business_hours
        WHERE entity_id = entity_id_param
          AND day_of_week = EXTRACT(DOW FROM work_date)
        ...
        IF day_start IS NOT NULL THEN
            ...
            current_start := GREATEST(
                start_ts,
                (work_date || ' ' || day_start)::TIMESTAMP AT TIME ZONE tz
            );
            current_end := LEAST(
                end_ts,
                (work_date || ' ' || day_end)::TIMESTAMP AT TIME ZONE tz
            );
...
SELECT 
    ...
    se.location,
    se.organizer_id,
    jsonb_array_length(COALESCE(se.participants, '[]'::jsonb)) as participant_count
