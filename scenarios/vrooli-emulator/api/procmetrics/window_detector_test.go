...
func TestXdotoolDetector_DisplayUnset_NoError(t *testing.T) {
	t.Setenv("DISPLAY", "")
	// Simulate xdotool installed but unable to connect when DISPLAY is empty.
	shell := func(_ context.Context, env []string, name string, _ ...string) ([]byte, error) {
		if name == "which" {
			return []byte("/usr/bin/xdotool"), nil
		}
		// Surface DISPLAY misconfiguration as a shell error, mirroring real xdotool behavior.
		for _, kv := range env {
			if kv == "DISPLAY=" {
				return nil, fmt.Errorf("Can't open display")
			}
		}
		return []byte(""), nil
	}
	...
	result, err := d.HasVisibleWindow(context.Background(), 1234, "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result {
		t.Error("expected false when DISPLAY is unset")
	}

	geo, err := d.LargestVisibleWindow(context.Background(), 1234, "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if geo != nil {
		t.Error("expected nil geometry when DISPLAY is unset")
	}
}
