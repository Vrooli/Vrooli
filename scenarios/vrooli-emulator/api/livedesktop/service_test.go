...
func TestStartSession_Success(t *testing.T) {
	...
	assert.False(t, session.Headless)
}

func TestStartSession_Headless(t *testing.T) {
	store := NewInMemoryStore()
	backend := newMockBackend()
	svc := newTestService(store, backend)
	...
	assert.Equal(t, 0, session.VNCPort)
	assert.Equal(t, 0, session.WSPort)
	assert.Nil(t, session.RemoteAccess, "headless sessions skip remote access")
	require.NotNil(t, session.Display)
	assert.True(t, session.Display.IsRunning())
}
