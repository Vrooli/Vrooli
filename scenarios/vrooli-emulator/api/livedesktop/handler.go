...
// Handler holds HTTP handlers for the session domain.
type Handler struct {
	service *Service
}
...
// RegisterRoutes registers all session API routes under /api/v1/sessions/.
func (h *Handler) RegisterRoutes(r *mux.Router) {
	r.HandleFunc("/api/v1/sessions", h.startSession).Methods("POST", "OPTIONS")
	r.HandleFunc("/api/v1/sessions", h.listSessions).Methods("GET")
	r.HandleFunc("/api/v1/sessions/{id}", h.getSession).Methods("GET")
	r.HandleFunc("/api/v1/sessions/{id}/heartbeat", h.heartbeat).Methods("POST", "OPTIONS")
	r.HandleFunc("/api/v1/sessions/{id}/launch", h.launchApp).Methods("POST", "OPTIONS")
	r.HandleFunc("/api/v1/sessions/{id}/control", h.controlAction).Methods("POST", "OPTIONS")
	r.HandleFunc("/api/v1/sessions/{id}/metrics", h.getMetrics).Methods("GET")
	r.HandleFunc("/api/v1/sessions/{id}/files/{filename}", h.serveFile).Methods("GET")
	r.HandleFunc("/api/v1/sessions/{id}", h.stopSession).Methods("DELETE")
	r.HandleFunc("/api/v1/sessions/{id}/ws", h.handleVNCProxy)
}
