...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start notification-hub` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start notification-hub` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.notification_hub\.v1\./
