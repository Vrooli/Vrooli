...
`template-manager orient notification-hub`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient notification-hub
...
```bash
cat > /tmp/prd_context_notification-hub.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start notification-hub --interactive
# …answer the interview, then:
business-health wizard preview notification-hub
business-health wizard apply notification-hub
```
...
```bash
vrooli scenario requirements validate notification-hub --json
```
...
```bash
cat > /tmp/requirements_context_notification-hub.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate notification-hub --json
business-health fix preview notification-hub
business-health fix apply notification-hub
```
...
validate notification-hub --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/notification-hub/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest notification-hub <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> notification-hub
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario notification-hub
```
