# Troubleshooting — Notification Hub
...
```bash
vrooli scenario status notification-hub
make stop
vrooli scenario start notification-hub --clean-stale
```
...
notification-hub status --auto-start

# Or override the API base for this invocation
notification-hub status --api-base "http://localhost:$(vrooli scenario port notification-hub API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/notification-hub/`.
...
The default route is `${SCENARIO_DATA_DIR}/notification-hub.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/notification-hub.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/notification-hub.db}")"
```
...
`packages/proto/gen/go/notification-hub/v1/`, TypeScript under
`packages/proto/gen/typescript/js/notification-hub/v1/`, and Python under
`packages/proto/gen/python/notification_hub/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/notification-hub/v1/<domain>/`.
