# API Endpoints — Notification Hub
...
`packages/proto/schemas/notification-hub/v1/<domain>/<file>.proto`.
...
envelope (`packages/proto/schemas/notification-hub/v1/shared/errors.proto`):
...
| **CLI** | `notification-hub status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/notification-hub/v1/shared/health.proto`
...
`POST /vrooli.notification_hub.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.notification_hub.v1.notes.NotesService/ListNotes`
...
| **CLI** | `notification-hub notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.notification_hub.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.notification_hub.v1.notes.NotesService/CreateNote`
...
| **CLI** | `notification-hub notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.notification_hub.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.notification_hub.v1.notes.NotesService/GetNote`
...
| **CLI** | `notification-hub notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.notification_hub.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `notification-hub notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/notification-hub/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/notification-hub/v1/<domain>/`, then run
   `make generate`.
