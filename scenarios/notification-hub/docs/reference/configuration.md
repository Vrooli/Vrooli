# Configuration — Notification Hub
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/notification-hub.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `notification-hub` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/notification-hub/config.json`
3. `~/.vrooli/config/notification-hub/config.json`
4. `~/.config/vrooli/notification-hub/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port notification-hub API_PORT`)
