...
```bash
vrooli scenario port notification-hub UI_PORT
```
...
notification-hub <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port notification-hub API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.notification_hub.v1.<domain>.<Service>/<Method>
```
...
notification-hub notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port notification-hub API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.notification_hub.v1.notes.NotesService/ListNotes" \
