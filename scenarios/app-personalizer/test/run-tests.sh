#!/bin/bash
# Phased test orchestrator for app-personalizer scenario
set -euo pipefail
...
# Initialise shared runner with runtime management enabled so integration/business
# phases can assume the API is available.
testing::runner::init \
  --scenario-name "app-personalizer" \
...
# Register phases with conservative timeouts tailored to API operations.
...
testing::runner::register_phase --name performance --script "$PHASE_DIR/test-performance.sh" --timeout 60 --requires-runtime false

# Handy presets for fast iteration.
