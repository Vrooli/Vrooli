...
func (identities) Validate(_ context.Context, token string) (owneridentity.Identity, error) {
	if token == "expired" {
		return owneridentity.Identity{Subject: "alice", ExpiresAt: time.Now().Add(-time.Second)}, nil
	}
	if token != "alice" && token != "bob" {
		return owneridentity.Identity{}, owneridentity.ErrUnauthenticated
	}
	return owneridentity.Identity{Subject: token, ExpiresAt: time.Now().Add(time.Minute)}, nil
}
