...
// generated reference domain and add their own domains, so pinning the count would
...
// domain's own *_test.go for canonical apply-and-query coverage.
...
// per-domain module tests) so the safety net applies
...
func TestProtoConnectParity(t *testing.T) {
	...
	if len(files) == 0 {
		t.Log("no Connect-mounted domain modules registered yet")
		return
	}
