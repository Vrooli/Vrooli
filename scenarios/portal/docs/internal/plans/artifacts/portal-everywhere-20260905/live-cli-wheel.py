"""Bounded real X11 wheel acceptance through the installed owner CLI.
...
name(display, window, b'Vrooli disposable wheel acceptance')
...
opened = None
try:
    with tempfile.TemporaryDirectory(prefix='wheel-fixture-', dir=base) as temporary:
        work = Path(temporary)
        ...
        try:
            ...
            wheel = {'session': opened['session'], 'command_id': 'fixture-wheel-once',
                     'geometry_revision': observed['geometry_revision'],
                     'action': {'wheel': {'display_id': observed['display_id'], 'x': px.value, 'y': py.value,
                                          'horizontal_ticks': 1, 'vertical_ticks': -1}}}
            receipt = call('act', wheel)
            duplicate = call('act', wheel)
            drain()
            assert receipt['receipt']['outcome'] == 'applied'
            assert duplicate == receipt
            assert [e['type'] for e in events] == ['press', 'release', 'press', 'release'], events
            assert [e['button'] for e in events] == [4, 4, 7, 7], events
            assert all(e['x'] == 80 and e['y'] == 50 for e in events)
