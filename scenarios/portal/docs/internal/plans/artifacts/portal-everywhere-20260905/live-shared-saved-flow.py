"""Physical desktop semantic fixture; Portal promotes; account-authenticated CLI replays the identical revision."""
import ctypes as c
import json
import os
...
try:
 with tempfile.TemporaryDirectory(prefix='unicode-fixture-',dir=base) as temporary:
  ...
  try:
   ...
   token_file=work/'access-token';token_file.write_text(token);token_file.chmod(0o600)
   cli_routes=set()
   def snake(value):
   ...
   def call(operation,body,extra=()):
    if operation in ('get-saved-flow','run-saved-flow') and operation not in cli_routes:
     cli_routes.add(operation)
     request_file=work/(operation+'.json');request_file.write_text(json.dumps(body));request_file.chmod(0o600)
     command=['device-control','desktop',operation,'--socket',str(base/'desktop-owner.sock'),'--request',str(request_file),'--json']
     if operation=='get-saved-flow':
      denied=subprocess.run(command,capture_output=True,text=True,timeout=10)
      assert denied.returncode!=0 and 'permission_denied' in denied.stderr, 'local principal inherited account session'
     result=subprocess.run(command+['--access-token-file',str(token_file)],capture_output=True,text=True,timeout=40)
   ...
   evidence['same_saved_revision_portal_and_cli']=True; evidence['cli_replay_portal_duplicate_record_equal']=True
   evidence['local_principal_cannot_inherit_account_session']=True
