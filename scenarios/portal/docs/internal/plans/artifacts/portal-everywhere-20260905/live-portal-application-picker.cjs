...
(async()=>{
 ...
 try {
  ...
  const evidence={timestamp:new Date().toISOString(),browser:'installed Chromium',physical_desktop_session:config.helper.session_id,fixture_pid:child.pid,login:true,control_open:true,application_discovery:true,named_application_selection:true,semantic_inspection:true,exact_opaque_field_selection:true,unicode_persisted_exactly:true,no_sensitive_browser_storage:true,stop:true,logout:true};
  fs.writeFileSync(path.join(__dirname,'live-portal-application-picker.json'),JSON.stringify(evidence,null,2)+'\n');console.log(JSON.stringify(evidence));
