...
## Native helper adapter boundary — 2026-09-06

The helper selects its native backend from the runtime host. Linux continues to
use the authenticated X11 plus AT-SPI path. Darwin and Windows builds use a
bounded user-session capture/input seam and lifecycle session monitor; X11
authority material is optional on those hosts. Semantic text actions are
rejected unless a platform Accessibility/UI Automation adapter owns the opaque
observed element, so foreground typing cannot masquerade as semantic control.
A Wayland login is never treated as X11 merely because XWayland exports a
`DISPLAY`; the helper and strategy return an explicit portal-backed-adapter
requirement. Cross-builds are packaging evidence only and cannot satisfy live
platform rows.
