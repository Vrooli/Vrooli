...
try:
 with tempfile.TemporaryDirectory(prefix='unicode-fixture-',dir=base) as temporary:
  ...
  try:
   ...
   try:
    observed=call('observe',{'session':opened['session'],'process_id':child.pid},('--output',str(work/'image.png')))
    ...
    action={'session':opened['session'],'command_id':'unicode-once','geometry_revision':observed['geometry_revision'],'action':{'text':{'text':text,'element_id':entries[0]['element_id'],'observation_revision':semantic['revision'],'position':2}}}
