...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/portal.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `portal` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/portal/config.json`
3. `~/.vrooli/config/portal/config.json`
4. `~/.config/vrooli/portal/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port portal API_PORT`)
