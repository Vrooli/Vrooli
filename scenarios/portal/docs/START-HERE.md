...
`vrooli scenario orient portal`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient portal
...
```bash
cat > /tmp/prd_context_portal.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start portal --interactive
# …answer the interview, then:
business-health wizard preview portal
business-health wizard apply portal
```
...
```bash
vrooli scenario requirements validate portal --json
```
...
```bash
cat > /tmp/requirements_context_portal.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate portal --json
business-health fix preview portal
business-health fix apply portal
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate portal --json`.

The notes page spec is part of the removable example domain. When you
run `vrooli scenario detemplate portal`, the notes page spec
...
- [ ] **Start in proto.** Author `packages/proto/schemas/portal/v1/<domain>/<domain>.proto`
