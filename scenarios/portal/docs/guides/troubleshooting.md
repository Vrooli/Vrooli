...
make stop
vrooli scenario start portal --clean-stale
```
...
```bash
make status
vrooli scenario port portal UI_PORT
```
...
portal status --auto-start

# Or override the API base for this invocation
portal status --api-base "http://localhost:$(vrooli scenario port portal API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/portal/`.
...
The default route is `${SCENARIO_DATA_DIR}/portal.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/portal.db"
```
...
`packages/proto/gen/go/portal/v1/`, TypeScript under
`packages/proto/gen/typescript/portal/v1/`, and Python under
`packages/proto/gen/python/portal/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/portal/v1/<domain>/`.
