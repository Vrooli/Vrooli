...
CREATE TABLE IF NOT EXISTS unstructured_sessions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX IF NOT EXISTS idx_tasks_content_embedding ON tasks USING ivfflat (content_embedding vector_cosine_ops);
...
CREATE INDEX IF NOT EXISTS idx_sessions_app ON unstructured_sessions(app_id);
CREATE INDEX IF NOT EXISTS idx_sessions_processed ON unstructured_sessions(processed);
...
CREATE INDEX IF NOT EXISTS idx_transitions_created ON task_transitions(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_research_task ON research_artifacts(task_id);
CREATE INDEX IF NOT EXISTS idx_research_embedding ON research_artifacts USING ivfflat (content_embedding vector_cosine_ops);
...
DROP TRIGGER IF EXISTS update_templates_updated_at ON task_templates;
