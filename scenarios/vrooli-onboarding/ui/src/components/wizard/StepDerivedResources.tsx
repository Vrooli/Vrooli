...
export function StepDerivedResources({ selected, operatorState, onToggle }: { selected: Set<string>; operatorState: OperatorState | null; onToggle: (name: string, enabled: boolean) => void }) {
  ...
  return <div data-testid="step-derived-resources">
    ...
    <p className="mt-2 text-sm text-slate-300">Resources are derived from the shared dependency closure. Required resources cannot be independently disabled here.</p>
    ...
    {!isLoading && !error && <div className="mt-6 space-y-5">
      ...
      {required.length + optional.length + standalone.length === 0 && <p data-testid="empty-note" role="status" className="text-slate-300">The selected scenarios do not require local resources.</p>}
    </div>}
  </div>;
}
...
function ResourceGroup({ title, items, locked = false, operatorState, onToggle }: { title: string; items: { name: string; description?: string; category?: string; enabled?: boolean }[]; locked?: boolean; operatorState: OperatorState | null; onToggle: (name: string, enabled: boolean) => void }) {
  return <section aria-labelledby={`resource-group-${title.toLowerCase()}`} data-testid={`resources-${title.toLowerCase()}`} role="group">
    <h2 id={`resource-group-${title.toLowerCase()}`} className="text-sm font-semibold uppercase tracking-wide text-slate-300">{title}{locked && " · always included"}</h2>
    {items.length === 0 ? <p className="mt-2 text-sm text-slate-300">No {title.toLowerCase()} resources.</p> : <ul className="mt-2 grid gap-2 sm:grid-cols-2">{items.map((resource) => { const checked = locked || operatorState?.resources?.[resource.name]?.enabled === true || resource.enabled === true; return <li key={resource.name} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm"><label className="flex gap-3"><input type="checkbox" data-testid="resource-entry" className="min-h-11 min-w-11" checked={checked} disabled={locked} onChange={(event) => onToggle(resource.name, event.target.checked)} /><span><span className="font-medium text-slate-100">{resource.name}</span>{resource.category && <span className="ml-2 text-xs text-slate-400">{resource.category}</span>}{resource.description && <span className="mt-1 block text-xs text-slate-300">{resource.description}</span>}{locked && <span className="mt-1 block text-xs text-emerald-200" data-testid="required-reason" role="note">Required by the selected scenario closure</span>}</span></label></li>; })}</ul>}
  </section>;
}
