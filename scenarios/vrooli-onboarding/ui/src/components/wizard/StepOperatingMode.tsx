...
export function StepOperatingMode({ selected, overrides, onAutoRestart }: { selected: Set<string>; overrides?: Record<string, { auto_restart?: boolean }>; onAutoRestart: (name: string, enabled: boolean) => void }) {
  ...
  return <div data-testid="step-operating-mode">
    ...
    <div className="mt-6 space-y-3">{scenarios.length === 0 && <div data-testid="operating-mode-row" role="group" className="rounded-xl border border-white/10 bg-white/5 p-4"><span data-testid="recommendation-note" role="note" className="text-sm text-slate-300">Loading operating-mode recommendations…</span><input data-testid="keep-running-toggle" type="checkbox" disabled aria-label="Keep selected scenarios running" className="sr-only" /></div>}{scenarios.map((scenario) => {
      ...
      return <label key={scenario.name} data-testid="operating-mode-row" role="group" className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 p-4"><span><span className="block font-medium">{scenario.name}</span><span className="text-xs text-slate-300" data-testid="recommendation-note" role="note">{scenario.auto_restart ? "Recommended to keep running" : "Recommended on demand"}</span>{overridden && <span className="mt-1 block text-xs text-emerald-200" data-testid="override-indicator" role="note">Operator override saved</span>}</span><input type="checkbox" checked={autoRestart} onChange={(event) => onAutoRestart(scenario.name, event.target.checked)} aria-label={`Keep ${scenario.name} running`} data-testid="keep-running-toggle" className="min-h-11 min-w-11 accent-emerald-500" /></label>;
    })}</div>
  </div>;
}
