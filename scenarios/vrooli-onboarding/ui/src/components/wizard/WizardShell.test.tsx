...
describe("WizardShell", () => {
  ...
  it("marks current step with aria-current", () => {
    render(
      <WizardShell {...defaultProps} currentStep={1}>
        Content
      </WizardShell>,
    );
  ...
  it("renders progress bar with correct aria attributes", () => {
    render(
      <WizardShell {...defaultProps} currentStep={2}>
        Content
      </WizardShell>,
    );
  ...
  it("hides Next button when showNext is false", () => {
    render(
      <WizardShell {...defaultProps} showNext={false}>
        Content
      </WizardShell>,
    );
  ...
  it("shows Back button when showPrev is true and step > 0", () => {
    render(
      <WizardShell {...defaultProps} currentStep={1} showPrev={true}>
        Content
      </WizardShell>,
    );
  ...
  it("hides Back button on first step", () => {
    render(
      <WizardShell {...defaultProps} showPrev={false}>
        Content
      </WizardShell>,
    );
  ...
  it("displays custom next label", () => {
    render(
      <WizardShell {...defaultProps} nextLabel="Generate Config">
        Content
      </WizardShell>,
    );
    expect(screen.getByTestId("wizard-next")).toHaveTextContent(
      "Generate Config",
    );
  });
  ...
  it("Back button has accessible label", () => {
    ...
    expect(screen.getByTestId("wizard-prev")).toHaveAttribute(
      "aria-label",
      "Go to previous step",
    );
  });
  ...
  it("Next button has accessible label matching nextLabel", () => {
    render(
      <WizardShell {...defaultProps} nextLabel="Get Started">
        Content
      </WizardShell>,
    );
    expect(screen.getByTestId("wizard-next")).toHaveAttribute(
      "aria-label",
      "Get Started",
    );
  });
  ...
  it("makes completed step indicators clickable when onGoToStep is provided", () => {
    ...
    render(
      <WizardShell {...defaultProps} currentStep={2} onGoToStep={goToStep}>
        Content
      </WizardShell>,
    );
  ...
  it("does not make future step indicators clickable", () => {
    const goToStep = vi.fn();
    render(
      <WizardShell {...defaultProps} currentStep={1} onGoToStep={goToStep}>
        Content
      </WizardShell>,
    );
  ...
  it("calls onNext when Next button is clicked", () => {
    const onNext = vi.fn();
    render(
      <WizardShell {...defaultProps} onNext={onNext}>
        Content
      </WizardShell>,
    );
  ...
  it("shows step number for non-completed future steps", () => {
    render(
      <WizardShell {...defaultProps} currentStep={0}>
        Content
      </WizardShell>,
    );
