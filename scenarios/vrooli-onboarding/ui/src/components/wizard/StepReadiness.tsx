        ...
        <div data-testid="credential-status">
          <SetupTask
            ...
            actions={canProvision ? <InputGroup className="credential-list__form" size="lg" shape="rounded" testId="credential-entry-group">
              <InputGroup.Field>
                <Input data-testid="credential-input" type="password" aria-label={i18n.t("onboarding.readiness.valueFor", { label: credential.label || credential.field })} placeholder={credential.placeholder} autoComplete="off" value={values[key] ?? ""} onChange={(event) => onValueChange(key, event.target.value)} className="min-w-0 flex-1" />
              </InputGroup.Field>
