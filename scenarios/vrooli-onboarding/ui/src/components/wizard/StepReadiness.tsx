...
export function StepReadiness({ title = "Validation" }: { title?: "Credentials" | "Validation" }) {
  ...
  return <div data-testid="step-readiness">
    ...
    {data?.recovery && <div className={`mt-4 rounded-lg border p-3 text-sm ${data.recovery.receipt_exists && data.recovery.uncovered.length === 0 ? "border-emerald-300/30 bg-emerald-300/10" : "border-amber-300/30 bg-amber-300/10"}`}><p className="font-medium text-amber-100">Recovery backup</p>{data.recovery.receipt_exists ? <p className="mt-1 text-slate-200">Verified bundle covers {data.recovery.entry_count} credential entries{data.recovery.uncovered.length ? `; ${data.recovery.uncovered.length} configured entries still need coverage.` : "."}</p> : <p className="mt-1 text-slate-200">Step: create and verify a recovery bundle with <code>secrets-manager backup export --output &lt;bundle&gt; &lt; passphrase</code>, then store the bundle and passphrase separately.</p>}</div>}
