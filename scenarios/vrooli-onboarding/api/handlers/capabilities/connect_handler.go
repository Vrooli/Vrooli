...
func (h *connectHandler) PreviewCapability(ctx context.Context, req *connect.Request[capabilitiesv1.PreviewCapabilityRequest]) (*connect.Response[capabilitiesv1.PreviewCapabilityResponse], error) {
	action, err := actionRequest(req.Msg.GetAction())
	if err != nil {
...
func actionRequest(action *capabilitiesv1.ActionRequest) (operatorcapability.ActionRequest, error) {
	if action == nil {
	...
	request := operatorcapability.ActionRequest{CapabilityID: action.GetCapabilityId(), IdempotencyKey: action.GetIdempotencyKey(), Confirm: action.GetConfirm(), Inputs: inputs}
...
func descriptorToProto(descriptor operatorcapability.Descriptor) *capabilitiesv1.CapabilityDescriptor {
	...
	return &capabilitiesv1.CapabilityDescriptor{
		...
		Evidence:    &capabilitiesv1.CapabilityEvidenceContract{Kinds: descriptor.Evidence.Kinds, RequiredFields: descriptor.Evidence.RequiredFields, SecretFree: descriptor.Evidence.SecretFree, Freshness: descriptor.Evidence.Freshness},
