...
var Endpoints = []module.EndpointDescriptor{
	...
	{ID: "session_profile_get", Path: sessionconnect.SessionServiceGetProfileSessionProcedure, Method: "POST", Summary: "Read profile session", Description: "Returns the verified actor's durable non-secret guided/manual purpose context.", Category: "session"},
	{ID: "session_profile_save", Path: sessionconnect.SessionServiceSaveProfileSessionProcedure, Method: "POST", Summary: "Save profile session", Description: "Persists bounded profile answers, manual provenance, target context, and revisions without applying host changes.", Category: "session", Request: &module.Schema{Type: "object", Properties: map[string]string{"target": "string (required)", "expected_revision": "string (recommended for concurrent clients)", "session": "profile session (required; secrets rejected)"}}},
}
