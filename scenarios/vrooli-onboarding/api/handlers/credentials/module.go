...
var Endpoints = []module.EndpointDescriptor{
	...
	{ID: "credentials_reveal", Path: credentialconnect.CredentialsServiceRevealCredentialProcedure, Method: "POST", Summary: "Reveal one credential value", Description: "Returns exactly one credential value to a verified human operator holding the onboarding write capability, only when the request explicitly confirms the reveal. Every call is audited without the value.", Category: "credentials", Errors: []module.ErrorDesc{{Status: 400, Code: "invalid_argument", Description: "Credential identity is missing"}, {Status: 401, Code: "unauthenticated", Description: "Verified operator authorization is required"}, {Status: 403, Code: "permission_denied", Description: "Onboarding write capability is required"}, {Status: 404, Code: "not_found", Description: "Credential is not configured"}, {Status: 412, Code: "failed_precondition", Description: "Explicit reveal confirmation is required"}, {Status: 503, Code: "unavailable", Description: "Credential authority is unavailable"}}},
}
