...
func readinessProvenanceToProto(items []readiness.CredentialProvenance) []*credentialsv1.CredentialProvenance {
	...
	for _, item := range items {
		...
		result = append(result, &credentialsv1.CredentialProvenance{Version: item.Version, Owner: item.Owner, SourceRef: item.SourceRef, Kind: item.Kind, Provider: item.Provider, AppliesWhen: applicabilityToCredentialsProto(item.AppliesWhen), RequirementGroup: item.RequirementGroup, ConsumerRefs: item.ConsumerRefs, CompanionSettings: item.CompanionSettings, CompanionCredentials: item.CompanionCredentials, AcquisitionRef: item.AcquisitionRef, VerificationRef: item.VerificationRef, RecoveryRef: item.RecoveryRef, HelpRef: item.HelpRef, EvidencePolicy: item.EvidencePolicy, ProviderVersion: item.ProviderVersion, Env: item.Env, Label: item.Label, Description: item.Description, ObtainUrl: item.ObtainURL, Provisioning: item.Provisioning, DerivedFrom: item.DerivedFrom, Required: item.Required, Consumers: consumers})
	}
	return result
}
