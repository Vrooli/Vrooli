...
- [x] OT-P1-002 | Re-enterable setup | When an operator re-opens the wizard on a configured install, it should load committed state, resume at the first unsatisfied step, and allow revision of any decision that is not deferred.
...
- [x] OT-P2-004 | Configuration discovery | The wizard may publish its configuration surface to search-hub so an operator may find a setting by intent rather than by path.
