...
| `CredentialsService` | `ListCredentials`, `ProvisionCredential`, `DiagnoseCredentials`, `RevealCredential` | Read metadata, relay a write-only value, diagnose the authority, and reveal exactly one value to a verified human with the onboarding write capability after explicit confirmation. `RevealCredential` is the single value-returning procedure; it is audited without the value and the local CLI keeps its own terminal-only reveal. |
