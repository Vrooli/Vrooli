...
| operator-state-authority | Commit every operator decision through one typed service that merges field-scoped patches, validates before writing, and preserves fields it does not own. | mutation | `~/.vrooli/state/vrooli/vrooli-onboarding/operator-state.json`, via the control-plane service | `internal/operatorstate/**` | API, CLI, control plane | `ONB-STATE-*`, `ONB-REENTRY-RESUMES` |
