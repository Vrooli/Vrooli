...
// the template itself does not build because of vrooli-memory placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_vrooli_memory_v1_notes_notes_proto, "NotesService")
}
