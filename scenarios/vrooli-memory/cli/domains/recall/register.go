...
func Register(core *cliapp.ScenarioApp, manifest []byte) (cliapp.SubcommandGroup, error) {
	h := newHandlers(core)
	g, err := cliapp.LoadFromManifestPrimitives(manifest, GroupName, map[string]cliapp.PrimitiveHandler{"RecallService.Recall": cliapp.ProtoList(h.recallCall, h.recallReport), "RecallService.Wake": cliapp.ProtoList(h.wakeCall, h.wakeReport)})
	if err != nil {
