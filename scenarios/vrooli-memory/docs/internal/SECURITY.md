...
| Memory content leaking into federated search results | Memory registers as a search-hub provider, so hits surface in cross-corpus query output. | Intended behaviour — federated reach is `VMEM-P0-009`. The same D-005 acceptance applies. | accepted |
