...
```bash
vrooli scenario status vrooli-memory
# Then either:
make stop
# Or, as last resort:
pkill -f 'vrooli-memory-api'
...
vrooli-memory status --auto-start

# Or override the API base for this invocation
vrooli-memory status --api-base "http://localhost:$(vrooli scenario port vrooli-memory API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/vrooli-memory/`.
...
The default route is `${SCENARIO_DATA_DIR}/vrooli-memory.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/vrooli-memory.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/vrooli-memory.db}")"
```
...
`packages/proto/gen/go/vrooli-memory/v1/`, TypeScript under
`packages/proto/gen/typescript/js/vrooli-memory/v1/`, and Python under
`packages/proto/gen/python/vrooli_memory/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/vrooli-memory/v1/<domain>/`.
