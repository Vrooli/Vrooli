...
envelope (`packages/proto/schemas/vrooli-memory/v1/shared/errors.proto`):
...
| **CLI** | `vrooli-memory status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/vrooli-memory/v1/shared/health.proto`
...
`POST /vrooli.vrooli_memory.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.vrooli_memory.v1.notes.NotesService/ListNotes`
...
| **CLI** | `vrooli-memory notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.vrooli_memory.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.vrooli_memory.v1.notes.NotesService/CreateNote`
...
| **CLI** | `vrooli-memory notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.vrooli_memory.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.vrooli_memory.v1.notes.NotesService/GetNote`
...
| **CLI** | `vrooli-memory notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.vrooli_memory.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `vrooli-memory notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/vrooli-memory/v1/notes/notes.proto`.
