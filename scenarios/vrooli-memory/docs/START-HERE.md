...
`template-manager orient vrooli-memory`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient vrooli-memory
...
```bash
cat > /tmp/prd_context_vrooli-memory.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start vrooli-memory --interactive
# …answer the interview, then:
business-health wizard preview vrooli-memory
business-health wizard apply vrooli-memory
```
...
```bash
vrooli scenario requirements validate vrooli-memory --json
```
...
```bash
cat > /tmp/requirements_context_vrooli-memory.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate vrooli-memory --json
business-health fix preview vrooli-memory
business-health fix apply vrooli-memory
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate vrooli-memory --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate vrooli-memory`, the notes page spec
...
validate vrooli-memory --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/vrooli-memory/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest vrooli-memory <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> vrooli-memory
...
1. Add proto messages and a service under
   `packages/proto/schemas/vrooli-memory/v1/<domain>/`, then run
