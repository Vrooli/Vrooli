...
| Curated topology repair | harness | Scenario startup. | Project-level `AGENTS.md` exists and declared curated aliases point to it as symlinks. | Recovers a missing canonical file from surviving non-generated peers; never injects prompt text or wake content into curated instruction files. | L2 — deterministic, fail-closed recovery. |
