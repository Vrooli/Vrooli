...
react-component-library adoptions resolve-path <component-id> vrooli-memory
react-component-library adoptions apply <component-id> vrooli-memory <adopted-path>
```
...
Run `experience-manager spec validate vrooli-memory --json` after route or
selector changes. The generated notes page spec is example-domain content and
is removed by `template-manager detemplate vrooli-memory`.
