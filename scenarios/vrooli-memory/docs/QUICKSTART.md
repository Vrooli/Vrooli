...
```bash
vrooli scenario port vrooli-memory UI_PORT
```
...
vrooli-memory <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port vrooli-memory API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.vrooli_memory.v1.<domain>.<Service>/<Method>
```
...
```bash
vrooli-memory notes list
vrooli-memory notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port vrooli-memory API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.vrooli_memory.v1.notes.NotesService/ListNotes" \
