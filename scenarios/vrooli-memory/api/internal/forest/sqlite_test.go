...
func TestSummaryAndEdgesCommitAtomicallyAndRebuild(t *testing.T) {
	...
	_, err = r.CreateSummary(context.Background(), Summary{ID: "summary", Body: "summary", FacetID: "episode", Depth: 1}, []Edge{{ParentID: "wrong", ChildID: "entry", ChildKind: "entry"}})
	require.Error(t, err)
	frontier, err := r.Frontier(context.Background())
	require.NoError(t, err)
	require.Empty(t, frontier)
	_, err = r.CreateSummary(context.Background(), Summary{ID: "summary", Body: "summary", FacetID: "episode", Depth: 1}, []Edge{{ParentID: "summary", ChildID: "entry", ChildKind: "entry"}})
	require.NoError(t, err)
	frontier, err = r.Frontier(context.Background())
	require.NoError(t, err)
