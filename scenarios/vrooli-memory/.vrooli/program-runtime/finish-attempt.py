...
def validate_attempt(a):
    ...
    choice(a.get("provenance"), ("operator", "test"), "provenance")
    choice(a.get("recall_status"), ("matched", "no_match", "unavailable"), "recall_status")
    ...
    seen = set()
    for use in advice:
        if not isinstance(use, dict) or set(use) - {"entry_id", "decision", "decision_change", "verdict", "evidence_refs"}:
...
def step_validate():
    if not isinstance(inputs, dict) or set(inputs) - {"scope", "attempt", "observations"}:
    ...
    if len(observations) > 10:
        raise ValueError("at most 10 observations")
    seen = set()
    for o in observations:
        ...
        if o["attempt_id"] != a["attempt_id"]:
            raise ValueError("observation must refer to this attempt")
        choice(o.get("disposition"), ("supported", "contradicted", "insufficient", "unavailable", "unresolved", "unknown"), "disposition")
        choice(o.get("provenance"), ("operator", "test"), "observation provenance")
    ...
    envelope["signals"] = {"task_outcome": a["outcome"], "capture_status": "pending",
                           "entry_id": None, "existing": False, "observations": [], "retry_inputs": request}
    return "report"
...
def step_report():
    ...
    if signals.get("capture_status") == "pending":
        ...
        where = "record"
        try:
            result = receipt(vrooli_memory.learning.record(scope=request["scope"], attempt=request["attempt"]))
