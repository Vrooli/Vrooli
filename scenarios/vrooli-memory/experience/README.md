...
Use `experience-manager spec validate vrooli-memory --json` after edits.
...
Each independently meaningful async region should expose its own stable
experience surface and lifecycle state. Passive UI primitives inherit the
state of their parent region.
