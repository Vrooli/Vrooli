...
`vrooli scenario orient audio-tools`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient audio-tools
...
```bash
prd-control-tower prd generate audio-tools \
...
```bash
prd-control-tower prd validate audio-tools --json
```
...
```bash
prd-control-tower requirements generate audio-tools \
...
```bash
prd-control-tower requirements validate audio-tools --json
```
