...
| `experiment.overlap_max_window_ms` | integer | `10000` | per-run experiment override | agent/operator | Hermetic eval/experiment-only ceiling for overlap-agree's uncommitted tail before force-commit. Lower = bounds worst-case tail latency and repeated Whisper work earlier; too low can force-commit less-agreed text. It is snapshotted in the experiment recipe and does not mutate live `stt_stream_config`. |
...
| `stt.session_idle_timeout_ms` | integer | `30000` | `> 0` (`<= 0` → default) | operator | Ends a streaming session after this many ms of **inactivity** (no inbound audio); the deadline resets on every audio frame. Replaces the old hard 5-minute *absolute* WebSocket cap that fired on active speakers and dropped the tail. Every close path (idle-timeout, client `done`, cancel) drains and flushes the in-flight tail **before** the socket closes (drain-then-close), so no teardown loses audio. Lower = frees the backend sooner after the user stops; higher = tolerates longer silences mid-session. Applies to both transports and both strategies. |
| `stt.overlap_max_stall_rejects` | integer | `0` | `0–10` (`0`=disabled) | operator | OverlapAgree **stall-fallback**. On hard audio / jittery word timestamps, LocalAgreement can keep producing hypotheses that diverge from the committed prefix; the divergence-reject path then commits nothing and the uncommitted tail grows toward the 10s `max_window_ms` net. The quality-first default is `0`: the bounded max-window path retains audio and retries with backoff instead of best-guessing divergent text. Operators may set 1–10 for snappier but lower-confidence recovery. It never silently discards audio: failed or empty provider results retain the pending window and emit an error. Only meaningful for Local Whisper + `overlap`. |
