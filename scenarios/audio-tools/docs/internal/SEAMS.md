...
| `sttengine.Registry` | Concrete (engine-capability manifest) | `internal/sttengine/registry.go` (`//go:embed manifest.json`) | `stt.Selector` (Local-tier eligible strategies) + `stt.Segmenter` (egress stage set); the single source of truth for selectable engines + speaker-isolation axis. Also `EgressStages` (manifest-derived stage set), `RequiresPCM` (passthrough normalization), `ScanResourceConsumers` (cross-scenario engine-switch impact via an injected `fs.FS`) |
