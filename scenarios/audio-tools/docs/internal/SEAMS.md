...
| `egress.SpeakerIsolation` | Interface | `internal/stt/egress/speaker_stage.go` (production adapter: `handlers/stt/speaker_config.go::speakerVerification` wrapping `pipeline.EvaluateSpeaker`; fake: `internal/stt/egress/mocks/speaker.go::FakeSpeakerIsolation`) | `egress.SpeakerStage` (audio-domain identity check); the manifest's active `speakerIsolation` method selects the impl — swapping it is a one-field manifest edit. Lives in the handler layer (not pipeline) to avoid the egress→sttchain→pipeline import cycle. NOTE: this gate only DROPS mis-attributed text on the Whisper PCM path; isolating the audio is the complementary `ingress.TargetExtractor` seam above |
