...
| `chains/tiered.Coordinator[Req,Resp]` | Generic | `internal/ai/chains/tiered/tiered.go` | embedded into each of `sttchain.Chain`, `ttschain.Chain`, `summarizechain.Chain` (method promotion) — owns configurable routing (speech Local->BYOK->Vrooli; summarization BYOK->Vrooli->Local), availability cache, Reconfigure, Probe, Eligible |
...
| `chains/tiered.NewCredentialProviderChain` + `ExecuteBYOK` | Generic | `internal/ai/chains/tiered/set.go` | TTS and summarize retain concrete typed providers while delegating credential-gated tier construction and BYOK dispatch to the common chain substrate; domain packages inject their capability-specific adapter method and result decoration. |
| `chains/tiered.LPBSProvider` | Generic | `internal/ai/chains/tiered/lpbs.go` | STT, TTS, and summarize retain typed LPBS execution methods while sharing safe LPBS client configuration, availability, and model identity behavior. |
...
| `stt.StreamWSHandler` | Concrete | `handlers/stt/stream_ws.go` | mounts `/api/v1/voice/stream` over `voice.Service.HandleStreamWS`; its deterministic qualification faults live in `stream_ws_faults.go` and are double-gated by an active server-owned DB-and-file isolation lease plus `X-Vrooli-Test-Mode: 1`. The bounded fault values exercise provider admission, post-chunk and post-commit close, reader backpressure, and processed-coverage acknowledgement behavior without emitting audio or transcript data. A suppressed acknowledgement retains the v2 replay tail and emits `incomplete_coverage`; terminal `done` is therefore not a success signal without full coverage. |
...
| `ingress.Enhancer` | Interface | `internal/stt/ingress/{pipeline,denoise}.go` (fake: `internal/stt/ingress/mocks/enhancer.go::FakeEnhancer`) | `stt.Segmenter` builds the per-session `ingress.Pipeline` (single PRE-recognition seam the canonical-PCM stream passes through before the VAD + strategy; symmetric counterpart of `egress.Stage`). Production stage is `DenoiseEnhancer` (ffmpeg afftdn via `audioformat.Engine.NewStreamFilter`), gated on `denoise_enabled` config + ffmpeg availability for the Whisper PCM-buffering strategies |
| `ingress.TargetExtractor` | Interface | `internal/stt/ingress/extraction.go` (production adapter: `handlers/stt/speaker_config.go::speakerExtraction` wrapping `pipeline.SpeakerClient.Extract`; fake: `internal/stt/ingress/mocks/enhancer.go::FakeTargetExtractor`) | `ingress.ExtractionEnhancer` runs it: buffers canonical PCM into utterance windows and replaces each with the enrolled speaker's isolated voice (source separation + ECAPA target-selection in the native `sherpa-onnx` resource) BEFORE the VAD/recognizer. Gated on `extraction_enabled` config + a bound profile in `Segmenter.buildIngress`, like `DenoiseEnhancer` (no manifest flag — extraction is ingress, not an egress method). Adapter lives in the handler layer to avoid the ingress→sttchain→pipeline import cycle |
| `egress.SpeakerIsolation` | Interface | `internal/stt/egress/speaker_stage.go` (production adapter: `handlers/stt/speaker_config.go::speakerVerification` wrapping `pipeline.EvaluateSpeaker` + a per-session `pipeline.SessionSpeakerState`; fake: `internal/stt/egress/mocks/speaker.go::FakeSpeakerIsolation`) | `egress.SpeakerStage` (audio-domain identity check); the manifest's active `speakerIsolation` method selects the impl — swapping it is a one-field manifest edit. Lives in the handler layer (not pipeline) to avoid the egress→sttchain→pipeline import cycle. The adapter is STATEFUL per session (warm-up window + EMA-smoothed score via `SessionSpeakerState`); safe without locking because `segmenter.runEgress` drives it from a single goroutine. NOTE: this gate only DROPS mis-attributed text on the Whisper PCM path; isolating the audio is the complementary `ingress.TargetExtractor` seam above |
| `sttengine.Registry` | Concrete (engine-capability manifest) | `internal/sttengine/registry.go` (`//go:embed manifest.json`) | `stt.Selector` (Local-tier eligible strategies) + `stt.Segmenter` (egress stage set) + `Registry.Resolve` (capability-derived engine selection); the single source of truth for selectable engines + speaker-isolation axis. Also `EgressStages` (manifest-derived stage set), `RequiresPCM` (passthrough normalization), `ScanResourceConsumers` (cross-scenario engine-switch impact via an injected `fs.FS`) |
...
| `experiment.Runner` / `experiment.Manager` | Function seam + concrete lifecycle owner | `internal/experiment/runner.go` | `internal/bootstrap` wires the current eval harness as the runner body; `handlers/experiment` exposes lifecycle RPCs. Work runs under server lifetime context, not request context, so Wait cancellation does not abort the experiment. Default execution is one worker; queue visibility is surfaced through `StreamExperimentEvents` messages like `queued; N experiments ahead` rather than unbounded concurrency |
...
| `experiment/recipe.ApplyAugmentation` | Pure function seam with injected voice synthesizer | `internal/experiment/recipe/augmentation.go` | `internal/bootstrap` supplies the TTS-to-canonical helper for sherpa-onnx/Kokoro voices; tests inject or omit `SynthesizeVoice` to prove deterministic noise and resource-down skip recording |
...
| **Production wiring** | `main.go` builds the three concrete tiers (`NewLocalProvider`, `NewBYOKProvider`, `NewVrooliProvider`) and hands them to `sttchain.NewChain(Options{…})`. Per-request, speech `Chain.Execute` selects the first eligible tier in Local → BYOK → Vrooli order. |
...
- `tts.HandlerService` — TTS application-layer seam consumed by the
  Connect handler.
- `tts.Synthesizer` — local-TTS engine seam (sherpa-onnx/Kokoro HTTP synthesizer in
  production).
...
- **Why a seam**: keeps catastrophic-failure policy out of UI/CLI code and
  makes the gates unit-testable without Whisper, sherpa-onnx, or other model resources,
