...
```bash
export SQLITE_PATH=/tmp/audio-tools.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/audio-tools.db}")"
```
...
`packages/proto/gen/typescript/js/audio-tools/v1/`, and Python under
