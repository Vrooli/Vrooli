...
func TestEndpointDescriptorJSONShape(t *testing.T) {
	descriptor := modulekit.EndpointDescriptor{
		ID:          "notes_create",
		Path:        "/api/v1/notes",
		...
		Category:    "notes",
		Request: &modulekit.Schema{
			Type:       "object",
			Properties: map[string]string{"title": "string"},
		},
		Response: &modulekit.Schema{Type: "object"},
		Errors: []modulekit.ErrorDesc{{
		...
		CLIMapping: &modulekit.CLIMapping{
			Command: "audio-tools notes create",
