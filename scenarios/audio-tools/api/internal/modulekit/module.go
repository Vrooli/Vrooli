// Package modulekit defines the domain-module seam: each feature in
// the API returns a Module from its handlers package, and main.go
// passes the slice into server.New. The server iterates and calls
// Mount on each — there is no central routes.go, no per-domain field
// on server.Deps, and no manual editing of .vrooli/endpoints.json.
//
// The endpoint descriptor types are the fleet-wide canonical shapes
// defined once in github.com/vrooli/api-core/endpoints and re-exported
// here as type aliases, so every construction site
// (EndpointDescriptor{…}, RESTReasonOpsProbe, …) stays unchanged while
// the shape lives in exactly one place. Only the mux-coupled Module
// seam is local to the scenario.
package modulekit
...
//
//   - Name is for diagnostics ("stt", "health", "tts"). Free-form;
