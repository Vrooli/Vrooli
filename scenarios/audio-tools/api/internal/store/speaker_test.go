...
func TestSpeakerStore_PersistsEnrollmentMetadata(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, ok)
	require.Equal(t, 2, got.ClipCount)
	require.InDelta(t, 6.5, got.TotalVoicedSeconds, 1e-9)
	...
	require.Equal(t, 2, rows[0].ClipCount)
	require.InDelta(t, 6.5, rows[0].TotalVoicedSeconds, 1e-9)
	...
	require.NoError(t, err)
	require.Equal(t, 3, got.ClipCount)
	require.InDelta(t, 9.0, got.TotalVoicedSeconds, 1e-9)
}
