...
type SpeakerProfile struct {
	...
	// enroll time. The resource owns the real ECAPA embedding; audio-tools
	...
	// being down). Zero values mean "enrolled before this metadata was
	// captured" — re-enroll to populate.
...
func (s *SpeakerStore) Upsert(ctx context.Context, p SpeakerProfile) error {
	...
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO speaker_profiles(
			...
			enrollment_audio_seconds, sample_rate, embedding_dim, model_name
		)
		VALUES (?,?,?,?,?,?,?,?,?)
		ON CONFLICT(id) DO UPDATE SET
			...
			enrollment_audio_seconds=excluded.enrollment_audio_seconds,
			sample_rate=excluded.sample_rate,
			embedding_dim=excluded.embedding_dim,
			model_name=excluded.model_name
	`, p.ID, p.Name, p.Embedding, bound, p.CreatedAt.UTC().Format(time.RFC3339),
		p.EnrollmentAudioSeconds, p.SampleRate, p.EmbeddingDim, p.ModelName)
	return err
}
...
func (s *SpeakerStore) Get(ctx context.Context, id string) (SpeakerProfile, bool, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id, name, embedding, bound_user_identity, created_at, enrollment_audio_seconds, sample_rate, embedding_dim, model_name FROM speaker_profiles WHERE id=?`, id)
	var p SpeakerProfile
	var bound sql.NullString
	var created string
	err := row.Scan(&p.ID, &p.Name, &p.Embedding, &bound, &created, &p.EnrollmentAudioSeconds, &p.SampleRate, &p.EmbeddingDim, &p.ModelName)
...
func (s *SpeakerStore) List(ctx context.Context) ([]SpeakerProfile, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, embedding, bound_user_identity, created_at, enrollment_audio_seconds, sample_rate, embedding_dim, model_name FROM speaker_profiles ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var bound sql.NullString
		var created string
		if err := rows.Scan(&p.ID, &p.Name, &p.Embedding, &bound, &created, &p.EnrollmentAudioSeconds, &p.SampleRate, &p.EmbeddingDim, &p.ModelName); err != nil {
			return nil, err
		}
		if bound.Valid {
			p.BoundUserIdentity = bound.String
