...
// registry's AllEndpoints(). The generator body (transport validation +
// the API↔CLI mapping cross-check against cli/manifest.json, the single
// source of truth for the CLI surface) lives in
// github.com/vrooli/api-core/endpoints/gen, so this file is a thin
// wrapper. CI runs `make endpoints && git diff --exit-code
// .vrooli/endpoints.json`; the fix on drift is always: run
