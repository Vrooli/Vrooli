...
// normalisation step: the endpoint's "audio-tools notes list"
...
func TestStripBinaryPrefix(t *testing.T) {
	cases := []struct {
		...
		{in: "audio-tools status", want: "status"},
		{in: "audio-tools notes list", want: "notes list"},
		{in: "already-stripped", want: "already-stripped"},
		{in: "audio-tools", want: "audio-tools"}, // no trailing space → preserved
	}
