...
// ReadTextInput returns the raw bytes of a source argument following the bash
// CLI's rules: "-" reads stdin, a path to an existing file reads that file,
// and anything else is treated as the literal text value.
func ReadTextInput(input string) (string, error) {
	if input == "-" {
