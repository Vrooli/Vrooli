...
func calculateTextStatistics(text string) TextStatistics {
	...
	readingMinutes := float64(len(words)) / 200.0
	if readingMinutes == 0 && len(words) > 0 {
		readingMinutes = 0.01
	}
	return TextStatistics{WordCount: len(words), SentenceCount: sentences, ParagraphCount: paragraphs, ReadingTime: readingMinutes}
}
