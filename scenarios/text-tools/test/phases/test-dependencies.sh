...
if command -v node >/dev/null 2>&1; then
    ...
    testing::phase::add_warning "⚠️  Node.js runtime not found (optional for text-tools)"
fi
...
if command -v npm >/dev/null 2>&1; then
    ...
    testing::phase::add_warning "⚠️  npm not found (optional for text-tools)"
fi

echo "🔍 Checking essential utilities..."
essential_tools=(jq curl bats)
for tool in "${essential_tools[@]}"; do
    if command -v "$tool" >/dev/null 2>&1; then
        ...
        if [ "$tool" = "bats" ]; then
            testing::phase::add_warning "⚠️  Optional utility missing: $tool (CLI tests will be skipped)"
        else
...
if [ -f "$TESTING_PHASE_SCENARIO_DIR/cli/text-tools" ]; then
    if [ -x "$TESTING_PHASE_SCENARIO_DIR/cli/text-tools" ]; then
        log::success "✅ CLI binary is executable"
    else
        testing::phase::add_error "❌ CLI binary exists but is not executable"
    fi
else
    testing::phase::add_warning "⚠️  CLI binary not found (may need to be built)"
fi
