...
// TestCommandGroups exercises the flat-commands aggregator. DTV ships
// zero flat commands today; the test exists so a future domain that adds
// CommandGroups gets caller-side wiring for free (the call goes through
// unchanged).
...
// domain packages over time, and pinning "exactly N groups named X"
// breaks the moment a domain is added. The catch-the-typo failure mode
// (a domain registers but forgets to set Name or has no subcommands)
