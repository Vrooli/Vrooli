...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start development-toolchain-validator` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start development-toolchain-validator` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.development_toolchain_validator\.v1\./

startScenarioServer({
  uiPort,
  apiPort,
  distDir: './dist',
  serviceName: 'development-toolchain-validator',
  corsOrigins: '*',
  setupRoutes(app) {
