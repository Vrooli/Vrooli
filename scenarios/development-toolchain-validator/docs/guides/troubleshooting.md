...
```bash
export SQLITE_PATH=/tmp/development-toolchain-validator.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/development-toolchain-validator.db}")"
```
...
`packages/proto/gen/typescript/js/development-toolchain-validator/v1/`, and Python under
