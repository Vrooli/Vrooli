# Troubleshooting — Development Toolchain Validator
...
```bash
vrooli scenario status development-toolchain-validator
make stop
vrooli scenario start development-toolchain-validator --clean-stale
```
...
development-toolchain-validator status --auto-start

# Or override the API base for this invocation
development-toolchain-validator status --api-base "http://localhost:$(vrooli scenario port development-toolchain-validator API_PORT)/api/v1"
```
...
### `development-toolchain-validator configure` doesn't persist

Check which config-file path resolved (precedence in
[`../reference/configuration.md`](../reference/configuration.md#cli-config-file)).
Most commonly `~/.vrooli/config/development-toolchain-validator/config.json`. The
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/development-toolchain-validator/`.
...
The default route is `${SCENARIO_DATA_DIR}/development-toolchain-validator.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/development-toolchain-validator.db"
```
...
`packages/proto/gen/go/development-toolchain-validator/v1/`, TypeScript under
`packages/proto/gen/typescript/development-toolchain-validator/v1/`, and Python under
`packages/proto/gen/python/development_toolchain_validator/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/development-toolchain-validator/v1/<domain>/`.
