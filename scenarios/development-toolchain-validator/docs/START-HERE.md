# Start Here — Development Toolchain Validator
...
`template-manager orient development-toolchain-validator`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient development-toolchain-validator
...
```bash
cat > /tmp/prd_context_development-toolchain-validator.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) development-toolchain-validator \
  --context-file /tmp/prd_context_development-toolchain-validator.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate development-toolchain-validator --json
```
...
```bash
cat > /tmp/requirements_context_development-toolchain-validator.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) development-toolchain-validator \
  --context-file /tmp/requirements_context_development-toolchain-validator.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/development-toolchain-validator/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest development-toolchain-validator <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> development-toolchain-validator
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario development-toolchain-validator
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> development-toolchain-validator <adopted-path>
```
