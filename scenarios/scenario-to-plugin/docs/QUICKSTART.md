...
scenario-to-plugin notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port scenario-to-plugin API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.scenario_to_plugin.v1.notes.NotesService/ListNotes" \
