...
`template-manager orient scenario-to-plugin`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient scenario-to-plugin
...
```bash
cat > /tmp/prd_context_scenario-to-plugin.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start scenario-to-plugin --interactive
# …answer the interview, then:
business-health wizard preview scenario-to-plugin
business-health wizard apply scenario-to-plugin
```
...
```bash
vrooli scenario requirements validate scenario-to-plugin --json
```
...
```bash
cat > /tmp/requirements_context_scenario-to-plugin.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate scenario-to-plugin --json
business-health fix preview scenario-to-plugin
business-health fix apply scenario-to-plugin
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate scenario-to-plugin --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate scenario-to-plugin`, the notes page spec
...
validate scenario-to-plugin --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/scenario-to-plugin/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest scenario-to-plugin <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> scenario-to-plugin
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario scenario-to-plugin
```
...
- Wire contracts belong in
  `packages/proto/schemas/scenario-to-plugin/v1/<domain>/`.
