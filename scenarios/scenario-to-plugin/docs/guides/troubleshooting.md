...
```bash
export SQLITE_PATH=/tmp/scenario-to-plugin.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/scenario-to-plugin.db}")"
```
...
`packages/proto/gen/typescript/js/scenario-to-plugin/v1/`, and Python under
