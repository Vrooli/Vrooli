# Troubleshooting — Scenario to Plugin
...
```bash
vrooli scenario status scenario-to-plugin
make stop
vrooli scenario start scenario-to-plugin --clean-stale
```
...
scenario-to-plugin status --auto-start

# Or override the API base for this invocation
scenario-to-plugin status --api-base "http://localhost:$(vrooli scenario port scenario-to-plugin API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/scenario-to-plugin/`.
...
The default route is `${SCENARIO_DATA_DIR}/scenario-to-plugin.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/scenario-to-plugin.db"
```
...
`packages/proto/gen/go/scenario-to-plugin/v1/`, TypeScript under
`packages/proto/gen/typescript/scenario-to-plugin/v1/`, and Python under
`packages/proto/gen/python/scenario_to_plugin/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/scenario-to-plugin/v1/<domain>/`.
