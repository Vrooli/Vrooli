...
Threat model, mandatory controls, and the OWASP Agentic Skills Top 10 mapping for every skill published from this repo. This is the **what** and **why**; for the **how** (CI pipeline, registry submission), see [build-and-publishing.md](../guides/build-and-publishing.md). The disclosure process and release policy are included in this ramp's security documents.
...
- [SECURITY-POLICY.md](SECURITY-POLICY.md) — disclosure process and PR checklist
- [build-and-publishing.md](../guides/build-and-publishing.md) — the pipeline that enforces this baseline

## Disclosure and release policy

Skills published from this ramp run with the full permissions of the host agent;
a compromised skill therefore compromises every installing user. Report active
vulnerabilities privately to `security@vrooli.com` and do not open a public issue
until a patched release exists. The project security contact must acknowledge a
report within 72 hours.

Every release also requires reviewer accountability: one reviewer must be on the
security-aware reviewer list, install scripts must be read line by line, and all
reference material loaded by an agent must be reviewed for prompt injection.
Published versions are immutable. A replacement is a new version, and every
release retains a documented kill-switch procedure covering Cosign revocation,
registry withdrawal, and a public advisory when necessary.

The ramp's release evidence is the source of truth for these claims: a green
status names the passing check, while missing external scanner or signing tools
remains an explicit unavailable state.
