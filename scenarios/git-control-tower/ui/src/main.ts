...
type AppMonitorProxyEndpoint = string | {
  url?: unknown;
  path?: unknown;
  target?: unknown;
};
...
declare global {
  interface Window {
    __gitControlTowerBridgeInitialized?: boolean;
...
export function ensureBridge(): void {
  ...
  if ((window as Window & Record<string, unknown>)[BRIDGE_FLAG]) {
    return;
  }
  ...
  try {
    ...
    (window as Window & Record<string, unknown>)[BRIDGE_FLAG] = true;
  } catch (error) {
...
export function resolveProxyBase(): string | undefined {
  if (typeof window === 'undefined') {
    return undefined;
  }
...
export function resolveApiBase(basePath = '/api/v1'): string {
  if (typeof window === 'undefined') {
    return basePath;
  }
...
export function buildApiUrl(path?: string, basePath = '/api/v1'): string {
