...
| `RunsClient` | `internal/baseline/seams.go` | `baselineRunsClient` (test-genie `RunsService` Connect-RPC) | `fakeRuns` in `fakes_test.go` | Pin/unpin the shared run; one empty-phase `CompareRuns` returns every comparable phase's delta (GCT exposes the phase list directly, computes the overall verdict from it, and buckets mapped phases to surfaces locally for presentation); `CompareRunVisuals` returns the per-page pixel deltas the **advisory** visuals surface renders as its `changed` tier (test-genie owns the analyzer — GCT never re-derives pixel math); `ListRunVisuals` enumerates the run's visual artifacts. |
...
- A baseline is ONE comprehensive run, pinned once; surfaces are phase-set views (`internal/baseline/views.go`). Diff issues one empty-phase `CompareRuns`, exposes the full comparable `PhaseDiff[]`, computes the overall verdict from it, then buckets mapped phases into surfaces locally (option-c). The visuals surface diffs at the metadata level over the two runs' visual comparisons; GCT no longer captures or stores baseline screenshots (that subsystem was deleted from the baseline path in Phase 4).
