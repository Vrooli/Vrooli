...
| `RunsClient` | `internal/baseline/seams.go` | `baselineRunsClient` (test-genie `RunsService` Connect-RPC) | `fakeRuns` in `fakes_test.go` | Pin/unpin the shared run; one empty-phase `CompareRuns` returns every phase's delta (GCT buckets to surfaces locally — option-c); `CompareRunVisuals` returns the per-page pixel deltas the **advisory** visuals surface renders as its `changed` tier (test-genie owns the analyzer — GCT never re-derives pixel math); `ListRunVisuals` enumerates the run's visual artifacts. |
...
- **Snapshot durability (return-fast):** `SnapshotForBaseline` STARTS the comprehensive run and returns immediately with `{run_id, estimated_total_seconds, eta_known}` (via `Service.StartCapture`). The pin + manifest write happen on a server-owned goroutine (`Service.FinalizeCapture` under `context.WithoutCancel(ctx)` + `snapshotTailCeiling`), so a client that disconnects can never abandon a half-pinned, manifest-less baseline. The heavy run itself is durable in test-genie's `runmanager`; GCT keeps NO parallel job system. The CLI returns fast (bounded by `snapshotStartCeiling`, ~2m) — no SIGINT-detach plumbing, because there is no long block.

### Observability Surface (baseline snapshot)
- **States/transitions:** `SnapshotForBaseline` logs a "started comprehensive run" line (`scenario`, `name`, `run`, `eta`) up front and, when the server-owned finalize completes, a "pinned" line (`run`, `surfaces`, `skipped`) or a "finalize FAILED" line. The CLI prints an up-front banner — run id + ETA + the streaming `test-genie runs follow <scenario> <run-id>` command — and returns immediately, so the snapshot never blocks or reads as a silent hang.
...
- **Signal stability:** the re-attach verb is the streaming `test-genie runs follow`; the baseline becomes queryable via `baseline show`/`diff` once the run completes. The visuals surface verdict vocabulary gains the advisory `changed` tier (never gates; diff exit code unchanged).
