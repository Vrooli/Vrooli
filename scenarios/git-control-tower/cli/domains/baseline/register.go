...
func Register(core *cliapp.ScenarioApp) cliapp.SubcommandGroup {
	subcommands := []cliapp.Command{
		...
		{Name: "collection", NeedsAPI: true, Description: "Start, wait, extend, and diff durable multi-scenario baseline collections: `collection capture --name N --member scenario ...` prints its native wait; `collection show --name N --wait` reattaches; `collection extend` is append-only before edits; `collection diff --member S ...` then `diff status --wait` owns the final verdict", Run: func(a []string) error { return runCollection(core, a) }},
