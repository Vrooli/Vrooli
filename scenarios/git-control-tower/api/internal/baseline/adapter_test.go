...
func TestDiffIssuesClassification(t *testing.T) {
	baseline := []Issue{
		{Key: "k-preexisting", Title: "old", FilePath: "a.go"},
		{Key: "k-cleared", Title: "fixed", FilePath: "b.go"},
	}
	current := []Issue{
		{Key: "k-preexisting", Title: "old", FilePath: "a.go"},
		{Key: "k-new", Title: "new", FilePath: "c.go"},
	}
	d := diffIssues(SurfaceRules, baseline, current)
	if d.Verdict != VerdictRegression {
		t.Fatalf("expected regression (new violation), got %s", d.Verdict)
	}
	if len(d.Regressions) != 1 || len(d.Preexisting) != 1 || len(d.Cleared) != 1 {
		t.Fatalf("unexpected buckets: %+v", d)
	}
}

func TestDiffIssuesCleanAndPreexisting(t *testing.T) {
	// No new violations, one preexisting -> preexisting verdict.
	same := []Issue{{Key: "k1", Title: "x"}}
	d := diffIssues(SurfaceRules, same, same)
	if d.Verdict != VerdictPreexisting {
		t.Fatalf("expected preexisting, got %s", d.Verdict)
	}
	// All cleared -> clean.
	d2 := diffIssues(SurfaceRules, []Issue{{Key: "k1"}}, nil)
	if d2.Verdict != VerdictClean || len(d2.Cleared) != 1 {
		t.Fatalf("expected clean with cleared, got %+v", d2)
	}
}
...
func TestAuditorAdapterCaptureDiff(t *testing.T) {
	aud := &fakeAuditor{results: [][]Issue{
		{{Key: "k1", Title: "v1"}},                           // capture
		{{Key: "k1", Title: "v1"}, {Key: "k2", Title: "v2"}}, // diff: one new
	}}
	store := newTestSnapshotStore(t)
	a := NewRulesAdapter(aud, store)

	ptr, err := a.Capture(context.Background(), Target{RepoID: 1, Scenario: "foo"}, CaptureOptions{})
	if err != nil {
		t.Fatalf("Capture: %v", err)
	}
	if ptr.Kind != KindGCTLocalSnapshot || ptr.Ref == "" {
		t.Fatalf("unexpected pointer: %+v", ptr)
	}
	d, err := a.Diff(context.Background(), Target{RepoID: 1, Scenario: "foo"}, ptr)
	if err != nil {
	...
	if d.Verdict != VerdictRegression || len(d.Regressions) != 1 {
		t.Fatalf("expected one regression, got %+v", d)
	}
}
