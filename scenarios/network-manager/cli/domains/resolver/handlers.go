...
func (h handlers) status(ctx cliapp.RunContext) error {
	resp, err := cliapp.Call[*resolverv1.GetResolverStatusRequest, *resolverv1.GetResolverStatusResponse](h.core, http.MethodPost, resolverconnect.ResolverServiceGetResolverStatusProcedure, &resolverv1.GetResolverStatusRequest{})
	if err != nil {
		return cliapp.WrapAPIError("get resolver status", err, nil)
	}
	return renderStatus(ctx, resp)
}
...
func (h handlers) configureAdGuard(ctx cliapp.RunContext) error {
	...
	resp, err := cliapp.Call[*resolverv1.ConfigureAdGuardHomeRequest, *resolverv1.ConfigureAdGuardHomeResponse](h.core, http.MethodPost, resolverconnect.ResolverServiceConfigureAdGuardHomeProcedure, req)
	if err != nil {
		return cliapp.WrapAPIError("configure AdGuard Home", err, nil)
	}
	return cliapp.RenderProtoMutation(ctx, resp, cliapp.MutationReport{Result: []string{formatStatus(resp.GetStatus())}, Changes: resp.GetNextSteps()})
}
...
func (h handlers) upstreams(ctx cliapp.RunContext) error {
	...
	resp, err := cliapp.Call[*resolverv1.UpdateUpstreamsRequest, *resolverv1.UpdateUpstreamsResponse](h.core, http.MethodPost, resolverconnect.ResolverServiceUpdateUpstreamsProcedure, &resolverv1.UpdateUpstreamsRequest{Upstreams: upstreams, DryRun: ctx.BoolFlag("dry-run")})
	if err != nil {
		return cliapp.WrapAPIError("update upstreams", err, nil)
	}
	return cliapp.RenderProtoMutation(ctx, resp, cliapp.MutationReport{Result: []string{formatStatus(resp.GetStatus())}, Changes: resp.GetChanges()})
}

func (h handlers) health(ctx cliapp.RunContext) error {
	resp, err := cliapp.Call[*resolverv1.CheckResolverHealthRequest, *resolverv1.CheckResolverHealthResponse](h.core, http.MethodPost, resolverconnect.ResolverServiceCheckResolverHealthProcedure, &resolverv1.CheckResolverHealthRequest{})
	if err != nil {
		return cliapp.WrapAPIError("check resolver health", err, nil)
	}
	return cliapp.RenderProtoList(ctx, resp, cliapp.ListReport{Summary: []string{formatStatus(resp.GetStatus())}, ResultsHeading: "Checks", Results: resp.GetChecks()})
}
