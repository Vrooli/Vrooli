...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("network-manager")
	if err != nil {
		return "", fmt.Errorf("resolve network-manager storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve network-manager db path: %w", err)
	}
	return sqliteFileDSN(path)
}
