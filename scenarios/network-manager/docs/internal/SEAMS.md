...
| `SecretResolver` | Resolves AdGuard credentials from the canonical credential authority using the stored `credential_ref`; never invokes a resource CLI or reads plaintext request payloads. | Fake secret resolver returning credentials or missing-credential failures. | `NM-P0-002`, `NM-P0-008` |
