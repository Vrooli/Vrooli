...
`template-manager orient network-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient network-manager
...
```bash
cat > /tmp/prd_context_network-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) network-manager \
  --context-file /tmp/prd_context_network-manager.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate network-manager --json
```
...
```bash
cat > /tmp/requirements_context_network-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) network-manager \
  --context-file /tmp/requirements_context_network-manager.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/network-manager/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest network-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> network-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario network-manager
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> network-manager <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/network-manager/v1/<domain>/`.
...
1. Add proto messages and a service under
   `packages/proto/schemas/network-manager/v1/<domain>/`, then run
