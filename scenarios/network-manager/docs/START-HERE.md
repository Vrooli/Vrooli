...
`vrooli scenario orient network-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient network-manager
...
```bash
prd-control-tower prd generate network-manager \
...
```bash
prd-control-tower prd validate network-manager --json
```
...
```bash
prd-control-tower requirements generate network-manager \
...
```bash
prd-control-tower requirements validate network-manager --json
```
