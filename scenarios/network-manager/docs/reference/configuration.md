...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/network-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `network-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/network-manager/config.json`
3. `~/.vrooli/config/network-manager/config.json`
4. `~/.config/vrooli/network-manager/config.json`
...
```bash
network-manager configure api_base http://localhost:15001/api/v1
network-manager configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port network-manager API_PORT`)
