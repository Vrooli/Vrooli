...
```bash
vrooli scenario status network-manager
make stop
vrooli scenario start network-manager --clean-stale
```
...
```bash
make status
vrooli scenario port network-manager UI_PORT
```
...
network-manager status --auto-start

# Or override the API base for this invocation
network-manager status --api-base "http://localhost:$(vrooli scenario port network-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/network-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/network-manager.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/network-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/network-manager.db}")"
```
...
`packages/proto/gen/go/network-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/js/network-manager/v1/`, and Python under
`packages/proto/gen/python/network_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/network-manager/v1/<domain>/`.
