...
`~/.vrooli/bin/personal-planner`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start personal-planner` if the API is unreachable |
...
```bash
personal-planner status
personal-planner status --json
```

### `personal-planner configure <key> <value>`
...
(`personal-planner <domain> <verb>`). Every command calls a single API
...
UI, and any registered tool share one application-command layer, so
authorization, idempotency, validation, and audit behavior are identical
across surfaces — a tool does not gain unrestricted database access merely
because it runs locally. Read versus write capability follows plan §21.2:
reading planning context and computing a labeled draft (forecast /
proposal / what-if) are distinct from capturing owned facts, and *both*
are distinct from **applying** a schedule change — apply is always an
explicit user-authorized command, never invoked automatically by a
forecast job. Every command emits machine-readable JSON (`--json`) and
concise human output; bulk input arrives through files/stdin, never by
parsing shell-like natural language.

The scaffold ships one fully worked CRUD command group as a copyable
reference (the Notes section below); `template-manager detemplate
personal-planner` removes it once your real domains are green.

---

## Scenario commands — `notes` (CRUD reference)

`notes` is the scaffold's canonical worked CRUD command group, including
the one multipart-upload REST exception. It is **not** product scope — it
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start personal-planner`" is good.
