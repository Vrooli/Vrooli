# Configuration — Personal Planner
...
`API_PORT` is allocated from the `15000-19999` band and `UI_PORT` from
`20000-24999`; the lifecycle sets both. There is no scenario-specific
runtime environment variable beyond the template set — provider
credentials and secrets are **never** read from the environment (see
"Provider and credential configuration" below).

### Provider and credential configuration (planned)

> **Status: planned.** These placeholders describe where the optional
> read-only external-calendar feature will read its configuration once
> the `integrations` domain lands. They are not consumed by the scaffold.

- **Provider tokens are never environment variables and never appear in
  logs, bundles, exports, or docs.** They live only in the shared
  credential-storage owner and are referenced by an opaque credential ID
  (see [`../concepts/INTEGRATIONS.md`](../concepts/INTEGRATIONS.md)).
- External calendars are strictly **read-only** in R1; there is no
  provider-write configuration to set.
- Provider refresh cadence (initial target ~5 minutes when active, slower
  when idle, respecting quotas/backoff) is adapter/deployment
  configuration, not a scenario env var (plan §28.1).
- Redact query parameters wherever a token could appear; a
  `PROVIDER_REAUTH_REQUIRED` state preserves last-known busy facts rather
  than logging a credential.

### Scenario-prefixed CLI variables

`cli-core` derives a standard set of env vars from the scenario name.
For `personal-planner` the prefix is the scenario id upper-cased with
...
Personal Planner depends on two other scenarios (declared under
`dependencies`, reconciled against
[`../concepts/INTEGRATIONS.md`](../concepts/INTEGRATIONS.md)); dependency
edits flow through the Scenario Dependency Analyzer, never hand-edited:

| Dependency | Required | Startup policy | Behavior if absent |
|---|---|---|---|
| scenario-authenticator | Yes | `must_start` | The scenario refuses to start — a private workspace cannot be scoped to an identity, and sharing needs its recipient-bound viewer identity. |
| notification-hub | No | `try_start` | Degrades to in-app notices only; the product stays fully usable. |

No mandatory Vrooli resource (Postgres/Redis/Qdrant/Ollama) is declared
for R1. SQLite is in-process; a relational resource is adopted only if
measured scale or the partial-unique one-exclusive-session guard justifies
it (see [`../concepts/DATA.md`](../concepts/DATA.md)).

### Product defaults (planned)

> **Status: planned.** These are the adjustable product/engineering
> defaults the plan fixes so the build does not re-ask each routine choice
> (plan §28.1). They are stored per-workspace once the `workspace` and
> `planning` domains land, not host environment variables, and none is a
> claim about a user's actual capacity or preferences.

**Appearance (day/night Observatory):**

| Setting | Default |
|---|---|
| Appearance mode | Auto (user may pin Day or Night) |
| Auto Day transition | 07:00 (editable, no location required) |
| Auto Night transition | 19:00 (editable) |
| Transition during focus | Postponed until the session ends |
| Ambient motion | Subtle; disabled under a reduced-motion preference |

**Planning:**

| Setting | Default |
|---|---|
| Interactive horizon | 28 days maximum |
| Forecast horizon | 90 days (configurable, bounded) |
| Reserve | 20% of potential capacity offered at onboarding (fixed minutes or explicit intervals allowed; applied only after the user accepts the profile) |
| Session split | 25-minute minimum, up to 90-minute preferred maximum; short final remainder allowed |
| Scheduling grid | 5-minute UI snapping; exact instants stored |
| Pomodoro | 25 minutes work / 5 minutes break (editable; next interval awaits explicit resume) |
| Scheduling action | Generate a preview, apply explicitly — accepted work is never moved automatically |
| Proposal expiry | 24 hours maximum |
| Forecast labels | Central / cautious scenarios (no statistical probability in R1) |
| Learning window | Most recent 90 days |
...
2. `${XDG_CONFIG_HOME}/vrooli/personal-planner/config.json`
3. `~/.vrooli/config/personal-planner/config.json`
4. `~/.config/vrooli/personal-planner/config.json`
...
```bash
personal-planner configure api_base http://localhost:15001/api/v1
personal-planner configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port personal-planner API_PORT`)
