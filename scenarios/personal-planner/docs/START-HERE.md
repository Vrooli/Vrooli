...
`template-manager orient personal-planner`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient personal-planner
...
```bash
cat > /tmp/prd_context_personal-planner.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start personal-planner --interactive
# …answer the interview, then:
business-health wizard preview personal-planner
business-health wizard apply personal-planner
```
...
```bash
vrooli scenario requirements validate personal-planner --json
```
...
```bash
cat > /tmp/requirements_context_personal-planner.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate personal-planner --json
business-health fix preview personal-planner
business-health fix apply personal-planner
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate personal-planner --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate personal-planner`, the notes page spec
...
validate personal-planner --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/personal-planner/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest personal-planner <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> personal-planner
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario personal-planner
```
...
- Wire contracts belong in
  `packages/proto/schemas/personal-planner/v1/<domain>/`.
