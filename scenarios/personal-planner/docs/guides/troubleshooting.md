# Troubleshooting — Personal Planner
...
```bash
vrooli scenario status personal-planner
make stop
vrooli scenario start personal-planner --clean-stale
```
...
### API starts but every request is unauthorized / no workspace

Personal Planner binds each private workspace to an authenticated subject
supplied by the **scenario-authenticator** dependency (required). If that
dependency is not running, the API can boot and answer `/health` but real
product calls have no subject to scope to.

```bash
vrooli scenario status scenario-authenticator
make restart   # the lifecycle brings the required dependency up with the scenario
```

`notification-hub` is optional — only the `notifications` domain needs it,
and its absence must degrade gracefully rather than block planning.
...
personal-planner status --auto-start

# Or override the API base for this invocation
personal-planner status --api-base "http://localhost:$(vrooli scenario port personal-planner API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/personal-planner/`.
...
[Shared harness recipes](/scenarios/template-manager/docs/internal/TESTING-RECIPES.md#coverage-thresholds).

## Storage

### The resolved database directory is not writable

The default route is `${SCENARIO_DATA_DIR}/personal-planner.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/personal-planner.db"
```
...
`packages/proto/gen/go/personal-planner/v1/`, TypeScript under
`packages/proto/gen/typescript/personal-planner/v1/`, and Python under
`packages/proto/gen/python/personal_planner/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/personal-planner/v1/<domain>/`.
