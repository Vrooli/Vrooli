...
The `health` proto in `packages/proto/schemas/personal-planner/v1/shared/`
...
### Planned product seams (DESIGN-STAGE)

**Status: none of the seams in this subsection exist in code yet.** Only
the `health` infrastructure domain and the fenced `notes` example (below)
are real. The entries here are the *planned* boundaries the adaptive
planner will introduce as its domains are built, derived from the
implementation plan §22 (modules/seams) and §23 (reliability). They are
registered now so the seam contract is designed before code, and so the
`notes` example (repository / service / attachment / schema seams) reads
as the worked reference for the abstract shapes below. Paths are the
intended structure, not files that exist; interface names are indicative
and should be treated as the design target, not a compiled symbol.

The scheduling/capacity/forecast core is a **pure deterministic kernel**
with no side effects. The seams below are exactly the injected boundaries
that keep it pure: every date-sensitive computation takes an explicit
clock, every external fact enters through an adapter interface, and every
persistence/delivery/credential effect is behind a repository- or
client-shaped interface. If any kernel code reaches for `time.Now()`, a
live provider, `*sql.DB`, or a secret store directly, that is an
undeclared seam and a bug against the plan's determinism invariants
(§25.3).

#### Clock / TimezoneService (injectable time — the determinism seam)

| | |
|---|---|
| **Seam** | Wall-clock time and timezone resolution for all date-sensitive logic |
| **Interface** | Planned `internal/temporal::Clock` (`Now() time.Time`) plus a `TimezoneService` resolving IANA zones, DST transitions, and `LocalTime`/`CivilDate`/`Instant` normalization. Extends the template's `internal/clock::Clock` into the temporal helper package. |
| **Production wiring** | `main.go` constructs the system clock + a real IANA timezone database resolver and injects them through `server.Deps` into every domain module that schedules, forecasts, or computes capacity. |
| **Test fake** | Extends `internal/testutil/mocks::FakeClock` (`Now`, `Advance`, `SetNow`) with a fixed-offset timezone fake so recurrence, DST, "remaining-today", and expiry are bit-for-bit deterministic. |
| **Why it exists** | The kernel is only deterministic if time is an input. Proposals, forecasts, capacity clips at `now`, routine occurrence expansion, and proposal/idempotency expiry all depend on the clock; a direct `time.Now()` makes property tests (deterministic proposals, recurrence identity) impossible to assert and reintroduces DST bugs the temporal model exists to prevent. See DATA.md §"Temporal and numeric typing". |

#### Source-app adapter (inbound scheduling intent)

| | |
|---|---|
| **Seam** | The contract by which a trusted source scenario (Daily, Cadence, agent work) submits normalized scheduling intent and reads accepted schedule |
| **Interface** | Planned `internal/integrations::SourceAdapter` — accepts a normalized `SchedulingIntent` (`human_work` / `fixed_event` / `marker` / `passive_wait`, with effort ranges, timing windows, attention, split policy), validates it, and returns a revisioned `SourceObjectProjection`. Wire shape lives in `packages/proto/schemas/personal-planner/v1/integrations/`. |
| **Production wiring** | `handlers/integrations/module.go` resolves workspace/actor from authenticated identity (never the payload) and persists the projection through the integrations repository. Each registered source is trusted, capability-scoped, and credential-referenced. |
| **Test fake** | A **fixture adapter** is the canonical substitute: where a real consumer scenario is absent or lacks an API, the exact contract ships with a fixture adapter and the live-integration gate stays explicitly open (plan §15.8, §21.4). Recorded/synthetic intents drive adapter contract tests. |
| **Why it exists** | Personal Planner is the single scheduling authority; sources own their content and completion state while the planner owns availability and reservations. The adapter interface keeps that ownership split enforceable and lets the planner be built and tested before any source scenario has a live API. See INTEGRATIONS.md §"Daily / Cadence / agent work". |

#### External-calendar provider adapter (read-only import)

| | |
|---|---|
| **Seam** | Provider-neutral, strictly read-only import of external calendar busy facts |
| **Interface** | Planned `internal/integrations::CalendarProviderAdapter` — list calendars, initial + incremental reads, identity/time normalization, deletion reporting, credential refresh through the credential seam, and health. Google Calendar is the recommended first implementation; Microsoft Graph and Apple/CalDAV are later adapters. |
| **Production wiring** | `handlers/integrations/module.go` constructs the concrete adapter (requested OAuth scopes read-only) and drives incremental sync jobs with one durable cursor per connection/calendar/query scope, advanced only after a batch is durably processed. |
| **Test fake** | Recorded/synthetic provider responses (initial read, incremental change batch incl. deletions, invalidated-cursor forced resync, moved recurring instance keeping its stable original-instance key). No real account is touched. |
| **Why it exists** | Read-only enforcement lives at both requested scope and adapter implementation — there is no provider mutation path behind the UI in R1. The adapter isolates each provider's quirks (sync tokens, delta model) and lets failure handling (retain stale busy facts with a freshness warning; never treat provider failure as free time) be tested against canned responses. See INTEGRATIONS.md §"Read-only external calendars". |

#### notification-hub delivery seam

| | |
|---|---|
| **Seam** | Outbound notification delivery to the optional notification-hub scenario |
| **Interface** | Planned `internal/notifications::Delivery` — accepts a `NotificationIntent` (recipient scope, type, dedup key, relevance/expiry, deep link, minimum safe content) and returns a delivery receipt. |
| **Production wiring** | `handlers/notifications/module.go` wires the notification-hub client when the optional dependency is present; when absent, delivery degrades to in-app only and the product stays fully usable. |
| **Test fake** | An in-memory delivery fake records intents and asserts dedup/idempotency (a retried intent never yields a second human-facing message) and that a private reason never appears in a shared-viewer message. |
| **Why it exists** | Notification-hub is optional; the seam lets the notifications domain be built and tested with the dependency absent, and keeps delivery + receipts owned by the hub rather than duplicated here. See INTEGRATIONS.md §"notification-hub". |

#### Credential-store seam

| | |
|---|---|
| **Seam** | Read/refresh of external provider tokens held by the platform credential owner |
| **Interface** | Planned `internal/integrations::CredentialStore` — resolve a credential reference to a usable token, refresh it, and surface reauth-required state, all by opaque reference. |
| **Production wiring** | `handlers/integrations/module.go` wires the platform credential owner; provider adapters receive only a credential *reference*, never a raw secret. |
| **Test fake** | An in-memory store returning canned tokens, an expired/revoked token (drives `PROVIDER_REAUTH_REQUIRED`), and a refresh path. |
| **Why it exists** | Secrets live in the credential owner and never appear in records, logs, analytics, bundles, or exports (DATA.md §"Privacy Notes"). Routing every token access through this seam keeps that rule enforceable and lets reauth flows be tested without a live provider. |

#### Command service (actor / workspace / idempotency / revision / outbox)

| | |
|---|---|
| **Seam** | The shared command envelope every mutating domain command passes through |
| **Interface** | Planned `internal/command::Service` (a shared envelope, listed as a non-domain in DOMAINS.md) — resolves actor + workspace from authenticated identity, enforces the idempotency key, checks the expected entity/plan revision, and stages the outbox record inside the same transaction. |
| **Production wiring** | Each domain module composes its repository behind the command service so mutations are uniformly scoped (INV-01), idempotent (INV-10), revision-checked, and outbox-staged atomically (INV-11). |
| **Test fake** | A fake envelope injecting a fixed actor/workspace, a controllable idempotency ledger, and a captured outbox, so a domain service test can assert conflict (`REVISION_CONFLICT`) and replay (returns prior outcome) without full transport. |
| **Why it exists** | Actor/workspace/idempotency/revision/outbox are cross-cutting reliability concerns; centralizing them keeps each domain's rules honest and identical, and makes the transaction/atomicity invariants testable in one place. See ERROR-HANDLING.md and the reliability discussion in the plan §23. |

#### Per-domain repository interfaces (engine-hidden)

| | |
|---|---|
| **Seam** | One persistence interface per product domain, hiding the SQLite engine |
| **Interface** | Planned `internal/<domain>::Repository` for each of `workspace`, `work`, `calendar`, `goals`, `commitments`, `planning`, `forecasts`, `focus`, `review`, `integrations`, `sharing`, `notifications`. Methods are exactly what the domain service needs; the concrete `sqliteRepository` translates to SQL. `capacity` owns no tables (derived) and therefore no repository. |
| **Production wiring** | `handlers/<domain>/module.go` constructs `NewSQLiteRepository(db, clk)` and passes it into `NewService(...)`; domain-specific dependencies never appear on `server.Deps`. Each domain contributes its own `schema.sql` via the per-domain `<domain>.Schema` seam. |
| **Test fake** | Co-located `internal/<domain>/mocks::FakeRepository` (in-memory CRUD + per-method error knobs + atomic counters) for service tests; the real handle is substituted in `internal/<domain>/sqlite_test.go` to pin SQL semantics, unique constraints (notably the one-exclusive-session guard in `focus`), and tenant isolation. |
| **Why it exists** | Repository ownership per domain keeps persistence engine-swappable and each domain deletable in one sweep. The `notes` seams below are the worked reference for this exact shape. The `focus` repository additionally owns the database-enforced one-running-exclusive-session guard (a partial unique index under Postgres, an equivalent transactional guard under SQLite) — see DATA.md §"the one-session guard". |
