...
export function HealthCard() {
  ...
  const { data, error, isFetching, isLoading, refetch } = useQuery({ queryKey: ["health"], queryFn: fetchHealth });
  const handleRefresh = () => {
  ...
  return (
    <LibraryHealthCard
      ...
      description={t(strings.health.description)}
      ...
      footer={<p data-testid={selectors.notifications.summary}>{t(strings.notifications.summary, { count: refreshCount })}</p>}
