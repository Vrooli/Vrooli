...
import { CalendarDays, CalendarRange, ChartNoAxesCombined, Clock3, Gauge, Handshake, ListTodo, Repeat2, TimerReset } from "lucide-react";
...
const mobilePlanViews = [
  { view: "agenda", label: "Agenda", Icon: ListTodo },
  { view: "month", label: "Month", Icon: CalendarRange },
  { view: "timeline", label: "Timeline", Icon: Clock3 },
  { view: "capacity", label: "Capacity", Icon: Gauge },
  { view: "commitments", label: "Commitments", Icon: Handshake },
  { view: "forecast", label: "Outlook", Icon: ChartNoAxesCombined },
] as const;
...
export function PlanPage() {
  ...
  const estimateEditor = (mobile = false) => <form className={`plan-estimate-editor${mobile ? " plan-estimate-sheet" : ""}`} onSubmit={(event) => { event.preventDefault(); estimateMutation.mutate(); }}>
    ...
    <Button type="submit" className="primary-action" pending={estimateMutation.isPending} pendingLabel="Remembering…" disabled={estimateMutation.isPending || estimateMinutes === ""}>Save estimate</Button>
  </form>;
  ...
  return <ObservatoryScene kind="plan"><section className="planner-surface plan-surface" data-testid={selectors.pages.plan} aria-labelledby="plan-heading">
    ...
    {plan.data && <>
      ...
      {isMobile && <Dialog open={mobileMoreOpen} title="More planning views" description="Open one supporting view without crowding the day plan." onClose={() => setMobileMoreOpen(false)} closeLabel="Close planning views" contentClassName="plan-more-dialog"><div className="plan-more-grid">{mobilePlanViews.map(({ view: nextView, label, Icon }) => <Button key={nextView} type="button" variant={view === nextView ? "primary" : "secondary"} icon={<Icon size={18} aria-hidden="true" />} onClick={() => selectMobileView(nextView)}>{label}</Button>)}<Button type="button" variant="secondary" icon={<Repeat2 size={18} aria-hidden="true" />} onClick={() => { setMobileMoreOpen(false); setMobileRoutinesOpen(true); }}>Routines</Button><Button type="button" variant="secondary" icon={<TimerReset size={18} aria-hidden="true" />} onClick={() => { setMobileMoreOpen(false); setMobileEstimateOpen(true); }}>Update estimate</Button></div></Dialog>}
