...
An adaptive personal planning and focus application that helps people make
realistic commitments, follow through, and plan better from what actually
happens — inside a calm, editorial **Observatory** interface (coordinated
day/night landscape) that is worth opening every morning. The two
differentiators are **beauty** and **honest accounting**: the numbers
underneath never lie green.
...
- A five-destination product surface — **Today, Plan, Goals, Focus,
  Review** (plus **Settings**) — carried by the `react-component-library`
  **navigated-console** shell and styled by the **Observatory** theme
  (see `DESIGN.md`). Today is the primary surface.
- Two declared scenario dependencies: **scenario-authenticator**
  (required — supplies the authenticated subject each private workspace
  is bound to) and **notification-hub** (optional — delivery for the
  `notifications` domain). SQLite is the default store (in-process, no
  resource container).
...
`DESIGN.md` at this scenario's root is binding. When you build UI, honor
it: the token plumbing, the Auto/Day/Night appearance model with
coordinated scenes, **truthful time geometry** (block width ∝ duration —
never stretch a 45-minute block to 90 to fit a label), honest state copy
(available time vs reserve, promise vs forecast, planned vs actual,
source status vs planner state, empty view vs failed query), and WCAG 2.2
AA as a release gate — including contrast over the brightest and darkest
landscape regions. The landscape scene is decorative and sits below the
work; do not bake task text, the date, buttons, or the timeline into the
image, and do not substitute a stock operational dashboard and call it
equivalent. See [`docs/concepts/EXPERIENCE.md`](docs/concepts/EXPERIENCE.md)
...
- The bare-minimum settings surface once your scenario needs more than
  theme and locale (the product needs a full Settings surface —
  availability, appearance, focus, integrations, sharing, notifications,
  data, and learning controls).

### Durable seams to preserve
...
| Map the shell/slot taxonomy and components | [`docs/concepts/UI-ARCHITECTURE.md`](docs/concepts/UI-ARCHITECTURE.md) |
...
For the product itself, look at the calm/editorial tier of personal
planners — Sunsama, Amie, Structured, Fantastical — for the bar on beauty
and daily-open appeal, and beat them on honest accounting (see
[`docs/concepts/EXPERIENCE.md`](docs/concepts/EXPERIENCE.md) and
[`docs/business/MONETIZATION.md`](docs/business/MONETIZATION.md)). Read
`DESIGN.md` and `docs/guides/choosing-ui.md` before opening `ui/`. For the
template mechanics, open `scenarios/browser-automation-studio/` to see the
same template shape taken to completion on the API and CLI side; its UI
predates the library shell.
