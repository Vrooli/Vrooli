# Troubleshooting — Business Health
...
```bash
vrooli scenario status business-health
make stop
vrooli scenario start business-health --clean-stale
```
...
```bash
make status
vrooli scenario port business-health UI_PORT
```
...
business-health status --auto-start

# Or override the API base for this invocation
business-health status --api-base "http://localhost:$(vrooli scenario port business-health API_PORT)/api/v1"
```
...
`vite build` processes the full module graph for the production bundle. This
...
together — never a subset of them.
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/business-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/business-health.db` via
...
SQLite single-writer behaviour. If more than one process (e.g., a stale API
plus a new one) holds the file, find and kill the older one:

```bash
fuser "${SCENARIO_DATA_DIR}/business-health.db"
```
...
The generated package paths follow `packages/proto/gen/go/business-health/v1/<domain>/`.
