...
`template-manager orient business-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient business-health
...
and the fenced example domain — mixes binding contract with illustrative example. Tokens, motion,
...
```bash
cat > /tmp/prd_context_business-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) business-health \
  --context-file /tmp/prd_context_business-health.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate business-health --json
```
...
```bash
cat > /tmp/requirements_context_business-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) business-health \
  --context-file /tmp/requirements_context_business-health.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/business-health/v1/<domain>/<domain>.proto`
      ...
      enumerated in `api/internal/module/module.go`.
...
```bash
react-component-library components ingest business-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> business-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario business-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> business-health <adopted-path>
```
