...
The `health` proto in `packages/proto/schemas/business-health/v1/shared/`
...
| **Why it exists** | Middleware computes request-duration log lines from a pair of `Now()` calls. With `time.Now()` direct, duration assertions are flaky on loaded CI and undefined on fast hardware. With `FakeClock.Advance(150 * time.Millisecond)` inside the inner handler, the duration string is bit-for-bit deterministic. See `internal/middleware/logging_test.go::TestLoggingMiddleware_LogsDuration`. |
...
### extraction.Extractor (the intent-go parser doorway)

| | |
|---|---|
| **Seam** | The only entry point to a target scenario's PRD.md / requirements/ artifacts |
| **Interface** | `internal/extraction/extraction.go::Extractor` (`Load(scenario, scenarioDir) (Contract, error)`) |
| **Production wiring** | `handlers/validation/module.go` wires `extraction.NewFileExtractor()` (composes `intent.FilePRDExtractor` + `intent.FileRequirementsExtractor` — packages/intent-go, the repo-wide single-parser ratchet). |
| **Test fake** | Any func/struct returning a fixture `Contract`; checks never read files directly. |
| **Why it exists** | The ratchet forbids a second PRD/requirements parser anywhere; giving checks a substitutable Contract source keeps every check test hermetic AND keeps parsing in exactly one package. |

### checks.Check (the contract-check registry)

| | |
|---|---|
| **Seam** | One business-contract check (template shape, registry structure, intent linkage, evidence drift) |
| **Interface** | `internal/checks/checks.go::Check` (`Name() string; Run(ctx, extraction.Contract) []intent.Finding`) |
| **Production wiring** | `internal/checks/registry.go::Registry()` returns the run-ordered set; `handlers/validation/module.go` passes it to `checks.New`. Domain checks land in plan phase 3 mapped 1:1 onto `.vrooli/maturity.json` codes. |
| **Test fake** | Inline `Check` structs emitting canned findings. |
| **Why it exists** | The engine (target resolution, metrics stages, severity cap) is invariant; the check set grows per phase. Severity policy: the engine caps at ERROR — the business dimension is advisory and never emits BLOCKER. |

### checks.WithMetrics (ExecutionMetrics ctx seam)

| | |
|---|---|
| **Seam** | Threading the api-core metrics collector through validation without signature changes |
| **Interface** | `internal/checks/metrics.go::WithMetrics(ctx, *metrics.Collector)` + private `metricsFrom` |
| **Production wiring** | `handlers/validation/connect_handler.go` starts a collector per request (`metrics.Start(WithEnvironment(...))`), stops it on every exit path, and attaches the result to the shared response. |
| **Test fake** | None needed — a nil collector is a safe no-op (proto-health's recipe). |
| **Why it exists** | `metrics_adopted` is a hard provider-contract requirement; the ctx seam keeps the engine's API clean. |

### assessment.Builder (findings → shared maturity envelope)

| | |
|---|---|
| **Seam** | Mapping neutral `intent.Finding`s into `common.v1.MaturityAssessment` |
| **Interface** | `internal/assessment/assessment.go::Builder` (constructed from the validated `.vrooli/maturity.json` spec) |
| **Production wiring** | `handlers/validation/module.go` loads the spec via `maturity.LoadSpecFromScenario` and fails fast if invalid. |
| **Test fake** | Build a Builder from the real spec (cheap, deterministic) — see `internal/assessment/spec_test.go`. |
| **Why it exists** | The finding vocabulary is frozen in the spec; the builder is the sole place a code acquires capability/ladder metadata, so checks stay ignorant of maturity mechanics. |

### autofix registry (shared Fix RPC)

| | |
|---|---|
| **Seam** | Deterministic remediation candidates behind PreviewFix/ApplyFix |
| **Interface** | `github.com/vrooli/maturity-go/autofix::Registry` via `internal/autofix::NewRegistry()` |
| **Production wiring** | Empty in phase 2 (the Fix RPCs answer honestly with no candidates); phase 6 registers fixers and flips `fixer_status: pending → implemented` in maturity.json. |
| **Test fake** | Fixture registries with canned candidates. |
| **Why it exists** | Dry-run-by-default is structural: Preview and Apply are distinct RPCs (and distinct CLI commands), and the registry's Candidate shape carries before/after diffs. |

### Evidence discipline (single-writer, phase 4 forward)

Not an interface seam but a boundary rule: test-genie is the ONLY writer
of run evidence (`coverage/requirements-sync/`). business-health reads
those artifacts and owns exactly one ledger of its own,
`coverage/manual-validations/log.jsonl` (appended via
`LogManualValidation` / `manual-log add`). The phase-4 evidence package
carries a test asserting it makes no write calls to sync paths.

### aisearch.IntentSource (the fleet intent corpus)

| | |
|---|---|
| **Seam** | The corpus loader behind the `business-health.intent` search leaf |
| **Interface** | `internal/aisearch/intent_index.go::IntentSource` (`LoadRecords(ctx) ([]IntentRecord, error)`) |
| **Production wiring** | `main.go` wires `NewFleetIntentSource(repoRoot)` (walks scenarios/*, composes `internal/extraction` — the intent-go ratchet holds). |
| **Test fake** | Fixture record sets; the recall gate uses the real source against a throwaway collection. |
| **Why it exists** | The engine (packages/ai-go/search) is generic; the corpus definition (one doc per PRD purpose / OT / requirement, D5 type facet) is this scenario's contract. Tuning lives in `.vrooli/search.json` (SSOT), never code literals. |

### wizard.Hinter ← aisearch (capability dedup)

| | |
|---|---|
| **Seam** | "Similar capability exists in scenario X" pointers before scaffolding |
| **Interface** | `internal/wizard/session.go::Hinter`; production impl `internal/aisearch/hinter.go::WizardHinter` |
| **Production wiring** | `main.go` hands `NewWizardHinter(aiService)` to the wizard module. |
| **Test fake** | `fakeHinter` in the wizard tests; `NoHints{}` is the default. |
| **Why it exists** | The interview must never block on search — the Hinter contract requires silent degradation (return nil on any backend failure), enforced by the in-process 3s timeout. |
