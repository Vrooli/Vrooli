...
business-health <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port business-health API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.business_health.v1.<domain>.<Service>/<Method>
```
...
- Read [`concepts/ARCHITECTURE.md`](concepts/ARCHITECTURE.md) for the
  mental model: the coordinated surfaces, proto bridge, layered API,
  where to add code.
