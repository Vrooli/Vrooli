...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/business-health.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `business-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/business-health/config.json`
3. `~/.vrooli/config/business-health/config.json`
4. `~/.config/vrooli/business-health/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port business-health API_PORT`)
