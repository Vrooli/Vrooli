import { ClipboardList, Grid2X2, LayoutDashboard, Network, Settings, WandSparkles } from "lucide-react";
...
const NAV_ICONS = {
  ...
  matrix: <Grid2X2 aria-hidden className="h-5 w-5" />,
  fleet: <Network aria-hidden className="h-5 w-5" />,
  wizard: <WandSparkles aria-hidden className="h-5 w-5" />,
  findings: <ClipboardList aria-hidden className="h-5 w-5" />,
  settings: <Settings aria-hidden className="h-5 w-5" />,
} as const;

/** Library-owned shell adapter for the Business Health navigated console. */
export function AppShell() {
  ...
  return (
    <LibraryAppShell
      ...
      sidebarStorageKey="business-health.sidebar-width"
