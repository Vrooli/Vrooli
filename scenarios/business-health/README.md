...
Business Health is the scenario that validates, visualizes, and scaffolds
every other scenario's business contract — its `PRD.md` and `requirements/`.
It checks template conformance, links requirements to intent, backs
traceability with evidence, and offers deterministic remediation. The
product surfaces — `validate`, `report`, `wizard`, `fix`, `fleet`, and
`matrix` across the CLI and Connect API, with matrix, fleet, wizard, and
findings views in the UI — are being built phase by phase; the current
scaffold ships the health endpoint only.

Underneath, the scenario packages the standard full-stack Vrooli shape:
...
The generated scaffold is intentionally not the product. As Business
Health grows its real surfaces, treat these as **placeholders** to
replace:
...
`test-genie execute business-health --preset comprehensive` directly for
