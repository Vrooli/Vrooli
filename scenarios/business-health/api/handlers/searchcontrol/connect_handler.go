// Package searchcontrol hosts business-health's implementation of the SHARED,
// token-gated search control plane (search-hub.v1.control.SearchControlService).
// It replaces the old business-health-private ReindexService proto: every search
...
func (g *Gate) authorize(logger *log.Logger, presented string) error {
	if g == nil || !g.tokenMatches(presented) {
		logger.Printf("[business-health] search control denied: control token missing or mismatched")
