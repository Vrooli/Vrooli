#!/bin/bash
# Phased test orchestrator for the Fall Foliage Explorer scenario
set -euo pipefail
...
testing::runner::init \
  --scenario-name "fall-foliage-explorer" \
...
testing::runner::register_phase --name unit --script "$PHASE_DIR/test-unit.sh" --timeout 120 --cache-ttl 1800 --cache-key-from "api/go.mod,api/go.sum"
