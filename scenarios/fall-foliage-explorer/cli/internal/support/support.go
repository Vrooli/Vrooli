...
const CLIName = "fall-foliage-explorer"
...
// If the response is a {status, data} envelope, the data field is unwrapped;
...
// EnvelopeMessage returns the top-level "message" string from a {status, message, data}
// envelope, else empty.
...
// unwrapEnvelope detects a {status, data} envelope (as used by the foliage API)
// and returns the inner data payload.
func unwrapEnvelope(body []byte) (json.RawMessage, bool) {
	...
	if strings.TrimSpace(env.Status) == "" || len(env.Data) == 0 {
		return nil, false
	}
