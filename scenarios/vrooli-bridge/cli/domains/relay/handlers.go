...
func newHandlers(core *cliapp.ScenarioApp) *handlers {
	...
	return &handlers{client: relayconnect.NewRelayServiceClient(httpClient, baseURL)}
}

func (h *handlers) call(ctx cliapp.RunContext) error {
	response, err := h.client.Call(context.Background(), connect.NewRequest(&relayv1.RelayCallRequest{
