...
```bash
export SQLITE_PATH=/tmp/vrooli-bridge.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/vrooli-bridge.db}")"
```
...
`packages/proto/gen/typescript/js/vrooli-bridge/v1/`, and Python under
