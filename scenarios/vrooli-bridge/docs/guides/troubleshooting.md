...
```bash
vrooli scenario status vrooli-bridge
make stop
vrooli scenario start vrooli-bridge --clean-stale
```
...
```bash
make status
vrooli scenario port vrooli-bridge UI_PORT
```
...
vrooli-bridge status --auto-start

# Or override the API base for this invocation
vrooli-bridge status --api-base "http://localhost:$(vrooli scenario port vrooli-bridge API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/vrooli-bridge/`.
...
The default route is `${SCENARIO_DATA_DIR}/vrooli-bridge.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/vrooli-bridge.db"
```
...
`packages/proto/gen/go/vrooli-bridge/v1/`, TypeScript under
`packages/proto/gen/typescript/vrooli-bridge/v1/`, and Python under
`packages/proto/gen/python/vrooli_bridge/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/vrooli-bridge/v1/<domain>/`.
