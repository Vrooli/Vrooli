...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/vrooli-bridge.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `vrooli-bridge` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/vrooli-bridge/config.json`
3. `~/.vrooli/config/vrooli-bridge/config.json`
4. `~/.config/vrooli/vrooli-bridge/config.json`
...
```bash
vrooli-bridge configure api_base http://localhost:15001/api/v1
...
4. Vrooli lifecycle port detection (`vrooli scenario port vrooli-bridge API_PORT`)
