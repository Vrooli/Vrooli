...
`~/.vrooli/bin/vrooli-bridge`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start vrooli-bridge` if the API is unreachable |
...
```bash
vrooli-bridge status
vrooli-bridge status --json
```

### `vrooli-bridge configure <key> <value>`
...
```bash
vrooli-bridge configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`vrooli-bridge <domain> <verb>`). Every command calls a single API
...
```bash
vrooli-bridge notes list
vrooli-bridge notes list --json
```

#### `vrooli-bridge notes create --title <title> [--body <body>]`
...
```bash
vrooli-bridge notes create --title "First note" --body "Hello world"
```
...
```bash
vrooli-bridge notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start vrooli-bridge`" is good.
