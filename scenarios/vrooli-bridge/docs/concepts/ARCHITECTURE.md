...
| Contracts (`packages/proto/schemas/vrooli-bridge/`) | Wire shape | Proto messages/services and generated clients | Hand-written route/type mirrors |
...
### Bridge's defining shape: the fleet

The standard three-surface shape above describes the **control plane**
(one Vrooli install running this scenario). Bridge's defining
characteristic is the second half of the picture: a fleet of remote
**node-agents** on the owner's other machines, each of which holds a
persistent **dial-out** channel back to the control plane. Nodes never
expose an inbound port — the agent always initiates the connection
(the NAT/firewall-proof pattern used by Tailscale, GitHub Actions
runners, and Buildkite agents). Off-LAN nodes reach the control plane
through tunnel-manager.

```
        CONTROL PLANE (one Vrooli install)                 NODES (owner's other machines)
   ┌──────────────────────────────────────┐
   │  ui/ (React fleet dashboard)          │        ┌──────────────────────────────┐
   │  cli/ (operator/agent)                │        │  node-agent (Go, cross-built)│
   │  api/ (Go, Connect-RPC + SSE edge) ◀──┼── dial ─│   linux / darwin / windows   │
   │  SQLite (api-core/storage)            │   out   │   installed as OS service    │
   └──────────────────────────────────────┘  ◀──────│   ├─ holds outbound channel  │
                │   pushes typed jobs / provisioning │   ├─ validates typed job vs  │
                │   down the held channel            │   │   the CLI manifest        │
                │                                    │   └─ shells to local `vrooli` │
                ▼   streams results/logs back up     │       (full Vrooli install)  │
        aggregates verdicts                          └──────────────────────────────┘
```

Two architectural rules are specific to bridge and load-bearing:

- **Two trust tiers.** A privileged *provisioning* path (install/update
  Vrooli, run `vrooli setup`, sync a node to a target revision R) is
  structurally separated from a non-privileged *job runner* (executes
  only allowlisted, manifest-declared CLI verbs — never a raw shell
  string). The runner can never escalate to provisioning. See
  [`../internal/SECURITY.md`](../internal/SECURITY.md).
- **Compose, don't reinvent.** Bridge orchestrates; it delegates byte
  transport to device-sync-hub, durable run lifecycle to test-genie,
  off-LAN reach to tunnel-manager, secrets to secrets-manager, owner
  auth to scenario-authenticator, and audit to workspace-sandbox. The
  contracts live in [`INTEGRATIONS.md`](INTEGRATIONS.md).

The node-agent is bridge-owned (not part of the root `vrooli` CLI):
the root CLI stays scenario-agnostic, while bridge owns the
...
- the cross-compiled **node-agent** (the thin client installed on each
  node), including its control-plane↔node Connect protocol and its
  platform-native service installers (systemd / launchd / Windows
  Service).
...
1. Add proto messages and service methods under
   `packages/proto/schemas/vrooli-bridge/v1/<domain>/`.
...
|---|---|---|---|
| 2026-06-18 | A cross-compiled **node-agent** process runs on remote machines beyond the standard single-host ui/api/cli shape. | Bridge is a fleet control plane; remote execution requires an agent on each node. | If the platform gains a generic node-agent the root CLI could host. |
| 2026-06-18 | The API hosts a persistent **dial-out** channel (SSE edge) for node presence and job push, alongside Connect/REST. | Nodes must be reachable without inbound ports; the agent initiates the connection. | If a different transport (e.g. WebTransport) supersedes SSE for fan-out. |
| 2026-06-18 | Two distinct trust tiers (privileged provisioning vs non-privileged allowlisted runner) rather than one execution path. | Remote code execution and remote provisioning are different risk classes and must be separated structurally. | If provisioning is ever delegated to an external orchestrator. |
