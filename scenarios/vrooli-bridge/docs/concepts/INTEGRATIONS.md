...
| device-sync-hub | required (P1) | Byte transport for inbound non-git artifacts (built installers, large fixtures) — Bridge transiently streams the configured source into the hub and never retains a blob. Produced run outputs use bridge's bounded authenticated artifact RPC instead. | authenticated multipart upload with explicit target-device mapping. |
