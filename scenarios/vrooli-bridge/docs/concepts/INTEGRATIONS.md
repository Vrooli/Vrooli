...
| device-sync-hub | required (P1) | Byte transport for inbound non-git artifacts (built installers, large fixtures) — Bridge transiently streams the configured source into the hub and never retains a blob; a signed Bridge channel instruction tells the target agent to pull and atomically place the item. Produced run outputs use bridge's bounded authenticated artifact RPC instead. | authenticated multipart upload with explicit target-device mapping plus a node-local pull token. |
