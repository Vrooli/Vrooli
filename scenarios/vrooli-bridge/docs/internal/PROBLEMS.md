...
- Evidence (2026-09-15, minimouse review):
  ...
  - Three ships then failed at `pair-redeem` with "pairing code not found" while the control plane held a valid, unredeemed row whose hash matched the code the node presented. Root cause: every Bridge CLI domain builds its Connect client inside `Register()`, which runs during `NewApp()` — before the global `--api-base` flag is parsed — so the base URL comes from the local port detector. Once minimouse ran the `vrooli-bridge` scenario itself (after Redis was qualified on macOS), that resolved to `http://localhost:18767` and the node redeemed against its OWN Bridge; the node's Bridge log shows the matching 401s at 17:07:07, 17:10:41 and 17:19:48. Reproduced deterministically: the same redeem command fails from `~/vrooli` and succeeds from `$HOME`. Fixed in `cli/internal/session` — the transport re-resolves `APIRootBase()` and retargets each request (`retargetURL`), so an explicit `--api-base` wins for all 20 Connect call sites. Both sides now fingerprint the code (hash prefix, never the code) when a redeem is rejected.
  - Open gap (not fixed): a ship that cannot trust its record — the node's ship digest is missing or disagrees — writes every snapshot file but deletes nothing, because deletions come only from the control plane's record. A node whose checkout moved ahead (a newer pushed commit, or a provision that replaced the tree) therefore keeps files the shipped tree no longer has. On minimouse this shipped the older single-file `internal/resources/managed_service_ownership.go` next to the node's newer `_linux.go`/`_other.go` split from commit `e8d83a7`, and the duplicate symbol broke the node's native CLI build. Repair was removing the two superseded files by hand. The fix is for a full (untrusted-record) ship to reconcile strays within the shipped path set — for example by sending the snapshot's file list and having the node delete tracked-looking paths absent from it — never touching ignored/node-owned paths.
- Blocker: none in code; minimouse's live validation continues in the session record.
- Measured: 2026-09-15

## Work ladder

- Rung: W3 (implementation/operational defects; contract and obligations unchanged)
- Evidence (2026-09-15, first controlled reboot of minimouse; the box had 42 days uptime, so unattended recovery had never been proven):
  - Recovers unattended, with nobody logged in and FileVault off: SSH answered ~51 s after the reboot; both system LaunchDaemons restarted themselves (`RunAtLoad`, `com.vrooli.bridge.vrooli-bridge-agent` as the owner, `…-provisioner` as root); the node rejoined the fleet (`online/channel/heartbeat/dispatchable`); and the credential store reached `unlocked: true` with no human action — the control plane re-pushed the escrowed passphrase, the agent held it in memory and served it on `~/.vrooli/state/credential-unlock.sock`, and `internal/securestore/chain.go` (`nodeUnlockPassphrase`) consumed it. This is the only path that opens the store on this host: the macOS Keychain write fails with `-25308` on a headless box, so there is no native unattended wrap. It confirms the claim the `credential-store` step already prints (ship op `65f30fff` step 061).
  - Does NOT recover: Colima has no launchd unit (nothing for colima/docker/lima), so after a reboot there is no Docker, every Docker-backed resource fails (`Cannot connect to the Docker daemon at unix:///Users/…/.colima/default/docker.sock`), and therefore no scenario can start at all; `colima start` takes ~23 s and clears it. Nothing autostarts scenarios either — 21 scenario APIs before the reboot, 0 after, and the only vrooli launchd units are the two Bridge daemons. No control-plane safeguard references container-runtime-at-boot. Whether to add boot units, and for which workload, is an operator decision.
  - Delivery-delay trap that makes a healthy node look permanently broken: at boot the agent logged `register node encryption key: deadline_exceeded … dial tcp [2600:4040:…]:18767: i/o timeout` — an IPv6 route to the control plane timing out. The control plane cannot seal grants to a node whose encryption key has not registered, so the `SyncNode` that fires on the presence online-hook has nothing deliverable; the passphrase landed ~11 minutes after boot. Throughout that window `credentials store status` reads `unlocked: false`, the unlock socket answers *"the control plane has not delivered this node's store passphrase yet"*, and scenarios start credential-blind — all indistinguishable from a permanent failure. Note also that the agent log line `credential unlock: serving this node's store passphrase …` means only that the server started listening; it is not evidence that a passphrase arrived.
  - Ship op `65f30fff` recorded SUCCEEDED while reporting `configuration is incomplete: 8 item(s)`, validating the blocker-keyed verdict on a live ship; the earlier op `f02f6cca` had recorded `failed exit=255 reason=onboarding_apply_failed` for the same condition.
  - Fixed outside Bridge while validating the node: five credential format validators (`storeFormat`, `normalizeKeyringOptions`, `Extensions`, `List`, `Status`) rejected `--format text`, the default the CLI itself declares for 13 credentials verbs, while each error message promised "text or json"; `doctor` had been repaired in isolation and its siblings left broken, so bare `vrooli credentials store status` failed on every host. All five now route through one `normalizeOutputFormat` helper in `internal/app/credentials` with regression tests, and the bare command renders again on both the control plane and the node.
- Blocker: partially repaired 2026-09-16 — the container runtime now returns by itself (`com.vrooli.colima-boot`, a system LaunchDaemon running `/usr/local/bin/colima start` as the owner with an explicit PATH/HOME and no `KeepAlive`, proved by `colima stop` + `launchctl kickstart` rather than a second reboot). Scenario autostart is still absent by choice: starting 21 scenarios at boot is a workload policy decision, and no control-plane-owned autostart feature exists to carry it.
- Measured: 2026-09-16

## Work ladder

- Rung: W3 (the credential-store copy had no operator surface at all)
- Evidence (2026-09-15, closing the "no backup sink" gap recorded above):
  - The encrypted credential-store copy was implemented and unreachable. `StoreCopy`, `StoreCopyConfigure` and `StoreCopyScheduled` existed with a complete `CopyConfig`, but `credentials store` bound only eight subcommands and none was `copy`; `grep` found zero callers outside the app package, and the manifest had no copy entry. So `vrooli credentials store copy configure --sink …` — the command this file told an operator to run — did not exist, and no host's store was ever copied. The surface is now wired: `copy`, `copy-configure` and `copy-scheduled` in `cli/manifest.json` plus `internal/cli/credentialshandlers`, pinned by `copy_paths_test.go` so the feature cannot be orphaned again.
  - Wiring the surface exposed two more instances of the `--format text` self-refusal, and a full re-sweep found two others that were already reachable and broken: `credentials recovery export` and `credentials recovery verify` both rejected the CLI's own declared default. An earlier pass had fixed five sites and claimed the class was closed; it was not. All four now route through `normalizeOutputFormat`. The two surviving strict checks (`credentials_doctor.go:305`, `credentials_keyring.go:200`) are unreachable for "text" because each is preceded by normalization.
  - The sink cannot be local on this control plane. `StoreCopy` applies `CopyPolicy{ProtectedRoots: [<vrooli home>], RequireIndependentDevice: true}`, and `internal/securestore/store_escrow.go:134-152` refuses a sink inside the store's parent, inside a protected root, or on the same device as the store. swarminator has one writable data device (`/dev/nvme0n1p2` at `/`); `/boot/efi` is a 1.1 GB EFI partition and the rest are snap loops and tmpfs. A sink under `~/.vrooli/backups` was refused as a protected root (the error says "kopia repository" because `SinkConflictError` is shared by both checks). The configuration is therefore left with the sink path and a 24h interval recorded but `enabled: false`, so no schedule fails silently; it needs one `--enabled` once a legal target exists. Choosing that target is an operator decision: an external mount, or an `s3://` sink, which `CopyConfig.Validate` additionally requires to carry an object-store credential identity and region (both are among the 8 items the onboarding selection reports as needing operator input).
- Blocker: no credential-store backup exists on this control plane until an independent-device or s3 sink is chosen; the surface to use one now exists.
- Measured: 2026-09-15

## Work ladder

- Rung: W3 (node-side blocker found while restoring minimouse; not a Bridge defect)
- Evidence (2026-09-16, restoring the 21-scenario baseline after the reboot test): 20 of 21 scenarios came back. `secrets-manager` fails its setup step `create-database` with `FATAL: database "vrooli" does not exist`. The scenario declaration is `resource-postgres content create-database vrooli_secrets_manager` and is correct — that database exists and survived the reboot (`pg_database` holds `postgres`, `template0`, `template1`, `vrooli_secrets_manager`), and postgres is healthy and serving on 5433. The defect is in the postgres resource CLI: `resource-postgres content` defaults its *connection* database to `vrooli`, so `content list`, the scenario's setup step, and even `content create-database vrooli` all fail the same way — the bootstrap is circular, because creating `vrooli` requires connecting to `vrooli` first. Connection info comes back fine for an explicit database (`content get --database postgres`), which is how the inventory above was read. Not repaired by hand: creating `vrooli` with raw SQL would hide a bootstrap path that breaks identically on any host without that database. The fix belongs in `resource-postgres` — connect through `postgres`/`template1` when the target database is absent.
- Blocker: `secrets-manager` cannot start on minimouse until the postgres resource CLI can bootstrap without a pre-existing `vrooli` database.
- Measured: 2026-09-16

## Work ladder

- Rung: W3 (implementation defects in the capability probe, the agent's service unit, and Web Console's refusal text; contract and obligations unchanged)
- Evidence (2026-09-16, operator report: creating a codex terminal session for minimouse in Web Console failed with `[failed_precondition] remote target unavailable: capability "codex" on minimouse is unknown; Refresh the capability probe and check that the node is reporting`, while the node's status looked healthy):
  - The node *was* reporting, and the advice was unactionable. `codex` and `grok` were present in the capability inventory as UNKNOWN with the detail "command was found but its version could not be read"; `agy`, `claude` and `opencode` were READY. The split is not random: the two failing tools are `#!/usr/bin/env node` trampolines symlinked into `~/.local/node_modules/.bin`, and the three working ones are native Mach-O binaries. `codex --version` exited **127** with `env: node: No such file or directory`.
  - Node was installed the whole time (Homebrew `/usr/local/bin/node` 26.5.0, plus nvm v24.11.1). The agent LaunchDaemon declared no `EnvironmentVariables`, so launchd gave it `PATH=/usr/bin:/bin:/usr/sbin:/sbin`, which contains neither. `capabilityprobe.ManagedLookPath` resolved the **tool** (it explicitly checks `~/.local/bin`), but a shebang resolves the **interpreter** against the PATH the process inherits, which the probe never set. Proved by changing only PATH on the same box: `env -i PATH=/usr/local/bin:... codex --version` → `codex-cli 0.141.0`, exit 0.
  - Three probe defects fixed in `packages/capabilityprobe`: (1) version commands now run with a managed PATH (`ManagedPathEntries`, mirroring platform-go's `DefaultPathEntries`), and `ManagedLookPath` searches that same list, so a tool we can find is one whose runtime we can find; (2) the per-command timeout was dead code — `runVersion` applied `DefaultCommandTimeout` only when the caller's context carried no deadline, and `ProbeWith` always supplies one, so a single hanging tool could consume the entire 15 s probe budget and leave every tool after it unreadable; (3) `Unknown` collapsed missing-interpreter, timeout and non-zero exit into one string — the detail now names the interpreter, the timeout, or the exit code plus the command's own output.
  - The Bridge agent was the only long-lived Vrooli unit rendering no environment at all. platform-go's `ServiceDefinition` already carries `Env`, both renderers already emit it (`EnvironmentVariables` / `Environment=`), and the emergency, supervisor and watchdog definitions all set `PATH: DefaultPath(target, home)`. `Definition.Artifact` now does the same, with home resolved from the service **principal** (`d.User`), so a LaunchDaemon installed under sudo does not bake root's home into the unit.
  - Web Console's refusal named an operation the product does not offer: `RefreshCapabilities` is reachable only through `changesCapabilities`, which matches exactly `resource install` and `resource uninstall`, so no operator-triggered capability refresh exists anywhere. The `Unknown` recovery text now points at the machine, and `ensureLaunchCapability` leads with the observation's own detail instead of the state plus generic advice.
  - Live repair on minimouse, which still runs the *shipped* agent and therefore carries none of the above: added `EnvironmentVariables`/`PATH` to `/Library/LaunchDaemons/com.vrooli.bridge.vrooli-bridge-agent.plist` (backup `.bak-20260916`) and reloaded. **`launchctl kickstart -k` restarts the already-loaded job and does not re-read the plist** — the first attempt looked applied and changed nothing; only `bootout` + `bootstrap` loads a changed definition. All five ai-cli tools then reported READY (`codex-cli 0.154.0`, `grok 0.2.72`) with the node online and dispatchable.
  - Side effect worth knowing: widening the probe's PATH can change *which* install a tool resolves to. `claude` moved from 2.1.185 to 2.1.236 on this node, because `exec.LookPath` now finds an install earlier in the new PATH than `~/.local/bin`.
- Blocker: minimouse runs the shipped agent build, so the probe and service-unit fixes reach it only on the next ship or service reinstall — at which point the rendered unit supersedes the hand-applied plist key (the intended outcome, not a conflict).
- Measured: 2026-09-16
