...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/vrooli-bridge/v1/health"`)
...
import (
    ...
    "vrooli-bridge/handlers/health"
    "vrooli-bridge/internal/clock"
    "vrooli-bridge/internal/module"
    "vrooli-bridge/internal/server"
    "vrooli-bridge/internal/testutil/assertx"
...
The proto schema in `packages/proto/schemas/vrooli-bridge/v1/health/health.proto`
...
dynamically from `go.mod`, so it works after `vrooli-bridge`
substitution.
...
under `packages/proto/schemas/vrooli-bridge/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/vrooli-bridge/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.vrooli_bridge.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/vrooli-bridge/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/vrooli-bridge/v1/<domain>/`,
   `packages/proto/gen/typescript/js/vrooli-bridge/v1/<domain>/`, and
   `packages/proto/gen/python/vrooli_bridge/v1/<domain>/`.
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/vrooli-bridge/v1/<domain>"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/vrooli-bridge/v1/<domain>/<domain>_pb";
