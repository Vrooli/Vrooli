...
| **Production wiring** | `agent/internal/channel/channel.go` wires `runEventReporter` (a signed `RunsService.ReportRunEvent` call) and the default `osCommandRunner` (`os/exec.CommandContext` over a pre-split argv — never `sh -c`). Unix cancellation targets the child process group; Windows attaches the child to a kill-on-close Job Object so cancellation also terminates descendants. |
...
| **Production wiring** | `agent/main.go`: `--print-service-unit` renders via `service.NewManager().Render(def)`; the `service install\|status\|uninstall` verbs build the SAME `Definition` (`serviceDefinition`) and call `Install/Status/Uninstall`. Ordinary Linux uses a systemd user unit; the privileged helper uses a machine-wide systemd unit. Ordinary macOS uses a LaunchAgent; the privileged helper uses a LaunchDaemon. Windows drives the Service Control Manager through typed `sc.exe` create/configure/start/query/stop/delete commands, verifies `BINARY_PATH_NAME` and `SERVICE_START_NAME` during status, and `main_windows.go` enters the SCM dispatcher with Stop/Shutdown cancellation when launched as a service. The privileged IPC endpoint uses a Windows named pipe with a principal-bound DACL; Unix retains peer UID checks. All supported paths converge idempotently through their native managers. `NewManager` mirrors `platform.NativeServiceManager()`. The `Definition.User` field carries the OS principal, making the two trust tiers distinct principals at install time. Native Windows install/onboarding, reboot, removal, and clean-host qualification remain open. |
...
| **Production wiring** | `handlers/artifacts/adapter.go::deviceSyncDelivery` streams a local-path, `file://`, or HTTP(S) source through device-sync-hub's authenticated multipart upload edge, records the returned item ref, and queues a signed `channel.ArtifactDelivery` frame. The target agent uses its node-local `BRIDGE_DEVICE_SYNC_DEVICE_TOKEN` to pull the item and atomically place it at the requested destination path. The hub URL, origin device token, target-node token, and explicit Bridge-node→hub-device mapping are operator configuration; missing trust or mapping fails closed. |
