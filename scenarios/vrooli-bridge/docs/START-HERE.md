...
`template-manager orient vrooli-bridge`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient vrooli-bridge
...
```bash
vrooli scenario requirements validate vrooli-bridge --json
```
