...
`vrooli scenario orient vrooli-bridge`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient vrooli-bridge
...
```bash
cat > /tmp/prd_context_vrooli-bridge.md <<'EOF'
Overview:
- Purpose:
...
```bash
prd-control-tower prd generate vrooli-bridge \
...
```bash
prd-control-tower prd validate vrooli-bridge --json
```
...
```bash
cat > /tmp/requirements_context_vrooli-bridge.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
prd-control-tower requirements generate vrooli-bridge \
...
```bash
prd-control-tower requirements validate vrooli-bridge --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/vrooli-bridge/v1/<domain>/<domain>.proto`
