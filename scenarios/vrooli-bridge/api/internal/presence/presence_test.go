...
func TestHub_HeartbeatStalenessMakesHalfOpenChannelUndispatchable(t *testing.T) {
	...
	h.Heartbeat("n1", presence.HealthSnapshot{ToolchainPresent: true})
	require.True(t, h.IsOnline("n1"))
	require.False(t, h.Dispatchable("n1"), "dispatch requires an explicit protocol verdict")
...
func TestHub_ReadinessFactsFlipIndependently(t *testing.T) {
	...
	require.False(t, facts.ProtocolCompatible, "an unreported protocol version is not compatible")
	require.False(t, facts.Dispatchable, "dispatch requires an explicit protocol verdict")
