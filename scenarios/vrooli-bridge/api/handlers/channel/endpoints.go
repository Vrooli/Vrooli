...
var Endpoints = []module.EndpointDescriptor{
	...
	},
	{
		...
		Description: "Opens a binary WebSocket session with explicit owner re-authentication, the vrooli-bridge:write node scope, bounded flow-control window, frame-rate/size limits, idle timeout and hard lifetime. Remote desktop sessions additionally bind device-control surface, lease epoch, policy revision and owner identity; SSE job delivery remains available separately.",
	...
	{
		ID:          "presence_report_artifact_receipt",
		Path:        presenceconnect.PresenceServiceReportArtifactReceiptProcedure,
		Method:      "POST",
		Summary:     "Report artifact placement receipt",
		Description: "Node-facing: records metadata-only success or failure after the target agent pulls and atomically places a directed artifact.",
		Category:    "channel",
		Request:     &module.Schema{Type: "object", Properties: map[string]string{"receipt": "ArtifactReceipt (distribution_id, node_id, item_id, destination_path, accepted, reason, sha256, size_bytes)"}},
		Response:    &module.Schema{Type: "object", Properties: map[string]string{"accepted": "boolean"}},
		Errors: []module.ErrorDesc{
			{Status: 400, Code: "invalid_argument", Description: "Missing artifact receipt metadata"},
			{Status: 401, Code: "unauthenticated", Description: "Missing or invalid node proof"},
			{Status: 403, Code: "permission_denied", Description: "Receipt does not match the distribution or target node"},
		},
	},
	{
	...
	{
		ID:          "presence_report_interactive_signal_response",
		Path:        presenceconnect.PresenceServiceReportInteractiveSignalResponseProcedure,
		Method:      "POST",
		Summary:     "Receive an authenticated interactive signal response",
		Description: "Node-facing: returns bounded opaque SDP/ICE response bytes from the node-local Device Control companion to the waiting owner signal call.",
		Category:    "channel",
		Request:     &module.Schema{Type: "object", Properties: map[string]string{"response": "InteractiveSignalResponse (channel_id, node_id, lease_epoch, request_id, accepted, payload)"}},
		Response:    &module.Schema{Type: "object", Properties: map[string]string{"accepted": "boolean"}},
		Errors: []module.ErrorDesc{
			{Status: 400, Code: "invalid_argument", Description: "Missing interactive signal response identity"},
			{Status: 401, Code: "unauthenticated", Description: "Missing or invalid node proof"},
			{Status: 403, Code: "permission_denied", Description: "Response node identity does not match node proof"},
		},
	},
	{
		ID:          "presence_report_companion_response",
		Path:        presenceconnect.PresenceServiceReportCompanionResponseProcedure,
		Method:      "POST",
		Summary:     "Receive an authenticated companion lifecycle response",
		Description: "Node-facing: returns the metadata-only observation for a Bridge-managed Device Control companion operation.",
		Category:    "channel",
		Request:     &module.Schema{Type: "object", Properties: map[string]string{"response": "CompanionResponse (operation_id, node_id, state, reason_code)"}},
		Response:    &module.Schema{Type: "object", Properties: map[string]string{"accepted": "boolean"}},
	},
	{
		ID: "channel_session_frame", Path: presenceconnect.PresenceServiceReportSessionFrameProcedure, Method: "POST",
		Summary: "Receive an authenticated node session frame", Description: "Node-facing Connect-RPC for bounded session output, evidence metadata, revoke, and close events. Requires the node proof and binds the frame to its Bridge session.", Category: "channel",
	},
}
