...
export function MachineLifecyclePanel() {
  ...
  return <section data-testid={selectors.fleet.machineLifecycle} aria-labelledby="machine-lifecycle-heading" className="rounded-panel border border-app-border bg-app-surface p-4">
    ...
    <ul className="mt-3 flex flex-col gap-2">
      {machines.data?.map((machine) => <li key={machine.id} data-testid={selectors.fleet.machineLifecycleRow({ id: machine.id })} className="rounded-control border border-app-border bg-app-background p-3">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <Button size="sm" type="button" variant="secondary" disabled={busy} onClick={() => setSelectedID(machine.id)}>{t(strings.fleet.machines.details)}</Button>
          <Button size="sm" type="button" variant="secondary" disabled={busy} onClick={() => archive.mutate(machine)}>{t(strings.fleet.machines.archive)}</Button>
          <Button size="sm" type="button" variant="secondary" disabled={busy} onClick={() => revoke.mutate(machine.id)}>{t(strings.fleet.machines.revokeNode)}</Button>
          <Button size="sm" type="button" variant="secondary" disabled={busy} onClick={() => cleanup.mutate(machine.id)}>{t(strings.fleet.machines.cleanup)}</Button>
          <Button size="sm" type="button" variant="secondary" disabled={busy} onClick={() => { if (window.confirm(t(strings.fleet.machines.removeConfirm, { id: machine.id }))) remove.mutate(machine); }}>{t(strings.fleet.machines.remove)}</Button>
        </div>
      </li>)}
    </ul>
    {selectedID && <section data-testid="machine-detail" className="mt-3 rounded-control border border-app-border bg-app-background p-3" aria-live="polite">
      <div className="flex items-center justify-between gap-2"><h4 className="text-sm font-semibold text-app-foreground">{t(strings.fleet.machines.detailHeading, { id: selectedID })}</h4><Button size="sm" type="button" variant="secondary" onClick={() => setSelectedID(null)}>{t(strings.fleet.machines.closeDetails)}</Button></div>
      ...
      {detail.data && <>
        ...
        {detail.data.effectivePolicy && <div data-testid="machine-effective-policy" className="mt-2 text-xs text-app-muted-foreground"><p><span className="font-medium text-app-foreground">Desired configuration:</span> {detail.data.effectivePolicy.profileId} · {detail.data.effectivePolicy.profileVersion} · {detail.data.effectivePolicy.setupPreset}</p><p className="mt-1">Scenarios: {detail.data.effectivePolicy.scenarios.join(", ") || "none"}</p><p className="mt-1">Optional resources: {detail.data.effectivePolicy.optionalResources.join(", ") || "none"}</p></div>}
        {(detail.data.drift ?? []).length > 0 && <div data-testid="machine-drift" className="mt-2 text-xs text-app-warning"><span className="font-medium">Drift:</span> {(detail.data.drift ?? []).map((item) => `${item.name}: ${item.reason}`).join("; ")}</div>}
