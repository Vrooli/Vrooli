...
// [REQ:BRG-P0-007] The systemd unit carries the configured argv, restarts on
// failure, and contains NO hardcoded path other than the ones supplied on the
// Definition (no Linux-only assumptions baked in). A user-scope unit is wanted
// by default.target: multi-user.target is a system target a user manager never
// reaches, which is why the agent unit was inactive after every boot until
// 2026-09-02. A user manager cannot switch principals, so User= is omitted.
func TestSystemdUnit_RendersConfiguredPaths(t *testing.T) {
	...
	require.NotContains(t, unit, "network-online.target")
	require.Contains(t, unit, "Documentation=https://github.com/Vrooli/Vrooli/blob/master/scenarios/vrooli-bridge/agent/internal/service/service.go")
	assertNoHardcodedHostPaths(t, unit, sampleDef())
}

// [REQ:BRG-P0-007] A system-scope unit (the privileged helper) keeps its
// principal and is wanted by multi-user.target.
func TestSystemdUnit_SystemScopeKeepsPrincipal(t *testing.T) {
	d := sampleDef()
	d.System = true
	unit, err := service.SystemdUnit(d)
	require.NoError(t, err)
...
func TestLaunchdPlist_RendersProgramArguments(t *testing.T) {
	...
	require.NotContains(t, plist, "<string>vrooli-agent</string>", "a LaunchAgent runs as the user whose domain loaded it")
	assertNoHardcodedHostPaths(t, plist, sampleDef())

	daemon := sampleDef()
	daemon.System = true
	daemonPlist, err := service.LaunchdPlist(daemon)
	require.NoError(t, err)
	require.Contains(t, daemonPlist, "<string>vrooli-agent</string>", "a LaunchDaemon keeps its principal")
}
...
func TestSystemdUnit_PrivilegedHelperSeparatePrincipal(t *testing.T) {
	agentDef := sampleDef()
	agentDef.System = true
	agent, err := service.SystemdUnit(agentDef)
	require.NoError(t, err)
