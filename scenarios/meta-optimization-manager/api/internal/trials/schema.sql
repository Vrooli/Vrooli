...
-- fixture_rev: the content revision of the fixture the run exercised, part of
-- the (task, model, fixture_rev) idempotency key. Nullable/'' for legacy rows
-- and no-fixture error runs.
CREATE TABLE IF NOT EXISTS trials_runs (
  ...
  fixture_rev      TEXT NOT NULL DEFAULT '',
...
-- The idempotency lookup: latest run for an exact (task, model, fixture_rev).
CREATE INDEX IF NOT EXISTS idx_trials_runs_idem ON trials_runs(task_id, model, fixture_rev, created_at);
