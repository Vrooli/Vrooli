...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("meta-optimization-manager")
	if err != nil {
		return "", fmt.Errorf("resolve meta-optimization-manager storage namespace: %w", err)
	}
	path, err := resolver.Path(
		storage.Options{ScenarioID: scenarioID},
		storage.ClassData,
		"meta-optimization-manager.db",
	)
	if err != nil {
		return "", fmt.Errorf("resolve meta-optimization-manager db path: %w", err)
	}
	return sqliteFileDSN(path)
}
