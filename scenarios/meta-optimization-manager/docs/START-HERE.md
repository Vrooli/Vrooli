...
`vrooli scenario orient meta-optimization-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient meta-optimization-manager
...
```bash
prd-control-tower prd generate meta-optimization-manager \
...
```bash
prd-control-tower prd validate meta-optimization-manager --json
```
...
```bash
prd-control-tower requirements generate meta-optimization-manager \
...
```bash
prd-control-tower requirements validate meta-optimization-manager --json
```
