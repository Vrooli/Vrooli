# Start Here — Meta-Optimization Manager
...
`template-manager orient meta-optimization-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient meta-optimization-manager
...
```bash
cat > /tmp/prd_context_meta-optimization-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) meta-optimization-manager \
  --context-file /tmp/prd_context_meta-optimization-manager.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate meta-optimization-manager --json
```
...
```bash
cat > /tmp/requirements_context_meta-optimization-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) meta-optimization-manager \
  --context-file /tmp/requirements_context_meta-optimization-manager.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/meta-optimization-manager/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest meta-optimization-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> meta-optimization-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario meta-optimization-manager
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> meta-optimization-manager <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/meta-optimization-manager/v1/<domain>/`.
