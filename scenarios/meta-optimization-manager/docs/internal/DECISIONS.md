...
| 2026-06-24 | Empirical trials use Agent Manager's sandboxed primitive after reconciling MoM's manifest-declared role-only profile (`profile reconcile-scenario` → `task create` → `run create --run-mode sandboxed` → poll `run get` → `run diff`); metrics are success + tokens + wall-time as a historical trend. | Readiness is ultimately empirical, not declarable from coverage. Agent Manager owns runner/model/resource resolution and the sandboxed-agent-spawn primitive; MoM must not choose runtime details. | `trials` is gated behind explicit invocation (expensive), always sandboxed; the Runner consumes Agent Manager's profile/task/run CLI through the `CommandRunner` seam. | Revisit when trial cost/latency justifies a cheaper proxy. |
| 2026-06-28 | Live **numerator** reads are **typed API↔API** (Connect-RPC via `api-core/discovery`), concurrent and ~3s-deadlined — **not** CLI shell-outs. Added a prompt-manager `GraphService.GetHealthScores` Connect contract for the Guide read (prompt-manager had no proto). | A `test-genie health --json` shell-out could hang ~25s while healthy; MoM's serial 30s per-owner timeout stalled the whole board. The owner CLIs are thin wrappers over RPCs that already exist; MoM should call those with a deadline it controls. | `NumeratorJoiner` resolves each owner and calls a typed client; a slow/unreachable owner is an honest per-projection `UNAVAILABLE`, never a board-wide hang. Denominator (`SpaceReader`) still uses the owner `space` verb (timeout tightened to 5s). | Revisit if an owner removes/renames its numerator RPC, or if discovery resolution becomes the bottleneck. |
...
| 2026-06-24 | Documentation-first: PRD + requirements authored before any domain code. PRD was **hand-authored to the canonical template + deterministically validated** because the legacy PRD tooling's LLM-generation backend returned HTTP 500 (that tooling is since retired; authoring now drives the deterministic `business-health wizard`). | The guide preferred generated PRDs, but AI generation was unavailable; the deterministic validator was healthy. | PRD validates clean (0 violations) and drives the requirements registry. | Optionally re-author via the `business-health` wizard if a different shape is wanted. |
| 2026-08-11 | Adopt the trustworthy-retrieval plan's ten cross-domain decisions as the coverage consumer contract. | Meta-optimization-manager consumes Search Hub's live registry, reachability, and eval evidence; it must not infer readiness from authored declarations alone. | D1–D8 govern upstream supply and evaluation; D9 makes Answer NOW require independent active, reachable, and fresh-eval signals; D10 keeps governance explicit and provider-owned. Missing or unavailable signals remain visible as IN-REACH/UNAVAILABLE, never fabricated completeness. | Revisit when the Search Hub evidence contract or distributed denominator ownership changes. |
