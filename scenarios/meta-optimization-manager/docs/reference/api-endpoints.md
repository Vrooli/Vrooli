# API Endpoints — Meta-Optimization Manager
...
envelope (`packages/proto/schemas/meta-optimization-manager/v1/shared/errors.proto`):
...
| **CLI** | `meta-optimization-manager status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/meta-optimization-manager/v1/shared/health.proto`
...
`POST /vrooli.meta_optimization_manager.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.meta_optimization_manager.v1.notes.NotesService/ListNotes`
...
| **CLI** | `meta-optimization-manager notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.meta_optimization_manager.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.meta_optimization_manager.v1.notes.NotesService/CreateNote`
...
| **CLI** | `meta-optimization-manager notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.meta_optimization_manager.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.meta_optimization_manager.v1.notes.NotesService/GetNote`
...
| **CLI** | `meta-optimization-manager notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.meta_optimization_manager.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `meta-optimization-manager notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/meta-optimization-manager/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/meta-optimization-manager/v1/<domain>/`, then run
   `make generate`.
