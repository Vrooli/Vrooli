# Configuration — Meta-Optimization Manager
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/meta-optimization-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `meta-optimization-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/meta-optimization-manager/config.json`
3. `~/.vrooli/config/meta-optimization-manager/config.json`
4. `~/.config/vrooli/meta-optimization-manager/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port meta-optimization-manager API_PORT`)
