# CLI Commands — Meta-Optimization Manager
...
`~/.vrooli/bin/meta-optimization-manager`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start meta-optimization-manager` if the API is unreachable |
...
### `meta-optimization-manager status`
...
```bash
meta-optimization-manager status
meta-optimization-manager status --json
```

### `meta-optimization-manager configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
meta-optimization-manager configure api_base http://localhost:15001/api/v1
meta-optimization-manager configure token <token>
```

Read values back without an argument:

```bash
meta-optimization-manager configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`meta-optimization-manager <domain> <verb>`). Every command calls a single API
...
#### `meta-optimization-manager notes list`
...
```bash
meta-optimization-manager notes list
meta-optimization-manager notes list --json
```

#### `meta-optimization-manager notes create --title <title> [--body <body>]`
...
```bash
meta-optimization-manager notes create --title "First note" --body "Hello world"
```
...
```bash
meta-optimization-manager notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start meta-optimization-manager`" is good.
