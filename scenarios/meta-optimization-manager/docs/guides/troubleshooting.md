# Troubleshooting — Meta-Optimization Manager
...
```bash
vrooli scenario status meta-optimization-manager
make stop
vrooli scenario start meta-optimization-manager --clean-stale
```
...
```bash
make status
vrooli scenario port meta-optimization-manager UI_PORT
```
...
meta-optimization-manager status --auto-start

# Or override the API base for this invocation
meta-optimization-manager status --api-base "http://localhost:$(vrooli scenario port meta-optimization-manager API_PORT)/api/v1"
```
...
### `meta-optimization-manager configure` doesn't persist

Check which config-file path resolved (precedence in
[`../reference/configuration.md`](../reference/configuration.md#cli-config-file)).
Most commonly `~/.vrooli/config/meta-optimization-manager/config.json`. The
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/meta-optimization-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/meta-optimization-manager.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/meta-optimization-manager.db"
```
...
`packages/proto/gen/go/meta-optimization-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/meta-optimization-manager/v1/`, and Python under
`packages/proto/gen/python/meta_optimization_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/meta-optimization-manager/v1/<domain>/`.
