...
```bash
export SQLITE_PATH=/tmp/meta-optimization-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/meta-optimization-manager.db}")"
```
...
`packages/proto/gen/typescript/js/meta-optimization-manager/v1/`, and Python under
