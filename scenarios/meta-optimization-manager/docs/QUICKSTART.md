# Quickstart — Meta-Optimization Manager
...
meta-optimization-manager <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port meta-optimization-manager API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.meta_optimization_manager.v1.<domain>.<Service>/<Method>
```
...
meta-optimization-manager notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port meta-optimization-manager API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.meta_optimization_manager.v1.notes.NotesService/ListNotes" \
...
CLI (`meta-optimization-manager --help`) — see
