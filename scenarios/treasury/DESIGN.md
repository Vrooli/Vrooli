...
## Why this design language fits Treasury

Treasury is a surface where a misread number moves real money, so the kit's
calm, dense, operational posture is the correct starting point rather than a
default accepted for convenience. Three properties of this design map
directly onto what this scenario needs, and one needs sharpening.

**Density serves scanning, not impressing.** The console's landing surface
is the pending-approval queue, and its job is to let an operator judge
several authorization requests without opening a detail view for each. The
kit's compact information surfaces and tight vertical rhythm are what make
that possible; a spacious consumer layout would turn a five-item queue into
scrolling.

**Precise status color is load-bearing here, not decorative.** This
scenario has four outcomes an operator must distinguish instantly —
approved, refused by policy, declined by a human, and expired — plus the
`unknown` settlement state, which is the one that needs a human most
urgently. The kit's semantic status palette carries that, and the token
contract stays intact.

**It is built for long-running workflows.** An approval can wait on a
person for hours, a standing mandate spans months, and a settlement can sit
in `unknown` until a rail answers. The kit's support for durable, resumable
state is the reason it suits this scenario better than a dashboard
language optimised for live metrics.

**Where this scenario goes beyond the kit's floor.** Status here must never
be conveyed by color alone. A declined charge and an expired mandate are
different facts with different remedies, and an operator who cannot
distinguish red from amber must still tell them apart — so every status
carries a shape or a text label in addition to its hue. Amounts use
tabular figures so columns align and a magnitude error is visible as a
ragged edge. Destructive and irreversible controls — freeze, revoke,
approve — are visually distinct from navigation and never adjacent to it,
because the cost of a misclick here is a real payment rather than a wrong
page. Accessible names on money-acting controls state the amount and
counterparty, so a screen-reader user hears what they are approving rather
than "button". These are additions to the kit's accessibility floor, not
departures from its token contract.

**Token contract:** kept intact. No kit substitution, no palette override.
The scenario-specific work is in what the surfaces communicate, not in
redefining the language they communicate in.
