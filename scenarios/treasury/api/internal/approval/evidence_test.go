...
func (c evidenceController) Get(context.Context, string) (authorization.Record, error) {
	return c.value, nil
}
...
func TestDeclinedApprovalRecordsCompleteAttempt(t *testing.T) {
	...
	_, err = handle.ExecContext(ctx, `CREATE TABLE authorization_book_bindings(authorization_id TEXT PRIMARY KEY, book_id TEXT NOT NULL)`)
	require.NoError(t, err)
	...
	auth := authorization.Record{ID: "auth-declined", IdempotencyKey: "proposal-key", BookID: "book-1", MandateID: "mandate-1", RequestingAgent: "agent:1", AmountMinor: 375, Currency: "USD", Counterparty: "vendor.example"}
	_, err = handle.ExecContext(ctx, `INSERT INTO authorizations(id) VALUES(?)`, auth.ID)
	require.NoError(t, err)
	_, err = handle.ExecContext(ctx, `INSERT INTO authorization_book_bindings(authorization_id,book_id) VALUES(?,?)`, auth.ID, auth.BookID)
	require.NoError(t, err)
	...
	require.NoError(t, service.Admit(ctx, authorization.ApprovalAdmission{ID: "approval-1", AuthorizationID: auth.ID, BookID: auth.BookID, MandateID: auth.MandateID, RequestingAgent: auth.RequestingAgent, AmountMinor: auth.AmountMinor, Currency: auth.Currency, Counterparty: auth.Counterparty, ExpiresAt: now.Add(time.Hour)}))
...
func TestExpiredApprovalRecordsCompleteAttempt(t *testing.T) {
	...
	require.NoError(t, database.EnsureSchemas(ctx, handle, database.SchemaProviderFunc(approval.Schema), database.SchemaProviderFunc(evidence.Schema)))
	now := time.Date(2026, 8, 18, 12, 0, 0, 0, time.UTC)
	auth := authorization.Record{ID: "auth-expired", IdempotencyKey: "expiry-key", BookID: "book-1", MandateID: "mandate-1", RequestingAgent: "agent:1", AmountMinor: 425, Currency: "USD", Counterparty: "vendor.example"}
	_, err = handle.ExecContext(ctx, `INSERT INTO authorizations(id) VALUES(?)`, auth.ID)
	require.NoError(t, err)
	_, err = handle.ExecContext(ctx, `INSERT INTO authorization_book_bindings(authorization_id,book_id) VALUES(?,?)`, auth.ID, auth.BookID)
	require.NoError(t, err)
	...
	require.NoError(t, service.Admit(ctx, authorization.ApprovalAdmission{ID: "approval-expired", AuthorizationID: auth.ID, BookID: auth.BookID, MandateID: auth.MandateID, RequestingAgent: auth.RequestingAgent, AmountMinor: auth.AmountMinor, Currency: auth.Currency, Counterparty: auth.Counterparty, ExpiresAt: now.Add(time.Minute)}))
