...
```bash
vrooli scenario status treasury
make stop
vrooli scenario start treasury --clean-stale
```
...
treasury status --auto-start

# Or override the API base for this invocation
treasury status --api-base "http://localhost:$(vrooli scenario port treasury API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/treasury/`.
...
The default route is `${SCENARIO_DATA_DIR}/treasury.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/treasury.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/treasury.db}")"
```
...
`packages/proto/gen/go/treasury/v1/`, TypeScript under
`packages/proto/gen/typescript/js/treasury/v1/`, and Python under
`packages/proto/gen/python/treasury/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/treasury/v1/<domain>/`.
