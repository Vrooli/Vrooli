...
`template-manager orient treasury`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient treasury
...
```bash
cat > /tmp/prd_context_treasury.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start treasury --interactive
# …answer the interview, then:
business-health wizard preview treasury
business-health wizard apply treasury
```
...
```bash
vrooli scenario requirements validate treasury --json
```
...
```bash
cat > /tmp/requirements_context_treasury.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate treasury --json
business-health fix preview treasury
business-health fix apply treasury
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate treasury --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate treasury`, the notes page spec
...
- [ ] **Start in proto.** Author `packages/proto/schemas/treasury/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest treasury <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> treasury
