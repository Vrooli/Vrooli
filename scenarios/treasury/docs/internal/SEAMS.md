...
`packages/proto/schemas/treasury/v1/<domain>/<file>.proto` —
...
The `health` proto in `packages/proto/schemas/treasury/v1/shared/`
...
| | |
|---|---|
| **Seam** | Immutable mandate signing |
| **Interface** | `internal/mandate/service.go::Signer` (`Sign(ctx, canonicalPayload)`). |
| **Production wiring** | The mandate module will resolve signing material at the composition edge and construct `mandate.HMACSigner`; no key is persisted with the grant. |
| **Test fake** | Mandate service tests inject a deterministic signer response or error; `signer_test.go` exercises the real HMAC implementation. |
| **Why it exists** | Grant creation must fail closed if signing fails, while unit tests need stable signatures and explicit failure injection. Keeping signing behind the domain boundary also prevents repositories from owning secret material. |

### Clock

| | |
|---|---|
| **Seam** | Wall-clock and domain decision time |
| **Interface** | `api-core/schedule::Clock` (`Now() time.Time`), consumed by middleware and time-bearing domain services. |
| **Production wiring** | `main.go` constructs the shared system clock and passes it via `server.Deps`; domain modules receive the same seam at composition time. |
| **Test fake** | `api-core/schedule::FakeClock` (`Advance`, `Set`) drives duration, timestamp, and expiry tests without sleeping. |
| **Why it exists** | Request durations and mandate expiry must be deterministic. In particular, expiry is authorization policy rather than a cleanup job: computing it through this seam proves authority stops exactly at the stored timestamp without wall-clock flakiness or operator action. |

### identity.Verifier

| | |
|---|---|
| **Seam** | Live agent identity verification |
| **Interface** | `internal/identity/service.go::Verifier` (`Verify(ctx, opaqueToken) (Claims, error)`). |
| **Production wiring** | `main.go` constructs `identity.HTTPVerifier` from `AGENT_MANAGER_API_URL` and a bounded `http.Client`. Missing configuration or transport failure wires an unavailable verifier; automated spend still starts but fails closed. |
| **Test fake** | Identity and authorization tests inject fixed claims or a concrete verification error. HTTP contract tests substitute `httpc.Doer`. |
| **Why it exists** | Every proposal must ask `agent-manager` live. There is deliberately no cache or grace window: an unavailable, invalid, expired, malformed, or subject-less verification result cannot become spend authority. |

### authorization.EvidenceRecorder

| | |
|---|---|
| **Seam** | Append-only decision evidence |
| **Interface** | `internal/authorization/service.go::EvidenceRecorder` (`RecordDecision(ctx, DecisionEvidence)`). |
| **Production wiring** | `handlers/agentspend.Module` wires `evidence.Recorder` to the routed SQLite evidence repository. |
| **Test fake** | Authorization tests collect immutable decision values in memory and assert refusal/approval evidence independently from authorization persistence. |
| **Why it exists** | Identity failures must leave evidence without creating spend authority, while later evidence storage and replay evolution must not leak into policy evaluation. |

### operatorauth.Authorizer

| | |
|---|---|
| **Seam** | Fail-closed operator-realm authorization |
| **Interface** | `internal/operatorauth/service.go::Authorizer` (`Authorize(ctx, headers) (Identity, error)`). |
| **Production wiring** | `main.go` constructs a constant-time `StaticToken` from `TREASURY_OPERATOR_TOKEN`, falling back to `API_TOKEN`, and injects it into `handlers/treasuryadmin.Module`. Missing configuration wires `Unavailable`, so every admin method fails closed. |
| **Test fake** | Handler integration tests use a fixed `StaticToken` through a real generated Connect client/server. Domain tests may inject a narrow fake authorizer when they need a specific operator identity or failure. |
| **Why it exists** | The two-service descriptor makes policy mutation absent from the agent API; this seam separately ensures that possession of a valid agent token cannot cross into the mounted operator service. Mixed agent/operator headers are rejected to avoid ambient-authority confused-deputy requests. |

### approval.Relay

| | |
|---|---|
| **Seam** | Optional approval notification relay |
| **Interface** | `internal/approval/service.go::Relay` (`Relay(ctx, approvalRequest) error`). |
| **Production wiring** | `main.go` constructs `approval.NotificationRelay` from `NOTIFICATION_HUB_API_URL` and a bounded HTTP client. Missing configuration wires `UnavailableRelay`; local queue admission and resolution remain active and record the failed attempt. |
| **Test fake** | Approval tests use `UnavailableRelay` with a concrete connection cause; an attended integration test targets the actually stopped Notification Hub endpoint. |
| **Why it exists** | Delivery is an enhancement, not part of the human gate. The local record is created first, every delivery outcome is durable, and relay failure cannot change approval state or held-headroom resolution. |
...
### instrument.CredentialResolver

| | |
|---|---|
| **Seam** | Resolve an instrument credential reference at use time |
| **Interface** | `internal/instrument/service.go::CredentialResolver` (`Resolve(ctx, logicalIdentity, field)`). |
