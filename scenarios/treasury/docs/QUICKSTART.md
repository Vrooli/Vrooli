...
```bash
vrooli scenario port treasury UI_PORT
```
...
```bash
treasury status
treasury <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port treasury API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.treasury.v1.<domain>.<Service>/<Method>
```
...
```bash
treasury notes list
treasury notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port treasury API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.treasury.v1.notes.NotesService/ListNotes" \
