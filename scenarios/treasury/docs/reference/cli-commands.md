...
`~/.vrooli/bin/treasury`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start treasury` if the API is unreachable |
...
```bash
treasury notes list
treasury notes list --json
```

#### `treasury notes create --title <title> [--body <body>]`
...
```bash
treasury notes create --title "First note" --body "Hello world"
```
...
```bash
treasury notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start treasury`" is good.
