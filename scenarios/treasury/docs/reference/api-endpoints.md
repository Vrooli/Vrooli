...
envelope (`packages/proto/schemas/treasury/v1/shared/errors.proto`):
...
The proto type lives at `packages/proto/schemas/treasury/v1/shared/health.proto`
...
`POST /vrooli.treasury.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.treasury.v1.notes.NotesService/ListNotes`
...
| **CLI** | `treasury notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.treasury.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.treasury.v1.notes.NotesService/CreateNote`
...
| **CLI** | `treasury notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.treasury.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.treasury.v1.notes.NotesService/GetNote`
...
| **CLI** | `treasury notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.treasury.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `treasury notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/treasury/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/treasury/v1/<domain>/`, then run
   `make generate`.
