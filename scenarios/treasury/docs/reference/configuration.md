...
For `treasury` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/treasury/config.json`
3. `~/.vrooli/config/treasury/config.json`
4. `~/.config/vrooli/treasury/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port treasury API_PORT`)
