...
type HelpTreeOptions struct {
	// Origin is the value placed on every emitted Command. Typically the binary
	// name ("vrooli") for an external CLI or the scenario name for a scenario CLI.
	Origin string
...
// ParseHelpTree recursively walks a CLI's `--help` tree and emits one Command
// per leaf (a command whose --help reveals no further subcommands or whose
// depth hit MaxDepth). On any failure a single Source=help-failed stub is
// returned so the CLI is still identifiable by name.
func ParseHelpTree(ctx context.Context, run Runner, bin string, opts HelpTreeOptions) []Command {
	...
	if err != nil || strings.TrimSpace(string(rootOut)) == "" {
		return []Command{stubRecord(origin, err)}
	}
	...
	if len(records) == 0 {
		...
		return []Command{rootLeafRecord(origin, rootOut)}
	}
	return records
}
...
func walkHelpTree(ctx context.Context, run Runner, bin, origin string, parents []string, groupDesc string, helpOut []byte, depth, maxDepth int, seen map[string]struct{}) []Command {
	entries := parseHelpEntries(helpOut)
	if len(entries) == 0 {
		return []Command{leafRecord(origin, parents, groupDesc, helpOut)}
	}
	if depth >= maxDepth {
...
func stubRecord(origin string, err error) Command {
...
func rootLeafRecord(origin string, helpOut []byte) Command {
	return Command{
		Origin:      origin,
		Name:        origin,
		FullPath:    origin,
...
func leafRecord(origin string, parents []string, groupDesc string, helpOut []byte) Command {
...
func leafRecordFromEntry(origin string, parents []string, groupDesc string, e helpEntry) Command {
...
func newRecord(origin string, path []string) Command {
	rec := Command{
		Origin: origin,
		Source: SourceHelp,
	}
