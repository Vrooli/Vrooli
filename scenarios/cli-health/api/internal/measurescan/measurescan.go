// Package measurescan is the cli-health-internal join between a scenario's raw
// cli/manifest.json and the shared measures contract library
// (github.com/vrooli/measures-go). It extracts each command's curated `measure`
// block (which cliapp.ParseManifest deliberately drops), assembles it against
// the proto-derived param schema, and grades the adoption tier. Both the
// discovery indexer (internal/aisearch) and the static validator
// (internal/services/manifestvalidation) consume it.
//
...
// cli-health validates elsewhere (flags, positionals, descriptions) are omitted.
...
// repoRoot. MEASURES_DESCRIPTOR_PATH overrides it (useful when cli-health runs
// from an unusual CWD or against a relocated image).
