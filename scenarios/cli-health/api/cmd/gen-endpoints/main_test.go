	...
	endpoints := []module.EndpointDescriptor{
		{
			...
			CLIMapping: &module.CLIMapping{
				Command: "cli-health lonely subcommand",
			},
		},
	}
...
func TestCrossCheck_PassesWhenRegistered(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "x", CLIMapping: &module.CLIMapping{Command: "cli-health x"}},
		{ID: "y_alias", CLIMapping: &module.CLIMapping{Command: "cli-health notes ll"}},
...
func TestBuildCLICommands_ResolvesEndpointIDs(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "health", CLIMapping: &module.CLIMapping{Command: "cli-health status"}},
		{ID: "notes_list", CLIMapping: &module.CLIMapping{Command: "cli-health notes list"}},
	}
	registered := []registeredCommand{
		{Name: "configure"},
		{Name: "notes list"},
...
// normalisation step: the endpoint's "cli-health notes list"
...
func TestStripBinaryPrefix(t *testing.T) {
	cases := []struct {
		...
		{in: "cli-health status", want: "status"},
		{in: "cli-health notes list", want: "notes list"},
		{in: "already-stripped", want: "already-stripped"},
		{in: "cli-health", want: "cli-health"}, // no trailing space → preserved
	}
