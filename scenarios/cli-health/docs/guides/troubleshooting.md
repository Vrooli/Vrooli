...
```bash
vrooli scenario status cli-health
make stop
vrooli scenario start cli-health --clean-stale
```
...
cli-health status --auto-start

# Or override the API base for this invocation
cli-health status --api-base "http://localhost:$(vrooli scenario port cli-health API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/cli-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/cli-health.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/cli-health.db"
```
...
`packages/proto/gen/go/cli-health/v1/`, TypeScript under
`packages/proto/gen/typescript/cli-health/v1/`, and Python under
`packages/proto/gen/python/cli_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/cli-health/v1/<domain>/`.
