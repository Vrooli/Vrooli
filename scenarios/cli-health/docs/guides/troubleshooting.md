...
```bash
export SQLITE_PATH=/tmp/cli-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/cli-health.db}")"
```
...
`packages/proto/gen/typescript/js/cli-health/v1/`, and Python under
