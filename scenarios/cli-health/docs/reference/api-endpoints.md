...
envelope (`packages/proto/schemas/cli-health/v1/errors/errors.proto`):
...
| **CLI** | `cli-health status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/cli-health/v1/health/health.proto`
...
### `POST /vrooli.cli_health.v1.notes.NotesService/ListNotes`
...
| **CLI** | `cli-health notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.cli_health.v1.notes.NotesService/ListNotes" \
...
### `POST /vrooli.cli_health.v1.notes.NotesService/CreateNote`
...
| **CLI** | `cli-health notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.cli_health.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
### `POST /vrooli.cli_health.v1.notes.NotesService/GetNote`
...
| **CLI** | `cli-health notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.cli_health.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `cli-health notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/cli-health/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/cli-health/v1/<domain>/`, then run
   `make generate`.
