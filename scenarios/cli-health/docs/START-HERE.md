...
`vrooli scenario orient cli-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient cli-health
...
```bash
prd-control-tower prd generate cli-health \
...
```bash
prd-control-tower prd validate cli-health --json
```
...
```bash
prd-control-tower requirements generate cli-health \
...
```bash
prd-control-tower requirements validate cli-health --json
```
