...
`template-manager orient cli-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient cli-health
...
```bash
cat > /tmp/prd_context_cli-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) cli-health \
  --context-file /tmp/prd_context_cli-health.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate cli-health --json
```
...
```bash
cat > /tmp/requirements_context_cli-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) cli-health \
  --context-file /tmp/requirements_context_cli-health.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/cli-health/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest cli-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> cli-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario cli-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> cli-health <adopted-path>
```
