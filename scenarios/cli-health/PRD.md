...
- [ ] OT-P0-005 | Test-genie Contracts phase integration | `test-genie`'s new Contracts phase (inserted between Structure and Standards) invokes `cli-health validate` via CLI subprocess and surfaces findings into the report; error findings fail the phase and warning findings produce warnings.
