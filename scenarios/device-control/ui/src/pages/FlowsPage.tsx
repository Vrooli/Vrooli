...
export function FlowsPage() {
  ...
  const [run, setRun] = useState<{ disposition: string; chapters: Array<{ id: string; disposition: string; message: string }>; evidence: Array<{ id: string; kind: string; checksum?: string; size_bytes: number; redaction_verified: boolean; applied_rules?: string[]; recording_method?: string; effective_fps?: number }> }>();
  ...
  return (
    <section data-testid={selectors.pages.flows} className="flex flex-col gap-4" aria-labelledby="flows-heading">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card data-testid={selectors.pages.flowRunReview}><CardHeader><CardTitle>{t(strings.pages.flows.runReview)}</CardTitle></CardHeader><CardContent>{run ? <div className="flex flex-col gap-2"><p role="status">{run.disposition}</p>{run.chapters.map((chapter) => <p key={chapter.id} className="text-sm">{chapter.id}: {chapter.disposition} — {chapter.message}</p>)}<p className="font-medium">{t(strings.pages.flows.retainedEvidence, { count: run.evidence.length })}</p>{run.evidence.map((ref) => <div key={ref.id} className="rounded-md border p-2"><p className="text-sm"><span>{ref.id}</span> · <span>{ref.kind}</span> · <span>{ref.checksum ?? t(strings.pages.flows.checksumUnavailable)}</span> · <span>{ref.size_bytes}</span> {t(strings.pages.flows.bytes)} · {t(strings.pages.flows.redactionVerified, { verified: String(ref.redaction_verified) })} · {(ref.applied_rules ?? []).join(", ")}</p>{ref.kind === "image" && <img data-testid={selectors.pages.flowEvidenceImage} src={buildApiUrl(`/api/v1/evidence/${encodeURIComponent(ref.id)}`, { baseUrl: API_BASE })} alt={t(strings.pages.flows.retainedEvidenceAlt, { id: ref.id })} className="mt-2 max-h-80 w-auto rounded border" />}</div>)}</div> : <p className="text-app-muted-foreground">{t(strings.pages.flows.noRun)}</p>}</CardContent></Card>
      </div>
    </section>
  );
}
