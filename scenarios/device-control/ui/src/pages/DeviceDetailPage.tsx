...
function propertyLabel(name: string, domain?: string) { return domain ? `${name} (${domain})` : name; }
function MediaPanel({ device, state, refresh }: PanelProps) { const currentProperty = state.properties?.volume; const current = currentProperty?.value; const muted = state.properties?.muted?.value; const receiverOnly = currentProperty?.state_domain === "receiver_volume"; const conflict = state.conflicts?.volume; return <Card data-testid={selectors.pages.deviceMediaPanel}><CardHeader><CardTitle>Media transport</CardTitle></CardHeader><CardContent className="flex flex-wrap gap-2"><div data-testid={selectors.pages.deviceNowPlaying} className="basis-full">Now playing: {String(state.properties?.application?.value ?? "unknown")} · {String(state.properties?.player_state?.value ?? "unknown")}</div>{receiverOnly && <p className="basis-full text-sm">Receiver volume is reported; physical output volume is unavailable.</p>}{conflict && <p className="basis-full text-sm">Volume transports disagree: {conflict}</p>}{["play", "pause", "stop", "previous", "next"].map((action) => <Button key={action} onClick={() => mediaActuate(device, action).then(refresh)}>{action}</Button>)}<label className="flex items-center gap-2">{receiverOnly ? "Receiver volume" : "Volume"}<input type="range" min="0" max="1" step="0.01" aria-label={receiverOnly ? "Receiver volume" : "Volume"} value={typeof current === "number" ? current : 0} onChange={(event) => void propertyActuate(device, "volume", Number(event.currentTarget.value)).then(refresh)} /></label><label className="flex items-center gap-2">{receiverOnly ? "Receiver mute" : "Mute"}<input type="checkbox" aria-label={receiverOnly ? "Receiver mute" : "Mute"} checked={muted === true} onChange={(event) => void propertyActuate(device, "muted", event.currentTarget.checked).then(refresh)} /></label></CardContent></Card>; }
function PropertyPanel({ device, state, refresh }: PanelProps) { const values = state.properties ?? {}; const descriptors: PropertyDescriptor[] = device.properties ?? Object.keys(values).map((name) => ({ name, value_type: typeof values[name]?.value, writable: true })); return <Card data-testid={selectors.pages.devicePropertyPanel}><CardHeader><CardTitle>Properties</CardTitle></CardHeader><CardContent className="flex flex-col gap-2">{descriptors.map((descriptor) => { const property = values[descriptor.name]; const value = property?.value; const label = propertyLabel(descriptor.name, descriptor.state_domain || property?.state_domain); const change = (next: unknown) => void propertyActuate(device, descriptor.name, next).then(refresh); if (descriptor.enumeration?.length) return <label key={descriptor.name}>{label}<select aria-label={descriptor.name} defaultValue={String(value ?? "")} onChange={(event) => change(event.currentTarget.value)}>{descriptor.enumeration.map((option) => <option key={option}>{option}</option>)}</select></label>; if (descriptor.value_type === "boolean") return <label key={descriptor.name}>{label}<input aria-label={descriptor.name} type="checkbox" checked={value === true} disabled={!descriptor.writable} onChange={(event) => change(event.currentTarget.checked)} /></label>; if (descriptor.value_type === "number" && descriptor.minimum !== undefined && descriptor.maximum !== undefined) return <label key={descriptor.name}>{label}<input aria-label={descriptor.name} type="range" min={descriptor.minimum} max={descriptor.maximum} value={typeof value === "number" ? value : descriptor.minimum} disabled={!descriptor.writable} onChange={(event) => change(Number(event.currentTarget.value))} /></label>; return <label key={descriptor.name}>{label}<input aria-label={descriptor.name} type="text" defaultValue={String(value ?? "")} disabled={!descriptor.writable} onBlur={(event) => change(event.currentTarget.value)} /></label>; })}</CardContent></Card>; }
