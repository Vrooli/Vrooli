...
#### `POST /vrooli.device_control.v1.notes.NotesService/ListNotes`
...
| **CLI** | `device-control notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.device_control.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.device_control.v1.notes.NotesService/CreateNote`
...
| **CLI** | `device-control notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.device_control.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.device_control.v1.notes.NotesService/GetNote`
...
| **CLI** | `device-control notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.device_control.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `device-control notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/device-control/v1/notes/notes.proto`.
