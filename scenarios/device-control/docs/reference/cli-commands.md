...
`~/.vrooli/bin/device-control`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start device-control` if the API is unreachable |
...
```bash
device-control status
device-control status --json
```

### `device-control configure <key> <value>`
...
```bash
device-control configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`device-control <domain> <verb>`). Every command calls a single API
...
```bash
device-control notes list
device-control notes list --json
```

#### `device-control notes create --title <title> [--body <body>]`
...
```bash
device-control notes create --title "First note" --body "Hello world"
```
...
```bash
device-control notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start device-control`" is good.
