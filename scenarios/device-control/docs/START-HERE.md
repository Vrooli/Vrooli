...
`template-manager orient device-control`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient device-control
...
```bash
cat > /tmp/prd_context_device-control.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start device-control --interactive
# …answer the interview, then:
business-health wizard preview device-control
business-health wizard apply device-control
```
...
```bash
vrooli scenario requirements validate device-control --json
```
...
```bash
cat > /tmp/requirements_context_device-control.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate device-control --json
business-health fix preview device-control
business-health fix apply device-control
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate device-control --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate device-control`, the notes page spec
...
validate device-control --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/device-control/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest device-control <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> device-control
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario device-control
```
