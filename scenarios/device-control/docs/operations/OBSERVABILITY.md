...
- **Capability snapshot age per device.** A stale snapshot presented as
  current is the failure mode this scenario exists to prevent.
- **Lease contention rate and refusal count.** Rising refusals mean
  consumers are competing for a device; rising *grants without releases*
  means a leak.
- **Resolution rung distribution per flow.** A flow drifting from `semantic`
  toward `vision` is getting slower, costlier, and less deterministic — and
  usually means a UI changed underneath it.
- **Redaction verification failures.** Should be zero. Any non-zero value is
  a security event, not a quality metric.
- **Unavailable-with-no-next-action count.** Should be zero by contract.
