...
## Scenario stance — deferred, with a named dependency

Device Control is **infrastructure, not a product surface**. It is not
independently monetized. Its commercial contribution is indirect but load
bearing: it is what lets `scenario-to-ios` and `scenario-to-android` produce
physical-device evidence, and those ramps are on the monetization path.

Revisit only if device control is ever exposed as a standalone capability to
someone other than the owner — at which point the security questions in
`../internal/SECURITY.md` become commercial questions too.
