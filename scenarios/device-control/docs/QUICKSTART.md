...
device-control <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port device-control API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.device_control.v1.<domain>.<Service>/<Method>
```
...
```bash
device-control notes list
device-control notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port device-control API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.device_control.v1.notes.NotesService/ListNotes" \
