...
```bash
vrooli scenario status device-control
make stop
vrooli scenario start device-control --clean-stale
```
...
```bash
make status
vrooli scenario port device-control UI_PORT
```
...
device-control status --auto-start

# Or override the API base for this invocation
device-control status --api-base "http://localhost:$(vrooli scenario port device-control API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/device-control/`.
...
The default route is `${SCENARIO_DATA_DIR}/device-control.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/device-control.db"
```
...
`packages/proto/gen/go/device-control/v1/`, TypeScript under
`packages/proto/gen/typescript/device-control/v1/`, and Python under
`packages/proto/gen/python/device_control/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/device-control/v1/<domain>/`.
