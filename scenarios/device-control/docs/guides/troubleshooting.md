...
```bash
export SQLITE_PATH=/tmp/device-control.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/device-control.db}")"
```
...
`packages/proto/gen/typescript/js/device-control/v1/`, and Python under
