...
|---|---|---|---|---|
| **Strategy** | `api/strategy/contract.go::Strategy` — `ID()`, `Describe()`; optional `Observer`, `InputActuator`, `StateReader`, `StateObserver`, `Pairer`, and other modalities | one fake per capability profile, so the executor can be tested with screenless, property-only, and full-tier transports without a device | The extension point. Every control mechanism is an implementation; adding one must require no engine change. | `DVC-P0-001`, `DVC-P0-002` |
...
| **Recorder** | strategy recording interfaces plus `internal/evidence` — captures carry `ref`, `method`, and `effectiveFPS` | native fake and synthesized fake | Preserves the native/synthesized distinction for frame-bearing transports; screenless transports declare recording unavailable. | `DVC-P0-012` |
