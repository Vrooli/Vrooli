...
## Scenario adaptation — Device Control

The Operational Console language fits this scenario without modification, and
the token contract is kept intact.

**Why it fits.** The primary user is an operator answering three dense
questions at a glance: which devices do I control, what can each one *actually*
do right now, and what is running on them. That is a fleet console, not a
content surface — it wants tabular density, tight vertical rhythm, and status
encoded in form as well as colour. The kit's `app-success` / `app-warning` /
`app-danger` / `app-info` roles map directly onto this scenario's disposition
vocabulary, which is the single most repeated element in the UI:

| Disposition | Token role | Meaning |
|---|---|---|
| available / passed | `app-success` | Probed and proven. |
| unavailable | `app-warning` | Missing right now; names the prerequisite and next action. |
| unsupported | `app-muted-foreground` | Categorically impossible here. Terminal, not degraded. |
| failed | `app-danger` | Attempted and did not hold. |
...
1. *A live session must be unmistakable.* This scenario can drive a personal
   ...
   and a one-click kill are persistently visible — not nested behind a detail
   view. This is the one place where the calm/dense default yields to
   deliberate prominence.
2. *The video frame is evidence, not the interface.* A live device view shows
   a frame, but every control and every piece of state must be operable and
   perceivable without it. A screen reader user, or a user whose frame has not
   loaded, must still be able to read device state, see the active step, and
   kill the session. Accessibility floors therefore apply to the surrounding
   chrome with no exemption for the media region, which is labelled and
   described rather than treated as the source of truth.
