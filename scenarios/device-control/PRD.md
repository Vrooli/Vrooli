...
- [ ] OT-P0-001 | Capability floor and probed strategy contract | Every strategy implements exactly three required operations — observe() returning a screen frame with size, scale and timestamp; actuate() accepting pointer and key events; describe() returning a capability declaration. Every richer capability (semantic tree, app lifecycle, permissions, network control, orientation, clipboard, file transfer, video recording, device logs, WebView attach) is optional and declared. Declarations are verified by probing at inventory time; an unprovable capability reports unavailable with the exact missing prerequisite and a next action, never silently present.
- [ ] OT-P0-002 | Strategy conformance suite | `device-control strategy verify <id>` runs a fixed suite against any registered strategy and reports which capability tiers it satisfies and therefore which flow step kinds it can execute. Adding a strategy means implementing the floor and passing the suite — no engine changes.
- [ ] OT-P0-003 | Bridge-backed device inventory and reachability | Devices are enumerated from the vrooli-bridge fleet, which owns identity, pairing, trust, scopes and audit; this scenario owns device-specific probing and health. An attached device whose host node is offline reports unreachable with that exact reason rather than disappearing.
- [ ] OT-P0-004 | Session leases and single-session safety | No verb executes without a held lease. Concurrent claims on the same device are refused rather than interleaved, because several strategies are physically single-session. Leases expire, are visible, and are releasable.
- [ ] OT-P0-005 | Flow schema, bounded waits, and pre-execution capability gap report | One flow envelope with composable step vocabularies: device verbs, AI-assisted steps, control flow, bounded readiness and settle policies, and delegation to a browser-automation-studio flow. Steps declare required capabilities so a flow is checked against a strategy before execution and yields a capability gap report instead of a mid-run failure. Waits are named bounded policies with upper bounds; an exceeded bound is an evidence-visible failure, never a longer sleep.
- [ ] OT-P0-006 | Target resolution ladder | A step names its target by intent. The resolver satisfies it at the highest rung the strategy supports: semantic match against an accessibility or view tree; visual-anchor match against a captured reference; or vision through ai-gateway. The chosen rung and its confidence are recorded in evidence, so a reviewer can see whether a result was deterministic or inferred.
- [ ] OT-P0-007 | All inference routed through ai-gateway | Every AI-assisted step routes through ai-gateway. No direct provider HTTP client, no concrete model slug, and no provider secret exists in this scenario. Visual understanding requires an ai-gateway request kind that does not yet exist; this scenario declares that dependency rather than working around it.
- [ ] OT-P0-008 | Evidence references with verified redaction | Captures are stored as references carrying checksum, size and creation time, conforming to common/v1 EvidenceRef. Screen frames can contain secrets, so redaction status is verified before any capture leaves the producer, and raw bytes are never handed to a consumer.
- [ ] OT-P0-009 | Audit trail and immediate session kill | Every verb dispatched to a device is audited with actor, device, lease and outcome. A live session is visible in the operator surface and can be killed immediately from CLI or UI.
- [ ] OT-P0-010 | CLI as the complete control surface | Every capability is reachable from the CLI with report-shaped JSON output, because the CLI is what the agent skill drives. A capability reachable only from the API or UI is treated as incomplete.
- [ ] OT-P0-011 | android-adb strategy | First full-tier strategy, serving emulator and physical Android devices identically through one adapter: semantic view hierarchy, app lifecycle, permission control, input, screenshot and screen recording. Proves the floor, the ladder and the conformance suite against a real device class.
- [ ] OT-P0-012 | Recording on every strategy, native or synthesized | Video evidence is a guaranteed capability, not an optional one. A strategy that declares native recording (adb screenrecord, simctl recordVideo) uses it. A strategy that does not gets a recording synthesized from its floor-level observe() frame stream, which every strategy can produce by definition. Evidence records which path produced the video and its effective frame rate, so a reviewer can tell a native capture from a synthesized one and never mistakes a low-rate reconstruction for a full-rate capture. This is what lets the delivery ramps claim video evidence on every device kind rather than only the well-instrumented ones.
...
- [ ] OT-P1-004 | host-desktop strategy | Drive the local machine as a device, reusing existing headless display and input tooling so desktop automation composes with the same flow vocabulary.
- [ ] OT-P1-005 | Agent mode via agent-manager and a prompt-manager skill | `device-control agent run --device <id> --goal "<goal>"` spawns an agent-manager run driven by a prompt-manager skill that teaches the CLI. Runs are bounded by step count, cost ceiling and lease scope, fully audited, and abortable.
- [ ] OT-P1-006 | Promote an agent run to a deterministic flow | Every agent action is recorded as a flow step, so a successful goal-directed run exports as a deterministic flow that replays without inference cost. This is the compounding loop: infer once, replay free.
...
- [ ] OT-P1-008 | Operator UI | Live device view with session controls, flow authoring and validation, run review with chaptered evidence, and a strategy conformance matrix showing what each device can actually prove right now.
...
1. *ai-gateway has no visual-understanding request kind.* Its request kinds are text generation, text embedding, structured extraction, image generation and video generation — all text-in. Vision-assisted steps are blocked until that capability exists. Compounding this, browser-automation-studio's existing vision agent bypasses ai-gateway entirely by calling OpenRouter, Anthropic and Ollama directly, which is exactly the coupling ai-gateway's conformance phase exists to flag. This scenario must not repeat that shortcut, so the gateway capability is a hard prerequisite for the AI step kinds.
