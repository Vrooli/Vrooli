...
- **Purpose**: Device Control is the permanent capability for driving owner-trusted devices — physical phones, Android TV/Google TV devices, emulators and simulators, and host machines — through pluggable control strategies. It separates *what you want done on a device* from *how a particular device can be driven*, so one automation flow can run across an Android phone or TV over ADB, an iOS simulator over simctl, an iPhone over accessibility-grade XCUITest, an iPhone over pixel-grade mirroring, or the local desktop. It also provides the governed application boundary: discover installed packages, verify an application artifact, install it, inspect its state, launch it, and control it under a lease. Vision-based screen understanding and control are built once against a minimal capability floor, so they work on every strategy — including strategies that expose no semantic accessibility tree at all.
...
- [x] OT-P0-001 | Capability floor and probed strategy contract | Every strategy implements identity and describe() and may add optional modalities — observe/frames, input actuation, media, properties, sensors, logs, semantic trees, app lifecycle, permissions, network control, orientation, clipboard, file transfer, recording, and WebView attach. Each capability is probed independently at inventory time; a transport failure, malformed response, unsupported operation, or missing prerequisite must not erase unrelated capabilities or leave a stale success claim. An unprovable capability reports unavailable with the exact reason and next action, never silently present. Screenless devices are valid strategies and do not claim observe or screenshot.
...
- [ ] OT-P0-010 | CLI as the complete control surface | Every capability is reachable from the CLI with report-shaped JSON output, because the CLI is what the agent skill drives. This includes discovery/onboarding, transport diagnostics, lease-owned actuation and state, strategy verification, application inventory and inspection, artifact installation, package state, launch/focus/stop/uninstall/clear-data, and evidence/audit review. A capability reachable only from the API or UI is treated as incomplete.
- [ ] OT-P0-011 | android-adb strategy | The first full-tier strategy serves Android emulators, physical phones, Android TV devices, and TV boxes through one adapter, while preserving target-specific transport profiles. It supports governed USB and network ADB onboarding, artifact install, package state and launch, semantic view hierarchy, permission control, input, screenshot, logs, and recording wherever the live target proves them. The strategy distinguishes classic authorized TCP ADB from Android Wireless Debugging/TLS and never treats an ambient serial or stale endpoint as authority. It proves the floor, the ladder, and the conformance suite against real phone and TV device classes.
...
- [ ] OT-P1-005 | Agent mode via agent-manager and a prompt-manager skill | `device-control agent run --device <id> --goal "<goal>"` uses the prompt-manager device-control skill and routes any inference through ai-gateway. Runs build a typed capability/state world model, are bounded by lease scope, fully audited, abortable, and remain valid when the target has no frame modality. Application goals are package-scoped, use verified app context, and stop for human confirmation when an install, permission, credential, destructive, or ambiguous action requires owner authority.
...
- [ ] OT-P1-008 | Operator UI | Live device view with session controls, flow authoring and validation, run review with chaptered evidence, capability-composed television/phone/property panels, LAN discovery and pairing, application inventory and launch/install status, app-scoped agent context, and a strategy conformance matrix showing what each device can actually prove right now.
...
4. *Android TV transport ambiguity.* A TV may expose Cast, Android TV Remote, classic TCP ADB, and Wireless Debugging as separate transports. Device Control must keep transport identities and capability evidence separate until an owner-asserted merge, while allowing one logical device to select the transport that can perform a requested operation.
5. *Application-control overreach.* Installing or launching arbitrary packages is a privileged operation. Artifact provenance, package identity, confirmation, lease ownership, audit, and post-action verification are required; a raw ADB command is not a product contract.
