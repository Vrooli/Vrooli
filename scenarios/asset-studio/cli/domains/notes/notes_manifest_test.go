...
// the template itself does not build because of asset-studio placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_asset_studio_v1_notes_notes_proto, "NotesService")
}
