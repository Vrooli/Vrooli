...
- Operational risks: Identity conformance is the central promise and it is unvalidated — nobody has yet rendered the same character twice through this pipeline, so reference-image comparison may prove too weak to catch the drift that matters or too strict to pass anything; generation spend is unbounded in a way editorial work is not, and a mis-specified multi-frame video spec can cost real money before any human sees a frame, which is why cost accounting is P0 rather than an operational nicety; the marketing rich-media catalogue has never been validated against a schema, so import will surface shape problems that currently look like working files; video model interfaces are moving quickly and a payload that is compatible today may not be in six months, which is the reason every call routes through ai-gateway rather than a vendor SDK; and no persona account exists on any platform, so the artifacts this scenario produces have nowhere to be published until a `channel-strategy-update` decision activates one.
- Launch sequencing: One image post type activated end to end first — a single-identity still image, composed from an imported character, rendered through ai-gateway, conformance-confirmed, released, and referenced by a `content-desk` draft. That narrow slice proves the whole spine and is deliberately chosen over building the general pipeline, because a render pipeline with no released artifact cannot validate its own conformance model. Then identity registry hardening and canon import. Then provenance and cost. Then multi-frame binding. Then video. Then capture and compositing. Persona accounts, the disclosure protocol, and the paired post-type skills that make image and video types active are outside this scenario and are the real gate on its output being publishable.
