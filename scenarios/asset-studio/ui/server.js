...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start asset-studio` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start asset-studio` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.asset_studio\.v1\./

startScenarioServer({
  uiPort,
  apiPort,
  distDir: './dist',
  serviceName: 'asset-studio',
  corsOrigins: '*',
  setupRoutes(app) {
