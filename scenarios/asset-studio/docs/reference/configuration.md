...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/asset-studio.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `asset-studio` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/asset-studio/config.json`
3. `~/.vrooli/config/asset-studio/config.json`
4. `~/.config/vrooli/asset-studio/config.json`
...
```bash
asset-studio configure api_base http://localhost:15001/api/v1
asset-studio configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port asset-studio API_PORT`)
