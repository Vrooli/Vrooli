...
envelope (`packages/proto/schemas/asset-studio/v1/shared/errors.proto`):
...
| **CLI** | `asset-studio status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/asset-studio/v1/shared/health.proto`
...
`POST /vrooli.asset_studio.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.asset_studio.v1.notes.NotesService/ListNotes`
...
| **CLI** | `asset-studio notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.asset_studio.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.asset_studio.v1.notes.NotesService/CreateNote`
...
| **CLI** | `asset-studio notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.asset_studio.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.asset_studio.v1.notes.NotesService/GetNote`
...
| **CLI** | `asset-studio notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.asset_studio.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `asset-studio notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/asset-studio/v1/notes/notes.proto`.
