...
`~/.vrooli/bin/asset-studio`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start asset-studio` if the API is unreachable |
...
```bash
asset-studio status
asset-studio status --json
```

### `asset-studio configure <key> <value>`
...
```bash
asset-studio configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`asset-studio <domain> <verb>`). Every command calls a single API
...
```bash
asset-studio notes list
asset-studio notes list --json
```

#### `asset-studio notes create --title <title> [--body <body>]`
...
```bash
asset-studio notes create --title "First note" --body "Hello world"
```
...
```bash
asset-studio notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start asset-studio`" is good.
