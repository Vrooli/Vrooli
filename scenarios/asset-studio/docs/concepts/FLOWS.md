...
| assets / release | `produced → in_review → released` (terminal), and `produced → discarded` (terminal). A job's candidates all start in `produced`; selection promotes one to `in_review` and discards the rest (D-018). | `produced → released` skipping review; releasing an unselected candidate; releasing a `discarded` candidate; release with any unresolved or failing frame verdict; release with non-empty credential claims, missing alt text, or missing disclosure state; any transition out of `released`. | Gate tests per blocking condition, each asserting the specific typed cause (`ASSET-P0-010`, `ASSET-P0-012`, `ASSET-P0-013`, `ASSET-P0-017`). |
| assets / refinement | A refined artifact enters at `produced` with its own unresolved verdict, linked to its parent (D-019). | Inheriting or copying the parent's conformance verdict; releasing a refined artifact on the strength of the parent's review; a refinement whose provenance omits the parent or the operation. | Verdict-isolation test between parent and derived artifact (`ASSET-P1-013`). |
