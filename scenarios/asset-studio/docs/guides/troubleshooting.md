...
```bash
vrooli scenario status asset-studio
make stop
vrooli scenario start asset-studio --clean-stale
```
...
asset-studio status --auto-start

# Or override the API base for this invocation
asset-studio status --api-base "http://localhost:$(vrooli scenario port asset-studio API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/asset-studio/`.
...
The default route is `${SCENARIO_DATA_DIR}/asset-studio.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/asset-studio.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/asset-studio.db}")"
```
...
The generated package paths follow `packages/proto/gen/go/asset-studio/v1/<domain>/`.
