...
`template-manager orient asset-studio`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient asset-studio
...
```bash
cat > /tmp/prd_context_asset-studio.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start asset-studio --interactive
# …answer the interview, then:
business-health wizard preview asset-studio
business-health wizard apply asset-studio
```
...
```bash
vrooli scenario requirements validate asset-studio --json
```
...
```bash
cat > /tmp/requirements_context_asset-studio.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate asset-studio --json
business-health fix preview asset-studio
business-health fix apply asset-studio
```
...
react-component-library adoptions resolve-path <component-id> asset-studio
react-component-library adoptions apply <component-id> asset-studio <adopted-path>
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate asset-studio --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate asset-studio`, the notes page spec
...
validate asset-studio --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/asset-studio/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest asset-studio <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> asset-studio
...
1. Add proto messages and a service under
   `packages/proto/schemas/asset-studio/v1/<domain>/`, then run
