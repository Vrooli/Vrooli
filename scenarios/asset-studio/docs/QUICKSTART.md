...
```bash
vrooli scenario port asset-studio UI_PORT
```
...
```bash
asset-studio status
asset-studio <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port asset-studio API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.asset_studio.v1.<domain>.<Service>/<Method>
```
...
```bash
asset-studio notes list
asset-studio notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port asset-studio API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.asset_studio.v1.notes.NotesService/ListNotes" \
