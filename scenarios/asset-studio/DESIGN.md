...
**Why this design language fits Asset Studio.** This is a tool for producing
marketing imagery, and the reflex is to make it look like the work it makes —
bold, visual, expressive. That would be a mistake here. The operator's actual
job on this surface is *comparison and judgement*: does this frame match the
reference sheet, what did this job cost, which identity version produced it.
Every one of those decisions is harmed by a surface that competes with the
content. A calm slate chrome makes the rendered frame the only saturated thing
on screen, which is exactly right when the frame is the evidence.

The density argument is the same one that produced this design language for
`swarm-manager` and Git Control Tower. An identity library, a render queue, and
a provenance chain are lists of records with state, and they are scanned rather
than read. Compact rows, semantic status colour paired with text, monospace for
identifiers, seeds, model names, and costs — that is the console vocabulary, and
the alternative is a gallery layout that shows six items where twenty are needed.

Two scenario-specific applications of the accessibility floor, both binding.
**Colour can never be the only carrier of conformance state**, because a
red-tinted "failed" chip beside a colour-rich rendered image is precisely where
a colour-only signal disappears; verdicts carry text and shape. And **alt text
is a required field rather than an optional one** — this scenario produces
images that other systems will publish, so alt text authored here is the only
place it can be authored honestly.

The one deliberate stretch: conformance review needs a genuinely large side-by-side
of frame and reference. That is a dense surface making room for one big
comparison, not a decorative hero, and it stays within the token contract.
