...
Scenario dependencies: search-hub (provider registration and cross-scenario URL resolution; the `SearchHit.measure` carrier shipped in Phase 3 is a prerequisite); test-genie (producer phase integration for the `measures` finding source); swarm-manager (ladder dimension consumer of `FINDING_SOURCE_MEASURES` results). swarm-manager is the first real adopter scenario (Phase 6) but is not a hard runtime dependency of measures-health itself.
