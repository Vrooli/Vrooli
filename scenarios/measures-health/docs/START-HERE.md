...
`template-manager orient measures-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient measures-health
...
```bash
cat > /tmp/prd_context_measures-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
cat > /tmp/requirements_context_measures-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
- [ ] **Start in proto.** Author `packages/proto/schemas/measures-health/v1/<domain>/<domain>.proto`
...
- [x] Delete `api/internal/notes`, `api/handlers/notes`,
      ...
      (Also: the top-level `packages/proto/schemas/measures-health/v1/notes`
      source proto + its `gen/{go,typescript,python}` outputs.)
- [x] Remove `notes` imports, module registrations, schema entries, CLI
      registration, and `<NotesCard />` render. (Plus the `notes` group in
      `cli/manifest.json`.)
...
- [x] Verify no product residue remains with focused searches for
      ...
      `.vrooli/`, and the scenario's proto schema directory. (Clean; the
      only remaining `Note` tokens are the legitimate waiver/tier `Note`
      vocabulary in `internal/validation`.)
- [x] Run `make test`.

**Exit criteria:** only health plus real scenario domains remain.
**Done 2026-06-08:** domains are now `health`, `validation`, `search`.
Deeper internal docs (`SEAMS.md`, `ARCHITECTURE.md`, reference pages)
still carry the template's notes-based worked examples — tracked as drift
in `docs/internal/PROBLEMS.md`, matching the shipped `security-health`
reference (Gate 7 scope is product code, not docs).
...
```bash
react-component-library components ingest measures-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> measures-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario measures-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> measures-health <adopted-path>
```
