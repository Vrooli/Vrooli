...
`packages/proto/schemas/measures-health/v1/<domain>/<file>.proto` —
...
The `health` proto in `packages/proto/schemas/measures-health/v1/health/`
...
### validation domain seams (coverage + probe)

The `validation` domain (`api/internal/validation/`) keeps its pure heart
(`Classify` — gathered inputs → coverage report + findings) free of I/O and
puts every filesystem/network read behind a seam, wired once in
`api/handlers/validation/module.go` via `NewFilesystemValidator`.
...
### index domain seams (the federated measures provider)

The `index` domain (`api/internal/measureindex/`) owns the central measures
corpus + the `measures-go` Engine wiring behind `SearchService`. The match →
resolve → gate → execute *brain* lives in `measures-go`; this domain supplies the
corpus and the three seams the Engine needs, wired once in
`api/handlers/search/module.go`.
