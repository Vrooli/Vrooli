# Troubleshooting — Measures Health
...
```bash
vrooli scenario status measures-health
make stop
vrooli scenario start measures-health --clean-stale
```
...
```bash
make status
vrooli scenario port measures-health UI_PORT
```
...
measures-health status --auto-start

# Or override the API base for this invocation
measures-health status --api-base "http://localhost:$(vrooli scenario port measures-health API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/measures-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/measures-health.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/measures-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/measures-health.db}")"
```
...
`packages/proto/gen/go/measures-health/v1/`, TypeScript under
`packages/proto/gen/typescript/js/measures-health/v1/`, and Python under
`packages/proto/gen/python/measures_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/measures-health/v1/<domain>/`.
