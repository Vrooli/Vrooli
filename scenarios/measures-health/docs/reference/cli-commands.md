...
`~/.vrooli/bin/measures-health`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start measures-health` if the API is unreachable |
...
```bash
measures-health status
measures-health status --json
```

### `measures-health configure <key> <value>`
...
```bash
measures-health configure api_base
```
...
```bash
measures-health notes list
measures-health notes list --json
```

### `measures-health notes create --title <title> [--body <body>]`
...
```bash
measures-health notes create --title "First note" --body "Hello world"
```
...
```bash
measures-health notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start measures-health`" is good.
