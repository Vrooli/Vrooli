...
envelope (`packages/proto/schemas/measures-health/v1/errors/errors.proto`):
...
| **CLI** | `measures-health status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/measures-health/v1/health/health.proto`
...
### `POST /vrooli.measures_health.v1.notes.NotesService/ListNotes`
...
| **CLI** | `measures-health notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.measures_health.v1.notes.NotesService/ListNotes" \
...
### `POST /vrooli.measures_health.v1.notes.NotesService/CreateNote`
...
| **CLI** | `measures-health notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.measures_health.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
### `POST /vrooli.measures_health.v1.notes.NotesService/GetNote`
...
| **CLI** | `measures-health notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.measures_health.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `measures-health notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/measures-health/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/measures-health/v1/<domain>/`, then run
   `make generate`.
