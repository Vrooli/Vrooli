...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/measures-health.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `measures-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/measures-health/config.json`
3. `~/.vrooli/config/measures-health/config.json`
4. `~/.config/vrooli/measures-health/config.json`
...
```bash
measures-health configure api_base http://localhost:15001/api/v1
measures-health configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port measures-health API_PORT`)
