...
// Endpoints slice.
//
// Deliberately loose on count (not "exactly N"): scenarios add and
// remove domains over time, so pinning the count would couple this test
...
// domain's own *_test.go (see handlers/health for the canonical
// schema-provider coverage).
...
//
// Lifted from a per-domain test (formerly each Connect domain's
// module_test.go) so the safety net applies
