// Package measurescan is the join between a scenario's raw cli/manifest.json and
// the shared measures contract library (github.com/vrooli/measures-go). It
// extracts each command's curated `measure` block (which cliapp.ParseManifest
// deliberately drops), assembles it against the proto-derived param schema, and
// grades the adoption tier. The measures-health validator + central index both
// consume it.
//
...
//
// NOTE (DECISIONS.md): this is intentionally a faithful sibling of
// cli-health/internal/measurescan. Both are candidates to be promoted into a
// shared packages/measures-go sub-package in a focused follow-up; that
// migration is out of this phase's scope because it would rewire cli-health.
// The logic here is kept byte-compatible with cli-health's so the eventual
// promotion is a mechanical de-duplication, not a reconciliation.
package measurescan
...
// the escape hatch for misclassified domains.
...
// rawManifest is the slice of cli/manifest.json measurescan needs. Fields
...
func Parse(raw []byte) (*ManifestMeasures, error) {
	...
	if err := json.Unmarshal(raw, &m); err != nil {
		return nil, fmt.Errorf("measurescan: parse manifest: %w", err)
	}
...
// legal canonical-convention upgrade.
...
// crashing construction or harvesting.
...
func (d *DescriptorSchemaReader) RequestParams(service, method string) ([]measures.ParamSchema, error) {
	r, err := d.load()
	if err != nil {
		return nil, fmt.Errorf("measurescan: descriptor image unavailable (%s): %w", d.path, err)
	}
