	...
	endpoints := []module.EndpointDescriptor{
		{
			...
			CLIMapping: &module.CLIMapping{
				Command: "measures-health lonely subcommand",
			},
		},
	}
...
func TestCrossCheck_PassesWhenRegistered(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "x", CLIMapping: &module.CLIMapping{Command: "measures-health x"}},
		{ID: "y_alias", CLIMapping: &module.CLIMapping{Command: "measures-health notes ll"}},
...
func TestBuildCLICommands_ResolvesEndpointIDs(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "health", CLIMapping: &module.CLIMapping{Command: "measures-health status"}},
		{ID: "notes_list", CLIMapping: &module.CLIMapping{Command: "measures-health notes list"}},
	}
	registered := []registeredCommand{
		{Name: "configure"},
		{Name: "notes list"},
...
// normalisation step: the endpoint's "measures-health notes list"
...
func TestStripBinaryPrefix(t *testing.T) {
	cases := []struct {
		...
		{in: "measures-health status", want: "status"},
		{in: "measures-health notes list", want: "notes list"},
		{in: "already-stripped", want: "already-stripped"},
		{in: "measures-health", want: "measures-health"}, // no trailing space → preserved
	}
