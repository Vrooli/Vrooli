#!/bin/bash
# Shared phased test orchestrator for chore-tracking
set -euo pipefail
...
# Initialise shared runner with managed runtime so integration-style phases
# can assume the scenario is available without manual prep.
testing::runner::init \
  --scenario-name "chore-tracking" \
...
# Register standard phase lineup
...
testing::runner::register_phase --name business --script "$PHASES_DIR/test-business.sh" --timeout 150 --requires-runtime true

testing::runner::register_phase --name performance --script "$PHASES_DIR/test-performance.sh" --timeout 180 --requires-runtime true

# Quick presets for local workflows
