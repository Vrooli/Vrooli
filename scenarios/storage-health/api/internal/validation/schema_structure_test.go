...
// -----------------------------------------------------------------------------
// Dogfood: storage-health itself must be clean across every Tier-1 analyzer.
// -----------------------------------------------------------------------------

func TestSchemaAnalyzers_DogfoodStorageHealthClean(t *testing.T) {
	// Resolve the real storage-health scenario dir from this test's location:
	// .../scenarios/storage-health/api/internal/validation → up 3 to the scenario.
	...
	for _, a := range analyzers {
		...
		if len(got) != 0 {
			t.Errorf("storage-health is not clean for %s: %+v", a.Name(), got)
		}
	}
}
