...
`~/.vrooli/bin/storage-health`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start storage-health` if the API is unreachable |
...
```bash
storage-health status
storage-health status --json
```

### `storage-health configure <key> <value>`
...
```bash
storage-health configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`storage-health <domain> <verb>`). Every command calls a single API
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start storage-health`" is good.
