...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/storage-health.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `storage-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/storage-health/config.json`
3. `~/.vrooli/config/storage-health/config.json`
4. `~/.config/vrooli/storage-health/config.json`
...
```bash
storage-health configure api_base http://localhost:15001/api/v1
storage-health configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port storage-health API_PORT`)
