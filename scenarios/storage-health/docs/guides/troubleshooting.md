# Troubleshooting — Storage Health
...
```bash
vrooli scenario status storage-health
# Then either:
make stop
# Or, as last resort:
pkill -f 'storage-health-api'
...
storage-health status --auto-start

# Or override the API base for this invocation
storage-health status --api-base "http://localhost:$(vrooli scenario port storage-health API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/storage-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/storage-health.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/storage-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/storage-health.db}")"
```
...
`packages/proto/gen/go/storage-health/v1/`, TypeScript under
`packages/proto/gen/typescript/js/storage-health/v1/`, and Python under
`packages/proto/gen/python/storage_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/storage-health/v1/<domain>/`.
