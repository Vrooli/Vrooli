...
| **file** | Filesystem data dirs. | Resolved through `storage.NewResolver` / `EnsureAllDirs`; the filesystem *portability* contract is deferred to a future cross-platform-readiness scenario. |
...
`STORAGE_NAMESPACE_HARDCODED` (L2). The four relational-isolation seams
(`database.Open → *RoutedDB`, `EnsureSchemas`, `TestModeMiddleware`,
`devrouting.Register`) are documented in full in
...
| **L2** | Isolation-safe | **The safety rung.** Test-isolation is statically proven (four routed seams wired) and namespaces are variant-aware (shadow-safe). A fail-closed precondition for destructive test-genie playbooks. | `ROUTED_SEAMS_UNWIRED` (`safety_blocker`), `STORAGE_NAMESPACE_HARDCODED`, `STORAGE_ISOLATION_UNVERIFIED` |
