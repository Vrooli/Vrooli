...
`template-manager orient storage-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient storage-health
...
been removed via `template-manager detemplate`; build the real storage
...
```bash
business-health wizard start  # (was: prd generate) storage-health \
...
```bash
vrooli scenario requirements validate storage-health --json
```
...
```bash
business-health wizard apply  # (was: requirements generate) storage-health \
