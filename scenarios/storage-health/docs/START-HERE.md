...
`vrooli scenario orient storage-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient storage-health
--finalize` to remove only that temporary orientation metadata.

Before writing code, read the two durable concept docs that define what
...
durable infrastructure below. The example `notes` domain has already
been removed via `vrooli scenario detemplate`; build the real storage
validation/isolation/fleet domains in its place. In particular:
...
```bash
cat > /tmp/prd_context_storage-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
prd-control-tower prd generate storage-health \
  --context-file /tmp/prd_context_storage-health.md \
  --publish \
  --json
```
...
```bash
prd-control-tower prd validate storage-health --json
```
...
```bash
cat > /tmp/requirements_context_storage-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
prd-control-tower requirements generate storage-health \
  --context-file /tmp/requirements_context_storage-health.md \
  --json
```
...
```bash
prd-control-tower requirements validate storage-health --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/storage-health/v1/<domain>/<domain>.proto`
...
- Wire contracts belong in
  `packages/proto/schemas/storage-health/v1/<domain>/`.
...
1. Add proto messages and a service under
   `packages/proto/schemas/storage-health/v1/<domain>/`, then run
