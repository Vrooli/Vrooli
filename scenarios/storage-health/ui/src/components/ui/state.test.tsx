...
describe("state primitives (cimode — copy-independent)", () => {
  describe("LoadingState", () => {
    ...
    it("renders a bespoke skeleton tree when provided", () => {
      renderWithProviders(
        <LoadingState label="custom-label" skeleton={<Skeleton className="h-4" />} />,
      );
      const block = screen.getByTestId(selectors.state.loading);
    ...
    it("renders an optional title above the spinner", () => {
      renderWithProviders(<LoadingState title={strings.common.loading} />);
      // Title + default label both resolve to the same cimode key here; the
      // spinner block still renders with its status role.
      expect(screen.getByTestId(selectors.state.loading)).toHaveAttribute("role", "status");
    });
  });
  ...
  describe("EmptyState", () => {
    ...
    it("renders a click action and fires onAction", () => {
      ...
      renderWithProviders(<EmptyState message="empty" actionLabel="Go" onAction={onAction} />);
    ...
    it("prefers a custom action slot over the click action", () => {
      renderWithProviders(
        <EmptyState
          ...
          onAction={vi.fn()}
          actionSlot={
            <a href="/x" data-testid="custom-slot">
              link-action
            </a>
          }
        />,
