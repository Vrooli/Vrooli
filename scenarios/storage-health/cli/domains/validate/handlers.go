...
// validateTimeout is the HTTP client timeout for the validate call. Storage
// analysis is static (AST + filesystem) so it is fast, but code-facts domain
// resolution can add a few seconds; a 60s ceiling is comfortable headroom.
...
// the assessment findings to human / JSON output, and returns a non-nil error
// when the shared status is failed or errored.
func (h *handlers) validateScenario(ctx cliapp.RunContext) error {
	...
	if msg.GetStatus() == scenariovalidationv1.ValidationStatus_VALIDATION_STATUS_FAILED {
		return fmt.Errorf("scenario %s did not pass storage validation (%d error finding(s))", msg.GetScenario(), errors)
	}
	if msg.GetStatus() == scenariovalidationv1.ValidationStatus_VALIDATION_STATUS_ERROR {
		return fmt.Errorf("scenario %s storage validation errored", msg.GetScenario())
	}
	return nil
}
