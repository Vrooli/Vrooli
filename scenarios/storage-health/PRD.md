...
- Scenario dependencies: `code-facts` (API-surface language + `FACT_FAMILY_FILE_DOMAIN` domain ownership), `data-backup-manager` (backup/restore-readiness detection), `test-genie` (downstream consumer via the `storage` phase and the fail-closed isolation gate), `swarm-manager` (consumes `FINDING_SOURCE_STORAGE` on the `storage` dimension).
