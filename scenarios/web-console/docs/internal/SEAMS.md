...
| `VoiceStreamConfig` on `Server` | Runtime-configurable struct with `FlushIntervalMs`, `MinDeltaBytes`, `OverlapBytes`, `CoverageThreshold`. Read once per session (snapshot pattern). Backed by `store/voice-config.json`. | Tests set config via `srv.setVoiceConfig(VoiceStreamConfig{...})` before dialing WS. Each test gets its own `srv` — no cleanup needed. |
