...
- **PostgreSQL available via `api-core/database`**: The database connection string is resolved by the `api-core` library from environment variables. Made in [CODE: api/main.go].
- **SQL files exist relative to binary**: `initSchema()` reads `../initialization/postgres/schema.sql` relative to `os.Executable()`. Breaks if binary is moved without its sibling directories. Made in [CODE: api/main.go].
