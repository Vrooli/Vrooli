...
```bash
react-component-library components ingest web-console <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> web-console
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario web-console
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> web-console <adopted-path>
```
