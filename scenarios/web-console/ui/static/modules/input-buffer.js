...
export function enqueueTerminalInput(tab, data, ensureSessionForPendingInput) {
  ...
  if (!tab.inputBatch) {
    ...
    if (INPUT_CONTROL_FLUSH_PATTERN.test(data)) {
      flushInputBatch(tab, { reason: 'control-char' }, ensureSessionForPendingInput)
    } else {
  ...
  if (tab.inputBatch.size >= INPUT_BATCH_MAX_CHARS) {
    flushInputBatch(tab, { reason: 'batch-size' }, ensureSessionForPendingInput)
    return
  }
...
export function flushInputBatch(tab, options = {}, ensureSessionForPendingInput) {
