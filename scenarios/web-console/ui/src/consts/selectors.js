...
const formatTemplate = (template, values, keyPath) => template.replace(TEMPLATE_TOKEN, (_match, token) => {
    if (!(token in values)) {
        throw new Error(`Missing parameter '${token}' for selector '${keyPath}'`);
    }
    return String(values[token]);
});
const toDataTestIdSelector = (testId) => `[data-testid="${testId}"]`;
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- type guard requires any for generic erasure
const isDynamicDefinition = (value) => Boolean(value && typeof value === "object" && value.kind === "dynamic-selector");
    ...
    for (const [key, value] of Object.entries(tree)) {
        ...
        if (isDynamicDefinition(value)) {
            ...
            target[manifestKey] = {
                ...
                selectorPattern: value.selectorPattern ?? (value.testIdPattern ? toDataTestIdSelector(value.testIdPattern) : ""),
...
const createSelectorRegistry = (literalTree, dynamicTree) => {
