...
import { createInputGate, } from "../../components/terminal/inputGate";
...
function appendOutputProbe(sessionId, data) {
...
function stripTerminalResponses(s) {
    if (s.indexOf("\x1b") === -1)
        return s;
 ...
 */
export function useTerminalSession({ sessionId, terminal, onExit, onReady, createSocket, }) {
    ...
    const declaredSizeRef = useRef(null);
    const leaseRequestInFlightRef = useRef(false);
    const [serverSize, setServerSize] = useState(null);
    ...
    const currentGen = useCallback(() => transportRef.current?.currentGen() ?? 0, []);
    const sendFrame = useCallback((msg) => transportRef.current?.sendJson(msg) ?? false, []);
    const stdin = useStdinAck({
        sendFrame,
        isSessionReady,
        currentGen,
    });
    const gate = useRef(createInputGate({
        transport: {
            send: stdin.send,
    ...
    const onTransportOpen = useCallback((wasReconnect, _gen) => {
    ...
    const requestLease = useCallback((explicit = false) => {
        if (!explicit && leaseRequestInFlightRef.current)
            return;
        ...
        if (declared)
            transport.sendJson({ type: "resize", cols: declared.cols, rows: declared.rows });
    ...
    const submitInput = useCallback((data, source) => {
        if (!holdsLease)
            requestLease();
        return gate.submit(data, source);
    }, [gate, holdsLease, requestLease]);
    const sendConversationAck = useCallback((eventId, source, stage, message, backend) => {
        if (!eventId || !source)
            return;
        transport.sendJson({
        ...
        const unsubscribe = transport.subscribe((msg) => {
            switch (msg.type) {
                ...
                case "error": {
                    ...
                    if (hint) {
                        terminalRef.current?.write(`${ANSI.gray}  ${hint}${ANSI.reset}\r\n`);
                    }
                    break;
                }
                case "sync_warning": {
                ...
                case "size_info": {
                    ...
                    if (nextHoldsLease || !leaseRequestInFlightRef.current)
                        leaseRequestInFlightRef.current = false;
