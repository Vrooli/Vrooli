...
async function probeKokoroAvailable() {
    ...
    try {
        ...
        return caps.capabilities.some((c) => c.status === "available" && ((c.id === AUDIO_TOOLS_CAPABILITY_SLUG && c.features.includes(voiceOutputSlug)) ||
...
export function useTextToSpeech(settings, diagnostics) {
    const startMutedOnLoad = useWorkspaceStore((store) => store.startMutedOnLoad);
    const onPlaybackEvent = useCallback((ev) => {
