...
import { saveWorkspaceLayout, updateWorkspacePane, createTabGroup, updateTabGroup, deleteTabGroup, } from "../api/workspace";
...
export function useWorkspaceSync() {
    ...
    const syncPaneOrder = useCallback((paneOrder, activePane) => {
        if (reorderTimer.current)
            clearTimeout(reorderTimer.current);
        reorderTimer.current = setTimeout(() => {
            reorderTimer.current = null;
            saveWorkspaceLayout({ active_pane: activePane, pane_order: paneOrder }).catch((err) => console.error("Failed to sync pane order:", err));
        }, REORDER_DEBOUNCE_MS);
    }, []);
    /**
     ...
     */
    const syncPaneMove = useCallback((sessionId) => {
    ...
    const syncPaneUpdate = useCallback((sessionId, update) => {
        updateWorkspacePane(sessionId, update).catch((err) => console.error("Failed to sync pane update:", err));
    }, []);
    ...
    const syncPaneUpdates = useCallback(async (sessionIds, update) => {
        const results = await Promise.allSettled(sessionIds.map((id) => updateWorkspacePane(id, update)));
        const failed = [];
    ...
    const syncActivePane = useCallback((paneOrder, activePane) => {
        saveWorkspaceLayout({ active_pane: activePane, pane_order: paneOrder }).catch((err) => console.error("Failed to sync active pane:", err));
    }, []);
    /** Create a group on the backend and return the server-generated group. */
    const syncCreateGroup = useCallback(async (name, color) => {
        return createTabGroup({ name, color });
    }, []);
    /** Update a group on the backend. */
    const syncUpdateGroup = useCallback((id, update) => {
        updateTabGroup(id, update).catch((err) => console.error("Failed to sync group update:", err));
    }, []);
    /** Delete a group on the backend. */
    const syncDeleteGroup = useCallback((id) => {
        deleteTabGroup(id).catch((err) => console.error("Failed to sync group delete:", err));
    }, []);
    return {
        syncPaneOrder,
        syncPaneMove,
