...
import { TOUCH_MOVE_THRESHOLD_PX, TOUCH_LONG_PRESS_MS, TOUCH_TAP_MAX_MS, TOUCH_DOUBLE_TAP_MS, TOUCH_SCROLL_CANCEL_PX, TOUCH_SCROLL_DECEL, TOUCH_SCROLL_MIN_VELOCITY, } from "../consts/config";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
export function touchToCell(terminal, container, clientX, clientY) {
 ...
 */
export function findWordBoundaries(text, col) {
    ...
    const wordRe = /\S/;
    let start = col;
    while (start > 0 && wordRe.test(text.charAt(start - 1)))
        start--;
    let end = col;
    while (end < text.length - 1 && wordRe.test(text.charAt(end + 1)))
        end++;
    return [start, end + 1];
}
/**
 ...
 */
function getCellHeight(terminal, container) {
    const screenEl = container.querySelector(".xterm-screen");
    if (!screenEl)
        return 0;
 ...
 */
function isAppMouseMode(terminal) {
 ...
 */
function sgrWheelSequence(up, col, row) {
...
// Hook
// ---------------------------------------------------------------------------
export function useTerminalTouch({ terminal, containerRef, enabled = true, onContextMenu, submitInput, }) {
    ...
    const resetGesture = useCallback(() => {
        ...
        if (g.type === "scrolling" && g.longPressTimer !== null)
            clearTimeout(g.longPressTimer);
        gestureRef.current = { type: "idle" };
    }, []);
    // ---- Main effect: attach touch listeners ----
    useEffect(() => {
        if (!terminal || !containerRef.current || !enabled)
            return;
        ...
        const cellH = () => getCellHeight(term, container);
        ...
        function onTouchStart(e) {
            ...
            if (prev &&
                ...
                const { col, row } = touchToCell(term, container, touch.clientX, touch.clientY);
            ...
            const longPressTimer = setTimeout(() => {
                const g = gestureRef.current;
                if (g.type !== "pending" && g.type !== "scrolling")
                    return;
