...
describe("Command execution", () => {
  it("new-tab calls createTab", () => {
    ...
    const cmd = VOICE_COMMANDS.find((c) => c.id === "new-tab")!;
    cmd.execute(ctx, {});
  ...
  it("switch-tab calls switchToTab with the right number", () => {
    const ctx = createMockContext();
    const cmd = VOICE_COMMANDS.find((c) => c.id === "switch-tab")!;
  ...
  it("close-tab calls closeTab", () => {
    const ctx = createMockContext();
    const cmd = VOICE_COMMANDS.find((c) => c.id === "close-tab")!;
    cmd.execute(ctx, {});
  ...
  it("send-enter sends carriage return", () => {
    const ctx = createMockContext();
    const cmd = VOICE_COMMANDS.find((c) => c.id === "send-enter")!;
    cmd.execute(ctx, {});
  ...
  it("cancel sends Ctrl+C", () => {
    const ctx = createMockContext();
    const cmd = VOICE_COMMANDS.find((c) => c.id === "cancel")!;
    cmd.execute(ctx, {});
  ...
  it("clear sends Ctrl+L", () => {
    const ctx = createMockContext();
    const cmd = VOICE_COMMANDS.find((c) => c.id === "clear")!;
    cmd.execute(ctx, {});
  ...
  it("tab-key sends Tab character", () => {
    const ctx = createMockContext();
    const cmd = VOICE_COMMANDS.find((c) => c.id === "tab-key")!;
    cmd.execute(ctx, {});
  ...
  it("stop-listening calls exitVoiceMode", () => {
    const ctx = createMockContext();
    const cmd = VOICE_COMMANDS.find((c) => c.id === "stop-listening")!;
    cmd.execute(ctx, {});
