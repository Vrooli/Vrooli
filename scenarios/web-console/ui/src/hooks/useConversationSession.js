 ...
 */
export async function refreshConversationSession(sessionId) {
    ...
    try {
        ...
        if (existing)
            ...
            state.hydrateSession(sessionId, data.events, data.cursor, { oldestSequence: data.oldestSequence ?? data.events[0]?.sequence ?? 0, hasOlder: data.hasMore ?? false, totalCount: data.totalCount ?? data.events.length });
        return true;
    }
    catch {
        return false;
...
export async function loadOlderConversationPage(sessionId) {
    const state = useConversationStore.getState();
    const session = state.sessions[sessionId];
    if (!session?.hasOlder || !session.windowOldestSequence || olderPageLoads.has(sessionId))
        return false;
...
export async function loadConversationPageContaining(sessionId, sequence) {
    if (sequence <= 0)
        return false;
    const state = useConversationStore.getState();
    ...
    try {
        ...
        if (!data.events.some((event) => event.sequence === sequence))
            return false;
...
export function useConversationSession(sessionId, options = {}) {
    ...
    const persistCursor = useCallback(async (cursor) => {
