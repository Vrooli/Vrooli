...
const EMPTY_CONVERSATION_EVENTS = [];
const EMPTY_CONVERSATION_CURSOR = { lastSeenSequence: 0, lastListenedSequence: 0 };
...
const AUTO_TTS_MAX_AGE_MS = 60000;
/**
 ...
 */
function ensureSpeechChunks(paragraphs) {
    const result = [];
...
const TerminalPane = forwardRef(function TerminalPane({ sessionId, onExit, onVoiceStart, onVoiceStop, onTtsSpeakingChange, onSpeakingEventChange, onNeedsUnlock, onConversationEventReceived }, ref) {
    ...
    const paneFontSize = useWorkspaceStore(useCallback((s) => s.deviceFontSize[sessionId] ?? s.panes.find((p) => p.sessionId === sessionId)?.fontSize ?? TERMINAL_FONT_SIZE, [sessionId]));
    const paneThemeId = useWorkspaceStore(useCallback((s) => s.panes.find((p) => p.sessionId === sessionId)?.themeId ?? DEFAULT_THEME_ID, [sessionId]));
    const paneTheme = useMemo(() => {
        const fallback = { background: "#0f172a", foreground: "#e2e8f0", cursor: "#38bdf8" };
    ...
    const { speakParagraphs, stop: ttsStop, pause: ttsPause, resume: ttsResume, seek: ttsSeek, setPlaybackRate: ttsSetPlaybackRate, setVolume: ttsSetVolume, setMuted: ttsSetMuted, getPlaybackState: ttsGetPlaybackState, supported: ttsSupported, isSpeaking: ttsSpeaking, needsUnlock, unlockAudio, } = useTextToSpeech(ttsSettings, {
    ...
    const followerFrame = useMemo(() => {
        ...
        if (!isFollower || !size || paneSize.width <= 0 || paneSize.height <= 0)
            return null;
        const currentTerminal = terminal;
        const screen = currentTerminal?.element?.querySelector(".xterm-screen");
    ...
    const autoTtsBaselineRef = useRef(null);
    useEffect(() => {
        ...
        if (maxSeq <= baseline)
            return;
        autoTtsBaselineRef.current = maxSeq;
        if (!ttsSupported || !onConversationEventReceived)
            return;
        ...
        for (let i = conversationEvents.length - 1; i >= 0; i--) {
            ...
            if (!candidate || candidate.sequence <= baseline)
                break;
        ...
        if (Number.isFinite(ageMs) && ageMs > AUTO_TTS_MAX_AGE_MS)
            return;
        const event = latest;
        onConversationEventReceived(sessionId, event, (stage, message, backend) => sendConversationAck(event.id, event.source, stage, message, backend));
    ...
    useEffect(() => {
        ...
        if (draft)
            submitInputRef.current(draft, "toolbar-submit");
        return () => {
            const text = getPendingInputSnapshotRef.current()
                .map((entry) => entry.data)
                .join("");
            if (text)
                setPendingInputDraft(sessionId, text);
        };
    ...
    useImperativeHandle(ref, () => ({
        ...
        speakText: (text, paragraphs, opts) => {
    ...
    const selfReportedSeqsRef = useRef(new Set());
    ...
    useEffect(() => {
        const unsub = subscribeInputSettled((seq, ok, reason) => {
            ...
            if (ok || wasSelfReported)
                return;
            if (reason === "ack-timeout" || reason === "connection-closed")
                return;
    ...
    const handleCtxPaste = useCallback((text) => {
    ...
    const handleCtxSpeak = useCallback(() => {
        ...
        if (selection)
            void speakParagraphs(ensureSpeechChunks([selection]));
        setContextMenu(null);
    ...
    const chromeDetectorEnabled = adaptiveChrome && isTabLikeDisplayMode(displayMode) && activePane === sessionId;
    useTerminalBackgroundDetector(terminal, {
        enabled: chromeDetectorEnabled,
        defaultBackground: paneTheme.background,
        onColor: useCallback((hex) => chromeTheme.setDetected(sessionId, hex), [sessionId]),
    });
    ...
    useEffect(() => {
        const container = containerRef.current;
        if (!container || !onVoiceStart || !onVoiceStop)
            return;
    ...
    useEffect(() => {
        const container = containerRef.current;
        if (!container || !terminal || !fitRef.current)
            return;
        let rafId = null;
    ...
    return (_jsxs("div", { ref: containerRef, "data-testid": "terminal-pane", "data-session-id": sessionId, 
        ...
        className: `h-full w-full overflow-hidden relative p-1${dragOver ? " ring-2 ring-inset ring-blue-400/60" : ""}`, onPasteCapture: handlePaste, onDragOver: handleDragOver, onDragLeave: handleDragLeave, onDrop: handleDrop, children: [followerFrame && _jsx(DeviceFrame, { archetype: followerFrame.archetype, chromeTier: followerFrame.tier, rect: followerFrame.rect, leaderDevice: leaderDevice, gridCols: followerFrame.cols, gridRows: followerFrame.rows, onTakeOver: takeLease }), uploading && (_jsx("div", { "data-testid": "upload-overlay", className: "absolute inset-0 z-wc-chrome-raised flex items-center justify-center bg-black/50 text-sm text-white", children: t(strings.terminalPane.uploadingImage) })), uploadError && (_jsx("div", { "data-testid": "upload-error", className: "absolute top-2 left-2 z-wc-chrome-raised rounded bg-red-600/90 px-3 py-1.5 text-xs text-white shadow-lg", children: uploadError })), inputError && (_jsx("div", { "data-testid": "input-error", role: "status", className: "absolute bottom-2 left-2 right-2 z-wc-chrome-raised rounded bg-red-600/90 px-3 py-1.5 text-xs text-white shadow-lg", children: inputError })), contextMenu && (_jsx(TerminalContextMenu, { position: contextMenu, hasSelection: hasSelection, onCopy: handleCtxCopy, onPaste: handleCtxPaste, onSelectAll: handleCtxSelectAll, onClear: handleCtxClear, onUploadImage: handleCtxUploadImage, onSpeak: ttsSupported ? handleCtxSpeak : undefined, onClose: closeContextMenu })), _jsx("input", { ref: fileInputRef, type: "file", accept: "image/*", hidden: true, onChange: handleFileInputChange })] }));
});
