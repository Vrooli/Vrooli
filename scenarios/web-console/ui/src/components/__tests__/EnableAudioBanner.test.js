...
describe("EnableAudioBanner", () => {
    it("renders enable and dismiss buttons with copy explaining the block", () => {
        render(_jsx(EnableAudioBanner, { onEnable: vi.fn().mockResolvedValue(true), onDismiss: vi.fn() }));
    ...
    it("clicking enable invokes onEnable exactly once", async () => {
        ...
        render(_jsx(EnableAudioBanner, { onEnable: onEnable, onDismiss: vi.fn() }));
    ...
    it("clicking dismiss invokes onDismiss exactly once", () => {
        ...
        render(_jsx(EnableAudioBanner, { onEnable: vi.fn().mockResolvedValue(true), onDismiss: onDismiss }));
    ...
    it("shows Enabling… and disables both buttons while enable is in flight", async () => {
        ...
        let resolveEnable;
        const onEnable = vi.fn(() => new Promise((r) => { resolveEnable = r; }));
        const onDismiss = vi.fn();
        render(_jsx(EnableAudioBanner, { onEnable: onEnable, onDismiss: onDismiss }));
        fireEvent.click(screen.getByTestId("enable-audio-banner-enable"));
