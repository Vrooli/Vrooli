...
describe("SummarizeErrorBanner", () => {
    it("renders the message and source label for auto failures", () => {
        render(_jsx(SummarizeErrorBanner, { state: makeState({ source: "auto" }), onRetry: vi.fn(), onDismiss: vi.fn() }));
    ...
    it("renders a different source label for on-demand failures", () => {
        render(_jsx(SummarizeErrorBanner, { state: makeState({ source: "on-demand" }), onRetry: vi.fn(), onDismiss: vi.fn() }));
        expect(screen.getByText(strings.summarizeError.failed)).toBeInTheDocument();
    });
    it("clicking retry fires onRetry", () => {
        const onRetry = vi.fn();
        render(_jsx(SummarizeErrorBanner, { state: makeState(), onRetry: onRetry, onDismiss: vi.fn() }));
        fireEvent.click(screen.getByTestId("summarize-error-retry"));
        expect(onRetry).toHaveBeenCalledTimes(1);
    });
    it("clicking dismiss fires onDismiss", () => {
        const onDismiss = vi.fn();
        render(_jsx(SummarizeErrorBanner, { state: makeState(), onRetry: vi.fn(), onDismiss: onDismiss }));
    ...
    it("disables retry and dismiss while retrying and shows a spinner", () => {
        render(_jsx(SummarizeErrorBanner, { state: makeState({ status: "retrying" }), onRetry: vi.fn(), onDismiss: vi.fn() }));
    ...
    it("banner carries data attributes for source and status", () => {
        render(_jsx(SummarizeErrorBanner, { state: makeState({ source: "on-demand", status: "retrying" }), onRetry: vi.fn(), onDismiss: vi.fn() }));
    ...
    it("applies horizontal safe-area padding", () => {
        render(_jsx(SummarizeErrorBanner, { state: makeState(), onRetry: vi.fn(), onDismiss: vi.fn() }));
