...
describe("VoiceRejectionBanner", () => {
    it("shows Transcribe-anyway and Dismiss for retryable rejection", () => {
        render(_jsx(VoiceRejectionBanner, { rejection: retryable(), onRetry: vi.fn(), onDismiss: vi.fn() }));
    ...
    it("only shows Dismiss for explanatory rejection", () => {
        render(_jsx(VoiceRejectionBanner, { rejection: explanatory(), onRetry: vi.fn(), onDismiss: vi.fn() }));
        expect(screen.queryByTestId("voice-rejection-retry")).toBeNull();
        expect(screen.getByTestId("voice-rejection-dismiss")).toBeInTheDocument();
    });
    it("calls onRetry when Transcribe-anyway is clicked", () => {
        const onRetry = vi.fn();
        render(_jsx(VoiceRejectionBanner, { rejection: retryable(), onRetry: onRetry, onDismiss: vi.fn() }));
        fireEvent.click(screen.getByTestId("voice-rejection-retry"));
        expect(onRetry).toHaveBeenCalledTimes(1);
    });
    it("calls onDismiss when Dismiss is clicked", () => {
        const onDismiss = vi.fn();
        render(_jsx(VoiceRejectionBanner, { rejection: retryable(), onRetry: vi.fn(), onDismiss: onDismiss }));
        fireEvent.click(screen.getByTestId("voice-rejection-dismiss"));
        expect(onDismiss).toHaveBeenCalledTimes(1);
    });
    it("disables both buttons while retrying", () => {
        render(_jsx(VoiceRejectionBanner, { rejection: retryable({ status: "retrying" }), onRetry: vi.fn(), onDismiss: vi.fn() }));
    ...
    it("shows Retry + error text on failed status", () => {
        render(_jsx(VoiceRejectionBanner, { rejection: retryable({ status: "failed", errorMessage: "Network error" }), onRetry: vi.fn(), onDismiss: vi.fn() }));
    ...
    it("shows the empty-transcript copy and a Retry action for cause=empty-transcript", () => {
        render(_jsx(VoiceRejectionBanner, { rejection: retryable({ cause: "empty-transcript", score: 0, threshold: 0 }), onRetry: vi.fn(), onDismiss: vi.fn() }));
    ...
    it("renders empty-transcript title/detail in en locale", async () => {
        ...
        render(_jsx(VoiceRejectionBanner, { rejection: retryable({ cause: "empty-transcript", durationMs: 8200 }), onRetry: vi.fn(), onDismiss: vi.fn() }));
    ...
    describe("interpolation (en locale)", () => {
        ...
        it("includes the score, threshold, and duration in the retryable variant", () => {
            render(_jsx(VoiceRejectionBanner, { rejection: retryable({ score: 0.42, threshold: 0.85, durationMs: 5500 }), onRetry: vi.fn(), onDismiss: vi.fn() }));
