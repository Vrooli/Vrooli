 ...
 */
const TabUnreadBadge = memo(function TabUnreadBadge({ sessionId, supportsMessagesView, manuallyUnread, }) {
    const unreadCount = useConversationStore((state) => supportsMessagesView ? getSessionUnreadCount(state, sessionId) : 0);
    if (unreadCount > 0) {
        return (_jsx("span", { className: "rounded-full bg-wc-accent px-1.5 py-0.5 text-[10px] font-semibold text-wc-accent-fg", children: unreadCount }));
    }
    // Manual flag: a dot with no number. It means "come back here", not
    // "N things happened", and a real count always outranks it.
    if (manuallyUnread) {
        return (_jsx("span", { "data-testid": `tab-manual-unread-${sessionId}`, className: "h-2 w-2 shrink-0 rounded-full bg-wc-accent" }));
    }
    return null;
});
function TabBar({ panes, activePane, onNewTerminal, onOpenLauncher, onClosePane, isCreating, trailingActions, }) {
    ...
    const editTabInputRef = useRef(null);
    /** Start inline rename for a tab. */
    const startTabRename = useCallback((sessionId, currentName) => {
    ...
    const mouseActivatedPaneRef = useRef(null);
    ...
    const tabPressGesture = usePressGesture({
        longPressMs: 500,
    ...
    const commitDrag = useCallback((paneId, target, pointerId) => {
    ...
    return (_jsxs("div", { "data-testid": "tab-bar", className: "wc-chrome-surface flex items-stretch h-9 border-b border-wc-default shrink-0 ps-[var(--wc-safe-left,0px)] pe-[var(--wc-safe-right,0px)]", role: "tablist", children: [_jsx("div", { ref: tabContainerRef, className: "flex-1 flex items-stretch overflow-x-auto wc-hide-scrollbar", children: renderItems.map((item) => {
                    if (item.kind === "group-label") {
                        const { group, tabCount } = item;
                        return (_jsxs("button", { "data-testid": `tab-group-${group.id}`, className: "flex items-center gap-1 px-2 text-xs shrink-0 border-r border-wc-default text-wc-text-secondary hover:bg-wc-surface-raised transition-colors", ...groupPressGesture.getGestureHandlers(group.id), onClick: () => {
                                if (groupPressGesture.shouldSuppressClick(group.id))
                                    return;
                                toggleGroupCollapsed(group.id);
                            }, title: group.isCollapsed ? t(strings.tabBar.expandGroup, { name: group.name }) : t(strings.tabBar.collapseGroup, { name: group.name }), children: [_jsx("span", { className: "h-2.5 w-2.5 rounded-full shrink-0", style: { backgroundColor: group.color } }), _jsx("span", { className: "truncate max-w-[80px] font-medium", children: group.name }), group.isCollapsed && (_jsx("span", { className: "text-[10px] bg-wc-surface-input rounded px-1", children: tabCount })), group.isCollapsed ? (_jsx(ChevronRight, { className: "h-3 w-3 shrink-0" })) : (_jsx(ChevronDown, { className: "h-3 w-3 shrink-0" }))] }, `group-${group.id}`));
                    }
                    const { pane, globalIndex: idx, group } = item;
                    const isActive = item.isActive;
                    const isBeingDragged = dragState?.paneId === pane.sessionId;
                    const isDropTarget = isDragging && !isBeingDragged && dragState?.dropIndex === idx;
                    const paneGroup = pane.groupId ? groupMap.get(pane.groupId) : undefined;
                    return (_jsxs("div", { ref: isActive ? activeTabRef : undefined, "data-testid": `tab-${pane.sessionId}`, "data-tab-index": idx, role: "tab", tabIndex: 0, "aria-selected": isActive, className: cn("group relative flex items-center gap-1.5 h-full px-3 text-xs shrink-0 border-r border-wc-default transition-colors", "hover:bg-wc-surface-raised focus:outline-none focus-visible:ring-1 focus-visible:ring-wc-accent", isActive
                            ? "bg-wc-surface-base text-wc-text-primary font-medium shadow-[inset_0_-2px_0_0_rgb(var(--wc-accent))]"
                            : "bg-wc-surface-header text-wc-text-secondary", isBeingDragged && "opacity-40", isDropTarget && "ring-2 ring-blue-400/60 ring-inset"), style: paneGroup
                        ...
                        }, onContextMenu: tabPressGesture.getGestureHandlers(pane.sessionId).onContextMenu, onPointerDown: (e) => {
                            ...
                            dragStartRef.current = null;
                                ...
                                return accentStyle ? (_jsx("span", { className: "absolute left-0 top-0 bottom-0 w-1", style: accentStyle })) : null;
                            })(), editingTabId === pane.sessionId ? (_jsx("input", { ref: editTabInputRef, "data-testid": `tab-rename-input-${pane.sessionId}`, className: "bg-wc-surface-input text-wc-text-primary text-xs px-1 rounded w-[100px] outline-none ring-1 ring-wc-accent", value: editTabName, onChange: (e) => setEditTabName(e.target.value), onKeyDown: (e) => {
                                    if (e.key === "Enter")
                                        commitTabRename();
                                    if (e.key === "Escape") {
                                        setEditingTabId(null);
                                ...
                                }, onBlur: commitTabRename, onClick: (e) => e.stopPropagation() })) : (_jsx("span", { className: "truncate max-w-[120px]", children: pane.name })), _jsx(TabUnreadBadge, { sessionId: pane.sessionId, supportsMessagesView: pane.supportsMessagesView, manuallyUnread: pane.manuallyUnread }), _jsx("button", { type: "button", className: cn("flex items-center justify-center h-4 w-4 rounded hover:bg-wc-surface-input", "opacity-0 group-hover:opacity-100 focus:opacity-100", isActive && "opacity-100"), onClick: (e) => {
                                    e.stopPropagation();
                                    onClosePane(pane.sessionId);
                                }, title: t(strings.tabBar.closeTabTitle), "aria-label": t(strings.tabBar.closeTabAria, { name: pane.name }), children: _jsx(X, { className: "h-3 w-3" }) })] }, pane.sessionId));
                }) }), trailingActions, _jsx(Button, { "data-testid": "tab-bar-new", variant: "ghost", size: "icon", className: "h-7 w-7 shrink-0 mx-1 self-center", disabled: isCreating, title: plusButtonBehavior === "launcher" ? t(strings.floatingToolbar.launcherFirstTitle) : t(strings.floatingToolbar.terminalFirstTitle), onPointerDown: plusHandlers.onPointerDown, onPointerUp: plusHandlers.onPointerUp, onPointerCancel: plusHandlers.onPointerCancel, onContextMenu: plusHandlers.onContextMenu, children: _jsx(Plus, { className: "h-4 w-4" }) }), tabContextMenu && (() => {
                const pane = panes.find((p) => p.sessionId === tabContextMenu.sessionId);
                if (!pane)
                    return null;
                return (_jsx(TabContextMenu, { position: tabContextMenu.position, sessionId: tabContextMenu.sessionId, currentGroupId: pane.groupId, isManuallyUnread: pane.manuallyUnread, onToggleManuallyUnread: () => toggleManualUnread(pane.sessionId), onRename: () => {
                        const p = panes.find((p) => p.sessionId === tabContextMenu.sessionId);
                        if (p)
                            startTabRename(p.sessionId, p.name);
                    }, onCustomize: () => setAppearanceModalPane(tabContextMenu.sessionId), onRemoveFromGroup: () => removePaneFromGroup(tabContextMenu.sessionId), onManageGroups: () => setManageGroupsTarget({ sessionId: tabContextMenu.sessionId }), onClose: onClosePane, onDismiss: () => setTabContextMenu(null) }));
            })(), groupMenu && (() => {
                const group = groups.find((g) => g.id === groupMenu.groupId);
                if (!group)
                    return null;
                return (_jsx(GroupContextMenu, { position: groupMenu.position, group: group, onToggleCollapse: () => toggleGroupCollapsed(group.id), onManageGroups: () => setManageGroupsTarget({ sessionId: null }), onDismiss: () => setGroupMenu(null) }));
            })()] }));
}
