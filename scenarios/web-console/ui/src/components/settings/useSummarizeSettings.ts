...
import {
  ...
  type TTSSummarizeModel,
} from "../../audio-integration";
import { toErrorInfo } from "../../lib/errors";

export function useSummarizeSettings() {
  const [config, setConfig] = useState<TTSSummarizeConfig | null>(null);
  const [models, setModels] = useState<TTSSummarizeModel[]>([]);
  ...
  const save = useCallback(async (patch: Partial<TTSSummarizeConfig>) => {
    setSaving(true);
    setError(null);
