...
import { distance, fitTransform, midpoint, panBy, parseViewBox, resetTransform, zoomAroundPoint, ZOOM_STEP, } from "./zoomTransform";
/**
 ...
 */
export const MermaidZoomSurface = forwardRef(function MermaidZoomSurface({ svgHtml, onScaleChange, ariaLabel, panStep = 48 }, ref) {
    const surfaceRef = useRef(null);
    const contentRef = useRef(null);
    const [transform, setTransform] = useState(() => resetTransform());
    ...
    const relativePoint = useCallback((clientX, clientY) => {
    ...
    return (_jsx("div", { ref: surfaceRef, "data-testid": "mermaid-zoom-surface", role: "img", "aria-label": ariaLabel, tabIndex: 0, onPointerDown: onPointerDown, onPointerMove: onPointerMove, onPointerUp: endPointer, onPointerCancel: endPointer, onKeyDown: onKeyDown, className: `relative h-full w-full touch-none select-none overflow-hidden bg-wc-surface outline-none focus-visible:ring-1 focus-visible:ring-wc-accent/40 ${isPanning ? "cursor-grabbing" : "cursor-grab"}`, children: _jsx("div", { ref: contentRef, 
            ...
            className: "absolute left-0 top-0 origin-top-left", style: { transform: `translate(${transform.x}px, ${transform.y}px) scale(${transform.scale})` }, dangerouslySetInnerHTML: { __html: svgHtml } }) }));
});
