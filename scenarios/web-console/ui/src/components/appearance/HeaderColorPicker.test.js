...
function Harness({ initial = "transparent", onChange }) {
    const [color, setColor] = useState(initial);
    return (_jsx(HeaderColorPicker, { currentColor: color, onSelectColor: (c) => {
            setColor(c);
            onChange?.(c);
        } }));
}
describe("HeaderColorPicker recent row", () => {
    it("does not render the recent row when there are no recents", () => {
        render(_jsx(HeaderColorPicker, { currentColor: "transparent", onSelectColor: vi.fn() }));
    ...
    it("renders recents and records a color on pick", () => {
        ...
        render(_jsx(Harness, { onChange: onChange }));
...
describe("HeaderColorPicker two-color UX", () => {
    ...
    it("removing the secondary returns to a single color", () => {
        const onChange = vi.fn();
        render(_jsx(Harness, { initial: "#4dabf7|#ff6b6b", onChange: onChange }));
