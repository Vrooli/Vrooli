...
function SessionRecoveryBannerInner() {
  ...
  return (
    <div
      ...
      role="status"
      className="wc-stable-theme flex items-center gap-2 border-b border-blue-500/30 bg-blue-500/10 py-2 ps-[max(0.75rem,var(--wc-safe-left,0px))] pe-[max(0.75rem,var(--wc-safe-right,0px))] text-xs text-blue-200"
    >
      {inProgress ? (
        <>
        ...
        <>
          ...
          <span className="min-w-0 flex-1 break-words">
            {t(strings.sessionRecovery.recovered, { count: recovered + adopted })}
          </span>
          <button
            type="button"
            data-testid="session-recovery-view"
            onClick={() => window.location.reload()}
            className="inline-flex shrink-0 items-center gap-1 rounded border border-blue-400/40 bg-blue-500/20 px-2 py-1 font-medium text-blue-100 transition active:bg-blue-500/30"
          >
            <RefreshCw className="h-3.5 w-3.5" aria-hidden />
            <span>{t(strings.sessionRecovery.view)}</span>
          </button>
        </>
      )}
    </div>
