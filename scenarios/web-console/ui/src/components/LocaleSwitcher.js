...
import { LOCALE_CODES, getCurrentLocale, getLocaleConfig, setLocale, } from "../i18n";
...
export default function LocaleSwitcher() {
    ...
    return (_jsx("div", { role: "group", "data-testid": localeSelectors.switcher, "aria-label": t(strings.settings.workspaceSection.localeSwitcherAria), className: "inline-flex items-center gap-1 rounded-lg border border-wc-default bg-wc-surface-base p-0.5", children: LOCALE_CODES.map((code) => {
            const config = getLocaleConfig(code);
            const isActive = code === active;
            return (_jsx("button", { type: "button", "data-testid": localeSelectors.toggle({ code }), "aria-pressed": isActive, "aria-label": t(strings.settings.workspaceSection.localeToggleAria, {
                    language: config.nativeLabel,
                }), onClick: () => {
                    void setLocale(code);
                }, className: cn("rounded-md px-2 py-1 text-xs font-medium transition-colors", isActive
                    ? "bg-wc-accent text-wc-accent-fg"
                    : "text-wc-text-secondary hover:bg-wc-surface-input hover:text-wc-text-primary"), children: config.nativeLabel }, code));
        }) }));
}
