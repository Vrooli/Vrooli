...
import { CHROME_PALETTE_TOKEN_NAMES, deriveChromePalette, oklchToRgb, paletteTripleToHex, rgbToOklch, } from "../lib/chromePalette";
import { contrastRatioRgb, hexToRgb } from "../lib/paneColor";
function triple(value) {
    const [r = 0, g = 0, b = 0] = value.split(/\s+/).map((part) => Number.parseInt(part, 10));
    return { r, g, b };
}
function token(palette, name) {
    const value = palette[name];
    expect(value).toBeDefined();
    return value ?? "";
}
function expectContrast(fg, bg, target) {
