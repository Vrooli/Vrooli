...
import { ambientBackground, ansi256ToHex, cellBackgroundHex, dominantBackground, dominantWeightedBackground, parseOscColor, } from "../lib/terminalBackground";
/** Build a synthetic xterm cell for a given color mode. */
function cell(mode, value = 0) {
    return {
...
describe("dominantWeightedBackground", () => {
    it("picks the color with the largest total weight above the threshold", () => {
        expect(dominantWeightedBackground([
            { hex: "#111111", weight: 3 },
            { hex: "#222222", weight: 1 },
        ], 0.6)).toBe("#111111");
    });
...
describe("ambientBackground", () => {
    ...
    it("excludes the bottom status row so a status band cannot hijack the color", () => {
        const grid = fillGrid(20, 40, BASE);
        for (let c = 0; c < 40; c++)
            setCell(grid, 19, c, "#0dbc79"); // green status line
        expect(ambientBackground(grid)).toBe(BASE);
    });
