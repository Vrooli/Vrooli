...
import { KEY_COMBOS, CATEGORY_ORDER, filterCombos, type KeyComboCategory } from "../consts/key-combos";
...
describe("Key combo definitions", () => {
  ...
  it("all combos use valid categories", () => {
    const validCategories = new Set<KeyComboCategory>(CATEGORY_ORDER);
    for (const combo of KEY_COMBOS) {
