...
describe("useCountdown", () => {
    ...
    it("returns null for never mode", () => {
        const { result } = renderHook(() => useCountdown(new Date().toISOString(), "never", undefined));
        expect(result.current).toBeNull();
    });
    it("returns null when duration is undefined", () => {
        const { result } = renderHook(() => useCountdown(new Date().toISOString(), "preset", undefined));
        expect(result.current).toBeNull();
    });
    it("returns a countdown string for a valid preset duration", () => {
        // Created now with 1h duration — should show ~59m remaining
        const createdAt = new Date().toISOString();
        const { result } = renderHook(() => useCountdown(createdAt, "preset", "1h"));
    ...
    it("returns expired for sessions past their TTL", () => {
        ...
        const { result } = renderHook(() => useCountdown(twoHoursAgo, "preset", "1h"));
    ...
    it("cleans up interval on unmount", () => {
        ...
        const { unmount } = renderHook(() => useCountdown(new Date().toISOString(), "preset", "1h"));
        unmount();
