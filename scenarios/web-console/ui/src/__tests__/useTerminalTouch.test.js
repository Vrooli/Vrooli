...
import { createMockTerminal } from "../test-utils/mocks";
import { TOUCH_LONG_PRESS_MS, TOUCH_DOUBLE_TAP_MS, TOUCH_MOVE_THRESHOLD_PX, } from "../consts/config";
// ---------------------------------------------------------------------------
// Test helpers
// ---------------------------------------------------------------------------
/** Build a fake .xterm-screen element inside the container for coordinate mapping. */
function setupScreenElement(container, width = 800, height = 480) {
 ...
 */
function fireTouchEvent(container, type, opts = {}) {
    ...
    const touchList = type === "touchend" || type === "touchcancel" ? [] : [touchObj];
...
function makeHookArgs(terminal, container, enabled = true) {
    return {
        terminal: terminal,
...
describe("touchToCell", () => {
    it("maps client coords to terminal cell positions", () => {
        ...
        const result = touchToCell(terminal, container, 55, // col = floor(55/10) = 5
        45);
    ...
    it("clamps to valid range", () => {
        ...
        const result = touchToCell(terminal, container, -10, 9999);
    ...
    it("returns (0,0) when .xterm-screen is missing", () => {
        ...
        const terminal = createMockTerminal();
        const result = touchToCell(terminal, container, 100, 100);
...
describe("useTerminalTouch", () => {
    ...
    beforeEach(() => {
        ...
        screen = setupScreenElement(container);
    ...
    it("does nothing when terminal is null", () => {
        const { result } = renderHook(() => useTerminalTouch(makeHookArgs(null, container)));
    ...
    it("does nothing when enabled is false", () => {
        renderHook(() => useTerminalTouch(makeHookArgs(terminal, container, false)));
    ...
    it("long press then drag extends selection", () => {
        const { result } = renderHook(() => useTerminalTouch(makeHookArgs(terminal, container)));
        act(() => {
            fireTouchEvent(container, "touchstart", { clientX: 50, clientY: 40 });
            vi.advanceTimersByTime(TOUCH_LONG_PRESS_MS + 10);
        });
