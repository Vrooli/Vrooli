...
const pane = (sessionId, manuallyUnread = false) => ({
    sessionId,
    name: sessionId,
...
function renderSidebar() {
    ...
    const buckets = buildOriginBucketedNavigation({
        ...
        groups: [],
        activePane,
        originBySession: Object.fromEntries(panes.map((p) => [p.sessionId, "ui"])),
    });
    return render(_jsx(SessionSidebar, { buckets: buckets, containerRef: { current: null }, isMobile: false, mobileOpen: false, onCloseMobile: () => { }, onActivatePane: () => { }, onClosePane: () => { }, onNewTerminal: () => { }, onOpenLauncher: () => { }, onNewSessionInGroup: () => { }, onOpenSettings: () => { } }));
}
