...
import { buildWorkspaceNavigationItems, buildOriginBucketedNavigation, countWorkspaceUnreadMessages, groupIdForDropPosition, orderPanesByGroupBlocks, originBucket, sortPanesForView } from "../lib/workspaceNavigation";
import { isTabLikeDisplayMode } from "../lib/workspaceDisplayMode";
const pane = (sessionId, groupId = null) => ({
    sessionId,
    name: sessionId,
...
const event = (sequence, createdAt, role = "assistant") => ({
    id: `evt-${sequence}`,
    sessionId: "one",
    source: "test",
    role,
...
describe("orderPanesByGroupBlocks", () => {
    const g1 = { id: "g1", name: "Work", color: "#ff6b6b", isCollapsed: false };
    /** Every group label the sidebar/tab strip would render, in order. */
    const groupLabels = (panes) => buildWorkspaceNavigationItems({ panes, groups: [g1], activePane: null })
    ...
    it("keeps one header in every sort mode", () => {
        ...
        for (const mode of ["manual", "name", "activity", "unread"]) {
...
describe("buildOriginBucketedNavigation", () => {
    ...
    it("splits sessions into ordered buckets, folding unspecified into programmatic", () => {
        ...
        expect(programmatic?.items.flatMap((i) => (i.kind === "pane" ? [i.pane.sessionId] : []))).toEqual(["prog1", "unspec1"]);
    });
