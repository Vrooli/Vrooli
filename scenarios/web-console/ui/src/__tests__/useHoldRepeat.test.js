...
describe("useHoldRepeat", () => {
    ...
    it("begins repeating after initialDelayMs and continues at repeatIntervalMs", () => {
        const onFire = vi.fn();
        const { result } = renderHook(() => useHoldRepeat({ onFire, initialDelayMs: 400, repeatIntervalMs: 40 }));
        act(() => result.current.onPointerDown(makePointerEvent()));
        expect(onFire).toHaveBeenCalledTimes(1);
    ...
    it("ignores pointerdown from non-primary mouse buttons", () => {
        const onFire = vi.fn();
        const { result } = renderHook(() => useHoldRepeat({ onFire }));
        act(() => result.current.onPointerDown(makePointerEvent({ pointerType: "mouse", button: 2 })));
        act(() => {
            vi.advanceTimersByTime(2000);
        });
    ...
    it("uses the latest onFire without re-subscribing timers", () => {
        const first = vi.fn();
        const second = vi.fn();
        const { result, rerender } = renderHook(({ fn }) => useHoldRepeat({ onFire: fn }), { initialProps: { fn: first } });
