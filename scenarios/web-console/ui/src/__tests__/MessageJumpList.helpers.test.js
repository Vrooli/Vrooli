...
import { assistantRoleLabelKey, availableSources, buildResults, computeExcerpt, DEFAULT_NAVIGATOR_STATE, detectBadges, formatRelativeTime, groupResults, noResultReason, normalizePreview, sourceIdFor, statusGlyphFor, } from "../components/MessageJumpList.helpers";
...
describe("formatRelativeTime", () => {
    ...
    it("returns 'just now' under 45 seconds", () => {
        ...
        expect(formatRelativeTime(at(44000), now)).toBe("just now");
    });
    it("returns minutes between 45s and 1h", () => {
        expect(formatRelativeTime(at(60000), now)).toBe("1m");
        expect(formatRelativeTime(at(59 * 60000), now)).toBe("59m");
    });
    it("returns hours between 1h and 24h", () => {
        expect(formatRelativeTime(at(60 * 60000), now)).toBe("1h");
    });
...
describe("statusGlyphFor", () => {
    ...
    it("falls back to consumptionState=listened as played when idle", () => {
        expect(statusGlyphFor(makeEvent({ id: "x", sequence: 1, ttsState: "idle", consumptionState: "listened" })).glyph).toBe("played");
    });
    it("defaults idle/unseen to unseen", () => {
        expect(statusGlyphFor(makeEvent({ id: "x", sequence: 1, ttsState: "idle", consumptionState: "unseen" })).glyph).toBe("unseen");
    });
});
...
describe("groupResults", () => {
    ...
    it("turn handles a leading run of assistant events with a null lead user", () => {
        const lead = buildResults([makeEvent({ id: "a0", sequence: 0 }), ...events], state());
