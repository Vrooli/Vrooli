...
vi.mock("../stores/useWorkspaceStore", () => ({
    useWorkspaceStore: (selector) => selector ? selector(storeState) : storeState,
}));
const { default: TerminalPane } = await import("../components/TerminalPane");
function makeEvent(id, sequence, text, speechParagraphs) {
    return {
        id,
        sessionId: "tts-test",
        source: "claude_hook",
...
describe("TerminalPane auto-TTS (store-driven)", () => {
    ...
    function renderPane() {
        const r = render(_jsx(TerminalPane, { sessionId: "tts-test", onConversationEventReceived: mockIncoming }));
    ...
    it("forwards a fresh assistant event for the active pane to the controller", () => {
        ...
        expect(mockIncoming).toHaveBeenCalledWith("tts-test", expect.objectContaining({ id: "evt-1", speechParagraphs: ["Hello world"] }), expect.any(Function));
    });
    it("forwards the event's own speechParagraphs", () => {
        renderPane();
        act(() => useConversationStore.getState().appendEvent(makeEvent("evt-2", 2, "a\n\nb\n\nc", ["First paragraph", "Second paragraph", "Third paragraph"])));
        expect(mockIncoming).toHaveBeenCalledWith("tts-test", expect.objectContaining({ id: "evt-2", speechParagraphs: ["First paragraph", "Second paragraph", "Third paragraph"] }), expect.any(Function));
    });
    ...
    it("does not auto-read a stale (old) event — guards SSE reconnect backlog", () => {
        ...
        stale.createdAt = new Date(Date.now() - 5 * 60000).toISOString();
