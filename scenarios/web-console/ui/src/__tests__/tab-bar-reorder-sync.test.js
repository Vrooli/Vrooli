...
vi.mock("../api/workspace", () => ({
    saveWorkspaceLayout: (...args) => mockSaveWorkspaceLayout(...args),
...
const makePanes = (...ids) => ids.map((id) => ({
    sessionId: id,
    name: id,
...
describe("TabBar reorder sync", () => {
    ...
    it("calls saveWorkspaceLayout after drag-drop reorder", async () => {
        render(_jsx(TabBar, { panes: useWorkspaceStore.getState().panes, activePane: "a", onNewTerminal: vi.fn(), onOpenLauncher: vi.fn(), onClosePane: vi.fn() }));
        ...
        const call = mockSaveWorkspaceLayout.mock.calls[0];
        expect(call?.[0]).toHaveProperty("pane_order");
        expect(call?.[0]?.pane_order).toBeInstanceOf(Array);
    });
    it("does not reorder or activate a tab after a touch swipe", () => {
        useWorkspaceStore.setState({ activePane: "b" });
        render(_jsx(TabBar, { panes: useWorkspaceStore.getState().panes, activePane: "b", onNewTerminal: vi.fn(), onOpenLauncher: vi.fn(), onClosePane: vi.fn() }));
        const tabA = screen.getByTestId("tab-a");
