...
import { INPUT_INTENTS } from "../components/terminal/inputGate";

/**
 * Static-assertion tests enforcing the greenfield constraints of the
 * terminal-session refactor. If any of these patterns reappear in the codebase, the
...
const PROD_FILES = ALL_SOURCE_FILES.filter(
  (p) => !/\/__tests__\//.test(p) && !/\.test\.(ts|tsx)$/.test(p),
);
...
describe("greenfield: terminal-session rework", () => {
  it("does not reintroduce useTerminalSocket", () => {
    const offenders = ALL_SOURCE_FILES.filter(
      (p) =>
        !p.endsWith("greenfield-assertions.test.ts") &&
  ...
  it("does not strip DEC mode 2026 on the client (server is authoritative)", () => {
    // The server-side strip lives in api/session/ansi_responder.go. Any UI
  ...
  it("every input source tag at gate.submit / submitInput call sites is a known InputSource", () => {
    ...
    const known = INPUT_INTENTS.filter((intent) => intent !== "control");
