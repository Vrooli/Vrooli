...
import { dispatchGlobalEvent, useGlobalEventStream, } from "../hooks/useGlobalEventStream";
...
class FakeEventSource {
    ...
    addEventListener(type, cb) {
        var _a;
        ((_a = this.listeners)[type] || (_a[type] = [])).push(cb);
    }
...
function conversationEnvelope(id, sessionId, sequence, role = "assistant") {
    return {
        id,
...
describe("dispatchGlobalEvent session_status (Layer 1)", () => {
    it("maps a created event into a full SessionInfo + derived supportsMessagesView", () => {
        ...
        const [session, supportsMessagesView] = onSessionCreated.mock.calls[0];
    ...
    it("derives supportsMessagesView=false for a plain shell (agent none/absent)", () => {
        const onSessionCreated = vi.fn();
        dispatchGlobalEvent({ id: 11, session_id: "ext-2", kind: "session_status", sequence: 0, payload: { action: "created", agent: "none", backend: "standard" } }, undefined, { onSessionCreated });
        const [session, supportsMessagesView] = onSessionCreated.mock.calls[0];
    ...
    it("routes a deleted event to onSessionEnded with reason 'deleted'", () => {
        ...
        dispatchGlobalEvent({ id: 12, session_id: "ext-3", kind: "session_status", sequence: 0, payload: { action: "deleted" } }, undefined, { onSessionEnded });
    ...
    it("routes a terminated event to onSessionEnded with reason 'terminated'", () => {
        const onSessionEnded = vi.fn();
        dispatchGlobalEvent({ id: 13, session_id: "ext-4", kind: "session_status", sequence: 0, payload: { action: "terminated", reason: "expired" } }, undefined, { onSessionEnded });
    ...
    it("ignores an unknown/absent action without invoking any lifecycle callback", () => {
        const onSessionCreated = vi.fn();
        const onSessionEnded = vi.fn();
        dispatchGlobalEvent({ id: 14, session_id: "ext-5", kind: "session_status", sequence: 0, payload: {} }, undefined, { onSessionCreated, onSessionEnded });
