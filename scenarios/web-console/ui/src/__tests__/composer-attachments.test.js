...
function AttachHarness({ onInput = () => ({ status: "sent", seq: 1 }), subscribe, resolve }) {
    const draft = useComposerDraft("sess-att");
    const att = useComposerAttachments();
    const [open, setOpen] = useState(true);
    return (_jsxs(_Fragment, { children: [_jsx("button", { "data-testid": "ext-open", onClick: () => setOpen(true) }), _jsx(FullScreenComposer, { open: open, onClose: () => setOpen(false), draft: draft, onInput: onInput, subscribeInputSettled: subscribe, attachments: att.attachments, onAttachFiles: att.addFiles, onRemoveAttachment: att.removeFile, resolveAttachmentPaths: resolve ?? (async () => att.attachments.map((_, i) => `/up/${i}.png`)), onClearAttachments: att.clearAll })] }));
}
...
describe("FullScreenComposer — staged attachments", () => {
    ...
    it("composes ONE payload = text + resolved paths in order, then clears on ok", async () => {
        ...
        render(_jsx(AttachHarness, { onInput: onInput, subscribe: settlement.subscribe, resolve: resolve }));
    ...
    it("keeps attachments + text and shows error when upload fails", async () => {
        ...
        render(_jsx(AttachHarness, { onInput: onInput, resolve: resolve }));
        ...
        expect(screen.getByTestId("composer-attachment-tray")).toBeTruthy();
        expect(screen.getByTestId("composer-input").value).toBe("with pic");
    });
