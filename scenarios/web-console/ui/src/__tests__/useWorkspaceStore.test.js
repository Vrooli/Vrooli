...
describe("useWorkspaceStore", () => {
    ...
    describe("setPaneColor", () => {
        it("sets a pane's header color", () => {
            ...
            expect(useWorkspaceStore.getState().panes[0]?.headerColor).toBe("#ff7a7a");
        });
    });
    ...
    describe("setPaneGroup", () => {
        const pane = (sessionId, groupId = null, headerColor = "transparent") => ({
            sessionId,
            name: sessionId,
            headerColor,
            themeId: "default",
        ...
        const g1 = { id: "g1", name: "Work", color: "#4dabf7", isCollapsed: false };
    ...
    describe("persist migration v14→v15", () => {
        it("seeds new fields without dropping existing persisted state", () => {
            ...
            if (!migrate)
                throw new Error("migrate function missing");
            const prior = {
            ...
            const migrated = migrate(prior, 14);
    ...
    describe("persist migration v16→v17", () => {
        it("seeds sidebarOriginTab on a pre-origin-tab snapshot and leaves prior state intact", () => {
            ...
            const migrated = migrate(prior, 16);
        ...
        it("does not clobber a persisted origin tab from a future-shaped snapshot", () => {
            ...
            if (!migrate)
                throw new Error("migrate function missing");
            const migrated = migrate({ sidebarOriginTab: "remote" }, 16);
