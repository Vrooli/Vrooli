...
describe("TerminalLauncher", () => {
    ...
    it("renders nothing when closed", () => {
        const { container } = render(_jsx(TerminalLauncher, { open: false, onClose: onClose, onLaunch: onLaunch }));
        expect(container.innerHTML).toBe("");
    });
    it("renders modal with 'New Terminal' title when open", () => {
        render(_jsx(TerminalLauncher, { open: true, onClose: onClose, onLaunch: onLaunch, shortcuts: testShortcuts }));
    ...
    it("calls onLaunch with shortcut command when shortcut is clicked", () => {
        ...
        expect(onLaunch).toHaveBeenCalledWith(expect.objectContaining({ command: "claude --dangerously-skip-permissions" }));
    });
    ...
    it("shows 'Creating session...' when isCreating is true", () => {
        render(_jsx(TerminalLauncher, { open: true, onClose: onClose, onLaunch: onLaunch, shortcuts: testShortcuts, isCreating: true }));
    ...
    it("fetches shortcuts via shortcutsClient.getEffective when no prop provided", async () => {
        ...
        render(_jsx(TerminalLauncher, { open: true, onClose: onClose, onLaunch: onLaunch }));
        await waitFor(() => {
