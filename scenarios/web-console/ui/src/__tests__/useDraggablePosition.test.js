...
describe("useDraggablePosition", () => {
    ...
    it("updates DOM transform directly during drag (no React render lag)", () => {
        ...
        const { result } = renderHook(() => useDraggablePosition({
            isActive: true,
            defaultPosition: DEFAULT_POS,
        }));
        // Attach the mock element to the ref
        act(() => {
            result.current.elementRef.current = el;
        });
        // Simulate pointerDown at (120, 24) — offset within element is (20, 12)
        act(() => {
            result.current.pointerHandlers.onPointerDown(makePointerEvent({ clientX: 120, clientY: 24 }));
        });
        // Move past the drag threshold (6px)
        act(() => {
            result.current.pointerHandlers.onPointerMove(makePointerEvent({ clientX: 140, clientY: 40 }));
        });
        ...
        // Move again
        act(() => {
            result.current.pointerHandlers.onPointerMove(makePointerEvent({ clientX: 200, clientY: 60 }));
        });
    ...
    it("syncs final position to React state on pointerUp", () => {
        ...
        act(() => {
            result.current.pointerHandlers.onPointerUp(makePointerEvent({ clientX: 200, clientY: 60 }));
        });
