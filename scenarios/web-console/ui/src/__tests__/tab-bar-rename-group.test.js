...
const renderTabBar = () => render(_jsx(TabBar, { panes: useWorkspaceStore.getState().panes, activePane: "a", onNewTerminal: vi.fn(), onOpenLauncher: vi.fn(), onClosePane: vi.fn() }));
...
describe("TabBar group quick paths", () => {
    const groupMeta = { id: "g1", name: "Work", color: HEADER_COLORS[0] ?? "#123456", isCollapsed: false };
    beforeEach(() => {
        vi.clearAllMocks();
        useWorkspaceStore.setState({
            panes: makePanes("a", "b"),
    ...
    it("grouped panes get one-tap remove plus the Manage Groups entry point", async () => {
        const panes = useWorkspaceStore.getState().panes.map((p) => p.sessionId === "a" ? { ...p, groupId: "g1" } : p);
