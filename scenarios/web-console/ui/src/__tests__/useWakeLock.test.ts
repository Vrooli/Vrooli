...
function createMockWakeLock() {
  ...
  return {
    ...
    triggerRelease: () => { if (releaseHandler) releaseHandler(); },
  };
}
...
function spyOnVideoCreation() {
  ...
  vi.spyOn(document, "createElement").mockImplementation((tag: string, options?: ElementCreationOptions) => {
    if (tag === "video") {
      const el = originalCreateElement(tag, options) as HTMLVideoElement;
      ...
      el.addEventListener = vi.fn((event: string, handler: EventListenerOrEventListenerObject, opts?: boolean | AddEventListenerOptions) => {
...
describe("useWakeLock", () => {
  ...
  it("handles rejected request gracefully", async () => {
    ...
    expect(debugSpy).toHaveBeenCalledWith(
      expect.stringContaining("[useWakeLock]"),
      expect.anything(),
      expect.anything(),
      expect.anything(),
