...
const mockUpdateHookConfig = vi.fn((patch) => Promise.resolve({
...
vi.mock("../audio-integration", async (importOriginal) => {
    ...
    return {
        ...
        updateTTSConfig: vi.fn((patch) => mockUpdateVoiceConfig(patch)),
        ...
        updateTTSSummarizeConfig: vi.fn((patch) => mockUpdateSummarizeConfig(patch)),
...
describe("TtsSettingsSection", () => {
    ...
    async function renderSection() {
        render(_jsx(TtsSettingsSection, {}));
        await waitFor(() => {
    ...
    it("renders summarize model catalog status", async () => {
        await renderSection();
        const select = screen.getByTestId("summarize-model-select");
