...
const UI_ROOT = resolve(THIS_FILE, "..", "..", "..");
const PUBLIC_DIR = resolve(UI_ROOT, "public", "public");
const INDEX_HTML = readFileSync(resolve(UI_ROOT, "index.html"), "utf8");
function readManifest() {
    return JSON.parse(readFileSync(resolve(PUBLIC_DIR, "site.webmanifest"), "utf8"));
}
...
function resolvesUnderPublic(ref) {
...
describe("public-assets convention: web-console branding/PWA/OG under /public/", () => {
    it("every <link> icon/apple-touch/manifest href resolves under /public/", () => {
        ...
        for (const { rel, href } of refs) {
            expect(resolvesUnderPublic(href), `${rel} href "${href}" must resolve under /public/`).toBe(true);
        }
    });
    it("og:image / twitter:image meta resolve under /public/", () => {
        ...
        for (const { key, content } of refs) {
            expect(resolvesUnderPublic(content), `${key} content "${content}" must resolve under /public/`).toBe(true);
        }
    });
    ...
    it("manifest icon srcs are co-located in /public/ (resolve relative to the manifest URL)", () => {
        ...
        for (const icon of icons) {
            ...
            expect(existsSync(resolve(PUBLIC_DIR, src)), `icon src "${src}" must exist in /public/`).toBe(true);
        }
    });
    it("the manifest <link> carries crossorigin=use-credentials", () => {
        ...
        expect(/rel="manifest"[^>]*crossorigin="use-credentials"/.test(INDEX_HTML)).toBe(true);
    });
});
