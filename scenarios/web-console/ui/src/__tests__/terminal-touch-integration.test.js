...
vi.mock("../hooks/terminal/useTerminalSession", () => {
    const gate = { submit: vi.fn(() => ({ status: "sent", seq: 1 })), dispose: vi.fn(), canAcceptPaste: () => true };
    const submitInput = vi.fn(() => ({ status: "sent", seq: 1 }));
...
vi.mock("../stores/useWorkspaceStore", () => {
    ...
    const useWorkspaceStore = (selector) => selector(baseState);
...
describe("TerminalPane touch integration", () => {
    ...
    it("does not render floating copy button (removed in favor of context menu)", () => {
        ...
        render(_jsx(TerminalPane, { sessionId: "test-session" }));
