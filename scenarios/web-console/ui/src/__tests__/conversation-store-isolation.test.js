...
import { useConversationStore, getSessionConversationEvents, getSessionUnreadCount, } from "../stores/useConversationStore";
function makeEvent(sessionId, sequence, role = "assistant") {
    return {
...
describe("conversation store subscription isolation (Layer 0.1)", () => {
    it("an event in session A does not re-render a subscriber to session B", () => {
        let aRenders = 0;
        let bRenders = 0;
        render(_jsxs(_Fragment, { children: [_jsx(EventsProbe, { sessionId: "A", onRender: () => { aRenders++; } }), _jsx(EventsProbe, { sessionId: "B", onRender: () => { bRenders++; } })] }));
    ...
    it("an event in session A does not re-render session B's unread badge", () => {
        let bBadgeRenders = 0;
        render(_jsx(BadgeProbe, { sessionId: "B", onRender: () => { bBadgeRenders++; } }));
