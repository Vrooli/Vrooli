...
describe("MessageExportDrawer", () => {
  ...
  it("renders nothing while closed", () => {
    render(<MessageExportDrawer open={false} events={twoEvents()} onClose={onClose} />);
  ...
  it("disables copy only for empty output", () => {
    render(<MessageExportDrawer open events={[]} onClose={onClose} />);
  ...
  it("bounds very large previews while keeping the full text for copy", async () => {
    ...
    render(<MessageExportDrawer open events={[big]} onClose={onClose} />);
    ...
    expect((writeText.mock.calls[0]?.[0] as string).length).toBeGreaterThan(30000);
  });
});
