...
export function VoiceInputButton({ state = "idle", mode = "always-on", size = "sm", density = "comfortable", level = 0, timeoutProgress = 0, error, rejectionReason, partialTranscript, onStart, onStop, onPrepare, onDismissError, onTranscribeAnyway, onCancel, onExitPassive, onReleaseMic, onTtsStop, onExportDiagnostic, staleLiveMic, isTtsSpeaking, canExportDiagnostic, backend, className, wrapperClassName, iconClassName, disabled, onPointerCancel, ...props }: VoiceInputButtonProps) {
  ...
  return <div className={`relative shrink-0 ${wrapperClassName ?? ""}`} data-voice-backend={backend}>
    ...
    {visibleError && <div className="absolute left-1/2 top-full z-10 mt-1 flex w-52 -translate-x-1/2 items-start gap-1.5 rounded border border-wc-accent/50 bg-wc-surface-raised px-2 py-1 text-[10px] text-wc-accent shadow-lg"><div role="status" className="min-w-0 flex-1 break-words">{error}</div><button data-testid="voice-input-error-dismiss" type="button" className="touch-target shrink-0 rounded p-0.5 hover:bg-wc-accent/15" aria-label="Dismiss voice input error" onClick={() => { setDismissedError(error); onDismissError?.(); }}><Glyph kind="close" className="h-3 w-3" /></button></div>}
    {active && partialTranscript && <div className="pointer-events-none absolute bottom-full left-1/2 mb-1 max-w-[200px] -translate-x-1/2 overflow-hidden text-ellipsis whitespace-nowrap rounded border border-wc-default bg-wc-surface-raised px-2 py-1 text-[10px] text-wc-text-muted shadow-lg">{partialTranscript}</div>}
    {rejectionReason && onTranscribeAnyway && <button type="button" className="ml-2 text-sm text-wc-accent underline" onClick={onTranscribeAnyway}>Transcribe anyway</button>}
    {(state === "transcribing" && onCancel) || (staleLiveMic && onReleaseMic) || (isTtsSpeaking && onTtsStop) || (canExportDiagnostic && onExportDiagnostic) ? (
      <div role="group" aria-label="Voice recovery actions" className="mt-1 flex flex-wrap justify-center gap-1 text-[10px]">
        {state === "transcribing" && onCancel && <button type="button" className="rounded border border-wc-default px-1.5 py-0.5 text-wc-text-muted hover:bg-wc-surface-input" onClick={onCancel}>Cancel</button>}
        {staleLiveMic && onReleaseMic && <button type="button" className="rounded border border-wc-accent px-1.5 py-0.5 text-wc-accent hover:bg-wc-accent/10" onClick={onReleaseMic}>Release mic</button>}
        {isTtsSpeaking && onTtsStop && <button type="button" className="rounded border border-wc-accent px-1.5 py-0.5 text-wc-accent hover:bg-wc-accent-active/10" onClick={onTtsStop}>Stop speech</button>}
        {canExportDiagnostic && onExportDiagnostic && <button type="button" className="rounded border border-wc-default px-1.5 py-0.5 text-wc-text-muted hover:bg-wc-surface-input" onClick={() => { onExportDiagnostic(); }}>Export diagnostic</button>}
      </div>
    ) : null}
  </div>;
}
