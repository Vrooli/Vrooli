...
import { getVoiceStreamConfig, useVoiceCore, AUDIO_TOOLS_CAPABILITY_SLUG, featureSlug, AudioToolsFeature, } from "../index";
...
let capabilitiesApiPromise;
function loadCapabilitiesApi() {
    capabilitiesApiPromise ?? (capabilitiesApiPromise = import("../../api/capabilities"));
 ...
 */
export async function probeWhisperHealth() {
    const { fetchCapabilities, getCapabilitiesLivenessSnapshot, refreshCapabilitiesLiveness, } = await loadCapabilitiesApi();
...
export function useScenarioVoiceInput(onTranscript) {
    ...
    useEffect(() => {
        ...
        getVoiceStreamConfig()
            ...
            if (cfg.persistentMode !== store.persistentMode)
                store.setPersistentMode(cfg.persistentMode);
            if (cfg.wakeWordEnabled !== store.wakeWordEnabled)
                store.setWakeWordEnabled(cfg.wakeWordEnabled);
            ...
            if (serverSilenceMs > 0) {
                if (serverSilenceMs !== store.segmentSilenceMs)
                    store.setSegmentSilenceMs(serverSilenceMs);
                if (serverSilenceMs !== store.vadSilenceTimeoutMs)
                    store.setVadSilenceTimeoutMs(serverSilenceMs);
            }
        })
            .catch(() => { });
