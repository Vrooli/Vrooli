 ...
 */
function synthWord(formantSegments, seed, amp = 1) {
    const segDur = 0.25;
...
describe("normalizeFeatures (CMVN)", () => {
    it("produces ~zero mean and ~unit variance per coefficient", () => {
        ...
        const feats = engine.extractFeatures(synthWord(WORD_A, 1), SR).data;
...
describe("MfccDtwEngine.extractFeatures", () => {
    it("emits 13-d mfcc-v1 features and does not throw on silence", () => {
        ...
        expect(feats.data[0]?.length).toBe(13);
...
describe("calibrate", () => {
    ...
    it("returns finite mu and floored sigma for >= 2 samples", () => {
        const engine = new MfccDtwEngine();
        const samples = [10, 11, 12].map((s) => engine.extractFeatures(synthWord(WORD_A, s), SR));
        const cal = engine.calibrate(samples);
        if (cal === null)
            throw new Error("expected non-null calibration");
...
describe("separation harness (synthetic words)", () => {
    it("calibrated scores separate same-word from cross-word", () => {
        ...
        const enroll = [20, 21, 22, 23, 24].map((s) => engine.extractFeatures(synthWord(WORD_A, s), SR));
        const cal = engine.calibrate(enroll);
        if (cal === null)
            throw new Error("expected non-null calibration");
        // Held-out same-word takes should score high.
        const sameScores = [30, 31, 32, 33, 34].map((s) => engine.compareBest(engine.extractFeatures(synthWord(WORD_A, s), SR), enroll, 0.7, cal).score);
        // Different-word takes should score low.
        const crossScores = [40, 41, 42, 43, 44].map((s) => engine.compareBest(engine.extractFeatures(synthWord(WORD_B, s), SR), enroll, 0.7, cal).score);
        const median = (xs) => {
