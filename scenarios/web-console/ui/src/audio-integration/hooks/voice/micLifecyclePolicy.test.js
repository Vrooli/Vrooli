...
import { decideMicLifecycle, selectStaleLeases, } from "./micLifecyclePolicy";
describe("decideMicLifecycle", () => {
    ...
    it.each(["pagehide", "freeze"])("releases ALL leases on %s regardless of platform", (event) => {
...
describe("selectStaleLeases", () => {
    const lease = (owner, id = `${owner}-1`) => ({ id, owner });
    const base = { passiveListenerActive: false };
    it("flags an active-recording lease only when the workflow is idle", () => {
        for (const owner of ["voice-stream", "whisper", "web-speech"]) {
            expect(selectStaleLeases({ ...base, leases: [lease(owner)], voiceState: "idle" })).toHaveLength(1);
            // Not flagged while genuinely capturing or settling.
            for (const voiceState of ["preparing", "recording", "listening", "transcribing"]) {
                expect(selectStaleLeases({ ...base, leases: [lease(owner)], voiceState })).toHaveLength(0);
            }
        }
    });
    it("flags a passive lease only when no passive listener is installed", () => {
        expect(selectStaleLeases({ ...base, passiveListenerActive: false, leases: [lease("passive-wake-word")], voiceState: "idle" })).toHaveLength(1);
        expect(selectStaleLeases({ ...base, passiveListenerActive: true, leases: [lease("passive-wake-word")], voiceState: "idle" })).toHaveLength(0);
    });
    it("never flags transient settings-capture owners", () => {
        ...
        expect(selectStaleLeases({ ...base, leases: owners.map((o) => lease(o)), voiceState: "idle" })).toHaveLength(0);
    });
});
