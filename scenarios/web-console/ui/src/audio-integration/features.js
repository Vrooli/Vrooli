...
export var AudioToolsFeature;
(function (AudioToolsFeature) {
    AudioToolsFeature[AudioToolsFeature["UNSPECIFIED"] = 0] = "UNSPECIFIED";
    AudioToolsFeature[AudioToolsFeature["VOICE_INPUT"] = 1] = "VOICE_INPUT";
    AudioToolsFeature[AudioToolsFeature["VOICE_STREAMING"] = 2] = "VOICE_STREAMING";
    AudioToolsFeature[AudioToolsFeature["VOICE_SPEAKER_VERIFICATION"] = 3] = "VOICE_SPEAKER_VERIFICATION";
    AudioToolsFeature[AudioToolsFeature["VOICE_ENROLLMENT"] = 4] = "VOICE_ENROLLMENT";
    AudioToolsFeature[AudioToolsFeature["VOICE_OUTPUT"] = 5] = "VOICE_OUTPUT";
    AudioToolsFeature[AudioToolsFeature["TTS_SUMMARIZATION"] = 6] = "TTS_SUMMARIZATION";
    AudioToolsFeature[AudioToolsFeature["TTS_CACHE"] = 7] = "TTS_CACHE";
    AudioToolsFeature[AudioToolsFeature["TTS_PARAGRAPH_SPLIT"] = 8] = "TTS_PARAGRAPH_SPLIT";
    AudioToolsFeature[AudioToolsFeature["AUDIO_PROVIDER_ROUTING"] = 9] = "AUDIO_PROVIDER_ROUTING";
})(AudioToolsFeature || (AudioToolsFeature = {}));
const FEATURE_SLUGS = {
...
(function assertCoverage() {
    for (const key of Object.keys(AudioToolsFeature)) {
        const numeric = Number(AudioToolsFeature[key]);
        if (!Number.isFinite(numeric))
            continue;
        if (numeric === AudioToolsFeature.UNSPECIFIED)
            continue;
        if (!FEATURE_SLUGS[numeric]) {
            throw new Error(`audio-integration/features.ts: missing slug for AudioToolsFeature.${key} (${numeric}). ` +
