...
describe("shared audio substrate boundary", () => {
    ...
    it("does not retain a private PCM implementation", () => {
        const provider = readFileSync(join(AUDIO_ROOT, "hooks", "voice", "PcmVoiceStreamProvider.ts"), "utf8");
    ...
    it("rejects the deleted legacy provider symbol anywhere in UI source", () => {
        const offenders = sourceFiles(UI_SOURCE).filter((file) => /\bVoiceStreamProvider\b/.test(readFileSync(file, "utf8")));
