...
import { VoiceInputButtonGlyph as Glyph, } from "./VoiceInputButtonGlyph";
...
export function VoiceInputButton({ state = "idle", mode = "always-on", size = "sm", density = "comfortable", level = 0, timeoutProgress = 0, onStart, onStop, onPrepare, onExitPassive, iconClassName, disabled, onPointerCancel, ...props }) {
    const pressStartedAt = useRef(0);
    const pressIntent = useRef("none");
    ...
    const finishPointer = useCallback(() => {
        ...
        if (intent === "stop")
            onStop?.();
        if (intent === "exit-passive")
            onExitPassive?.();
        if (intent === "start" && Date.now() - pressStartedAt.current >= LONG_PRESS_MS)
            onStop?.();
    ...
    const handleClick = useCallback(() => {
        ...
        if (!canInteract)
            return;
        if (active) {
            if (state === "recovering" && onExitPassive)
                onExitPassive();
            else
                onStop?.();
        }
    ...
    return (_jsxs(IconButton, { ...props, "data-testid": props["data-testid"] ?? "voice-input-control", type: "button", "aria-label": accessibleLabel, "aria-pressed": active, disableTooltip: true, disabled: disabled || state === "unavailable", onFocus: onPrepare, onPointerEnter: onPrepare, onPointerDown: handlePointerDown, onPointerUp: finishPointer, onClick: handleClick, onPointerCancel: (event) => { finishPointer(); onPointerCancel?.(event); }, size: size, density: density, variant: "secondary", className: `relative overflow-hidden touch-manipulation ${stateClasses[state]} ${props.className ?? ""}`, children: [_jsx(Glyph, { kind: glyph, className: glyphClass }), active && _jsx("span", { "aria-hidden": "true", className: `pointer-events-none absolute inset-x-0 bottom-0 z-0 rounded-[inherit] transition-[height] duration-75 ${state === "recovering" || mode === "always-on" ? "bg-wc-accent-active/60" : "bg-wc-accent/70"}`, style: { height: `${Math.round(normalizedLevel * 100)}%` } }), state === "recording" && mode === "timeout" && _jsx("svg", { "aria-hidden": "true", className: "pointer-events-none absolute left-1/2 top-1/2 z-10 aspect-square h-[calc(100%-4px)] min-h-6 max-h-8 -translate-x-1/2 -translate-y-1/2 -rotate-90 overflow-visible", viewBox: "0 0 44 44", children: _jsx("circle", { cx: "22", cy: "22", r: "18", fill: "none", stroke: "currentColor", strokeWidth: "3", className: "text-wc-accent/80", strokeDasharray: RING_CIRCUMFERENCE, strokeDashoffset: RING_CIRCUMFERENCE * (1 - progress), strokeLinecap: "round" }) }), _jsx("span", { className: "sr-only", children: labels[state] })] }));
}
