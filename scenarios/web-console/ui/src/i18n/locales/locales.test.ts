...
const catalogModules = import.meta.glob<{ default: Catalog }>("./*.json", {
  eager: true,
});
...
const stripPluralSuffix = (key: string): string => key.replace(PLURAL_SUFFIX, "");

const isSentinelKey = (key: string): boolean => key.startsWith("_");
...
const logicalKeys = (catalog: Catalog): Set<string> => {
 ...
 */
const tokensPerBase = (catalog: Catalog): Map<string, Set<string>> => {
  const flat = flatten(catalog);
