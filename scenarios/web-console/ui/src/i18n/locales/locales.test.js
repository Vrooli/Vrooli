 ...
 *
 * The same sentinel skip lives in `scripts/gen-strings.mjs` so they don't
 * leak into `strings.generated.ts`; the convention is duplicated by intent
 * so each consumer documents its own contract.
 */
...
const TOKEN_RE = /\{\{(\w+)\}\}/g;
const stripPluralSuffix = (key) => key.replace(PLURAL_SUFFIX, "");
const isSentinelKey = (key) => key.startsWith("_");
const isCatalogObject = (value) => typeof value === "object" && value !== null;
const flatten = (obj, prefix = "") => {
    ...
    for (const [k, v] of Object.entries(obj)) {
        ...
        else if (isCatalogObject(v)) {
            Object.assign(out, flatten(v, path));
        }
    }
    return out;
};
const logicalKeys = (catalog) => {
 ...
 */
const tokensPerBase = (catalog) => {
    const flat = flatten(catalog);
    const byBase = new Map();
    for (const [key, value] of Object.entries(flat)) {
        const base = stripPluralSuffix(key);
        const tokens = byBase.get(base) ?? new Set();
...
describe("locale catalogs", () => {
    ...
    it("LOCALE_CODES matches the locale JSON files on disk", () => {
        ...
        expect(new Set(LOCALE_CODES)).toEqual(new Set(Object.keys(catalogs)));
    });
    ...
    it("every logical key uses the same interpolation tokens across locales", () => {
        ...
        // `ja.notifications.summary` would render the literal with no number and
        ...
        for (const [locale, catalog] of Object.entries(catalogs)) {
            ...
            for (const [base, refTokens] of referenceTokens) {
                const localTokens = candidateTokens.get(base) ?? new Set();
                expect([...localTokens].sort(), `${locale}.json key '${base}' has different interpolation tokens than en.json`).toEqual([...refTokens].sort());
            }
        }
    });
