...
export function setDeviceLabel(label) { localStorage.setItem(DEVICE_LABEL_KEY, label.trim()); }
function defaultDeviceLabel() {
    ...
    if (/iPhone/i.test(agent))
        return "iPhone";
    if (/iPad|Tablet/i.test(agent))
        return "Tablet";
    if (/Android/i.test(agent) && Math.min(width, height) < 900)
        return "Android phone";
    if (Math.min(width, height) > 0 && Math.min(width, height) < 600)
        return "Phone";
    if (Math.min(width, height) > 0 && Math.min(width, height) < 900)
        return "Tablet";
    return "Desktop";
}
