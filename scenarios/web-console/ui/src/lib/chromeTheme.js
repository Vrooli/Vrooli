...
let config = { enabled: false, ownerSessionId: null, fallbackColor: null };
...
function resolveColor() {
    if (!config.enabled || !config.ownerSessionId)
        return null;
    const d = detected.get(config.ownerSessionId);
    if (d && isHexColor(d))
        return d;
    if (config.fallbackColor && isHexColor(config.fallbackColor))
...
function apply() {
    ...
    if (initialized && color === appliedColor)
        return; // change-guard: no-op tick
    appliedColor = color;
    initialized = true;
...
export const chromeTheme = {
     ...
     */
    setDetected(sessionId, color) {
        if (color && isHexColor(color)) {
            if (detected.get(sessionId) === color)
                return; // identical — skip apply
        ...
        if (sessionId === config.ownerSessionId)
            apply();
    },
