...
export type ChromePalette = Record<string, string>;
...
function rgbToTriple(rgb: Rgb): string {
...
function tone(seed: Oklch, lightness: number, chromaScale = 0.55): Rgb {
  return oklchToRgb({
...
function textTone(surface: Rgb, seed: Oklch, target: number, polarity: "light" | "dark", baseL: number): Rgb {
  ...
  for (let i = 0; i <= 90; i += 1) {
    ...
    best = candidate;
    if (contrastRatioRgb(candidate, surface) >= target) return candidate;
  }
  return best;
}

function accentTone(surface: Rgb, polarity: "light" | "dark"): Rgb {
  ...
  for (let i = 0; i <= 80; i += 1) {
    ...
    for (const c of [0.12, 0.1, 0.08, 0.06]) {
      ...
      best = candidate;
      if (contrastRatioRgb(candidate, surface) >= 3) return candidate;
    }
  }
  return best;
}

function readableTextPair(background: Rgb): Rgb {
...
export function deriveChromePalette(seedHex: string): ChromePalette {
...
export function paletteTripleToHex(value: string): string {
