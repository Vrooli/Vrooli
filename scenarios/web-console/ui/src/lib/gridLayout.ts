...
export const reconcileTrackFractions = (
  ...
  if (!Number.isFinite(nextCount) || nextCount <= 0) return [1];
...
export const buildGridTrackTemplate = (
  ...
  if (fractions.length <= 1) return "minmax(0, 1fr)";

  const safeSplitter =
    Number.isFinite(splitterSize) && splitterSize > 0 ? splitterSize : 6;
 ...
 */
export const updateAdjacentFractions = ({
  startValues,
  index,
  delta,
  containerSize,
  ...
  if (index < 0 || index >= startValues.length - 1) return startValues;

  const safeSplitter =
    Number.isFinite(splitterSize) && splitterSize > 0 ? splitterSize : 8;
  ...
  if (pairSize <= 0) return startValues;

  const boundedMin = Math.max(48, Math.min(minTrackPx, pairSize / 2 - 1));
  if (!Number.isFinite(boundedMin) || boundedMin <= 0) return startValues;

  const currentPx = current * usableSize;
  const nextCurrentPx = clamp(
    currentPx + delta,
    boundedMin,
    pairSize - boundedMin,
  );
