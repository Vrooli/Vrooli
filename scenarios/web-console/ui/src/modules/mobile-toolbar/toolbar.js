...
export function initializeMobileToolbar(getActiveTabFn, sendKeyToTerminalFn) {
  ...
  const persistedDebugMode =
    localStorage.getItem("debugToolbarEnabled") === "true";
  ...
  const shouldEnableDebug = debugToggle
    ? debugToggle.checked
    : persistedDebugMode;
  const initialMode =
    window.__keyboardToolbarMode ||
    (shouldEnableDebug ? "floating" : "disabled");
  ...
  const setupMode = (mode, forceDebug = false) => {
    ...
    if (!isMobile && !debugModeActive) {
      ...
      if (debugToolbar) {
        console.log(
          "Toolbar mode set to: disabled (desktop - use debug toggle to test)",
        );
      }
      return;
    }
  ...
  if (debugToolbar) {
    console.log(
      `Mobile toolbar initialized with mode: ${initialMode} (${deviceType})`,
    );
  }
