...
export function configureAIIntegration({ getActiveTab, transmitInput, queueInput, startSession }) {
  getActiveTabFn = typeof getActiveTab === 'function' ? getActiveTab : (() => null)
  transmitInputFn = typeof transmitInput === 'function' ? transmitInput : (() => false)
  queueInputFn = typeof queueInput === 'function' ? queueInput : (() => {})
  startSessionFn = typeof startSession === 'function' ? startSession : (() => Promise.resolve())
}
...
async function initAICommandAsync() {
  try {
    const { generateAICommand, showSnackbar, initializeMobileToolbar } = await import('./mobile-toolbar.js')
    ...
    const mobileToolbar = initializeMobileToolbar(getActiveTabLocal, sendKeyToTerminal)
    ...
    if (elements.aiGenerateBtn && elements.aiCommandInput) {
      elements.aiGenerateBtn.addEventListener('click', async () => {
        ...
        try {
          const result = await generateAICommand(prompt, getActiveTabLocal, sendKeyToTerminal)
