...
describe("useTtsPlaybackController", () => {
    ...
    it("summarizes the next message when summarized playback is the active preference", async () => {
        ...
        const applySummarizeResult = vi.fn((sessionId, eventId, speechParagraphs) => {
        ...
        await waitFor(() => {
            expect(speakText).toHaveBeenCalledWith("sess-1", "Original 2", ["Summarized 2"], { eventId: "e2", version: "active", initiatedBy: "manual" });
        });
    });
    ...
    it("normalizes unavailable summarize transport failures for the banner", async () => {
        ...
        await waitFor(() => {
            expect(onSummarizeFailed).toHaveBeenCalledWith("sess-1", "e-fail", expect.stringContaining("audio-tools is unavailable"));
        });
    ...
    it("normalizes summarize timeout failures separately from availability", async () => {
        ...
        await waitFor(() => {
            expect(onSummarizeFailed).toHaveBeenCalledWith("sess-1", "e-timeout", expect.stringContaining("timed out"));
        });
    ...
    it("normalizes missing summarize model failures separately from availability", async () => {
        ...
        await waitFor(() => {
            expect(onSummarizeFailed).toHaveBeenCalledWith("sess-1", "e-missing-model", expect.stringContaining("model is not installed"));
        });
        expect(onSummarizeFailed.mock.calls[0]?.[2]).not.toContain("audio-tools is unavailable");
    });
    it("carries original-mode preference across messages", async () => {
        ...
        await waitFor(() => {
            expect(speakText).toHaveBeenLastCalledWith("sess-1", "Message 2", ["Original 2"], { eventId: "e2", version: "original", initiatedBy: "manual" });
        });
    });
    ...
    it("auto-plays an incoming active-pane assistant event when intent is continuous", async () => {
        ...
        await waitFor(() => {
            expect(speakText).toHaveBeenCalledWith("sess-1", event.text, event.speechParagraphs, { eventId: "e-live", version: "active", initiatedBy: "auto" });
        });
    ...
    it("queues an assistant message that arrives mid-playback and speaks it after the current one", async () => {
        ...
        await waitFor(() => expect(speakText).toHaveBeenCalledWith("sess-1", e1.text, e1.speechParagraphs, { eventId: "e1", version: "active", initiatedBy: "auto" }));
        ...
        await waitFor(() => expect(speakText).toHaveBeenCalledWith("sess-1", e2.text, e2.speechParagraphs, { eventId: "e2", version: "active", initiatedBy: "auto" }));
    });
    it("clears the pending auto-play queue on an explicit user stop", async () => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 0));
        expect(speakText).toHaveBeenCalledTimes(1);
        expect(speakText).not.toHaveBeenCalledWith("sess-1", e2.text, e2.speechParagraphs, expect.objectContaining({ eventId: "e2" }));
    });
