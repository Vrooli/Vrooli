...
func TestConnectHandlerSettings(t *testing.T) {
	...
	if resp, err := h.GetSessionDefaults(nil, connect.NewRequest(&settingsv1.GetSessionDefaultsRequest{})); err != nil || resp.Msg.Defaults.DefaultBackend != "standard" {
		t.Fatalf("get: %#v %v", resp, err)
	}
	backend := "tmux"
	if resp, err := h.UpdateSessionDefaults(nil, connect.NewRequest(&settingsv1.UpdateSessionDefaultsRequest{DefaultBackend: &backend, DefaultPolicy: &settingsv1.ExpirationPolicy{Mode: "never"}})); err != nil || resp.Msg.Defaults == nil {
	...
	for _, in := range []error{ErrInvalidArgument, errors.New("db")} {
		...
		_, err := h.UpdateSessionDefaults(nil, connect.NewRequest(&settingsv1.UpdateSessionDefaultsRequest{}))
		if err == nil {
