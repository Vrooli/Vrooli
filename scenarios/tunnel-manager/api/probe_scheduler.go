...
func NewProbeScheduler(probeSvc *ProbeService, interval time.Duration) *ProbeScheduler {
	if interval <= 0 {
...
func (ps *ProbeScheduler) runOnce(ctx context.Context) {
	...
	_, err := ps.probeSvc.RunAll(probeCtx)
	...
	if err != nil {
		log.Printf("probe scheduler: cycle failed: %v", err)
	}
}
