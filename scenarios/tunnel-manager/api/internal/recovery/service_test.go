...
// host that has the managed cloudflared resource (the default for every test that
...
func newEngine(t *testing.T, health *fakeHealth, runner *mocks.FakeCmdRunner, repo *fakeRepo, cfg recovery.Config) recovery.Service {
	...
	return recovery.NewService(repo, health, fakePresence{present: true}, recovery.NewControlPlaneLifecycle(runner.Run), clk, cfg, noopSleep)
}
...
func TestRecover_SuccessRestartsAndResets(t *testing.T) {
	...
	require.Equal(t, 1, runner.CallCount(), "one managed-service restart")
	require.Equal(t, []string{"resource", "restart", "cloudflared"}, runner.Calls[0].Args)
...
func TestRecover_CircuitOpensAfterMaxFailures(t *testing.T) {
	...
	// Force bypasses the breaker and requests one managed-service restart.
...
func TestRecover_IdempotentWhileInFlight(t *testing.T) {
	...
	clk := scheduletest.New(time.Date(2026, 5, 1, 12, 0, 0, 0, time.UTC))
	svc := recovery.NewService(repo, health, fakePresence{present: true}, recovery.NewControlPlaneLifecycle(runner.Run), clk, recovery.Config{ReadyPollAttempts: 100}, noopSleep)
	...
	require.Equal(t, 1, runner.CallCount(), "twice == once: a single recovery attempt, not two")
}
...
func TestEvaluate_DormantWhenNoCloudflaredUnit(t *testing.T) {
	...
	health := &fakeHealth{ready: false}
	clk := scheduletest.New(time.Date(2026, 5, 1, 12, 0, 0, 0, time.UTC))
	svc := recovery.NewService(repo, health, fakePresence{present: false}, recovery.NewControlPlaneLifecycle(runner.Run), clk, recovery.Config{ConsecutiveFailures: 1}, noopSleep)
...
func TestEvaluate_ResumesWhenUnitAppears(t *testing.T) {
	...
	svc := recovery.NewService(repo, health, presence, recovery.NewControlPlaneLifecycle(runner.Run), clk, recovery.Config{}, noopSleep)
...
func TestExecuteRecovery_UsesManagedLifecycle(t *testing.T) {
	repo := &fakeRepo{}
	runner := &mocks.FakeCmdRunner{}
	health := &fakeHealth{ready: true}
	svc := newEngine(t, health, runner, repo, recovery.Config{})
	...
	require.Equal(t, recovery.OutcomeSuccess, outcome)
	require.Equal(t, []string{"resource", "restart", "cloudflared"}, runner.Calls[0].Args)
	require.Contains(t, evt.Details, "managed-service")
}

func TestExecuteRecovery_RestartFailureIsFatal(t *testing.T) {
	repo := &fakeRepo{}
	// The managed-resource restart fails — the recovery fails.
	runner := &mocks.FakeCmdRunner{Err: errors.New("boom")}
	health := &fakeHealth{ready: false}
	svc := newEngine(t, health, runner, repo, recovery.Config{})
	...
	require.NoError(t, err)
	require.Equal(t, recovery.OutcomeFailure, outcome)
	require.Equal(t, 1, runner.CallCount(), "managed-service restart attempted once")
}
