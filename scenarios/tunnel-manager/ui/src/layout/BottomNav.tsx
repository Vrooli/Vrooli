...
export function BottomNav() {
  ...
  return (
    <nav
      ...
      className="flex min-w-0 shrink-0 items-stretch justify-around overflow-hidden border-t border-app-border bg-app-surface pb-safe shadow-[0_-6px_18px_rgba(15,23,42,0.06)] md:hidden"
    >
      {NAV_ITEMS.map((item) => (
        <NavLink
          key={item.path}
          ...
          className={({ isActive }) =>
            [
              "min-h-11 min-w-0 flex flex-1 items-center justify-center break-words px-1 py-3 text-center text-[11px] font-medium",
