...
`template-manager orient tunnel-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient tunnel-manager
...
```bash
business-health wizard start  # (was: prd generate) tunnel-manager \
...
```bash
business-health wizard apply  # (was: requirements generate) tunnel-manager \
...
```bash
react-component-library components ingest tunnel-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> tunnel-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario tunnel-manager
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> tunnel-manager <adopted-path>
```
