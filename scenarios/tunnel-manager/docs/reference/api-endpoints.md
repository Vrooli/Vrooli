...
### Notes (CRUD reference) — example domain (removed by `vrooli scenario detemplate`)
...
#### `POST /vrooli.tunnel_manager.v1.notes.NotesService/ListNotes`
...
| **CLI** | `tunnel-manager notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.tunnel_manager.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.tunnel_manager.v1.notes.NotesService/CreateNote`
...
| **CLI** | `tunnel-manager notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.tunnel_manager.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.tunnel_manager.v1.notes.NotesService/GetNote`
...
| **CLI** | `tunnel-manager notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.tunnel_manager.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `tunnel-manager notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/tunnel-manager/v1/notes/notes.proto`.
