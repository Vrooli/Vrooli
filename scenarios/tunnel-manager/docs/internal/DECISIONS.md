...
| 2026-06-21 | **Tunnel-presence self-gate on `cloudflared.service` unit existence**, checked at the top of every `Evaluate()`. | Default-on recovery must not flap on a tunnel-less host. Gating on live `/ready` would disable recovery exactly when it's needed (ready is false during an outage); unit presence is stable regardless of running state. | New `UnitPresence` seam (`systemctl list-unit-files cloudflared.service`); when absent the engine logs `recovery dormant`, stays idle, counts no failure, attempts no restart. Picked up on the next tick if cloudflared is installed later. | Revisit if a host runs cloudflared without a systemd unit (e.g. container) — would need a different presence probe. |
| 2026-06-21 | **`reset-failed` before `restart`** in `executeRecovery` (reset-failed non-fatal; restart failure = recovery failure). | After cloudflared flaps past systemd `StartLimitBurst`, a bare `systemctl restart` is rejected until the start-limit is cleared — the exact slow/hung/flap-exhausted case TM adds value over systemd's own `Restart=on-failure`. | Recovery succeeds even post-StartLimitBurst; a healthy unit's no-op `reset-failed` error is recorded for forensics and ignored. | Revisit only if a future action (e.g. config re-push) supersedes restart. |
| 2026-06-21 | **The cloudflared-restart privilege lives in a safeguard, not the cloudflared tool.** | Tool = binary presence; privilege/policy = a host-level, visudo-validated, risk-surfaced grant that outlives any one scenario. Conflating them would put a sudoers write behind a binary installer. | New `cloudflared_recovery_privileges` safeguard writes `/etc/sudoers.d/tunnel-manager` (0440, visudo-validated, exact argv, no wildcards) during `sudo vrooli setup`; self-gates on cloudflared presence, self-promotes to required under root. Sudo paid once, at setup. | Revisit if TM ever needs a broader systemd grant (keep it minimal). |

## Superseded Decisions

| Date | Superseded Decision | Replacement | Details |
|---|---|---|---|
| 2026-06-21 | **Auto-recovery scheduler opt-in (`TUNNEL_MANAGER_RECOVERY_SCHEDULER_ENABLED`, default-off)** — the 2026-06-18 "LIVE from day one" decision was wired as opt-in pending the autoheal handover. | Opt-out (default-on) `TUNNEL_MANAGER_RECOVERY_SCHEDULER_DISABLED` + sudoers safeguard. | The detection engine was always live; only the background driver + sudo privilege were missing. See the 2026-06-21 rows above. The "single authoritative owner of cloudflared restart" decision (2026-06-18) stands and is now actually true. |
