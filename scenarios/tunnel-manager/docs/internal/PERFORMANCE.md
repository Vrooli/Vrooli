...
| Recovery evaluation cadence | `TUNNEL_MANAGER_RECOVERY_EVALUATE_INTERVAL`, default `1m`, default-on scheduler (opt out with `TUNNEL_MANAGER_RECOVERY_SCHEDULER_DISABLED=1`) | Recovery can restart cloudflared, so background actuation is bounded by cadence + backoff + circuit breaker, but a tunnel control plane that watches the tunnel die without acting is useless — hence default-on. | `recovery` |
| Post-restart readiness poll | `ReadyPollAttempts × ReadyPollInterval`, default `60 × 1s` = 60s | After `systemctl restart cloudflared` the new process must re-register HA connections with the Cloudflare edge before `/ready` returns 200. That reconnect was measured live at ~28s and the edge can throttle it toward 40s+, so the budget must comfortably exceed it — a tighter window misclassifies a succeeding recovery as a failure. | `recovery` |
