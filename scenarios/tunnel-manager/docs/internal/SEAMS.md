...
| **Production wiring** | `internal/config/production.go::NewProductionService(...)` wires a `CredentialStore` plus a resolving `IngressClient` that re-resolves Cloudflare credentials for each remote read/push before constructing `NewCFClient` over `internal/httpc.Doer`. Canonical `CLOUDFLARE_*` env values are read-only runtime overrides; otherwise credentials come from scenario-scoped then shared user secret files under the operator runtime home. Legacy `CF_*` aliases are intentionally not accepted. Both `handlers/config/module.go` and `handlers/exposure/module.go` use this builder; exposure wraps the resulting config service with `ingressAdapter{cfg}.Reconcile`. |
