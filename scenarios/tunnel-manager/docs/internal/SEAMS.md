...
| **Production wiring** | `internal/config/production.go::NewProductionService(...)` resolves Cloudflare credentials through the config-domain `CredentialStore` seam. Canonical `CLOUDFLARE_*` env values are read-only runtime overrides; otherwise credentials come from scenario-scoped then shared user secret files under the operator runtime home. Legacy `CF_*` aliases are intentionally not accepted. The builder constructs `NewCFClient` over `internal/httpc.Doer` and wires the routes manifest reader plus local-mode runner. Both `handlers/config/module.go` and `handlers/exposure/module.go` use this builder; exposure wraps the resulting config service with `ingressAdapter{cfg}.Reconcile`. |
