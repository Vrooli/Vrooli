...
Requirements trace back to operational targets defined in `PRD.md`:
- **P0 targets** (OT-P0-001 through OT-P0-010): Route manifest, port compliance, tunnel health, liveness probes, auto-recovery, CLI commands
- **P1 targets** (OT-P1-001 through OT-P1-009): Cloudflare API, local config, mode switching, Prometheus scraping, web dashboard, recovery log, failure classification
- **P2 targets** (OT-P2-001 through OT-P2-008): Multi-tunnel, auto-registration, webhooks, analytics

## Auto-sync

Requirements are auto-synced with PRD operational targets by `prd-control-tower`. When OTs change in the PRD, run:
```bash
...
prd-control-tower requirements fix tunnel-manager --json
```
