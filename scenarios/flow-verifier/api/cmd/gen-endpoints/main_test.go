...
func TestCrossCheck_PassesWhenRegistered(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		...
		{ID: "y_alias", CLIMapping: &module.CLIMapping{Command: "flow-verifier notes ll"}},
...
func TestBuildCLICommands_ResolvesEndpointIDs(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "health", CLIMapping: &module.CLIMapping{Command: "flow-verifier status"}},
		{ID: "notes_list", CLIMapping: &module.CLIMapping{Command: "flow-verifier notes list"}},
	}
	registered := []registeredCommand{
		{Name: "configure"},
		{Name: "notes list"},
