...
```bash
vrooli scenario port flow-verifier UI_PORT
```
...
flow-verifier notes create --title "First note" --body "Hello"
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port flow-verifier API_PORT)
curl -s "http://localhost:${API_PORT}/health"
curl -s -X POST "http://localhost:${API_PORT}/vrooli.flow_verifier.v1.notes.NotesService/ListNotes" \
