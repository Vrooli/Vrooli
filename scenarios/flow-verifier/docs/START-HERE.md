...
`template-manager orient flow-verifier`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient flow-verifier
...
```bash
cat > /tmp/prd_context_flow-verifier.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) flow-verifier \
  --context-file /tmp/prd_context_flow-verifier.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate flow-verifier --json
```
...
```bash
cat > /tmp/requirements_context_flow-verifier.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) flow-verifier \
  --context-file /tmp/requirements_context_flow-verifier.md \
  --json
```
...
```bash
react-component-library components ingest flow-verifier <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> flow-verifier
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario flow-verifier
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> flow-verifier <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/flow-verifier/v1/<domain>/`.
...
1. Add proto messages and a service under
   `packages/proto/schemas/flow-verifier/v1/<domain>/`, then run
