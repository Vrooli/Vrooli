...
`vrooli scenario orient flow-verifier`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient flow-verifier
...
```bash
prd-control-tower prd generate flow-verifier \
...
```bash
prd-control-tower prd validate flow-verifier --json
```
...
```bash
prd-control-tower requirements generate flow-verifier \
...
```bash
prd-control-tower requirements validate flow-verifier --json
```
