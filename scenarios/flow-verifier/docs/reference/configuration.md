...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/flow-verifier.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `flow-verifier` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/flow-verifier/config.json`
3. `~/.vrooli/config/flow-verifier/config.json`
4. `~/.config/vrooli/flow-verifier/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port flow-verifier API_PORT`)
