...
envelope (`packages/proto/schemas/flow-verifier/v1/errors/errors.proto`):
...
| **CLI** | `flow-verifier status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/flow-verifier/v1/health/health.proto`
...
### `POST /vrooli.flow_verifier.v1.notes.NotesService/ListNotes`
...
| **CLI** | `flow-verifier notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.flow_verifier.v1.notes.NotesService/ListNotes" \
...
### `POST /vrooli.flow_verifier.v1.notes.NotesService/CreateNote`
...
| **CLI** | `flow-verifier notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.flow_verifier.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
### `POST /vrooli.flow_verifier.v1.notes.NotesService/GetNote`
...
| **CLI** | `flow-verifier notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.flow_verifier.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `flow-verifier notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/flow-verifier/v1/notes/notes.proto`.
