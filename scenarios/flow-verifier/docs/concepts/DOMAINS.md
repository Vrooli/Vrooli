...
| runs | Persist a row per verification: status, hashes, counterexample, duration. | Repository / store | `verification_runs` SQLite table. | API, CLI | RUNS-001, RUNS-002, RUNS-003 | `api/internal/runs/`, `api/internal/<domain>/migrations/`, `cli/domains/runs/` (planned) |
