...
```bash
vrooli scenario status flow-verifier
make stop
vrooli scenario start flow-verifier --clean-stale
```
...
flow-verifier status --auto-start

# Or override the API base for this invocation
flow-verifier status --api-base "http://localhost:$(vrooli scenario port flow-verifier API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/flow-verifier/`.
...
The default route is `${SCENARIO_DATA_DIR}/flow-verifier.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/flow-verifier.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/flow-verifier.db}")"
```
...
The generated package paths follow `packages/proto/gen/go/flow-verifier/v1/<domain>/`.
