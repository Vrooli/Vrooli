...
try:
    ...
    inputs = {}
envelope={"program":"knowledge-observatory.learning-read","version":"1","status":"failed","phase":"validate","inputs":{},
...
def step_collect():
    ...
    try:
        handles["learning"]=vrooli_memory.learning.measure(scope="knowledge-observatory-usage",rows="cohorts",**inputs)
        envelope["evidence"].append("vrooli-memory/learning/measure")
    except Exception as exc:
        status,klass=classify_transport(exc)
        if "scope is not provisioned" in str(exc): klass="scope_unprovisioned"
        handles["reason"]="scenario_unreachable" if klass=="scenario_unreachable" else "unreliable:"+klass
        envelope["errors"].append({"class":klass,"detail":str(exc)[:160],"where":"collect"})
    return "act"
def step_act():
    envelope["phase"]="act"
