...
type ingestHealthResponse struct {
	RunnerIntervalMS   int64  `json:"runner_interval_ms"`
...
func (s *Server) handleDeleteDocument(w http.ResponseWriter, r *http.Request) {
	...
	req.Collection = normalizeKnowledgeCollection(req.Collection)
	req.DocumentID = strings.TrimSpace(req.DocumentID)
	req.ExternalID = strings.TrimSpace(req.ExternalID)

	if req.Namespace == "" {
		s.respondError(w, http.StatusBadRequest, "namespace is required")
		return
	}
	if req.DocumentID == "" && req.ExternalID == "" {
		s.respondError(w, http.StatusBadRequest, "document_id or external_id is required")
		return
	}

	if req.DocumentID == "" && req.ExternalID != "" && s.metadata != nil {
		mapping, ok, err := s.metadata.LookupExternalIDMapping(r.Context(), req.Namespace, req.ExternalID, "document")
		if err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to resolve external_id")
			return
		}
		if ok && strings.TrimSpace(mapping.DocumentID) != "" {
			req.DocumentID = strings.TrimSpace(mapping.DocumentID)
		}
	}
	if req.DocumentID == "" {
		s.respondError(w, http.StatusNotFound, "document mapping not found for external_id")
		return
	}
	...
	filter := map[string]interface{}{
		"must": []map[string]interface{}{
			...
			{"key": "document_id", "match": map[string]interface{}{"value": req.DocumentID}},
		},
	}
	points, err := s.scrollCollectionPoints(r.Context(), req.Collection, 100000, false, filter)
	if err != nil {
		s.respondError(w, http.StatusInternalServerError, "failed to inspect document points")
		return
	}
	...
	if !req.DryRun && len(ids) > 0 {
		if err := s.deletePointsByIDBatch(r.Context(), req.Collection, ids); err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to delete document points")
			return
		}
		deleted = len(ids)
	}
...
func (s *Server) handleIngestHealth(w http.ResponseWriter, r *http.Request) {
	response := ingestHealthResponse{
		RunnerIntervalMS: ingestRunnerIntervalMS(s),
		Status:           "healthy",
...
`)
	if err != nil {
		s.respondError(w, http.StatusInternalServerError, "failed to query ingest jobs")
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&status, &count); err != nil {
			s.respondError(w, http.StatusInternalServerError, "failed to parse ingest jobs")
			return
		}
		switch strings.TrimSpace(status) {
		case "pending":
			response.PendingJobs = count
		case "running":
			response.RunningJobs = count
		case "failure":
			response.FailedJobs = count
		case "success":
			response.SuccessfulJobs = count
		}
	}
...
`).Scan(&pendingCreatedAt); err == nil && pendingCreatedAt != nil {
		age := time.Since(pendingCreatedAt.UTC()).Milliseconds()
		oldestPendingAgeMS = &age
	}
	response.OldestPendingAgeMS = oldestPendingAgeMS

	if response.PendingJobs > 25 || response.FailuresLast24H > 10 {
		response.Status = "degraded"
	} else if response.PendingJobs > 0 || response.FailuresLast24H > 0 {
...
func ingestRunnerIntervalMS(s *Server) int64 {
	if s == nil || s.ingestJobRunner == nil {
		return 500
	}
	if s.ingestJobRunner.Interval <= 0 {
		return 500
	}
	return s.ingestJobRunner.Interval.Milliseconds()
}
...
func (s *Server) pruneDocumentExtraChunks(ctx context.Context, collection, namespace, documentID string, keepIDs []string) (int, error) {
	namespace = strings.TrimSpace(namespace)
	documentID = strings.TrimSpace(documentID)
	collection = normalizeKnowledgeCollection(collection)
	...
	points, err := s.scrollCollectionPoints(ctx, collection, 100000, false, filter)
	if err != nil {
		return 0, err
	}
	...
	deletes := make([]string, 0, 16)
	for _, point := range points {
		if _, ok := keep[point.ID]; ok {
			continue
		}
		deletes = append(deletes, point.ID)
	}
	if len(deletes) == 0 {
		return 0, nil
	}
	if err := s.deletePointsByIDBatch(ctx, collection, deletes); err != nil {
		return 0, err
	}
