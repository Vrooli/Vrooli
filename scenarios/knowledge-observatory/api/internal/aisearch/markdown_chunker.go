...
// chunk with a small token overlap of the previous one. It implements
// github.com/vrooli/aisearch-go.Chunker.
type MarkdownChunker struct{}

var _ pkg.Chunker = MarkdownChunker{}
...
func (MarkdownChunker) Chunk(doc pkg.SourceDoc) ([]pkg.Chunk, error) {
	units := parseUnits(doc.Body)
	if len(units) == 0 {
		return nil, nil
	}

	var chunks []pkg.Chunk
	var cur []unit
	curTokens := 0
...
func newChunk(doc pkg.SourceDoc, units []unit, index int) pkg.Chunk {
...
// self-contained (Anthropic-style contextual retrieval). It implements
// github.com/vrooli/aisearch-go.EmbeddingTextComposer.
type ContextualComposer struct{}

var _ pkg.EmbeddingTextComposer = ContextualComposer{}
...
func (ContextualComposer) Compose(chunk pkg.Chunk) string {
