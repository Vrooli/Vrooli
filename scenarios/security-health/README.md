...
- The bare-minimum settings surface (currently just locale + theme).

> The template `notes` worked-example domain (proto, API, CLI, UI) has
> already been removed (Gate 7); the real domains are `validation`,
> `dependencies`, `reindex`, and `health`.
...
`test-genie execute security-health --preset comprehensive` directly for
