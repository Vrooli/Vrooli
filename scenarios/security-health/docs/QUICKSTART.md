...
security-health notes create --title "First note" --body "Hello"
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port security-health API_PORT)
curl -s "http://localhost:${API_PORT}/health"
curl -s -X POST "http://localhost:${API_PORT}/vrooli.security_health.v1.notes.NotesService/ListNotes" \
