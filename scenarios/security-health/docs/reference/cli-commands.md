...
`~/.vrooli/bin/security-health`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start security-health` if the API is unreachable |
...
```bash
security-health status
security-health status --json
```

### `security-health configure <key> <value>`
...
```bash
security-health configure api_base
```
...
```bash
security-health notes list
security-health notes list --json
```

### `security-health notes create --title <title> [--body <body>]`
...
```bash
security-health notes create --title "First note" --body "Hello world"
```
...
```bash
security-health notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start security-health`" is good.
