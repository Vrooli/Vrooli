...
`vrooli scenario orient security-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient security-health
...
```bash
cat > /tmp/prd_context_security-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
prd-control-tower prd generate security-health \
  --context-file /tmp/prd_context_security-health.md \
  --publish \
  --json
```
...
```bash
prd-control-tower prd validate security-health --json
```
...
```bash
cat > /tmp/requirements_context_security-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
prd-control-tower requirements generate security-health \
  --context-file /tmp/requirements_context_security-health.md \
  --json
```
...
```bash
prd-control-tower requirements validate security-health --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/security-health/v1/<domain>/<domain>.proto`
