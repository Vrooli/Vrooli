...
`template-manager orient security-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient security-health
...
```bash
business-health wizard start  # (was: prd generate) security-health \
...
```bash
vrooli scenario requirements validate security-health --json
```
...
```bash
business-health wizard apply  # (was: requirements generate) security-health \
