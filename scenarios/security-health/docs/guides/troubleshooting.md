# Troubleshooting — Security Health
...
```bash
vrooli scenario status security-health
make stop
vrooli scenario start security-health --clean-stale
```
...
```bash
make status
vrooli scenario port security-health UI_PORT
```
...
security-health status --auto-start

# Or override the API base for this invocation
security-health status --api-base "http://localhost:$(vrooli scenario port security-health API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/security-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/security-health.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/security-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/security-health.db}")"
```
...
`packages/proto/gen/go/security-health/v1/`, TypeScript under
`packages/proto/gen/typescript/js/security-health/v1/`, and Python under
`packages/proto/gen/python/security_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/security-health/v1/<domain>/`.
