...
// the template itself does not build because of security-health placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_security_health_v1_notes_notes_proto, "NotesService")
}
