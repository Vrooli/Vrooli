...
func TestHTTPWebViewAttacherUsesLeasedDeviceAndReturnsEndpoint(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"endpoint":{"cdp_endpoint":"http://127.0.0.1:43123","renderer_id":"renderer-1"}}`))
	}))
	defer server.Close()
	...
	require.Equal(t, WebViewAttachment{CDPEndpoint: "http://127.0.0.1:43123", RendererID: "renderer-1"}, endpoint)
}
