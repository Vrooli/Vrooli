# Start Here — Scenario to Android
...
`template-manager orient scenario-to-android`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient scenario-to-android
...
```bash
cat > /tmp/prd_context_scenario-to-android.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start scenario-to-android --interactive
# …answer the interview, then:
business-health wizard preview scenario-to-android
business-health wizard apply scenario-to-android
```
...
```bash
vrooli scenario requirements validate scenario-to-android --json
```
...
```bash
cat > /tmp/requirements_context_scenario-to-android.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate scenario-to-android --json
business-health fix preview scenario-to-android
business-health fix apply scenario-to-android
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate scenario-to-android --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate scenario-to-android`, the notes page spec
...
validate scenario-to-android --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/scenario-to-android/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest scenario-to-android <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> scenario-to-android
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario scenario-to-android
```
