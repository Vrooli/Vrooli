# Troubleshooting — Scenario to Android
...
```bash
vrooli scenario status scenario-to-android
make stop
vrooli scenario start scenario-to-android --clean-stale
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/scenario-to-android/`.
...
The default route is `${SCENARIO_DATA_DIR}/scenario-to-android.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/scenario-to-android.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/scenario-to-android.db}")"
```
...
`packages/proto/gen/go/scenario-to-android/v1/`, TypeScript under
`packages/proto/gen/typescript/js/scenario-to-android/v1/`, and Python under
`packages/proto/gen/python/scenario_to_android/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/scenario-to-android/v1/<domain>/`.
