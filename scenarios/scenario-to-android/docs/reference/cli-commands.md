...
| `--auto-start` | Run `vrooli scenario start scenario-to-android` if the API is unreachable |
...
```bash
scenario-to-android status
scenario-to-android status --json
```

### `scenario-to-android configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
scenario-to-android configure api_base http://localhost:15001/api/v1
scenario-to-android configure token <token>
```

Read values back without an argument:

```bash
scenario-to-android configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`scenario-to-android <domain> <verb>`). Every command calls a single API
...
```bash
scenario-to-android notes list
scenario-to-android notes list --json
```

#### `scenario-to-android notes create --title <title> [--body <body>]`
...
```bash
scenario-to-android notes create --title "First note" --body "Hello world"
```
...
```bash
scenario-to-android notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start scenario-to-android`" is good.
