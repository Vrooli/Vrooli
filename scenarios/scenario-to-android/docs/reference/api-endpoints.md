# API Endpoints — Scenario to Android
...
envelope (`packages/proto/schemas/scenario-to-android/v1/shared/errors.proto`):
...
| **CLI** | `scenario-to-android status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/scenario-to-android/v1/shared/health.proto`
...
`POST /vrooli.scenario_to_android.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.scenario_to_android.v1.notes.NotesService/ListNotes`
...
| **CLI** | `scenario-to-android notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.scenario_to_android.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.scenario_to_android.v1.notes.NotesService/CreateNote`
...
| **CLI** | `scenario-to-android notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.scenario_to_android.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.scenario_to_android.v1.notes.NotesService/GetNote`
...
| **CLI** | `scenario-to-android notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.scenario_to_android.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `scenario-to-android notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/scenario-to-android/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/scenario-to-android/v1/<domain>/`, then run
   `make generate`.
