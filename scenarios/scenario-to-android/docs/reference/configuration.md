# Configuration — Scenario to Android
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/scenario-to-android.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `scenario-to-android` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/scenario-to-android/config.json`
3. `~/.vrooli/config/scenario-to-android/config.json`
4. `~/.config/vrooli/scenario-to-android/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port scenario-to-android API_PORT`)
