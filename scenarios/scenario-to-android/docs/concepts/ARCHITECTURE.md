# Architecture — Scenario to Android
...
| Contracts (`packages/proto/schemas/scenario-to-android/`) | Wire shape | Proto messages/services and generated clients | Hand-written route/type mirrors |
...
- scenario proto schemas relocated to
  `packages/proto/schemas/scenario-to-android/`.
...
This ramp is one layer in a four-layer system, and most of what an
Android delivery pipeline appears to need is owned elsewhere. Reaching
across a line below is a design error, not a shortcut.

| Question | Owner | This scenario's relationship |
|---|---|---|
| What devices and nodes do I control, and may I send this? | `vrooli-bridge` | Consumes reach and durable dispatch. Never stores device identity or trust. |
| What can be done *to* a device? | `device-control` | Consumes every device verb under a held lease. Implements no device driving. |
...
| How is the Android toolchain acquired? | `android-sdk` resource | Consumes it. Acquires no toolchain bytes. |
| Where does signing material live? | `secrets-manager` | References an identity. Holds no key material. |

What remains, and is genuinely this scenario's: turning a scenario into
a buildable Android project, producing a signed artifact with an asserted
target API, defining what "behaves like a Vrooli app on Android" means as
a chapter set, deciding which distribution channels are legitimately
available, and modelling the ladder that stands between the owner and
shipping.
...
1. Add proto messages and service methods under
   `packages/proto/schemas/scenario-to-android/v1/<domain>/`.
