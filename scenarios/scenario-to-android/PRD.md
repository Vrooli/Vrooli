...
- [ ] OT-P0-012 | hello-mobile fixture proves the ramp end to end | The `hello-mobile` fixture scenario is generated, built, installed, driven, and recorded on a physical Android device, producing a complete conformance journey with useful decoded video and correlated device events. The executable physical chapters are proven with reference-only evidence; the target's missing network-control capability remains explicitly unavailable and keeps the required-cell release gate closed.
- [ ] OT-P0-013 | Generated application branding reaches Android | Generated Android applications carry the target scenario's brand-manager output into adaptive and legacy launcher icons, splash, notification icon, shortcuts, and a share-target intent filter. Scenarios without an assigned brand receive an explicit generic placeholder set and the build reports that fact.
