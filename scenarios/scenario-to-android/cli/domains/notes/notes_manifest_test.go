...
// the template itself does not build because of scenario-to-android placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_scenario_to_android_v1_notes_notes_proto, "NotesService")
}
