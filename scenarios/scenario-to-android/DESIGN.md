...
### Why this design language fits Scenario to Android
...
target matrix — is scanned rather than read: which Android targets can I
prove right now, what did the last run show, what is blocking me. The
Operational Console's density, tabular emphasis, and information-first
posture are exactly right for that.

Two adaptations matter for this scenario specifically:

- **Disposition is the core vocabulary, so it must be encoded in form as
  well as colour.** `pass`, `failed`, `unavailable`, `unsupported`, and
  `not_run` are semantically distinct in ways a hue alone cannot carry.
  Use the status-colour roles as the binding contract, but always pair
  them with a shape or label — `unsupported` in particular must not read
  as a milder `unavailable`, because one is terminal and the other is a
  task. Colour never carries disposition alone; that is an accessibility
  floor here, not a preference.

- **Journey video is evidence, not interface.** Recordings are a first-class
  artifact of this scenario, which makes it tempting to centre the UI on a
  player. Every verdict, chapter assertion, and readiness rung must be
  fully perceivable and operable without playing anything — full keyboard
  operation, visible focus, semantic roles and names, stable
  `data-testid` selectors.

The token contract is otherwise adopted unchanged. Generated Android
applications carry the target scenario's branding, not this ramp's, so
nothing here propagates into a shipped app.
