...
func handleDailyLog(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if err := json.NewDecoder(r.Body).Decode(&dailyLog); err != nil {
			http.Error(w, "Invalid request body", http.StatusBadRequest)
			return
		}
		...
		if dailyLog.Notes != "" {
			encrypted, err := encrypt(dailyLog.Notes)
			if err == nil {
		...
		symptomsJSON, _ := json.Marshal(dailyLog.Symptoms)
		photosJSON, _ := json.Marshal(dailyLog.Photos)
