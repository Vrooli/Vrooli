...
if command -v jq >/dev/null 2>&1; then
    if ! jq empty < .vrooli/service.json >/dev/null 2>&1; then
        ...
        if [ $TESTING_PHASE_ERROR_COUNT -eq 0 ]; then
            ...
            if [ "$service_name" = "pregnancy-tracker" ]; then
...
# Check CLI tooling structure
echo "🔍 Validating CLI tooling..."
if [ -f "cli/pregnancy-tracker" ]; then
    if head -1 cli/pregnancy-tracker | grep -q "^#!/"; then
        log::success "✅ CLI has proper shebang"
    else
        testing::phase::add_error "❌ CLI missing shebang"
    fi
else
...
if [ -d "initialization/postgres" ]; then
    if [ -f "initialization/postgres/schema.sql" ]; then
        ...
        testing::phase::add_warning "⚠️  PostgreSQL schema file missing"
    fi
else
