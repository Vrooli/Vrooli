// Package startup measures scenario startup performance — time-to-healthy plus a
// resource envelope — behind a runner seam, and persists the measurements as a
// per-scenario trend. It is the new home for structure-health's former `perf`
...
// Schema returns the declarative DDL for the startup trend store; it is
// idempotent (CREATE TABLE IF NOT EXISTS) so EnsureSchemas can run it on boot.
...
func (s *Store) Insert(ctx context.Context, m Measurement) error {
	if s == nil || s.db == nil {
		return errors.New("startup: nil store")
	}
	if m.Scenario == "" {
		return errors.New("startup: scenario is required")
	}
	...
	_, err = s.db.ExecContext(ctx, `
		INSERT INTO startup_measurements
	...
	if err != nil {
		return fmt.Errorf("startup: insert measurement: %w", err)
	}
	return nil
}
...
func (s *Store) Series(ctx context.Context, scenario string, limit int) ([]Measurement, error) {
	if s == nil || s.db == nil {
		return nil, errors.New("startup: nil store")
	}
	if scenario == "" {
		return nil, errors.New("startup: scenario is required")
	}
	if limit <= 0 {
		limit = 50
	}
	rows, err := s.db.QueryContext(ctx, `
		...
		FROM startup_measurements
		WHERE scenario = ?
		ORDER BY captured_at DESC, id DESC
		LIMIT ?`, scenario, limit)
	if err != nil {
		return nil, fmt.Errorf("startup: query series: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if scanErr := rows.Scan(&m.Scenario, &capturedAt, &m.TimeToHealthyMs, &healthy, &surfacesJSON, &metricsJSON, &m.Note); scanErr != nil {
			return nil, fmt.Errorf("startup: scan measurement: %w", scanErr)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("startup: iterate series: %w", err)
	}
	return out, nil
}
...
func marshalSurfaceTimings(timings []SurfaceTiming) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("startup: marshal surface timings: %w", err)
	}
	return string(b), nil
}
...
func marshalMetrics(m *commonv1.ExecutionMetrics) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("startup: marshal metrics: %w", err)
	}
	return string(b), nil
}
