...
`template-manager orient performance-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient performance-health
...
```bash
cat > /tmp/prd_context_performance-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) performance-health \
  --context-file /tmp/prd_context_performance-health.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate performance-health --json
```
...
```bash
cat > /tmp/requirements_context_performance-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) performance-health \
  --context-file /tmp/requirements_context_performance-health.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/performance-health/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest performance-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> performance-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario performance-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> performance-health <adopted-path>
```
