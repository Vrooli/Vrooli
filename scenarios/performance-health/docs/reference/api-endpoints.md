# API Endpoints — Performance Health
...
envelope (`packages/proto/schemas/performance-health/v1/shared/errors.proto`):
...
| **CLI** | `performance-health status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/performance-health/v1/shared/health.proto`
...
`POST /vrooli.performance_health.v1.<domain>.<Domain>Service/<Method>`
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/performance-health/v1/<domain>/`, then run
   `make generate`.
