# Configuration — Performance Health
...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/performance-health.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `performance-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/performance-health/config.json`
3. `~/.vrooli/config/performance-health/config.json`
4. `~/.config/vrooli/performance-health/config.json`
...
```bash
performance-health configure api_base http://localhost:15001/api/v1
performance-health configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port performance-health API_PORT`)
