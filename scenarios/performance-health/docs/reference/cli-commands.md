# CLI Commands — Performance Health
...
`~/.vrooli/bin/performance-health`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start performance-health` if the API is unreachable |
...
```bash
performance-health status
performance-health status --json
```

### `performance-health configure <key> <value>`
...
```bash
performance-health configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`performance-health <domain> <verb>`). Every command calls a single API
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start performance-health`" is good.
