...
performance-health <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port performance-health API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.performance_health.v1.<domain>.<Service>/<Method>
```
...
CLI (`performance-health --help`) — see
