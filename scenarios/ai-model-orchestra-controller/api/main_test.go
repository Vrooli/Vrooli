...
func TestSelectOptimalModelBasic(t *testing.T) {
	...
	// Test without Ollama client (uses defaults)
	response, err := selectOptimalModel("completion", map[string]interface{}{}, nil, logger)
...
func TestSelectOptimalModelWithRequirements(t *testing.T) {
	...
	response, err := selectOptimalModel("completion", requirements, nil, logger)
...
func TestSelectOptimalModelHighMemoryPressure(t *testing.T) {
	...
	// This tests that the function doesn't crash under various task types
	response, err := selectOptimalModel("reasoning", map[string]interface{}{}, nil, logger)
...
func TestSelectOptimalModelUnsupportedTaskType(t *testing.T) {
	logger := log.New(os.Stdout, "[test] ", log.LstdFlags)

	// Test with a task type that no models support
	response, err := selectOptimalModel("unsupported-task-type", map[string]interface{}{}, nil, logger)

	// Should return error for unsupported task type
	assert.Error(t, err)
...
func TestValidateEnvironment(t *testing.T) {
	...
	originalEnvVars := map[string]string{
		...
		"RESOURCE_PORTS_POSTGRES": os.Getenv("RESOURCE_PORTS_POSTGRES"),
		"RESOURCE_PORTS_REDIS":    os.Getenv("RESOURCE_PORTS_REDIS"),
		"RESOURCE_PORTS_OLLAMA":   os.Getenv("RESOURCE_PORTS_OLLAMA"),
	...
	t.Run("AllRequiredVariablesSet", func(t *testing.T) {
		// Set all required variables
		os.Setenv("API_PORT", "8080")
		os.Setenv("ORCHESTRATOR_HOST", "localhost")
		os.Setenv("RESOURCE_PORTS_POSTGRES", "5432")
		os.Setenv("RESOURCE_PORTS_REDIS", "6379")
		os.Setenv("RESOURCE_PORTS_OLLAMA", "11434")
		os.Setenv("POSTGRES_USER", "test")
		os.Setenv("POSTGRES_PASSWORD", "test")

		err := validateEnvironment(logger)
		assert.NoError(t, err)
	})
...
func BenchmarkSelectOptimalModel(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		selectOptimalModel("completion", requirements, nil, logger)
	}
}
