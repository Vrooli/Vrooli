...
`vrooli scenario orient brand-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient brand-manager
...
```bash
prd-control-tower prd generate brand-manager \
...
```bash
prd-control-tower prd validate brand-manager --json
```
...
```bash
prd-control-tower requirements generate brand-manager \
...
```bash
prd-control-tower requirements validate brand-manager --json
```
