...
`template-manager orient brand-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient brand-manager
...
```bash
cat > /tmp/prd_context_brand-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) brand-manager \
  --context-file /tmp/prd_context_brand-manager.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate brand-manager --json
```
...
```bash
cat > /tmp/requirements_context_brand-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) brand-manager \
  --context-file /tmp/requirements_context_brand-manager.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/brand-manager/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest brand-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> brand-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario brand-manager
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> brand-manager <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/brand-manager/v1/<domain>/`.
