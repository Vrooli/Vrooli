...
brand-manager <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port brand-manager API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.brand_manager.v1.<domain>.<Service>/<Method>
```
...
brand-manager notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port brand-manager API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.brand_manager.v1.notes.NotesService/ListNotes" \
