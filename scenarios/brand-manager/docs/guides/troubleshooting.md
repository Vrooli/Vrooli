...
```bash
export SQLITE_PATH=/tmp/brand-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/brand-manager.db}")"
```
...
`packages/proto/gen/typescript/js/brand-manager/v1/`, and Python under
