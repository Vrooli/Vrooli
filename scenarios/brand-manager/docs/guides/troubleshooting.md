...
```bash
vrooli scenario status brand-manager
make stop
vrooli scenario start brand-manager --clean-stale
```
...
```bash
make status
vrooli scenario port brand-manager UI_PORT
```
...
brand-manager status --auto-start

# Or override the API base for this invocation
brand-manager status --api-base "http://localhost:$(vrooli scenario port brand-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/brand-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/brand-manager.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/brand-manager.db"
```
...
`packages/proto/gen/go/brand-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/brand-manager/v1/`, and Python under
`packages/proto/gen/python/brand_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/brand-manager/v1/<domain>/`.
