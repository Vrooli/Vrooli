...
`~/.vrooli/bin/brand-manager`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start brand-manager` if the API is unreachable |
...
```bash
brand-manager status
brand-manager status --json
```

### `brand-manager configure <key> <value>`
...
```bash
brand-manager configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`brand-manager <domain> <verb>`). Every command calls a single API
...
```bash
brand-manager notes list
brand-manager notes list --json
```

#### `brand-manager notes create --title <title> [--body <body>]`
...
```bash
brand-manager notes create --title "First note" --body "Hello world"
```
...
```bash
brand-manager notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start brand-manager`" is good.
