...
|---|---|---|---|---|
| 2026-09-15 | **Explore renders finished concepts with an illustration model; vector is the pick step, not the drawing medium.** Supersedes the brand-identity-pipeline plan's "mark-only prompts on the SVG-native role". | The first live explore (Vega, system-monitor) produced primitive, geometric marks from `recraft/recraft-v4.1-vector` via `image.generate.logo`, which the operator rejected next to Aquila's approved concept rendered by `bytedance-seed/seedream-4.5`. | Explore defaults to `image.generate.default` → `image.generate.quality`, writes a finished-icon prompt from the brand's container style, and supports `style_reference_brand`/`style_reference_asset_id` (edit role `image.edit.default`) so a product line renders as one family. Pick still vectorizes; `prefer_vector` remains for deliberately flat directions. | Revisit if a vector-native model matches the illustration models on the operator's comparison, or if image-tools gains a first-class style-reference op. |
