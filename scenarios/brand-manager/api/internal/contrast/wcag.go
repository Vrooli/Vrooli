// Package contrast implements WCAG 2.1 AA contrast-ratio calculation and
// validation. Ported (transport-agnostic) from the prior brand-manager build;
// the only change is dropping the external config dependency in favor of
// in-package WCAG AA spec defaults.
package contrast
...
// WCAG AA minimum contrast ratios (spec values) and the default rounding
// precision used when reporting ratios.
const (
...
func DefaultChecker() *Checker {
	return &Checker{
		AANormal:  DefaultAANormalText,
		AALarge:   DefaultAALargeText,
...
// meet WCAG AA contrast. Extend StandardPairings to check additional combinations.
type StandardPairing struct {
	Foreground ColorRole
	Background ColorRole
}

// StandardPairings lists the WCAG AA pairings checked for every brand. These
// represent the most common text-on-surface combinations a user will see.
