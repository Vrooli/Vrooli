...
| 2026-07-02 | Codex | done | Hardened the migrated health_check / preflight / server_run contracts as native Structure Health validators. API lifecycle now requires a critical `${API_PORT}/health` service.json probe plus an actual root `/health` route; `api/main.go` must run api-core preflight before service initialization; direct `ListenAndServe` without `api-core/server.Run` is flagged. Added maturity mappings and a structured autofix for malformed service.json health checks. Validation: API + CLI `go test ./...` green; live `structure-health validate scenario structure-health --json` passes with no new-rule findings; comprehensive run `20260703-001247-eed41ef3` still fails on unrelated gates, while structure phase passes and no new-rule findings appear. |
