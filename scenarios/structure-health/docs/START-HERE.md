...
`template-manager orient structure-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient structure-health
...
```bash
cat > /tmp/prd_context_structure-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) structure-health \
  --context-file /tmp/prd_context_structure-health.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate structure-health --json
```
...
```bash
cat > /tmp/requirements_context_structure-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
business-health wizard apply  # (was: requirements generate) structure-health \
  --context-file /tmp/requirements_context_structure-health.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/structure-health/v1/<domain>/<domain>.proto`
