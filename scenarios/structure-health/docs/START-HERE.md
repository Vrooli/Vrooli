...
`vrooli scenario orient structure-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient structure-health
...
```bash
prd-control-tower prd generate structure-health \
...
```bash
prd-control-tower prd validate structure-health --json
```
...
```bash
prd-control-tower requirements validate structure-health --json
```
