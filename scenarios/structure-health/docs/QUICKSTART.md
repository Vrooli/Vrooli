...
```bash
vrooli scenario port structure-health UI_PORT
```
...
structure-health <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port structure-health API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.structure_health.v1.<domain>.<Service>/<Method>
```
