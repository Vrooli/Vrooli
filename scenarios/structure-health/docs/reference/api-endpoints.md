# API Endpoints — Structure Health
...
envelope (`packages/proto/schemas/structure-health/v1/shared/errors.proto`):
...
| **CLI** | `structure-health status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/structure-health/v1/shared/health.proto`
...
`POST /vrooli.structure_health.v1.<domain>.<Domain>Service/<Method>`
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/structure-health/v1/<domain>/`, then run
   `make generate`.
