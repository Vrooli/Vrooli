# Configuration — Structure Health
...
For `structure-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/structure-health/config.json`
3. `~/.vrooli/config/structure-health/config.json`
4. `~/.config/vrooli/structure-health/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port structure-health API_PORT`)
