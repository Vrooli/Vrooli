...
`~/.vrooli/bin/structure-health`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start structure-health` if the API is unreachable |
...
```bash
structure-health status
structure-health status --json
```

### `structure-health configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
structure-health configure api_base http://localhost:15001/api/v1
structure-health configure token <token>
```

Read values back without an argument:

```bash
structure-health configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`structure-health <domain> <verb>`). Every command calls a single API
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start structure-health`" is good.
