...
func CheckUISharedLifecycleLaunch(content []byte, scenarioPath string, scenario string) ([]auditrules.Violation, error) {
	var payload uiLifecyclePayload
	if err := json.Unmarshal(content, &payload); err != nil {
		return []auditrules.Violation{newUIViolation("ui", fmt.Sprintf("UI lifecycle payload is invalid JSON: %v", err), "high")}, nil
	}
