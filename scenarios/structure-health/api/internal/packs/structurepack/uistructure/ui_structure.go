...
func CheckUIStructure(content []byte, scenarioPath string, scenario string) ([]auditrules.Violation, error) {
	return CheckUICore(content, scenarioPath, scenario)
}

// CheckUICore validates the UI shell for required entry assets.
func CheckUICore(content []byte, scenarioPath string, scenario string) ([]auditrules.Violation, error) {
	var payload uiStructurePayload
	if err := json.Unmarshal(content, &payload); err != nil {
		return []auditrules.Violation{newUIViolation("ui", fmt.Sprintf("UI structure payload is invalid JSON: %v", err), "high")}, nil
	}
