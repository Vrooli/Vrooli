...
var catalog = []CatalogEntry{
	...
	{Code: "PACKAGE_CONSUMER_CLASS_SCAN_FAILED", TargetKind: "package", Severity: "error", Enforcement: Enforced, WhatItChecks: "Package consumer discovery is available to evaluate declared adoption boundaries.", Remediation: "Restore the control-plane package registry and re-run the package structure gate.", Claim: "package.consumer-discovery"},
	{Code: "PACKAGE_CONSUMER_CLASS_VIOLATION", TargetKind: "package", Severity: "error", Enforcement: Enforced, WhatItChecks: "Discovered package consumers belong to classes explicitly allowed by the package manifest.", Remediation: "Declare the consumer class in package.adoption.allowed_consumers or move the consumer behind an appropriate package boundary.", Claim: "package.allowed-consumers"},
	{Code: "PACKAGE_RESOURCE_ENV_OWNER_INVALID", TargetKind: "package", Severity: "error", Enforcement: Enforced, WhatItChecks: "Package resource-environment ownership claims name existing resource manifests.", Remediation: "Declare an existing resources/<name>/resource.json or remove the ownership claim.", Claim: "package.resource-environment-ownership"},
	...
	{Code: "SCENARIO_README_POLICY", TargetKind: "scenario", Severity: "error", Enforcement: Enforced, WhatItChecks: "Scenario README files describe the permanent scenario capability rather than template provenance.", Remediation: "Rewrite the README to remove template scaffold language and describe the scenario.", Claim: "scenario.readme-identity"},
	...
	{Code: "SCENARIO_MANIFEST_INVALID", TargetKind: "scenario", Severity: "error", Enforcement: Enforced, WhatItChecks: "Scenario manifests validate against the canonical service schema.", Remediation: "Repair .vrooli/service.json against .vrooli/schemas/service.schema.json, or correct the schema if it no longer describes the implemented contract.", Claim: "scenario.manifest"},
	{Code: "SCENARIO_SHELL_FORBIDDEN", TargetKind: "scenario", Severity: "error", Enforcement: Enforced, WhatItChecks: "Declared scenario lifecycle and component processes never invoke a shell interpreter or shell file.", Remediation: "Replace the declared shell invocation with component metadata, data_dirs, an argv-native command, or a Go CLI subcommand.", Claim: "scenario.no-shell-lifecycle"},
	{Code: "SCENARIO_COMPONENT_INVALID", TargetKind: "scenario", Severity: "warning", Enforcement: Enforced, WhatItChecks: "Component builds, ports, supervisors, dependencies, reuse edges, and graph cycles resolve within the scenario manifest.", Remediation: "Repair components so every build and process reference resolves without a cycle.", Claim: "scenario.component-contract"},
	{Code: "SCENARIO_PEER_BINDING_INVALID", TargetKind: "scenario", Severity: "warning", Enforcement: Enforced, WhatItChecks: "Scenario peer bindings name ports declared by the peer and use availability policies consistent with startup and degraded behavior.", Remediation: "Repair the binding against the peer manifest and align when_unavailable with startup_policy and degraded_behavior.", Claim: "scenario.peer-bindings"},
	{Code: "SCENARIO_HARDCODED_PEER_ADDRESS", TargetKind: "scenario", Severity: "error", Enforcement: Enforced, WhatItChecks: "Scenarios with peer dependencies do not hardcode loopback peer ports in component environment values.", Remediation: "Declare a dependencies.scenarios binding for the peer port.", Claim: "scenario.no-hardcoded-peer-address"},
	{Code: "SCENARIO_SECRET_LITERAL", TargetKind: "scenario", Severity: "error", Enforcement: Enforced, WhatItChecks: "Component environment declarations contain neither secret-bearing keys nor high-entropy literals.", Remediation: "Declare secret inputs through the credential authority.", Claim: "scenario.no-secret-literals"},
	{Code: "SCENARIO_REDECLARES_RESOURCE_ENV", TargetKind: "scenario", Severity: "error", Enforcement: Enforced, WhatItChecks: "Component environment values do not redeclare variables exported by enabled resources.", Remediation: "Delete the duplicate component environment value and consume the resource export.", Claim: "scenario.resource-env-authority"},
	{Code: "SCENARIO_UI_SERVES_BUILD", TargetKind: "scenario", Severity: "error", Enforcement: Enforced, WhatItChecks: "Every declared UI component serves a built bundle rather than a development or preview server.", Remediation: "Build the UI during setup and run a production static server from the component declaration.", Claim: "scenario.ui-production-serving"},
	{Code: "SCENARIO_PORT_ENV_CONVENTION", TargetKind: "scenario", Severity: "error", Enforcement: Enforced, WhatItChecks: "Every declared port key NAME publishes the environment variable NAME_PORT used by component port ownership.", Remediation: "Rename ports.<name>.env_var to the uppercase port key followed by _PORT and update its consumers.", Claim: "scenario.port-env-convention"},
	{Code: "SCENARIO_BUILD_KIND_UNKNOWN", TargetKind: "scenario", Severity: "warning", Enforcement: Advisory, WhatItChecks: "Every component build kind is registered by the lifecycle component builder contract.", Remediation: "Use a registered component build kind or add a complete builder-registry implementation before adopting a new kind.", Claim: "scenario.build-kind"},
	...
	{Code: "DEPLOYABILITY_INSTANCE_IDENTIFIER", TargetKind: "control-plane", Severity: "error", Enforcement: Enforced, WhatItChecks: "Deployability decision code does not hard-code the name of a declared resource, safeguard, or scenario.", Remediation: "Remove the instance literal and pass manifest declarations into the pure resolver.", Claim: "control-plane.no-instance-identifiers"},
	...
	{Code: "PROJECT_CREDENTIAL_DESCRIPTOR_DUPLICATE", TargetKind: "project", Severity: "error", Enforcement: Enforced, WhatItChecks: "A single manifest does not declare one logical credential field more than once.", Remediation: "Remove the duplicate logical_id/field declaration from the manifest.", Claim: "project.credential-descriptor-uniqueness"},
	...
	{Code: "REPO_SCHEMA_ID_UNIQUE", TargetKind: "project", Severity: "error", Enforcement: Enforced, WhatItChecks: "Every repository JSON Schema declares a unique $id.", Remediation: "Delete schema forks or assign a distinct authoritative $id to each genuinely independent schema.", Claim: "project.schema-id-uniqueness"},
}
