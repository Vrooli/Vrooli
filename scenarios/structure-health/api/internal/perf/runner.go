...
// unit-tested with a fake (no real restarts), and so the real implementation's
// lifecycle/exec dependencies stay out of the service.
...
func (r *CLIRunner) Measure(ctx context.Context, scenario string, timeout time.Duration) (Measurement, error) {
	if strings.TrimSpace(scenario) == "" {
		return Measurement{}, fmt.Errorf("perf: scenario is required")
	}
	if timeout <= 0 {
	...
	if err := restart(ctx, scenario); err != nil {
		...
		return m, fmt.Errorf("perf: restart %q: %w", scenario, err)
	}
