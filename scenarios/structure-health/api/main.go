...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("structure-health")
	if err != nil {
		return "", fmt.Errorf("resolve structure-health storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve structure-health db path: %w", err)
	}
	return sqliteFileDSN(path)
}
...
func main() {
	...
	if preflight.Run(preflight.Config{ScenarioName: "structure-health"}) {
		return
	}
