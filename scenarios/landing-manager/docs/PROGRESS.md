...
|------------|-------------------|-----------------|-------|
| 2025-11-24 | Current Agent | Agent handoff via app-issue-tracker + factory UI improvements | Integrated customize endpoint with app-issue-tracker (auto-create issue + trigger investigation) and added unit test. Factory UI now surfaces agent customization form, richer generation guidance, and move/start commands with preview instructions. Fixed Go generator tests by allowing payload override in tests. Updated design requirements in template payload to mark implemented CSS vars, custom font, and gradient backgrounds. |
