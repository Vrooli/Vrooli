...
description: "How to deploy generated landing pages to production"
...
This guide covers how to deploy your generated landing page from development to production.
...
Landing pages go through three stages:

```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│    GENERATED    │    │    PROMOTED     │    │   DEPLOYED      │
│    (Staging)    │───▶│  (Production)   │───▶│   (Live)        │
│                 │    │                 │    │                 │
│ generated/slug/ │    │ scenarios/slug/ │    │ your-domain.com │
│ Safe testing    │    │ Ready for prod  │    │ Public access   │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```

| Stage | Location | Purpose |
|-------|----------|---------|
| **Generated** | `landing-manager/generated/<slug>/` | Test and customize safely |
| **Promoted** | `scenarios/<slug>/` | Production-ready, managed by Vrooli |
| **Deployed** | Cloud/CDN | Publicly accessible with custom domain |

---

## Local Development

### Starting a Generated Landing Page

After generating a landing page, start it for local testing:

```bash
# From the Factory UI: Click "Start" on the scenario card

# Or via CLI:
cd scenarios/landing-manager/generated/<your-slug>
make start
```
...
When you're satisfied with your landing page in staging, promote it to production.

### Via Factory UI (Recommended)

1. Open the Factory UI
2. Find your scenario in "Generated Scenarios"
3. Ensure the scenario is **stopped**
4. Click **Promote**
...
```bash
# Stop the staging scenario first
vrooli scenario stop <slug>

# Promote via API
curl -X POST http://localhost:<FACTORY_PORT>/api/v1/lifecycle/<slug>/promote
```

### What Promotion Does

1. **Moves files** from `generated/<slug>/` to `scenarios/<slug>/`
2. **Updates paths** in configuration files
3. **Registers** with Vrooli's scenario manager
4. **Preserves** database data (if using shared PostgreSQL)

### Post-Promotion Checklist

After promoting:

- [ ] Start the production scenario: `cd scenarios/<slug> && make start`
