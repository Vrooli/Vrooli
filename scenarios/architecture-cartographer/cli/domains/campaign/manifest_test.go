...
// TestManifestCoversCampaignService asserts that every RPC declared on
// CampaignService either has a manifest command binding or is documented
// in the manifest's `omitted` array. Catches proto↔CLI drift: adding a new
// RPC without binding/omitting it fails here.
func TestManifestCoversCampaignService(t *testing.T) {
	manifest := readManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, campaignv1.File_architecture_cartographer_v1_campaign_campaign_proto, "CampaignService")
}
...
func TestRegisterWiresAllCommands(t *testing.T) {
	...
	want := map[string]bool{
		"create": false, "list": false, "status": false, "next": false, "resolve": false,
