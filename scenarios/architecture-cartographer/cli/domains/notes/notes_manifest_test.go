...
// the template itself does not build because of architecture-cartographer placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_architecture_cartographer_v1_notes_notes_proto, "NotesService")
}
