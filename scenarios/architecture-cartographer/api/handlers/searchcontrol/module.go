...
// ProtoFile exposes the shared control domain's proto FileDescriptor so a global
...
// Module returns the search-control domain's contribution to the API: the
// Connect SearchControlService handler (the shared, token-gated reindex +
// config-write control plane) mounted at the generated procedure paths.
func Module(logger *log.Logger, deps Deps) module.Module {
	pattern, connectHandler := controlconnect.NewSearchControlServiceHandler(NewConnectHandler(deps))
...
// Schema returns "" — reindex job state is held in-process; the index lives in
// Qdrant and the tuning SSOT in search.json, not the scenario database.
...
var Endpoints = []module.EndpointDescriptor{
	{
		...
		Description: "Queues a reconcile job that upserts/deletes domain points in Qdrant. Requires the provider's control token.",
	...
	},
	{
		...
		Description: "Validates a new tuning block and atomically rewrites the provider's search.json, triggering a reindex when an index-time factor changed. The search-hub sweep's write-back verb.",
		Category:    "searchcontrol",
		Request: &module.Schema{
			Type: "object",
			Properties: map[string]string{
		...
		Examples: []module.Example{
			{Name: "Write config (dry-run)", Curl: "curl http://localhost:${API_PORT}/vrooli.search_hub.v1.control.SearchControlService/WriteConfig -H 'Content-Type: application/json' -d '{\"provider_id\":\"architecture-cartographer.domain-map\",\"dry_run\":true,\"control_token\":\"<token>\",\"tuning\":{\"engine\":\"dense\"}}'"},
		},
	},
	{
		...
		Description: "Validates a new evaluation corpus (the tests block, carried as an eval EvalSuite) and atomically rewrites ONLY that block in the provider's search.json. The search-hub optimizer's corpus write-back verb. Never triggers a reindex.",
		Category:    "searchcontrol",
		Request: &module.Schema{
			Type: "object",
			Properties: map[string]string{
		...
		Examples: []module.Example{
			{Name: "Write corpus (dry-run)", Curl: "curl http://localhost:${API_PORT}/vrooli.search_hub.v1.control.SearchControlService/WriteCorpus -H 'Content-Type: application/json' -d '{\"provider_id\":\"architecture-cartographer.domain-map\",\"dry_run\":true,\"control_token\":\"<token>\",\"corpus\":{\"suite_id\":\"architecture-cartographer.domain-map.primary\",\"cases\":[]}}'"},
		},
	},
}
