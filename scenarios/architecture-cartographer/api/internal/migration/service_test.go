...
func TestCreateIngestsFindings(t *testing.T) {
	...
	if st.Migration.Status != MigrationOpen {
		t.Errorf("migration should be open, got %s", st.Migration.Status)
	}
...
func TestNextPrioritizesRegressionsThenCyclesThenSeverity(t *testing.T) {
	svc := newTestService(t)
	ctx := context.Background()
	st, _ := svc.Create(ctx, "demo", "", []*architecturev1.ArchitectureFinding{
...
func TestCloseMarksMigrationClosed(t *testing.T) {
	svc := newTestService(t)
	ctx := context.Background()
	...
	if closed.Migration.Status != MigrationClosed {
		t.Errorf("expected closed, got %s", closed.Migration.Status)
	}
}
