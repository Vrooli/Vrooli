# API Endpoints — Architecture Cartographer
...
envelope (`packages/proto/schemas/architecture-cartographer/v1/errors/errors.proto`):
...
| **CLI** | `architecture-cartographer status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/architecture-cartographer/v1/health/health.proto`
...
### `POST /vrooli.architecture_cartographer.v1.notes.NotesService/ListNotes`
...
| **CLI** | `architecture-cartographer notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.architecture_cartographer.v1.notes.NotesService/ListNotes" \
...
### `POST /vrooli.architecture_cartographer.v1.notes.NotesService/CreateNote`
...
| **CLI** | `architecture-cartographer notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.architecture_cartographer.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
### `POST /vrooli.architecture_cartographer.v1.notes.NotesService/GetNote`
...
| **CLI** | `architecture-cartographer notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.architecture_cartographer.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `architecture-cartographer notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/architecture-cartographer/v1/notes/notes.proto`.
...
1. Add or extend the `.proto` messages and service in
   `packages/proto/schemas/architecture-cartographer/v1/<domain>/`, then run
   `make generate`.
