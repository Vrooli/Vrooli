# CLI Commands — Architecture Cartographer
...
`~/.vrooli/bin/architecture-cartographer`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start architecture-cartographer` if the API is unreachable |
...
```bash
architecture-cartographer status
architecture-cartographer status --json
```

### `architecture-cartographer configure <key> <value>`
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start architecture-cartographer`" is good.
