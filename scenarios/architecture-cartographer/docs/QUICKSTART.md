# Quickstart — Architecture Cartographer
...
```bash
architecture-cartographer status
architecture-cartographer notes list
architecture-cartographer notes create --title "First note" --body "Hello"
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port architecture-cartographer API_PORT)
curl -s "http://localhost:${API_PORT}/health"
curl -s -X POST "http://localhost:${API_PORT}/vrooli.architecture_cartographer.v1.notes.NotesService/ListNotes" \
...
CLI (`architecture-cartographer --help`) — see
