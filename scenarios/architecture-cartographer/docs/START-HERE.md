...
`vrooli scenario orient architecture-cartographer`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient architecture-cartographer
...
```bash
prd-control-tower prd generate architecture-cartographer \
...
```bash
prd-control-tower prd validate architecture-cartographer --json
```
...
```bash
prd-control-tower requirements generate architecture-cartographer \
...
```bash
prd-control-tower requirements validate architecture-cartographer --json
```
