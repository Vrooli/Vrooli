# Start Here — Architecture Cartographer
...
`template-manager orient architecture-cartographer`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient architecture-cartographer
...
```bash
cat > /tmp/prd_context_architecture-cartographer.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) architecture-cartographer \
  --context-file /tmp/prd_context_architecture-cartographer.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate architecture-cartographer --json
```
...
```bash
cat > /tmp/requirements_context_architecture-cartographer.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) architecture-cartographer \
  --context-file /tmp/requirements_context_architecture-cartographer.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/architecture-cartographer/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest architecture-cartographer <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> architecture-cartographer
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario architecture-cartographer
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> architecture-cartographer <adopted-path>
```
