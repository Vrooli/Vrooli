...
```bash
export SQLITE_PATH=/tmp/architecture-cartographer.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/architecture-cartographer.db}")"
```
...
`packages/proto/gen/typescript/js/architecture-cartographer/v1/`, and Python under
