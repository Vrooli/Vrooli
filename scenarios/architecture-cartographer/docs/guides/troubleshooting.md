# Troubleshooting — Architecture Cartographer
...
```bash
vrooli scenario status architecture-cartographer
make stop
vrooli scenario start architecture-cartographer --clean-stale
```
...
```bash
make status
vrooli scenario port architecture-cartographer UI_PORT
```
...
architecture-cartographer status --auto-start

# Or override the API base for this invocation
architecture-cartographer status --api-base "http://localhost:$(vrooli scenario port architecture-cartographer API_PORT)/api/v1"
```
...
### `architecture-cartographer configure` doesn't persist

Check which config-file path resolved (precedence in
[`../reference/configuration.md`](../reference/configuration.md#cli-config-file)).
Most commonly `~/.vrooli/config/architecture-cartographer/config.json`. The
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/architecture-cartographer/`.
...
The default route is `${SCENARIO_DATA_DIR}/architecture-cartographer.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/architecture-cartographer.db"
```
...
`packages/proto/gen/go/architecture-cartographer/v1/`, TypeScript under
`packages/proto/gen/typescript/architecture-cartographer/v1/`, and Python under
`packages/proto/gen/python/architecture_cartographer/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/architecture-cartographer/v1/<domain>/`.
