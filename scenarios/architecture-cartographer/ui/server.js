...
if (!uiPort) {
  throw new Error(
    'UI_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start architecture-cartographer` rather than running server.js directly.',
  )
}
...
if (!apiPort) {
  throw new Error(
    'API_PORT is not set. The scenario lifecycle supplies it — start this scenario with `vrooli scenario start architecture-cartographer` rather than running server.js directly.',
  )
}

const connectRpcPath = /^\/vrooli\.architecture_cartographer\.v1\./

startScenarioServer({
  uiPort,
  apiPort,
  distDir: './dist',
  serviceName: 'architecture-cartographer',
  corsOrigins: '*',
  setupRoutes(app) {
