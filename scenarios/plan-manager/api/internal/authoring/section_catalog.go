...
var defaultSkeleton = []sectionSpec{
	...
	},
	{
		...
		Summary:        "Run server-side context discovery for decomposed concepts, then apply the curated batch (take concrete improvements, sweep noise).",
		Instructions:   []string{"Decompose the work into 2-5 concepts via four lenses: domain, technology, problem type, scenario surface.", "Run context-discover — the probes execute server-side and return a curated shortlist plus batch notes.", "Run context-apply once: accept only items whose concrete improvement you can articulate; sweep the rest with a note. High-confidence drops require a reason.", "If no internal skill applies, record NO_SKILL_CONTEXT with the reason.", "Do not add generic feedback reminders here; the rendered plan includes the default plan-manager log capture workflow."},
		RequiredInputs: []string{"context concepts", "an applied context discovery batch or explicit NO_SKILL_CONTEXT reason", "or explicit NO_CONTEXT reason"},
		Examples:       []string{"author context-discover <session> --concepts 'plan-manager execution resume,authoring context discovery' --complexity architectural", "author context-apply <session> --take c1 --note 'reviewed shortlist'", "NO_SKILL_CONTEXT: no internal skill applies beyond accepted docs/search setup."},
		CommonMistakes: []string{"Accepting raw discovery output without review.", "Putting phase-only setup in global context.", "Adding docs/search context but never applying the skill sweep.", "Relying on the legacy required_reading section as the primary model."},
		Placeholder:    "NO_SKILL_CONTEXT: <reason>",
	},
	{
