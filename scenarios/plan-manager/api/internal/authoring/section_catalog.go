...
var defaultSkeleton = []sectionSpec{
	...
	{
		Key:            SectionRisksHazards,
		Label:          "Risks / Hazards",
		Mandatory:      false,
		StepKind:       "risks_hazards",
		Summary:        "Record what could go wrong during execution and what that would cost.",
		Instructions:   []string{"List a hazard per line: what could go wrong, and the blast radius if it does.", "Cover hazards of doing the work, not assumptions about the environment; preconditions belong in Assumptions.", "Prefer hazards a reviewer can act on: data loss, irreversible migrations, shared surfaces, live traffic."},
		RequiredInputs: []string{"risks and hazards (optional)"},
		Examples:       []string{"Reconcile writes to the shared plan mirror root, so a wrong intake filter can duplicate the corpus."},
		CommonMistakes: []string{"Restating assumptions as risks.", "Listing hazards with no consequence attached."},
		Placeholder:    "<risks and hazards, one per line>",
	},
	{
	...
	},
	{
		...
		Instructions:   []string{"Decompose the work into 2-5 concepts via four lenses: domain, technology, problem type, scenario surface.", "Run author skill-pack so prompt-manager skills are added directly as global relevant context.", "Run author skill-pack --phase <phase> when one phase needs a skill the rest of the plan does not, such as a phase that enters a governed surface or uses a different working method.", "Keep most returned skills unless they are clearly irrelevant; skills are allowed to support professional code quality, not only the literal task noun.", "Run search-hub directly when docs, records, or code context would help, then submit only durable references or commands that remain useful.", "Do not add generic feedback reminders here; the rendered plan includes the default plan-manager log capture workflow."},
		RequiredInputs: []string{"skill-pack concepts, optional durable context, or explicit NO_CONTEXT reason"},
		Examples:       []string{"author skill-pack <session> --concepts 'plan-manager execution resume,authoring context discovery' --complexity architectural", "author skill-pack <session> --phase 3 --concepts 'embedding metadata,vector storage' --complexity moderate", "prompt-manager skill read implementation-plan-authoring ecosystem-fit", "NO_CONTEXT: documentation-only operator decision."},
