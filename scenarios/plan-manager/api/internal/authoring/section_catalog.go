...
var defaultSkeleton = []sectionSpec{
	...
	},
	{
		...
		Summary:        "Run server-side context discovery for decomposed concepts, then disposition every candidate (accept with a concrete improvement, or reject with a reason).",
		Instructions:   []string{"Decompose the work into 2-5 concepts via four lenses: domain, technology, problem type, scenario surface.", "Run context-discover — the probes execute server-side and return reviewed-ready candidates.", "Disposition every candidate: accept only items whose concrete improvement you can articulate; reject the rest with a reason.", "If no internal skill applies, record NO_SKILL_CONTEXT with the reason.", "Do not add generic feedback reminders here; the rendered plan includes the default plan-manager log capture workflow."},
		RequiredInputs: []string{"context concepts", "a disposition for every candidate or explicit NO_SKILL_CONTEXT reason", "or explicit NO_CONTEXT reason"},
		Examples:       []string{"author context-discover <session> --concepts 'plan-manager execution resume,authoring context discovery' --complexity architectural", "NO_SKILL_CONTEXT: no internal skill applies beyond accepted docs/search setup."},
		CommonMistakes: []string{"Accepting raw discovery output without a reason.", "Putting phase-only setup in global context.", "Adding docs/search context but never deciding skill setup.", "Relying on the legacy required_reading section as the primary model."},
