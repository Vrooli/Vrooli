...
var Endpoints = []module.EndpointDescriptor{
	...
	endpoint("plans_candidate_create", plansconnect.PlansServiceCreateCandidateRevisionProcedure, "Create candidate revision", "Stores a whole-plan candidate without changing canonical plan content."),
	endpoint("plans_candidate_get", plansconnect.PlansServiceGetCandidateRevisionProcedure, "Get candidate revision", "Returns one durable candidate revision."),
	endpoint("plans_candidate_preview", plansconnect.PlansServicePreviewCandidateRevisionProcedure, "Preview candidate revision", "Computes a candidate diff, validation diagnostics, and rendered preview without mutation."),
	endpoint("plans_candidate_validate", plansconnect.PlansServiceValidateCandidateRevisionProcedure, "Validate candidate revision", "Validates a candidate revision without changing canonical plan content."),
	endpoint("plans_candidate_apply", plansconnect.PlansServiceApplyCandidateRevisionProcedure, "Apply candidate revision", "Applies a candidate once under base-hash, acknowledgment, and execution-active guards."),
	endpoint("plans_candidate_discard", plansconnect.PlansServiceDiscardCandidateRevisionProcedure, "Discard candidate revision", "Marks an unapplied candidate as discarded."),
	...
	endpoint("plans_add_relevant_context", plansconnect.PlansServiceAddRelevantContextProcedure, "Add relevant context", "Adds one structured setup item with a server-owned identity."),
	...
	endpoint("plans_add_reference", plansconnect.PlansServiceAddReferenceProcedure, "Add a reference", "Adds one structured reference with a server-owned identity."),
	...
	endpoint("plans_import", plansconnect.PlansServiceImportPlanProcedure, "Import a markdown plan", "Canonicalizes a markdown plan from a configured source location into the structured model."),
	endpoint("plans_migrate", plansconnect.PlansServiceMigratePlanProcedure, "Canonicalize a fallback plan", "Ensures a fallback-resolved plan resides in the canonical home store."),
