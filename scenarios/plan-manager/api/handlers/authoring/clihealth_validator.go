...
func (v cliHealthCommandValidator) ValidateCommandReference(ctx context.Context, req internalauthoring.CommandReferenceRequest) (internalauthoring.CommandReferenceResult, error) {
	resolver := v.resolver
	if resolver == nil {
		resolver = discovery.NewResolver(discovery.ResolverConfig{})
	}
	...
	if err != nil {
		return internalauthoring.CommandReferenceResult{}, fmt.Errorf("resolve cli-health URL: %w", err)
	}
	...
	if err != nil {
		return internalauthoring.CommandReferenceResult{}, fmt.Errorf("marshal request: %w", err)
	}
	...
	if err != nil {
		return internalauthoring.CommandReferenceResult{}, fmt.Errorf("create request: %w", err)
	}
	call.Header.Set("Content-Type", "application/json")
	call.Header.Set("Accept", "application/json")
	resp, err := client.Do(call)
	if err != nil {
		return internalauthoring.CommandReferenceResult{}, err
	}
	defer resp.Body.Close()
	respBody, _ := io.ReadAll(io.LimitReader(resp.Body, 64*1024))
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return internalauthoring.CommandReferenceResult{}, fmt.Errorf("status %d: %s", resp.StatusCode, strings.TrimSpace(string(respBody)))
	}
	var decoded struct {
		Result struct {
	...
	if err := json.Unmarshal(respBody, &decoded); err != nil {
		return internalauthoring.CommandReferenceResult{}, fmt.Errorf("decode response: %w", err)
	}
