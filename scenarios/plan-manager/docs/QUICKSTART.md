...
```bash
API_PORT=$(vrooli scenario port plan-manager API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.plan_manager.v1.<domain>.<Service>/<Method>
```
...
```bash
plan-manager notes list
plan-manager notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port plan-manager API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.plan_manager.v1.notes.NotesService/ListNotes" \
