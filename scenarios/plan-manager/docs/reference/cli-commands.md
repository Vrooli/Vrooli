...
| `log bug-add <plan-or-execution> --title <t> [--severity --phase <phase-id\|ordinal> --detail --evidence --source-command --idempotency-key --run-id]` | `LogService.AddBug` | File a bug report; forwarded to scenario-qa through Plan Manager's internal bug adapter. Downstream unavailability persists `pending`; rejection persists `sync_failed`; retry with `log sync`. |
| `log record-add <plan-or-execution> --title <t> [--phase <phase-id\|ordinal> --detail --evidence --source-command --idempotency-key --run-id]` | `LogService.AddRecord` | Capture a reusable record; forwarded to Swarm Manager through Plan Manager's internal record adapter. Downstream unavailability persists `pending`; rejection persists `sync_failed`; retry with `log sync`. |
