...
#### `POST /vrooli.plan_manager.v1.notes.NotesService/ListNotes`
...
| **CLI** | `plan-manager notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.plan_manager.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.plan_manager.v1.notes.NotesService/CreateNote`
...
| **CLI** | `plan-manager notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.plan_manager.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.plan_manager.v1.notes.NotesService/GetNote`
...
| **CLI** | `plan-manager notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.plan_manager.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `plan-manager notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/plan-manager/v1/notes/notes.proto`.
