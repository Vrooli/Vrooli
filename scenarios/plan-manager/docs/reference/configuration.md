...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/plan-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `plan-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/plan-manager/config.json`
3. `~/.vrooli/config/plan-manager/config.json`
4. `~/.config/vrooli/plan-manager/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port plan-manager API_PORT`)
