...
`template-manager orient plan-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient plan-manager
...
```bash
cat > /tmp/prd_context_plan-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) plan-manager \
  --context-file /tmp/prd_context_plan-manager.md \
  --publish \
  --json
```
...
```bash
cat > /tmp/requirements_context_plan-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) plan-manager \
  --context-file /tmp/requirements_context_plan-manager.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/plan-manager/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest plan-manager <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> plan-manager
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario plan-manager
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> plan-manager <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/plan-manager/v1/<domain>/`.
