...
```bash
vrooli scenario status plan-manager
make stop
vrooli scenario start plan-manager --clean-stale
```
...
```bash
make status
vrooli scenario port plan-manager UI_PORT
```
...
plan-manager status --auto-start

# Or override the API base for this invocation
plan-manager status --api-base "http://localhost:$(vrooli scenario port plan-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/plan-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/plan-manager.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/plan-manager.db"
```
...
`packages/proto/gen/go/plan-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/plan-manager/v1/`, and Python under
`packages/proto/gen/python/plan_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/plan-manager/v1/<domain>/`.
