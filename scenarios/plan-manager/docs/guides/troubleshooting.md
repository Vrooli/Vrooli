...
```bash
export SQLITE_PATH=/tmp/plan-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/plan-manager.db}")"
```
...
`packages/proto/gen/typescript/js/plan-manager/v1/`, and Python under
