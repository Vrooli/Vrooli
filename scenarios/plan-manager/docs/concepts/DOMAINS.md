...
- Purpose: the guided composer wizard (OT-P0-002) — walk the plan's sections in order, then author each phase as a first-class draft object with API-owned guided steps at every transition. The wizard validates structure as it goes, derives the regression-anchor **intent**, bootstraps a prompt-manager skill pack into relevant context, and tells agents to use search-hub directly when docs/records/code context is useful.
- Primary archetype: orchestration / workflow.
- Owns: authoring-session progression, phase-draft progression, the **change-boundary section** (`acceptance_allow`/`acceptance_deny`, with an `OPERATOR_ONLY` escape) authored before references/anchor, prompt-manager skill-pack discovery (`skill-pack`) that auto-adds global relevant-context skills, typed relevant-context submit/update/remove, API-owned guided-step payloads, the structure-validation gate (incl. boundary placeholder rejection), authoring-time `cli:` command-reference feedback, **boundary-native** regression-anchor **intent** derivation (deterministic, derived from the change boundary; no git-control-tower) and the remaining mechanical autofill behind seams.
- Does not own: the plan record itself (delegates writes to `plans`); command truth (delegates to CLI Health); reference *discovery providers* (consumes `search-hub` as-is); the actual baseline **snapshot/diff** and reference staleness computation (delegates to `validation` — captured only for explicit certification; authoring records intent only).
...
- Purpose: the guided runner (OT-P0-003, OT-P1-001/002) — phase status transitions, just-in-time setup context injection (`start`/`status`/`context`/`resume`/`continue`/`next`), an optional execution-start **freshen inputs** step for explicit certification (delegated to `validation`), outcome-assessed ordinary completion, strict certification when selected, the canonical structured handoff, and per-plan velocity. Decisions/findings/bugs/records are no longer captured here — they are recorded in the `log` domain, and execution reads compact log summaries through the read-only `LogLedger` seam for its just-in-time context and handoff.
...
- Does not own: the typed work-product ledger itself (delegates to `log` — decisions, findings, bug reports, records, notes); the prose final-message handoff (orchestration-layer concern — see [`INTEGRATIONS.md`](INTEGRATIONS.md)); promotion of candidate findings to real bugs (`log promote` / operator triage); the validation it surfaces and optional certification baseline capture (delegates to `validation`, which owns git-control-tower — execution never imports it directly, via the `InputFreshener` seam).
...
- Tests: resume-point derivation, continue-loop guidance, advisory outcome-assessment and certification evidence gates, typed completion nudges (`record_finding`/`file_bug`/`capture_record`/`confirm_phase_status`), handoff assembly from the log summary read through `LogLedger`, once-per-execution first-start-via-continue context emission, and execution-start completion-policy capture.
...
- Seams: `Resolver` (plan-id/slug or execution-id handle → canonical plan+execution scope), `BugReporter` (forward bug reports → scenario-qa), `RecordWriter` (forward records → Swarm Manager records). Bug/record forwarding is owned INTERNALLY; production uses live local HTTP adapters with explicit pending-stub fallbacks, and a failed forward is never fatal — the local entry persists `pending`/`sync_failed` and is retried via `plan-manager log sync`.
...
- Tests: typed Add* + dedup, promotion preserves the finding, live downstream adapter request/reference/idempotency shape, downstream-unavailable → pending (non-fatal), sync retry, summary roll-up read by execution.
