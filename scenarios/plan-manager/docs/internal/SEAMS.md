...
| | |
|---|---|
| **Seam** | Authoring discovery + intent-derivation dependencies (`search-hub` for references and broad recall context, `prompt-manager` for curated skill/action context; the regression-anchor intent is pure/local) |
| **Interface** | `api/internal/authoring/seams.go::{AnchorIntentDeriver,ReferenceSuggester,ContextDiscoverer,CommandRunner}`. `ContextDiscoverer` EXECUTES the discovery probes server-side through `api/internal/probes` (contract decision D5): per (concept, probe) pair it runs `prompt-manager discover --type skill/--type all --json` and `search-hub query --type record,skill,doc --json` concurrently with capped fan-out and a per-probe timeout (default 20s, `NewCommandContextDiscovererWithTimeout`), parses the JSON into typed provenance (skill candidates carry the BARE slug — the renderer alone assembles the read command, D6), deduplicates by (kind, target), and converts each failed probe into a typed batch note — the step always returns. Authoring then persists a curated discovery batch: shortlist/longlist tiers, handles, no-resurrection merge semantics, and one-call disposition through `context-apply` (with `context-accept`/`context-reject` as a one-item lane). `ReferenceSuggester` shells `search-hub query --json`, routes hits by locator shape into a curated reference batch, and shares the same batch apply shape through `reference-apply`; `AnchorIntentDeriver` derives the **boundary-native** typed anchor intent block deterministically from the plan's `change_boundary`. |
| **Production wiring** | `api/handlers/authoring/module.go::Module` creates one LookPath-guarded `CommandRunner` (`internalauthoring.DefaultRunner()`) shared by the probe runner and reference suggester, and wires the pure `internalauthoring.DefaultAnchorIntentDeriver()` for the anchor. `prompt-manager` and `search-hub` are declared `required:false` dependencies in `.vrooli/service.json` (D8). |
| **Test fake** | `api/internal/probes/probes_test.go` drives the probe runner with fake runners (contract-fixture JSON in `internal/probes/testdata/`, hanging→timeout, exit-1, garbage output, nil runner) — no live network. `api/internal/authoring/hardening_test.go::TestDiscoverContextExecutesProbesAndDeduplicates` covers dedupe/provenance/per-probe degradation at the service level. |
| **Why it exists** | Discovery used to hand the agent probe *commands* to run manually; agents skipped the step under pressure. Executing and curating the probes server-side makes the sweep cheap and honest: the agent reviews a small proposal, applies it once, down dependencies degrade as batch notes, and the wizard never blocks. References remain *discovered* (recall-oriented) so suggestions must be reviewed, never auto-committed. |
