...
| **Interface** | `api/internal/authoring/seams.go::{AnchorIntentDeriver,ReferenceSuggester,ContextDiscoverer,CommandRunner}`. `ContextDiscoverer` EXECUTES the discovery probes server-side through `api/internal/probes` (contract decision D5): per (concept, probe) pair it runs `prompt-manager discover --type skill/--type all --json` and `search-hub query --type record,skill,doc --json` concurrently with a per-probe timeout (default 20s, `NewCommandContextDiscovererWithTimeout`), parses the JSON into typed candidates (skill candidates carry the BARE slug — the renderer alone assembles the read command, D6), deduplicates by (kind, target) with provenance (probe, score), and converts each failed probe into a typed degraded note — the step always returns. `ReferenceSuggester` shells `search-hub query --json` and routes hits by locator shape into reviewable `ReferenceCandidate`s; `AnchorIntentDeriver` derives the **boundary-native** typed anchor intent block deterministically from the plan's `change_boundary`. |
...
| **Why it exists** | Discovery used to hand the agent probe *commands* to run manually; agents skipped the step under pressure. Executing the probes server-side makes the sweep cheap and honest: candidates arrive reviewed-ready, a down dependency degrades per-probe with a typed note, and the wizard never blocks. References remain *discovered* (recall-oriented) so suggestions must be reviewed, never auto-committed. |
