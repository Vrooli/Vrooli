...
| | |
|---|---|
| **Seam** | Authoring skill-pack discovery + intent derivation (`prompt-manager` for skill packs; search-hub is direct agent/operator discovery; regression-anchor intent is pure/local). |
| **Interface** | `api/internal/authoring/seams.go::{AnchorIntentDeriver,SkillPackDiscoverer,CommandRunner}`. `SkillPackDiscoverer` runs `prompt-manager discover <concepts...> --type skill --json [--complexity <c>]`, parses ids/names/descriptions/scores/read commands/budget status, and returns ready-to-store global `RelevantContextItem` skills. There is no candidate queue, discovery batch, accept/reject lane, or search-hub mirroring in plan-manager. `AnchorIntentDeriver` derives the **boundary-native** typed anchor intent block deterministically from the plan's `change_boundary`. |
| **Production wiring** | `api/handlers/authoring/module.go::Module` creates one LookPath-guarded `CommandRunner` (`internalauthoring.DefaultRunner()`), wires `internalauthoring.NewCommandSkillPackDiscoverer(runner)`, and wires the pure `internalauthoring.DefaultAnchorIntentDeriver()` for the anchor. `prompt-manager` and `search-hub` remain `required:false` dependencies in `.vrooli/service.json`; search-hub is intentionally surfaced as a direct command recommendation rather than an authoring RPC. |
| **Test fake** | `api/internal/authoring/authoring_test.go` and `hardening_test.go` cover multi-skill auto-add, idempotence, manual-skill preservation, resolver suggestions, and degraded prompt-manager availability without blocking authoring. |
| **Why it exists** | The curated middle layer became a crutch: agents accepted the smallest proposal, batch ceremony hid useful upstream attribution, and plan-manager duplicated prompt-manager/search-hub UX. The seam now does one cheap, useful thing: bootstrap a broad prompt-manager skill pack into execution context while leaving broader search-hub discovery in the native tool output where confidence and attribution are visible. |

### Authoring SkillApplicabilityResolver (D7 steer seam)

| | |
|---|---|
| **Seam** | Boundary-driven skill steering at the global skill checkpoint. |
...
| **Why it exists** | Execution owns progress, outcome assessments, handoffs, and local velocity; plans own phase status and completion policy. Validation projects canonical Test Genie observations without deciding ordinary outcome acceptance. Baseline synchronization operates on requested evidence; ordinary start does not require capture. Validator unavailability remains unknown and does not independently prevent ordinary completion. Explicit certification retains required evidence. Execution reads the log ledger without owning downstream capture; optional capture does not gate ordinary completion. Velocity is persisted locally before best-effort sink emission. |
...
| **Production wiring** | `api/handlers/planlog/module.go` wires the `Resolver` over the plans service (slug→id) and the execution store (execution id → plan id). It also wires live HTTP adapters: `NewScenarioQABugReporter()` resolves Prompt Manager and writes `scenario-qa` `bug-inbox/code-defect/<slug>` knowledge entries; `NewSwarmRecordWriter()` resolves Swarm Manager and writes `/api/v1/records` records. The explicit pending stubs (`DefaultBugReporter()`/`DefaultRecordWriter()`) remain test/degraded fallbacks. |
| **Test fake** | `api/internal/planlog/planlog_test.go` injects a fake `Resolver` and fake `BugReporter`/`RecordWriter` (success / unavailable / error). `api/internal/planlog/downstream_test.go` pins live adapter request shape, lookup-before-create behavior, references, and unavailable mapping. |
| **Why it exists** | A bug report (→ scenario-qa) and a record (→ swarm-manager) are forwarded INTERNALLY so an agent never shells an external scenario CLI from the plan workflow. Forwarding is **never fatal**: discovery/network/5xx unavailability leaves the entry `pending`, downstream rejection leaves it `sync_failed`, and the local entry is always durable and retryable via `plan-manager log sync`. A nil `Resolver` degrades to treating the handle verbatim as a plan id (no execution binding). |
