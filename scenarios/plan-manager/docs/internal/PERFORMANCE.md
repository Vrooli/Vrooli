...
- Intended budget shape: interactive plan reads/writes via the API and the
  ...
  any fan-out to other scenarios (skill discovery, staleness, baseline)
...
- Focused unit, race, lint, type-check, and UI test runs have validated the
  current skill-pack implementation, but no stable latency budget has
...
- Skill-pack discovery is one prompt-manager command and degrades without
  blocking authoring when prompt-manager is unavailable. Search-hub discovery is
  intentionally run directly by the agent/operator, outside plan-manager's
  authoring latency path.
