...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("unit-health")
	if err != nil {
		return "", fmt.Errorf("resolve unit-health storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve unit-health db path: %w", err)
	}
	return sqliteFileDSN(path)
}
