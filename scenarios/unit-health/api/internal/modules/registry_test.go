...
import (
	...
	"unit-health/internal/testutil/db"
...
// domains over time, so pinning the count would couple this test to
// whichever domains happen to be present.
...
// table-owning domain's own *_test.go (the canonical apply-and-query
// coverage).
...
//
// Lifted from a per-domain test into this global parity check so the
// safety net applies automatically to every Connect-mounted domain
// registered in
