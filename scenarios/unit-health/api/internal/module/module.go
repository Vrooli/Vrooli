...
// (validation, health, …) returns a Module from its handlers package, and main.go
...
//
//   - Name is for diagnostics ("validation", "health", "tasks"). Free-form;
...
// errors) and validation (full set) round-trip the same way.
...
const (
	...
	// A file-upload sub-resource is the canonical worked example.
...
type RESTException struct {
	...
	ProtoPayloads RESTExceptionPayloads `json:"proto_payloads"`
}

// RESTExceptionPayloads declares the proto intent behind a REST exception:
// even though the transport is hand-authored REST, the request/response/error
// payload shapes still trace back to proto messages (or are explicitly "none"
// for a probe with no body). The structure phase's schema validation requires
// this block on every rest_exception.
type RESTExceptionPayloads struct {
	Request  RESTExceptionPayload `json:"request"`
	Response RESTExceptionPayload `json:"response"`
	Error    RESTExceptionPayload `json:"error"`
}

// RESTExceptionPayload is one side (request/response/error) of a REST
// exception's proto intent.
type RESTExceptionPayload struct {
...
// it. Command uses unit-health as the binary name placeholder, e.g.
// "unit-health validate scenario". Args lists positional/flag tokens for
