// Package discovery is Unit Health's Code Facts intake seam. It asks Code Facts
// for the target scenario's surfaces and parse units and normalizes them into a
// language-tagged Inventory the planner can turn into test workspaces.
//
// Code Facts is the primary inventory source for supported languages (Go,
// TypeScript). When Code Facts is unavailable or returns nothing, discovery
// degrades to a filesystem fallback and records an explicit DegradedReason so
// the engine never silently invents surfaces.
package discovery
...
// Surface is a discovered scenario surface tagged with the language facts Unit
// Health needs to choose a canonical test framework.
type Surface struct {
	ID             string
	Kind           string
	Language       string
...
// CodeFactsClient discovers surfaces by calling the Code Facts service.
