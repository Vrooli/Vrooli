...
func tsProjectionChecks(scenarioRoot string, ws Workspace) []ProjectionCheck {
	...
	checks := []ProjectionCheck{
		...
		projectionCheck(ws, "script.coverage", "package.json scripts", filepath.Join(ws.RootPath, "package.json"), "contains coverage", manifest.Scripts["test:coverage"], strings.Contains(manifest.Scripts["test:coverage"], "coverage"),
