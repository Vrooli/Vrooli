	...
	endpoints := []module.EndpointDescriptor{
		{
			...
			CLIMapping: &module.CLIMapping{
				Command: "unit-health lonely subcommand",
			},
		},
	}
...
func TestCrossCheck_PassesWhenRegistered(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "x", CLIMapping: &module.CLIMapping{Command: "unit-health x"}},
		{ID: "y_alias", CLIMapping: &module.CLIMapping{Command: "unit-health notes ll"}},
...
func TestBuildCLICommands_ResolvesEndpointIDs(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "health", CLIMapping: &module.CLIMapping{Command: "unit-health status"}},
		{ID: "notes_list", CLIMapping: &module.CLIMapping{Command: "unit-health notes list"}},
	}
	registered := []registeredCommand{
		{Name: "configure"},
		{Name: "notes list"},
...
// normalisation step: the endpoint's "unit-health notes list"
...
func TestStripBinaryPrefix(t *testing.T) {
	cases := []struct {
		...
		{in: "unit-health status", want: "status"},
		{in: "unit-health notes list", want: "notes list"},
		{in: "already-stripped", want: "already-stripped"},
		{in: "unit-health", want: "unit-health"}, // no trailing space → preserved
	}
