...
// Static label map so the `theme.choice.*` catalog keys have a literal
// callsite (the `strings.theme.choice[c]` dynamic index below resolves the
// same keys at runtime, but the no-unused-keys audit only sees static
// accessors).
const THEME_LABEL_KEYS = {
  light: strings.theme.choice.light,
  dark: strings.theme.choice.dark,
  system: strings.theme.choice.system,
} as const satisfies Record<ThemeChoice, (typeof strings.theme.choice)[keyof typeof strings.theme.choice]>;
