...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/unit-health/v1/health/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/unit-health/v1/health"`)
...
import (
    ...
    "unit-health/handlers/health"
    "unit-health/internal/clock"
    "unit-health/internal/module"
    "unit-health/internal/server"
    "unit-health/internal/testutil/assertx"
    "unit-health/internal/testutil/httpx"
    "unit-health/internal/testutil/mocks"
)
...
The proto schema in `packages/proto/schemas/unit-health/v1/health/health.proto`
...
|---|---|---|
| Wire contract | `packages/proto/schemas/unit-health/v1/notes/notes.proto` | `Note`, `service NotesService`, `ListNotesResponse`, `CreateNoteRequest`, `CreateNoteResponse`, `GetNoteRequest`, `GetNoteResponse` |
| REST metadata contract | `packages/proto/schemas/unit-health/v1/notes/attachments.proto` | `Attachment` and `UploadAttachmentResponse` for the multipart upload exception |
...
| REST error envelope | `packages/proto/schemas/unit-health/v1/errors/errors.proto` + `internal/httpx/errors.go::WriteError` | Typed body for REST exceptions, with canonical codes (`invalid_request`, `not_found`, `internal`) |
...
dynamically from `go.mod`, so it works after `unit-health`
substitution.
...
import (
    ...
    clitest "unit-health/cli/internal/testutil"
)
...
under `packages/proto/schemas/unit-health/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/unit-health/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.unit_health.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/unit-health/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/unit-health/v1/<domain>/`,
   `packages/proto/gen/typescript/js/unit-health/v1/<domain>/`, and
   `packages/proto/gen/python/unit_health/v1/<domain>/`.
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import notesv1 "github.com/vrooli/vrooli/packages/proto/gen/go/unit-health/v1/notes"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/unit-health/v1/notes/notes_pb";
