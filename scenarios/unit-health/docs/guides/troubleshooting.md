...
```bash
vrooli scenario status unit-health
# Then either:
make stop
# Or, as last resort:
pkill -f 'unit-health-api'
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/unit-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/unit-health.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/unit-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/unit-health.db}")"
```
...
`packages/proto/gen/go/unit-health/v1/`, TypeScript under
`packages/proto/gen/typescript/js/unit-health/v1/`, and Python under
`packages/proto/gen/python/unit_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/unit-health/v1/<domain>/`.
