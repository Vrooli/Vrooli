...
`vrooli scenario orient unit-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient unit-health
...
```bash
cat > /tmp/prd_context_unit-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
prd-control-tower prd generate unit-health \
...
```bash
prd-control-tower prd validate unit-health --json
```
...
```bash
cat > /tmp/requirements_context_unit-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
prd-control-tower requirements generate unit-health \
...
```bash
prd-control-tower requirements validate unit-health --json
```
