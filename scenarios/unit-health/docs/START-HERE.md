...
`template-manager orient unit-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient unit-health
...
```bash
vrooli scenario requirements validate unit-health --json
```
