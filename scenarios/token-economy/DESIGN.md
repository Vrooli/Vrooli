...
## Scenario adaptation — Token Economy

**This scenario has two audiences with opposite density needs, and the design
language is adopted for one of them directly and stretched for the other.**

*The minter console* is exactly what the Operational Console language is for. A
minter reviews a pending-approval queue, audits an append-only journal, manages
token types and catalog entries, and needs to see economy state at a glance.
That is dense, tabular, evidence-first operational work, and the kit's calm
neutral palette, semantic status colors, and information density are adopted
unchanged.

*The holder view* is the deliberate stretch. It may be used by a child. It must
be sparse, large-target, few-decision, and legible to someone who is not an
operator and never will be. It shows a balance, what that balance buys, what
they did to earn it, and nothing about the machinery underneath.

**How the two stay one product.** The token contract is binding for both and is
not forked: identical color tokens, identical type scale, identical status
semantics, identical motion rules. What differs is *composition*, not
vocabulary — the holder view uses the upper end of the spacing scale, the
larger end of the type scale, and a much smaller set of primitives per screen.
A holder surface that used different colors for the same states would make the
product feel like two products; one that used the minter's density would be
unusable by its actual reader.

**Where the holder view exceeds the kit's floor rather than relaxing it.**
Accessibility is stricter here, not looser: touch targets at the upper end of
the guidance rather than the minimum, icon-plus-label everywhere (never icon
alone), no state conveyed by color alone, and every earn and redeem action
fully reachable by keyboard and screen reader. No celebratory motion is ever
the only signal that something succeeded, and all motion respects
`prefers-reduced-motion`. WCAG 2.2 AA is the floor for both surfaces.

**Branding.** The product chrome stays neutral so the household's own economy
is the thing with personality — a minter names a token type and gives it a
symbol and color, and that identity carries through both views, exports, and
notifications. `brand-manager` tokens are referenced, never redefined.

The full experience intent, including per-state communication priorities, lives
in [`experience/`](experience/); the domain surfaces those pages present are
mapped in [`docs/concepts/DOMAINS.md`](docs/concepts/DOMAINS.md).
