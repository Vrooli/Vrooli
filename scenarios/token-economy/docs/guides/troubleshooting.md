...
```bash
vrooli scenario status token-economy
make stop
vrooli scenario start token-economy --clean-stale
```
...
token-economy status --auto-start

# Or override the API base for this invocation
token-economy status --api-base "http://localhost:$(vrooli scenario port token-economy API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/token-economy/`.
...
The default route is `${SCENARIO_DATA_DIR}/token-economy.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/token-economy.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/token-economy.db}")"
```
...
`packages/proto/gen/go/token-economy/v1/`, TypeScript under
`packages/proto/gen/typescript/js/token-economy/v1/`, and Python under
`packages/proto/gen/python/token_economy/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/token-economy/v1/<domain>/`.
