...
`template-manager orient token-economy`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient token-economy
...
```bash
cat > /tmp/prd_context_token-economy.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start token-economy --interactive
# …answer the interview, then:
business-health wizard preview token-economy
business-health wizard apply token-economy
```
...
```bash
vrooli scenario requirements validate token-economy --json
```
...
```bash
cat > /tmp/requirements_context_token-economy.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate token-economy --json
business-health fix preview token-economy
business-health fix apply token-economy
```
...
validate token-economy --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/token-economy/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest token-economy <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> token-economy
