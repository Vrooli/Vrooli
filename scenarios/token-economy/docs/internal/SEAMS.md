...
| **Interface** | `internal/journal/provenance.go::Resolver` (`Resolve(ctx) (Actor, VerificationStatus)`) |
| **Production wiring** | Wraps the shared `packages/cli-core` verifier against agent-manager. |
| **Test fake** | Fake returning each of verified / unverified / absent. |
| **Why it exists** | `TKE-P0-011` requires all three statuses to be recorded correctly, including when agent-manager is unreachable. Testing that matrix against a live agent-manager would be slow and non-deterministic. The seam also keeps this scenario from inventing a second attribution model — it adapts the ecosystem's, and the adapter is one file. |
