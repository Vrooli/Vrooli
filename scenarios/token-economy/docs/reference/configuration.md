...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/token-economy.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `token-economy` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/token-economy/config.json`
3. `~/.vrooli/config/token-economy/config.json`
4. `~/.config/vrooli/token-economy/config.json`
...
```bash
token-economy configure api_base http://localhost:15001/api/v1
token-economy configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port token-economy API_PORT`)
