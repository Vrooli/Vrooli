...
`~/.vrooli/bin/token-economy`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start token-economy` if the API is unreachable |
...
```bash
token-economy status
token-economy status --json
```

### `token-economy configure <key> <value>`
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start token-economy`" is good.
