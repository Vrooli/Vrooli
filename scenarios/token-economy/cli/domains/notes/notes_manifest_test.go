...
// the template itself does not build because of token-economy placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_token_economy_v1_notes_notes_proto, "NotesService")
}
