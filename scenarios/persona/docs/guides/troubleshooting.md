...
```bash
vrooli scenario status persona
make stop
vrooli scenario start persona --clean-stale
```
...
```bash
make status
vrooli scenario port persona UI_PORT
```
...
persona status --auto-start

# Or override the API base for this invocation
persona status --api-base "http://localhost:$(vrooli scenario port persona API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/persona/`.
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/persona.db}")"
```
...
`packages/proto/gen/go/persona/v1/`, TypeScript under
`packages/proto/gen/typescript/js/persona/v1/`, and Python under
`packages/proto/gen/python/persona/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/persona/v1/<domain>/`.
