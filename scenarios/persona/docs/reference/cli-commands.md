...
`~/.vrooli/bin/persona`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start persona` if the API is unreachable |
...
```bash
persona notes list
persona notes list --json
```

#### `persona notes create --title <title> [--body <body>]`
...
```bash
persona notes create --title "First note" --body "Hello world"
```
...
```bash
persona notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start persona`" is good.
