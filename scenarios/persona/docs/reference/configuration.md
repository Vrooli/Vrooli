...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/persona.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `persona` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/persona/config.json`
3. `~/.vrooli/config/persona/config.json`
4. `~/.config/vrooli/persona/config.json`
...
```bash
persona configure api_base http://localhost:15001/api/v1
persona configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port persona API_PORT`)
