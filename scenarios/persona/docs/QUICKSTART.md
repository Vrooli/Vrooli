...
```bash
persona status
persona <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port persona API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.persona.v1.<domain>.<Service>/<Method>
```
...
```bash
persona notes list
persona notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port persona API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.persona.v1.notes.NotesService/ListNotes" \
