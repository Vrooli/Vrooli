...
## Scenario Adaptation — Persona

The Vrooli Operational Console language is adopted **unchanged in its
binding contract**: tokens, color roles, typography scale, spacing,
radius, motion rules, status-color semantics, responsive
transformations, and accessibility floors all apply as written. What
follows records why it fits and where this scenario pushes on it.

**Users and posture.** The operator visits this console rarely and
almost always under mild pressure: something is blocked and waiting on
them. That is the opposite of a monitoring dashboard someone watches all
day. The calm, operational feel target is right; maximum information
density is not. Where the language offers a density choice, this
scenario takes the lower one and spends the space on making *state*
unambiguous.

**Workflow.** The dominant surface is the handoff queue, and it should
read as an inbox — a list of things needing a person, each with one
plain sentence saying what that person must do — rather than as a grid
of metrics. Persona detail pages read as records, not forms: what is
configured, what is missing, and what this identity has been used for.
The scenario's second most-used surface is the journal, which is a
read-only log and benefits directly from the language's dense table
treatment.

**Status semantics carry unusual weight here.** The whole product turns
on distinguishing *waiting*, *expired*, *refused*, and *completed*, and
those four are easy to blur. The semantic status colors are used
strictly for those meanings and never decoratively, and every one is
paired with an icon and a text label so the distinction never rests on
hue alone.

**Accessibility.** WCAG 2.2 AA is the floor, with three commitments the
kit does not supply on its own. No state is conveyed by color alone.
Every handoff action is completable by keyboard, because a handoff
arriving on a phone at an awkward moment is the normal case. One-time
codes and their expiry are announced through a live region, since a code
that silently expires while being read is an accessibility failure with
a real cost.

**Where this scenario deliberately does not use the language.** Nothing
in the token contract is overridden. The one intentional restraint is
motion: transitions that would animate a status change are suppressed,
because an animating status badge on a security surface reads as
uncertainty. `prefers-reduced-motion` is honored throughout regardless.
