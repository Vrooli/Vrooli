#!/bin/bash
# Phased test orchestrator for recommendation-engine scenario
set -euo pipefail
...
# Initialize shared runner with runtime auto-management so integration/business phases
# can assume the scenario is available when they execute.
testing::runner::init \
  --scenario-name "recommendation-engine" \
