...
func NewRecommendationService(db *sql.DB, qdrantConn *grpc.ClientConn) *RecommendationService {
	var qdrantClient pb.QdrantClient
	var pointsClient pb.PointsClient

	if qdrantConn != nil {
		qdrantClient = pb.NewQdrantClient(qdrantConn)
		pointsClient = pb.NewPointsClient(qdrantConn)
	}
...
func (s *RecommendationService) CreateUserInteraction(interaction *UserInteraction) error {
	...
	// Get scenario_id and item UUID from item external_id
	var scenarioID, itemUUID string
	err := s.db.QueryRow("SELECT scenario_id, id FROM items WHERE external_id = $1 LIMIT 1",
		interaction.ItemID).Scan(&scenarioID, &itemUUID)
	if err != nil {
		return fmt.Errorf("item not found: %v", err)
	}

	// Ensure user exists - try to insert, if conflict then select existing
	...
	`, userID, scenarioID, interaction.UserID, time.Now())
	...
	err = s.db.QueryRow("SELECT id FROM users WHERE scenario_id = $1 AND external_id = $2",
		scenarioID, interaction.UserID).Scan(&userUUID)
	if err != nil {
	...
	_, err = s.db.Exec(query, interaction.ID, userUUID, itemUUID,
		interaction.InteractionType, interaction.InteractionValue,
		contextJSON, interaction.Timestamp)
...
func (s *RecommendationService) GetSimilarItems(itemID, scenarioID string, limit int, threshold float64) ([]SimilarItem, error) {
	...
	limitUint := uint32(limit)
	searchResponse, err := s.pointsClient.Scroll(context.Background(), &pb.ScrollPoints{
		CollectionName: "item-embeddings",
		Filter: &pb.Filter{
			Must: []*pb.Condition{
				{
	...
	thresholdFloat32 := float32(threshold)
	searchRequest := &pb.SearchPoints{
		CollectionName: "item-embeddings",
		Vector:         referenceVector,
		Limit:          uint64(limit),
		ScoreThreshold: &thresholdFloat32,
		Filter: &pb.Filter{
			Must: []*pb.Condition{
				{
					ConditionOneOf: &pb.Condition_Field{
	...
	response, err := s.pointsClient.Search(context.Background(), searchRequest)
	if err != nil {
		return nil, err
	}

	// Initialize as empty slice (not nil) to ensure JSON marshaling returns []
	items := make([]SimilarItem, 0)
...
func (s *RecommendationService) GetRecommendations(userID, scenarioID string, limit int, algorithm string, excludeItems []string) ([]Recommendation, error) {
	...
	args := []interface{}{scenarioID, userID}

	// Add exclude items clause if provided
	if len(excludeItems) > 0 {
		placeholders := make([]string, len(excludeItems))
		for i, item := range excludeItems {
			args = append(args, item)
			placeholders[i] = fmt.Sprintf("$%d", len(args))
		}
		query += fmt.Sprintf(" AND i.external_id NOT IN (%s)", strings.Join(placeholders, ","))
	}
