...
`template-manager orient quality-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient quality-health
...
- The generated example domain has been removed. Add real Quality Health
  domains directly rather than recreating template sample behavior.
...
and generated examples — mixes two kinds of guidance. Tokens, motion,
...
```bash
cat > /tmp/prd_context_quality-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) quality-health \
  --context-file /tmp/prd_context_quality-health.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate quality-health --json
```
...
```bash
cat > /tmp/requirements_context_quality-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) quality-health \
  --context-file /tmp/requirements_context_quality-health.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/quality-health/v1/<domain>/<domain>.proto`
...
- [x] Delete the generated sample domain from API, CLI, UI, proto,
      selectors, locale strings, endpoint metadata, tests, and BAS.
- [x] Verify no active product surface or generated proto output refers
      to the sample domain.
- [x] Run scenario validation.
...
```bash
react-component-library components ingest quality-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> quality-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario quality-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> quality-health <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/quality-health/v1/<domain>/`.
...
- The generated example domain has been removed; only real Quality
  Health domains should be added from here.
...
## Adding The First Product Domain

Use plural package/folder names such as `audits`, `contracts`, or
`findings`; use PascalCase for components and Go exported names.

For a normal proto-backed CRUD domain:

1. Add proto messages and a service under
   `packages/proto/schemas/quality-health/v1/<domain>/`, then run
