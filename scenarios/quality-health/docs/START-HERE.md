...
`vrooli scenario orient quality-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient quality-health
...
```bash
prd-control-tower prd generate quality-health \
...
```bash
prd-control-tower prd validate quality-health --json
```
...
```bash
prd-control-tower requirements generate quality-health \
...
```bash
prd-control-tower requirements validate quality-health --json
```
