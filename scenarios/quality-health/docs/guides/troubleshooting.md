# Troubleshooting — Quality Health
...
```bash
vrooli scenario status quality-health
make stop
vrooli scenario start quality-health --clean-stale
```
...
```bash
make status
vrooli scenario port quality-health UI_PORT
```
...
quality-health status --auto-start

# Or override the API base for this invocation
quality-health status --api-base "http://localhost:$(vrooli scenario port quality-health API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/quality-health/`.
...
The default route is `${SCENARIO_DATA_DIR}/quality-health.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/quality-health.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/quality-health.db}")"
```
...
`packages/proto/gen/go/quality-health/v1/`, TypeScript under
`packages/proto/gen/typescript/js/quality-health/v1/`, and Python under
`packages/proto/gen/python/quality_health/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/quality-health/v1/<domain>/`.
