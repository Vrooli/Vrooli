...
Routing lives in **one** registry: `api/internal/rules`. Each rule declares applicability, fix class, contract pack ID, and evaluator seam. Surface-level rules declare a non-empty language; scenario-level rules attach to the synthetic `scenario` surface. `api/internal/contracts` groups those rules into operator-facing packs, so dispatch and contract output cannot drift.
...
| Rule ID | Scope | Fix Class | Required v1 Behavior |
|---|---|---|---|
| `TS_CONFIG_STRICT` | any TypeScript surface | `autofix` | `strict: true`, `noUncheckedIndexedAccess: true`, parseable JSONC, and required protective phrases. Self-skips for JavaScript-only surfaces. |
...
| `GO_LINT_REQUIRED_LINTERS` | any Go surface | `autofix` | Required linters must be effectively enabled, not merely mentioned: `errcheck`, `gofumpt`, `govet`, `ineffassign`, `staticcheck`, `typecheck`, `unused`. |
| `GO_DANGEROUS_PATTERNS` | Go source | `detection_only` (source-semantic repair) | Bare `//nolint` directives are warning findings; reasoned suppressions are allowed. |
| `MAKEFILE_QUALITY_GATES` | Scenario root | `autofix` | Makefile must expose quality gates for Node and Go formatting/linting. Commented-out targets do not satisfy this rule. |
| `QUALITY_COVERAGE_GAP` | any uncovered surface | `detection_only` | Info-only honesty signal: a discovered surface matched no contract pack. See [Coverage Gaps](#coverage-gaps). |
...
Suppressions are governed, not banned. A suppression with a written reason is allowed; a bare suppression is a warning finding.

Allowed examples:

- `@ts-expect-error third-party theme package lacks published types`
- `//nolint:errcheck // best-effort cleanup during process shutdown`

Bare `@ts-ignore`, bare `@ts-expect-error`, and bare `//nolint` hide contract failures without intent and should be replaced with reasoned suppressions or real code fixes.
