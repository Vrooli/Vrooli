...
| Strict config or protective comment missing/weakened (`TS_CONFIG_STRICT`, `ESLINT_*`, `TYPECHECK_PLANNER_COVERAGE`, `GO_LINT_*`) | Restore the contract or planner input, then fix the underlying code that motivated the weakening. | `ui-health` for UI typing/hook fixes; `test` for build/test-gate wiring |
| Dangerous-pattern cluster (`TS_DANGEROUS_PATTERNS`, `GO_DANGEROUS_PATTERNS`; `autofix_available: false`) | Replace unsafe assertions/suppressions with validated types, narrower guards, or a written `//nolint` reason; document any intentional residue. | `ui-health` for UI type ambiguity |
