...
| Strict config or protective comment missing/weakened (`TS_CONFIG_STRICT`, `ESLINT_*`, `NODE_BUILD_TYPECHECK`, `GO_LINT_*`) | Restore the contract via autofix, then fix the underlying code that motivated the weakening. | `ui-health` for UI typing/hook fixes; `test` for build/test-gate wiring |
