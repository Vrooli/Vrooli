...
//
// quality-health shells out to lint tools (golangci-lint, shellcheck, bash)
// that may live on PATH or in a per-user bin directory a sudo'd PATH cannot
// see. This package centralizes that resolution so platform behavior
// (PATHEXT/.exe on Windows, ~/.local/bin on POSIX) lives in one place. The
// repo-root internal/hostreqkit does the same for the platform binary, but
// Go's internal-package rule makes it unimportable from this scenario's module
// — this is the scenario-local equivalent, not a fork of it.
package hostbin
...
// which a sudo'd PATH commonly misses.
