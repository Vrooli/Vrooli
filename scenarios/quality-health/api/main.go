...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("quality-health")
	if err != nil {
		return "", fmt.Errorf("resolve quality-health storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve quality-health db path: %w", err)
	}
	return sqliteFileDSN(path)
}
