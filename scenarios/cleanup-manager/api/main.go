...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("cleanup-manager")
	if err != nil {
		return "", fmt.Errorf("resolve cleanup-manager storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve cleanup-manager db path: %w", err)
	}
	return sqliteFileDSN(path)
}
...
func main() {
	...
	if preflight.Run(preflight.Config{ScenarioName: "cleanup-manager"}) {
		return
	}
