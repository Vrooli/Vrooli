...
// the template itself does not build because of cleanup-manager placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_cleanup_manager_v1_notes_notes_proto, "NotesService")
}
