...
// architecture metadata: every proto-bound command must carry cli-core primitive
// evidence matching its manifest architecture.primitive, so CLI Health can verify
// (not merely trust) the L4 claim. `attach` is the documented REST upload
// exception appended outside the manifest binding path, so it carries no
// primitive evidence by design.
func TestNotesCommands_CarryVerifiedPrimitiveEvidence(t *testing.T) {
	...
	cliapptest.AssertPrimitiveEvidence(t, attach, "")
}
