...
> **Validation**: Enforced by `business-health` (`validate scenario cleanup-manager`)
...
- **Deployment surfaces**: Go CLI (`cleanup-manager` scan, plan, policy, apply, audit, provider), Connect-RPC API for scenario integrations, and a React operational UI for disk pressure, provider estimates, policy controls, plan review, apply status, and audit history.
...
- [ ] OT-P1-003 | Adjacent scenario handoff | Document or wire system-monitor and vrooli-autoheal remediation paths through cleanup-manager while owner scenarios retain private deletion logic through provider hooks.
...
- Integration strategy: cleanup-manager is a meta-scenario/interface-enabler. Other scenarios call it through CLI or Connect, while scenario-private cleanup remains implemented by owner scenarios through provider hooks. system-monitor observes disk pressure; vrooli-autoheal requests remediation; cleanup-manager governs policy and audit.
