...
Cleanup Manager's product architecture is intentionally policy-first:
providers do not decide whether cleanup is allowed. Providers describe
what they can estimate, preview, apply, and verify; the planner combines
those observations with operator policy; the apply service refuses to
mutate unless the caller presents the exact plan id, policy version,
provider version, approval mode, and idempotency key.

All host effects must pass through typed seams: filesystem, process,
Docker, journald, clock, and owner-scenario provider clients. Production
constructors are the only place concrete host executors are wired. Unit,
integration, and scenario tests use fakes, spies, and temp roots so the
test suite cannot delete real files, prune Docker, vacuum journals, or
...
| Contracts (`packages/proto/schemas/cleanup-manager/`) | Wire shape | Proto messages/services and generated clients | Hand-written route/type mirrors |
...
- scenario proto schemas relocated to
  `packages/proto/schemas/cleanup-manager/`.
...
packages/proto/schemas/cleanup-manager/v1/<domain>/<file>.proto
       ...
       make generate
       │
       ├──▶ packages/proto/gen/go/cleanup-manager/v1/...              (api, cli)
       ├──▶ packages/proto/gen/go/cleanup-manager/v1/...connect       (Connect-Go)
       ├──▶ packages/proto/gen/typescript/js/cleanup-manager/v1/...   (ui)
...
|---|---|---|---|
| 2026-07-08 | None yet. | Generated from `react-vite`. | Update when the scenario intentionally diverges. |
