...
`template-manager orient cleanup-manager`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient cleanup-manager
...
```bash
cat > /tmp/prd_context_cleanup-manager.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start cleanup-manager --interactive
# …answer the interview, then:
business-health wizard preview cleanup-manager
business-health wizard apply cleanup-manager
```
...
```bash
vrooli scenario requirements validate cleanup-manager --json
```
...
```bash
cat > /tmp/requirements_context_cleanup-manager.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate cleanup-manager --json
business-health fix preview cleanup-manager
business-health fix apply cleanup-manager
```
...
- [ ] Run `make test`.
- [ ] Confirm the gate: `template-manager orient cleanup-manager` should not
...
- Wire contracts belong in
  `packages/proto/schemas/cleanup-manager/v1/<domain>/`.
