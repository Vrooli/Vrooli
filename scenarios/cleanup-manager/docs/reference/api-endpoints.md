...
#### `POST /vrooli.cleanup_manager.v1.notes.NotesService/ListNotes`
...
| **CLI** | `cleanup-manager notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.cleanup_manager.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.cleanup_manager.v1.notes.NotesService/CreateNote`
...
| **CLI** | `cleanup-manager notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.cleanup_manager.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.cleanup_manager.v1.notes.NotesService/GetNote`
...
| **CLI** | `cleanup-manager notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.cleanup_manager.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `cleanup-manager notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/cleanup-manager/v1/notes/notes.proto`.
