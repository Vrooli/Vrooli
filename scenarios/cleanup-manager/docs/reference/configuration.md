...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/cleanup-manager.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `cleanup-manager` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/cleanup-manager/config.json`
3. `~/.vrooli/config/cleanup-manager/config.json`
4. `~/.config/vrooli/cleanup-manager/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port cleanup-manager API_PORT`)
