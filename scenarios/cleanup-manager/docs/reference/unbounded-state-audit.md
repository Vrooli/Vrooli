...
across 2,469 rows — and cleanup-manager could not see a byte of it, because
...
| `registered` | Registered as a cleanup-manager owner provider. |
| `exempt` | Growth is bounded by construction, or the footprint is negligible. |
| **`unbounded`** | Grows without a bound and is not reachable by cleanup-manager. |
...
Bounded by an owner-side policy. Not registered with cleanup-manager, which is
