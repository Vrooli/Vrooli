...
`~/.vrooli/bin/cleanup-manager`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start cleanup-manager` if the API is unreachable |
...
```bash
cleanup-manager status
cleanup-manager status --json
```

### `cleanup-manager configure <key> <value>`
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start cleanup-manager`" is good.
