# Troubleshooting — Cleanup Manager
...
```bash
vrooli scenario status cleanup-manager
# Then either:
make stop
# Or, as last resort:
pkill -f 'cleanup-manager-api'
...
cleanup-manager status --auto-start

# Or override the API base for this invocation
cleanup-manager status --api-base "http://localhost:$(vrooli scenario port cleanup-manager API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/cleanup-manager/`.
...
The default route is `${SCENARIO_DATA_DIR}/cleanup-manager.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/cleanup-manager.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/cleanup-manager.db}")"
```
...
`packages/proto/gen/go/cleanup-manager/v1/`, TypeScript under
`packages/proto/gen/typescript/js/cleanup-manager/v1/`, and Python under
`packages/proto/gen/python/cleanup_manager/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/cleanup-manager/v1/<domain>/`.
