...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/cleanup-manager/v1/shared/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/cleanup-manager/v1/shared"`)
...
import (
    ...
    "cleanup-manager/internal/testutil/assertx"
    "cleanup-manager/internal/testutil/httpx"
    "cleanup-manager/internal/testutil/mocks"
)
...
The proto schema in `packages/proto/schemas/cleanup-manager/v1/shared/health.proto`
...
|---|---|---|
| Wire contract | `packages/proto/schemas/cleanup-manager/v1/notes/notes.proto` | `Note`, `service NotesService`, `ListNotesResponse`, `CreateNoteRequest`, `CreateNoteResponse`, `GetNoteRequest`, `GetNoteResponse` |
| REST metadata contract | `packages/proto/schemas/cleanup-manager/v1/notes/attachments.proto` | `Attachment` and `UploadAttachmentResponse` for the multipart upload exception |
...
| REST error envelope | `packages/proto/schemas/cleanup-manager/v1/shared/errors.proto` + `internal/httpx/errors.go::WriteError` | Typed body for REST exceptions, with canonical codes (`invalid_request`, `not_found`, `internal`) |
...
dynamically from `go.mod`, so it works after `cleanup-manager`
substitution.
...
import (
    ...
    clitest "cleanup-manager/cli/internal/testutil"
)
...
under `packages/proto/schemas/cleanup-manager/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/cleanup-manager/v1/<domain>/<name>.proto`.
   ...
   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/cleanup-manager/v1/<domain>;<domain>_v1";
   ```
...
2. **Regenerate.** From the repo root:
   ...
   `packages/proto/gen/go/cleanup-manager/v1/<domain>/`,
   `packages/proto/gen/typescript/js/cleanup-manager/v1/<domain>/`, and
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import domainv1 "github.com/vrooli/vrooli/packages/proto/gen/go/cleanup-manager/v1/<domain>"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/cleanup-manager/v1/<domain>/<domain>_pb";
