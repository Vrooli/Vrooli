...
export interface VoiceInputButtonProps extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, "children" | "onPointerDown" | "onPointerUp"> {
 ...
 */
export function VoiceInputButton({ state = "idle", mode = "always-on", level = 0, timeoutProgress = 0, error, rejectionReason, partialTranscript, onStart, onStop, onPrepare, onDismissError, onTranscribeAnyway, className = "", disabled, onPointerCancel, ...props }: VoiceInputButtonProps) {
  ...
  return <div className={`relative shrink-0 ${className}`}>
    <button
      ...
      {...props}
    >
      {active && <span aria-hidden="true" className={`absolute inset-x-0 bottom-0 rounded-[inherit] transition-[height] duration-75 ${state === "recovering" || mode === "always-on" ? "bg-app-info/30" : "bg-app-danger/30"}`} style={{ height: `${Math.round(normalizedLevel * 100)}%` }} />}
