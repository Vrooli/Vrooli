...
describe("ExperienceSurface", () => {
  it("exposes the stable semantic lifecycle contract without prescribing layout", () => {
    render(<ExperienceSurface surfaceId="results" state="ready" className="custom-layout"><p>Ready results</p></ExperienceSurface>);
  ...
  it("announces transient and failure state only when a message is declared", () => {
    render(<ExperienceSurface surfaceId="results" state="error" statusMessage="Results could not load"><p>Try again</p></ExperienceSurface>);
    expect(screen.getByRole("status", { name: "Results could not load" })).toHaveAttribute("aria-live", "polite");
  });

  it("marks loading surfaces busy for assistive technology", () => {
    render(<ExperienceSurface surfaceId="results" state="loading"><p>Loading results</p></ExperienceSurface>);
    expect(screen.getByText("Loading results").closest("section")).toHaveAttribute("aria-busy", "true");
  });
});
