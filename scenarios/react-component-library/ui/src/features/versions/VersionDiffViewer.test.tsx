...
const rows = [
  makeDiffRow(
    { lineNumber: 1, text: "before", op: DiffOp.REMOVE },
    { lineNumber: 1, text: "after", op: DiffOp.ADD },
  ),
  makeDiffRow(
    { lineNumber: 2, text: "same", op: DiffOp.EQUAL },
    { lineNumber: 2, text: "same", op: DiffOp.EQUAL },
  ),
];
...
describe("VersionDiffViewer", () => {
  ...
  it("switches modes and copies the right-side source", async () => {
    ...
    expect(screen.getByRole("button", { name: "versions.diff.unified" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
    ...
    expect(screen.getByRole("button", { name: "versions.diff.split" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
  });
