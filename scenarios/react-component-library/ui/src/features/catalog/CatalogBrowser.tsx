...
import { ExperienceSurface, type ExperienceSurfaceState } from "../../components/ExperienceSurface/versions/1.0.0/ExperienceSurface";
  ...
  const content = (
    <>
      ...
      <span className="flex gap-space-3xs text-[11px] text-app-muted-foreground">
        <span className="rounded-pill bg-app-surface-muted px-space-2xs py-space-3xs">{t("catalog.adoptions", { defaultValue: "{{count}} adoptions", count: counts.direct })}</span>
        {isHook && <span className="rounded-pill bg-app-surface-muted px-space-2xs py-space-3xs">{t("catalog.effectiveAdoptions", { defaultValue: "{{count}} effective", count: counts.effective })}</span>}
      </span>
    </>
  );
  return (
    <Link
      ...
      className={[
        presentation === "cards" ? "flex min-h-24 flex-col justify-between rounded-panel border p-space-xs" : "touch-target flex items-center justify-between gap-space-2xs rounded-control px-space-2xs py-space-2xs",
...
function CatalogActions() {
  ...
  return <div className="space-y-space-2xs">
    ...
    {showAssisted && <form onSubmit={submit} className="grid gap-space-2xs rounded-panel border border-app-border p-space-xs text-sm">
      ...
      <label>{t("catalog.sourceScenario", { defaultValue: "Source scenario" })}<Input value={sourceScenario} onChange={(event) => setSourceScenario(event.target.value)} required className="mt-space-3xs" /></label>
      <label>{t("catalog.sourcePath", { defaultValue: "Source path" })}<Input value={sourcePath} onChange={(event) => setSourcePath(event.target.value)} required className="mt-space-3xs" placeholder={t(strings.catalog.sourcePathPlaceholder)} /></label>
  ...
  const readinessState: ExperienceSurfaceState = query.isLoading
    ...
    : query.error
      ...
      : assets.length === 0
        ...
        : "ready";
  const content = (
    <section data-testid={selectors.catalog.browser} className={compact ? "flex min-h-0 flex-1 flex-col gap-space-2xs" : "flex max-w-5xl flex-col gap-space-sm"}>
      {!compact && <CatalogActions />}
      <div className="flex items-center gap-space-3xs" role="tablist" aria-label={t("catalog.kindTabs", { defaultValue: "Asset kind" })}>
        {(["components", "hooks"] as const).map((kind) => <button key={kind} type="button" role="tab" aria-selected={tab === kind} data-testid={kind === "components" ? selectors.catalog.componentsTab : selectors.catalog.hooksTab} onClick={() => setTab(kind)} className={tab === kind ? "touch-target rounded-control bg-app-surface-muted px-space-xs py-space-2xs text-sm font-medium" : "touch-target rounded-control px-space-xs py-space-2xs text-sm text-app-muted-foreground hover:bg-app-surface-muted"}>{kind === "components" ? t("catalog.components", { defaultValue: "Components" }) : t("catalog.hooks", { defaultValue: "Hooks" })}</button>)}
      </div>
      <label className="relative block"><span className="sr-only">{t("catalog.search", { defaultValue: "Search catalog" })}</span><Search aria-hidden className="absolute start-2 top-2.5 h-4 w-4 text-app-muted-foreground" /><Input data-testid={selectors.catalog.search} value={match} onChange={(event) => setMatch(event.target.value)} className="ps-space-lg" placeholder={t("catalog.searchPlaceholder", { defaultValue: "Search assets" })} /></label>
      <div className="flex items-center gap-space-3xs" aria-label={t("catalog.presentation", { defaultValue: "Catalog presentation" })}>
        {([ ["tree", Network], ["list", List], ["cards", Grid2X2] ] as const).map(([mode, Icon]) => <button key={mode} type="button" data-testid={selectors.catalog.presentation} aria-pressed={presentation === mode} aria-label={t(`catalog.${mode}`, { defaultValue: mode })} onClick={() => setPresentation(mode)} className={presentation === mode ? "touch-target rounded-control bg-app-surface-muted p-space-2xs" : "touch-target rounded-control p-space-2xs text-app-muted-foreground hover:bg-app-surface-muted"}><Icon aria-hidden className="h-4 w-4" /></button>)}
      </div>
      ...
      {!query.isLoading && !query.error && assets.length === 0 && <p className="rounded-control border border-dashed border-app-border p-space-xs text-sm text-app-muted-foreground">{t("catalog.empty", { defaultValue: "No matching assets." })}</p>}
      {presentation === "tree" ? <div className="space-y-space-xs">{groups.map(([group, groupedAssets]) => <section key={group}><div className="mb-space-3xs flex items-center justify-between px-space-3xs text-xs font-medium uppercase text-app-muted-foreground"><span>{group}</span><span>{groupedAssets.reduce((total, asset) => total + adoptionCounts(asset).direct, 0)}</span></div><div className="space-y-space-3xs">{groupedAssets.map((asset) => <AssetRow key={asset.id} asset={asset} presentation="tree" selected={selectedID === asset.id} onNavigate={onNavigate} currentTab={selectedID ? assetInfoTab(new URLSearchParams(location.search)) : undefined} />)}</div></section>)}</div> : <div className={presentation === "cards" ? "grid gap-space-2xs sm:grid-cols-2" : "space-y-space-3xs"}>{assets.map((asset) => <AssetRow key={asset.id} asset={asset} presentation={presentation} selected={selectedID === asset.id} onNavigate={onNavigate} currentTab={selectedID ? assetInfoTab(new URLSearchParams(location.search)) : undefined} />)}</div>}
    </section>
  );
