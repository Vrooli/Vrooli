...
const deviceGroup = (id: DevicePresetId): "phone" | "tablet" | "desktop" | "responsive" => {
  if (id === "iphone-14" || id === "iphone-se") return "phone";
  if (id === "ipad" || id === "ipad-pro") return "tablet";
    ...
    >
      ...
      >
        ...
        {(["phone", "tablet", "desktop"] as const).map((group) => {
          ...
          return (
            <section key={group} className="border-b border-app-border py-3 first:pt-0">
        ...
        <section className="border-b border-app-border py-3">
          ...
          <p className="mt-1 text-xs text-app-muted-foreground">{t(strings.components.emulator.responsiveDescription)}</p>
          <Button
            type="button"
            data-testid={selectors.components.emulator.presetOption}
            ...
            className="mt-2 h-9 px-2.5 text-xs"
            onClick={() => emulator.setPreset("responsive")}
          >
            {t(strings.components.emulator.responsiveLabel)}
          </Button>
          {emulator.isResponsive && (
            <div data-testid={selectors.components.emulator.dimensions} className="mt-2 grid grid-cols-2 gap-2">
              <label className="text-xs text-app-muted-foreground" htmlFor={widthInputId}>
                ...
                <Input
                  ...
                  className="mt-1 h-9 min-h-9 font-mono text-xs"
                />
              </label>
        ...
        <section className="pt-3">
          <label className="text-xs text-app-muted-foreground" htmlFor={zoomId}>
            <span className="flex items-center gap-1.5"><ZoomIn aria-hidden className="h-3.5 w-3.5" />{t(strings.components.emulator.zoomLabel)}</span>
            <Select
              id={zoomId}
          ...
          <div className="mt-3 flex gap-2">
            <Button
              ...
              variant="secondary"
              className="h-9 flex-1 gap-1.5 px-2 text-xs"
            >
