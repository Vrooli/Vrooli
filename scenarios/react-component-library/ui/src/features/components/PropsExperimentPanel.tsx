...
interface ControlDefinition { key: string; label?: string; kind: ControlKind; options?: string[]; minimum?: number; maximum?: number; minLength?: number; maxLength?: number; defaultValue?: unknown }
    ...
    try {
      ...
      if (Array.isArray(parsed.fields)) {
        return parsed.fields.flatMap((field) => {
          ...
          const value = field as { path?: unknown; label?: unknown; kind?: unknown; options?: unknown; default?: unknown };
          ...
          return [{
            ...
            minimum: typeof (value as { minimum?: unknown }).minimum === "number" ? (value as { minimum: number }).minimum : undefined,
            maximum: typeof (value as { maximum?: unknown }).maximum === "number" ? (value as { maximum: number }).maximum : undefined,
            minLength: typeof (value as { minLength?: unknown }).minLength === "number" ? (value as { minLength: number }).minLength : undefined,
            maxLength: typeof (value as { maxLength?: unknown }).maxLength === "number" ? (value as { maxLength: number }).maxLength : undefined,
            defaultValue: value.default,
          }];
        });
      }
    } catch { /* validation errors are surfaced by the server/runtime */ }
  }
  return [];
}

function storyArgs(storyContract?: ComponentStory, storyID?: string): Record<string, unknown> | undefined {
  if (!storyContract?.storiesJson || !storyID) return undefined;
  try {
    const stories = JSON.parse(storyContract.storiesJson) as Array<{ id?: unknown; args?: unknown }>;
    const story = stories.find((candidate) => candidate?.id === storyID);
    if (!story?.args || typeof story.args !== "object" || Array.isArray(story.args)) return undefined;
    return story.args as Record<string, unknown>;
  } catch { return undefined; }
}

function defaultsFor(controls: ControlDefinition[]): Record<string, unknown> {
  return controls.reduce<Record<string, unknown>>((values, control) => (
    control.defaultValue === undefined ? values : withValueAtPath(values, control.key, control.defaultValue)
  ), {});
}

function environmentControls(storyContract?: ComponentStory): Array<{ key: string; label: string; options: string[] }> {
  if (!storyContract?.environmentJson) return [];
  try {
    const parsed = JSON.parse(storyContract.environmentJson) as { fixtures?: Array<{ key?: unknown; options?: unknown }> };
    return Array.isArray(parsed.fixtures) ? parsed.fixtures.flatMap((fixture) => typeof fixture?.key === "string" && Array.isArray(fixture.options) && fixture.options.every((option) => typeof option === "string") ? [{ key: fixture.key, label: fixture.key, options: fixture.options }] : []) : [];
  } catch { return []; }
}

function valueAtPath(values: Record<string, unknown>, path: string): unknown {
  return path.split(".").reduce<unknown>((current, segment) => current && typeof current === "object" && !Array.isArray(current) ? (current as Record<string, unknown>)[segment] : undefined, values);
}

function withValueAtPath(values: Record<string, unknown>, path: string, value: unknown): Record<string, unknown> {
  const [head, ...tail] = path.split(".");
  if (!head) return values;
  if (tail.length === 0) return { ...values, [head]: value };
  const current = values[head];
  return { ...values, [head]: withValueAtPath(current && typeof current === "object" && !Array.isArray(current) ? current as Record<string, unknown> : {}, tail.join("."), value) };
}

export function PropsExperimentPanel({ storyId, storyName, initialArgs = {}, initialEnvironment = {}, storyContract, status = "idle", message, onApply, onReset }: PropsExperimentPanelProps) {
  ...
  const values = (() => { try { const parsed = JSON.parse(draft); return parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed as Record<string, unknown> : {}; } catch { return {}; } })();
  ...
  useEffect(() => {
    ...
    setDraft(JSON.stringify({ ...defaultsFor(controlsFor(storyContract)), ...(storyArgs(storyContract, storyId) ?? initialArgs) }, null, 2));
    setParseError("");
