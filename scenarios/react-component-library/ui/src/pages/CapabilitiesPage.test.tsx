...
describe("CapabilitiesPage", () => {
  ...
  it("makes availability and recovery guidance readable without the CLI", async () => {
    describeCapabilities.mockResolvedValue({ states: [{ id: "agent-manager", name: "Agent Manager", description: "Workflow execution", dependencyKind: "scenario", dependencySlug: "agent-manager", status: "unavailable", message: "Scenario is stopped", actionLabel: "Start Agent Manager", operatorCommand: "vrooli scenario start agent-manager --json" }] });

    renderWithProviders(<CapabilitiesPage />);

    expect(await screen.findByRole("heading", { name: "Capability readiness" })).toBeInTheDocument();
  ...
  it("distinguishes available and unknown states with dependency fallback copy", async () => {
    describeCapabilities.mockResolvedValue({ states: [
      { id: "ready", name: "Ready", description: "Available", dependencyKind: "resource", dependencySlug: "postgres", status: "available" },
      { id: "unknown", name: "Unknown", description: "Needs inspection", dependencyKind: "scenario", dependencySlug: "worker", status: "unknown" },
    ] });
    renderWithProviders(<CapabilitiesPage />);
