...
function countAdoptionIssues(
  ...
  return adoptions.filter(
    (adoption) =>
      ...
      adoption.localStatus === LocalStatus.MODIFIED ||
      adoption.localStatus === LocalStatus.MISSING,
  ).length;
}
...
function Metric({
  ...
  return (
    <div className="rounded-panel border border-app-border bg-app-surface p-space-xs">
      <dt className="text-xs text-app-muted-foreground">{label}</dt>
      <dd
        className={
          tone === "warning"
            ? "mt-space-3xs text-2xl font-semibold text-app-danger"
            : "mt-space-3xs text-2xl font-semibold text-app-foreground"
        }
      >
        {value}
      </dd>
...
function AssetLink({ asset, tab }: { asset: CatalogAsset; tab?: ReturnType<typeof assetInfoTab> }) {
  return (
    <Link
      to={assetPath(asset.id, tab ? { tab } : {})}
      className="flex items-center justify-between gap-space-xs rounded-control px-space-2xs py-space-2xs text-sm hover:bg-app-surface-muted"
    >
...
export function DashboardPage() {
  ...
  const assets = useQuery({
    ...
    queryFn: () => listCatalogAssets({ limit: 200, assetKind: 1 }),
    staleTime: 30_000,
  });
  const adoptions = useQuery({
    queryKey: ["dashboard", "adoptions"],
    queryFn: () => adoptionsClient.listAdoptions({ limit: 500 }),
    staleTime: 30_000,
  });
  ...
  for (const adoption of allAdoptions)
    scenarios.set(adoption.scenario, (scenarios.get(adoption.scenario) ?? 0) + 1);
  const issueCount = countAdoptionIssues(allAdoptions);
  const recentlyEvolved = [...allAssets]
    .sort((a, b) => ((b.updatedAt?.seconds ?? 0n) > (a.updatedAt?.seconds ?? 0n) ? 1 : -1))
    .slice(0, 4);

  return (
    <div
      data-testid="dashboard-page"
      className="mx-auto flex w-full max-w-6xl flex-col gap-space-md"
    >
      <section aria-labelledby="dashboard-welcome">
        <p className="text-sm text-app-muted-foreground">
          {t("dashboard.greeting", { defaultValue: "Good morning" })}
        </p>
        <h2 id="dashboard-welcome" className="mt-space-3xs text-2xl font-semibold tracking-tight">
          {t("dashboard.review", { defaultValue: "Review attention items" })}
        </h2>
      </section>

      <dl className="grid gap-space-xs sm:grid-cols-2 xl:grid-cols-3">
        <Metric
          label={t("dashboard.assets", { defaultValue: "Assets" })}
          value={allAssets.length}
        />
        <Metric
          label={t("dashboard.adoptionCurrency", { defaultValue: "Adoption currency" })}
          value={allAdoptions.length - issueCount}
        />
        <Metric
          label={t("dashboard.needsReview", { defaultValue: "Need review" })}
          value={issueCount}
      ...
      <div className="grid gap-space-sm lg:grid-cols-2">
        ...
        >
          ...
          {adoptions.isLoading ? (
            <p className="mt-space-xs text-sm text-app-muted-foreground">
              {t("dashboard.loading", { defaultValue: "Loading dashboard…" })}
            </p>
          ) : issueCount > 0 ? (
            <p className="mt-space-xs text-sm text-app-muted-foreground">
              {t("dashboard.attentionDetail", {
                defaultValue: "{{count}} adoption(s) have version or local-change drift.",
                count: issueCount,
              })}
            </p>
          ) : (
        ...
        >
          ...
          <div className="mt-space-2xs divide-y divide-app-border">
            {recentlyEvolved.length ? (
              recentlyEvolved.map((asset) => (
                <AssetLink key={asset.id} asset={asset} tab={currentTab} />
              ))
            ) : (
              <p className="py-space-2xs text-sm text-app-muted-foreground">
                {t("dashboard.noAssets", { defaultValue: "No assets yet." })}
              </p>
            )}
          </div>
        </section>
        ...
        >
          ...
          <div className="mt-space-xs space-y-space-2xs">
            {scenarios.size ? (
              ...
              <p className="text-sm text-app-muted-foreground">
                {t("dashboard.noAdoptions", { defaultValue: "No adoptions recorded yet." })}
              </p>
            )}
          </div>
        </section>
        ...
        >
          ...
          <div className="mt-space-xs space-y-space-2xs text-sm">
            <Link
              to="/catalog"
              className="flex items-center justify-between rounded-control px-space-2xs py-space-2xs hover:bg-app-surface-muted"
            >
              <span>
