...
describe("api/catalog helpers", () => {
  ...
  it("reads coverage and ranked next work through the generated Connect client", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ rows: [], maturity: { total: 0, atOrAboveTarget: 0, byRung: {} } }),
        { status: 200, headers: { "content-type": "application/json" } },
      ),
    );
  ...
  it("reports missing coverage and capability registry failures", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(
      new Response(
        JSON.stringify({ definitions: [], states: [{ id: "agent-manager", status: "available" }] }),
        { status: 200, headers: { "content-type": "application/json" } },
      ),
    );
    await expect(describeCapabilities()).resolves.toMatchObject({
      states: [{ id: "agent-manager" }],
    });
