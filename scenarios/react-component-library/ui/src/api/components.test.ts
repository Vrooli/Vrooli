...
describe("api/components catalog helpers", () => {
  ...
  it("forwards catalog and story reads to the generated Components client", async () => {
    ...
    await expect(listCatalogAssets({ limit: 20, assetKind: 1 })).resolves.toMatchObject({
      components: [],
    });

    fetchSpy.mockResolvedValueOnce(
      new Response(JSON.stringify({ component: { id: "asset-1" } }), {
        status: 200,
        headers: { "content-type": "application/json" },
      }),
    );
    await expect(getCatalogAsset("asset-1")).resolves.toMatchObject({
      component: { id: "asset-1" },
    });
    ...
    await expect(
      listComponentStories({ componentId: "asset-1", version: "1.0.0" }),
  ...
  it("returns experience data and rejects a response with no experience contract", async () => {
    fetchSpy.mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          experience: { componentId: "asset-1", states: [], claims: [], evidence: [] },
        }),
        { status: 200, headers: { "content-type": "application/json" } },
      ),
    );
    await expect(getComponentExperience("asset-1")).resolves.toMatchObject({
      componentId: "asset-1",
    });
