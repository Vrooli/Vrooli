...
describe("api/componentTests", () => {
  ...
  it("posts a test request and returns its report", async () => {
    fetchSpy.mockResolvedValueOnce(
      new Response(JSON.stringify({ report: { id: "report-1", verdict: "passed", results: [] } }), {
        status: 200,
      }),
    );

    await expect(
      runComponentTest({ componentId: "asset-1", version: "1.0.0", includeClosure: true }),
    ).resolves.toMatchObject({ id: "report-1" });
    expect(fetchSpy).toHaveBeenCalledWith(
      expect.stringContaining("/RunComponentTest"),
  ...
  it("returns a saved report and normalizes an empty report collection", async () => {
    fetchSpy.mockResolvedValueOnce(
      new Response(JSON.stringify({ report: { id: "report-2", verdict: "passed", results: [] } }), {
        status: 200,
      }),
    );
