...
| Component versions | versions | SQLite `component_versions`, `component_version_files` | Manifest indexer | Rebuilt by `components index` | `component_versions` mirrors the entry for legacy reads; `component_version_files` is the authoritative per-file path/content/hash set. |
...
| `component_version_files` | components | `api/internal/components/schema.sql` | File tabs, file-scoped content RPCs, preview/adoption units |
