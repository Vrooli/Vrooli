...
|---|---|---|---|---|---|
| Component manifests | components | `library/components/<slug>/component.json` | Git-tracked file | Until component is removed | Manifest owns `libraryId`, display metadata, `slot`, tags, design-style affinities, latest, draft, and deprecated versions. |
...
| Component registry | components | SQLite `components` | Manifest indexer | Rebuilt by `components index` | Soft-referenced by other domains through component id/library id. Includes the indexed adoption `slot`. |
...
| Component headers | components | SQLite `component_headers` | Latest version source header | Rebuilt by `components index` | Stores non-structural header metadata such as `category`; identity, version, and deps are stored in typed columns/tables instead. |
| Component design affinities | components | SQLite `component_design_affinities` | `component.json` `designStyles[]` | Rebuilt by `components index` | Validated against `templates/design/*/metadata.json`; advisory fit signal for search, adoption workflows, and UI. |
...
| `component_headers` | components | `api/internal/components/schema.sql` | Component get, category search facet |
