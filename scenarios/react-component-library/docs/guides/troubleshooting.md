...
```bash
vrooli scenario status react-component-library
make stop
vrooli scenario start react-component-library --clean-stale
```
...
react-component-library status --auto-start

# Or override the API base for this invocation
react-component-library status --api-base "http://localhost:$(vrooli scenario port react-component-library API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/react-component-library/`.
...
The default route is `${SCENARIO_DATA_DIR}/react-component-library.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/react-component-library.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/react-component-library.db}")"
```
...
`packages/proto/gen/go/react-component-library/v1/`, TypeScript under
`packages/proto/gen/typescript/js/react-component-library/v1/`, and Python under
`packages/proto/gen/python/react_component_library/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/react-component-library/v1/<domain>/`.
