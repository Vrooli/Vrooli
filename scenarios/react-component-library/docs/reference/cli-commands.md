...
hash. It applies only when `--confirm` and the matching plan hash are provided.
Draft cleanup remains an internal lifecycle operation; drafts are mutable
work-in-progress and are removed from the manifest rather than retained as
released history.
