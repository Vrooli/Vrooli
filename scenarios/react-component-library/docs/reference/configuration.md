...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/react-component-library.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `react-component-library` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/react-component-library/config.json`
3. `~/.vrooli/config/react-component-library/config.json`
4. `~/.config/vrooli/react-component-library/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port react-component-library API_PORT`)
