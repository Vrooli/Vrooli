...
- **API**: `api/handlers/health/handler_test.go` — table-driven, real
  ...
  in `packages/proto/schemas/react-component-library/v1/health/health.proto`;
...
5. **Generated proto types** — every endpoint's wire shape lives in
   ...
   (`healthv1 "github.com/vrooli/vrooli/packages/proto/gen/go/react-component-library/v1/health"`)
...
The proto schema in `packages/proto/schemas/react-component-library/v1/health/health.proto`
...
dynamically from `go.mod`, so it works after `react-component-library`
substitution.
...
under `packages/proto/schemas/react-component-library/`.

Steps:

1. **Author the schema.** In a generated scenario, add
   `packages/proto/schemas/react-component-library/v1/<domain>/<name>.proto`.
   Use snake_case in the proto package directive
   (`package vrooli.react_component_library.v1.<domain>;`) and add a
   `go_package` option pointing at the per-scenario gen path:

   ```protobuf
   option go_package = "github.com/vrooli/vrooli/packages/proto/gen/go/react-component-library/v1/<domain>;<domain>_v1";
   ```
...
3. **Wire it on the API side.** Import the generated Go type in your
   ...
   ```go
   import notesv1 "github.com/vrooli/vrooli/packages/proto/gen/go/react-component-library/v1/notes"
...
4. **Wire it on the UI side.** Import the generated TS schema and use
   ...
   import { ListResponseSchema } from "@vrooli/proto-types/react-component-library/v1/notes/notes_pb";
