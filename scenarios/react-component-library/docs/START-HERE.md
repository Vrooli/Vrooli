# Start Here — React Component Library
...
`template-manager orient react-component-library`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient react-component-library
...
```bash
cat > /tmp/prd_context_react-component-library.md <<'EOF'
Overview:
- Purpose:
...
```bash
cat > /tmp/requirements_context_react-component-library.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
