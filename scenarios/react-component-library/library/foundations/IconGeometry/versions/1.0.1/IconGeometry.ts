 ...
 * @description Provides the shared geometry contract for consistent icon sizing and alignment.
 * @version 1.0.1
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:IconGeometry
 * @vrooliComponentSourceSlot foundations.icon-geometry */
...
function segmentCount(approximateLength: number): number {
  ...
  return Math.max(
    MIN_SEGMENTS,
    Math.min(MAX_SEGMENTS, Math.ceil(approximateLength / FLATTEN_STEP)),
  );
}
 ...
 */
export function geometryFromElement(
  ...
  const geometry = geometryFromNodes(
    nodes,
    root.getAttribute("viewBox") ?? DEFAULT_VIEW_BOX,
    sampleCount,
  );
...
export function morphCompatibility(from: IconGeometry, to: IconGeometry): MorphCompatibility {
  ...
  const closedAgreement =
    matchedPairs.length === 0
      ? 0
      : matchedPairs.filter((pair) => pair.from.closed === pair.to.closed).length /
        matchedPairs.length;
