 ...
 * @description A small global layer normalizing browser defaults, focus rings, scrolling, safe areas, reduced motion, forced colours, text selection, and print output, so no component compensates for browser inconsistency locally.
 * @version 1.2.5
 * @tags []
...
:where([data-rcl-control]) { appearance: none; box-sizing: border-box; border: var(--border-hairline) solid var(--color-border); border-radius: var(--radius-control); font: inherit; }
...
export function BaseStyles() {
  useLibraryStyleSheet("react-component-library:BaseStyles", "1.2.5", baseStyles);
  return null;
}
