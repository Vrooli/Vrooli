 ...
 * @description Resolves logical gesture directions from writing direction and anchor edge.
 * @version 1.3.1
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:GestureDirection
 * @vrooliComponentSourceSlot foundations.gesture-direction */
...
const MIRRORED: Record<PhysicalEdge, PhysicalEdge> = {
  left: "right",
  right: "left",
  top: "bottom",
  bottom: "top",
 ...
 */
export function resolveAnchorEdge(direction: WritingDirection, anchor: AnchorEdge): PhysicalEdge {
...
/** The logical axis on which a gesture claims ownership. */
export type Axis = "inline" | "block";

/** Resolves a physical edge to the logical axis it occupies. */
export function axisOf(edge: PhysicalEdge): Axis {
  return edge === "top" || edge === "bottom" ? "block" : "inline";
}

/** The PointerEvent coordinate corresponding to a physical edge. */
export function coordinateOf(edge: PhysicalEdge): "clientX" | "clientY" {
  return axisOf(edge) === "inline" ? "clientX" : "clientY";
}

/**
 * Safely performs pointer capture where browsers and test DOMs support it.
 * Capture is a capability, not a precondition for a gesture.
 */
export function withPointerCapture(
  target: Element,
  pointerId: number,
  ...
  try {
    ...
    // A pointer that was never captured, or is already released, is harmless.
  }
}
