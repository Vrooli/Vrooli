 ...
 * @description The typed variant system that maps component variants and states to tokenized styles and keeps arbitrary one-off visual values from spreading, exporting each recipe in a shape tools can enumerate.
 * @version 1.0.3
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:VisualRecipes
 * @vrooliComponentSourceSlot foundations.visual-recipes */
import { COMPONENT_TOKENS, SEMANTIC_TOKENS } from "@vrooli/react-component-library/Tokens/1";
...
export const CONTROL_SIZES = {
  sm: recipe("min-h-[var(--control-size-sm)]", "px-[var(--space-sm)]", "text-[var(--text-label)]"),
  md: recipe(
    "min-h-[var(--control-size-md)]",
    `px-[${COMPONENT_TOKENS.controlPadding}]`,
    `text-[${SEMANTIC_TOKENS.foreground}]`,
  ),
  lg: recipe("min-h-[var(--control-size-lg)]", "px-[var(--space-md)]", "text-[var(--text-label)]"),
} as const;
