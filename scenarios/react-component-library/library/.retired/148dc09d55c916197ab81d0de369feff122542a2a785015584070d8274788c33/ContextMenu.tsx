...
export function ContextMenu({
  ...
  const anchoredPosition =
    desktop && anchor ? anchor.getBoundingClientRect() : null;
  const viewportBounds =
    typeof window !== "undefined"
      ? { height: window.innerHeight, width: window.innerWidth }
      : null;
  const position = desktop
    ? requestedPosition && viewportBounds
      ? {
          top: Math.max(
            8,
            Math.min(requestedPosition.y, viewportBounds.height - 320),
          ),
          left: Math.max(
            8,
            Math.min(requestedPosition.x, viewportBounds.width - 240),
          ),
        }
      : anchoredPosition
          ...
          >
            ...
            {items.map((item, index) => (
              ...
              >
                ...
                >
                  {item.icon ? (
                    <span data-rcl-context-menu-item-icon>{item.icon}</span>
                  ) : null}
