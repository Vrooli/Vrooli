 ...
 * @description Priority-resolved OS status-bar and app-chrome appearance, written to both the theme-color meta tag and the safe-area strip so platforms cannot disagree.
 * @version 1.0.0
 * @tags ["runtime","state","chrome","safe-area","mobile"]
...
function resolve(): ResolvedChrome | null {
  ...
  if (winner) {
    return { statusColor: winner.chrome.statusColor, fillColor: fillOf(winner.chrome), source: winner.key };
  }
...
export function useChromeContribution(
  ...
  useEffect(() => {
    ...
    return () => { chromeTheme.release(key); };
