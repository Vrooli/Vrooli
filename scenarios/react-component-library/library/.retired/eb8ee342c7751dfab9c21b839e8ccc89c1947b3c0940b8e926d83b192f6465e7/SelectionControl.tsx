...
[data-rcl-selection-row][data-kind="switch"] {
  ...
  --rcl-switch-travel: calc(
    var(--rcl-switch-w) - 2 * var(--border-strong) - var(--rcl-switch-thumb) - 2 * var(--rcl-switch-inset)
  );
...
function SelectionStyles() {
  return <StyleSheet name="selectioncontrol-1-0-6-1" css={styleSheet} />;
}
...
export const SelectionControl = withClassName(function SelectionControl({
  ...
  const finish = (pointerId: number, clientX: number) => {
    ...
    if (!drag || drag.pointerId !== pointerId) return;
    detachWindow();
    const committed = drag.moved
      ? offsetFor(drag, clientX) > drag.travel / 2
      : null;
  ...
  return (
    <>
      ...
      <label
        ...
        data-kind={kind}
        data-state={
          indeterminate ? "mixed" : shownChecked ? "checked" : "unchecked"
        }
