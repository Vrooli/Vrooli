...
function statusSignal(status: ApprovalPromptStatus) {
  ...
  if (status === "request-error" || status === "permission-denied")
...
export const ApprovalPrompt = withClassName(function ApprovalPrompt({
  ...
  title = resolveStrings(
    "ai.approval-prompt.review-this-action",
    "Review this action",
  ),
  description,
  consequences,
  ...
  const expiryText =
    expiresLabel ?? (expiry && now ? formatExpiry(expiry, now) : undefined);
  ...
  const stateAlert =
    resolvedStatus === "success" ? (
      <Alert
        ...
        tone="success"
        title={strings(
          "ai.approval-prompt.approval-recorded",
          "Approval recorded",
        )}
        description={strings(
    ...
    >
      ...
      <header data-rcl-approval-header>
        ...
        >
          <span data-rcl-approval-eyebrow>
            {strings(
              "ai.approval-prompt.permission-request",
              "Permission request",
            )}
          </span>
          <StatusIndicator
      ...
      {consequences ? (
        ...
        >
          <h3 id={`${titleId}-consequences`}>
            {strings(
              "ai.approval-prompt.what-happens-next",
              "What happens next",
            )}
          </h3>
      ...
      <div data-rcl-approval-consent>
        <span data-rcl-approval-consent-label>
          {strings(
            "ai.approval-prompt.confirm-the-exact-scope",
      ...
      {resolvedStatus !== "success" ? (
        <ButtonGroup
          label={strings(
            "ai.approval-prompt.approval-actions",
            "Approval actions",
          )}
