/**
 * @libraryId react-component-library:provenance-ink
 * @displayName ProvenanceInk
 * @description The honesty system for ambient displays: one resolver turns a reading's coverage and trust into one of four material inks, plus the ink stylesheet, chip and legend swatch.
 * @version 0.1.0
 * @tags ["ambient-display","provenance","foundation"]
...
export function qualify(reading: ProvenanceReading, resolution: Resolution, now = Date.now()): Qualifier {
...
const styles = `
  [data-figure][data-ink="solid"] { color: var(--color-foreground, #e8ecf3); text-shadow: 0 0 40px color-mix(in srgb, var(--color-glow, rgba(51,214,255,.5)) 40%, transparent); }
  ...
  [data-figure][data-ink="hollow"] [data-rcl-figure-affix] { color: transparent; -webkit-text-stroke: max(1.5px, 0.014em) color-mix(in srgb, var(--color-foreground, #e8ecf3) 78%, var(--color-provenance-gap, #b7a6ff)); }
  [data-figure][data-ink="dotted"] [data-rcl-figure-digits],
  [data-figure][data-ink="dotted"] [data-rcl-figure-affix] { color: transparent; -webkit-text-stroke: 0.7px color-mix(in srgb, var(--color-foreground, #e8ecf3) 40%, var(--color-provenance-gap, #b7a6ff)); background-image: radial-gradient(circle, color-mix(in srgb, var(--color-foreground, #e8ecf3) 92%, var(--color-provenance-gap, #b7a6ff)) 40%, transparent 44%); background-size: clamp(5px, 0.048em, 16px) clamp(5px, 0.048em, 16px); -webkit-background-clip: text; background-clip: text; }
  ...
  [data-rcl-ink-mark] { display: inline-block; margin-right: 0.6em; padding: 0.18em 0.55em; border: 1px solid currentColor; border-radius: var(--radius-pill, 9999px); font: 600 var(--text-micro, 0.7rem) var(--font-mono, ui-monospace, monospace); letter-spacing: 0.14em; text-transform: uppercase; vertical-align: middle; }
  [data-rcl-ink-mark][data-ink="solid"] { background: currentColor; color: var(--color-background, #05070e); }
  [data-rcl-ink-mark][data-ink="hollow"] { border-style: solid; color: color-mix(in srgb, var(--color-foreground, #e8ecf3) 70%, var(--color-provenance-gap, #b7a6ff)); }
  [data-rcl-ink-mark][data-ink="dotted"] { border-style: dotted; border-width: 1.5px; color: color-mix(in srgb, var(--color-foreground, #e8ecf3) 70%, var(--color-provenance-gap, #b7a6ff)); }
  [data-rcl-ink-mark][data-ink="unavailable"] { border-style: dashed; color: var(--color-warning, #f5b544); }
  [data-rcl-ink-swatch] { display: inline-flex; width: 1.6em; height: 1.6em; align-items: center; justify-content: center; margin-right: 0.45em; font: 700 1.05em var(--font-sans, system-ui, sans-serif); font-variant-numeric: tabular-nums; }
  ...
  [data-rcl-ink-swatch][data-ink="hollow"] { color: transparent; -webkit-text-stroke: 1.2px color-mix(in srgb, var(--color-foreground, #e8ecf3) 78%, var(--color-provenance-gap, #b7a6ff)); }
  [data-rcl-ink-swatch][data-ink="dotted"] { color: transparent; -webkit-text-stroke: 0.5px color-mix(in srgb, var(--color-foreground, #e8ecf3) 34%, var(--color-provenance-gap, #b7a6ff)); background-image: radial-gradient(circle, var(--color-foreground, #e8ecf3) 34%, transparent 38%); background-size: 3px 3px; -webkit-background-clip: text; background-clip: text; }
`;
...
export const InkMark = withClassName(function InkMark({ ink, children, className }: { ink: Exclude<Ink, "none">; children?: ReactNode; className?: string }) {
  return (
    <>
      <ProvenanceInkStyles />
      <span data-rcl-ink-mark data-ink={ink} className={className}>{children ?? INK_LABELS[ink]}</span>
    </>
  );
});

/** A legend glyph: the digit 8 in one ink, so a legend shows the materials themselves. */
export const InkSwatch = withClassName(function InkSwatch({ ink, className }: { ink: Exclude<Ink, "none">; className?: string }) {
  return (
    <>
      <ProvenanceInkStyles />
      <span data-rcl-ink-swatch data-ink={ink} className={className} aria-hidden="true">8</span>
    </>
  );
});