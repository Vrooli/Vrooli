...
import {
  Form,
  type FormValues,
} from "@vrooli/react-component-library/Form/1.0.1";
...
export interface AsyncFormSubmitError<TValues extends FormValues>
  extends Error {
...
function useFormSnapshot<TValues extends FormValues>(
  store: FormStore<TValues>,
) {
...
>({
  ...
  title =
    title ??
    libraryStrings("patterns.async-form-flow.create-or-edit", "Create or edit");
  const store = useMemo(
    () => createFormStore<TValues>({ initialValues, validate }),
    [initialValues, validate],
  );
  const [loadState, setLoadState] = useState<AsyncFormLoadState>(
    load ? "loading" : "success",
  );
  ...
  const showSubmitError =
    phase === "error" && (Boolean(formError) || submitCancelled);
        ...
        <header data-rcl-async-form-header>
          <div>
            <div data-rcl-async-form-kicker>
              {libraryStrings(
                "patterns.async-form-flow.async-workflow",
                "ASYNC WORKFLOW",
              )}
            </div>
          ...
          <div data-rcl-async-form-content>
            {showEmpty ? (
              <div data-rcl-async-form-empty>
                ...
                >
                  {libraryStrings(
                    "patterns.async-form-flow.start-fresh",
                    "Start fresh",
                  )}
                </button>
              </div>
              ...
              >
                {showSubmitError && (
                  <div
                    data-rcl-async-form-submit-status
                    data-phase="error"
                    role="alert"
                  >
                ...
                {phase === "success" && (
                  ...
                  >
                    ...
                    <span data-rcl-async-form-submit-copy>
                      <strong data-rcl-async-form-submit-title>
                        {libraryStrings(
                          "patterns.async-form-flow.all-set",
                          "All set",
                        )}
                      </strong>
                      ...
                      {onNavigate && (
                        ...
                        >
                          {libraryStrings(
                            "patterns.async-form-flow.continue",
                            "Continue",
                          )}
                        </button>
                      )}
                ...
                >
                  ...
                  >
                    {libraryStrings(
                      "patterns.async-form-flow.cancel-request",
                      "Cancel request",
                    )}
                  </button>
                  <button
