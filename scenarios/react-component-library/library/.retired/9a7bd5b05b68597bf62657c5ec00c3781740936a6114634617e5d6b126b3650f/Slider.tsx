...
[data-rcl-slider-input] {
  ...
  appearance: none;
  /* Vertical panning stays with the page; the horizontal drag is the control's. */
  touch-action: pan-y;
}
...
function SliderStyles() {
  return <StyleSheet name="slider-1-0-1-1" css={styleSheet} />;
}
...
export const Slider = withClassName(function Slider({
  ...
  const span = max - min;
  const fraction =
    span > 0 ? Math.min(1, Math.max(0, (resolved - min) / span)) : 0;
  ...
  const positionOf = (at: number) =>
    span > 0 ? Math.min(1, Math.max(0, (at - min) / span)) : 0;
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    const raw =
      latestRef.current + direction * numericStep * COARSE_STEP_MULTIPLIER;
    ...
    <>
      ...
      >
        ...
        <div data-rcl-slider-row>
          ...
          {showValue === "inline" && (
            <span data-rcl-slider-readout>{display}</span>
          )}
        </div>
