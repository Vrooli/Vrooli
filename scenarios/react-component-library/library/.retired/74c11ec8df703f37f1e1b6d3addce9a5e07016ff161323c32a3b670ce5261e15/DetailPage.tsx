...
const panel = {
  ...
  boxShadow:
    "var(--elev-raised, 0 1px 2px rgba(9, 18, 22, .06), 0 1px 3px rgba(9, 18, 22, .10))",
};
...
function StateView({ state, children }: { state: State; children: ReactNode }) {
  ...
  if (state === "loading")
    return (
      <div
        data-testid="templates.detail-page"
        role="status"
  ...
  if (state === "refreshing")
    return (
      <div role="status" style={panel}>
        <strong>
          {strings("templates.detail-page.refreshing", "Refreshing")}
        </strong>
        {children}
      </div>
    );
  ...
  if (state === "fatal-error")
    return (
      ...
      >
        {strings(
          "templates.detail-page.unable-to-load-this-page",
    ...
    <StateView state={state}>
      <div style={{ display: "grid", gap: 16 }}>
        <header>
          ...
          <p style={muted}>
            {strings(
              "templates.detail-page.a-focused-view-of-this-resource",
              "A focused view of this resource.",
            )}
          </p>
        </header>
          ...
          <dl>
            {(data?.entries ?? []).map((entry) => (
              <div key={entry.term}>
                ...
                <dd style={{ margin: "4px 0 12px", fontWeight: 600 }}>
                  {entry.description}
                </dd>
              </div>
            ))}
