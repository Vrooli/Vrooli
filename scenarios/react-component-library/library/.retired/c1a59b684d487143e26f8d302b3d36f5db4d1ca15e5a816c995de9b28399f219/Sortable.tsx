...
import { AutoAnimateLayout } from "@vrooli/react-component-library/AutoAnimateLayout/1.0.1";
...
import { useOptimisticAction } from "@vrooli/react-component-library/useOptimisticAction/1.0.1";
...
export const Sortable = withClassName(function Sortable<T>({
  ...
  label =
    label ??
    libraryStrings("manipulation.sortable.sortable-list", "Sortable list");
  ...
  useEffect(() => {
    if (optimistic.status === "error") {
      ...
      announce(
        "The new order could not be saved. The previous order was restored.",
      );
    }
  ...
  const drag = useDrag({
    ...
    onStart: () => {
      ...
      setKeyboardDragging(false);
      announce(
        `${id} picked up. Use arrow keys to move, Enter to save, or Escape to cancel.`,
      );
    },
    onMove: (event) => {
      ...
      const target = itemsRef.current
        ...
        .find(({ rect }) =>
          rect ? event.clientY < (rect.top + rect.bottom) / 2 : false,
        );
    ...
    onKeyboardMove: (_dx, dy) => {
      ...
      const to = Math.max(
        0,
        Math.min(itemsRef.current.length - 1, from + direction),
      );
      const next = move(itemsRef.current, from, to);
