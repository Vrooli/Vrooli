...
describe("dismissing from anywhere on the row", () => {
  /** Drags starting on the given element, as the browser delivers it. */
  const dragFrom = (target: Element, from: number, to: number) => {
    fireEvent.pointerDown(target, {
      pointerId: 1,
      clientX: from,
      clientY: 0,
  ...
  const action = (id: string) =>
    screen.getByTestId(`patterns.swipe-actions.action.${id}`);

  // Once the actions are revealed they cover most of the row, so a dismiss
  // bound to the face alone leaves the user grabbing at a shrinking sliver.
  it("closes when the drag begins on a revealed action", () => {
    renderWithProviders(<Row />);
    drag([0, 200]);
    expect(root().getAttribute("data-open")).toBe("true");
    dragFrom(action("close"), 200, 20);
    expect(root().getAttribute("data-open")).toBe("false");
  });

  it("does not press the action the dismiss began on", () => {
    const close = vi.fn();
    renderWithProviders(<Row actions={makeActions({ close })} />);
    drag([0, 200]);
    dragFrom(action("close"), 200, 20);
    // A real browser follows the drag with a pointer click, which reports a
    // non-zero detail; that click belongs to the drag.
    fireEvent.click(action("close"), { detail: 1 });
    expect(close).not.toHaveBeenCalled();
  });

  // A tap has no movement, so it never locks an axis and stays a press.
  it("still presses an action that is tapped rather than dragged", () => {
    const close = vi.fn();
    renderWithProviders(<Row actions={makeActions({ close })} />);
    drag([0, 200]);
    fireEvent.pointerDown(action("close"), {
      pointerId: 2,
      clientX: 10,
      clientY: 0,
      button: 0,
    ...
    fireEvent.pointerUp(action("close"), {
      pointerId: 2,
      clientX: 10,
      clientY: 0,
      pointerType: "touch",
  ...
  it("closes on a short drag back rather than demanding the whole travel", () => {
    renderWithProviders(<Row />);
    drag([0, 200]);
    // A little over one action's width back, far short of the full reveal.
    dragFrom(face(), 200, 200 - WIDTH * 0.7);
    expect(root().getAttribute("data-open")).toBe("false");
  });

  it("stays open when the drag back is too small to mean anything", () => {
    renderWithProviders(<Row />);
    drag([0, 200]);
    dragFrom(face(), 200, 190);
    expect(root().getAttribute("data-open")).toBe("true");
  });

  it("restores the revealed position when the browser cancels a dismiss", () => {
    renderWithProviders(<Row />);
    drag([0, 200]);
    const target = face();
    fireEvent.pointerDown(target, {
...
describe("keyboard activation", () => {
  // Enter on a focused action fires a click with no gesture behind it. Gating
  // on the moved flag alone would swallow it after any earlier swipe.
  it("runs an action from the keyboard after a drag", () => {
    const close = vi.fn();
    renderWithProviders(<Row actions={makeActions({ close })} />);
    drag([0, 200]);
    fireEvent.click(screen.getByRole("button", { name: "Close" }), {
