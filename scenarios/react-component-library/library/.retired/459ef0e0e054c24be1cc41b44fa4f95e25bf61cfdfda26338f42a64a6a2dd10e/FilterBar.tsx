...
export const FilterBar = withClassName(function FilterBar({
  ...
  return (
    <>
      <StyleSheet name="filterbar-1-1-4-1" css={filterBarStyles} />
      <form
        role="search"
        ...
        <div className="rcl-filter-bar__controls">
          <div className="rcl-filter-bar__query">
            <SearchInput
              aria-label={queryLabel}
              placeholder={strings(
                "data-display.filter-bar.search-records",
                "Search records",
              )}
              value={resolvedQuery}
        ...
        {options.length > 0 && (
          <fieldset style={{ border: 0, margin: 0, padding: 0 }}>
            <legend className="rcl-filter-bar__legend">
              {strings(
                "data-display.filter-bar.filter-by-status",
                "Filter by status",
              )}
            </legend>
            <Cluster
              gap="xs"
              aria-label={strings(
                "data-display.filter-bar.available-filters",
                "Available filters",
              )}
              role="group"
            >
