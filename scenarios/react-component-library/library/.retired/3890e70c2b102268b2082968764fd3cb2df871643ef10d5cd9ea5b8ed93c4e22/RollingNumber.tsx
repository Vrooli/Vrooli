 ...
 * @description A number drawn in an ink. Identical glyph box in every ink so a metric going live never moves a pixel; only the digits that changed roll.
 * @version 0.1.0
 * @tags ["ambient-display","figure","motion"]
...
const styles = `
  ...
  [data-rcl-digit="rolled"] { animation: rcl-figure-roll var(--motion-digit, 380ms) cubic-bezier(.2,.7,.2,1); }
 ...
 */
export const RollingNumber = withClassName(function RollingNumber({ value, format, unit, ink, scale, placeholder = "—", className }: RollingNumberProps) {
  ...
  return (
    <>
      ...
      <ProvenanceInkStyles />
      <span data-rcl-figure={scale} data-figure data-ink={ink} data-value={value ?? undefined} className={className}>
        {formatted.prefix ? <span data-rcl-figure-affix>{formatted.prefix}</span> : null}
        <span data-rcl-figure-digits aria-label={`${formatted.prefix}${formatted.text}${formatted.suffix}`}>
          {glyphs.map((glyph, i) => (
            <span key={`${rollKey}-${i}`} data-rcl-digit={glyph.changed ? "rolled" : "held"} aria-hidden="true">
              {glyph.char}
            </span>
          ))}
        </span>
