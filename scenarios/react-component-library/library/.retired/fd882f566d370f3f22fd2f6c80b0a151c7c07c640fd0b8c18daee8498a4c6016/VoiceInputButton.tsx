...
export function VoiceInputButton({
  ...
  const testId = (props as VoiceInputButtonProps & { "data-testid"?: string })[
    "data-testid"
  ];
  ...
  return (
    ...
    >
      ...
      {visibleError && (
        <div
          className="absolute left-1/2 top-full z-10 mt-1 flex w-52 -translate-x-1/2 items-start gap-1.5 rounded border border-app-warning/50 bg-app-surface px-2 py-1 text-[10px] text-app-warning shadow-lg"
          style={{
            left: "50%",
        ...
        >
          ...
          <button
            ...
            type="button"
            className="touch-target shrink-0 rounded p-0.5 hover:bg-app-warning/15"
      ...
      {active && partialTranscript && (
        <div
          className="pointer-events-none absolute bottom-full left-1/2 mb-1 max-w-[200px] -translate-x-1/2 overflow-hidden text-ellipsis whitespace-nowrap rounded border border-app-border bg-app-surface px-2 py-1 text-[10px] text-app-muted-foreground shadow-lg"
          style={{
            left: "50%",
            maxWidth: "calc(100vw - 32px)",
            boxSizing: "border-box",
      ...
      (canExportDiagnostic && onExportDiagnostic) ? (
        ...
        >
          {state === "transcribing" && onCancel && (
            <button
              type="button"
              className="rounded border border-app-border px-1.5 py-0.5 text-app-muted-foreground hover:bg-app-surface-muted"
              onClick={onCancel}
            >
              Cancel
            </button>
          ...
          {staleLiveMic && onReleaseMic && (
            <button
              type="button"
              className="rounded border border-app-warning px-1.5 py-0.5 text-app-warning hover:bg-app-warning/10"
          ...
          {isTtsSpeaking && onTtsStop && (
            <button
              type="button"
              className="rounded border border-app-primary px-1.5 py-0.5 text-app-primary hover:bg-app-primary/10"
              onClick={onTtsStop}
            >
              Stop speech
            </button>
