...
/* Every descendant, not just the shell's own content box. A consumer's own
   ...
   A child that genuinely scrolls horizontally opts out with data-rcl-pan-x. */
[data-rcl-sidebar-shell][data-swipe="true"] *:not([data-rcl-pan-x]) { touch-action: pan-y; }
...
/* Matched to the closed rule's specificity on purpose. That rule is
   [shell][mode][open="false"] — three attributes — and it sets
   visibility: hidden. A two-attribute opening rule loses the cascade, so
   the panel tracked the finger correctly and did all of it invisibly:
   the drag appeared to do nothing until release, when data-open flipped
   and the ordinary transition slid the panel in. */
[data-rcl-sidebar-shell][data-mode="overlay"][data-opening="true"],
[data-rcl-sidebar-shell][data-mode="responsive"][data-opening="true"] { visibility: visible; transition: none; will-change: transform; }
