...
describe("rendering arbitrary children", () => {
  it("renders the child icon untouched while idle", () => {
    renderWithProviders(<MorphingIcon><BubbleIcon /></MorphingIcon>);
  ...
  it("is hidden from assistive technology unless it is labelled", () => {
    const { rerender } = renderWithProviders(<MorphingIcon><BubbleIcon /></MorphingIcon>);
    expect(root()).toHaveAttribute("aria-hidden", "true");
    expect(root()).not.toHaveAttribute("role");
    rerender(<MorphingIcon label="Messages"><BubbleIcon /></MorphingIcon>);
    expect(root()).toHaveAttribute("role", "img");
    expect(root()).toHaveAccessibleName("Messages");
  });

  it("sizes from the token scale and from an explicit pixel value", () => {
    const { rerender } = renderWithProviders(<MorphingIcon size="lg"><BubbleIcon /></MorphingIcon>);
    expect(root().style.inlineSize).toBe("var(--icon-size-lg, 1.5rem)");
    rerender(<MorphingIcon size={32}><BubbleIcon /></MorphingIcon>);
    expect(root().style.inlineSize).toBe("32px");
  });

  it("clamps an out-of-range pixel size", () => {
    const { rerender } = renderWithProviders(<MorphingIcon size={4}><BubbleIcon /></MorphingIcon>);
    expect(root().style.inlineSize).toBe("12px");
    rerender(<MorphingIcon size={999}><BubbleIcon /></MorphingIcon>);
...
describe("transitions", () => {
  ...
  function Swapper({ morph }: { morph?: "auto" | "morph" | "crossfade" | "none" }) {
    ...
    return (
      <>
        <button type="button" onClick={() => { setOn((v) => !v); }}>swap</button>
        <MorphingIcon morph={morph}>{on ? <TerminalIcon /> : <BubbleIcon />}</MorphingIcon>
      </>
    );
  }

  const swap = () => act(() => { fireEvent.click(screen.getByText("swap")); });
  ...
  it("paints interpolated geometry that changes across frames", () => {
    ...
    swap();
    const at = () =>
      root().querySelector("[data-rcl-morphing-icon-frame] path")!.getAttribute("d");
    const first = at();
    advance(80);
    const second = at();
    advance(80);
  ...
  it.each([
    ...
    renderWithProviders(<MorphingIcon strategy={strategy}><BubbleIcon /></MorphingIcon>);
    expect(root()).toHaveAttribute("data-rcl-transition-mode", mode);
  });

  it("lets an explicit morph prop win over a legacy strategy", () => {
    renderWithProviders(
      <MorphingIcon strategy="transform" morph="morph"><BubbleIcon /></MorphingIcon>,
    );
