...
export type UndoableDestructiveActionState =
  | "idle"
  | "submitting"
  | "success"
  | "error";
...
function ActionContent({
  ...
  const [state, setState] = useState<UndoableDestructiveActionState>(
    defaultState ?? "idle",
  );
  ...
  const resolvedDescription =
    description ??
    `Remove ${itemLabel} now. You can undo this change for a few seconds.`;
    ...
    <>
      ...
      >
        <div data-rcl-undoable-action-header>
          ...
          <span data-rcl-undoable-action-description>
            {resolvedDescription}
          </span>
        </div>
        {children}
        ...
        {state === "submitting" && (
          <div
            data-rcl-undoable-action-status
            role="status"
            aria-live="polite"
            aria-busy="true"
          >
...
export const UndoableDestructiveAction = withClassName(
  function UndoableDestructiveAction(props: UndoableDestructiveActionProps) {
    return (
      <UndoManagerProvider
        maxVisible={1}
        defaultExpiresMs={props.expiresMs ?? 8000}
      >
        <ActionContent {...props} />
      </UndoManagerProvider>
    );
