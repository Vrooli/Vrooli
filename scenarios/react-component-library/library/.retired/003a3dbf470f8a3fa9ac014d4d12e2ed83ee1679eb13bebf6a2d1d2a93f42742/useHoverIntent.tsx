...
export interface HoverRect { left: number; top: number; right: number; bottom: number }
...
export function useHoverIntent(options: UseHoverIntentOptions) {
  ...
  const canHover = () => typeof window === "undefined" || window.matchMedia?.("(hover: hover) and (pointer: fine)").matches === true;
  const open = useCallback(() => {
    if (!canHover()) return;
    clear();
    openTimer.current = setTimeout(() => { opened.current = true; latest.current.onOpen(); }, latest.current.openDelay ?? resolveGestureFeel().hoverOpenDelay);
  }, [clear]);
  const close = useCallback(() => {
    clear();
    closeTimer.current = setTimeout(() => { if (opened.current) { opened.current = false; latest.current.onClose(); } }, latest.current.closeDelay ?? resolveGestureFeel().hoverCloseDelay);
  }, [clear]);
  const onPointerEnter = useCallback((event: { clientX: number; clientY: number; pointerType?: string }) => {
  ...
  const onPointerMove = useCallback((event: { clientX: number; clientY: number }) => {
    ...
    const towardChild = insideTriangle(point, last, { x: rect.left, y: rect.top }, { x: rect.left, y: rect.bottom });
    if (!towardChild) close();
    else {
      clear();
      closeTimer.current = setTimeout(close, latest.current.fuse ?? resolveGestureFeel().safePolygonFuse);
    }
  }, [clear, close]);
