...
function normalizedResult(
  result: AsyncOptionsResult | AsyncOption[],
): AsyncOptionsResult {
...
export const AsyncOptionsField = withClassName(function AsyncOptionsField({
    ...
    (page: number, replace: boolean) => {
      ...
      void runRetry(() => runLatest().then(normalizedResult))
        .then((result) => {
          ...
          setOptions((previous) =>
            replace ? result.options : [...previous, ...result.options],
          );
          ...
          setHighlighted((current) =>
            current >= 0
              ? current
              : result.options.findIndex((option) => !option.disabled),
          );
        })
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "ArrowDown") {
      ...
      setOpen(true);
      setHighlighted((current) =>
        Math.min(availableOptions.length - 1, current + 1),
      );
  ...
  return (
    ...
    >
      ...
      <div data-rcl-async-options-status role="status" aria-live="polite">
        {status === "loading" ? (
          <>
            <span data-rcl-async-options-spinner aria-hidden="true" />{" "}
            {loadingText}
          </>
        ) : status === "offline" ? (
          offlineText
                  ...
                  <div key={option.value}>
                    ...
                    >
                      <span data-rcl-async-options-option-copy>
                        <span data-rcl-async-options-option-label>
                          {option.label}
                        </span>
                        {option.description && (
