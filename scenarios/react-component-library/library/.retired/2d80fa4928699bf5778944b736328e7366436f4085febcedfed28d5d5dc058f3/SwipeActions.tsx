...
export function SwipeActions({
  ...
  /**
   * Whether this row owns the drag currently in flight.
   *
   ...
   * from contending while the row is shut. Once the actions are showing, that
   * symmetry breaks: putting them away travels the same way as dismissing the
   * drawer, so the ancestor would close out from under the gesture.
   *
   * Ownership is decided once, at pointerdown, from whether the actions were
   * already open. A drag that begins on an open row is the user putting those
   * actions away, so the row keeps the whole gesture; a drag that begins on a
   * shut row is not about this row at all and is left to propagate. Deciding at
   * the start rather than continuously means a drag never changes hands
   * halfway, which is what would make the drawer lurch mid-swipe.
   */
  ...
  const claimIfOwned = (event: ReactPointerEvent) => {
    if (ownsGesture.current) event.stopPropagation();
  };

  const endOwnedGesture = (event: ReactPointerEvent) => {
    claimIfOwned(event);
  ...
  return (
    <>
      <StyleSheet name="rcl-swipe-actions-1-0-5" css={swipeActionsStyles} />
      <div
        ref={rootRef}
        data-testid={testId}
      ...
      >
        ...
        <div
          ...
          onPointerDown={
            enabled
              ? (event) => {
                  ownsGesture.current = openRef.current;
                  claimIfOwned(event);
                  onPointerDown(event);
                }
              : undefined
          }
          onPointerMove={enabled ? claimIfOwned : undefined}
          onPointerUp={enabled ? endOwnedGesture : undefined}
          onPointerCancel={enabled ? endOwnedGesture : undefined}
