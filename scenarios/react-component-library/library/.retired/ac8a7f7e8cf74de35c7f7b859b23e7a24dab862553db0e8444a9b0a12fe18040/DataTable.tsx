...
function searchableText(value: ReactNode): string {
  ...
  if (isValidElement(value)) {
    return searchableText(
      (value.props as { children?: ReactNode } | undefined)?.children,
    );
  }
  return "";
}
...
function asyncStatus(status: DataTableStatus): AsyncBoundaryStatus {
  ...
  if (status === "refreshing" || status === "stale" || status === "offline")
    return status;
  return "idle";
}
        ...
        <div data-rcl-data-table-query-row>
          ...
          {filters.length > 0 ? (
            ...
            >
              ...
              >
                {libraryStrings("data-display.data-table.all", "All")}
              </button>
              {filters.map((filter) => (
                <button
                  data-testid="data-display.data-table"
      ...
      <div data-rcl-data-table-summary aria-live="polite">
        ...
        {!hideDensityControl ? (
          ...
          >
            ...
            >
              {libraryStrings("data-display.data-table.roomy", "Roomy")}
            </button>
            <button
              data-testid="data-display.data-table"
              type="button"
            ...
            >
              {libraryStrings("data-display.data-table.dense", "Dense")}
            </button>
          </div>
        ) : null}
      </div>
                        ...
                        >
                          {column.sortValue ? (
                            ...
                            >
                              ...
                              <span
                                data-rcl-data-table-sort-indicator
                                aria-hidden="true"
                              >
                  ...
                  <tbody>
                    {pageRows.map((row, index) => {
                      ...
                      return (
                        <tr
                          key={key}
                          data-rcl-data-table-row-selected={
                            rowSelected || undefined
                          }
                          aria-selected={
                            enableSelection ? rowSelected : undefined
                          }
                        >
                          {enableSelection ? (
                            <td>
            ...
            <div data-rcl-data-table-page>
              ...
              <div data-rcl-data-table-page-controls>
                ...
                <button
                  ...
                  onClick={() => changePage(currentPage + 1)}
                >
                  {libraryStrings("data-display.data-table.next", "Next")}
                </button>
              </div>
            </div>
          ) : null}
  ...
  return (
    ...
    >
      ...
      <AsyncBoundary
        ...
        error={errorMessage}
        retry={onRetry}
        preserveContent={["refreshing", "stale", "partial", "offline"].includes(
          status,
        )}
      >
        {tableContent}
