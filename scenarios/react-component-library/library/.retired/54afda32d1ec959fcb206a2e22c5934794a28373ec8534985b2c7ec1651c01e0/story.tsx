...
function Specimen({
  ...
  const { onPointerDown } = useSwipeGesture({
    ...
    onMove: ({ translate }) => {
      if (surface.current)
        surface.current.style.transform = `translateX(${String(translate)}px)`;
    },
    ...
    onRelease: (release) => {
      ...
      if (surface.current) {
        surface.current.style.transform =
          release.outcome === "rest"
            ? `translateX(${String(STAGES[STAGES.length - 1])}px)`
            : "";
      }
    },
  });
...
export function AutoCommit() {
  return (
    <Specimen testId="hooks.use-swipe-gesture.commit" releaseMode="commit" />
  );
}
