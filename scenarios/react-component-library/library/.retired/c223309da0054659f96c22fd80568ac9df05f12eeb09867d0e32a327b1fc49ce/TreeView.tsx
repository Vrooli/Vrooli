    ...
    return (
      <div key={node.id}>
        <div
          ...
          tabIndex={
            selectedId === node.id ||
            (!selectedId && visibleNodes[0]?.node.id === node.id)
              ? 0
              : -1
          }
        ...
        >
          ...
          <span aria-hidden className="rcl-tree-icon">
            {node.icon ??
              (hasChildren ? <FolderOpen size={17} /> : <File size={17} />)}
          </span>
    ...
    <div data-rcl-tree role="tree" aria-label={label}>
      ...
      {resolvedNodes.length ? (
        ...
        <div className="rcl-tree-empty" role="status">
          {strings(
            "data-display.tree-view.nothing-to-display",
            "Nothing to display.",
          )}
        </div>
      )}
