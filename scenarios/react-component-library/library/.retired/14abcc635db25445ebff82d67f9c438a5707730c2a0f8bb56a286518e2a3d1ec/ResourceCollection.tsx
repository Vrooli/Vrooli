...
export const ResourceCollection = withClassName(function ResourceCollection<
  Row,
>({
  title,
  description,
  ...
  const [densityState, setDensityState] =
    useState<DataToolbarDensity>(defaultDensity);
    ...
    >
      ...
      <header data-rcl-resource-collection-header>
        <div data-rcl-resource-collection-heading>
          <span data-rcl-resource-collection-eyebrow>
            {libraryStrings(
              "patterns.resource-collection.resource-library",
              "Resource library",
            )}
          </span>
        ...
        {headerAside ? (
          <div data-rcl-resource-collection-aside>{headerAside}</div>
        ) : null}
      </header>
      {status !== "idle" && status !== "success" ? (
        ...
        >
          ...
          <span>
            {statusMessage ?? "Your collection context is preserved."}
          </span>
        </div>
      ) : null}
      <DataToolbar
