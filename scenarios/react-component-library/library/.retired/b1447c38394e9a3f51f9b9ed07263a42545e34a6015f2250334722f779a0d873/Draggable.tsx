...
function clamp(position: DragPosition, bounds: DragBounds): DragPosition {
  return {
    x: Math.max(
      ...
      Math.min(bounds.right ?? Infinity, position.x),
    ),
    y: Math.max(
      bounds.top ?? -Infinity,
      Math.min(bounds.bottom ?? Infinity, position.y),
    ),
  };
}
...
export const Draggable = withClassName(function Draggable({
  ...
  label =
    label ??
    libraryStrings("manipulation.draggable.draggable-item", "Draggable item");
