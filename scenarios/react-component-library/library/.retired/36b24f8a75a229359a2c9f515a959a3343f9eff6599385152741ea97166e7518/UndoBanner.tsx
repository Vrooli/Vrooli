 ...
 * @description A forgiving reversible-action surface with expiry, rollback progress, and recovery when undo fails.
 * @version 1.0.6
...
import {
  useUndoManager,
  type UndoRecord,
} from "@vrooli/react-component-library/UndoManager/1";
  ...
  const progress = Math.max(
    ...
    Math.min(
      ...
      (record.expiresAt - now) /
        Math.max(record.expiresAt - record.createdAt, 1),
    ),
  );
  const title =
    ...
    <Presence present duration="quick">
      ...
      >
        ...
        <button
          ...
          type="button"
          aria-label={strings(
            "feedback.undo-banner.dismiss-undo-message",
...
export const UndoBanner = withClassName(function UndoBanner({
  className,
  style,
}: UndoBannerProps) {
  const strings = useStrings();
  const manager = useUndoManager();
  const visible = manager.records.filter(
  ...
  return (
    <>
      ...
      <div
        ...
        style={style}
        aria-label={strings(
          "feedback.undo-banner.undo-messages",
          "Undo messages",
        )}
      >
        {visible.map((record) => (
          <UndoItem
            ...
            record={record}
            onUndo={() => {
              void (record.status === "error"
                ? manager.retry(record.id)
