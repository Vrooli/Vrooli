...
export function Bare() {
  ...
  return (
    <Showcase>
      ...
      >
        <span
          id="host-owned-switch-label"
          style={{ font: "var(--text-body)", fontWeight: 650 }}
        >
          {label}
        </span>
        {/* The row owns the label, so the switch adopts it by id rather than
            repeating the string — and must not discard it. */}
        <Switch aria-labelledby="host-owned-switch-label" defaultChecked />
      </div>
    </Showcase>
  );
}
