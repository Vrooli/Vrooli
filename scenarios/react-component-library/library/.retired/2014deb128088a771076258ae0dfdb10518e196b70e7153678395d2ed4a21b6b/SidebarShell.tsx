...
import { useSwipe } from "@vrooli/react-component-library/useSwipe/2";
import {
  useGestureDirection,
  type AnchorEdge,
  ...
  function SidebarShell(
    ...
    const writeDismissed = useCallback(() => {
      ...
      if (panel && dragExtent.current > 0) {
        const signed = dismissLeftward.current
          ? -dragExtent.current
          : dragExtent.current;
    ...
    const dragProps = swipeEnabled
      ? {
          onPointerDown: (event: ReactPointerEvent<HTMLDivElement>) => {
            ...
            dragDeclined.current = Boolean(
              target?.closest?.("[data-rcl-gesture-claim]"),
            );
