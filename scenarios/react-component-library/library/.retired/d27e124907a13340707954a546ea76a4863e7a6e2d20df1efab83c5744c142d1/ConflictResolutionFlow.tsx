 ...
 * @description A field-level concurrency recovery surface that compares local and remote values, preserves intent, and retries resolution honestly.
 * @version 1.0.4
...
export const ConflictResolutionFlow = withClassName(
  function ConflictResolutionFlow<T>({
    fields,
    status,
    onResolve,
    onRetry,
    ...
    const busy =
      resolvedStatus === "loading" || resolvedStatus === "submitting";
    const statusText =
    ...
    return (
      <div data-rcl-conflict-flow className={className} style={style}>
        <StyleSheet name="conflictresolutionflow-1-0-4-1" css={styles} />
        <Form
          ...
          footer={
            <div data-rcl-conflict-flow-actions>
              ...
              >
                {resolveStrings(
                  "patterns.conflict-resolution-flow.keep-editing",
                  "Keep editing",
                )}
              </button>
