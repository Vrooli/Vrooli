...
function isBetween(date: Date, start?: Date, end?: Date) {
  return Boolean(
    start && end && date >= startOfDay(start) && date <= startOfDay(end),
  );
}
...
export const Calendar = withClassName(function Calendar({
  ...
  label = label ?? libraryStrings("forms.calendar.calendar", "Calendar");
  ...
  const selectDay = (day: Date) => {
    ...
    if (mode === "week") {
      const start = addDays(
        clean,
        -((clean.getDay() - firstDayOfWeek + 7) % 7),
      );
      next = { start, end: addDays(start, 6) };
    }
    if (mode === "month")
      next = { start: monthStart(clean), end: monthEnd(clean) };
    ...
    >
      ...
      <header data-rcl-calendar-header>
        ...
        <nav
          ...
          aria-label={strings(
            "forms.calendar.calendar-navigation",
            "Calendar navigation",
          )}
        >
          <button
      ...
      <div
        data-rcl-calendar-grid
        role="grid"
        aria-label={formatter.format(currentMonth)}
      >
        {days.map((day, index) => {
          ...
          const edge = Boolean(
            (range?.start && sameDay(range.start, day)) ||
      ...
      <footer data-rcl-calendar-footer>
        ...
        <span aria-live="polite">
          {selectedDate ? fullFormatter.format(selectedDate) : ""}
        </span>
      </footer>
    </section>
  );
