...
export function useResizablePanel(
  ...
  const [size, setSize] = useState(() =>
    readPersisted(store, storageKey, defaultSize, min, max),
  );
  ...
  const [scopedDirection, setScopedDirection] = useState<"ltr" | "rtl" | null>(
    null,
  );
  ...
  const growsTowardEnd =
    axis === "inline"
      ? (edge === "end") !== (direction === "rtl")
      : edge === "end";

  const effectiveMax = useMemo(() => {
    const available =
      axis === "inline" ? containerRect?.width : containerRect?.height;
    ...
    const ratioCap =
      maxRatio === undefined ? max : Math.floor(available * maxRatio);
  ...
  const drag = useDrag({
    ...
    onEnd: (event) => {
      ...
      const snapped = snapPoints?.length
        ? snapTo(raw, snapPoints, snapTolerance)
        : null;
      const settled =
        snapped === null ? raw : clamp(snapped, min, effectiveMax);
  ...
  useLayoutEffect(() => {
    ...
    setScopedDirection(
      scope === "rtl" ? "rtl" : scope === "ltr" ? "ltr" : null,
    );
  }, [panelRef]);
  ...
  const panelProps: ResizePanelProps = {
    id: panelId,
    style: {
      [axis === "inline" ? "inlineSize" : "blockSize"]:
        `var(${RESIZE_SIZE_PROPERTY}, ${size}px)`,
    },
    "data-rcl-resizable-panel": "",
    "data-axis": axis,
    "data-collapsed": isCollapsed ? "true" : "false",
