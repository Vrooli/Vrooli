...
export function PlanBadge({ plan, size = "md" }: { plan: PlanTier; size?: "sm" | "md" }) {
  ...
  return <span className={`${config.color} ${config.bgColor} ${config.borderColor} inline-flex items-center rounded border px-space-2xs py-space-4xs text-caption font-medium ${size === "sm" ? "text-caption" : ""}`}>{config.label}</span>;
}
