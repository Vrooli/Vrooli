...
import { useDirection } from "@vrooli/react-component-library/useDirection/2.1.1";
 ...
 */
export interface GestureDirectionOptions {
  /**
   * Pins this surface to one edge regardless of the app-wide preference. A
   * component prop is a deliberate local decision and outranks the default;
   * omitting it is what lets a surface follow the user's setting.
   */
  anchor?: AnchorEdge;
  /**
   * The surface being mirrored. Writing direction is inherited, so a component
   * that can sit inside a mirrored region must resolve against its own position
   * in the tree rather than against the document.
   */
...
export function useGestureDirection(
  ...
  options: GestureDirectionOptions = {},
): ResolvedGestureDirection {
  const { anchor, elementRef } = options;
  const writingDirection = useDirection(elementRef);
  const preferred = useHandedness();
  const resolved = anchor ?? preferred;
  return useMemo(
    () => ({
      anchorEdge: resolveAnchorEdge(writingDirection, resolved),
      direction: resolveGestureDirection(writingDirection, resolved, intent),
      sign: gestureSign(writingDirection, resolved, intent),
    }),
    [writingDirection, resolved, intent],
  );
}
