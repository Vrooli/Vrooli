/**
 * @libraryId react-component-library:ShortcutRegistry
 * @displayName Shortcut Registry
 * @description The collision-aware registry resolving active keyboard shortcuts by scope, priority, platform, modal layer, and currently focused context.
 * @version 1.0.1
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:ShortcutRegistry
 * @vrooliComponentSourceSlot services.shortcut-registry */
...
const normalize = (keys: string) =>
  keys
    ...
    .split("+")
    .map((part) => part.trim())
    .map((part) => (["cmd", "command", "control", "ctrl", "meta"].includes(part) ? "mod" : part))
    .filter(Boolean)
    .sort()
    .join("+");

export function createShortcutRegistry(initialShortcuts: Shortcut[] = []): ShortcutRegistry {
  ...
  return {
    ...
    resolve: (keys, context = {}) => {
      ...
      return [...shortcuts.values()]
        .filter((shortcut) => {
          if (shortcut.disabled || normalize(shortcut.keys) !== requested) return false;
          if (context.scope && shortcut.scope && context.scope !== shortcut.scope) return false;
          return true;
        })
