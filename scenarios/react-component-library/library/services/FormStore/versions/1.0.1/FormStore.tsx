...
function createFields<TValues extends Record<string, unknown>>(values: TValues) {
...
function valuesFromState<TValues extends Record<string, unknown>>(
  ...
  return Object.fromEntries(
    (Object.entries(state.fields) as Array<[string, FormFieldState<unknown>]>).map(
...
export function createFormStore<TValues extends Record<string, unknown>>(
  ...
  const patch = (updater: (state: FormState<TValues>) => FormState<TValues>) => store.set(updater);
  const getValues = () => valuesFromState(store.get());
  const getField = <K extends keyof TValues>(field: K) => store.get().fields[field];
  ...
  const setValue = <K extends keyof TValues>(field: K, value: TValues[K]) => {
    patch((state) => ({
      ...
      error: undefined,
      conflict: undefined,
      phase: state.phase === "success" || state.phase === "error" ? "idle" : state.phase,
    }));
  };
  ...
  const validate = async () => {
    ...
    patch((state) => ({
      ...
      fields: Object.fromEntries(
        Object.entries(state.fields).map(([key, field]) => [key, { ...field, validating: true }]),
      ) as FormState<TValues>["fields"],
    }));
    const result = options.validate?.(values);
    const formErrors = ((isPromise(result) ? await result : result) ?? {}) as Partial<
  ...
  return {
    ...
    clearErrors: () =>
      patch((state) => ({
        ...
        error: undefined,
        fields: Object.fromEntries(
          Object.entries(state.fields).map(([key, field]) => [key, { ...field, error: undefined }]),
        ) as FormState<TValues>["fields"],
      })),
...
export function useFormStore<TValues extends Record<string, unknown>, K extends keyof TValues>(
  store: FormStore<TValues>,
  field: K,
) {
  const [, rerender] = useState(0);
  const stableStore = useRef(store).current;
  useEffect(() => stableStore.subscribe(() => rerender((count) => count + 1)), [stableStore]);
