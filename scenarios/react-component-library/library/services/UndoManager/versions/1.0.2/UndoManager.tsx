/**
 * @libraryId react-component-library:UndoManager
 * @displayName Undo Manager
 * @description The scoped command-history service supporting reversible operations, grouping, expiry, redo, persistence policy, and user-visible undo affordances.
 * @version 1.0.2
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:UndoManager
 * @vrooliComponentSourceSlot services.undo-manager */
import {
  createContext,
  useCallback,
  useContext,
...
export type UndoStatus = "available" | "submitting" | "success" | "error" | "expired";
...
export function UndoManagerProvider({
  ...
  const performUndo = useCallback(
    async (id: string) => {
      ...
      if (!record || (record.status !== "available" && record.status !== "error")) return;
      setRecords((current) =>
      ...
      try {
        ...
        setRecords((current) =>
          current.map((candidate) =>
            candidate.id === id ? { ...candidate, status: "success" } : candidate,
          ),
        );
        announce(record.successMessage ?? `${record.title} restored.`);
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "The change could not be restored.";
        setRecords((current) =>
          current.map((candidate) =>
  ...
  const push = useCallback(
    (input: UndoInput) => {
      ...
      setRecords((current) => [...current, record].slice(-Math.max(1, maxVisible)));
  ...
  return <UndoManagerContext.Provider value={handle}>{children}</UndoManagerContext.Provider>;
}

export function useUndoManager(): UndoManagerHandle {
  const manager = useContext(UndoManagerContext);
  if (!manager) throw new Error("useUndoManager must be used within UndoManagerProvider");
  return manager;
}
