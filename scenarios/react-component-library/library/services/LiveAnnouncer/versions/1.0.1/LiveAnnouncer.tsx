/**
 * @libraryId react-component-library:LiveAnnouncer
 * @displayName Live Announcer
 * @description The single announcement service used by forms, drag operations, navigation, loading states, and async results, queueing and timing messages through one live region.
 * @version 1.0.1
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:LiveAnnouncer
 * @vrooliComponentSourceSlot services.live-announcer */
import {
  createContext,
  useCallback,
  useContext,
...
function getFallbackHandle(): LiveAnnouncerHandle {
  ...
  const ensureNode = () => {
    ...
    node ??= document.querySelector<HTMLElement>("[data-vrooli-live-announcer]") ?? undefined;
    if (!node) {
...
export function LiveAnnouncer({ children }: { children?: ReactNode }) {
  ...
  const handle = useMemo<LiveAnnouncerHandle>(() => ({ announce, clear }), [announce, clear]);
