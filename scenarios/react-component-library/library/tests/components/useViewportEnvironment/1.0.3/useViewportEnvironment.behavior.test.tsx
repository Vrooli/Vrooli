...
describe("useViewportEnvironment", () => {
  ...
  it("enters keyboard state only after editable focus and settled occlusion", () => {
    ...
    expect(result.current).toMatchObject({ keyboardVisible: true, keyboardInset: 324 });
    act(() => {
      input.blur();
      document.dispatchEvent(new FocusEvent("focusout", { bubbles: true }));
      flushFrames();
    });
    expect(result.current).toMatchObject({ keyboardVisible: false, keyboardInset: 0 });
  });
  ...
  it("uses an authoritative host override", () => {
    ...
    const wrapper = ({ children }: { children: React.ReactNode }) => (
      <ViewportEnvironmentProvider value={override}>{children}</ViewportEnvironmentProvider>
    );
