...
} from "../../../../hooks/useSwipeGesture/versions/1.1.3/useSwipeGesture.ts";
...
function Harness({ onFrames, ...overrides }: HarnessProps) {
  ...
  return (
    <div
      ...
      data-started-open={release ? String(release.startedOpen) : ""}
    />
  );
}
...
describe("closing from a rested position", () => {
  /** Opens the harness, then drags back by `pullBack` px and releases. */
  const openThenPullBack = (pullBack: number, step = 200) => {
    drag([
      { x: 0, t: 0 },
      { x: 300, t: 400 },
    ]);
    const surface = screen.getByTestId("surface");
    fireEvent.pointerDown(surface, pointer({ x: 0, t: 1000 }));
    fireEvent.pointerMove(window, pointer({ x: -pullBack, t: 1000 + step }));
    fireEvent.pointerUp(window, pointer({ x: -pullBack, t: 1000 + step * 2 }));
    return surface;
  };

  it("rests open before the drag has come back a threshold's worth", () => {
    renderWithProviders(<Harness />);
    expect(openThenPullBack(20).getAttribute("data-outcome")).toBe("rest");
  });

  // The bug this replaces: the decision read `stage`, which a fully revealed
  // surface is already past, so closing demanded almost the whole travel back
  // and a half drag sprang open again.
  it("closes once the drag has come back as far as opening asked for", () => {
    renderWithProviders(<Harness />);
    expect(openThenPullBack(STAGES[0] + 5).getAttribute("data-outcome")).toBe("return");
  });

  it("does not demand the whole travel back", () => {
    renderWithProviders(<Harness />);
    const halfway = Math.round(STAGES[STAGES.length - 1] / 2);
    expect(openThenPullBack(halfway).getAttribute("data-outcome")).toBe("return");
  });

  it("closes on an inward flick that barely moved", () => {
    renderWithProviders(<Harness />);
    expect(openThenPullBack(30, 10).getAttribute("data-outcome")).toBe("return");
  });

  it("asks the same distance closing as it did opening", () => {
    renderWithProviders(<Harness />);
    // Opening commits at the first stage; closing now asks for the same travel.
    expect(openThenPullBack(STAGES[0] - 5).getAttribute("data-outcome")).toBe("rest");
    expect(openThenPullBack(STAGES[0] + 5).getAttribute("data-outcome")).toBe("return");
  });

  it("reports where the gesture began so an abort can restore it", () => {
    renderWithProviders(<Harness />);
    drag([
      { x: 0, t: 0 },
      { x: 300, t: 400 },
    ...
    const surface = screen.getByTestId("surface");
    fireEvent.pointerDown(surface, pointer({ x: 0, t: 1000 }));
    fireEvent.pointerMove(window, pointer({ x: -60, t: 1200 }));
    fireEvent.pointerCancel(window, pointer({ x: -60, t: 1400 }));
    expect(surface.getAttribute("data-outcome")).toBe("abort");
    expect(surface.getAttribute("data-started-open")).toBe("true");
  });
});
