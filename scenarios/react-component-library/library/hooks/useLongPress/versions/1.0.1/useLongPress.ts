...
export function useLongPress(options: UseLongPressOptions): {
  ...
  const onPointerDown = useCallback((event: ReactPointerEvent) => {
    ...
    if (current.disabled || active.current || (event.pointerType === "mouse" && event.button !== 0))
      return;
  ...
  const onPointerMove = useCallback(
    (event: ReactPointerEvent) => {
      ...
      if (gesture && Math.hypot(event.clientX - gesture.x, event.clientY - gesture.y) > tolerance)
        cancel("moved");
    },
    [cancel],
  );
  const onPointerUp = useCallback(() => {
    if (active.current) cancel("released");
  }, [cancel]);
  const onPointerCancel = useCallback(() => {
    if (active.current) cancel("aborted");
  }, [cancel]);
