 ...
 * @displayName Use Icon Morph
 * @description Coordinates icon transition timing and technique without coupling the caller to a renderer.
 * @version 1.1.3
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:useIconMorph
 * @vrooliComponentSourceSlot hooks.use-icon-morph */
...
import {
  ...
  morphCompatibility,
  type IconGeometry,
} from "@vrooli/react-component-library/IconGeometry/1";
...
/**
 * Measurement and transition start must run before the browser paints.
 *
 * React commits new children to the DOM and lets the browser paint *before*
 * passive effects run. Starting the transition in `useEffect` therefore paints
 * one frame of the incoming icon, unhidden, before the effect hides it and
 * draws frame 0 of the morph — which is the *outgoing* shape. The result is a
 * visible flicker: the new icon appears for a frame, snaps back to the old one,
 * then morphs forward. A layout effect runs synchronously after mutation and
 * before paint, so the commit that installs the new icon is the same commit
 * that hides it.
 *
 * `useLayoutEffect` warns when it runs on the server, where there is no paint
 * to beat, so fall back to `useEffect` there.
 */
