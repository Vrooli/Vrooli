 ...
 */
/** @vrooliComponentSource react-component-library:useCollection
 * @vrooliComponentSourceSlot hooks.use-collection */
import {
  useCallback,
  useEffect,
  useMemo,
...
export interface UseCollectionOptions<T> {
  ...
  /** Reveal an unmounted or clipped row before keyboard focus is transferred. */
  onRevealRow?: (key: string) => void;
  onOpen?: (row: T) => void;
...
export function useCollection<T>(
  ...
  const focusRow = (key: string) => {
    ...
    if (element) {
      ...
      element.focus({ preventScroll: !!options.onRevealRow });
      if (!options.onRevealRow) element.scrollIntoView?.({ block: "nearest" });
    }
  };
  ...
  const keyboard = (event: KeyboardEvent) => {
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      if (found) focusRow(getKey(found));
    }
  };
