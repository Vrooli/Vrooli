 ...
 * @displayName useCollection
 * @description Headless collection controller for stable rows, keyboard cursor, keyed selection, and one resolved action declaration.
 * @version 1.0.2
 * @tags []
...
export function useCollection<T>(
  ...
  const rows = useMemo(() => {
    const result = items.filter(
      (row) => (!query || !search || search(row, query)) && (!filter || filter(row)),
    );
  ...
  useEffect(() => {
    if (selected && onChange) {
      ...
      if (current.size !== next.size || [...current].some((key) => !next.has(key)))
  ...
  const reasonFor = useCallback((row: T) => selectable?.(row) || undefined, [selectable]);
    ...
    (row: T): CollectionRowState => {
      ...
      return {
        ...
        selection: {
          ...
          disabledReason,
          onToggleSelect: disabledReason ? undefined : () => mutate(() => store.toggle(key)),
        },
      };
    },
  ...
  const keyboard = (event: KeyboardEvent) => {
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      if (
        ...
        selectedKey &&
        !reasonFor(row) &&
        (store.getSnapshot().mode !== "none" || enterSelectionFromKeyboard())
      )
      ...
      const row = rows.find((candidate) => getKey(candidate) === cursor);
      const action =
        (row && actionsFor(row).find((entry) => entry.id === options.primaryActionId)) ||
        (row && actionsFor(row)[0]);
      if (row && !reasonFor(row)) {
        event.preventDefault();
        if (action) {
          if (!action.disabledReason) void action.run();
        } else onOpen?.(row);
      }
    } else if (event.key.length === 1 && !event.altKey && !event.ctrlKey && !event.metaKey) {
      const found = rows.find((row) =>
        (getSearchText?.(row) ?? String(row))
          .toLocaleLowerCase()
          .includes(event.key.toLocaleLowerCase()),
      );
  ...
  const getRowProps = (row: T) => {
    ...
    return {
      ...
      "aria-selected": store.getSnapshot().mode === "none" ? undefined : state.selection.selected,
      ...
      onClick: (event: React.MouseEvent<HTMLElement>) => {
        ...
        else if (event.metaKey || event.ctrlKey || store.getSnapshot().mode !== "none")
  ...
  const bulkActions = actions
    ...
    .flatMap((action) => {
      ...
      if (!sample) return [];
      const resolved = actionsFor(sample).find((entry) => entry.id === action.id);
      if (!resolved) return [];
      return {
        ...resolved,
        run: async () => {
