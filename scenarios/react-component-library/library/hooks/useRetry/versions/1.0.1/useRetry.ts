/**
 * @libraryId react-component-library:useRetry
 * @displayName useRetry
 * @description A retry controller with attempt limits, backoff, cancellation, and retryability classification, so failures that cannot succeed on repeat are not repeated.
 * @version 1.0.1
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:useRetry
 * @vrooliComponentSourceSlot hooks.use-retry */
import { useCallback, useEffect, useRef, useState } from "react";

export type RetryStatus = "idle" | "running" | "success" | "error" | "cancelled";
...
const defaultBackoff = (attempt: number) => Math.min(1000, 150 * 2 ** Math.max(0, attempt - 1));
...
export function useRetry<T>({
  ...
  const cancel = useCallback(() => {
    ...
    setState((previous) =>
      previous.status === "running" ? { ...previous, status: "cancelled" } : previous,
    );
  }, []);
    ...
    async (task: (context: RetryContext) => Promise<T>) => {
      ...
      for (let attempt = 1; attempt <= attempts; attempt += 1) {
        ...
        try {
          ...
          const duration = typeof backoff === "function" ? backoff(attempt) : backoff;
