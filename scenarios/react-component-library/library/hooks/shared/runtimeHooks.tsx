...
export function useControllableState<T>({
    ...
    (next: T | ((previous: T) => T)) => {
      const resolved =
        typeof next === "function"
          ? (next as (previous: T) => T)(current as T)
          : next;
  ...
  return [current as T, setValue] as const;
}
...
export function useElementRect<T extends HTMLElement>(element: T | null) {
  const [rect, setRect] = useState<DOMRectReadOnly | null>(null);
  useEffect(() => {
    if (!element) return;
...
export function useAnnounce() {
  return useCallback((message: string) => {
    const node =
      document.querySelector("[data-vrooli-live-announcer]") ??
      document.body.appendChild(
        Object.assign(document.createElement("div"), {
          dataset: { vrooliLiveAnnouncer: "true" },
          role: "status",
          textContent: "",
        }),
      );
