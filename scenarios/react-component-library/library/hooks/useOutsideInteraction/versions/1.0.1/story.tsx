...
export function Default({ args }: StoryHarnessProps<{ active: boolean }>) {
  ...
  useOutsideInteraction({
    active: args.active,
    surfaceRef,
    onPointerDownOutside: () => setMessage("Dismissed from outside pointer interaction"),
    onFocusOutside: () => setMessage("Dismissed from outside focus interaction"),
    onEscape: () => setMessage("Dismissed with Escape"),
  });
  return (
    <div
      data-testid="hooks.use-outside-interaction"
      style={{
        display: "grid",
        gap: "var(--space-md)",
        minBlockSize: 220,
