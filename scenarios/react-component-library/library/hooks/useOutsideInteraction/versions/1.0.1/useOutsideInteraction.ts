/**
 * @libraryId react-component-library:useOutsideInteraction
 * @displayName useOutsideInteraction
 * @description A dismissal primitive that understands pointer, focus, nested layers, portals, and interactions that begin inside a surface but end outside it.
 * @version 1.0.1
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:useOutsideInteraction
 * @vrooliComponentSourceSlot hooks.use-outside-interaction */
...
function isInside(
  ...
  return (excludeRefs ?? []).some((ref) => eventPathContains(event, ref.current));
}
...
export function useOutsideInteraction({
  ...
  useEffect(() => {
    ...
    const click = (event: MouseEvent) => {
      ...
      callbacks.current.onPointerDownOutside?.(event as unknown as PointerEvent);
    };
    const focusIn = (event: FocusEvent) => {
      if (!dismissOnFocus || event.defaultPrevented || isInside(event, surfaceRef, excludeRefs))
        return;
  ...
  }, [active, dismissOnEscape, dismissOnFocus, dismissOnPointerDown, excludeRefs, surfaceRef]);
}
