...
function Demo() {
  ...
  const hover = useHover({
    ...
    onChange: (next) => setEvents((previous) => [...previous.slice(-2), next ? "entered" : "left"]),
  });
  return (
    <div data-testid="hooks.use-hover" style={{ display: "grid", gap: "var(--space-sm)" }}>
      <button
        ...
        style={{
          ...
          color: hover.hovered ? "var(--color-primary-foreground)" : "var(--color-primary)",
          font: "var(--text-label)",
          cursor: "pointer",
        }}
      >
      ...
      >
        <strong>{hover.hovered ? "Intent acquired" : "Waiting for hover intent"}</strong>
        <span
          style={{
            display: "block",
      ...
      >
        Pointer: {hover.pointerType ?? "none"} · Events: {events.join(" → ") || "none yet"}
      </span>
    </div>
  );
}
  ...
  return (
    <section style={frame}>
      <div style={{ display: "grid", gap: "var(--space-2xs)" }}>
        ...
        >
          Hover opens after intent; keyboard focus remains an explicit alternative, and moving into
          the related surface does not close it.
        </span>
      </div>
      <Demo />
    </section>
