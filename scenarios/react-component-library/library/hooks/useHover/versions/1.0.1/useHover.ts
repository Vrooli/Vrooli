/**
 * @libraryId react-component-library:useHover
 * @displayName useHover
 * @description A hover-intent primitive accounting for pointer type, entry and exit delay, and movement between related floating surfaces.
 * @version 1.0.1
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:useHover
 * @vrooliComponentSourceSlot hooks.use-hover */
import {
  useCallback,
  useEffect,
  useMemo,
...
export function useHover({
  ...
  const [pointerType, setPointerType] = useState<"mouse" | "pen" | "touch" | null>(null);
  ...
  const isRelated = useCallback(
    (target: EventTarget | null) =>
      target instanceof Node && relatedRefs.some((ref) => ref.current?.contains(target)),
    [relatedRefs],
  );
  const onPointerEnter = useCallback(
    (event: PointerEvent<HTMLElement>) => {
      const nextPointer =
        event.pointerType === "mouse" ? "mouse" : event.pointerType === "touch" ? "touch" : "pen";
      setPointerType(nextPointer);
      if (event.pointerType === "mouse" || event.pointerType === "pen") schedule(true, enterDelay);
    },
    [enterDelay, schedule],
  );
