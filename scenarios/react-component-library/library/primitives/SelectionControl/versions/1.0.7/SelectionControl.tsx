...
/**
 * How far the thumb can travel, measured rather than assumed.
 *
 * An earlier version derived this from a ratio of the track's width, which
 * quietly encoded a `content-box` assumption: under the `border-box` reset most
 * apps ship, the same declaration yields a narrower padding box, and the drag
 * and the resting position disagreed by the width of the borders. Reading the
 * rendered geometry is exact under either box model, at any root font size.
 */
function switchTravel(indicator: HTMLElement): number {
  const thumb = getComputedStyle(indicator, "::before");
  const size = Number.parseFloat(thumb.width);
  const inset = Number.parseFloat(thumb.insetInlineStart);
  if (!Number.isFinite(size) || !Number.isFinite(inset)) return 0;
  // clientWidth is the padding box: the same box the thumb is positioned in.
  return Math.max(0, indicator.clientWidth - size - inset * 2);
}
...
[data-rcl-selection-row][data-kind="switch"] {
  --rcl-switch-w: 2.5rem;
  --rcl-switch-h: 1.5rem;
  /* The thumb fills the track. A circle whose diameter equals the track height
     shares its radius with the track's end cap, so at either end the two curves
     coincide exactly rather than leaving a crescent of track showing. A border
     would reintroduce that gap on top of any inset, so the switch draws its
     unchecked edge with an inset shadow instead. */
  --rcl-switch-inset: 0px;
  --rcl-switch-thumb: calc(var(--rcl-switch-h) - 2 * var(--rcl-switch-inset));
  --rcl-switch-travel: calc(
    var(--rcl-switch-w) - var(--rcl-switch-thumb) - 2 * var(--rcl-switch-inset)
  );
  --rcl-switch-stretch: calc(var(--rcl-switch-thumb) + .25rem);
...
[data-rcl-selection-row][data-kind="switch"] [data-rcl-selection-indicator] {
  /* Declared explicitly: a host's global reset would otherwise decide whether
     these are content or border dimensions, and the thumb's arithmetic depends
     on knowing which. */
  box-sizing: border-box;
  inline-size: var(--rcl-switch-w);
  block-size: var(--rcl-switch-h);
  margin-block-start: calc((var(--tap-target-min) - var(--rcl-switch-h)) / 2);
  /* No border: it would sit between the thumb and the track's edge, which is
     exactly the gap this geometry removes. The edge is drawn inside instead. */
  border: 0;
  border-radius: var(--radius-pill);
  background: var(--color-surface-muted);
  box-shadow: inset 0 0 0 var(--border-hairline) var(--color-border);
...
[data-rcl-selection-row][data-kind="switch"] [data-rcl-selection-indicator]::before {
  content: "";
  position: absolute;
  /* Centred on the axis, not positioned by a computed offset: the offset has
     to agree with the track's inner height, and it is the borders that make
     those two disagree. */
  ...
  transform: translateY(-50%) translateX(var(--rcl-switch-x, 0px));
  transition:
...
[data-rcl-selection-row][data-kind="switch"][data-state="checked"] [data-rcl-selection-indicator]::before {
  ...
  transform: translateY(-50%) translateX(var(--rcl-switch-x, var(--rcl-switch-travel)));
}
...
[data-rcl-selection-row][data-kind="switch"][data-pressed="true"][data-state="checked"]:not([data-dragging="true"]) [data-rcl-selection-indicator]::before {
  transform: translateY(-50%)
    translateX(calc(var(--rcl-switch-travel) - (var(--rcl-switch-stretch) - var(--rcl-switch-thumb))));
}
...
export const SelectionControl = withClassName(function SelectionControl({
  ...
  /**
   * Once a drag starts, the pointer stream is taken from the window rather than
   * from the indicator. A switch is roughly 40px wide, so a drag that carries
   * it from one end to the other almost always finishes past the edge — and an
   * element-scoped listener stops hearing about a pointer that has left it.
   * Pointer capture would usually paper over that, but it is exactly the kind
   * of thing that fails quietly (and does fail, for a pointer id the browser
   * does not consider active). Listening on the window makes off-target
   * completion the default rather than something capture has to rescue.
   */
  ...
  const advance = (pointerId: number, clientX: number) => {
    const drag = dragRef.current;
    if (!drag || drag.pointerId !== pointerId) return;
    if (!drag.moved) {
      if (Math.abs(clientX - drag.startX) < AXIS_SLOP) return;
      drag.moved = true;
    }
    const offset = offsetFor(drag, clientX);
  ...
  const finish = (pointerId: number, clientX: number) => {
    const drag = dragRef.current;
    if (!drag || drag.pointerId !== pointerId) return;
    detachWindow();
    const committed = drag.moved ? offsetFor(drag, clientX) > drag.travel / 2 : null;
    const wasChecked = drag.startChecked;
    clearDrag();
    ...
    // a real event.
    armClickSuppression();
    if (committed !== wasChecked) inputRef.current?.click();
  };
  ...
  const onPointerDown = (event: PointerEvent<HTMLSpanElement>) => {
    ...
    dragRef.current = {
      ...
      travel: switchTravel(el),
      moved: false,
    };
    setPressed(true);

    const move = (e: globalThis.PointerEvent) => {
      advance(e.pointerId, e.clientX);
    };
    const up = (e: globalThis.PointerEvent) => {
      finish(e.pointerId, e.clientX);
    };
