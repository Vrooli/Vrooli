  ...
  for (const reverse of [false, true])
    for (const offset of offsets) {
      ...
      const score = points.reduce((sum, point, index) => {
        ...
        const sx = (sourcePoint.x - sourceCenter.x) / sourceRadius;
        const sy = (sourcePoint.y - sourceCenter.y) / sourceRadius;
        const tx = (point.x - targetCenter.x) / targetRadius;
        const ty = (point.y - targetCenter.y) / targetRadius;
        return sum + (sx - tx) ** 2 + (sy - ty) ** 2;
      }, 0);
      if (score < bestScore) {
        bestScore = score;
        best = points;
...
function matches(from: Subpath[], to: Subpath[]) {
  return from.map((source, sourceIndex) => {
    let best = 0;
    let bestScore = Number.POSITIVE_INFINITY;
    to.forEach((target, targetIndex) => {
      const a = centroid(source.points);
      const b = centroid(target.points);
      const score =
