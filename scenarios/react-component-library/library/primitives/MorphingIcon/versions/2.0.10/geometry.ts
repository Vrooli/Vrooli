...
const EMPTY_SUBPATH: Subpath = { points: [], closed: false, opacity: 0 };
...
function center(points: Point[]): Point {
  if (points.length === 0) return EMPTY_POINT;
...
function candidate(points: Point[], reverse: boolean, offset: number) {
  ...
  return ordered.map((_, index) => ordered[(index + offset) % ordered.length] ?? EMPTY_POINT);
}
...
function alignment(source: Point[], target: Point[]): Alignment {
  ...
  for (let index = 0; index < source.length; index += 1) {
    const left = source[index] ?? source[source.length - 1] ?? EMPTY_POINT;
    const right = target[index] ?? target[target.length - 1] ?? EMPTY_POINT;
    const ax = left.x - a.x;
    const ay = left.y - a.y;
  ...
  const scaleNumerator = Math.cos(angle) * (xx + yy) + Math.sin(angle) * (xy - yx);
  ...
  const residual = Math.sqrt(
    source.reduce((sum, point, index) => {
      const targetPoint = target[index] ?? target[target.length - 1] ?? EMPTY_POINT;
      const x = point.x - a.x;
      const y = point.y - a.y;
...
export function interpolateGeometry(from: IconGeometry, to: IconGeometry, progress: number) {
  ...
  return {
    ...
    subpaths: from.subpaths.map((source, index) => {
      const target = to.subpaths[pairing[index] ?? 0] ?? to.subpaths[0] ?? EMPTY_SUBPATH;
      const targetPoints =
        target.points.length === source.points.length
          ? target.points
          : Array.from(
              { length: source.points.length },
              (_, pointIndex) =>
                target.points[
                  Math.floor((pointIndex * target.points.length) / source.points.length)
                ] ?? EMPTY_POINT,
            );
      ...
      return {
        ...
        opacity: opacityValue(
          source.opacity + ((shouldFade ? 0 : target.opacity) - source.opacity) * Math.max(0, t),
        ),
        points: source.points.map((point, pointIndex) => {
          const targetPoint =
            correspondence.points[pointIndex] ??
            correspondence.points[correspondence.points.length - 1] ??
            EMPTY_POINT;
          const rigid = transform(point, sourceCenter, targetCenter, correspondence.alignment, t);
          const rigidEndpoint = transform(
            point,
            sourceCenter,
            targetCenter,
