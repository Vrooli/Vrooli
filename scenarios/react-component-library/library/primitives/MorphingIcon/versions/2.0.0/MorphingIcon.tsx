...
export function MorphingIcon({
  ...
  const [geometry, setGeometry] = useState<IconGeometry>(() =>
    normalizeIcon(from ?? icon),
  );
  ...
  useEffect(() => {
    ...
    if (reducedMotion || strategy !== "morph" || duration <= 0) {
    ...
    const tick = (now: number) => {
      ...
      const spring = 1 - Math.exp(-10 * raw) * Math.cos(8 * raw);
      const next = interpolateGeometry(source, target, Math.min(1, spring));
