/**
 * @libraryId react-component-library:Progress
 * @displayName Progress
 * @description A token-backed progress family for determinate, buffered, segmented, and indeterminate work.
 * @version 1.0.1
...
export const Progress = withClassName(function Progress({
  ...
  const bufferedPercentage = (clamp(bufferedValue ?? value, safeMax) / safeMax) * 100;
  const displayValue = `${Math.round(percentage)}%`;
  const common = {
    "aria-label": label,
    "aria-valuemin": 0,
    "aria-valuemax": safeMax,
    ...(mode === "indeterminate" ? {} : { "aria-valuenow": clamp(value, safeMax) }),
  };
  const mergedStyle = {
          ...
          <div data-rcl-progress-track>
            ...
            {mode === "segmented" ? (
              <span data-rcl-progress-segments>
                {Array.from({ length: Math.max(2, Math.min(12, segments)) }, (_, index) => (
                  <span
                    data-rcl-progress-segment
                    data-filled={index < Math.round((percentage / 100) * segments)}
                    key={index}
                  />
                ))}
              </span>
              ...
              <span
                ...
                style={{
                  transform: mode === "indeterminate" ? undefined : `scaleX(${percentage / 100})`,
                }}
              />
            )}
          </div>
