...
export function createMotionValue<T extends MotionScalar>(
  initial: T,
): MotionValue<T> {
  let current = initial;
...
export function useMotionValue<T extends MotionScalar>(
  initial: T,
): MotionValue<T> {
...
export const MotionPrimitive = forwardRef<HTMLElement, MotionPrimitiveProps>(
  function MotionPrimitive(
    ...
    const reduced =
      ...
      (reducedMotion === "respect" && reducedPreference);
    ...
    useEffect(() => {
      ...
      const cleanups = Object.entries(motionValues).map(([property, value]) => {
        ...
        return value.subscribe((next) =>
          applyMotionValue(element, property, next),
        );
      });
    ...
    return createElement(
      ...
      <style
        data-rcl-motion-styles
        dangerouslySetInnerHTML={{ __html: motionStyles }}
      />,
      children,
    );
  },
