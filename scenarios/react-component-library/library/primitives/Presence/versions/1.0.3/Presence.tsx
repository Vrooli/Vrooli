 ...
 * @description The mount and unmount lifecycle primitive letting content animate out before removal, handling interruption, nesting, and reduced-motion collapse.
 * @version 1.0.3
 * @tags []
...
export const Presence = withClassName(function Presence({
  ...
  useEffect(() => {
    ...
    if (present) {
      ...
      setPhase("entering");
      // The enter animation is attached by the "entering" phase, so the phase has to
      // outlive the animation. Advancing on the next frame removed the declaration about
      // one frame in and the element snapped to its settled state, which reads as no
      // enter at all. The frame is still used, but only so the first keyframe paints
      // before the clock starts.
      const settle = () => {
        timer = setTimeout(() => setPhase("entered"), resolvedDuration.ms);
      };
