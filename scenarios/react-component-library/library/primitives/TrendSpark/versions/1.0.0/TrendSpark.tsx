 ...
 * @description Accessible inline trend visualization for compact history rows.
 * @version 1.0.0
...
export interface TrendSparkProps extends Omit<SVGAttributes<SVGSVGElement>, "children" | "role" | "values"> {
  values: number[];
  label: string;
...
function pointsFor(values: number[]): string {
  const points = values.slice(-12).length >= 2 ? values.slice(-12) : [values[0] ?? 0, values[0] ?? 0];
  const min = Math.min(...points);
  const max = Math.max(...points);
  const range = max - min || 1;
  return points
    .map((value, index) => `${(index / (points.length - 1)) * 100},${28 - ((value - min) / range) * 24}`)
    .join(" ");
}

export function TrendSpark({ values, label, tone = "neutral", className, ...props }: TrendSparkProps) {
