...
import {
  jsdomEnv,
  runStory,
} from "../api/handlers/preview/assets/story-evaluator.js";
...
const contracts = import.meta.glob("./**/story.json", {
  eager: true,
  import: "default",
}) as Record<string, StoryContract>;
const modules = import.meta.glob("./**/versions/**/*.{ts,tsx}") as Record<
  string,
  () => Promise<StoryModule>
>;
...
const resolveStoryValue = (
  value: unknown,
  log: (...args: unknown[]) => void,
): unknown => {
  if (Array.isArray(value))
    return value.map((item) => resolveStoryValue(item, log));
  if (!value || typeof value !== "object") return value;
  const structured = value as Record<string, unknown>;
  if (Object.prototype.hasOwnProperty.call(structured, "$text"))
  ...
  if (Object.prototype.hasOwnProperty.call(structured, "$rowKey")) {
    ...
    return (row: Record<string, unknown> | null | undefined, index: number) =>
      String(row?.[field] ?? index);
  }
  if (Object.prototype.hasOwnProperty.call(structured, "$icon")) {
    const icon =
      (Icons as Record<string, unknown>)[String(structured.$icon)] ??
      Icons.Circle;
    return React.createElement(icon as React.ElementType, {
      "aria-hidden": true,
      className:
        typeof structured.className === "string"
  ...
  if (Object.prototype.hasOwnProperty.call(structured, "$node")) {
    ...
    const props = resolveStoryValue(structured.props || {}, log) as Record<
      string,
      unknown
    >;
  ...
  if (Object.prototype.hasOwnProperty.call(structured, "$columns")) {
    return (Array.isArray(structured.$columns) ? structured.$columns : []).map(
      (column) => {
          ...
          accessor: (row: Record<string, unknown>) =>
            definition.badge
              ? React.createElement(
                  ...
                  {
                    className:
                      "inline-flex rounded-pill border border-app-border px-2 py-1 text-xs",
                  },
                  row[field],
                )
              : row[field],
          sortValue: definition.sortable
            ? (row: Record<string, unknown>) => row[field]
            : undefined,
          searchValue: definition.searchable
            ? (row: Record<string, unknown>) => String(row[field] ?? "")
            : undefined,
        };
      },
    );
  ...
  if (Object.prototype.hasOwnProperty.call(structured, "$filters")) {
    return (Array.isArray(structured.$filters) ? structured.$filters : []).map(
      (filter) => {
        ...
        return {
          ...
          predicate: (row: Record<string, unknown>) =>
            row[field] === definition.equals,
        };
      },
    );
  }
  return Object.fromEntries(
    Object.entries(structured).map(([key, child]) => [
      key,
      resolveStoryValue(child, log),
    ]),
  );
};
...
const sourceForContract = (contractPath: string) => {
  ...
  const candidates = Object.entries(modules).filter(([path]) =>
    path.startsWith(`${versionDirectory}/`),
  );
  const preferred = componentNameForContract(contractPath)
    ...
    .join("");
  return (
    candidates.find(([path]) => path.endsWith(`/${preferred}.tsx`))?.[1] ??
...
const storyModuleForContract = (contractPath: string) => {
  ...
  return Object.entries(modules).find(
    ([path]) => path === `${versionDirectory}/story.tsx`,
  )?.[1];
};
...
const componentFromModule = (module: StoryModule, contractPath: string) => {
  ...
  if (isRenderableComponent(module[preferredName]))
...
const harnessForStory = (story: Record<string, unknown>) => {
  const name = (
    story.composition as { harness?: { export?: string } } | undefined
  )?.harness?.export;
...
const hookFixture = (
  ...
  return function HookFixture() {
    const fixture =
      (environment as Record<string, unknown>)?.voiceInput || "idle";
    const media = {
      acquire: () =>
    ...
    return React.createElement(
      ...
      React.createElement(
        "output",
        null,
        voice.state || voice.voiceState || "idle",
      ),
    );
  };
};
  ...
  for (const [contractPath, contract] of Object.entries(contracts)) {
    for (const story of contract.stories || []) {
      ...
      it(`${contract.title || contractPath} > ${storyName}`, async () => {
        ...
        expect(
          loader,
          `missing component source for ${contractPath}`,
        ).toBeDefined();
        const component = componentFromModule(await loader!(), contractPath);
        const log = () => undefined;
        const args = resolveStoryValue(
          story.args && typeof story.args === "object" ? story.args : {},
          log,
          ...
          args.items ??= [
            {
              id: "home",
              label: "Home",
              icon: React.createElement(Icons.Circle),
              href: "#home",
            },
          ];
        ...
        if (componentName === "ConflictResolutionFlow") {
          args.fields ??= [
            { id: "sample", label: "Sample", local: "local", remote: "remote" },
          ];
        }
        if (componentName === "Select") args.options ??= [];
        if (componentName === "markdown-renderer")
          args.content ??= "# Markdown renderer";
        ...
        if (
          componentName === "ComputedField" ||
          componentName === "ConditionalField" ||
        ...
        if (componentName === "ConditionalField") {
          args.when ??= () => true;
          args.children ??= React.createElement(
            "span",
            null,
            "Conditional content",
          );
        ...
        if (componentName === "GlobalCommandSystem") {
          args.commands ??= [
            { id: "help", label: "Help", run: () => undefined },
          ];
        }
          ...
          args.columns ??= [
            {
              id: "name",
              header: "Name",
              accessor: (row: Record<string, unknown>) => row.name,
            },
          ];
          args.getRowKey ??= (row: Record<string, unknown>, index: number) =>
            String(row.id ?? index);
        }
        ...
        const environment =
          story.environment && typeof story.environment === "object"
            ? story.environment
            : {};
        ...
        const specimenExport = (
          story.composition as { specimen?: { export?: string } } | undefined
        )?.specimen?.export;
        const storyLoader = storyModuleForContract(contractPath);
        const storyModule = storyLoader ? await storyLoader() : {};
        const specimen = specimenExport
          ? storyModule[specimenExport]
          : undefined;
        const harness = harnessForStory(story);
        expect(
          specimen || component,
          `missing renderable export for ${contractPath}`,
        ).toBeDefined();
        await setLocale("en");

        const directArgs = { ...args };
        if (componentName === "Icon")
          directArgs.label ??= String(directArgs.name || "Icon");
        if (componentName === "Dialog") {
          directArgs.open ??= true;
          directArgs.title ??= "Dialog";
          directArgs.children ??= React.createElement(
            "p",
            null,
            "Dialog content",
          );
          ...
          if (componentName === "UndoBanner") {
            render(
              React.createElement(
                UndoManagerProvider,
                null,
                React.createElement(component as React.ElementType, directArgs),
              ),
            );
        ...
        await act(async () => {
          if (harness) {
            expect(
              component,
              `missing renderable subject for ${contractPath}`,
            ).toBeDefined();
            render(
              React.createElement(harness, {
                ...
                log,
                config: (
                  story.composition as
                    | { harness?: { config?: Record<string, unknown> } }
                    | undefined
                )?.harness?.config,
              }),
            );
            ...
            render(
              React.createElement(
                UndoManagerProvider,
                null,
                React.createElement(component as React.ElementType, args),
              ),
            );
          } else if (componentName === "Toast") {
            render(
        ...
        const result = await runStory(
          ...
          { document, window },
          {
            ...
            actInteraction: async (dispatch: () => void) => {
              await act(async () => {
                dispatch();
              });
            },
