...
const styles = `
[data-rcl-settings-list] { --rcl-settings-section-gap: var(--space-xl, 2rem); --rcl-settings-group-gap: var(--space-md, 1rem); --rcl-settings-row-block: var(--space-sm, .75rem); --rcl-settings-row-inline: var(--space-md, 1rem); display: grid; gap: var(--rcl-settings-section-gap); min-inline-size: 0; container: rcl-settings / inline-size; }
[data-rcl-settings-list][data-density="compact"] { --rcl-settings-section-gap: var(--space-lg, 1.5rem); --rcl-settings-group-gap: var(--space-sm, .75rem); --rcl-settings-row-block: var(--space-xs, .5rem); --rcl-settings-row-inline: var(--space-sm, .75rem); }
[data-rcl-settings-intro] { display: grid; gap: var(--space-3xs, .25rem); }
[data-rcl-settings-intro-eyebrow], [data-rcl-settings-group-label] { color: var(--color-muted-foreground, #64748b); font: var(--text-overline, 650 .6875rem/1rem system-ui, sans-serif); letter-spacing: var(--tracking-caps, .08em); text-transform: uppercase; }
[data-rcl-settings-intro-title] { margin: 0; color: var(--color-foreground, #0f172a); font: var(--text-title, 650 1.125rem/1.5rem system-ui, sans-serif); }
[data-rcl-settings-intro-description] { margin: 0; max-inline-size: 68ch; color: var(--color-muted-foreground, #64748b); font: var(--text-body-sm, 400 .875rem/1.375rem system-ui, sans-serif); }
[data-rcl-settings-group] { display: grid; gap: var(--rcl-settings-group-gap); min-inline-size: 0; }
[data-rcl-settings-group-surface] { min-inline-size: 0; border: 0 solid transparent; border-radius: 0; background: transparent; overflow: hidden; }
[data-rcl-settings-row] { display: grid; grid-template-columns: minmax(0, 1fr) auto; grid-template-areas: "label control" "hint control"; align-items: center; column-gap: var(--space-md, 1rem); min-block-size: 48px; padding: var(--rcl-settings-row-block) var(--rcl-settings-row-inline); color: var(--color-foreground, #0f172a); }
[data-rcl-settings-row] + [data-rcl-settings-row] { border-block-start: 1px solid var(--color-border, #cbd5e1); }
[data-rcl-settings-row-label] { grid-area: label; min-inline-size: 0; font: var(--text-label, 500 .875rem/1.25rem system-ui, sans-serif); }
[data-rcl-settings-row-hint] { grid-area: hint; min-inline-size: 0; color: var(--color-muted-foreground, #64748b); font: var(--text-caption, 400 .75rem/1rem system-ui, sans-serif); overflow-wrap: anywhere; }
[data-rcl-settings-row-control] { grid-area: control; min-inline-size: 0; max-inline-size: 100%; justify-self: end; }
[data-rcl-settings-row][data-control="wide"] { grid-template-columns: minmax(0, 1fr); grid-template-areas: "label" "hint" "control"; row-gap: var(--space-xs, .5rem); }
[data-rcl-settings-row][data-control="wide"] [data-rcl-settings-row-control] { inline-size: 100%; justify-self: stretch; }
[data-rcl-settings-list][data-variant="card"] [data-rcl-settings-group-surface] { border-color: var(--color-border, #cbd5e1); border-width: 1px; border-radius: var(--radius-panel, .75rem); background: var(--color-surface-raised, #fff); }
...
@container rcl-settings (min-width: 34rem) { [data-rcl-settings-list][data-variant="auto"] [data-rcl-settings-group-surface] { border-color: var(--color-border, #cbd5e1); border-width: 1px; border-radius: var(--radius-panel, .75rem); background: var(--color-surface-raised, #fff); } [data-rcl-settings-list][data-variant="auto"] [data-rcl-settings-group-label] { padding-inline: var(--rcl-settings-row-inline); } }
...
function SettingsListRoot({
  ...
  return (
    ...
    >
      <StyleSheet name="settings-list-0-1-2" css={styles} />
      {children}
    </section>
  );
}
