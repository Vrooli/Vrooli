...
.rcl-full-page-drawer__panel { --rcl-overlay-progress: 0; position: absolute; inset-inline: 0; inset-block-start: calc(var(--rcl-safe-top, 0px) + var(--overlay-drawer-top-gap, 32px)); inset-block-end: 0; display: flex; flex-direction: column; min-block-size: 0; overflow: hidden; border-radius: var(--radius-sheet) var(--radius-sheet) 0 0; background: var(--color-surface-raised); color: var(--color-foreground); box-shadow: var(--elev-modal); pointer-events: auto; transform: translateY(calc(var(--rcl-overlay-progress) * 100%)); transition: transform var(--dur-moderate) var(--ease-standard); animation: rcl-full-page-drawer-enter var(--dur-moderate) var(--ease-enter); }
.rcl-full-page-drawer__panel[data-dragging="true"] { transition: none; }
...
.rcl-full-page-drawer__grabber { flex: 0 0 auto; display: grid; place-items: center; inline-size: 100%; min-block-size: var(--tap-target-min); margin: 0; padding: 0; border: 0; background: transparent; color: inherit; touch-action: none; cursor: grab; }
