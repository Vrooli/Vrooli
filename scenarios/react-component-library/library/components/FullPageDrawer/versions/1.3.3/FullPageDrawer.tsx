...
export function FullPageDrawer({
  ...
  const collapseSwipe = useSwipeGesture({
    ...
    releaseMode: "commit",
    disabled: !showGrabber || !expandable || !expanded,
    onStart: () => console.info("FullPageDrawer collapse swipe started"),
    onStageChange: (stage) => console.info("FullPageDrawer collapse swipe stage", stage),
    onMove: ({ offset }) => {
    ...
    onRelease: (release) => {
      console.info("FullPageDrawer collapse swipe release", release);
      const surface = overlay.surfaceRef.current;
      if (surface) {
