...
function Glyph({ kind, className = "" }: { kind: GlyphKind; className?: string }) {
  return (
    <svg aria-hidden="true" className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      {kind === "mic" && <><rect x="6" y="3" width="12" height="12" rx="6" /><path d="M4 12a8 8 0 0 0 16 0M12 20v-3" /></>}
...
export function VoiceInputButton({ state = "idle", mode = "always-on", level = 0, timeoutProgress = 0, error, rejectionReason, partialTranscript, onStart, onStop, onPrepare, onDismissError, onTranscribeAnyway, className = "", disabled, onPointerCancel, ...props }: VoiceInputButtonProps) {
  ...
  const glyph = state === "preparing" || state === "recovering" ? "mic" : state === "transcribing" ? "loader" : state === "error" || state === "unavailable" ? "alert" : "mic";
  ...
  return <div className={`relative shrink-0 ${className}`}>
    ...
    >
      ...
      <Glyph kind={glyph} className={`relative h-3.5 w-3.5 ${state === "preparing" || state === "recovering" ? "animate-pulse" : state === "transcribing" ? "animate-spin" : ""}`} />
    </button>
