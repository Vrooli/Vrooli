...
[data-rcl-voice-level] { position: absolute; inset-inline: 0; inset-block-end: 0; z-index: 0; inline-size: 100%; block-size: 0; border-radius: inherit; background: color-mix(in srgb, var(--rcl-voice-accent) 58%, transparent); pointer-events: none; transition: block-size var(--dur-quick) var(--ease-standard); } [data-rcl-voice-level][data-mode="always-on"] { background: color-mix(in srgb, var(--color-primary) 60%, transparent); } [data-rcl-voice-level][data-mode="timeout"] { background: color-mix(in srgb, var(--color-warning) 70%, transparent); }
...
export interface VoiceInputButtonProps
  ...
  /** Resting chrome for the host surface. Active states retain voice feedback. */
  readonly surface?: IconButtonSurface;
  /** Geometry of the control; rounded is useful when the control is wider than its glyph. */
...
export function VoiceInputButton({
  ...
  return (
    <IconButton
      ...
      size={size === "default" || size === "icon" ? "sm" : size === "xl" ? "lg" : size}
      surface={surface}
      shape={shape}
      className={className}
