...
import { Button, type ButtonSize } from "../../../Button/versions/1.3.0/Button";
...
export function VoiceInputButton({
  ...
  const glyphClass = cn(
    "relative z-10",
    size === "xs" ? "h-3 w-3" : size === "icon" ? "h-5 w-5" : "h-4 w-4",
      ...
      >
        {active && (
          <span
            aria-hidden="true"
            className={cn(
              "pointer-events-none absolute inset-x-0 bottom-0 z-0 rounded-[inherit] transition-[height] duration-75",
              state === "recovering" || mode === "always-on"
                ? "bg-app-primary/60"
                : "bg-app-warning/70",
            )}
