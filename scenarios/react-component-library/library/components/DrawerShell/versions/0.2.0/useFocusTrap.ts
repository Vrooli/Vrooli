...
export function useFocusTrap(active: boolean, containerRef: RefObject<HTMLElement | null>): void {
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.shiftKey && (current === first || current === null || !panel.contains(current))) { event.preventDefault(); last.focus(); }
      if (!event.shiftKey && (current === last || current === null || !panel.contains(current))) { event.preventDefault(); first.focus(); }
    };
