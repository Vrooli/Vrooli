 ...
 * @displayName Message
 * @description The structured conversational item carrying actor, content, status, citations, attachments, tool activity, actions, timestamp, error state, and expandable metadata as one coherent unit.
 * @version 1.1.1
 * @tags []
...
import { Avatar, type AvatarPresence } from "@vrooli/react-component-library/Avatar/1";
...
function stateStatus(state: MessageState) {
  if (state === "success") return { status: "success" as const, label: "Delivered" };
  if (state === "request-error") return { status: "error" as const, label: "Response unavailable" };
  if (state === "loading") return { status: "pending" as const, label: "Generating response" };
  if (state === "partial") return { status: "pending" as const, label: "Streaming response" };
  if (state === "retry") return { status: "error" as const, label: "Ready to retry" };
  return null;
}
        ...
        {items.map((item) => (
          <li key={item.id} data-rcl-message-attachment>
            ...
            <span data-rcl-message-attachment-copy>
              ...
              {item.detail ? <span data-rcl-message-detail>{item.detail}</span> : null}
              {item.status && item.status !== "ready" ? (
                <span data-rcl-message-detail>
                  {item.status === "processing" ? "Processing" : "Needs attention"}
                </span>
              ) : null}
            </span>
          </li>
        ...
        {items.map((item, index) => (
          <li key={item.id} data-rcl-message-citation>
            ...
            <span data-rcl-message-citation-copy>
              ...
              {item.source ? <span data-rcl-message-detail>{item.source}</span> : null}
              {item.excerpt ? <span data-rcl-message-detail>{item.excerpt}</span> : null}
            </span>
          </li>
        ))}
      </ol>
  ...
  return (
    ...
    >
      ...
      <header data-rcl-message-header>
        ...
        <div data-rcl-message-actor>
          <div data-rcl-message-actor-line>
            <Text as="strong" id={titleId} textStyle="label" data-rcl-message-actor-name>
              {actor.name}
            </Text>
            {actor.role ? <span data-rcl-message-actor-role>{actor.role}</span> : null}
          </div>
          {timestamp ? (
      ...
      <div data-rcl-message-body>
        {isLoading ? (
          <div
            id={content === undefined ? descriptionId : undefined}
        ...
        {activity ? (
          <div data-rcl-message-state aria-label={activity.label}>
            ...
            />
            {activity.detail ? <span data-rcl-message-state-copy>{activity.detail}</span> : null}
          </div>
        ) : null}
        ...
        {state === "request-error" || state === "retry" ? (
          <div data-rcl-message-error role={state === "request-error" ? "alert" : "status"}>
