...
/* The native control sits transparent on top and keeps every behaviour that is
   expensive to re-implement: click-to-position, drag, keyboard, form
   participation, and assistive-technology semantics. */
[data-rcl-slider-input] {
  ...
  appearance: none;
  /* Deliberately "none" rather than "pan-y". "pan-y" merely asks the browser to prefer vertical
     panning, and it then claims any horizontal drag that drifts off-axis —
     which is every drag made with a thumb. A press still worked, so the
     failure looked like "the thumb won't move" rather than a cancelled
     gesture. Owning the target is what every slider ends up doing. */
  touch-action: none;
}
[data-rcl-slider-input]:disabled { cursor: not-allowed; }
...
function SliderStyles() {
  return <StyleSheet name="slider-1-0-2-1" css={styleSheet} />;
}
    ...
    <>
      ...
      >
        ...
        <div data-rcl-slider-row>
          <div data-rcl-slider-area>
            <div data-rcl-slider-track>
              ...
              <span data-rcl-slider-thumb aria-hidden="true" />
            </div>
            {showValue === "tooltip" && (
            ...
            <input
              ...
              onPointerCancel={() => {
                // The value the control already holds is the one it keeps; the
                // interaction is simply over.
                setActive(false);
                commit();
              }}
