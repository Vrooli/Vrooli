 ...
 * @displayName Slider
 * @description The single-value range control with keyboard stepping, snapping, marks, formatted output, touch-friendly hit area, and accessible value text.
 * @version 1.2.3
 * @tags []
...
/** Controls where the formatted slider value appears. */
export type SliderValueDisplay = "none" | "inline" | "tooltip";

/** Props for the token-bound range control. */
...
export const Slider = withClassName(function Slider({
  ...
  const drag = useDrag({
    ...
    onMove: (event) => emit(valueAt(event.clientX)),
    onEnd: () => {
      setActive(false);
      commit();
    },
  ...
  const { isDragging: dragActive, ...dragHandlers } = drag;
