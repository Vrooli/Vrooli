...
function Showcase({ title, hint, children }: { title: string; hint: string; children: ReactNode }) {
  return (
    <section
      style={{
    ...
    >
      <div style={{ display: "grid", gap: "var(--space-2xs)" }}>
        ...
        <span style={{ color: "var(--color-muted-foreground)", font: "var(--text-body)" }}>
          {hint}
        </span>
      </div>
      {children}
...
export function Marked() {
  const [value, setValue] = useState(1);
  return (
    <Showcase title="Speech rate" hint="Marks carry words where a bare number would not help.">
      <Slider
        label="Speech rate"
        min={0.5}
        max={2}
...
export function Bare() {
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ font: "var(--text-body)", fontWeight: 650 }}>Output volume</span>
        <Slider
...
export function Disabled() {
  return (
    <Showcase title="Unavailable" hint="A disabled slider keeps its value legible.">
      <Slider label="Output volume" defaultValue={40} disabled formatValue={(v) => `${v}%`} />
    </Showcase>
  );
}
