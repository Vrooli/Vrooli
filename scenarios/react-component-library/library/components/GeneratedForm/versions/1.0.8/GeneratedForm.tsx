...
import { ArrayField } from "@vrooli/react-component-library/ArrayField/1";
import { ComputedField } from "@vrooli/react-component-library/ComputedField/1";
import { ConditionalField } from "@vrooli/react-component-library/ConditionalField/1";
...
import { ObjectField } from "@vrooli/react-component-library/ObjectField/1";
import { ValidationSummary } from "@vrooli/react-component-library/ValidationSummary/1";
import { createFormStore, type FormStore } from "@vrooli/react-component-library/FormStore/1";
...
export interface GeneratedField<TValues extends Record<string, unknown> = Record<string, unknown>> {
...
function RenderField<TValues extends Record<string, unknown>>({
  ...
  if (field.type === "checkbox") return <InputControl field={field} store={store} />;
  return (
    <FormField
      label={field.label}
      description={field.description}
          ...
          {sections.map((section) => (
            ...
            >
              <div data-rcl-generated-form-section-fields>
                {fields.filter((field) => field.section === section.id).map(renderField)}
              </div>
            </FormSection>
          ))}
        </div>
