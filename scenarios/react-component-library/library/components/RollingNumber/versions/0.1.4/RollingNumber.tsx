...
const compact = (value: number, digits = 1): string => {
  ...
  if (abs >= 1_000) return `${trim((value / 1_000).toFixed(digits))}k`;
 ...
 */
function formatFigure(value: number, format?: string, unit?: string): FormattedFigure {
  switch (format) {
    case "currency":
    ...
    default:
      return {
        prefix: "",
        text: group(value, Number.isInteger(value) ? 0 : 1),
...
export const RollingNumber = withClassName(function RollingNumber({
  ...
  const formatted =
    value === null ? { prefix: "", text: empty, suffix: "" } : formatFigure(value, format, unit);
