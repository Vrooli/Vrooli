...
export const PermissionState = withClassName(function PermissionState({
  ...
  return (
    <div role="status" style={{ ...panel, display: "grid", gap: 10 }}>
      <strong>{strings("feedback.permission-state.permission-required", "Permission required")}</strong>
      <span style={muted}>
        {strings("feedback.permission-state.request-access-to-continue", "Request access to continue.")}
      </span>
      {action && (
