...
/** @vrooliComponentSource react-component-library:Popover
 * @vrooliComponentSourceSlot overlays.popover */
import {
  Children,
  cloneElement,
  createContext,
...
import { useOutsideInteraction } from "@vrooli/react-component-library/useOutsideInteraction/1";
...
const styles = `
[data-rcl-popover-content] { --rcl-popover-top: 0px; --rcl-popover-left: 0px; --rcl-popover-arrow-left: 50%; position: fixed; inset-block-start: var(--rcl-popover-top); inset-inline-start: var(--rcl-popover-left); z-index: var(--layer-menu, 610); inline-size: min(calc(100vw - (var(--space-lg) * 2)), 24rem); max-block-size: min(32rem, calc(100vh - (var(--space-lg) * 2))); overflow: auto; overscroll-behavior: contain; transform-origin: var(--rcl-popover-arrow-left) top; background: linear-gradient(var(--color-surface-raised), var(--color-surface-raised)), var(--color-background); border: 1px solid var(--color-border); border-radius: var(--radius-panel); box-shadow: var(--elev-floating); color: var(--color-foreground); pointer-events: auto; }
[data-rcl-popover-content] :is(button, a[href], input, select, textarea, [role="button"]) { min-block-size: var(--tap-target-min); min-inline-size: var(--tap-target-min); }
...
[data-rcl-popover-trigger]:not([data-rcl-popover-trigger-as-child]) { min-block-size: var(--tap-target-min); padding-inline: var(--space-md); border: 1px solid var(--color-border-strong); border-radius: var(--radius-control); background: var(--color-surface); color: var(--color-foreground); cursor: pointer; font: var(--text-label); transition: background-color var(--dur-quick) var(--ease-standard), border-color var(--dur-quick) var(--ease-standard), box-shadow var(--dur-quick) var(--ease-standard); }
[data-rcl-popover-trigger]:not([data-rcl-popover-trigger-as-child]):hover { background: var(--color-surface-raised); border-color: var(--color-primary); }
[data-rcl-popover-trigger]:focus-visible { outline: var(--border-strong) solid var(--color-focus); outline-offset: var(--space-3xs); box-shadow: 0 0 0 var(--space-3xs) color-mix(in srgb, var(--color-focus) 18%, transparent); }
...
export const Popover = withClassName(function Popover({
  ...
  return (
    <PopoverContext.Provider value={context}>
      <style data-rcl-popover-styles dangerouslySetInnerHTML={{ __html: styles }} />
...
function PopoverPositioner({
  ...
  useLayoutEffect(() => {
    ...
    const position = () => {
      ...
      if (placement.startsWith("top")) top = triggerRect.top - contentRect.height - margin;
      ...
      if (placement.startsWith("right") && left + contentRect.width > window.innerWidth - margin) {
        left = triggerRect.left - contentRect.width - margin;
        ...
        left =
          placement.startsWith("right") || placement.startsWith("left") ? left : triggerRect.left;
      if (
      ...
      left = Math.min(Math.max(margin, left), window.innerWidth - contentRect.width - margin);
      top = Math.min(Math.max(margin, top), window.innerHeight - contentRect.height - margin);
    ...
    const positionWhenReady = () => {
      ...
      if (attempts < 12) frameId = window.requestAnimationFrame(positionWhenReady);
    };
...
export const PopoverTrigger = withClassName(function PopoverTrigger({
  ...
  if (asChild) {
    ...
    const childRef = readChildRef(child, childProps);
    return cloneElement(child as ReactElement<Record<string, unknown>>, {
      ...sharedProps,
      "data-rcl-popover-trigger-as-child": true,
      ref: mergeRefs(context.triggerRef, childRef),
...
function readChildRef(child: ReactElement, props: { ref?: TriggerRef }): TriggerRef {
  // React 18 stores ref outside props and warns when it is read through
  // child.props. React 19 promotes ref into props and warns when the legacy
  // element.ref accessor is used. Select the compatible path at runtime so the
  // shared trigger remains warning-free on both supported React generations.
  const major = Number.parseInt(reactVersion.split(".")[0] ?? "0", 10);
  if (major >= 19) return props.ref;
  return (child as ReactElement & { ref?: TriggerRef }).ref;
}
...
export const PopoverContent = withClassName(function PopoverContent({
  ...
  const context = usePopoverContext();
  useLayoutEffect(() => {
    if (!context.open || initialFocus === "none" || typeof window === "undefined") return;
  ...
  return (
    <Portal>
      <Presence
        ...
        duration="quick"
        initial={false}
        // Presence intentionally uses `will-change`, which creates its own
        // stacking context. Promote that context with the same menu layer as
        // the surface so a portalled popover can sit above an open modal or
        // responsive drawer. A z-index on Surface alone cannot escape this
        // wrapper's stacking context.
        style={{
          position: "fixed",
          inset: 0,
          zIndex: "var(--layer-menu, 610)",
          pointerEvents: "none",
        }}
      >
        <Surface
