...
export const HealthCard = withClassName(function HealthCard({
  ...
  const resolvedDescription =
    description ??
    strings("data-display.health-card.description", "Runtime status and reachability");
  const resolvedStatusLabel = statusLabel ?? strings("data-display.health-card.status", "Status");
  const resolvedServiceLabel =
    serviceLabel ?? strings("data-display.health-card.service", "Service");
  ...
  const resolvedRefreshLabel =
    refreshLabel ?? strings("data-display.health-card.refresh", "Refresh");
    ...
    >
      <CardHeader>
        <div data-rcl-health-card-header>
          ...
          {data ? (
            <StatusBadge tone={statusTone ?? toneForStatus(data.status)}>{data.status}</StatusBadge>
          ) : null}
        </div>
      </CardHeader>
        ...
        {data ? (
          <dl data-rcl-health-card-details>
            <div>
              ...
              <dd data-testid={ids.statusValue}>{data.status}</dd>
            </div>
            <div>
              <dt>{resolvedServiceLabel}</dt>
              <dd data-testid={ids.serviceValue}>{data.service}</dd>
            </div>
            <div>
