  ...
  const dragProps = swipeEnabled
    ? {
        onPointerDown: (event: ReactPointerEvent<HTMLDivElement>) => {
          ...
          // Promote before the first move rather than between the first two
          // frames, so the gesture does not open with a dropped frame.
          markDragging(true);
        ...
        onPointerMove: (event: ReactPointerEvent<HTMLDivElement>) => {
          ...
          writeOffset(Math.max(0, dismissTowardStart.current ? -delta : delta));
        },
