...
import { useElementRect } from "@vrooli/react-component-library/useElementRect/2";
import { useLocale } from "@vrooli/react-component-library/useLocale/1";
...
[data-rcl-chart-plot] { position: relative; min-block-size: 16rem; padding: var(--space-sm, 16px) var(--space-lg, 32px) 0; }
...
export const Chart = withClassName(function Chart({
  ...
  const [selected, setSelected] = useState<string | null>(data.at(-1)?.id ?? null);
  const titleId = useId();
      ...
      >
        ...
        status === "partial-error" ? (
          <div data-rcl-chart-surface data-presentation={presentation}>
            ...
            <div ref={setPlotElement} data-rcl-chart-plot data-rcl-chart-part="plot">
              {data.length > 0 ? (
                <svg
                  role="img"
                    ...
                    return (
                      <g key={tick} data-rcl-chart-part="axis">
                        <line
                          ...
                          y2={y}
                        />
                        <text data-rcl-chart-axis x={padding.left - 8} y={y + 4} textAnchor="end">
              ...
              >
                {data.map((point) => (
                  ...
                  >
                    ...
                    <span data-rcl-chart-legend-value>{valueFormatter(point.value, locale)}</span>
                  </button>
                ))}
              </div>
            )}
