 ...
 * @displayName Tabs
 * @description The content-switching component with roving focus, a shared moving indicator, lazily mounted panels, optional URL synchronization, overflow handling, and reduced-motion behavior.
 * @version 1.3.1
 * @tags []
...
/** Spacing density for the tab strip. */
...
export interface TabsProps {
  ...
  /**
   * Automation selector for the strip itself.
   *
   * It defaults to the catalog id, which is the right name for the only strip
   * on a surface — but it was previously hardcoded, so two strips mounted at
   * once (a dialog's over a sidebar's, say) resolved to the same id and every
   * root-level selector for either became ambiguous. Naming one is how a
   * caller says which strip its flows mean.
   */
  testId?: string;
...
export const Tabs = withClassName(function Tabs({
  ...
  // One owner/version key owns one complete sheet; separate registrations
  // with the same key would discard the tab rules after mounting the base.
  useLibraryStyleSheet("react-component-library:Tabs", "1.3.1", `${baseStyles}\n${styleSheet}`);
