 ...
 * @displayName CardShell
 * @description Row chrome for scenario-owned cards with selection, disabled, cursor, focus, and press states.
 * @version 1.0.5
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/* eslint-disable jsx-a11y/no-static-element-interactions, jsx-a11y/no-noninteractive-tabindex */
/** @vrooliComponentSource react-component-library:CardShell
 * @vrooliComponentSourceSlot data-display.card-shell */
...
const css = `
[data-rcl-card-shell] { box-sizing: border-box; position: relative; display: grid; inline-size: 100%; min-inline-size: 0; grid-template-columns: minmax(0, 1fr); align-items: stretch; overflow: hidden; border: var(--border-hairline) solid var(--color-border); border-radius: var(--radius-card); background: var(--color-surface-raised); color: var(--color-foreground); transition: box-shadow var(--dur-quick) var(--ease-standard), border-color var(--dur-quick) var(--ease-standard), transform var(--dur-quick) var(--ease-standard); }
...
[data-rcl-card-shell][data-interactive=true] { cursor: pointer; }
[data-rcl-card-shell][data-interactive=true]:focus-visible { outline: var(--border-strong) solid var(--color-focus); outline-offset: 2px; }
[data-rcl-card-shell] [data-rcl-card-rail] { display: grid; place-items: start center; min-inline-size: 3.25rem; padding: var(--space-md) var(--space-xs) var(--space-md) var(--space-sm); border-inline-end: 1px solid var(--color-border-muted); background: color-mix(in srgb, var(--color-surface-muted) 42%, transparent); }
[data-rcl-card-shell] [data-rcl-card-content] { inline-size: 100%; min-inline-size: 0; padding: var(--space-md); overflow-wrap: anywhere; }
  ...
  return (
    <>
      ...
      <div
        ...
        aria-pressed={selectionMode ? selection?.selected : undefined}
        role={interactive ? "button" : undefined}
        className={className}
        data-testid={testId}
        tabIndex={interactive ? 0 : undefined}
        onClick={
          interactive ? () => (selectionMode ? toggleSelection?.() : onPress?.()) : undefined
        }
        ...
        {selectionMode ? (
          ...
          >
            <SelectionControl
              kind="checkbox"
              aria-label={
                selectLabel ?? strings("data-display.card-shell.select-row", "Select row")
              }
