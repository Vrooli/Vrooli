 ...
 * @description The declarative boundary coordinating loading, empty, partial, stale, refreshing, offline, error, and success presentation around arbitrary content, so no surface hand-rolls its own state ladder.
 * @version 1.0.7
 * @tags []
...
import { useNetworkStatus } from "@vrooli/react-component-library/useNetworkStatus/1";
...
const styles = `
  ...
  /* The accessible description, and only that.
     This span exists to be the aria-describedby target named above it, and it
     was never given a rule — so it laid out as ordinary text and printed a
     generic line above every state this component renders. On an error that
     put "Content failed to load." directly over whatever specific title the
     caller passed as errorTitle, which is two headings for one failure and
     buries the sentence that actually says what to do. Assistive technology
     still reads it; the eye no longer has to. */
