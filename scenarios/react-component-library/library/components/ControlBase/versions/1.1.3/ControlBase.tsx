...
// Static recipe values live in zero-specificity CSS so consumer classes win
// regardless of when the library stylesheet is mounted. Explicit style props
// retain their normal inline precedence.
  ...
  return Object.entries(recipes)
    .map(([value, declarations]) => {
      const css = Object.entries(declarations)
        ...
        .join(" ");
      return `:where([data-rcl-control][${attribute}="${value}"]) { ${css} }`;
    })
    .join("\n");
}
...
:where([data-rcl-control][data-control-density="comfortable"]) { gap: var(--space-2xs); }
:where([data-rcl-control][data-control-density="compact"]) { gap: var(--space-3xs); }
:where([data-rcl-control][data-control-shape="pill"]) { border-radius: var(--radius-pill); }
:where([data-rcl-control][data-control-shape="square"]) { border-radius: var(--radius-control); }
