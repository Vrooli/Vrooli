 ...
 * @description Reusable control base component implementation for the React component library.
 * @version 1.1.5
 * @tags []
...
// Component recipes use one host-attribute of specificity. This beats browser
// and Tailwind control resets while consumer utility classes, which mount
// later, can still override an equal-specificity default.
function recipeRules(attribute: string, recipes: Record<string, CSSProperties>) {
  return Object.entries(recipes)
    .map(([value, declarations]) => {
      const css = Object.entries(declarations)
        .map(
          ([property, declaration]) =>
            `${property.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`)}: ${declaration};`,
        )
        .join(" ");
      return `[data-rcl-control]:where([${attribute}="${value}"]) { ${css} }`;
    })
    .join("\n");
}

const styleSheet = `
${recipeRules("data-control-variant", variantStyles)}
${recipeRules("data-control-size", sizeStyles)}
[data-rcl-control]:where([data-control-density="comfortable"]) { gap: var(--space-2xs); }
[data-rcl-control]:where([data-control-density="compact"]) { gap: var(--space-3xs); }
[data-rcl-control]:where([data-control-shape="pill"]) { border-radius: var(--radius-pill); }
[data-rcl-control]:where([data-control-shape="square"]) { border-radius: var(--radius-control); }
...
export const ControlBase = forwardRef<HTMLButtonElement, ControlBaseProps>(function ControlBase(
  ...
  useLibraryStyleSheet("react-component-library:ControlBase", "1.1.5", styleSheet);
