...
/** @vrooliComponentSource react-component-library:Menu
 * @vrooliComponentSourceSlot overlays.menu */
import {
  createContext,
  useCallback,
  useContext,
...
import { useHoverIntent } from "@vrooli/react-component-library/useHoverIntent/1";
...
[data-rcl-menu-submenu-arrow] { inline-size: var(--icon-size-sm); block-size: var(--icon-size-sm); flex: 0 0 auto; }
  ...
  @media (hover: hover) and (pointer: fine) { [data-rcl-menu-submenu][data-trigger="hover"] { } }
   (prefers-reduced-motion: reduce) { [data-rcl-menu-item], [data-rcl-menu-checkbox], [data-rcl-menu-radio] { transition: none; } }
`;
...
interface MenuRegistry {
  ...
  register: (id: string, label: string, ref: RefObject<HTMLElement>, disabled: boolean) => number;
  close: () => void;
}
...
export const MenuContent = withClassName(function MenuContent({
  ...
  const registryRef = useRef(
    new Map<string, { label: string; ref: RefObject<HTMLElement>; disabled: boolean }>(),
  );
  const [, rerender] = useState(0);
  const register = useCallback(
    (id: string, label: string, ref: RefObject<HTMLElement>, disabled: boolean) => {
  ...
  const disabledIndices = entries.flatMap((entry, index) => (entry.disabled ? [index] : []));
...
function MenuItemBase({
  ...
  useEffect(() => setIndex(menu.register(id, label, ref, disabled)), [disabled, id, label, menu]);
  ...
  return (
    <button
      ...
      data-rcl-menu-part={
        kind === "checkbox" ? "checkbox-item" : kind === "radio" ? "radio-item" : "item"
      }
...
function SubmenuTrigger({
  ...
  const { onChildEnter: _onChildEnter, cancel: _cancel, ...hoverHandlers } = hover;
...
export const MenuSubmenu = withClassName(function MenuSubmenu({
  ...
  return (
    <div data-rcl-menu-submenu data-trigger={trigger}>
      <Popover placement="right-start" responsive="none">
        <SubmenuTrigger
          label={label}
          disabled={disabled}
          trigger={trigger}
          childRect={() => childRef.current?.getBoundingClientRect() ?? null}
        />
        <div ref={childRef}>
