 ...
 * @description The compact icon-only action with a mandatory accessible name, expanded touch target beyond its visual bounds, tooltip integration, loading state, and optional toggle semantics.
 * @version 3.1.6
 * @tags []
 * @warning Managed by React Component Library. Preserve this header when editing adopted copies.
 */
/** @vrooliComponentSource react-component-library:IconButton
 * @vrooliComponentSourceSlot controls.icon-button */
...
/* The glyph dims under the spinner rather than disappearing. Hiding it
   outright meant any icon change that coincided with the pending window — the
   normal shape of "act, then load" — animated where nobody could see it. */
[data-rcl-icon-button][data-rcl-pending="true"] [data-rcl-icon-button-glyph] { opacity: .35; }
...
/**
 * Three corner families. `circle` is `--radius-pill`, `rounded` is
 * `--radius-control`, `square` is a literal 0 radius.
 *
 * `Button` expresses the same decision with a different vocabulary: its `pill`
 * is this `circle`, and its `square` is this `rounded`. Nothing on `Button`
 * corresponds to this `square` (0 radius), which exists for controls that butt
 * flush against a bounding edge. The overlap is a known wart — when a Button
 * and an IconButton sit in the same row, pair `pill`/`circle` or
 * `square`/`rounded`, never one of each.
 */
...
export interface IconButtonProps
   ...
   * ── Choosing a shape ──────────────────────────────────────────────────────
   *
   * Same doctrine as `Button`, stated in full on that component's `shape`
   * prop. Translated into this vocabulary:
   *
   *     circle is for controls that ACT. rounded is for controls that REPEAT.
   *
   * `rounded` is the exception, and it has three cases:
   *
   *   S1  keypad   Identical controls tiled in a strip or grid, sized by the
   *                grid rather than by their own glyph — a key row, an arrow
   *                cluster, a header icon strip aligned to a list below it.
   *   S2  column   One control repeated once per row down a list or table —
   *                a delete per profile, a reorder pair per pane.
   *   S3  welded   The control shares a border with a field: an `InputGroup`
   *                trailing action, a Send attached to a `Textarea`.
   *
   * Everything else is a `circle`: a lone close or refresh, an action over
   * media, a control floating in a pill-shaped dock. Never mix corner families
   * inside a single action row.
   *
   * For a ghost icon button the shape is only ever painted as the hover and
   * pressed halo, so this prop decides the halo, not a resting outline. That
   * makes it easy to under-think — a round halo in a text-aligned table row
   * still reads wrong even though nothing is visible at rest.
   *
   * The default is `circle`, unlike `Button`, whose default is `square`
   * (== this `rounded`). That asymmetry is historical, not doctrinal: 2.x
   * hard-coded `shape="square"` and could not express a round icon button at
   * all, so 3.0 made the round case reachable and default. Read it as "the
   * library already thinks a lone icon button is round", not as a rule that
   * icon buttons differ from text buttons.
   *
   * @default "circle"
   */
