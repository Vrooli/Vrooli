 ...
 * @description The secure text field with a reveal control, requirement feedback, strength indication, caps-lock warning, autofill compatibility, and guidance that does not leak the value.
 * @version 2.0.2
 * @tags []
 * @deps {"react":"^18"}
...
const PasswordControl = forwardRef<HTMLInputElement, PasswordInputProps>(function PasswordControl(
  ...
  const resolvedRevealLabel =
    revealLabel ?? strings("forms.password-input.show-password", "Show password");
  const resolvedConcealLabel =
    concealLabel ?? strings("forms.password-input.hide-password", "Hide password");
    ...
    <div data-rcl-password="true" className={className} data-testid={testId}>
      ...
      >
        ...
        {revealable && (
          <InputGroup.Action>
            <IconButton
              aria-label={isRevealed ? resolvedConcealLabel : resolvedRevealLabel}
              selected={isRevealed}
              disabled={disabled}
              size={size === "lg" ? "md" : "sm"}
