...
export function InlineCode({
  ...
  const tokenClass =
    "rounded bg-[var(--color-surface-muted)] [padding-inline:var(--space-3xs)] [padding-block:var(--space-3xs)] font-mono text-[var(--color-foreground)]";
  ...
  if (resolution)
    return (
      <a
        ...
        className={`${tokenClass} text-[var(--color-accent)] underline`}
      >
        {text}
      </a>
    );
  if (isFile)
    return (
      <button
        ...
        className={`${tokenClass} text-[var(--color-accent)] hover:bg-[var(--color-surface-raised)]`}
      >
        {text}
      </button>
    );
  return (
    <span className="group relative inline-flex items-center">
      ...
      <button
        ...
        className="[margin-inline-start:var(--space-3xs)] hidden rounded [padding-inline:var(--space-3xs)] [font-size:var(--text-caption-size)] text-[var(--color-muted-foreground)] group-hover:inline hover:bg-[var(--color-surface-muted)]"
      >
        {copied ? "Copied" : "Copy"}
      </button>
    </span>
