...
export interface AppShellNavItem {
  ...
  current?: boolean;
  disabled?: boolean;
  /** Exclude a destination from phone tabs when it remains reachable via utility. Defaults to true. */
  mobile?: boolean;
...
export interface AppShellProps {
  ...
  header?: ReactNode;
  /** Session, settings, theme: the column foot on desktop and a shell-owned header on phone tabs. */
  utility?: ReactNode;
...
export const AppShell = withClassName(function AppShell({
  ...
  const column = (
    <div data-rcl-app-shell-column="">
      ...
      {utility && (desktop || drawer) ? <div data-rcl-app-shell-utility="">{utility}</div> : null}
    </div>
  );

  const tabItems: BottomNavItem[] = items
  ...
  const mobileUtility = !desktop && !drawer ? utility : undefined;
  const showHeader = header !== undefined || drawer || Boolean(mobileUtility);
