 ...
 * @description A token-bound conversation composer with preserved drafts, async send, attachments and contextual commands.
 * @version 0.1.0
 * @tags ["ai","composer","messaging","token-bound"]
...
import { ComposerAttachmentTray, type ComposerAttachment } from "@vrooli/react-component-library/ComposerAttachmentTray/1";
import { StyleSheet } from "@vrooli/react-component-library/StyleSheet/1";
import type { CommandRegistry } from "@vrooli/react-component-library/CommandRegistry/1";

export interface ComposerSubmission { text: string; attachmentIds: string[] }
...
export function PromptComposer({ value, onValueChange, onSend, onSent, attachments = [], onFiles, onAttachmentRemove, onAttachmentRetry, onAttachmentCancel, onAttachmentReorder, commands, context, label = "Message", placeholder = "Write a message…", disabledReason, offline, generating, onStop, maxLength, accept, sendLabel = "Send message", className }: PromptComposerProps) {
  const id = useId();
  ...
  useSyncExternalStore<unknown>(commands?.subscribe ?? noSubscribe, commands?.getSnapshot ?? emptySnapshot, commands?.getSnapshot ?? emptySnapshot);
  const choices = commands && value.startsWith("/") && !value.includes("\n") && dismissedCommand !== value ? commands.search(value.slice(1)).slice(0, 5) : [];
  const unavailable = disabledReason || (offline ? "You’re offline. Your draft is saved here; reconnect to send." : "");
  ...
  const cannotSend = Boolean(unavailable || pending || generating || tooLong || waitingAttachments || (!value.trim() && attachments.length === 0));
  useLayoutEffect(() => {
  ...
  return <form data-rcl-prompt-composer className={className} aria-label={`${label} composer`} onSubmit={event => { event.preventDefault(); submitButton.current?.click(); }}>
    ...
    <div data-rcl-composer-surface data-dragging={dragging} onDragOver={event => {
    ...
    }} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setDragging(false); }} onDrop={event => {
      ...
      {choices.length > 0 && <div data-rcl-composer-commands role="group" aria-label="Suggested commands">
        {choices.map(command => <CommandButton key={command.id} type="button" variant="ghost" disabled={command.disabled || pending} action={() => commands!.execute(command.id, { query: value.slice(1) })} onSuccess={() => { setDismissedCommand(value); textarea.current?.focus(); }} onError={() => setError("Command failed. Your draft is still here.")}>{command.label}</CommandButton>)}
      </div>}
      <Textarea ref={textarea} aria-label={label} aria-describedby={`${id}-hint ${id}-feedback`} aria-invalid={tooLong || undefined} value={value} placeholder={placeholder}
        onChange={event => { onValueChange(event.target.value); setSent(false); }}
        onPaste={event => { if (onFiles && !pending && event.clipboardData.files.length) { event.preventDefault(); onFiles(Array.from(event.clipboardData.files)); } }}
        onKeyDown={event => {
          if (event.key === "Escape" && choices.length) { event.preventDefault(); setDismissedCommand(value); }
          if (event.key === "Enter" && !event.shiftKey && !event.altKey && !event.nativeEvent.isComposing && event.nativeEvent.keyCode !== 229) { event.preventDefault(); if (!cannotSend) submitButton.current?.click(); }
        }} />
      <div data-rcl-composer-toolbar>
        <div data-rcl-composer-tools>
          {onFiles && <><input ref={fileInput} type="file" multiple accept={accept} hidden onChange={event => { if (event.target.files?.length) onFiles(Array.from(event.target.files)); event.target.value = ""; }} /><Button type="button" variant="ghost" size="sm" disabled={pending} onClick={() => fileInput.current?.click()}>Attach files</Button></>}
          <p id={`${id}-hint`} data-rcl-composer-caption><span data-rcl-composer-keyboard-hint>Enter to send · Shift+Enter for a new line</span>{maxLength !== undefined && <span> {value.length}/{maxLength}</span>}</p>
        </div>
        {generating && onStop ? <CommandButton type="button" variant="secondary" action={onStop} pendingLabel="Stopping…">Stop response</CommandButton> : <CommandButton ref={submitButton} type="button" disabled={cannotSend} action={send} pendingLabel="Sending…" successLabel={sendLabel} errorLabel="Retry send">{sendLabel}</CommandButton>}
      </div>
    </div>
    <p id={`${id}-feedback`} data-rcl-composer-feedback data-error={Boolean(error || tooLong)} role="status" aria-live="polite">{feedback}</p>
  </form>;
}