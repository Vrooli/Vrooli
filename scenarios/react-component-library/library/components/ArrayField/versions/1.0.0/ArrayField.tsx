...
>({
  ...
  const items = Array.isArray(fieldState.value)
    ? (fieldState.value as unknown as TItem[])
    : [];
  ...
  return (
    <div
      className={className}
      style={style}
      data-rcl-array-field
      data-field={String(field)}
    >
      <StyleSheet name="arrayfield-1-0-0-1" css={styles} />
      <div data-rcl-array-header>
        ...
        <span
          data-rcl-array-count
          aria-label={`${items.length} ${listLabel.toLowerCase()}`}
        >
          {items.length}
          ...
          items.map((item, index) => {
            const actions: ArrayItemActions<TItem> = {
              ...
              moveDown: () =>
                ...
                update(move(items, index, index + 1)),
            };
