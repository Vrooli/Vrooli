...
[data-rcl-responsive-dialog] { box-sizing: border-box; min-inline-size: 0; position: fixed; inset: 0; z-index: var(--layer-modal); display: grid; align-items: end; pointer-events: none; }
...
.rcl-responsive-dialog__backdrop { position: absolute; inset: 0; margin: 0; padding: 0; border: 0; background: var(--color-scrim); pointer-events: auto; opacity: 1; transition: opacity var(--dur-quick) var(--ease-standard); }
[data-rcl-responsive-dialog][data-state="closed"] .rcl-responsive-dialog__backdrop { opacity: 0; }

.rcl-responsive-dialog__panel { box-sizing: border-box; min-inline-size: 0; max-inline-size: 100%; position: relative; display: flex; flex-direction: column; inline-size: 100%; max-block-size: calc(100% - var(--rcl-safe-top, 0px) - var(--overlay-drawer-top-gap)); overflow: hidden; border: var(--border-hairline) solid var(--color-border); border-block-end: 0; border-radius: var(--radius-sheet) var(--radius-sheet) 0 0; background: var(--color-surface-raised); color: var(--color-foreground); box-shadow: var(--elev-modal); pointer-events: auto; transition: transform var(--dur-quick) var(--ease-standard); animation: rcl-responsive-dialog-enter-sheet var(--dur-moderate) var(--ease-enter); }
...
.rcl-responsive-dialog__grabber > span { inline-size: var(--overlay-grabber-inline); block-size: var(--overlay-grabber-block); border-radius: var(--radius-pill); background: var(--color-border-strong); }
...
export function ResponsiveDialog({
  ...
  useLibraryStyleSheet("react-component-library:ResponsiveDialog", "1.3.1",   responsiveDialogStyles);
