      ...
      <Stack gap="lg" inset="xl">
        <Stack gap="2xs">
          <Text textStyle="overline" tone="accent">
            {useStrings("primitives.avatar.identity-primitive", "Identity primitive")}
          </Text>
...
export function Default({ args }: StoryHarnessProps<AvatarStoryArgs>) {
  return (
    <Showcase
      title={useStrings("primitives.avatar.title", "Identity without ambiguity")}
...
function DetailsClose() {
  ...
  return (
    <Button type="button" variant="ghost" size="sm" onClick={() => setOpen(false)}>
      {useStrings("primitives.avatar.close-details", "Close details")}
    </Button>
  );
}
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button type="button" variant="secondary">
              {useStrings("primitives.avatar.open-profile-card", "Open profile card")}
            </Button>
          </PopoverTrigger>
          <PopoverContent initialFocus="none" aria-label={`${args.name} profile details`}>
            <Stack gap="sm" inset="md">
              <Stack gap="2xs">
                <Text textStyle="overline" tone="accent">
                  {useStrings("primitives.avatar.profile", "Profile")}
                </Text>
    ...
    >
      <Stack gap="sm" align="center">
        ...
        <Button type="button" variant="secondary" onClick={updatePresence}>
          {useStrings("primitives.avatar.cycle-status", "Cycle status")}
        </Button>
...
export function RequestError({ args }: StoryHarnessProps<AvatarStoryArgs>) {
  return (
    <Showcase
      title={useStrings("primitives.avatar.title.a-useful-image-failure", "A useful image failure")}
...
export function Group({ args }: StoryHarnessProps<AvatarStoryArgs>) {
  return (
    ...
    >
      <AvatarGroup maxVisible={3} label={useStrings("primitives.avatar.label", "Reviewers")}>
