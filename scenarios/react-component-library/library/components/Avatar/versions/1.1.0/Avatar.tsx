 ...
 * @displayName Avatar
 * @description An identity visual with reserved image loading, deterministic initials fallback, presence, status, and group overflow semantics.
 * @version 1.1.0
...
[data-rcl-avatar] [data-rcl-progressive-image] { inline-size: 100%; block-size: 100%; overflow: hidden; border: 0; border-radius: inherit; background: transparent; }
...
[data-rcl-avatar-loading] { display: block; inline-size: 100%; block-size: 100%; border-radius: inherit; background: linear-gradient(120deg, var(--color-surface-muted), var(--color-surface-raised), var(--color-surface-muted)); background-size: 220% 100%; animation: rcl-avatar-shimmer var(--dur-slow) var(--ease-standard) infinite; }
[data-rcl-avatar-presence] { position: absolute; inset-block-end: calc(var(--space-3xs) * -1); inset-inline-end: calc(var(--space-3xs) * -1); display: grid; place-items: center; box-sizing: border-box; inline-size: var(--space-sm); block-size: var(--space-sm); border: var(--border-strong) solid var(--color-surface); border-radius: var(--radius-pill); background: var(--color-muted-foreground); color: var(--color-surface); font: var(--text-caption); }
...
@keyframes rcl-avatar-shimmer { 0% { background-position: 100% 0; } 100% { background-position: -100% 0; } }
@media (prefers-reduced-motion: reduce) { [data-rcl-avatar-loading] { animation: none; } }
        ...
        >
          {src ? (
            <ProgressiveImage
              ...
              ratio="1 / 1"
              frameStyle={{ boxSizing: "border-box", overflow: "hidden" }}
