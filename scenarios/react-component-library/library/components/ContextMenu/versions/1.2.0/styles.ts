...
.rcl-context-menu__surface { --rcl-overlay-progress: 0; position: relative; display: grid; gap: var(--space-3xs); align-content: start; inline-size: 100%; max-block-size: calc(var(--rcl-viewport-height, 100dvh) - var(--rcl-safe-top, 0px) - var(--overlay-drawer-top-gap, 32px)); overflow: auto; overscroll-behavior: contain; padding: var(--space-2xs); padding-block-end: calc(var(--space-2xs) + var(--rcl-safe-bottom, 0px)); border: var(--border-hairline) solid var(--color-border); border-block-end: 0; border-radius: var(--radius-sheet) var(--radius-sheet) 0 0; background: var(--color-surface-raised); color: var(--color-foreground); box-shadow: var(--elev-modal); pointer-events: auto; transform: translateY(calc(var(--rcl-overlay-progress) * 100%)); transition: transform var(--dur-quick) var(--ease-standard); animation: rcl-context-menu-enter-sheet var(--dur-moderate) var(--ease-enter); }
.rcl-context-menu__surface[data-dragging="true"] { transition: none; }
