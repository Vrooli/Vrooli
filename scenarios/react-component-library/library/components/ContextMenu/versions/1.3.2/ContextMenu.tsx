 ...
 * @description The pointer- or keyboard-invoked menu positioned around an interaction point, with viewport collision handling, nested actions, and awareness of the current selection.
 * @version 1.3.2
 * @tags []
...
[data-rcl-context-menu] { position: fixed; inset: 0; z-index: var(--layer-menu); pointer-events: none; }
[data-rcl-context-menu][data-presentation="sheet"] { display: grid; align-items: end; }

.rcl-context-menu__backdrop { position: absolute; inset: 0; margin: 0; padding: 0; border: 0; background: var(--color-scrim); pointer-events: auto; opacity: 1; transition: opacity var(--dur-quick) var(--ease-standard); }
[data-rcl-context-menu][data-state="closed"] .rcl-context-menu__backdrop { opacity: 0; }

.rcl-context-menu__surface { position: relative; display: grid; gap: var(--space-3xs); align-content: start; inline-size: 100%; max-block-size: calc(100% - var(--rcl-safe-top, 0px) - var(--overlay-drawer-top-gap)); overflow: auto; overscroll-behavior: contain; padding: var(--space-2xs); padding-block-end: calc(var(--space-2xs) + var(--rcl-safe-bottom, 0px)); border: var(--border-hairline) solid var(--color-border); border-block-end: 0; border-radius: var(--radius-sheet) var(--radius-sheet) 0 0; background: var(--color-surface-raised); color: var(--color-foreground); box-shadow: var(--elev-modal); pointer-events: auto; transition: transform var(--dur-quick) var(--ease-standard); animation: rcl-context-menu-enter-sheet var(--dur-moderate) var(--ease-enter); }
...
.rcl-context-menu__handle > span { display: block; inline-size: var(--overlay-grabber-inline); block-size: var(--overlay-grabber-block); border-radius: var(--radius-pill); background: var(--color-border-strong); }
...
@media (min-width: 48rem) {
  .rcl-context-menu__surface { position: absolute; inset: auto; inline-size: auto; min-inline-size: 14rem; max-inline-size: 24rem; max-block-size: calc(100% - (var(--space-lg) * 2)); padding-block-end: var(--space-2xs); border-block-end: var(--border-hairline) solid var(--color-border); border-radius: var(--radius-panel); box-shadow: var(--elev-overlay); transform: translateX(var(--overlay-menu-align)); animation-name: rcl-context-menu-enter-menu; }
  [data-rcl-context-menu][data-state="closed"] .rcl-context-menu__surface { transform: translateX(var(--overlay-menu-align)) translateY(var(--space-3xs)); opacity: 0; }
...
export const ContextMenu = forwardRef<HTMLDivElement, ContextMenuProps>(function ContextMenu(
  ...
  useLibraryStyleSheet("react-component-library:ContextMenu", "1.3.2", contextMenuStyles);
  ...
  // The sheet dismisses through its backdrop. The anchored menu has no
  // backdrop, so a press or focus anywhere outside its surface dismisses it.
  // A consumer-supplied anchor is excluded so its own toggle can close the
  // menu; the cloned child trigger is not, because it may wrap many rows.
  const excludeRefs = useMemo(() => (anchorRef ? [anchorRef] : []), [anchorRef]);
  useOutsideInteraction({
    active: overlay.open && desktop,
    surfaceRef: overlay.surfaceRef,
    excludeRefs,
    dismissOnEscape: false,
    onPointerDownOutside: overlay.close,
    onFocusOutside: overlay.close,
  });
  ...
  const longPress = useLongPress({
    onLongPress: openAt,
    disabled: !triggers.includes("long-press"),
  });
  useEffect(() => {
    if (overlay.open)
      setActive(
          ...
          return cloneElement(child, {
            ...
            onContextMenu: (event: React.MouseEvent<HTMLElement>) => {
              ...
              if (triggers.includes("contextmenu"))
                openAt({ x: event.clientX, y: event.clientY, pointerType: "mouse" });
            },
            onPointerDown: (event: React.PointerEvent<HTMLElement>) => {
              childProps.onPointerDown?.(event);
              // A mouse opens the menu with its secondary button; holding the
              // primary button is an ordinary press, not a menu gesture.
              if (event.pointerType !== "mouse") longPress.longPressProps.onPointerDown?.(event);
            },
            ...
            onClick: (event: React.MouseEvent<HTMLElement>) => {
              ...
              if (!event.defaultPrevented) childProps.onClick?.(event);
            },
          });
        })()
      : null;
