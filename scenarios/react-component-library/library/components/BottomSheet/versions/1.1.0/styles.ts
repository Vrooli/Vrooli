...
[data-rcl-bottom-sheet] { position: fixed; inset: 0; z-index: var(--layer-modal, 400); display: grid; align-items: end; padding: var(--space-xs); padding-block-end: max(var(--space-xs), var(--rcl-safe-bottom, 0px)); pointer-events: none; }
[data-rcl-bottom-sheet][data-avoid-keyboard] { inset-block-end: var(--rcl-keyboard-inset, 0px); padding-block-end: var(--space-xs); }
...
.rcl-bottom-sheet__panel { --rcl-overlay-progress: 0; position: relative; display: flex; flex-direction: column; inline-size: min(100%, 42rem); max-block-size: calc(var(--rcl-viewport-height, 100dvh) - (var(--space-lg) * 2)); margin-inline: auto; overflow: hidden; border: var(--border-hairline) solid var(--color-border); border-radius: var(--radius-sheet); background: var(--color-surface-raised); color: var(--color-foreground); box-shadow: var(--elev-modal); pointer-events: auto; transform: translateY(calc(var(--rcl-overlay-progress) * 100%)); transition: transform var(--dur-quick) var(--ease-standard); animation: rcl-bottom-sheet-enter var(--dur-moderate) var(--ease-enter); }
.rcl-bottom-sheet__panel[data-dragging="true"] { transition: none; }
[data-rcl-bottom-sheet][data-state="closed"] .rcl-bottom-sheet__panel { transform: translateY(calc(100% + var(--space-xs))); animation: none; }
@keyframes rcl-bottom-sheet-enter { from { transform: translateY(calc(100% + var(--space-xs))); } }

.rcl-bottom-sheet__grabber { flex: 0 0 auto; display: grid; place-items: center; inline-size: 100%; min-block-size: var(--tap-target-min); margin: 0; padding: 0; border: 0; background: transparent; color: inherit; touch-action: none; cursor: grab; }
...
.rcl-bottom-sheet__footer { border-block-start: var(--border-hairline) solid var(--color-border); }
...
.rcl-bottom-sheet__content { flex: 1 1 auto; min-block-size: 0; overflow: auto; overscroll-behavior: contain; }
