...
[data-rcl-stat-card] { position: relative; display: grid; min-block-size: 7.375rem; box-sizing: border-box; gap: var(--space-2xs); padding: var(--space-md); border: var(--border-hairline) solid var(--rcl-stat-border); border-radius: var(--radius-panel); background: var(--rcl-stat-surface); color: var(--rcl-stat-value); box-shadow: var(--elev-raised); }
...
[data-rcl-stat-trend] { color: var(--rcl-stat-value); font-size: var(--font-size-sm); font-weight: 700; }
`;
export const StatCard = withClassName(function StatCard({
  ...
  useLibraryStyleSheet("react-component-library:StatCard", "1.0.10",   statCardStyles);
  return (
    <article
      {...props}
      className={className}
