  ...
  return (
    <section style={shell}>
      <div style={{ display: "grid", gap: "var(--space-2xs)" }}>
        ...
        >
          {libraryStrings("ai.approval-prompt.human-in-the-loop", "Human in the loop")}
        </span>
...
export function Default() {
  ...
  return (
    <Showcase
      title={libraryStrings("ai.approval-prompt.title", "Consent should feel deliberate")}
      detail="The exact scope sits directly above the approval action, with target, consequence, and expiry in view."
    >
      <ApprovalPrompt {...common} density="comfortable" />
    </Showcase>
  );
}
...
export function Denied() {
  return <ApprovalPrompt {...common} status="denied" />;
}
export function Expired() {
  return <ApprovalPrompt {...common} status="expired" />;
}
...
export function Compact() {
  return (
    <ApprovalPrompt
      {...common}
      density="compact"
      title="Allow attachment inspection?"
      onApprove={async () => {}}
      onDeny={async () => {}}
    />
  );
