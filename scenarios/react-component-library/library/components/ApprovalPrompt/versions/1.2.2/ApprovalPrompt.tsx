...
/** @vrooliComponentSource react-component-library:ApprovalPrompt
 * @vrooliComponentSourceSlot ai.approval-prompt */
...
[data-rcl-approval-prompt][data-density="compact"] { gap: var(--space-sm); padding: var(--space-md); box-shadow: none; }
[data-rcl-approval-prompt][data-density="compact"] [data-rcl-approval-title] { font: var(--text-label); }
[data-rcl-approval-prompt][data-density="compact"] [data-rcl-approval-eyebrow],
[data-rcl-approval-prompt][data-density="compact"] [data-rcl-approval-facts],
[data-rcl-approval-prompt][data-density="compact"] [data-rcl-approval-consent-label] { display: none; }
[data-rcl-approval-prompt][data-density="compact"] [data-rcl-approval-consent] { padding: 0; border: 0; background: transparent; }
...
function statusSignal(status: ApprovalPromptStatus) {
  ...
  if (status === "denied" || status === "expired") return "idle" as const;
...
export const ApprovalPrompt = withClassName(function ApprovalPrompt({
  ...
  title = title ?? libraryStrings("ai.approval-prompt.review-this-action", "Review this action");
  ...
  const lastDecision = useRef<"approve" | "deny">("approve");
  const requestedStatus = status ?? localStatus;
  ...
  const expired =
    ...
    (requestedStatus !== "success" &&
      ...
      expiry !== undefined &&
      expiry <= now);
  const resolvedStatus = expired ? "expired" : requestedStatus;
  const terminal =
    resolvedStatus === "success" || resolvedStatus === "denied" || resolvedStatus === "expired";
  const busy = disabled || pending || resolvedStatus === "submitting" || terminal;
  const retryAction = onRetry ?? (lastDecision.current === "deny" ? onDeny : onApprove);
  const canApprove =
    !busy &&
    Boolean(
      resolvedStatus === "request-error" || resolvedStatus === "retry" ? retryAction : onApprove,
    ) &&
    resolvedStatus !== "permission-denied";

  const run = async (kind: "approve" | "deny" | "retry") => {
    if (busy || inFlight.current) return;
    if (expiry !== undefined && expiry <= Date.now()) {
      setNow(Date.now());
      return;
    }
    const accessRequest = resolvedStatus === "permission-denied";
    if (accessRequest && kind !== "retry") return;
    const callback = kind === "approve" ? onApprove : kind === "deny" ? onDeny : retryAction;
    if (!callback) return;
    if (kind !== "retry") lastDecision.current = kind;
    inFlight.current = true;
    setPending(true);
    if (!status && !accessRequest) setLocalStatus("submitting");
    try {
      await callback();
      if (!status && !accessRequest)
        setLocalStatus(lastDecision.current === "deny" ? "denied" : "success");
    } catch {
      if (!status && !accessRequest) setLocalStatus("request-error");
    } finally {
  ...
  const stateAlert =
    resolvedStatus === "denied" ? (
      <Alert
        tone="info"
        title={strings("ai.approval-prompt.declined", "Request declined")}
        description={strings(
          "ai.approval-prompt.declined-detail",
          "No permission was granted. The requested action will not continue.",
        )}
      />
    ) : resolvedStatus === "expired" ? (
      <Alert
        tone="warning"
        title={strings("ai.approval-prompt.expired", "Request expired")}
        description={strings(
          "ai.approval-prompt.expired-detail",
          "This request can no longer be answered. A new request is required.",
        )}
      />
    ) : resolvedStatus === "success" ? (
      <Alert
        data-testid="ai.approval-prompt"
        tone="success"
      ...
      <Alert
        tone="danger"
        title={strings(
          "ai.approval-prompt.approval-could-not-be-recorded",
      ...
      <Alert
        tone="warning"
        title={strings(
          "ai.approval-prompt.permission-was-not-granted",
  ...
  return (
    <section
      ...
      aria-busy={pending || resolvedStatus === "submitting"}
      className={className}
      style={style}
    >
      <StyleSheet libraryId="react-component-library:ApprovalPrompt" version="1.2.1" css={styles} />
      ...
      <div data-rcl-approval-consent>
        ...
        <p data-rcl-approval-consent-line>
          {strings("ai.approval-prompt.approve", "Approve")} <strong>{action}</strong>{" "}
          {strings("ai.approval-prompt.for", "for")} <strong>{target}</strong> within{" "}
          <strong>{scope}</strong>.
        </p>
      </div>

      {density === "compact" && expiryText ? (
        <span data-rcl-approval-expiry data-expired={expired || undefined}>
          {expiryText}
        </span>
      ) : null}
      {stateAlert}
      ...
      {!terminal ? (
        ...
        >
          <Button
            ...
            pending={pending || resolvedStatus === "submitting"}
            pendingLabel={pendingLabel}
            disabled={resolvedStatus === "permission-denied" ? busy || !onRetry : !canApprove}
            onClick={() => {
          ...
          {resolvedStatus !== "submitting" && resolvedStatus !== "permission-denied" ? (
            <Button variant="ghost" disabled={busy || !onDeny} onClick={() => void run("deny")}>
              {denyLabel}
            </Button>
          ) : null}
        </ButtonGroup>
