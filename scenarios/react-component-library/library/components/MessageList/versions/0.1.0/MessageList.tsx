 ...
 * @description A message collection with shared scroll anchoring, history and branch ports, retained content during recovery, and a return-to-latest indicator.
 * @version 0.1.0
 * @tags ["ai","conversation","collection"]
...
import { VirtualList, type VirtualListHandle, type VirtualListPosition } from "@vrooli/react-component-library/VirtualList/1";
import { AsyncBoundary, type AsyncBoundaryStatus } from "@vrooli/react-component-library/AsyncBoundary/1";
import { StyleSheet } from "@vrooli/react-component-library/StyleSheet/1";
import { withClassName } from "@vrooli/react-component-library/ClassMerge/1";
import { useStrings } from "@vrooli/react-component-library/useLocale/1";

export type MessageListState = "default" | "loading" | "refreshing" | "empty" | "partial" | "stale" | "request-error" | "offline" | "retry";
...
export interface MessageListBranch { id: string; label: string; disabled?: boolean }
...
const states: Record<MessageListState, AsyncBoundaryStatus> = {
  default: "success", loading: "pending", refreshing: "refreshing", empty: "idle",
  partial: "partial-error", stale: "stale", "request-error": "error", offline: "offline", retry: "refreshing",
};
const css = `
...
export const MessageList = withClassName(function MessageList({
  messages = none, state = "default", label, height = 480, emptyLabel, errorMessage,
  onRetry, onRetryMessage, hasEarlier = false, loadingEarlier = false, historyError,
  onLoadEarlier, earlierLabel, loadingEarlierLabel, latestLabel, newMessagesLabel,
  branches = noBranches, branchId, branchLabel, onBranchChange, initialScrollTop,
  onViewportChange, className, style,
}: MessageListProps) {
  ...
  loadingEarlierLabel = loadingEarlierLabel ?? strings("ai.message-list.loading-earlier", "Loading earlier messages…");
  latestLabel = latestLabel ?? strings("ai.message-list.latest", "Return to latest");
  newMessagesLabel = newMessagesLabel ?? strings("ai.message-list.new-messages", "Show {count} new messages");
  ...
  useEffect(() => {
    ...
    if (old.branchId !== branchId || atEnd.current) {
      setUnseen(0); setChanged(false); return;
    }
    ...
    const added = lastIndex >= 0 ? messages.slice(lastIndex + 1).filter(message => !oldKeys.has(message.id)).length : 0;
    if (added) setUnseen(count => count + added);
    const currentLast = lastIndex >= 0 ? messages[lastIndex] : undefined;
    if (last && currentLast && (last.content !== currentLast.content || last.revision !== currentLast.revision || last.state !== currentLast.state)) setChanged(true);
  }, [messages, branchId]);
  const positionChanged = (position: VirtualListPosition) => {
    atEnd.current = position.atEnd;
    if (position.atEnd) { setUnseen(0); setChanged(false); }
  ...
  return <section data-rcl-message-list data-state={state} aria-label={label} className={className} style={style}>
    ...
    {branches.length > 0 && <label data-rcl-message-list-toolbar>{branchLabel}
      <select value={branchId ?? ""} disabled={!onBranchChange} onChange={event => onBranchChange?.(event.currentTarget.value)}>
        ...
        {branches.map(branch => <option key={branch.id} value={branch.id} disabled={branch.disabled}>{branch.label}</option>)}
      </select>
    </label>}
    <AsyncBoundary status={boundaryStatus} preserveContent={messages.length > 0} detectOffline={false}
      offline={state === "offline"} error={errorMessage} retry={onRetry} loadingDelay={0}>
      {(hasEarlier || loadingEarlier || historyError) && <div data-rcl-message-list-history>
        <button type="button" data-message-list-action="history" disabled={loadingEarlier || !onLoadEarlier}
          aria-busy={loadingEarlier || undefined} onClick={onLoadEarlier}>{loadingEarlier ? loadingEarlierLabel : earlierLabel}</button>
        {historyError && <p data-rcl-message-list-history-error role="alert">{historyError}</p>}
      </div>}
      <VirtualList key={branchId ?? "$default"} items={messages} getItemKey={keyOf} label={label}
        height={height} empty={emptyLabel} estimateItemHeight={220} followEnd controllerRef={controller}
        initialScrollTop={initialScrollTop} onViewportChange={positionChanged}
        renderItem={(message, index) => {
          const { id, group, revision: _revision, ...props } = message;
          return <div role={group ? "group" : undefined} aria-label={group}>
            {group && messages[index - 1]?.group !== group && <h3 data-rcl-message-list-group>{group}</h3>}
            <Message {...props} onRetry={onRetryMessage ? () => onRetryMessage(id) : undefined}/>
          </div>;
        }}/>
    </AsyncBoundary>
    {(unseen > 0 || changed) && <div data-rcl-message-list-latest>
      <button type="button" data-message-list-action="latest" onClick={() => controller.current?.scrollToEnd()}>
