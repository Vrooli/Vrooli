  ...
  return (
    ...
    >
      <div style={{ display: "grid", gap: "var(--space-2xs)" }}>
        ...
        >
          {resolveStrings("data-display.list.activity-stream", "Activity stream")}
        </span>
...
export function Default() {
  return (
    <Showcase
      title={resolveStrings("data-display.list.title", "A list with hierarchy, not just rows")}
      detail="Each item has a primary action cue, supporting context, and a time marker that remains readable on narrow screens."
    >
      <List
        title={resolveStrings("data-display.list.title.recent-activity", "Recent activity")}
        description={resolveStrings(
          "data-display.list.description",
          "Changes across the design workspace",
        )}
        items={activity}
        label={resolveStrings("data-display.list.label", "Recent activity")}
      />
    </Showcase>
  );
}
...
export function LongContent() {
  return (
    <Showcase
      title={resolveStrings("data-display.list.title.long-labels-are-part-of-the-product", "Long labels are part of the product")}
    ...
    >
      <List
        ...
        label={resolveStrings("data-display.list.label.review-activity", "Review activity")}
      />
    </Showcase>
  );
}
...
export function Empty() {
  return (
    <Showcase
      title={resolveStrings("data-display.list.title.an-empty-list-still-explains-itself", "An empty list still explains itself")}
      detail="Empty space becomes a useful state with calm copy and a bounded footprint."
    >
      <List
        title={resolveStrings("data-display.list.title.recent-activity", "Recent activity")}
        empty="No activity has been recorded yet."
        label={resolveStrings("data-display.list.label.recent-activity", "Recent activity")}
      />
    </Showcase>
  );
}
