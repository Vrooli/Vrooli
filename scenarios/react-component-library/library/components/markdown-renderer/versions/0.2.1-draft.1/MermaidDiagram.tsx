...
export function MermaidDiagram({
  ...
  return (
    <section className="my-3 overflow-x-auto rounded border border-[var(--markdown-border,#334155)] bg-[var(--markdown-code-surface,#020617)]">
        ...
        <div className="flex gap-2">
          ...
          {onMermaidOpen && (
            <button
              ...
              className="text-[var(--markdown-link,#67e8f9)]"
            >
              {openLabel}
            </button>
          )}
      ...
      {showSource ? (
        <pre className="p-3 text-xs text-[var(--markdown-code-text,#e2e8f0)]">
          {code}
        </pre>
      ) : error ? (
        <>
          <p
            role="alert"
            className="p-3 text-xs text-[var(--markdown-error,#fca5a5)]"
          >
            {error}
          </p>
          <pre className="p-3 text-xs text-[var(--markdown-code-text,#e2e8f0)]">
        ...
        <p className="p-3 text-xs text-[var(--markdown-muted,#94a3b8)]">
          Rendering diagram…
        </p>
      ) : (
        <div
