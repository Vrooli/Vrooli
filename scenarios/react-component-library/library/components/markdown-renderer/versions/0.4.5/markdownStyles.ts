...
/**
 * The resizable table's own rules, keyed to the table rather than to the
 * renderer.
 *
 * 0.4.4 scoped these under `[data-rcl-markdown]` and shipped them in the
 * renderer's sheet. A consumer that renders its own markdown and uses only
 * `ResizableMarkdownTable` — which is what the export exists for — got neither
 * the scope nor the sheet: the handle was an unstyled, zero-size button with
 * no `touch-action`, so no column could be resized, by mouse or by touch.
 *
 * A touch screen has no hover to reveal a handle, so a coarse pointer gets a
 * wider target and a grip that is always drawn.
 */
export const resizableTableStyles = `
[data-rcl-md-table] { margin-block: var(--space-sm); max-inline-size: 100%; overflow-x: auto; }
[data-rcl-md-table] th { position: relative; }
[data-rcl-md-resize-handle] { position: absolute; z-index: 1; inset-block: 0; inset-inline-end: -6px; inline-size: 12px; margin: 0; padding: 0; border: 0; background: transparent; cursor: col-resize; touch-action: none; }
[data-rcl-md-resize-handle]::after { content: ""; position: absolute; inset-block: 20%; inset-inline-start: 5px; inline-size: 2px; border-radius: var(--radius-pill); background: transparent; transition: background-color var(--dur-quick) var(--ease-standard); }
[data-rcl-md-resize-handle]:hover::after, [data-rcl-md-resize-handle]:focus-visible::after { background: var(--color-accent); }
@media (pointer: coarse) {
  [data-rcl-md-resize-handle] { inset-inline-end: -12px; inline-size: 24px; }
  [data-rcl-md-resize-handle]::after { inset-inline-start: 11px; background: var(--color-border-strong); }
}
`;
