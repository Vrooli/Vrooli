...
import { StyleSheet, useLibraryStyleSheet } from "@vrooli/react-component-library/StyleSheet/1";
...
import { markdownStyles, resizableTableStyles } from "./markdownStyles";
 ...
 *
 * It carries its own stylesheet, so it resizes wherever it is rendered, not
 * only inside this package's renderer. See `resizableTableStyles`.
 */
export function ResizableMarkdownTable({
  children,
  minColumnWidth = 72,
}: ResizableMarkdownTableProps) {
  useLibraryStyleSheet(
    "react-component-library:ResizableMarkdownTable",
    "0.4.5",
  ...
  const enhanceHeader = (part: ReactNode) => {
    ...
    const enhancedRow = cloneElement(
      ...
      cells.map((cell, index) => {
        if (
          !isValidElement<{ children?: ReactNode; "aria-label"?: string }>(cell) ||
  ...
  return (
    <div className="rcl-md__table-scroll" data-rcl-md-table="">
      <table
        ref={tableRef}
