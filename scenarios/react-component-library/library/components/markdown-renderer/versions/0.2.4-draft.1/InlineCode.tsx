...
export function InlineCode({
  ...
  const tokenClass =
    "rounded bg-slate-800 px-1 py-0.5 font-mono text-slate-200";
  ...
  if (resolution)
    return (
      <a
        ...
        className={`${tokenClass} text-cyan-200 underline`}
      >
        {text}
      </a>
    );
  if (isFile)
    return (
      <button
        ...
        className={`${tokenClass} text-cyan-200 hover:bg-slate-700`}
      >
        {text}
      </button>
    );
  return (
    <span className="group relative inline-flex items-center">
      ...
      <button
        ...
        className="ml-1 hidden rounded px-1 text-[10px] text-slate-400 group-hover:inline hover:bg-slate-800"
      >
        {copied ? "Copied" : "Copy"}
      </button>
    </span>
