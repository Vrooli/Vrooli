...
/**
 * Tone icons are drawn rather than typed. The previous version rendered the characters
 * "i", "✓" and "!" inside a bordered circle, which sat off its optical centre at every
 * size and gave warning and error the same glyph — the two tones a reader most needs to
 * tell apart before reading a word.
 */
...
function ToastItem({
  ...
  return (
    <Presence present={present} duration="quick" onExitComplete={onExitComplete}>
      <Surface
        elevation="floating"
...
export const Toast = withClassName(function Toast({ className, label, style }: ToastProps) {
  ...
  return (
    <>
      <StyleSheet name="toast-1-0-11" css={styles} />
      <div
        className={className}
        style={style}
      ...
      >
        {rendered.map((item) => (
          <ToastItem
            ...
            onExitComplete={() => forget(item.record.id)}
          />
        ))}
      </div>
    </>
