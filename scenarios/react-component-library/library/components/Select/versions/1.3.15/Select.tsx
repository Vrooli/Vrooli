...
[data-rcl-select-listbox] { box-sizing: border-box; display: grid; gap: var(--space-3xs); inline-size: var(--rcl-select-popover-width, 13rem); min-inline-size: var(--rcl-select-popover-width, 13rem); max-inline-size: calc(100vw - (var(--space-lg) * 2)); max-block-size: min(22rem, calc(100vh - (var(--space-lg) * 2))); overflow-y: auto; overscroll-behavior: contain; scrollbar-gutter: stable; padding: var(--space-2xs); }
...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(function Select(
  ...
  const buttonProps = triggerProps as unknown as ButtonHTMLAttributes<HTMLButtonElement>;
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLButtonElement>) => {
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      if (match >= 0) {
        const matchOption = options[match];
        if (!open && matchOption) choose(matchOption);
  ...
  return (
    <>
      <StyleSheet libraryId="react-component-library:Select" version="1.0.0" css={styleSheet} />
      <Popover open={open} onOpenChange={setOpen} placement="bottom-start" responsive="none">
        <span data-rcl-select-field>
          ...
          <PopoverParts.Trigger asChild aria-haspopup="listbox">
            <button
              ...
              ref={triggerRef}
              type="button"
              data-testid={testId ?? "forms.select"}
        ...
        <PopoverParts.Content
          ...
          style={
            {
              "--rcl-select-popover-width": triggerWidth ? `${triggerWidth}px` : undefined,
            } as CSSProperties
          }
        >
