...
export function TargetSwitcher({
  ...
  useEffect(() => {
    if (!open) return;
    const index = Math.max(0, options.findIndex((option) => option.id === selected?.id));
  ...
  return (
    <div data-rcl-target-switcher className={className} data-testid={testId}>
      <StyleSheet libraryId="react-component-library:TargetSwitcher" version="1.0.0" css={styleSheet} />
      <Popover open={open} onOpenChange={setOpen} placement="bottom-end" responsive="none">
        <PopoverParts.Trigger asChild>
          <button type="button" className={triggerClassName} data-rcl-target-switcher-trigger aria-label={label} aria-haspopup="dialog" aria-expanded={open} data-value={selected.id}>
            <span data-rcl-target-switcher-dot data-tone={selected.statusTone ?? "neutral"} aria-hidden="true" />
        ...
        <PopoverParts.Content role="dialog" aria-label={label} initialFocus="none">
          <div data-rcl-target-switcher-menu>
            <div data-rcl-target-switcher-summary>
              ...
              {(selected.description ?? description) && <p data-rcl-target-switcher-summary-description>{selected.description ?? description}</p>}
            </div>
            {(statusValue ?? selected.status) && <div data-rcl-target-switcher-status><span>{statusLabel}</span><strong data-rcl-target-switcher-status-value><span data-rcl-target-switcher-dot data-tone={statusTone ?? selected.statusTone ?? "neutral"} aria-hidden="true" />{statusValue ?? selected.status}</strong></div>}
            <div data-rcl-target-switcher-options role="listbox" aria-label={label} onKeyDown={handleOptionKeyDown}>
              {options.map((option, index) => <button key={option.id} ref={(element) => { optionRefs.current[index] = element; }} type="button" role="option" aria-selected={option.id === selected.id} aria-disabled={option.disabled || undefined} tabIndex={index === activeIndex ? 0 : -1} data-active={index === activeIndex} data-rcl-target-switcher-option onFocus={() => setActiveIndex(index)} onClick={() => choose(option)}><span data-rcl-target-switcher-dot data-tone={option.statusTone ?? "neutral"} aria-hidden="true" /><span data-rcl-target-switcher-option-body><span data-rcl-target-switcher-option-name>{option.label}</span>{(option.meta ?? option.status) && <span data-rcl-target-switcher-option-meta>{[option.meta, option.status].filter(Boolean).join(" · ")}</span>}</span>{option.id === selected.id ? <span data-rcl-target-switcher-check aria-hidden="true">✓</span> : option.badge && <span data-rcl-target-switcher-option-badge>{option.badge}</span>}</button>)}
            </div>
            {onAddTarget && addTargetLabel && <button type="button" data-rcl-target-switcher-add onClick={() => { setOpen(false); onAddTarget(); }}>{addTargetLabel}</button>}
          </div>
