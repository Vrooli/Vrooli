 ...
 * @description A native searchable selection field with local filtering, creation, keyboard navigation, and an async-options composition path.
 * @version 1.0.3
...
const styles = `
  [data-rcl-combobox] { position: relative; display: grid; gap: var(--space-2xs, .5rem); min-inline-size: 0; color: var(--color-foreground, #0f172a); }
  [data-rcl-combobox-label] { display: grid; gap: var(--space-3xs, .25rem); font: var(--text-label, 650 .8125rem/1.25rem system-ui, sans-serif); }
  [data-rcl-combobox-label] small { color: var(--color-muted-foreground, #64748b); font: var(--text-caption, 400 .75rem/1rem system-ui, sans-serif); }
  ...
  [data-rcl-combobox-chevron] { position: absolute; inset-inline-end: .875rem; inset-block-start: 50%; pointer-events: none; color: var(--color-muted-foreground, #64748b); transform: translateY(-50%); transition: transform var(--dur-quick, 160ms) var(--ease-standard, ease); }
  [data-rcl-combobox-input][aria-expanded="true"] + [data-rcl-combobox-chevron] { transform: translateY(-50%) rotate(180deg); }
  [data-rcl-combobox-panel] { position: absolute; z-index: 10; inset-inline: 0; inset-block-start: calc(100% - var(--space-2xs, .5rem)); overflow: hidden; border: 1px solid var(--color-border, #cbd5e1); border-block-start: 0; border-radius: 0 0 var(--radius-control, .625rem) var(--radius-control, .625rem); background: var(--color-surface-raised, #fff); box-shadow: var(--elev-overlay, 0 16px 36px rgb(15 23 42 / .16)); }
  [data-rcl-combobox][data-open="true"] [data-rcl-combobox-status] { block-size: 0; min-block-size: 0; overflow: hidden; }
  [data-rcl-combobox-list] { display: grid; max-block-size: min(20rem, 42vh); overflow: auto; padding: var(--space-2xs, .5rem); overscroll-behavior: contain; }
  ...
  [data-rcl-combobox-state], [data-rcl-combobox-status] { color: var(--color-muted-foreground, #64748b); font: var(--text-body, 400 .875rem/1.35rem system-ui, sans-serif); }
