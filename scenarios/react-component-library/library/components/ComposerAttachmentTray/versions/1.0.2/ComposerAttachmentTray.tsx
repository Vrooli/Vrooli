 ...
 * @description The attachment manager for pending uploads, files, links, images, and contextual records, with progress, errors, cancellation, reordering, and removal per item.
 * @version 1.0.2
 * @tags []
...
} from "@vrooli/react-component-library/AttachmentPreview/1";
import { Cluster } from "@vrooli/react-component-library/Cluster/1";
import { Button } from "@vrooli/react-component-library/Button/2";
import { StyleSheet } from "@vrooli/react-component-library/StyleSheet/1";

export interface ComposerAttachment
  extends Omit<AttachmentPreviewProps, "onRemove" | "onRetry" | "onCancel"> {
  id: string;
}
...
[data-rcl-composer-attachment] { flex: 1 1 16rem; min-inline-size: 0; max-inline-size: 100%; padding: var(--space-2xs); border: var(--border-hairline) solid var(--color-border); border-radius: var(--radius-panel); background: var(--color-surface); transition: border-color var(--dur-quick) var(--ease-standard), background-color var(--dur-quick) var(--ease-standard); }
...
export function ComposerAttachmentTray({
  ...
  const move = (index: number, offset: number) => {
    ...
    ids[index + offset] = item.id;
    onReorder(ids);
    setAnnouncement(`${item.name} moved to position ${index + offset + 1} of ${items.length}.`);
  };
  const remove = (item: ComposerAttachment, index: number) => {
    ...
    requestAnimationFrame(() => {
      ...
      (
        cards?.[Math.min(index, (cards?.length ?? 0) - 1)]?.querySelector<HTMLElement>(
          "button:not(:disabled)",
        ) ?? root.current
      )?.focus();
    });
    ...
    >
      ...
      <Cluster gap="2xs">
        {items.map(({ id, ...item }, index) => (
          <div key={id} data-rcl-composer-attachment>
            <AttachmentPreview
              {...item}
            ...
            {onReorder && items.length > 1 && (
              <div data-rcl-composer-attachment-order>
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
