...
const styles = `
  ...
  [data-rcl-virtual-list-viewport] { position: relative; overflow: auto; overscroll-behavior: contain; scrollbar-color: var(--color-border-strong, color-mix(in srgb, var(--color-border) 72%, var(--color-foreground))) transparent; }
...
export const VirtualList = withClassName(function VirtualList<T>({
  ...
  label = label ?? libraryStrings("data-display.virtual-list.virtual-list", "Virtual list");
  ...
  const offsets = useMemo(() => {
    ...
    items.forEach((_, index) => {
      result.push((result[index] ?? 0) + (measurements[index] ?? estimateItemHeight));
    });
    return result;
  ...
  const lastVisible = items.length
    ? Math.min(items.length - 1, lowerBound(offsets, scrollTop + viewportHeight))
    : 0;
        ...
        {items.length === 0 ? (
          ...
          <>
            ...
            <div data-rcl-virtual-list-status role="status" aria-live="polite">
              Showing {start + 1}–{Math.min(items.length, end + 1)} of {items.length} items
            </div>
          </>
        )}
      </section>
