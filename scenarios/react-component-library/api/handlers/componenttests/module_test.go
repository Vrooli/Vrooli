...
func TestModuleRunsAndListsDurableContractReport(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(root, "components", "Button", "versions", "1.0.0", "examples.json"), []byte(`{"examples":[{"name":"idle","expect":[{"kind":"role","role":"button","name":"Button"}]}]}`), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(root, "components", "Button", "versions", "1.0.0", "test-contract.json"), []byte(`{"schemaVersion":"1","examples":[{"example":"idle","assertions":[{"kind":"role","role":"button","name":"Button"}]}]}`), 0o644))
