...
func parseRampFile(raw string) (rampFile, error) {
	header := raw[:min(len(raw), 512)]
	if strings.Contains(header, "Generated by ") && strings.Contains(header, "DO NOT EDIT") {
		return rampFile{}, fmt.Errorf("token ramp is generated; update its declared token authority and run the generator named in the file header")
	}
