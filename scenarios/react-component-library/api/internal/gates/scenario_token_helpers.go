...
func unresolvedScenarioImportFinding(root, scenario string, specifier components.LibraryPackageSpecifier, reason string) Finding {
	...
	return Finding{
		Code: "catalog.scenario_token_import_unresolved", AssetID: "__corpus__.scenario-token-requirements",
...
func scanScenarioLibraryImports(sourceRoot string) ([]components.LibraryPackageSpecifier, map[string]bool, error) {
	...
	err := librarywalk.Walk(sourceRoot, func(path string, entry os.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		if entry.IsDir() {
