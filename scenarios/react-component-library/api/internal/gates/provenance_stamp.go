...
func ValidateProvenanceStamp(scope Scope) (Result, error) {
	...
	for _, path := range paths {
		...
		if pathWithin(filepath.Join(root, "scenarios", "react-component-library", "library"), path) {
			...
			if !valid && owner != "" {
				valid = strings.EqualFold(strings.TrimPrefix(owner, "react-component-library:"), key) || strings.Contains(strings.ToLower(owner), strings.ToLower(key)) || compactIdentity(owner) == compactIdentity(stamp) || compactIdentity(owner) == compactIdentity(key)
			}
			if !valid {
