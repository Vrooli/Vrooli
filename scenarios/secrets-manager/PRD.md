...
- Preferred approach: React + TypeScript + Vite for the operator surface; Go for the API and CLI proxy; Postgres for persisted metadata; and the managed Vault resource for secret validation.
...
secrets-manager vault status        # All secrets with validation status
