...
func (h *handlers) query(ctx cliapp.RunContext) error {
	...
	for i, r := range resp.Msg.Results {
		full := strings.TrimSpace(strings.Join([]string{r.GetScenario(), r.GetSlot(), r.GetDisplayName()}, " "))
		desc := truncate(r.GetDescription(), 80)
		provenance := ""
		if p := r.GetProvenance(); p != nil {
			provenance = " provenance=" + p.GetProvenance().String()
		}
		results = append(results, fmt.Sprintf("%d. %s — %s [score=%.3f file=%s%s]",
			i+1, full, desc, r.GetScore(), r.GetFilePath(), provenance))
	}
	if len(results) == 0 {
