...
// Ported from app-monitor's rules engine — it is the conformance harness that
// keeps each interop rule honest: every rule must declare a passing and a
// failing <test-case> in its docstring, and the harness executes them against a
// temp scenario built from [filepath] markers.
// =============================================================================
...
func TestAllDocCases(t *testing.T) {
	...
	for _, r := range allRules {
		...
		if src == "" {
			continue // meta-test catches missing doc cases
		}
...
func findRuleSource(t *testing.T, ruleID string) string {
	...
	for _, r := range allRules {
		...
		for _, fsys := range uiinterop.EmbedFSes() {
			var found string
...
// TestAllRulesHaveDocCases verifies every rule has at least one pass and one fail
// test case in its docstring.
