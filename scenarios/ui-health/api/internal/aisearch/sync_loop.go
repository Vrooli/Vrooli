...
func (s *SyncLoop) Start(ctx context.Context) {
	if s == nil || s.Reconciler == nil {
		log.Printf("[ui-health/aisearch] sync_loop disabled (no reconciler)")
		return
	}
	if s.Disabled {
		log.Printf("[ui-health/aisearch] sync_loop disabled via %s", EnvSyncDisabled)
		return
	}
	if s.Interval <= 0 {
		log.Printf("[ui-health/aisearch] sync_loop disabled (non-positive interval)")
		return
	}

	log.Printf("[ui-health/aisearch] sync_loop enabled (interval=%s)", s.Interval)
	...
	for {
		select {
		case <-ctx.Done():
			log.Printf("[ui-health/aisearch] sync_loop stopping: %v", ctx.Err())
			return
		case <-t.C:
			s.tick(ctx)
		}
...
func (s *SyncLoop) tick(ctx context.Context) {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("[ui-health/aisearch] sync_loop tick panic: %v\n%s", r, debug.Stack())
		}
	}()
	plan, apply, err := s.RunOnce(ctx)
	if err != nil {
		log.Printf("[ui-health/aisearch] sync_loop tick failed: %v", err)
		return
	}
	if plan != nil && apply != nil && (plan.HasWork() || len(apply.Errors) > 0) {
		log.Printf("[ui-health/aisearch] sync_loop tick: plan=%d collections, errors=%d",
