	...
	endpoints := []module.EndpointDescriptor{
		{
			...
			CLIMapping: &module.CLIMapping{
				Command: "ui-health lonely subcommand",
			},
		},
	}
...
func TestCrossCheck_PassesWhenRegistered(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "x", CLIMapping: &module.CLIMapping{Command: "ui-health x"}},
		{ID: "y_alias", CLIMapping: &module.CLIMapping{Command: "ui-health notes ll"}},
...
func TestBuildCLICommands_ResolvesEndpointIDs(t *testing.T) {
	endpoints := []module.EndpointDescriptor{
		{ID: "health", CLIMapping: &module.CLIMapping{Command: "ui-health status"}},
		{ID: "notes_list", CLIMapping: &module.CLIMapping{Command: "ui-health notes list"}},
	}
	registered := []registeredCommand{
		{Name: "configure"},
		{Name: "notes list"},
...
// normalisation step: the endpoint's "ui-health notes list"
...
func TestStripBinaryPrefix(t *testing.T) {
	cases := []struct {
		...
		{in: "ui-health status", want: "status"},
		{in: "ui-health notes list", want: "notes list"},
		{in: "already-stripped", want: "already-stripped"},
		{in: "ui-health", want: "ui-health"}, // no trailing space → preserved
	}
