...
The `health` proto in `packages/proto/schemas/ui-health/v1/health/`
...
| **Production wiring** | `main.go` and per-domain sqlite tests pass a real `*sql.DB`; each domain helper composes the scenario-specific providers it needs (e.g. `localdb.SystemSchema` plus any per-domain `Schema()` re-exports) before applying them via `apidb.EnsureSchemas`. |
...
| **Production wiring** | `cli/manifest_embed.go` embeds the manifest bytes; `cli/app.go` passes them to `domains.SubcommandGroups(core, manifest)`; each domain's `Register(core, manifest)` calls `cliapp.LoadFromManifest` with its group name and a bindings map keyed by `Service.Method`. Multipart REST endpoints (if a scenario adds them) are appended outside the manifest path because cli-manifest/v1 only models `binding.kind=connect-rpc`. |
| **Test fake** | `cli-core/cliapp::RequireProtoServiceCoverage(t, manifest, fd, serviceName)` asserts every RPC on the bound proto service has either a binding or an entry in the manifest's `omitted` array — each `cli/domains/<domain>/*_manifest_test.go` exercises the relevant proto file. `cliapp.ParseManifest` covers structural validation in isolation. |
...
| **Production wiring** | `main.go` calls each domain's `Module(...)` (`healthH.Module(...)`, `reindexH.Module(...)`, `searchH.Module(...)`, `validationH.Module(...)`) and passes the slice to `server.New(deps, modules...)`. The server iterates `m.Mount(s.router)` after registering the logging middleware. |
| **Test fake** | A literal `module.Module{Name: "stub", Mount: func(r){...}}` in `internal/server/server_test.go` proves the iteration; per-domain `module_test.go` files (e.g. `handlers/health/module_test.go`) exercise the real constructors against in-memory fixtures. |
...
`internal/services/`. The canonical layout (mirrored from the
react-vite template's reference notes module) is:

```
internal/<domain>/
  types.go         # Domain types, CreateInput, sentinel errors
...
Note: this scenario currently ships transport-only domains (health,
reindex, search, validation) so the live tree does not contain a
worked example of the full repository/service split. The layout above
is the contract any new persistence-backed domain should follow.
...
### UI ↔ API per-domain endpoints (rebuilt domains)

| | |
|---|---|
| **Seam** | UI ↔ API per-domain endpoints for the rebuilt UI shell: `api/inventory.ts`, `api/reindex.ts`, `api/validation.ts`, `api/search.ts`. |
| **Module** | Each module exports a `createClient(<Service>, transport)` client plus plain-TS converter wrappers (e.g. `scanScenario`, `reindex`, `reindexStatus`, `reindexCancel`, `validateScenario`, `searchSurfaces`, `searchStatus`) so callers depend on a domain-local shape (`InventoryScan`, `ReindexStatus`, `SearchHit`, etc.) instead of generated proto messages. Pure helpers (`filterHits`, `filterSurfaces`, `countSurfacesByKind`, `encodeSurfaceId`/`decodeSurfaceId`, `reindexStateFromString`, `isTerminal`) live next to the client for testability. |
| **Production wiring** | Feature hooks (`useSearch`, `useInventory`, `useValidateScenario`, `useTrackedJobs` / `useJobStatus` / `useTriggerReindex` / `useCancelJob`) wrap each call in TanStack Query, with URL-sync, debounce, polling, and localStorage tracking applied at the hook layer — pages stay declarative. |
| **Test fake** | Inline `vi.mock("../../api/<domain>", async (importOriginal) => ...)` per page test; for FromProto coverage, `vi.spyOn(<domain>Client, "<rpc>")` in `api/*.client.test.ts` returns synthesised proto messages. Page-level tests work against the plain-TS converted shapes — never the proto types. |
| **Why it exists** | Keeps every page-level component decoupled from generated Connect message shapes (so a proto rename is a one-file change), centralises the polling / debounce / localStorage / URL-sync logic at the hook layer (one composition point per domain), and gives BAS automation a stable selector registry to drive the same flows. |

### Dashboard activity feed (cross-feature aggregation)

| | |
|---|---|
| **Seam** | Merge recent validation runs + tracked reindex jobs into a single time-sorted feed for the dashboard. |
| **Module** | `ui/src/features/dashboard/useActivityFeed.ts` exports `useActivityFeed()` (default 12 items) and the pure `mergeActivity(runs, jobs, limit)` aggregator. |
| **Production wiring** | `pages/DashboardPage.tsx` calls `useActivityFeed()`; the hook composes `useRecentRuns` (validation) and `useTrackedJobs` (reindex) and exposes a single `ActivityItem` union. |
| **Test fake** | Pure-function tests against `mergeActivity(...)` in `useActivityFeed.test.ts`; page-level tests seed localStorage with the two underlying caches. |
| **Why it exists** | The dashboard is the only surface that wants both feeds at once. Centralising the merge keeps page-level rendering branch-free and lets each underlying feed evolve independently without re-touching the dashboard. |
