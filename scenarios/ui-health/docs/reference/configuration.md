...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/ui-health.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `ui-health` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/ui-health/config.json`
3. `~/.vrooli/config/ui-health/config.json`
4. `~/.config/vrooli/ui-health/config.json`
...
4. Vrooli lifecycle port detection (`vrooli scenario port ui-health API_PORT`)
