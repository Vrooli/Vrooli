...
`~/.vrooli/bin/ui-health`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start ui-health` if the API is unreachable |
...
```bash
ui-health status
ui-health status --json
```

### `ui-health configure <key> <value>`

Persist a setting to the per-user CLI config file (location resolved
per [`configuration.md`](configuration.md#cli-config-file)).

```bash
ui-health configure api_base http://localhost:15001/api/v1
ui-health configure token <token>
```
...
```bash
ui-health notes list
ui-health notes list --json
```

### `ui-health notes create --title <title> [--body <body>]`
...
```bash
ui-health notes create --title "First note" --body "Hello world"
```
...
```bash
ui-health notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start ui-health`" is good.
