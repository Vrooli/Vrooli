...
```bash
make status
vrooli scenario port ui-health UI_PORT
```
...
ui-health status --auto-start

# Or override the API base for this invocation
ui-health status --api-base "http://localhost:$(vrooli scenario port ui-health API_PORT)/api/v1"
```
...
The generated package paths follow `packages/proto/gen/go/ui-health/v1/<domain>/`.
