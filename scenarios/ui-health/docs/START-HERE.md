...
`vrooli scenario orient ui-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `vrooli scenario orient ui-health
...
```bash
prd-control-tower prd generate ui-health \
...
```bash
prd-control-tower prd validate ui-health --json
```
...
```bash
prd-control-tower requirements generate ui-health \
...
```bash
prd-control-tower requirements validate ui-health --json
```
