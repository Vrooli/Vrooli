...
`template-manager orient ui-health`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient ui-health
...
```bash
cat > /tmp/prd_context_ui-health.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start  # (was: prd generate) ui-health \
  --context-file /tmp/prd_context_ui-health.md \
  --publish \
  --json
```
...
```bash
vrooli scenario requirements validate ui-health --json
```
...
```bash
cat > /tmp/requirements_context_ui-health.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
business-health wizard apply  # (was: requirements generate) ui-health \
  --context-file /tmp/requirements_context_ui-health.md \
  --json
```
...
- [ ] **Start in proto.** Author `packages/proto/schemas/ui-health/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest ui-health <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> ui-health
...
react-component-library workflows promotion-readiness <asset-id> \
  --origin-scenario ui-health
```

- [ ] Adopt the published version back, then delete the local original:

```bash
react-component-library adoptions apply <component-id> ui-health <adopted-path>
```
...
- Wire contracts belong in
  `packages/proto/schemas/ui-health/v1/<domain>/`.
