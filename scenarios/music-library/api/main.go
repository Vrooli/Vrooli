...
import (
	...
	"music-library/internal/modules"
	...
	capsH "music-library/handlers/capabilities"
	healthH "music-library/handlers/health"
	notesH "music-library/handlers/notes" // EXAMPLE-DOMAIN:notes
)
...
func sqliteDSN() (string, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("music-library")
	if err != nil {
		return "", fmt.Errorf("resolve music-library storage namespace: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("resolve music-library db path: %w", err)
	}
	return sqliteFileDSN(path)
}
...
func scenarioStorageRoots() (storage.Paths, error) {
	...
	scenarioID, err := storage.ScenarioNamespace("music-library")
	if err != nil {
		return storage.Paths{}, fmt.Errorf("resolve music-library storage namespace: %w", err)
	}
...
func main() {
	...
	if preflight.Run(preflight.Config{ScenarioName: "music-library"}) {
		return
	}
	...
	srv := server.New(
		...
		healthH.Module(db, "music-library-api", "1.0.0"),
