...
`template-manager orient music-library`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient music-library
...
```bash
cat > /tmp/prd_context_music-library.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start music-library --interactive
# …answer the interview, then:
business-health wizard preview music-library
business-health wizard apply music-library
```
...
```bash
vrooli scenario requirements validate music-library --json
```
...
```bash
cat > /tmp/requirements_context_music-library.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate music-library --json
business-health fix preview music-library
business-health fix apply music-library
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate music-library --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate music-library`, the notes page spec
...
validate music-library --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/music-library/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest music-library <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> music-library
...
- Wire contracts belong in
  `packages/proto/schemas/music-library/v1/<domain>/`.
...
1. Add proto messages and a service under
   `packages/proto/schemas/music-library/v1/<domain>/`, then run
