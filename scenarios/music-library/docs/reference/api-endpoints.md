...
envelope (`packages/proto/schemas/music-library/v1/shared/errors.proto`):
...
| **CLI** | `music-library status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/music-library/v1/shared/health.proto`
...
`POST /vrooli.music_library.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.music_library.v1.notes.NotesService/ListNotes`
...
| **CLI** | `music-library notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.music_library.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.music_library.v1.notes.NotesService/CreateNote`
...
| **CLI** | `music-library notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.music_library.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.music_library.v1.notes.NotesService/GetNote`
...
| **CLI** | `music-library notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.music_library.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `music-library notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/music-library/v1/notes/notes.proto`.
