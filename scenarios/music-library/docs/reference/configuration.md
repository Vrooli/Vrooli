...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/music-library.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `music-library` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/music-library/config.json`
3. `~/.vrooli/config/music-library/config.json`
4. `~/.config/vrooli/music-library/config.json`
...
```bash
music-library configure api_base http://localhost:15001/api/v1
music-library configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port music-library API_PORT`)
