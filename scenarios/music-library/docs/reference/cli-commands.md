...
`~/.vrooli/bin/music-library`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start music-library` if the API is unreachable |
...
```bash
music-library status
music-library status --json
```

### `music-library configure <key> <value>`
...
```bash
music-library configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`music-library <domain> <verb>`). Every command calls a single API
...
```bash
music-library notes list
music-library notes list --json
```

#### `music-library notes create --title <title> [--body <body>]`
...
```bash
music-library notes create --title "First note" --body "Hello world"
```
...
```bash
music-library notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start music-library`" is good.
