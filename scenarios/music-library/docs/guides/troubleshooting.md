...
```bash
vrooli scenario status music-library
make stop
vrooli scenario start music-library --clean-stale
```
...
```bash
make status
vrooli scenario port music-library UI_PORT
```
...
music-library status --auto-start

# Or override the API base for this invocation
music-library status --api-base "http://localhost:$(vrooli scenario port music-library API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/music-library/`.
...
The default route is `${SCENARIO_DATA_DIR}/music-library.db` via
...
```bash
fuser "${SCENARIO_DATA_DIR}/music-library.db"
```
...
`packages/proto/gen/go/music-library/v1/`, TypeScript under
`packages/proto/gen/typescript/music-library/v1/`, and Python under
`packages/proto/gen/python/music_library/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/music-library/v1/<domain>/`.
