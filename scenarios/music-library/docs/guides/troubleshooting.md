...
```bash
export SQLITE_PATH=/tmp/music-library.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/music-library.db}")"
```
...
`packages/proto/gen/typescript/js/music-library/v1/`, and Python under
