...
music-library <domain> <command>   # e.g. list/create commands for your domain
```

Or directly via HTTP:

```bash
API_PORT=$(vrooli scenario port music-library API_PORT)
curl -s "http://localhost:${API_PORT}/health"
# Proto-typed calls hit /vrooli.music_library.v1.<domain>.<Service>/<Method>
```
...
```bash
music-library notes list
music-library notes create --title "First note" --body "Hello"
```

```bash
API_PORT=$(vrooli scenario port music-library API_PORT)
curl -s -X POST "http://localhost:${API_PORT}/vrooli.music_library.v1.notes.NotesService/ListNotes" \
