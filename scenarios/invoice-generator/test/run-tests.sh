#!/bin/bash
# Main test orchestrator for invoice-generator scenario
...
SCENARIO_NAME="invoice-generator"
    ...
    case $test_type in
        go)
            if [ -d "api" ] && [ -f "api/go.mod" ]; then
                ...
                cd api && go test ./... -v || return 1
                cd ..
            else
                log::warning "⚠️  No Go module found"
            fi
            ;;
        bats)
            if [ -d "cli" ] && command -v bats >/dev/null 2>&1; then
                log::info "   Running BATS tests..."
                cd cli && bats *.bats || return 1
                cd ..
            else
                log::warning "⚠️  No BATS tests found or bats not installed"
            fi
            ;;
        *)
            log::warning "⚠️  Test type '$test_type' not implemented"
            ;;
    esac
}
...
main() {
    ...
    log::header "🧪 Invoice Generator Comprehensive Test Suite"
    ...
    for item in "${SELECTED_PHASES[@]}"; do
        ...
        elif [[ "$item" == "go" ]] || [[ "$item" == "bats" ]]; then
            if ! run_test_type "$item"; then
                item_result=1
            fi
        else
            log::warning "⚠️  Unknown phase/type: $item"
        fi
    ...
    if [ $execution_result -eq 0 ] && [ $failed_phases -eq 0 ]; then
        ...
        log::success "✅ invoice-generator testing infrastructure is working correctly"
        ...
        echo "   • Ensure invoice-generator scenario is running: vrooli scenario start invoice-generator"
        echo "   • Check dependencies: vrooli resource start postgres ollama browserless"
