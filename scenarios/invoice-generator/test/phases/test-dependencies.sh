...
if [ -f "$TESTING_PHASE_SCENARIO_DIR/api/go.mod" ]; then
    ...
    testing::phase::add_warning "⚠️  No go.mod found in api directory"
fi

echo "🔍 Checking for required database resources..."
if command -v resource-postgres >/dev/null 2>&1; then
    if resource-postgres status >/dev/null 2>&1; then
        log::success "✅ PostgreSQL resource is available"
    else
        testing::phase::add_error "❌ PostgreSQL resource is not running"
    fi
else
    testing::phase::add_error "❌ PostgreSQL resource CLI not found"
fi
