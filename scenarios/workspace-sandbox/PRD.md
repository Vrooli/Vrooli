...
- [x] OT-P1-001 | Hunk-Level Approval | UI and API support for selecting individual hunks/sections to approve, not just full files
...
- [x] OT-P1-003 | GC/Prune Operations | Garbage collection by age, max-total-size, inactive TTL policies
- [x] OT-P1-004 | Audit Trail Metadata | Immutable metadata per sandbox: id, scope, creator/owner, timestamps, driver, status, size, changed files
- [x] OT-P1-005 | Pre-commit Validation Hooks | Configurable hooks to run tests/lint before final commit of approved changes
- [x] OT-P1-006 | Commit Attribution Policy | Configurable commit message templates and author attribution for applied patches
- [x] OT-P1-007 | Safe-Git Wrapper | PATH-injected wrapper that explains correct workflow for blocked git commands during agent runs
- [x] OT-P1-008 | Metrics/Observability | Active sandbox count, total disk usage, per-sandbox size distribution, create latency, cleanup success rates
- [x] OT-P1-009 | CLI Interface | Full-featured CLI for create, list, inspect, diff, approve, reject, delete, gc operations

### 🟢 P2 – Future / expansion
- [x] OT-P2-001 | Diff Viewer UI | Web UI with file-level toggle approval, hunk selection, sandbox metadata display, one-click approve/reject
- [x] OT-P2-002 | Conflict Detection | Detect when canonical repo has changed since sandbox creation, surface patch application failures clearly
- [x] OT-P2-003 | Retry/Rebase Workflow | Support re-running sandbox or regenerating diff when conflicts occur
- [x] OT-P2-004 | Cross-Platform Driver Interface | Pluggable SandboxDriver interface with clear fallback strategy for non-Linux environments
- [x] OT-P2-005 | Size Accounting | Real-time tracking of sandbox disk usage with configurable limits per-sandbox and total
- [x] OT-P2-006 | Policy Guardrails | Configurable max lifetime/TTL, max size, require explicit human approval, allow sandbox reuse
- [x] OT-P2-007 | Structured Event Logging | Emit structured logs for create/mount/run/diff/approve/reject/delete/gc with failure details
