...
## Scenario adaptation — Prose Studio

The Operational Console language is kept intact, tokens and all. It fits because
this scenario's primary user is an operator making a **comparison** decision
...
1. **No candidate ever renders a quality score, and no candidate list carries a
   ...
   rule, because layout is how ranking gets smuggled back in. A single number
   invites ranking; a feature vector invites reading. Candidate cards therefore
   show a small row of measurement chips — reading grade, burstiness sparkline,
   lexicon flags, axis cell — never a headline figure. Ordering is by generation
   order or axis cell, and the operator may re-sort explicitly.

2. **Density is tuned for side-by-side comparison of running text.** The
   Variation Board is the signature surface and it breaks the usual dashboard
   assumption: cards are text-first and generously sized for legibility, while
   *everything around them* is compressed. The status-color semantics stay
   exactly as the kit defines them, applied to eligibility state rather than to
   quality.

3. **Every disabled control states its reason inline.** Prose Studio refuses
   things often and for good reasons — an unregistered profile, an infeasible
   context budget, an exhausted session budget, an ineligible candidate. A dead
   control with no explanation is the specific failure this surface exists to
   avoid, so the reason text is part of the control, not a tooltip.

Accessibility exceeds the kit floor in one place, because measurement is the
product: every sparkline carries a text alternative stating its underlying
numbers, and every measurement chip carries text rather than colour alone. A
reader using a screen reader must be able to compare candidates on the same
evidence a sighted reader uses. Candidate cards are fully keyboard-traversable
