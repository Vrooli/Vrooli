...
```bash
vrooli scenario status prose-studio
make stop
vrooli scenario start prose-studio --clean-stale
```
...
prose-studio status --auto-start

# Or override the API base for this invocation
prose-studio status --api-base "http://localhost:$(vrooli scenario port prose-studio API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/prose-studio/`.
...
The default route is `${SCENARIO_DATA_DIR}/prose-studio.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/prose-studio.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/prose-studio.db}")"
```
...
`packages/proto/gen/go/prose-studio/v1/`, TypeScript under
`packages/proto/gen/typescript/js/prose-studio/v1/`, and Python under
`packages/proto/gen/python/prose_studio/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/prose-studio/v1/<domain>/`.
