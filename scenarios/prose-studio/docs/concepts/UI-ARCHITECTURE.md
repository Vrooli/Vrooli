...
react-component-library adoptions resolve-path <component-id> prose-studio
react-component-library adoptions apply <component-id> prose-studio <adopted-path>
```
...
Run `experience-manager spec validate prose-studio --json` after route or
selector changes. The generated notes page spec is example-domain content and
is removed by `template-manager detemplate prose-studio`.
