...
`template-manager orient prose-studio`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient prose-studio
...
```bash
cat > /tmp/prd_context_prose-studio.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start prose-studio --interactive
# …answer the interview, then:
business-health wizard preview prose-studio
business-health wizard apply prose-studio
```
...
```bash
cat > /tmp/requirements_context_prose-studio.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate prose-studio --json
business-health fix preview prose-studio
business-health fix apply prose-studio
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate prose-studio --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate prose-studio`, the notes page spec
...
validate prose-studio --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/prose-studio/v1/<domain>/<domain>.proto`
...
1. Add proto messages and a service under
   `packages/proto/schemas/prose-studio/v1/<domain>/`, then run
