...
`~/.vrooli/bin/prose-studio`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start prose-studio` if the API is unreachable |
...
```bash
prose-studio status
prose-studio status --json
```

### `prose-studio configure <key> <value>`
...
```bash
prose-studio configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`prose-studio <domain> <verb>`). Every command calls a single API
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start prose-studio`" is good.
