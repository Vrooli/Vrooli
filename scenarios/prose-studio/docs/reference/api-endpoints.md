...
envelope (`packages/proto/schemas/prose-studio/v1/shared/errors.proto`):
...
| **CLI** | `prose-studio status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/prose-studio/v1/shared/health.proto`
...
`POST /vrooli.prose_studio.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.prose_studio.v1.notes.NotesService/ListNotes`
...
| **CLI** | `prose-studio notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.prose_studio.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.prose_studio.v1.notes.NotesService/CreateNote`
...
| **CLI** | `prose-studio notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.prose_studio.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.prose_studio.v1.notes.NotesService/GetNote`
...
| **CLI** | `prose-studio notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.prose_studio.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `prose-studio notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/prose-studio/v1/notes/notes.proto`.
