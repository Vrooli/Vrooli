...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/prose-studio.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `prose-studio` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/prose-studio/config.json`
3. `~/.vrooli/config/prose-studio/config.json`
4. `~/.config/vrooli/prose-studio/config.json`
...
```bash
prose-studio configure api_base http://localhost:15001/api/v1
prose-studio configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port prose-studio API_PORT`)
