...
// the template itself does not build because of prose-studio placeholders.
func TestNotesManifestCoversNotesService(t *testing.T) {
	manifest := readNotesManifest(t)
	cliapp.RequireProtoServiceCoverage(t, manifest, notesv1.File_prose_studio_v1_notes_notes_proto, "NotesService")
}
