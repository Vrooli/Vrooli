#!/bin/bash
# Main test orchestrator for social-media-scheduler
set -euo pipefail
...
testing::runner::init \
  --scenario-name "social-media-scheduler" \
...
testing::runner::register_phase --name unit --script "$PHASE_DIR/test-unit.sh" --timeout 180

testing::runner::register_phase --name integration --script "$PHASE_DIR/test-integration.sh" --timeout 240 --requires-runtime true

testing::runner::register_phase --name business --script "$PHASE_DIR/test-business.sh" --timeout 240 --requires-runtime true

testing::runner::register_phase --name performance --script "$PHASE_DIR/test-performance.sh" --timeout 60 --requires-runtime true --optional true

# Presets for faster feedback
