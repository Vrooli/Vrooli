...
// template reference and add their own domains, so pinning the count would
...
// domain's own *_test.go (see internal/artifacts/repository_test.go for the
...
// api/handlers/artifacts/module.go) so the safety net applies
