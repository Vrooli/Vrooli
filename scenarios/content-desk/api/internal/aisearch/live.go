...
// authoritative editorial corpus plus the truthful corpus status. Both the
// lexical Service (the offline projection) and the engine-backed LiveSearch
// satisfy it.
...
// projects the editorial corpus, while the lexical Service stays the
// authoritative status/generation source and the engine's offline TextFallback.
// When no engine could be assembled (an unresolved embedding policy or an
// unreachable vector store) the LiveSearch serves the lexical Service directly,
// so retrieval is never worse than the pre-engine baseline.
...
// not a search result: status truth stays derived from the corpus.
...
func (l *LiveSearch) Search(ctx context.Context, query string, limit int) (*SearchResponse, error) {
	...
	hits, _, err := l.engine.Search(ctx, normalizeProductQueryAliases(query), limit)
	if err != nil {
		return l.lexical.Search(ctx, query, limit)
	}
...
// Status reports the authoritative corpus state plus the index's currency.
...
// it runs the same editorial projection and returns the shared SearchResult
// shape (with the typed projection in Payload) so a down vector backend yields
