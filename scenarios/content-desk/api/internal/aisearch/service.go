...
// StatusReport is the truthful status of the live editorial projection. Both
// Generation and LastIndexedAt are derived from the corpus's own data, so
...
// Service ranks records from a live editorial snapshot. It is the degraded,
...
// NewService builds a service over an editorial source.
...
func (s *Service) Search(ctx context.Context, query string, limit int) (*SearchResponse, error) {
	...
	tokens := tokenize(normalizeProductQueryAliases(query))
	if len(tokens) == 0 {
...
func lexicalScore(tokens []string, r Record) float64 {
	...
	// Require every token to land somewhere, so gibberish with a single
	// accidental token does not surface as a hit.
