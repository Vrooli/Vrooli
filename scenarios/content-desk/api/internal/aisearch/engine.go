...
// Shared-engine adoption for Content Desk. The engine ranks the same bounded
// records the lexical Service projects; it never invents a second corpus. The
...
const (
	// ProviderID is Content Desk's provider identity in .vrooli/search.json and
	// the search-hub registry. It is the SSOT both the boot tuning read and the
	// self-registration push resolve.
	ProviderID = "content-desk.editorial-history"
	// Collection is Content Desk's dedicated Qdrant collection. The collection
	// name appears once; the shared engine derives the rest of the layout.
	Collection = "content-desk-editorial-history"
	// idPrefix namespaces Content Desk's point IDs inside the shared engine.
	idPrefix = "content-desk:"
	// editorialKind is the single logical index kind. The finer draft/publish
	// kind rides in each record's own kind and payload, so one collection holds
	// every editorial record class without a second router.
	editorialKind = "editorial"
)

// EditorialSource adapts the read-only editorial projection to the shared
// engine's Source contract: one SourceDoc per bounded editorial record.
type EditorialSource struct {
	source *StoreSource
}

// NewEditorialSource adapts a StoreSource to the engine Source contract.
func NewEditorialSource(source *StoreSource) *EditorialSource {
	return &EditorialSource{source: source}
}

// LoadAll projects the current editorial snapshot to indexable documents.
func (s *EditorialSource) LoadAll(ctx context.Context) ([]pkg.SourceDoc, error) {
	snapshot, err := s.source.Load(ctx)
	if err != nil {
		return nil, fmt.Errorf("editorial source: %w", err)
	}
...
// Engine is Content Desk's production-capable search provider: the shared
// read-path Service bound to the editorial source, plus the collection identity
...
// It is the single place the Content Desk source binding and read-path seams are
...
func NewEngineFromComponents(c EngineComponents, source pkg.Source, tuning pkg.TuningConfig, textFallback pkg.TextFallbackFunc, parallelism, maxEmbedsPerTick int) *Engine {
	...
	if c.SparseEncoder != nil {
		...
		binding = pkg.NewHybridBinding(editorialKind, idPrefix, c.VectorStore, source, pkg.NewIdentityChunker(), pkg.NewIdentityComposer(), c.SparseEncoder)
	} else {
		binding = pkg.NewDenseBinding(editorialKind, idPrefix, c.VectorStore, source)
	}
...
// Search ranks the indexed corpus and projects each hit into Content Desk's
// typed search shape. It returns the retrieval method the engine used.
