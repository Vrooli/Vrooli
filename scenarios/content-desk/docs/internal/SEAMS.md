...
`packages/proto/schemas/content-desk/v1/<domain>/<file>.proto` —
...
The `health` proto in `packages/proto/schemas/content-desk/v1/shared/`
...
| | |
|---|---|
| **Seam** | Re-runnable factual-evidence check execution |
| **Interface** | `internal/claims/check_runner.go::Runner` (`Run(context.Context, EvidenceCheck) (CheckResult, error)`) |
| **Production wiring** | `claims.LocalRunner{}` is the only local implementation. Claim services receive the interface when the claims domain is composed. |
| **Test fake** | `internal/claims/mocks::FakeRunner` records requested checks and returns an arranged result or error without creating a process. |
| **Why it exists** | A stored shell command is appropriate for local-first use but would be remote code execution in hosted deployment. The domain therefore depends only on `Runner`; a sandboxed hosted runner is a future implementation, not a retrofit. No domain package may invoke a shell API outside `LocalRunner`. |
