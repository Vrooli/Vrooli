...
| Contracts (`packages/proto/schemas/content-desk/`) | Wire shape | Proto messages/services and generated clients | Hand-written route/type mirrors |
...
- scenario proto schemas relocated to
  `packages/proto/schemas/content-desk/`.
...
|---|---|---|---|
| 2026-07-28 | `claims` exposes no hard delete, unlike a normal CRUD domain. | A claim outlives every draft citing it. Deleting one would break the contamination report and erase the record of what was asserted. | Never — this is the scenario. |
| 2026-07-28 | The scenario has a UI but generates none of its own primary content. | Copy is produced by skills; the desk is a verification and record surface. It is a workbench over other agents' output rather than an authoring tool. | If drafts prove impossible to constrain to the required field set from the skill side. |
| 2026-07-28 | Approval is gated on an actor kind rather than a permission role. | There is one operator and no multi-tenant model. A role system would be ceremony over a single-user boundary. | If this scenario is ever operated by more than one approver. |

## Layering — the scenario-specific shape

On top of the standard scenario shape, one structure governs everything: **state
flows one way, and every gate refuses rather than warns.**

```
  CANON     ── docs/marketing/**. Strategy, craft doctrine, post-type purpose.
     │         Operator-curated, decision-gated. The desk READS it and never writes it.
     ▼  (seeds the post-type registry)
  SKILLS    ── the paired x-<type> specs. Produce drafts with declared claims.
     │         Mutable, independently reviewable, outside this scenario.
     ▼  (submit)
...
Three invariants to defend in review:

1. **A gate refuses; it does not warn.** Every blocked transition names its
   specific cause — an unverified claim, a failed review mode, an inactive post
   type, an exhausted slot budget. A warning that can be ignored is not a gate,
   and this scenario exists because prose rules were already ignorable.
2. **Judgement stays outside.** The desk decides whether evidence *exists*, never
   whether a strategy is *right*. Strategy is canon; craft is doctrine; the desk
   holds neither.
3. **The truth gate and the policy gate are separate mechanisms.** Conflating
   them makes the truth gate fail on unfalsifiable statements until someone
   switches it off. See `DECISIONS.md`.
