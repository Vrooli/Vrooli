...
envelope (`packages/proto/schemas/content-desk/v1/shared/errors.proto`):
...
| **CLI** | `content-desk status` |

```bash
curl "http://localhost:${API_PORT}/health"
```

The proto type lives at `packages/proto/schemas/content-desk/v1/shared/health.proto`
...
`POST /vrooli.content_desk.v1.<domain>.<Domain>Service/<Method>`
...
#### `POST /vrooli.content_desk.v1.notes.NotesService/ListNotes`
...
| **CLI** | `content-desk notes list` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.content_desk.v1.notes.NotesService/ListNotes" \
...
#### `POST /vrooli.content_desk.v1.notes.NotesService/CreateNote`
...
| **CLI** | `content-desk notes create --title <title> [--body <body>]` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.content_desk.v1.notes.NotesService/CreateNote" \
  -H 'Content-Type: application/json' \
...
#### `POST /vrooli.content_desk.v1.notes.NotesService/GetNote`
...
| **CLI** | `content-desk notes get <id>` |

```bash
curl -X POST "http://localhost:${API_PORT}/vrooli.content_desk.v1.notes.NotesService/GetNote" \
  -H 'Content-Type: application/json' \
  -d '{"id":"abc123"}'
```
...
| **CLI** | `content-desk notes attach <id> --file <path>` |
...
Defined in `packages/proto/schemas/content-desk/v1/notes/notes.proto`.
