...
|---|---|---|
| `SQLITE_PATH` | `${SCENARIO_DATA_DIR}/content-desk.db` | Override SQLite file location. The default routes through `api-core/storage` and resolves to a writable per-scenario data directory. |
...
For `content-desk` the prefix is the scenario id upper-cased with
...
2. `${XDG_CONFIG_HOME}/vrooli/content-desk/config.json`
3. `~/.vrooli/config/content-desk/config.json`
4. `~/.config/vrooli/content-desk/config.json`
...
```bash
content-desk configure api_base http://localhost:15001/api/v1
content-desk configure token <token>
```
...
4. Vrooli lifecycle port detection (`vrooli scenario port content-desk API_PORT`)
