...
`~/.vrooli/bin/content-desk`, and rebuilt automatically when its
...
| `--auto-start` | Run `vrooli scenario start content-desk` if the API is unreachable |
...
```bash
content-desk status
content-desk status --json
```

### `content-desk configure <key> <value>`
...
```bash
content-desk configure api_base
```

## Scenario commands — `<domain>`

Each product domain exposes its commands as a subcommand group
(`content-desk <domain> <verb>`). Every command calls a single API
...
```bash
content-desk notes list
content-desk notes list --json
```

#### `content-desk notes create --title <title> [--body <body>]`
...
```bash
content-desk notes create --title "First note" --body "Hello world"
```
...
```bash
content-desk notes attach abc123 --file ./example.png
```
...
- **Error messages must be actionable.** "API unreachable" is bad;
  ...
  `vrooli scenario start content-desk`" is good.
