...
```bash
vrooli scenario status content-desk
make stop
vrooli scenario start content-desk --clean-stale
```
...
```bash
make status
vrooli scenario port content-desk UI_PORT
```
...
content-desk status --auto-start

# Or override the API base for this invocation
content-desk status --api-base "http://localhost:$(vrooli scenario port content-desk API_PORT)/api/v1"
```
...
`${XDG_DATA_HOME:-~/.local/share}/vrooli/content-desk/`.
...
The default route is `${SCENARIO_DATA_DIR}/content-desk.db` via
`api-core/storage`. If your filesystem is unusual (read-only home,
strict sandboxing), override:

```bash
export SQLITE_PATH=/tmp/content-desk.db
make start
```
...
```bash
fuser "$(echo "${SQLITE_PATH:-${SCENARIO_DATA_DIR}/content-desk.db}")"
```
...
`packages/proto/gen/go/content-desk/v1/`, TypeScript under
`packages/proto/gen/typescript/js/content-desk/v1/`, and Python under
`packages/proto/gen/python/content_desk/v1/`.

### Codegen ran but Go imports still fail

The generated package paths follow `packages/proto/gen/go/content-desk/v1/<domain>/`.
