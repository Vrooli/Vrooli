...
`template-manager orient content-desk`, which reads the temporary
`.vrooli/orientation.json` manifest generated by the template. After
all required gates pass, run `template-manager orient content-desk
...
```bash
cat > /tmp/prd_context_content-desk.md <<'EOF'
Overview:
- Purpose:
...
```bash
business-health wizard start content-desk --interactive
# …answer the interview, then:
business-health wizard preview content-desk
business-health wizard apply content-desk
```
...
```bash
vrooli scenario requirements validate content-desk --json
```
...
```bash
cat > /tmp/requirements_context_content-desk.md <<'EOF'
Validation approach:
- Unit:
- Integration:
- Business / BAS:
...
```bash
vrooli scenario requirements validate content-desk --json
business-health fix preview content-desk
business-health fix apply content-desk
```
...
react-component-library adoptions resolve-path <component-id> content-desk
react-component-library adoptions apply <component-id> content-desk <adopted-path>
```
...
- [ ] Validate after every meaningful edit:
      `experience-manager spec validate content-desk --json`.

The notes page spec is part of the removable example domain. When you
run `template-manager detemplate content-desk`, the notes page spec
...
validate content-desk --json` passes.
...
- [ ] **Start in proto.** Author `packages/proto/schemas/content-desk/v1/<domain>/<domain>.proto`
...
```bash
react-component-library components ingest content-desk <tsx-path> <slug> \
...
```bash
react-component-library components test <component-id>
react-component-library components style-fit <component-id> content-desk
...
- Wire contracts belong in
  `packages/proto/schemas/content-desk/v1/<domain>/`.
...
1. Add proto messages and a service under
   `packages/proto/schemas/content-desk/v1/<domain>/`, then run
