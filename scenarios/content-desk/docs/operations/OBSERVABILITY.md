...
Metrics are deferred until the domains exist, but the ones that will matter are
already visible from the design: unverified-claim count per draft, time a draft
spends blocked, stale-claim count, undeclared claims found by review versus
declared by authors, and operator approval latency. That last one is the health
metric for the whole scenario — if approval gets slow, the simplification it
enables stalls behind it.
