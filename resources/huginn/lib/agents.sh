...
# 
# Tracks running Huginn agents for coordination and management
################################################################################
...
HUGINN_AGENTS_REGISTRY="${APP_ROOT}/.vrooli/huginn-agents.json"
...
agents::init_registry() {
    local registry_dir
    registry_dir=$(dirname "$HUGINN_AGENTS_REGISTRY")
    ...
    if [[ ! -f "$HUGINN_AGENTS_REGISTRY" ]]; then
        echo '{"agents": {}}' > "$HUGINN_AGENTS_REGISTRY" || return 1
    fi
...
agents::register() {
    ...
    local temp_file="${HUGINN_AGENTS_REGISTRY}.tmp.$$"
    ...
    if ! jq --arg id "$agent_id" \
            ...
            '.agents[$id] = {
                ...
                "command": $cmd,
                "last_seen": $start
            }' "$HUGINN_AGENTS_REGISTRY" > "$temp_file"; then
    ...
    if ! mv "$temp_file" "$HUGINN_AGENTS_REGISTRY"; then
...
agents::unregister() {
    ...
    [[ -f "$HUGINN_AGENTS_REGISTRY" ]] || return 0
    
    # Create temporary file for atomic update
    local temp_file="${HUGINN_AGENTS_REGISTRY}.tmp.$$"
    
    # Remove agent from registry
    if ! jq --arg id "$agent_id" 'del(.agents[$id])' "$HUGINN_AGENTS_REGISTRY" > "$temp_file"; then
        log::error "Failed to update agent registry"
        rm -f "$temp_file"
        return 1
    fi
...
agents::heartbeat() {
    ...
    [[ -f "$HUGINN_AGENTS_REGISTRY" ]] || return 1
    ...
    if ! jq --arg id "$agent_id" \
            ...
            '.agents[$id].last_seen = $time' "$HUGINN_AGENTS_REGISTRY" > "$temp_file"; then
        log::error "Failed to update agent registry"
        rm -f "$temp_file"
        return 1
    fi
...
agents::cleanup() {
    ...
    temp_file="${HUGINN_AGENTS_REGISTRY}.tmp.$$"
    
    # Get list of all agents and check their PIDs
    local agent_ids
    agent_ids=$(jq -r '.agents | keys[]' "$HUGINN_AGENTS_REGISTRY" 2>/dev/null || echo "")
    ...
    cp "$HUGINN_AGENTS_REGISTRY" "$temp_file" || return 0
    ...
    if [[ $cleaned -gt 0 ]]; then
        mv "$temp_file" "$HUGINN_AGENTS_REGISTRY" || rm -f "$temp_file"
    else
        rm -f "$temp_file"
    fi
...
agents::list() {
    ...
    if [[ "$output_format" == "json" ]]; then
        if [[ -n "$status_filter" ]]; then
            ...
            }' "$HUGINN_AGENTS_REGISTRY"
        else
            cat "$HUGINN_AGENTS_REGISTRY"
        fi
    else
        # Table format
        local agents_data
        if [[ -n "$status_filter" ]]; then
            agents_data=$(jq -r --arg status "$status_filter" \
                ...
                "$HUGINN_AGENTS_REGISTRY" 2>/dev/null || echo "")
        else
...
agents::stop() {
    ...
    if [[ "$target" == "all" ]]; then
        ...
        while IFS= read -r agent_id; do
            ...
            local pid
            pid=$(jq -r --arg id "$agent_id" '.agents[$id].pid // empty' "$HUGINN_AGENTS_REGISTRY" 2>/dev/null)
        ...
        # Stop specific agent
        local pid
        pid=$(jq -r --arg id "$target" '.agents[$id].pid // empty' "$HUGINN_AGENTS_REGISTRY" 2>/dev/null)
...
agents::info() {
    ...
    local agent_data
    agent_data=$(jq --arg id "$agent_id" '.agents[$id] // empty' "$HUGINN_AGENTS_REGISTRY" 2>/dev/null)
...
#   0 on success
#######################################
huginn::setup_agent_cleanup() {
    local agent_id="$1"
    
    if [[ -z "$agent_id" ]]; then
        log::error "huginn::setup_agent_cleanup: Agent ID required"
        return 1
    fi
    
    # Set cleanup trap for common exit signals
    trap "agents::unregister '$agent_id' >/dev/null 2>&1" EXIT INT TERM
    
    # Store agent ID for cleanup functions
    export HUGINN_CURRENT_AGENT_ID="$agent_id"
...
huginn::agents::command() {
    ...
    case "$subcommand" in
        ...
        "help"|"-h"|"--help")
            echo "Usage: resource-huginn agents <subcommand> [options]"
            echo ""
            echo "Subcommands:"
        ...
        *)
            ...
            log::error "Use 'resource-huginn agents help' for usage information"
            return 1
            ;;
    esac
}
...
export -f huginn::setup_agent_cleanup
