...
func TestHandleStatsGracefulWhenDbstatMissing(t *testing.T) {
	cfg := env.Config{CLITimeout: time.Second}
	cli := &CLI{
...
func TestManageInstallInvokesRebuilderAndCreatesDirs(t *testing.T) {
	...
	cli := &CLI{
		Service:   stub,
		Config:    cfg,
		Runtime:   discovery.Runtime{SourceRoot: root},
		Installer: install.Installer{Rebuild: func() error { rebuilt = true; return nil }},
	}
...
func TestManageInstallWarnsButSucceedsWhenRebuilderFails(t *testing.T) {
	...
	cli := &CLI{
		...
		Config:    cfg,
		Runtime:   discovery.Runtime{SourceRoot: root},
		Installer: install.Installer{Rebuild: func() error { return fmt.Errorf("boom") }},
	}
...
func TestReplicateAddParsesFlags(t *testing.T) {
	...
	cli := &CLI{
		Service: stub,
		Config:  env.Config{CLITimeout: time.Second},
	}
