...
import (
	...
	"github.com/vrooli/resources/sqlite/cli/internal/install"
	"github.com/vrooli/resources/sqlite/cli/internal/sqlite"
	"github.com/vrooli/resources/sqlite/cli/internal/version"
)
...
func New(cfg env.Config, runtime discovery.Runtime) *CLI {
	return &CLI{
		Service:   sqlite.NewService(cfg),
		Config:    cfg,
		Runtime:   runtime,
		Now:       time.Now,
		Installer: install.New(runtime),
		Manifest: version.Manifest{
...
func (c *CLI) handleManage(args []string) int {
	...
	case "install":
		...
		if installer.Rebuild == nil {
			installer = install.New(c.Runtime)
		}
		if err := installer.Rebuild(); err != nil {
