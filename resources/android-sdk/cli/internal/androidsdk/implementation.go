...
func Run(args []string) error {
	...
	case "kvm-check":
		return reportKVM()
	case "avd-create":
		return avdCreate(os.Getenv("AVD_NAME"), os.Getenv("SYSTEM_IMAGE"))
	case "avd-start":
		return avdStart(os.Getenv("AVD_NAME"))
	case "avd-stop":
		return avdStop()
	case "avd-delete":
		return avdDelete(os.Getenv("AVD_NAME"))
	case "toolchain-install":
		return installHostToolchain()
	default:
		return fmt.Errorf("unknown command %q (want install, status, kvm-check, avd-create, avd-start, avd-stop, toolchain-install, or version)", command)
	}
}
...
func waitForBoot(ctx context.Context, serial string, timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		...
		case <-ctx.Done():
			return ctx.Err()
		case <-time.After(tuning.LifecycleTransitionTimeout()):
		}
	}
