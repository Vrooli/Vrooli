...
# These environment variables control parallel workflow injection performance
#######################################
# N8N_ENABLE_PARALLEL_INJECTION=yes|no    - Enable/disable parallel processing (default: yes)
# N8N_PARALLEL_BATCH_SIZE=N                - Workflow batch size (default: 5, recommended: 3-8)
# N8N_PARALLEL_CREDENTIAL_BATCH_SIZE=N     - Credential batch size (default: 3, recommended: 2-5)
#
# Performance Benefits:
# - Sequential: ~28 workflows = 2-3 minutes (140+ API calls)
# - Parallel (batch=5): ~28 workflows = 30-45 seconds (~5x faster)
#
# Usage Examples:
#   # Use default settings (batch size 5 for workflows, 3 for credentials)
#   ./inject.sh
#   
#   # Customize batch sizes for your environment
#   N8N_PARALLEL_BATCH_SIZE=8 N8N_PARALLEL_CREDENTIAL_BATCH_SIZE=4 ./inject.sh
#   
#   # Disable parallel processing (fallback to sequential)
#   N8N_ENABLE_PARALLEL_INJECTION=no ./inject.sh
#
# Troubleshooting:
# - If you see timeouts/errors, reduce batch size: N8N_PARALLEL_BATCH_SIZE=3
# - For resource-constrained environments, use: N8N_PARALLEL_BATCH_SIZE=2
# - Always falls back to sequential processing if parallel fails
#######################################
...
n8n::inject_data() {
    ...
    if echo "$config" | jq -e '.workflows' >/dev/null 2>&1; then
        ...
        # Process workflows using parallel batches for improved performance
        # Use environment variable to control batch size and parallel processing
        local batch_size="${N8N_PARALLEL_BATCH_SIZE:-5}"
        local enable_parallel="${N8N_ENABLE_PARALLEL_INJECTION:-yes}"
        
        if ! inject_framework::process_array_parallel "$workflows" "n8n::import_workflow" "workflows" "$batch_size" "$enable_parallel"; then
            log::warn "Some workflows failed to import - check individual workflow logs for details"
            # Fall back to sequential processing on failure
            if [[ "$enable_parallel" == "yes" ]]; then
                log::info "Retrying with sequential processing as fallback..."
                if inject_framework::process_array "$workflows" "n8n::import_workflow" "workflows"; then
                    log::success "All workflows imported successfully using sequential fallback"
                else
                    log::error "Both parallel and sequential processing failed"
                    return 1
                fi
            fi
        else
            log::success "All workflows imported successfully using parallel processing"
        fi
    else
    ...
    if echo "$config" | jq -e '.credentials' >/dev/null 2>&1; then
        ...
        # Process credentials using parallel batches (smaller batch size for credentials)
        local cred_batch_size="${N8N_PARALLEL_CREDENTIAL_BATCH_SIZE:-3}"
        local enable_parallel="${N8N_ENABLE_PARALLEL_INJECTION:-yes}"
        
        if ! inject_framework::process_array_parallel "$credentials" "n8n::create_credential" "credentials" "$cred_batch_size" "$enable_parallel"; then
            log::warn "Some credentials failed to create - check individual credential logs for details"
            # Fall back to sequential processing on failure  
            if [[ "$enable_parallel" == "yes" ]]; then
                log::info "Retrying credential creation with sequential processing as fallback..."
                if inject_framework::process_array "$credentials" "n8n::create_credential" "credentials"; then
                    log::success "All manual credentials created successfully using sequential fallback"
                else
                    log::warn "Both parallel and sequential credential creation had failures - partial success acceptable"
                fi
            fi
            # Don't rollback or fail completely - partial success is acceptable
        else
            log::success "All manual credentials created successfully using parallel processing"
        fi
    else
