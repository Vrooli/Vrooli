...
// target config file (typically ~/.codex/.vrooli-permissions-state.json
// for user scope, ~/.codex/.vrooli-permissions-state.admin.json for
// admin scope) and is authoritative for drift detection.
type State struct {
...
// StatePath returns the conventional state-file path. Admin scope gets
// a distinct filename so user and admin state never collide.
