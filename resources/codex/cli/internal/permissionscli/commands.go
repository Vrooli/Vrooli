...
// on resource-codex. It is the only place that ties the adapter, the
// policy gate, and the agent-detection substrate together.
//
// Codex-specific delta from claude-code/opencode: the `--scope`
// (user|admin) flag selects between ~/.codex/config.toml and
// ~/.codex/requirements.toml. `permissions doctor` surfaces a clear
// warning that Codex does NOT honour per-command-pattern rules
// natively — the `[vrooli.permissions]` section records intent for
// cross-agent uniformity, not for live enforcement.
...
// Default returns Handlers wired to ~/.codex/{config,requirements}.toml
// and the live agent-detection substrate.
...
func (h *Handlers) Doctor(args []string) error {
	...
	// Critical caveat: Codex doesn't honour per-pattern rules natively.
	fmt.Fprintln(h.Stdout, "note: Codex does NOT enforce per-command-pattern deny/allow/ask natively today.")
	fmt.Fprintln(h.Stdout, "      The `[vrooli.permissions]` section records Vrooli's intent for cross-agent")
	fmt.Fprintln(h.Stdout, "      uniformity; for hard enforcement use sandbox_mode/approval_policy.")
	return nil
}
...
func (h *Handlers) mutate(verb string, args []string) error {
	...
	pattern := fs.Arg(0)
	// Ensure pattern doesn't end up in multiple buckets.
...
func (h *Handlers) gate(verb string, mutating bool, authorized bool) error {
	...
	case agentpolicy.DecisionWarn:
		fmt.Fprintln(h.Stderr, agentpolicy.RenderDenyMessage(agentpolicy.DenyContext{ResourceLabel: "resource-codex", ConfigPath: "~/.codex/config.toml [vrooli.permissions]"}, cmd, h.Policy))
		return nil
	case agentpolicy.DecisionDeny:
		return errors.New(agentpolicy.RenderDenyMessage(agentpolicy.DenyContext{ResourceLabel: "resource-codex", ConfigPath: "~/.codex/config.toml [vrooli.permissions]"}, cmd, h.Policy))
	default:
