#!/usr/bin/env node
// Playwright driver exposed over HTTP for the Go AutomationEngine.
// Non-Docker, PID-file friendly, optional Chromium override via PLAYWRIGHT_CHROMIUM_PATH.
...
async function resetSession(_req, res, sessionId) {
...
async function closeSession(_req, res, sessionId) {
  const session = sessions.get(sessionId);
  if (!session) return json(res, 404, { error: 'session not found' });
...
async function runInstruction(req, res, sessionId) {
  ...
  try {
    const outcome = await applyInstruction(page, instr);
    if (outcome && outcome.extractedData) {
      extractedData = outcome.extractedData;
    }
    if (outcome && outcome.assertion) {
  ...
  if (session.video && typeof page.video === 'function') {
    try {
...
async function applyInstruction(page, instr) {
  ...
  switch ((instr.type || '').toLowerCase()) {
    ...
    case 'uploadfile': {
      const selector = params.selector;
      const uploadPath = params.filePath || params.file_path || params.path;
      if (!selector) throw new Error('uploadFile missing selector');
      if (!uploadPath) throw new Error('uploadFile missing file path');
      await page.setInputFiles(selector, uploadPath, { timeout: params.timeoutMs || 20000 });
      break;
    }
    case 'scroll': {
