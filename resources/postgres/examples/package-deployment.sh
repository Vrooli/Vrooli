...
# Setup sudo permissions for PostgreSQL service management
setup_postgresql_sudo_permissions() {
    local client_name="$1"
    local actual_user
    ...
    if [[ -z "$actual_user" || "$actual_user" == "root" ]]; then
        echo "Cannot determine actual user or user is root, skipping sudo permissions setup"
        return 0
    fi
    
    local sudoers_file="/etc/sudoers.d/postgresql-service"
    local sudoers_content="# PostgreSQL service management permissions
# Generated by Vrooli PostgreSQL package deployment
$actual_user ALL=(ALL) NOPASSWD: /usr/bin/systemctl start postgresql-${client_name}, /usr/bin/systemctl stop postgresql-${client_name}, /usr/bin/systemctl restart postgresql-${client_name}, /usr/bin/systemctl status postgresql-${client_name}, /usr/bin/systemctl is-active postgresql-${client_name}"
    ...
    if sudo::exec_with_fallback "echo '$sudoers_content' > '$sudoers_file'"; then
        ...
        if sudo::exec_with_fallback "chmod 0440 '$sudoers_file'"; then
            echo "Sudo permissions configured for user '$actual_user'"
            return 0
        else
...
if [[ -f "/etc/systemd/system/postgresql-${CLIENT_NAME}.service" ]]; then
    ...
    if setup_postgresql_sudo_permissions "$CLIENT_NAME"; then
        echo "✅ Sudo permissions configured for PostgreSQL service management"
    else
        echo "⚠️  Failed to setup sudo permissions - you may need to manually run systemctl commands with sudo"
    fi
fi

# Start service
echo "Starting PostgreSQL service..."
# Use sudo permissions if available, otherwise fall back to sudo::exec_with_fallback
if sudo systemctl start "postgresql-${CLIENT_NAME}" 2>/dev/null; then
    echo "✅ PostgreSQL service started using sudo permissions"
else
    echo "⚠️  Using fallback sudo method for service start"
...
if sudo::exec_with_fallback "-u postgres pg_isready -p 5432"; then
    ...
    echo "To stop: systemctl stop postgresql-${CLIENT_NAME} (no sudo required)"
    echo "To check status: systemctl status postgresql-${CLIENT_NAME} (no sudo required)"
    echo "To manage service: ./scripts/manage-service.sh {start|stop|restart|status|logs|check-permissions|cleanup}"
else
    echo "❌ PostgreSQL deployment failed"
    # Use sudo permissions if available, otherwise fall back to sudo::exec_with_fallback
    if sudo systemctl status "postgresql-${CLIENT_NAME}" 2>/dev/null; then
        echo "Service status check completed using sudo permissions"
    else
...
EOF
    ...
    # Create service management script (no sudo required)
    cat > "$package_path/scripts/manage-service.sh" << 'EOF'
#!/bin/bash
# PostgreSQL Service Management Script (No sudo required)
# Generated by Vrooli PostgreSQL package deployment

CLIENT_NAME="${CLIENT_NAME}"
SERVICE_NAME="postgresql-${CLIENT_NAME}"
...
case "${1:-}" in
    start)
        ...
        ;;
    stop)
        echo "Stopping PostgreSQL service: $SERVICE_NAME"
        systemctl stop "$SERVICE_NAME"
        ;;
    restart)
        echo "Restarting PostgreSQL service: $SERVICE_NAME"
        systemctl restart "$SERVICE_NAME"
        ;;
    status)
        echo "PostgreSQL service status: $SERVICE_NAME"
        systemctl status "$SERVICE_NAME"
        ;;
    enable)
        echo "Enabling PostgreSQL service: $SERVICE_NAME"
        systemctl enable "$SERVICE_NAME"
        ;;
    disable)
        echo "Disabling PostgreSQL service: $SERVICE_NAME"
        systemctl disable "$SERVICE_NAME"
        ;;
    logs)
        echo "PostgreSQL service logs: $SERVICE_NAME"
        journalctl -u "$SERVICE_NAME" -f
        ;;
    check-permissions)
        echo "Checking PostgreSQL service permissions: $SERVICE_NAME"
        if [[ -f "/etc/sudoers.d/postgresql-service" ]]; then
            echo "✅ Sudoers file exists: /etc/sudoers.d/postgresql-service"
            echo "Contents:"
            cat "/etc/sudoers.d/postgresql-service"
        else
            echo "❌ Sudoers file not found: /etc/sudoers.d/postgresql-service"
        fi
        
        echo ""
        echo "Testing service management permissions:"
        if systemctl status "$SERVICE_NAME" >/dev/null 2>&1; then
            echo "✅ Service status check works (no sudo required)"
        else
            echo "❌ Service status check failed (may need sudo)"
        fi
        ;;
    cleanup)
        echo "Cleaning up PostgreSQL service: $SERVICE_NAME"
        echo "This will remove the sudoers file and stop the service"
        ...
        if [[ "$response" =~ ^[Yy]$ ]]; then
            ...
            sudo rm -f "/etc/sudoers.d/postgresql-service" 2>/dev/null || true
            sudo systemctl daemon-reload 2>/dev/null || true
            echo "✅ PostgreSQL service cleanup completed"
        else
            echo "Cleanup cancelled"
        fi
        ;;
    *)
        echo "Usage: $0 {start|stop|restart|status|enable|disable|logs|check-permissions|cleanup}"
        echo ""
        echo "PostgreSQL Service Management (No sudo required)"
...
EOF
    ...
    chmod +x "$package_path/scripts/manage-service.sh"
