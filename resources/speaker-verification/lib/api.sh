...
speaker_verification::get_api_info() {
    ...
    echo "Enroll clip:     POST   /v1/profiles        (multipart: profile_id, display_name, label, notes, audio)"
    echo "Profile detail:  GET    /v1/profiles/{id}    (+ /v1/profiles/{id}/clips)"
    echo "Verify:          POST   /v1/verify          (multipart: profile_id, threshold, audio)"
    echo "Extract:         POST   /v1/extract         (target-speaker extraction; returns cleaned PCM)"
    echo "Delete clip:     DELETE /v1/profiles/{id}/clips/{clip_id}"
