...
// on resource-claude-code. It is the only place that ties the adapter,
...
// Default returns Handlers wired to ~/.claude/settings.json and the
// live agent-detection substrate. pinnedVersion is the upstream CLI
// version this build was authored against; doctor warns when the
...
func Commands(h *Handlers) cliapp.SubcommandGroup {
	...
	return cliapp.SubcommandGroup{
		...
		Subcommands: []cliapp.Command{
			...
			{Name: "show", Description: "Print the full settings.json (raw or pretty)", Run: h.Show},
...
func (h *Handlers) List(args []string) error {
	...
	fmt.Fprintf(h.Stdout, "hooks-paired: %v\n", p.Hooks)
	return nil
}
...
func (h *Handlers) DriftCheck(args []string) error {
	...
	if st.Fingerprint == liveFP {
		fmt.Fprintln(h.Stdout, "clean — settings.json matches the last Vrooli write")
		return nil
	}
...
func (h *Handlers) Doctor(args []string) error {
	...
	fmt.Fprintf(h.Stdout, "resource-claude-code version: %s\n", h.CLIVersion)
...
func (h *Handlers) gate(verb string, mutating bool, authorized bool) error {
	...
	case agentpolicy.DecisionWarn:
		fmt.Fprintln(h.Stderr, agentpolicy.RenderDenyMessage(agentpolicy.DenyContext{ResourceLabel: "resource-claude-code", ConfigPath: "~/.claude/settings.json"}, cmd, h.Policy))
		return nil
	case agentpolicy.DecisionDeny:
		return errors.New(agentpolicy.RenderDenyMessage(agentpolicy.DenyContext{ResourceLabel: "resource-claude-code", ConfigPath: "~/.claude/settings.json"}, cmd, h.Policy))
	default:
