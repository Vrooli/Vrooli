...
resource-vault content put-secret \
...
resource-vault content list-secrets --path "environments/dev/"
pause
...
resource-vault content get-secret --path "environments/dev/database" --key "connection_string"
echo
echo "Stripe API key:"
resource-vault content get-secret --path "environments/dev/stripe"
echo
echo "JWT secret (as JSON):"
resource-vault content get-secret --path "environments/dev/auth" --format json
pause

# 5. Rotate a secret
echo -e "${GREEN}5. Rotating API Key${NC}"
echo "Let's generate a new API key..."
resource-vault content rotate-secret --path "environments/dev/api-key" --type api-key
echo
echo "New API key:"
resource-vault content get-secret --path "environments/dev/api-key"
pause
...
resource-vault content migrate-env --env-file /tmp/demo.env --vault-prefix "demo/migrated"
echo -e "${GREEN}✓ Migration complete${NC}"
echo
echo "Migrated secrets:"
resource-vault content list-secrets --path "demo/migrated/"
...
resource-vault content bulk-put --json-file /tmp/demo-bulk.json --base-path "demo/tokens"
echo -e "${GREEN}✓ Bulk import complete${NC}"
echo
echo "Imported tokens:"
resource-vault content list-secrets --path "demo/tokens/"
...
resource-vault content export-secrets --path "demo/tokens" > /tmp/exported-tokens.json
...
for path in "environments/dev/database" "environments/dev/stripe" "environments/dev/auth" "environments/dev/api-key"; do
    resource-vault content delete-secret --path "$path" 2>/dev/null || true
done

# Clean up migrated and bulk secrets
resource-vault content list-secrets --path "demo/" 2>/dev/null | while read -r secret; do
    [[ -n "$secret" ]] && resource-vault content delete-secret --path "demo/$secret" 2>/dev/null || true
done
