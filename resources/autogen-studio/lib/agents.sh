...
# 
# Tracks running AutoGen Studio agents for coordination and management
################################################################################
...
AUTOGEN_STUDIO_AGENTS_REGISTRY="${APP_ROOT}/.vrooli/autogen-studio-agents.json"
...
agents::init_registry() {
    local registry_dir
    registry_dir=$(dirname "$AUTOGEN_STUDIO_AGENTS_REGISTRY")
    ...
    if [[ ! -f "$AUTOGEN_STUDIO_AGENTS_REGISTRY" ]]; then
        echo '{"agents": {}}' > "$AUTOGEN_STUDIO_AGENTS_REGISTRY" || return 1
    fi
...
#######################################
agents::generate_id() {
    echo "autogen-studio-agent-$(date +%s)-$$"
}
...
agents::register() {
    ...
    local temp_file="${AUTOGEN_STUDIO_AGENTS_REGISTRY}.tmp.$$"
    ...
    if ! jq --arg id "$agent_id" \
            ...
            '.agents[$id] = {
                ...
                "command": $cmd,
                "last_seen": $start
            }' "$AUTOGEN_STUDIO_AGENTS_REGISTRY" > "$temp_file"; then
    ...
    if ! mv "$temp_file" "$AUTOGEN_STUDIO_AGENTS_REGISTRY"; then
...
agents::unregister() {
    ...
    [[ -f "$AUTOGEN_STUDIO_AGENTS_REGISTRY" ]] || return 0
    
    # Create temporary file for atomic update
    local temp_file="${AUTOGEN_STUDIO_AGENTS_REGISTRY}.tmp.$$"
    
    # Remove agent from registry
    if ! jq --arg id "$agent_id" 'del(.agents[$id])' "$AUTOGEN_STUDIO_AGENTS_REGISTRY" > "$temp_file"; then
        log::error "Failed to update agent registry"
        rm -f "$temp_file"
        return 1
    fi
...
agents::heartbeat() {
    ...
    [[ -f "$AUTOGEN_STUDIO_AGENTS_REGISTRY" ]] || return 1
    ...
    if ! jq --arg id "$agent_id" \
            ...
            '.agents[$id].last_seen = $time' "$AUTOGEN_STUDIO_AGENTS_REGISTRY" > "$temp_file"; then
        log::error "Failed to update agent registry"
        rm -f "$temp_file"
        return 1
    fi
...
agents::cleanup() {
    ...
    temp_file="${AUTOGEN_STUDIO_AGENTS_REGISTRY}.tmp.$$"
    
    # Get list of all agents and check their PIDs
    local agent_ids
    agent_ids=$(jq -r '.agents | keys[]' "$AUTOGEN_STUDIO_AGENTS_REGISTRY" 2>/dev/null || echo "")
    ...
    cp "$AUTOGEN_STUDIO_AGENTS_REGISTRY" "$temp_file" || return 0
    ...
    if [[ $cleaned -gt 0 ]]; then
        mv "$temp_file" "$AUTOGEN_STUDIO_AGENTS_REGISTRY" || rm -f "$temp_file"
    else
        rm -f "$temp_file"
    fi
...
agents::list() {
    ...
    [[ -f "$AUTOGEN_STUDIO_AGENTS_REGISTRY" ]] || {
    ...
    if [[ "$output_format" == "json" ]]; then
        if [[ -n "$status_filter" ]]; then
            ...
            }' "$AUTOGEN_STUDIO_AGENTS_REGISTRY"
        else
            cat "$AUTOGEN_STUDIO_AGENTS_REGISTRY"
        fi
    else
        # Table format
        local agents_data
        if [[ -n "$status_filter" ]]; then
            agents_data=$(jq -r --arg status "$status_filter" \
                ...
                "$AUTOGEN_STUDIO_AGENTS_REGISTRY" 2>/dev/null || echo "")
        else
...
agents::stop() {
    ...
    if [[ "$target" == "all" ]]; then
        ...
        while IFS= read -r agent_id; do
            ...
            local pid
            pid=$(jq -r --arg id "$agent_id" '.agents[$id].pid // empty' "$AUTOGEN_STUDIO_AGENTS_REGISTRY" 2>/dev/null)
        ...
        # Stop specific agent
        local pid
        pid=$(jq -r --arg id "$target" '.agents[$id].pid // empty' "$AUTOGEN_STUDIO_AGENTS_REGISTRY" 2>/dev/null)
...
agents::info() {
    ...
    local agent_data
    agent_data=$(jq --arg id "$agent_id" '.agents[$id] // empty' "$AUTOGEN_STUDIO_AGENTS_REGISTRY" 2>/dev/null)
...
autogen_studio::agents::command() {
    ...
    case "$subcommand" in
        ...
        "help"|"-h"|"--help")
            echo "Usage: resource-autogen-studio agents <subcommand> [options]"
            echo ""
            echo "Subcommands:"
        ...
        *)
            ...
            log::error "Use 'resource-autogen-studio agents help' for usage information"
            return 1
            ;;
    esac
}
...
#   $1 - Agent ID
#######################################
autogen_studio::setup_agent_cleanup() {
    local agent_id="$1"
    
    # Export the agent ID so trap can access it
    export AUTOGEN_STUDIO_CURRENT_AGENT_ID="$agent_id"
    
    # Set up signal handlers for cleanup
    trap 'agents::unregister "$AUTOGEN_STUDIO_CURRENT_AGENT_ID" 2>/dev/null || true; exit' INT TERM EXIT
}
...
export -f autogen_studio::agents::command
export -f autogen_studio::setup_agent_cleanup