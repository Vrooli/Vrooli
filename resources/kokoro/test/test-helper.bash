...
vrooli_setup_service_test() {
    local service_name="${1:-kokoro}"
    export VROOLI_TEST_SERVICE="$service_name"
    export VROOLI_TEST_TMPDIR="${BATS_TEST_TMPDIR:-$(mktemp -d /tmp/kokoro-bats.XXXXXX)}"
...
vrooli_cleanup_test() {
    if [[ -n "${KOKORO_TEST_TMPDIR:-}" ]] && [[ -d "${KOKORO_TEST_TMPDIR}" ]]; then
        rm -rf "${KOKORO_TEST_TMPDIR}"
    fi
}

vrooli_make_temp_dir() {
    local prefix="${1:-kokoro-test}"
