...
// StateSchemaVersion bumps when the on-disk State shape changes incompatibly.
// drift-check refuses to compare across versions.
const StateSchemaVersion = 1

// State records the adapter's last-write summary. It lives next to the target
// settings file (~/.gemini/antigravity-cli/.vrooli-permissions-state.json) and
// is authoritative for drift detection: if the live settings fingerprint
// diverges, the user (or Antigravity itself, persisting an in-session grant)
// changed the managed grants since the last Vrooli write.
type State struct {
...
// StatePath returns the conventional state-file path (next to the settings file).
