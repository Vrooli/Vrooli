...
| **Surface** | `server/server.py` — `start`/binary-PCM/`end` client frames; `partial`/`segment`/`done`/`error` server frames. Input is canonical PCM s16le 16 kHz mono. |
...
| **Surface** | `server/server.py::Model.resample_to_model` (uses torch linear interpolation, 16000 → `mimi.sample_rate`). |
...
| | |
|---|---|
| **Seam** | Declarative contract for ports, env exports, health, GPU selection, and freshness — consumed by the shared `vrooli resource ...` control plane and the `cli-core` resource app. |
