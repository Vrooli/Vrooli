...
# 
# Tracks running Gemini agents for coordination and management
################################################################################
...
GEMINI_AGENTS_REGISTRY="${APP_ROOT}/.vrooli/gemini-agents.json"
...
agents::init_registry() {
    local registry_dir
    registry_dir=$(dirname "$GEMINI_AGENTS_REGISTRY")
    ...
    if [[ ! -f "$GEMINI_AGENTS_REGISTRY" ]]; then
        echo '{"agents": {}}' > "$GEMINI_AGENTS_REGISTRY" || return 1
    fi
...
agents::register() {
    ...
    local temp_file="${GEMINI_AGENTS_REGISTRY}.tmp.$$"
    ...
    if ! jq --arg id "$agent_id" \
            ...
            '.agents[$id] = {
                ...
                "command": $cmd,
                "last_seen": $start
            }' "$GEMINI_AGENTS_REGISTRY" > "$temp_file"; then
    ...
    if ! mv "$temp_file" "$GEMINI_AGENTS_REGISTRY"; then
...
agents::unregister() {
    ...
    [[ -f "$GEMINI_AGENTS_REGISTRY" ]] || return 0
    
    # Create temporary file for atomic update
    local temp_file="${GEMINI_AGENTS_REGISTRY}.tmp.$$"
    
    # Remove agent from registry
    if ! jq --arg id "$agent_id" 'del(.agents[$id])' "$GEMINI_AGENTS_REGISTRY" > "$temp_file"; then
        log::error "Failed to update agent registry"
        rm -f "$temp_file"
        return 1
    fi
...
agents::heartbeat() {
    ...
    [[ -f "$GEMINI_AGENTS_REGISTRY" ]] || return 1
    ...
    if ! jq --arg id "$agent_id" \
            ...
            '.agents[$id].last_seen = $time' "$GEMINI_AGENTS_REGISTRY" > "$temp_file"; then
        log::error "Failed to update agent registry"
        rm -f "$temp_file"
        return 1
    fi
...
agents::cleanup() {
    ...
    temp_file="${GEMINI_AGENTS_REGISTRY}.tmp.$$"
    
    # Get list of all agents and check their PIDs
    local agent_ids
    agent_ids=$(jq -r '.agents | keys[]' "$GEMINI_AGENTS_REGISTRY" 2>/dev/null || echo "")
    ...
    cp "$GEMINI_AGENTS_REGISTRY" "$temp_file" || return 0
    ...
    if [[ $cleaned -gt 0 ]]; then
        mv "$temp_file" "$GEMINI_AGENTS_REGISTRY" || rm -f "$temp_file"
    else
        rm -f "$temp_file"
    fi
...
agents::list() {
    ...
    if [[ "$output_format" == "json" ]]; then
        if [[ -n "$status_filter" ]]; then
            ...
            }' "$GEMINI_AGENTS_REGISTRY"
        else
            cat "$GEMINI_AGENTS_REGISTRY"
        fi
    else
        # Table format
        local agents_data
        if [[ -n "$status_filter" ]]; then
            agents_data=$(jq -r --arg status "$status_filter" \
                ...
                "$GEMINI_AGENTS_REGISTRY" 2>/dev/null || echo "")
        else
...
agents::stop() {
    ...
    if [[ "$target" == "all" ]]; then
        ...
        while IFS= read -r agent_id; do
            ...
            local pid
            pid=$(jq -r --arg id "$agent_id" '.agents[$id].pid // empty' "$GEMINI_AGENTS_REGISTRY" 2>/dev/null)
        ...
        # Stop specific agent
        local pid
        pid=$(jq -r --arg id "$target" '.agents[$id].pid // empty' "$GEMINI_AGENTS_REGISTRY" 2>/dev/null)
...
agents::info() {
    ...
    local agent_data
    agent_data=$(jq --arg id "$agent_id" '.agents[$id] // empty' "$GEMINI_AGENTS_REGISTRY" 2>/dev/null)
...
gemini::agents::command() {
    ...
    case "$subcommand" in
        ...
        "help"|"-h"|"--help")
            echo "Usage: resource-gemini agents <subcommand> [options]"
            echo ""
            echo "Subcommands:"
        ...
        *)
            ...
            log::error "Use 'resource-gemini agents help' for usage information"
            return 1
            ;;
    esac
}
