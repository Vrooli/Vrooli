#!/usr/bin/env bash
# Browserless Resource Test Runner - Main entry point for Browserless resource validation
...
TEST_DIR="${BROWSERLESS_CLI_DIR}/test"
...
show_usage() {
    cat << 'EOF'
Browserless Resource Test Runner - Validate Browserless service functionality
...
NOTE: These tests validate Browserless as a RESOURCE (health, connectivity).
      For Browserless API testing, use: resource-browserless test api
EOF
}

# Test phase functions
run_smoke_tests() {
    log::info "=== BROWSERLESS RESOURCE SMOKE TESTS ==="
...
run_unit_tests() {
    log::info "=== BROWSERLESS RESOURCE UNIT TESTS ==="
...
run_integration_tests() {
    log::info "=== BROWSERLESS RESOURCE INTEGRATION TESTS ==="
...
main() {
    ...
    if [[ "$verbose" == "true" ]]; then
        ...
        export BROWSERLESS_TEST_VERBOSE="true"
    fi
    
    log::info "🧪 Starting Browserless Resource Validation Tests"
    ...
    if [[ "$no_summary" != "true" ]]; then
        echo "================================"
        log::info "🧪 Browserless Resource Test Summary"
        echo "================================"
        ...
        if [[ $overall_status -eq 0 ]]; then
            echo ""
            log::success "🎉 ALL BROWSERLESS RESOURCE TESTS PASSED"
            echo "Browserless service is healthy and fully functional"
        else
            echo ""
            log::error "💥 SOME BROWSERLESS RESOURCE TESTS FAILED"
            echo "Browserless service needs attention before use"
        fi
    fi
