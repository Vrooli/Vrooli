...
- `operating-mode.schema.json`
  Defines a swarm-manager **operating mode** as data (`$id: operating-mode/v1`) — a reusable, inspectable, testable methodology loop for agentic software engineering. One schema covers two document kinds discriminated by the top-level `kind`: `operating-mode` (a `mode.json`: identity + decision metadata, scope, run strategy, phase graph with generic field-predicate guards, per-phase declared output schema, and the prompt/artifact/profile/backlog-sync/metrics/lock/ui policy blocks) and `operating-mode-example-run` (a simulation fixture that seeds phase outputs and asserts an `expected_path` through the real guards). The vocabulary is generic — it expresses arbitrary phase DAGs and branching with no vocabulary bespoke to a named mode. Mode data lives in `scenarios/swarm-manager/modes/<id>/` (`mode.json` + `example-runs/`); phase prompt bodies live as prompt-manager skills reached through `prompt.catalog_prefix` plus each phase prompt suffix. One generic engine under `scenarios/swarm-manager/api/internal/operatingmode/` interprets the data. Doctrine and vocabulary: `scenarios/swarm-manager/docs/concepts/EXECUTION-MODES.md`. What JSON Schema cannot express (cross-phase reference resolution, guard field-paths resolving against declared output, prompt skill/catalog resolution, example-run path assertions) is enforced by that engine's semantic validator.
