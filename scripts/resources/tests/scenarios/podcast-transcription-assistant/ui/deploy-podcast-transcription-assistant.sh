#!/bin/bash
# ====================================================================
# Enterprise Podcast Transcription Assistant - Windmill Deployment Script
# ====================================================================
# 
# This script deploys the complete Enterprise Podcast Transcription Assistant
# to a running Windmill instance, including all backend scripts and the
# professional UI for podcast producers and content marketing teams.
#
# Revenue Potential: $8,000 - $18,000 per project
# Target Market: Podcast producers, content creators, marketing agencies
#
# Usage:
#   ./deploy-podcast-transcription-assistant.sh [options]
#
# Options:
...
#   --app-name NAME        Application name (default: enterprise-podcast-transcription)
...
APP_DEFINITION_FILE="$SCRIPT_DIR/enterprise-podcast-transcription-app.json"
...
APP_NAME="${APP_NAME:-enterprise-podcast-transcription}"
...
show_help() {
    echo "🎙️ Enterprise Podcast Transcription Assistant - Windmill Deployment"
    echo "💰 Revenue Potential: \$8,000 - \$18,000 per project"
    echo ""
    ...
    echo "  --app-name NAME        Application name (default: enterprise-podcast-transcription)"
    ...
    echo "  🎤 High-Accuracy Podcast Transcription"
    echo "  📝 Multi-Format Content Generation"
    echo "  🚀 Automated Publishing Workflows"
    echo "  📊 Performance Analytics & Insights"
    echo "  🎨 Brand Voice Consistency Management"
    echo "  🔄 Batch Processing & Automation"
    echo "  💼 Enterprise Project Management"
}
...
check_service_dependencies() {
    ...
    local services=(
        ...
        "Vault:${VAULT_BASE_URL}/v1/sys/health"
    )
    ...
    if [[ $available_services -lt 2 ]]; then
        log_warning "Less than 2 core services are available. Some features may not work."
        log_warning "For full podcast transcription functionality, ensure Whisper and Ollama are running."
    fi
}
...
ensure_workspace() {
    ...
    if echo "$workspace_response" | jq -e '.id' >/dev/null 2>&1; then
        ...
        create_response=$(curl -s -X POST \
            ...
            -d "{\"id\":\"$WORKSPACE\",\"name\":\"Enterprise Podcast Transcription - $WORKSPACE\"}" || echo '{"error":"workspace_creation_failed"}')
...
set_workspace_variables() {
    ...
    for var_info in "${variables[@]}"; do
        ...
        var_response=$(curl -s -X POST \
            ...
            -d "{\"path\":\"$var_name\",\"value\":\"$var_value\",\"is_secret\":false,\"description\":\"Enterprise Podcast Transcription - $var_name\"}" || echo '{"error":"variable_set_failed"}')
    done
...
deploy_scripts() {
    ...
    local script_files=(
        "podcast_transcription_engine.ts"
        "content_generation_assistant.ts"
    )
    ...
    for script_file in "${script_files[@]}"; do
        ...
        script_payload=$(jq -n \
            ...
            --arg description "Enterprise Podcast Transcription - $script_name (Revenue: \$8K-18K)" \
            --arg tag "enterprise" \
            '{
                path: $path,
                content: $content,
...
cleanup_existing_app() {
    if [[ "$CLEANUP" == "true" ]]; then
        log_info "Cleaning up existing podcast transcription assistant '$APP_NAME'..."
...
deploy_app() {
    ...
    log_enterprise "Deploying Enterprise Podcast Transcription Assistant UI..."
    log_info "💰 Revenue Potential: \$8,000 - \$18,000 per project"
    ...
    app_payload=$(echo "$app_definition" | jq \
        ...
        '. + {
            ...
            deployment: {
                ...
                revenue_potential: "$8K-18K",
                target_market: "Podcast Producers, Content Creators, Marketing Agencies"
            }
        }')
    ...
    if echo "$deploy_response" | jq -e '.path' >/dev/null 2>&1; then
        log_success "🎉 Enterprise Podcast Transcription Assistant deployed successfully!"
        APP_URL="$WINDMILL_URL/apps/$WORKSPACE/$APP_NAME"
        log_enterprise "Application URL: $APP_URL"
    else
        log_error "Failed to deploy podcast transcription assistant '$APP_NAME'"
        log_verbose "Deploy response: $deploy_response"
        exit 1
    fi
}
...
show_deployment_summary() {
    echo ""
    echo "==================================================="
    echo "🎙️ ENTERPRISE PODCAST TRANSCRIPTION DEPLOYED"
    echo "==================================================="
    echo ""
    echo "💰 Revenue Potential: \$8,000 - \$18,000 per project"
    echo "🎯 Target Market: Podcast Producers, Content Creators, Marketing Agencies"
    echo ""
    ...
    if [[ "$SCRIPTS_ONLY" != "true" && "$UI_ONLY" != "true" ]]; then
        ...
        echo "🎛️ UI Components: Complete podcast transcription platform"
    ...
    echo "   🎤 High-Accuracy Podcast Transcription with Speaker ID"
    echo "   📝 Multi-Format Content Generation (10+ formats)"
    echo "   🚀 Automated Publishing Workflows & Scheduling"
    echo "   📊 Performance Analytics & Content Insights"
    echo "   🎨 Brand Voice Consistency Management"
    echo "   🔄 Batch Processing & Workflow Automation"
    echo "   💼 Enterprise Project Management Dashboard"
    echo ""
    ...
    echo "   2. Upload your first podcast episode"
    echo "   3. Configure transcription settings and brand profile"
    echo "   4. Generate multi-format content automatically"
    echo "   5. Set up publishing workflows and content calendar"
    echo "   6. Monitor performance with built-in analytics"
    echo ""
    echo "💼 BUSINESS OPPORTUNITIES:"
    echo "   • Podcast content automation and repurposing"
    echo "   • Marketing agency podcast services"
    echo "   • Content creation workflow optimization"
    echo "   • Multi-platform content distribution"
    echo "   • Brand consistency management at scale"
    echo ""
}

# Main deployment process
main() {
    echo "🎙️ Starting Enterprise Podcast Transcription Assistant Deployment"
    echo "💰 Revenue Potential: \$8,000 - \$18,000 per project"
