    ...
    case "$(uname -s)" in
        ...
        Linux*)
            ...
            if command -v apt-get &>/dev/null; then
                ...
                # Only try to install if we have sudo without password
                if sudo -n true 2>/dev/null; then
                    sudo apt-get update || true
                    sudo apt-get install -y tesseract-ocr tesseract-ocr-eng python3-pip python3-venv scrot imagemagick 2>/dev/null || true
                else
                ...
                # RHEL/CentOS
                sudo yum install -y tesseract tesseract-langpack-eng python3-pip scrot ImageMagick 2>/dev/null || true
            else
...
EOF
    ...
    # Create systemd service file (if on Linux with systemd)
    if [[ -d /etc/systemd/system ]] && command -v systemctl &>/dev/null; then
        sudo tee /etc/systemd/system/vocr.service > /dev/null << EOF
[Unit]
...
vocr::uninstall() {
    ...
    if [[ -f /etc/systemd/system/vocr.service ]]; then
        sudo systemctl stop vocr 2>/dev/null || true
        sudo systemctl disable vocr 2>/dev/null || true
        sudo rm /etc/systemd/system/vocr.service
        sudo systemctl daemon-reload
    fi
