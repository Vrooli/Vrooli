...
test_scenario_catalog() {
    ...
    local catalog_file="$PROJECT_ROOT/scripts/scenarios/catalog.json"
    local relative_path
...
test_scenario_validation() {
    ...
    local scenarios_dir="$PROJECT_ROOT/scripts/scenarios"
    ...
    for scenario_dir in "${scenario_dirs[@]}"; do
        ...
        local validation_script="$PROJECT_ROOT/scripts/scenarios/tools/validate-scenario.sh"
...
test_converted_scenarios() {
    ...
    for app_dir in "${generated_apps[@]}"; do
        ...
        if test_app_is_current "$app_dir"; then
            ...
            local integration_test="$PROJECT_ROOT/scripts/scenarios/tools/run-integration-test.sh"
...
test_app_is_current() {
    ...
    if [[ -f "$hash_file" ]]; then
        ...
        local app_name
        app_name=$(basename "$app_dir")
        local source_scenario_dir="$PROJECT_ROOT/scripts/scenarios/core/$app_name"
...
main() {
    if is_dry_run; then
        ...
        log_info "Would test scenario catalog: $PROJECT_ROOT/scripts/scenarios/catalog.json"
