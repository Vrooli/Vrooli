#!/usr/bin/env bash
# Shared unit test helpers (validate_all + runner bindings)
set -euo pipefail

SHELL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
UNIT_TEST_DIR="${SHELL_DIR}/../unit"
...
testing::unit::_cleanup_work_dir() {
    local work_dir="${TESTING_UNIT_WORK_DIR:-}"
    if [ -n "$work_dir" ]; then
        rm -rf "$work_dir"
        local parent_dir
        parent_dir=$(dirname "$work_dir")
        rmdir "$parent_dir" 2>/dev/null || true
        unset TESTING_UNIT_WORK_DIR
    fi
    ...
    unset TESTING_NODE_COVERAGE_DIR
}
...
testing::unit::run_all_tests() {
    ...
    local unit_work_dir="$scenario_root/coverage/.work/unit"
    rm -rf "$unit_work_dir"
    mkdir -p "$unit_work_dir"
    export TESTING_UNIT_WORK_DIR="$unit_work_dir"
    trap testing::unit::_cleanup_work_dir RETURN
    ...
    if [ "$skip_node" = false ]; then
        ...
        if testing::unit::run_node_tests $node_args; then
            ...
            if [ "${TESTING_NODE_COVERAGE_COLLECTED:-}" = "true" ]; then
                ...
                if [ -n "${TESTING_NODE_COVERAGE_DIR:-}" ] && [ -d "${TESTING_NODE_COVERAGE_DIR}" ]; then
                    local node_report_dir="$node_dest_dir/report"
                    rm -rf "$node_report_dir"
                    mkdir -p "$node_report_dir"
                    cp -R "${TESTING_NODE_COVERAGE_DIR}/." "$node_report_dir"
                fi
            fi
        else
            ((error_count++)) || true
...
export -f testing::unit::validate_all
