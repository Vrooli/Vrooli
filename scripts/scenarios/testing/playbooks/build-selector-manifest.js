...
const resolveSelectorsPath = (scenarioDir) => {
  ...
  for (const dirName of SELECTOR_DIR_CANDIDATES) {
    const resolved = path.resolve(scenarioDir, 'ui', 'src', dirName, 'selectors.ts');
    tried.push(resolved);
...
const writeManifest = (selectorsPath, payload) => {
  const manifestDir = path.dirname(selectorsPath);
  const manifestPath = path.join(manifestDir, MANIFEST_FILENAME);
  const envelope = {
    schemaVersion: '2025.11',
    generatedAt: new Date().toISOString(),
    _note: '⚠️  DO NOT EDIT THIS FILE MANUALLY! This file is auto-generated from selectors.ts during the testing process. To add or modify selectors, update the literalSelectors or dynamicSelectorDefinitions objects in ui/src/{consts|constants}/selectors.ts instead.',
    ...payload,
  };
...
try {
  ...
  const manifestPath = writeManifest(selectorsPath, payload);
