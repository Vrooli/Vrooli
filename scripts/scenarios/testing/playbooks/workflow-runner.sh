...
_testing_playbooks__name_from_path() {
...
_testing_playbooks__project_root() {
...
_testing_playbooks__ensure_project() {
    local api_base="$1"
    ...
    if [ -z "$project_id" ]; then
        echo "❌ Unable to resolve project ID for workflow automation" >&2
        return 1
    fi
...
_testing_playbooks__import_workflow() {
    local api_base="$1"
    local project_id="$2"
    local folder_path="$3"
    ...
    if [ -z "$workflow_id" ]; then
        echo "❌ Failed to import workflow" >&2
        printf '%s\n' "$resp" >&2
        return 1
    fi
...
_testing_playbooks__delete_workflow() {
    local api_base="$1"
    local project_id="$2"
    local workflow_id="$3"
...
_testing_playbooks__execute_workflow() {
    local api_base="$1"
    local workflow_id="$2"
...
testing::playbooks::run_workflow() {
    ...
    if [ -z "$workflow_path" ] && [ -z "$workflow_id_input" ]; then
        echo "testing::playbooks::run_workflow requires --file or --workflow-id" >&2
        return 1
    fi
    ...
    if [ -n "$workflow_path" ] && [ -z "$workflow_id_input" ]; then
        ...
        local execution_id
        if execution_id=$(_testing_playbooks__execute_adhoc_workflow "$api_base" "$workflow_path" 2>&1); then
    ...
    if [ -n "$workflow_path" ]; then
        workflow_name=$(_testing_playbooks__name_from_path "$workflow_path")
        local project_folder
        project_folder=$(_testing_playbooks__project_root)
        local project_id
        project_id=$(_testing_playbooks__ensure_project "$api_base" "Testing Harness" "$project_folder") || {
            if [ "$started_scenario" = true ]; then
                vrooli scenario stop "$scenario_name" >/dev/null 2>&1 || true
            fi
            return 1
        ...
        workflow_id=$(_testing_playbooks__import_workflow "$api_base" "$project_id" "$folder_path" "$workflow_path" "$workflow_name") || {
            if [ "$started_scenario" = true ]; then
                vrooli scenario stop "$scenario_name" >/dev/null 2>&1 || true
            fi
            return 1
    ...
    local execution_id
    execution_id=$(_testing_playbooks__execute_workflow "$api_base" "$workflow_id") || {
        if [ "$started_scenario" = true ]; then
            vrooli scenario stop "$scenario_name" >/dev/null 2>&1 || true
        fi
        return 1
    ...
    if ! execution_summary=$(_testing_playbooks__wait_for_execution "$api_base" "$execution_id" 360); then
        ...
        if [ "$keep_workflow" = false ] && [ "$created_workflow" = true ] && [ -n "$imported_project_id" ]; then
            _testing_playbooks__delete_workflow "$api_base" "$imported_project_id" "$workflow_id"
        fi
        if [ "$started_scenario" = true ]; then
            vrooli scenario stop "$scenario_name" >/dev/null 2>&1 || true
        fi
    ...
    TESTING_PLAYBOOKS_LAST_WORKFLOW_ID="$workflow_id"
    TESTING_PLAYBOOKS_LAST_EXECUTION_ID="$execution_id"
    TESTING_PLAYBOOKS_LAST_SCENARIO="$scenario_name"
