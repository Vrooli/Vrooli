...
check_database_health() {
    ...
    if command -v pg_isready >/dev/null 2>&1; then
        if PGPASSWORD=postgres pg_isready -h localhost -p 5433 -U postgres -d "$db_name" >/dev/null 2>&1; then
            ...
            local start_time=$(date +%s%3N)
            if PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -d "$db_name" -c "SELECT COUNT(*) FROM audio_intelligence_platform.app_config;" >/dev/null 2>&1; then
                local end_time=$(date +%s%3N)
...
start_monitoring() {
    ...
    # Verify we have the expected resources for audio intelligence
