#!/bin/bash
# Validation script for Podcast Transcription Assistant
...
SCENARIO_ID="podcast-transcription-assistant"
SCENARIO_NAME="Podcast Transcription Assistant"
...
validate_ollama() {
    ...
    for model in "${required_models[@]}"; do
        if echo "$models_response" | jq -r '.models[].name' | grep -q "$model"; then
            ...
            log_error "✗ Model '$model' missing"
        fi
    done
    
    # Test generation endpoint
    local test_payload='{"model": "llama3.1:8b", "prompt": "Test", "stream": false}'
    if curl -sf -X POST http://localhost:11434/api/generate \
        -H "Content-Type: application/json" \
        -d "$test_payload" >/dev/null 2>&1; then
        log_success "✓ Ollama generation endpoint responds"
    else
        log_warning "Ollama generation endpoint test failed"
    fi
