...
startup::load_configuration() {
    ...
    if [[ ! -f "$SCENARIO_DIR/.vrooli/service.json" ]]; then
        startup::log_error "service.json not found in $SCENARIO_DIR/.vrooli/"
        exit 1
    fi
        ...
        case "$resource" in
            ...
            "searxng")
                if ! curl -sf "http://localhost:$(resources::get_default_port "searxng")/" >/dev/null 2>&1; then
                    ...
                    startup::log_success "✓ SearXNG is healthy"
                fi
                ;;
            "unstructured-io")
                if ! curl -sf "http://localhost:$(resources::get_default_port "unstructured-io")/healthcheck" >/dev/null 2>&1; then
                    ...
                    startup::log_success "✓ Unstructured-IO is healthy"
                fi
                ;;
            "browserless")
                if ! curl -sf "http://localhost:$(resources::get_default_port "browserless")/pressure" >/dev/null 2>&1; then
                    ...
                    startup::log_success "✓ Browserless is healthy"
                fi
                ;;
            *)
...
startup::main() {
    ...
    if [[ "$REQUIRED_RESOURCES" =~ "n8n" ]]; then
        startup::log_info "  📡 Webhook: http://localhost:$(resources::get_default_port "n8n")/webhook/${SCENARIO_ID}-webhook"
    fi
    
    if [[ "$REQUIRES_UI" == "true" && "$REQUIRED_RESOURCES" =~ "windmill" ]]; then
        startup::log_info "  🖥️  UI: http://localhost:$(resources::get_default_port "windmill")/app/$SCENARIO_ID"
    fi
    
    if [[ "$REQUIRED_RESOURCES" =~ "postgres" ]]; then
        startup::log_info "  🗄️  Database: postgresql://postgres:postgres@localhost:$(resources::get_default_port "postgres")/${SCENARIO_ID//-/_}"
    fi
