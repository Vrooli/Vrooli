#!/bin/bash
# ====================================================================
# Enterprise Research Assistant - Windmill Deployment Script
# ====================================================================
# 
# This script deploys the complete Enterprise Research Assistant
# to a running Windmill instance, including all backend scripts and the
# professional UI for strategic consultants and market analysts.
#
# Revenue Potential: $12,000 - $20,000 per project
# Target Market: Strategic consulting firms, market research agencies
#
# Usage:
#   ./deploy-research-assistant.sh [options]
#
# Options:
...
#   --app-name NAME        Application name (default: enterprise-research-assistant)
...
APP_DEFINITION_FILE="$SCRIPT_DIR/enterprise-research-assistant-app.json"
...
APP_NAME="${APP_NAME:-enterprise-research-assistant}"
...
show_help() {
    echo "🔍 Enterprise Research Assistant - Windmill Deployment"
    echo "💰 Revenue Potential: \$12,000 - \$20,000 per project"
    echo ""
    ...
    echo "  --app-name NAME        Application name (default: enterprise-research-assistant)"
    ...
    echo "  🔍 Privacy-Respecting Multi-Engine Search"
    echo "  🤖 AI-Powered Content Analysis & Insights"
    echo "  🗄️ Intelligent Knowledge Base Management"
    echo "  📄 Professional Report Generation"
    echo "  👥 Multi-Project Team Collaboration"
    echo "  🔒 Enterprise Security & Access Controls"
    echo "  📊 Custom Research Dashboards"
}
...
check_service_dependencies() {
    ...
    if [[ $available_services -lt 3 ]]; then
        ...
        log_warning "For full enterprise functionality, ensure SearXNG, Ollama, and Qdrant are running."
    fi
}
...
ensure_workspace() {
    ...
    if echo "$workspace_response" | jq -e '.id' >/dev/null 2>&1; then
        ...
        create_response=$(curl -s -X POST \
            ...
            -d "{\"id\":\"$WORKSPACE\",\"name\":\"Enterprise Research Assistant - $WORKSPACE\"}" || echo '{"error":"workspace_creation_failed"}')
...
set_workspace_variables() {
    ...
    for var_info in "${variables[@]}"; do
        ...
        var_response=$(curl -s -X POST \
            ...
            -d "{\"path\":\"$var_name\",\"value\":\"$var_value\",\"is_secret\":false,\"description\":\"Enterprise Research Assistant - $var_name\"}" || echo '{"error":"variable_set_failed"}')
    done
...
deploy_scripts() {
    ...
    local script_files=(
        "research_search_engine.ts"
        "ai_research_analyzer.ts"
    )
    ...
    for script_file in "${script_files[@]}"; do
        ...
        script_payload=$(jq -n \
            ...
            --arg description "Enterprise Research Assistant - $script_name (Revenue: \$12K-20K)" \
            --arg tag "enterprise" \
            '{
                path: $path,
                content: $content,
...
cleanup_existing_app() {
    if [[ "$CLEANUP" == "true" ]]; then
        log_info "Cleaning up existing research assistant '$APP_NAME'..."
...
deploy_app() {
    ...
    log_enterprise "Deploying Enterprise Research Assistant UI..."
    log_info "💰 Revenue Potential: \$12,000 - \$20,000 per project"
    ...
    app_payload=$(echo "$app_definition" | jq \
        ...
        '. + {
            ...
            deployment: {
                ...
                revenue_potential: "$12K-20K",
                target_market: "Strategic Consultants, Market Research Agencies"
            }
        }')
    ...
    if echo "$deploy_response" | jq -e '.path' >/dev/null 2>&1; then
        log_success "🎉 Enterprise Research Assistant deployed successfully!"
        APP_URL="$WINDMILL_URL/apps/$WORKSPACE/$APP_NAME"
        log_enterprise "Application URL: $APP_URL"
    else
        log_error "Failed to deploy research assistant '$APP_NAME'"
        log_verbose "Deploy response: $deploy_response"
        exit 1
    fi
}
...
show_deployment_summary() {
    echo ""
    echo "=================================================="
    echo "🔍 ENTERPRISE RESEARCH ASSISTANT DEPLOYED"
    echo "=================================================="
    echo ""
    echo "💰 Revenue Potential: \$12,000 - \$20,000 per project"
    echo "🎯 Target Market: Strategic Consultants, Market Research Agencies"
    echo ""
    ...
    if [[ "$SCRIPTS_ONLY" != "true" && "$UI_ONLY" != "true" ]]; then
        ...
        echo "🎛️ UI Components: Complete research platform"
    ...
    echo "   2. Create your first research project"
    echo "   3. Execute privacy-protected searches"
    echo "   4. Analyze content with AI-powered insights"
    echo "   5. Build comprehensive knowledge base"
    echo "   6. Generate professional research reports"
    echo ""
    echo "💼 BUSINESS OPPORTUNITIES:"
    echo "   • Strategic consulting research automation"
    echo "   • Market intelligence gathering and analysis"
    echo "   • Competitive intelligence workflows"
    echo "   • Investment research and due diligence"
    echo "   • Academic and scientific research support"
    echo ""
}

# Main deployment process
main() {
    echo "🔍 Starting Enterprise Research Assistant Deployment"
    echo "💰 Revenue Potential: \$12,000 - \$20,000 per project"
