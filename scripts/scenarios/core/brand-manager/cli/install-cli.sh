#!/usr/bin/env bash
# Install brand-manager CLI globally
set -euo pipefail
...
INSTALL_PATH="/usr/local/bin/brand-manager"
...
# Install the CLI
echo "📦 Installing brand-manager CLI to $INSTALL_PATH..."
...
if [[ -x "$INSTALL_PATH" ]]; then
    ...
    echo "🔧 Test with: brand-manager help"
    
    # Show version info
    if command -v brand-manager >/dev/null 2>&1; then
        echo ""
