#!/bin/bash
# Startup script for Intelligent Desktop Assistant
...
SCENARIO_ID="intelligent-desktop-assistant"
SCENARIO_NAME="Intelligent Desktop Assistant"
...
initialize_database() {
    if [[ "$REQUIRED_RESOURCES" =~ "postgres" ]]; then
        ...
        if [[ -f "$schema_file" ]]; then
            ...
            sed "s/intelligent-desktop-assistant/$db_name/g" "$schema_file" | \
        ...
        if [[ -f "$seed_file" ]]; then
            ...
            # Simple template variable substitution
            sed "s/intelligent-desktop-assistant/$db_name/g" "$seed_file" | \
            psql -h localhost -p "$(resources::get_default_port "postgres")" -U postgres -d "$db_name" -v ON_ERROR_STOP=1
