...
INSERT INTO vault-secrets-integration.app_metadata (
...
INSERT INTO vault-secrets-integration.app_config (config_key, config_value, config_type, description) VALUES 
    -- Feature flags
    ...
    ('app_name', '"Vault Secrets Integration"'::jsonb, 'setting', 'Application display name'),
    ...
    ('ui_brand_name', '"Vault Secrets Integration"'::jsonb, 'setting', 'Brand name displayed in UI'),
    ('ui_show_advanced_features', 'false'::jsonb, 'setting', 'Show advanced features in UI');

-- Insert sample user sessions for demonstration
INSERT INTO vault-secrets-integration.user_sessions (
...
INSERT INTO vault-secrets-integration.activity_log (
    ...
    ('app_initialized', 
     ...
     (SELECT id FROM vault-secrets-integration.user_sessions WHERE session_id = 'demo-session-001'),
     'demo-user', 150, 'completed'),
     
    ('configuration_loaded',
     '{"config_count": ' || (SELECT COUNT(*) FROM vault-secrets-integration.app_config) || '}'::jsonb,
     (SELECT id FROM vault-secrets-integration.user_sessions WHERE session_id = 'demo-session-001'),
     'demo-user', 25, 'completed');

-- Insert sample resource metrics for demonstration
INSERT INTO vault-secrets-integration.resource_metrics (
...
INSERT INTO vault-secrets-integration.activity_log (event_type, event_data, session_id, user_identifier, status) VALUES
    ('ai_query_processed', 
...
SELECT 'Application metadata:' as info, COUNT(*) as count FROM vault-secrets-integration.app_metadata
UNION ALL
SELECT 'Configuration entries:', COUNT(*) FROM vault-secrets-integration.app_config
UNION ALL
SELECT 'Sample sessions:', COUNT(*) FROM vault-secrets-integration.user_sessions
UNION ALL
SELECT 'Activity log entries:', COUNT(*) FROM vault-secrets-integration.activity_log
UNION ALL
SELECT 'Resource metrics:', COUNT(*) FROM vault-secrets-integration.resource_metrics;