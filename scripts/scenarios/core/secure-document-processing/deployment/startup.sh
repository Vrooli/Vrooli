#!/bin/bash
# Startup script for Secure Document Processing Pipeline
...
initialize_database() {
    if [[ "$REQUIRED_RESOURCES" =~ "postgres" ]]; then
        ...
        if [[ -f "$schema_file" ]]; then
            ...
            sed "s/secure-document-processing/$db_name/g" "$schema_file" | \
        ...
        if [[ -f "$seed_file" ]]; then
            ...
            # Simple template variable substitution
            sed "s/secure-document-processing/$db_name/g" "$seed_file" | \
            psql -h localhost -p 5433 -U postgres -d "$db_name" -v ON_ERROR_STOP=1
