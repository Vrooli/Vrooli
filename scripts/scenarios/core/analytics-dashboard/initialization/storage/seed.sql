...
INSERT INTO analytics-dashboard.app_metadata (
...
INSERT INTO analytics-dashboard.app_config (config_key, config_value, config_type, description) VALUES 
    -- Feature flags
    ...
    ('app_name', '"Real-time Analytics Dashboard"'::jsonb, 'setting', 'Application display name'),
    ...
    ('ui_brand_name', '"Real-time Analytics Dashboard"'::jsonb, 'setting', 'Brand name displayed in UI'),
    ('ui_show_advanced_features', 'false'::jsonb, 'setting', 'Show advanced features in UI');

-- Insert sample user sessions for demonstration
INSERT INTO analytics-dashboard.user_sessions (
...
INSERT INTO analytics-dashboard.activity_log (
    ...
    ('app_initialized', 
     ...
     (SELECT id FROM analytics-dashboard.user_sessions WHERE session_id = 'demo-session-001'),
     'demo-user', 150, 'completed'),
     
    ('configuration_loaded',
     '{"config_count": ' || (SELECT COUNT(*) FROM analytics-dashboard.app_config) || '}'::jsonb,
     (SELECT id FROM analytics-dashboard.user_sessions WHERE session_id = 'demo-session-001'),
     'demo-user', 25, 'completed');

-- Insert sample resource metrics for demonstration
INSERT INTO analytics-dashboard.resource_metrics (
...
INSERT INTO analytics-dashboard.activity_log (event_type, event_data, session_id, user_identifier, status) VALUES
    ('ai_query_processed', 
...
SELECT 'Application metadata:' as info, COUNT(*) as count FROM analytics-dashboard.app_metadata
UNION ALL
SELECT 'Configuration entries:', COUNT(*) FROM analytics-dashboard.app_config
UNION ALL
SELECT 'Sample sessions:', COUNT(*) FROM analytics-dashboard.user_sessions
UNION ALL
SELECT 'Activity log entries:', COUNT(*) FROM analytics-dashboard.activity_log
UNION ALL
SELECT 'Resource metrics:', COUNT(*) FROM analytics-dashboard.resource_metrics;