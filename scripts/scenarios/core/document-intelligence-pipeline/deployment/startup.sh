#!/bin/bash
# Startup script for Document Intelligence Pipeline
...
initialize_database() {
    if [[ "$REQUIRED_RESOURCES" =~ "postgres" ]]; then
        ...
        if [[ -f "$schema_file" ]]; then
            ...
            sed "s/document-intelligence-pipeline/$db_name/g" "$schema_file" | \
        ...
        if [[ -f "$seed_file" ]]; then
            ...
            # Simple template variable substitution
            sed "s/document-intelligence-pipeline/$db_name/g" "$seed_file" | \
            psql -h localhost -p 5433 -U postgres -d "$db_name" -v ON_ERROR_STOP=1
