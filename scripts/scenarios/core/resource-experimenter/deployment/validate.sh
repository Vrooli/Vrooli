...
validate_storage() {
    ...
    if [[ ${available_gb} -lt 10 ]]; then
        echo "❌ Insufficient disk space. Need at least 10GB, have ${available_gb}GB"
        return 1
    fi
...
validate_memory() {
    ...
    if ! command -v free >/dev/null 2>&1; then
        echo "⚠️  Cannot check memory, assuming sufficient"
        return 0
    fi
    ...
    if [[ ${available_mb} -lt 4096 ]]; then
        echo "❌ Insufficient memory. Need at least 4GB available, have ${available_mb}MB"
        return 1
    fi
    
    echo "✅ Sufficient memory available (${available_mb}MB)"
    return 0
}
...
validate_permissions() {
    ...
    if [[ -S "/var/run/docker.sock" ]] && [[ ! -w "/var/run/docker.sock" ]]; then
        echo "⚠️  No write access to Docker socket - may need to add user to docker group"
    fi
    
    echo "✅ File permissions are adequate"
    return 0
}
...
validate_configuration() {
    ...
    if [[ ! -f "${service_json}" ]]; then
        echo "❌ Missing service.json configuration"
        return 1
    fi
    ...
    echo "✅ Scenario configuration is valid"
    return 0
}

# Main validation function
main() {
    echo "🔍 Resource Experimenter - Environment Validation"
    echo "=============================================="
    echo ""
