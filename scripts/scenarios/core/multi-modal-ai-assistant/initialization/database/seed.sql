...
INSERT INTO multi-modal-ai-assistant.app_metadata (
...
INSERT INTO multi-modal-ai-assistant.app_config (config_key, config_value, config_type, description) VALUES 
    -- Feature flags
    ...
    ('app_name', '"Multi-Modal AI Assistant"'::jsonb, 'setting', 'Application display name'),
    ...
    ('ui_brand_name', '"Multi-Modal AI Assistant"'::jsonb, 'setting', 'Brand name displayed in UI'),
    ('ui_show_advanced_features', 'false'::jsonb, 'setting', 'Show advanced features in UI');

-- Insert sample user sessions for demonstration
INSERT INTO multi-modal-ai-assistant.user_sessions (
...
INSERT INTO multi-modal-ai-assistant.activity_log (
    ...
    ('app_initialized', 
     ...
     (SELECT id FROM multi-modal-ai-assistant.user_sessions WHERE session_id = 'demo-session-001'),
     'demo-user', 150, 'completed'),
     
    ('configuration_loaded',
     '{"config_count": ' || (SELECT COUNT(*) FROM multi-modal-ai-assistant.app_config) || '}'::jsonb,
     (SELECT id FROM multi-modal-ai-assistant.user_sessions WHERE session_id = 'demo-session-001'),
     'demo-user', 25, 'completed');

-- Insert sample resource metrics for demonstration
INSERT INTO multi-modal-ai-assistant.resource_metrics (
...
INSERT INTO multi-modal-ai-assistant.activity_log (event_type, event_data, session_id, user_identifier, status) VALUES
    ('ai_query_processed', 
...
SELECT 'Application metadata:' as info, COUNT(*) as count FROM multi-modal-ai-assistant.app_metadata
UNION ALL
SELECT 'Configuration entries:', COUNT(*) FROM multi-modal-ai-assistant.app_config
UNION ALL
SELECT 'Sample sessions:', COUNT(*) FROM multi-modal-ai-assistant.user_sessions
UNION ALL
SELECT 'Activity log entries:', COUNT(*) FROM multi-modal-ai-assistant.activity_log
UNION ALL
SELECT 'Resource metrics:', COUNT(*) FROM multi-modal-ai-assistant.resource_metrics;