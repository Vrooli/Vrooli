...
| `name` | string | Scenario ID (use `{{SCENARIO_ID}}` placeholder) |
...
```json
{
  ...
  "setup": {
    "steps": [
      { "name": "build-api", "run": "cd api && go build -o {{SCENARIO_ID}}-api ." }
    ]
  },
  "develop": {
    "steps": [
      { "name": "start-api", "run": "cd api && exec ./{{SCENARIO_ID}}-api", "background": true }
    ]
  }
}
```
