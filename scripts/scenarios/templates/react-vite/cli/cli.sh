...
api_request() {
    ...
    if [[ -z "$API_BASE" ]]; then
        echo -e "${RED}✗${NC} API base URL is not configured. Start the scenario via 'vrooli scenario run {{SCENARIO_ID}}' or run '$CLI_NAME configure api_base <url>'." >&2
        return 1
    fi
