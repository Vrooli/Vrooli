...
vault::check_health() {
    ...
    if ! echo "$response_body" | jq -e '.initialized == true and .sealed == false and .standby == false' > /dev/null; then
        ...
        if echo "$response_body" | jq -e '.sealed == true' >/dev/null; then log::error "Reason: Vault is sealed."; fi
        if echo "$response_body" | jq -e '.standby == true' >/dev/null; then log::error "Reason: Vault is in standby mode."; fi
