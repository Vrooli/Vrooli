...
bats::install_dependency() {
    ...
    if [ ! -d "$dir_name" ]; then
        # Clone with proper ownership using helper
        sudo::git_clone_as_user "$repo_url" "$dir_name"
...
bats::install_core() {
    ...
    sudo::restore_owner "$BATS_DEPENDENCIES_DIR" "recursive"
    
    if [ ! -d "bats-core" ]; then
        # Clone with proper ownership using helper
        sudo::git_clone_as_user "https://github.com/bats-core/bats-core.git" "bats-core"
        cd bats-core
