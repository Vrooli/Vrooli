...
# Scenario port allocation (lock files under ~/.vrooli/state/scenarios/,
# ports::allocate_scenario and friends) used to live below this line. That
# layer is retired: the Go CLI's runtime registry claims (SQLite) are the
# single authority for scenario port allocation, and nothing reads or writes
# .port_<N>.lock files anymore. See docs/reference/port-allocation.md.
