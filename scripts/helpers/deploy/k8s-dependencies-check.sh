...
k8s_dependencies::check_operators() {
    ...
    if kubectl get crd postgresclusters.postgres-operator.crunchydata.com > /dev/null 2>&1; then
        ...
        operator_status+="PGO: ✗ Missing - Install with: bash scripts/helpers/setup/target/k8s_cluster.sh\n"
        ((issues++))
    fi
    ...
    if kubectl get crd redisfailovers.databases.spotahome.com > /dev/null 2>&1; then
        ...
        operator_status+="Redis Operator: ✗ Missing - Install with: bash scripts/helpers/setup/target/k8s_cluster.sh\n"
        ((issues++))
    fi
    ...
    if kubectl get crd vaultsecrets.secrets.hashicorp.com > /dev/null 2>&1; then
        ...
        operator_status+="VSO: ✗ Missing - Install with: bash scripts/helpers/setup/target/k8s_cluster.sh\n"
        ((issues++))
    fi
...
k8s_dependencies::generate_setup_instructions() {
    ...
    if [[ "$issues_found" -gt 0 ]]; then
        ...
        echo "   bash scripts/helpers/deploy/k8s-prerequisites.sh --yes"
        echo ""
        
        log::info "2. VAULT SETUP (Development):"
        echo "   Enable Vault for development:"
        echo "   export SECRETS_SOURCE=vault"
        echo "   bash scripts/main/develop.sh --target k8s-cluster"
        echo ""
        ...
        echo "bash scripts/helpers/deploy/k8s-dependencies-check.sh --environment=\$ENVIRONMENT"
        echo ""
    fi
}
...
k8s_dependencies::main() {
    ...
    if [[ $total_issues -eq 0 ]]; then
        ...
        log::info "  bash scripts/main/deploy.sh --source k8s --environment $environment"
        exit 0
    else
