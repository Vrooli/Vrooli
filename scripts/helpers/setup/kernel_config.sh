...
kernel_config::configure_judge0() {
    ...
    if kernel_config::check_parameter "$param" "$required_value"; then
        log::success "✅ Judge0 kernel parameters already configured"
        return 0
    fi
    
    log::warning "⚠️  Judge0 requires unprivileged user namespaces to be enabled"
    ...
    if flow::can_run_sudo "kernel parameter configuration"; then
        if sudo sysctl -w "$param=$required_value" >/dev/null 2>&1; then
            log::success "✅ Applied kernel parameter for current session"
        else
        ...
        if kernel_config::make_persistent "$param" "$required_value" "Judge0 isolate sandbox"; then
            log::success "✅ Configuration saved for persistence across reboots"
        else
...
# Generated by: scripts/helpers/setup/kernel_config.sh
# 
...
kernel_config::configure_for_resources() {
    log::header "🔧 Configuring kernel parameters for resources"
    ...
    if [[ "$configs_applied" == "true" ]]; then
        log::success "✅ Kernel parameters configured for resources"
...
kernel_config::cleanup() {
    log::header "🧹 Cleaning up Vrooli kernel configurations"
    
    if [[ -f "$SYSCTL_VROOLI_CONF" ]]; then
        if flow::can_run_sudo "kernel configuration cleanup"; then
            if sudo rm -f "$SYSCTL_VROOLI_CONF"; then
                log::success "✅ Removed Vrooli kernel configuration file"
