...
secrets::get_project_root() {
    ...
    # Fallback: assume we're in scripts/helpers/utils and go up 3 levels
...
secrets::get_secrets_file() {
    local project_root
    project_root="$(secrets::get_project_root)"
    echo "$project_root/.vrooli/secrets.json"
}

secrets::get_service_file() {
    local project_root
    project_root="$(secrets::get_project_root)"
    echo "$project_root/.vrooli/service.json"
}
...
secrets::source_port_registry() {
    local project_root
    project_root="$(secrets::get_project_root)"
    local port_registry="$project_root/scripts/resources/port-registry.sh"
