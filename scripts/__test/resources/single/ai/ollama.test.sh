...
test_ollama_interface_compliance() {
    ...
    # Run interface compliance test (non-blocking)
    if test_resource_interface_compliance "$TEST_RESOURCE" "$manage_script"; then
        echo "✅ Ollama interface compliance test passed"
        return 0
    else
        echo "⚠️  Ollama interface compliance test failed (continuing with functional tests)"
        ...
        echo "📋 This is a warning - functional tests will continue"
        return 0  # Return success to allow functional tests to continue
    fi
}
