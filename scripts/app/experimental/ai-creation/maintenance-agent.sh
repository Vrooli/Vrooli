...
# Set SKIP_PRIVILEGE_DROP=1 to disable privilege dropping (for containers/testing)
FALLBACK_USER=${FALLBACK_USER:-matt}
SKIP_PRIVILEGE_DROP=${SKIP_PRIVILEGE_DROP:-}

if [[ $EUID -eq 0 && -z "$SKIP_PRIVILEGE_DROP" ]]; then
...
elif [[ $EUID -eq 0 && -n "$SKIP_PRIVILEGE_DROP" ]]; then
  echo "⚠️  Running as root with privilege drop disabled"
fi
