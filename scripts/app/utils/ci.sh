...
1. SSH into your staging server and authorize the staging public key:
   ...
   bash scripts/app/utils/authorize-key.sh --file $var_STAGING_CI_SSH_PUB_KEY_FILE --name "GitHub Actions Staging"

2. SSH into your production server and authorize the production public key:
   ssh <production-user>@<production-host>  
   bash scripts/app/utils/authorize-key.sh --file $var_PRODUCTION_CI_SSH_PUB_KEY_FILE --name "GitHub Actions Production"
