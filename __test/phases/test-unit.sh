#!/usr/bin/env bash
# Unit Testing Phase - Vrooli Testing Infrastructure
# 
# Executes all unit tests (primarily BATS) with caching:
...
# Get APP_ROOT using cached value or compute once (2 levels up: __test/phases/test-unit.sh)
...
show_usage() {
    cat << 'EOF'
Unit Testing Phase - Run all unit tests (BATS) with caching optimization

Usage: ./test-unit.sh [OPTIONS]
...
OPTIONS:
  ...
  --clear-cache     Clear unit test cache before running
...
WHAT THIS PHASE TESTS:
  1. Discovers all .bats unit test files in the project (or scoped subset)
...
SCOPING EXAMPLES:
  ./test-unit.sh --resource=ollama          # Only test ollama BATS files
  ./test-unit.sh --scenario=app-generator   # Only test app-generator BATS files
  ./test-unit.sh --path=resources   # Only test BATS in specific path

EXAMPLES:
  ./test-unit.sh                           # Run all unit tests
  ./test-unit.sh --verbose --parallel      # Run in parallel with details
  ./test-unit.sh --filter "resource"       # Only run tests with "resource" in path
  ./test-unit.sh --no-cache                # Force re-run all tests
  ./test-unit.sh --clear-cache             # Clear cache and re-run
EOF
}
...
discover_bats_tests() {
    ...
    if [[ -n "${SCOPE_RESOURCE:-}" ]] || [[ -n "${SCOPE_SCENARIO:-}" ]] || [[ -n "${SCOPE_PATH:-}" ]]; then
        ...
        done < <(get_scoped_files "*.bats" "$PROJECT_ROOT")
    else
        # No scope - search standard locations
        local search_dirs=(
            "$PROJECT_ROOT/__test"
...
# Main unit testing execution function
run_unit_tests() {
    log_section "Unit Testing Phase"
...
main() {
    if is_dry_run; then
        log_info "🧪 [DRY RUN] Unit Testing Phase"
        log_info "Would discover all .bats files in:"
        log_info "  - $PROJECT_ROOT/__test"
    ...
    if run_unit_tests; then
        log_success "✅ Unit testing phase completed successfully"
        return 0
    else
        log_error "❌ Unit testing phase completed with failures"
        return 1
    fi
}

# Export functions for testing
export -f run_unit_tests check_bats_availability discover_bats_tests
