...
> **⚠️ WARNING**: CLI tests using BATS have unique safety risks. A misconfigured teardown can delete your entire project. Read [Safety Guidelines](../safety/GUIDELINES.md) before proceeding.
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run safety linter
        run: scripts/scenarios/testing/lint-tests.sh test/
      
      - name: Run CLI tests
        run: bats --tap test/cli/*.bats
```
