...
- **Unattached team.** A team appearing in no row is doing work that traces to no stated outcome. All six currently appear. `meta-optimization` and `infra-health` appear only as supporting contributors by design — they are second-order teams whose output is the capacity of the other four, and their own first-order targets live in `path:docs/agent-system/FRAMEWORK_HEALTH.md` and `path:docs/infra-health/strategy/RELIABILITY_TARGETS.md` respectively.
