...
- Gap-tracked metrics: every metric in Command Center is tagged `live` / `partial` / `gap` via the `/api/v1/gaps` endpoint. Gaps become backlog signals — [`director-dashboard-gap-workflow`](ROADMAP.md) wires this loop.
...
Outcomes are organized by the six Command Center dashboard pages (see [`command-center-dashboards`](ROADMAP.md) for the live plan). Each page is a category.
...
- Authoritative metrics: `pending-command-center`
- Cross-reference: [monetization catalog](../monetization/CATALOG.md) for SKU-to-scenario mapping.
...
- Authoritative metrics: `pending-command-center`
- Cross-reference: [monetization/REVENUE_LINES.md](../monetization/REVENUE_LINES.md) for the instrumentation contract each active revenue line must meet.
...
- Authoritative metrics: `pending-command-center`
- Cross-reference: [monetization/FUNNEL.md](../monetization/FUNNEL.md).
...
- **Does not replace the monetization team's financial model.** For revenue math, see [monetization/FINANCIAL_MODEL.md](../monetization/FINANCIAL_MODEL.md).
