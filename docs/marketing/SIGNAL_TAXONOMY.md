...
Cross-team-readable canon for how marketing research signals are partitioned, classified, dispatched, and shaped. This file is the human-readable view of `docs/marketing/signal-taxonomy.json` (the parseable sidecar consumed by the heartbeat builder and the `unknown_taxonomy` / `missing_destination_schema` validation rules).
