...
Some marketing knowledge is neither tied to one post-type (so it doesn't belong under [`post-types/`](../post-types/)) nor a cross-cutting voice/structure rule (so it doesn't belong under [`post-techniques/`](../post-techniques/)). Examples: *AI-UGC personas* (a whole class of marketing approach with its own rules), *hook library* (a curated list of hooks, not itself a technique), *funnel patterns* (multi-step flows from post to product).
...
- [`../post-types/`](../post-types/) — strategies often inform multiple post-types; each post-type references the strategies it depends on.
- [`../post-techniques/`](../post-techniques/) — voice and structure techniques. A strategy may invoke multiple techniques.
...
- [`../rich-media/`](../rich-media/) — schemas for character/scene/product data that AI-UGC content depends on.
