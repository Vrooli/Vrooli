...
2. **Embed Source-Aware Metadata in Requirement Files**
   ...
   - **Drop legacy single-file registries**: Remove `docs/requirements.json` handling from discovery, validators, docs, and templates so every scenario uses the modular `requirements/` layout required for folder-based operational-target rollups.
...
1. **Evidence Loader Upgrades**
   - Extend `scripts/requirements/lib/evidence.js` so it ingests three sources **and tags each requirement evidence entry with the relative JSON file that produced it** (so `_sync_metadata.phase_results` can reference concrete artifacts):
     1. Existing `coverage/phase-results/*.json` (tests + automation) with relative file tracking.
     2. `coverage/phase-results/lighthouse.json` (Lighthouse runner) with page/score metadata.
     3. A new manual validation manifest (`coverage/manual-validations/log.jsonl`) produced by the CLI.
...
3. **Schema & `_sync_metadata` Enhancements**
   ...
   - Remove `docs/requirements.json` handling from discovery/validators/templates to fully standardize on the modular registry.

4. **Scenario Snapshot Writer**
   - Extend `report.js` sync mode to write `coverage/<scenario>/requirements-sync/latest.json` with:
