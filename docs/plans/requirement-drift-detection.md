...
## Progress Log
- **2025-11-20:** Added test command capture + `_sync_metadata` provenance during auto-sync and introduced the first version of the scenario snapshot writer. Each `report.js --mode sync` run now records the executed commands, logs a manifest entry via `_testing_suite_sync_requirements`, and emits `coverage/requirements-sync/latest.json` with hashes, requirement metadata, and operational-target rollups. Documentation updated (`docs/testing/architecture/REQUIREMENT_FLOW.md`, `docs/testing/guides/requirement-tracking.md`) accordingly.
- **2025-11-21:** Implemented drift detection in `scenario status`. `scripts/prd/parser.js` exposes the operational-target parser, `scripts/requirements/drift-check.js` compares requirement hashes + PRD status + evidence timestamps, and `scenario::requirements::quick_check` surfaces the warnings along with remediation advice. Requirement tracking docs now describe the new guardrails.
- **2025-11-22:** Landed the manual validation workflow. `vrooli scenario requirements manual-log` appends signed entries to `coverage/manual-validations/log.jsonl`, `report.js --mode sync` ingests them, embeds `_sync_metadata.manual` on validations, and captures the manifest snapshot so `drift-check`/`scenario status` warn about expired or unsynced manual coverage. Docs, prompts, and the template README now point agents to the CLI instead of editing requirements by hand.
- **2025-11-23:** Auto-sync now updates the PRD operational target checkboxes directly from the requirement rollups, eliminating the last excuse for manual checkbox flipping. The snapshot writer returns its payload so `report.js --mode sync` can compare operational-target states, patch `PRD.md`, and record which targets changed. A new helper (`vrooli scenario requirements snapshot`) summarizes `coverage/requirements-sync/latest.json`, including executed test commands and expiring manual validations, so agents can verify provenance without spelunking JSON.
- **2025-11-24:** Locked down snapshot generation so `_testing_suite_sync_requirements` refuses to call `report.js --mode sync` when any required phase was skipped (manual invocations must pass `--allow-partial-sync`/`REQUIREMENTS_SYNC_ALLOW_PARTIAL=1`). Artifact cleanup now treats `coverage/<scenario>/requirements-sync/` as reserved, new CLI helper `vrooli scenario requirements lint-prd` enforces PRD ↔ requirement mapping, and generator/improver prompts reference both the snapshot + lint helpers so agents know where to inspect evidence.
