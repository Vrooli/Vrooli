...
- **Delete** `path:scripts/migrate_candidates/tools/buf.sh` once the Go handler lands. Do not preserve it as a fallback or leave a "use this if Go handler fails" path.
...
`vrooli-onboarding` v2 acceptance criterion **OT-V2-FEATURE-COMPLETE** requires the wizard to cover *every* integration documented in `path:docs/configuration/integrations/`. BSR login is not currently documented there, so even after the v2 rework lands, operators of a fresh clone get no guidance about it. We need to add the per-integration documentation page **before** the v2 rework runs, so it gets picked up by the contract-driven wizard automatically.

## 5. Scope

### In scope

- Switch `path:packages/proto/buf.gen.yaml` from 5 `remote:` plugins to 5 `local:` plugins.
...
- Replace bash installer `path:scripts/migrate_candidates/tools/buf.sh` with a Go handler `path:internal/tools/buf/handler.go`.
...
- Add `path:docs/configuration/integrations/buf-bsr.md` declaring BSR login as an `external_sign_in_command` integration so onboarding's V2 wizard picks it up automatically.
...
- Update `path:docs/configuration/host/tools.md` cross-references to point at the new plugin tools.
...
- Implementing the `integration-hub` scenario (deferred per `path:docs/configuration/integrations/README.md`). The buf-bsr connector page is added speculatively-but-deliberately because OT-V2-FEATURE-COMPLETE makes the docs the contract; the wizard step lights up when integration-hub ships.
- Migrating other scenarios' protobuf usages — there is one buf module (`path:packages/proto/`) and everything routes through it.
...
- Pinning Buf CLI to a specific minor version beyond what `path:internal/tools/buf/tool.json` already does (currently floats; we'll add a pin as part of this work).
...
|---|---|---|
| buf module config | `path:packages/proto/buf.yaml` | v2 module, two BSR deps (`googleapis`, `protovalidate`), lint default w/ exceptions |
| Codegen config | `path:packages/proto/buf.gen.yaml` | 5 `remote:` plugins, 2 `module:` inputs, 1 `directory:` input |
| Generated code | `path:packages/proto/gen/{go,typescript,python}` | committed, validated by `make check` diff |
| Codegen entry | `path:packages/proto/Makefile` | `make generate`, `make lint`, `make breaking`, `make check` |
| buf binary install | `path:internal/tools/buf/tool.json` (manifest only, no handler) + `path:scripts/migrate_candidates/tools/buf.sh` (bash, downloads GitHub release v1.37.0) | Fragmented; bash script not registered with Vrooli's tool runtime |
| Host-tool registry | `path:internal/tools/<name>/tool.json` + optional `path:internal/tools/<name>/handler.go`, registered in `path:internal/runtime/registry.go` `customToolHandlers` map, drift-tested by `TestToolManifestsReferenceRegisteredHandlers` | Pattern is mature (`cloudflared`, `stripe`, `vault` all use it). Cross-platform: linux/darwin/windows |
| Project-level required tools | `.vrooli/service.json` `hostTools[]` | Has `git`, `curl`, `jq`, `yq`, `docker`, `go`, `node`. **Does not include `buf`.** |
| Validation flow that hit the rate limit | `path:internal/cli/scenariohandlers/template_runtime.go:813` (`validateRelocationProtoSources`) calls `buf lint` (not generate) but the rate-limit hit happened during full template iteration that also runs `buf generate` | `buf lint` does not contact BSR for plugins, only for module deps if cache-miss |
| Configuration substrate the wizard reads | `path:docs/configuration/` (README, host/, integrations/, secrets.md, scenarios.md, etc.) — **the wizard is contractually bound to implement what's documented here** (PRD OT-V2-FEATURE-COMPLETE) | Does not currently document buf BSR |
| Existing `external_sign_in_command` documentation | `path:docs/configuration/integrations/external-auth.md` lines 78-103 — exactly the pattern that fits buf, claude-code, codex, gh, cloudflared, stripe | Pattern is settled; needs a per-integration page |
| Onboarding wizard plan | `path:scenarios/vrooli-onboarding/docs/WIZARD_FLOW.md` Step 5 | Empty until integration-hub ships, but **picks up integration pages automatically** |
...
- `buf.gen.yaml`:
  ...
  plugins:
    ...
    - local: protoc-gen-pyi
      out: gen/python
  ```
- `path:packages/proto/vendor/{googleapis,protovalidate}/` contains the exported BSR modules used by Vrooli, refresh script documented.
- `path:packages/proto/buf.yaml` `deps:` is removed (the `directory:` inputs replace the BSR module fetches).

### Host tools

- `path:internal/tools/buf/` — `tool.json` (with `handler: "buf"`) + `handler.go` + `handler_test.go`. Cross-platform: downloads GitHub release binary on linux/darwin/windows.
- `path:internal/tools/protoc-gen-go/` — same shape; installs via `go install google.golang.org/protobuf/cmd/protoc-gen-go@<pinned>` using the `go` host tool already required.
- `path:internal/tools/protoc-gen-es/` — same shape; installs the npm package `@bufbuild/protoc-gen-es@<pinned>` to a vendored `node_modules/.bin/` location and adds it to `PATH` (via setup symlink), or downloads the published-binary build if upstream provides one.
- `path:internal/tools/protoc-gen-python/` — same shape; installs via `pip install --user protobuf==<pinned>` (which ships `protoc-gen-python` and `protoc-gen-pyi`).
- `path:internal/runtime/registry.go` `customToolHandlers` registers all four.
- `.vrooli/service.json` `hostTools[]` adds entries for all four with `required: true`, `when: ["setup", "develop"]`.
- `path:scripts/migrate_candidates/tools/buf.sh` is **deleted** (greenfield rule — see §3).

### BSR login surface

- `path:docs/configuration/integrations/buf-bsr.md` exists, declaring:
...
- `vrooli auth status` command exists in `path:cmd/vrooli/` (or the appropriate root CLI domain), reporting per-tool sign-in state, designed to be extended for `claude`, `codex`, `gh`, `cloudflared`. Output is human-friendly by default, `--json` available for scripting.
- The command is referenced from `path:docs/configuration/host/tools.md` and from `buf-bsr.md` so future onboarding picks it up.

### Onboarding alignment

- No code change in `path:scenarios/vrooli-onboarding/`. The integration page in `path:docs/configuration/integrations/buf-bsr.md` is enough — when V2 wizard runs, Step 5 reads the integrations folder and renders a card for buf-bsr automatically. (V2 ships separately; this plan only ensures the contract entry exists.)

### Documentation

- `path:docs/development/proto.md` (new) — codegen pipeline, why local plugins, how to refresh vendored modules, troubleshooting.
- `path:docs/configuration/host/tools.md` — cross-reference the new plugin tools.
- `path:templates/scenarios/react-vite/` — README/PRD reference updated to point at `path:docs/development/proto.md`.

## 8. Contract Decisions

### CD-1: Local plugins are the only plugin path

`remote:` plugins are forbidden in `path:packages/proto/buf.gen.yaml`. A test (§9 T-2) greps the file and fails CI on any `remote:` line. Future plugins added must follow the local-handler pattern.

### CD-2: BSR module deps are vendored, not fetched

Codegen reads from `path:packages/proto/vendor/googleapis/` and `path:packages/proto/vendor/protovalidate/` — the BSR `module:` input form is removed. A documented refresh script (`path:packages/proto/scripts/refresh-vendor.sh` or `make refresh-vendor`) regenerates the vendor tree for occasional bumps; this is the **one** code path that requires BSR access (and can run logged-in or anonymously).

### CD-3: Plugin versions are pinned and tracked centrally

Each plugin's pinned version lives in **two places**: the tool manifest's `version` field (e.g. `path:internal/tools/protoc-gen-go/tool.json#/version`) and the install command in the handler. Test T-3 asserts they match. Bumping a plugin = updating the manifest + handler in one PR; the `make check` proto-diff gate catches bit-level drift.
...
Per `cli-steer`: human-default output, `--json` available, built on `cli-core`'s `cliapp` scaffolding. The command lives in `path:internal/cli/<...>/` Go-only — no bash. Lives at the **vrooli root CLI** (not a scenario CLI) because sign-in state is a host-level concern.
...
3. Add `path:packages/proto/scripts/refresh-vendor.sh` (Go would be nicer but a 30-line shell helper is acceptable for this single tooling script — alternative: tiny `path:cmd/refresh-proto-vendor/main.go`; pick Go to honor the cross-platform constraint).
...
1. **Phase B.1 — `protoc-gen-go`**:
   - Add `path:internal/tools/protoc-gen-go/tool.json` (`handler: "protoc_gen_go"`, `version: "v1.36.x"`, `commands: ["protoc-gen-go"]`, `versionArgs: ["--version"]`).
   - Add `path:internal/tools/protoc-gen-go/handler.go` with `Inspect`/`Apply` mirroring `cloudflared/handler.go` shape; install via `go install google.golang.org/protobuf/cmd/protoc-gen-go@<version>` with `GOBIN` set to a registry-known location (e.g. `$XDG_BIN_HOME` falling back to `~/.local/bin`).
   ...
   - Register in `path:internal/runtime/registry.go` `customToolHandlers["protoc_gen_go"] = protocgengo.NewHandler`.
...
4. **Phase B.4 — `buf` handler upgrade**:
   - Add `path:internal/tools/buf/handler.go` (currently only `tool.json` exists).
   - Replace shell installer with Go handler that downloads `buf-<OS>-<ARCH>` from GitHub releases (matches `path:scripts/migrate_candidates/tools/buf.sh` behavior) — pin to v1.37.0 in `tool.json#/version`.
   - Update `path:internal/tools/buf/tool.json` to set `handler: "buf"` and `version: "v1.37.0"`.
   - Promote `buf` to `.vrooli/service.json` `hostTools[]` with `required: true`.
   - **Delete** `path:scripts/migrate_candidates/tools/buf.sh` (greenfield rule).
   - Add `handler_test.go`.
...
4. Add a guard test under `path:packages/proto/scripts_test.go` (or appropriate test location) that asserts `buf.gen.yaml` contains zero `remote:` lines (CD-1).
...
1. Add `path:docs/configuration/integrations/buf-bsr.md` following the per-integration page pattern in `path:docs/configuration/integrations/README.md`. Page sections (mirroring `video-providers.md`):
...
2. Cross-link from `path:docs/configuration/integrations/external-auth.md` "Examples" line and from `path:docs/configuration/integrations/README.md` "Existing per-integration pages" list.
3. Cross-link from `path:docs/configuration/host/tools.md` (proto plugins reference buf-bsr for context).

**Stop / acceptance:** `path:docs/configuration/integrations/buf-bsr.md` exists; lint passes (`lychee` for link-checking already in repo).

### Phase E: `vrooli auth status` command (1 commit)

1. Decide command location — most likely a new domain in the root vrooli CLI (`path:internal/cli/authcli/` or similar) since this is a host-level concern, not scenario-specific. Per `cli-steer` Section 4, follow the standard structure (`domains/auth/{register,status,output}.go`).
...
1. Add `path:docs/development/proto.md` covering:
...
2. Update `path:templates/scenarios/react-vite/` proto-related references to point here.
3. Add a brief mention in repo-root `path:docs/README.md` index.

**Stop / acceptance:** `path:docs/development/proto.md` linked from at least 2 other docs; lychee link-check green.

## 10. Testing Plan

| ID | Test | Where | What it asserts |
|---|---|---|---|
| T-1 | Manifest-handler invariant | `path:internal/runtime/registry_test.go` (existing `TestToolManifestsReferenceRegisteredHandlers`) | New tool.json files reference handlers registered in `customToolHandlers` |
| T-2 | No `remote:` plugins in `buf.gen.yaml` | `path:packages/proto/scripts_test.go` (new) — Go test that reads `buf.gen.yaml` and fails if any line matches `^\s*-\s*remote:` | Enforces CD-1 in CI |
| T-3 | Plugin pin consistency | `path:internal/tools/<plugin>/handler_test.go` | Asserts manifest `version` field matches the version string baked into the install command |
| T-4 | Codegen byte-stability | `path:packages/proto/Makefile` `check` target (already exists) | `make generate` produces no diff vs committed `gen/` — runs in CI |
| T-5 | Handler install paths | `path:internal/tools/<plugin>/handler_test.go` | Linux apt path, darwin brew path, windows winget/`go install` path each install correctly (use httptest fixture for download steps) |
| T-6 | `vrooli auth status` probe states | `path:internal/cli/authcli/status_test.go` (new) | Each `SignInProbe` reports correct state for {missing .netrc, present .netrc, expired token (with --check-expiry)} |
| T-7 | Codegen offline acceptance | `path:packages/proto/scripts_test.go` (new) — runs `make generate` with `HTTPS_PROXY=http://127.0.0.1:9` (deliberately unreachable) and asserts success | Codegen does not require network |
| T-8 | Setup installs plugins idempotently | `path:internal/setup/setup_test.go` (existing harness) — extend with new tools | Re-running `vrooli setup` after plugins installed is a no-op |
...
| T-10 | Documented integration is parseable | `path:docs/configuration/integrations/buf-bsr.md` — add to existing markdown lint / link-check pass; future: schema-validate against a connector schema once integration-hub ships | Page format matches the contract template |
...
- [ ] `path:docs/configuration/integrations/buf-bsr.md` is linked from `external-auth.md` and `integrations/README.md`.
- [ ] `path:docs/development/proto.md` is linked from `path:docs/README.md`.
- [ ] `path:scripts/migrate_candidates/tools/buf.sh` is **deleted**.
...
| Vendored module drift from upstream | Medium | Low (only matters when consuming code uses new fields) | `path:packages/proto/scripts/refresh-vendor.sh` is documented; bump cadence is "when needed", not scheduled. CI check: `make check` post-refresh must be green or the bump is rolled back. |
...
- **No bash installer scripts** for the new plugins or for buf. All install logic is Go in `path:internal/tools/<name>/handler.go`.
...
- **No bespoke BSR sign-in UI** in vrooli-onboarding pre-V2. The contract page in `path:docs/configuration/integrations/buf-bsr.md` is enough; the V2 wizard rework picks it up. Adding a one-off wizard step now would be thrown away.
...
1. `path:packages/proto/buf.gen.yaml` contains **zero** `remote:` plugin entries (T-2 enforces).
...
5. `path:internal/runtime/registry.go` `customToolHandlers` includes `buf`, `protoc_gen_go`, `protoc_gen_es`, `protoc_gen_python` (T-1 passes).
6. `path:scripts/migrate_candidates/tools/buf.sh` is deleted from the working tree.
7. `.vrooli/service.json` `hostTools[]` lists `buf` and the four plugin tools as `required: true`.
8. `path:docs/configuration/integrations/buf-bsr.md` exists, follows the established per-integration page format, and is linked from `integrations/README.md` and `external-auth.md`.
9. `path:docs/development/proto.md` exists and is linked from `path:docs/README.md`.
