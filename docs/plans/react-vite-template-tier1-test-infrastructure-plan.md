...
The `path:templates/scenarios/react-vite` template now ships with seam-disciplined
...
(`path:scenarios/smoke-tier1/`), since the template itself is not runnable —
...
1. **No worked repository/integration test.** `path:api/internal/testutil/db/sqlite.go`
...
2. **No Go-side coverage gate.** `.github/workflows/test.yml` runs
   ...
   can rot silently.
3. **`server.Deps` nil-handling is asymmetric.** `path:internal/server/server.go::New`
...
4. **`interp()` lives in `path:ui/src/test-setup.ts` but is imported by tests
...
**In scope:**
- Edits inside `path:templates/scenarios/react-vite/`
- Edits inside `path:scenarios/smoke-tier1/` (the verification scenario) only
  to keep it in sync with the regenerated template
- Updates to `path:docs/internal/SEAMS.md` and `path:docs/internal/TESTING.md`
...
- `path:templates/scenarios/react-vite/api/internal/store/store.go`
- `path:templates/scenarios/react-vite/api/internal/store/schema.sql`
- `path:templates/scenarios/react-vite/api/internal/store/schema.go`
- `path:templates/scenarios/react-vite/api/internal/store/store_test.go` (new)
- `path:templates/scenarios/react-vite/api/internal/server/server.go`
- `path:templates/scenarios/react-vite/api/internal/server/server_test.go` (new)
- `path:templates/scenarios/react-vite/.github/workflows/test.yml`
- `path:templates/scenarios/react-vite/api/.golangci.yml` (touch only if a new lint surfaces)
- `path:templates/scenarios/react-vite/ui/src/test-setup.ts`
- `path:templates/scenarios/react-vite/ui/src/test-utils/index.ts`
- `path:templates/scenarios/react-vite/ui/src/test-utils/interp.ts` (new)
- `path:templates/scenarios/react-vite/ui/src/test-utils/interp.test.ts` (new)
- `path:templates/scenarios/react-vite/ui/src/App.test.tsx` (import path update)
...
- `path:scenarios/smoke-tier1/` is regenerated (or selectively patched) from
  the updated template, then full gate is run end-to-end.

### Existing seams the work must respect
- `path:internal/clock.Clock` interface + `mocks.FakeClock` (do not change)
- `path:internal/store.Pinger` interface (extend, not replace)
- `path:internal/testutil/db.NewSQLite` (consume from new repo test)
- `path:internal/testutil/no_prod_import_test.go` AST guardrail (the new
...
- The test scenario `path:scenarios/smoke-tier1/` reflects the latest generation;
...
1. Contain a working `tasks` table demonstrating the full storage seam:
   - `path:internal/store/store.go` declares a `TaskStore` interface
   - `path:internal/store/sqlite.go` implements it against `*sql.DB`
   - `path:internal/store/store_test.go` uses `testutil/db.NewSQLite` to
...
1. Add a minimal `tasks` domain to the template's storage seam:
   - **`path:api/internal/store/schema.sql`** — replace the placeholder with:
     ```sql
...
2. Extend `path:api/internal/store/store.go`:
   ```go
   type Task struct {
       ID        string
       Title     string
...
3. Add `path:api/internal/store/sqlite.go`:
...
4. Add `path:api/internal/store/sqlite_test.go` (production-tree, not testutil):
...
5. Update `path:api/internal/store/store.go` package comment to point at
   `sqlite.go` as the canonical reference for "how to add a table."

6. Update `path:docs/internal/SEAMS.md`:
...
7. Update `path:docs/internal/TESTING.md`:
   - Add a third canonical example file under "TL;DR — the canonical
     examples": `path:api/internal/store/sqlite_test.go` for repository tests
...
1. **`path:api/.golangci.yml`** — no change (lint, not coverage).
...
3. Floor justification: the template's current api/ and cli/ packages
   ...
   in `path:docs/internal/TESTING.md` (next to the existing "Coverage
...
5. Optional: add a `make test` (Go) target in `path:api/Makefile` /
   `path:cli/Makefile` if they exist, mirroring the CI invocation. Skip if
...
1. Edit `path:api/internal/server/server.go::New`:
   ```go
   ...
   ```
2. Update `path:api/main.go` to pass an explicit logger:
   ```go
   ...
   ```
3. Update `path:api/handlers/health/handler_test.go` — already passes
   `log.New(discardWriter{}, "", 0)`, so no change.
4. Update `path:api/internal/testutil/httpx/server_test.go` — already passes
   `log.New(io.Discard, "", 0)`, so no change.
5. Add `path:api/internal/server/server_test.go` (new):
...
6. Update `path:docs/internal/SEAMS.md` "How to add a new seam" guidance
   to mention: "if the new dep is required, add a nil check in
   `server.New`'s switch — do not introduce silent defaults."
7. Update `path:docs/internal/TESTING.md` "Common patterns and anti-patterns"
   table:
...
1. Create `path:ui/src/test-utils/interp.ts`:
...
2. Add `path:ui/src/test-utils/interp.test.ts`:
...
3. Re-export from `path:ui/src/test-utils/index.ts`:
   ```ts
   export { interp } from "./interp";
   ```
4. Edit `path:ui/src/test-setup.ts`:
   ...
   - Keep the jest-dom import, the cimode `beforeEach`, and the canvas
     ...
     here
5. Edit `path:ui/src/App.test.tsx`:
...
7. The ESLint `no-restricted-imports` rule already covers
   ...
   `path:src/test-utils/**`.
8. Update `path:docs/internal/TESTING.md` — anywhere it references "interp
...
`path:templates/scenarios/react-vite/ui/`. Rationale: the template's
...
2. **Verify the lockfile substitutes cleanly**: lockfiles store package
   ...
   list in the generator (`path:scripts/scenarios/generate.go` or wherever
   `vrooli scenario generate` resolves placeholders).
3. Commit `path:templates/scenarios/react-vite/ui/pnpm-lock.yaml`. **Do not**
   add it to `.gitignore`.
...
5. Update `path:templates/scenarios/react-vite/README.md`:
...
1. Regenerate or hard-sync `path:scenarios/smoke-tier1/` from the updated
   template:
   ```bash
   ...
   none.)
2. Inside `path:scenarios/smoke-tier1/`, run the full local gate exactly as
   CI would:
...
6. Update `path:docs/PROGRESS.md` inside the template (and the smoke
...
| `interp` location | `path:ui/src/test-utils/interp.ts`, exported from `test-utils/index.ts`. | `test-setup.ts` is side-effects only; consumers expect `test-utils/` for helpers. |
| Lockfile shipping | Committed in template at `path:ui/pnpm-lock.yaml`. | Lowest-friction; pnpm `--frozen-lockfile` works on first push. |
...
- New: `path:api/internal/store/sqlite_test.go` covers the four cases listed
  in §8 Phase 1.4
- Updated: `path:api/internal/store/schema_test.go::TestEnsureSchema_AppliesTasksTable`
...
- New: `path:api/internal/server/server_test.go` covers all five panic paths
  + happy path. Uses `defer recover()` to assert panic messages.

### Phase 4 — `interp` move
- New: `path:ui/src/test-utils/interp.test.ts` covers happy-path
  interpolation, number coercion, missing-variable error, empty template.
- Existing: `path:ui/src/App.test.tsx` continues to pass (proves the new
...
- CI's `pnpm install --frozen-lockfile --ignore-workspace` step is the
  test. A passing UI job in `path:scenarios/smoke-tier1/` after regeneration
...
1. A fresh `vrooli scenario generate react-vite --id <new> ...` produces
   ...
   - `(cd ui && pnpm install --frozen-lockfile --ignore-workspace && pnpm strings:check && pnpm type-check && pnpm lint && pnpm test:coverage && pnpm build)`
2. `path:scenarios/smoke-tier1/` is regenerated from the updated template,
...
6. `interp` lives in `path:ui/src/test-utils/interp.ts`, has a self-test,
   is re-exported from `test-utils/index.ts`, and `test-setup.ts`
   contains no exports.
7. `path:templates/scenarios/react-vite/ui/pnpm-lock.yaml` is committed and
   travels through `vrooli scenario generate` to produce a generated
   scenario that passes `pnpm install --frozen-lockfile`.
8. `path:docs/PROGRESS.md` (template) carries a one-line entry naming this
   plan and the date.
