...
- The Facebook-Marketplace-bidding-tactics bookmark that surfaced this revenue line at vision walk #4 (2026-04-27 chore-audit) is captured as an operational-reference inline in monetization team knowledge entry `flipping-revenue-line-seed/marketplace-bidding-reference` — see `scenarios/prompt-manager/store/teams/monetization/shared/knowledge.jsonl`. When the marketplace-negotiation-assistant scenario gets built, that bookmark is the seed reference for prompt patterns.
...
- Cross-reference: `scenarios/bookmark-intelligence-hub/` rework initiative (`bookmark-intelligence-hub-rework-and-ideation`) — once that ships, bookmarks like the source one will surface through the ideation pipeline rather than requiring vision-walk-divergence to capture.
