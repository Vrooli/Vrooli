...
The **ways users discover Vrooli** — distinct from bundles (what we sell), tiers (how it's delivered), and revenue lines (how money flows). Each channel has its own audience, owner, telemetry, and discipline, and lives in its own file under [`channels/`](channels/).

## Disambiguation: this is not docs/marketing/CHANNELS.md

[`docs/marketing/CHANNELS.md`](../marketing/CHANNELS.md) is **per-platform publishing rules** for the marketing-crew — what to post on X, blog, YouTube, TikTok, etc., and how. It is an operational doc owned by marketing.
...
|---|---|---|---|---|
| `web-seo` | Web SEO + landing pages | `active` | humans | [channels/web-seo.md](channels/web-seo.md) |
| `app-stores` | Apple / Google / Microsoft app stores | `candidate` | humans | [channels/app-stores.md](channels/app-stores.md) |
| `skill-registries` | Claude Skills, ClawHub, skills.sh, etc. | `candidate` | agents | [channels/skill-registries.md](channels/skill-registries.md) |
| `oss-discovery` | GitHub stars, trending, awesome-lists, README-as-marketing | `active` (uninstrumented) | humans (developers) | [channels/oss-discovery.md](channels/oss-discovery.md) |
| `community-content` | Long-form blog, social, community presence (HN/Reddit/forums) | `candidate` | humans | [channels/community-content.md](channels/community-content.md) |
| `in-product-expansion` | Agents inside the bundle suggesting other bundle apps | `active` | both (humans via UI, agents structurally) | [channels/in-product-expansion.md](channels/in-product-expansion.md) |
...
Mirror the [`REVENUE_LINES.md`](REVENUE_LINES.md) "what's NOT a revenue line" section. These are explicitly off-table; proposals to enable them require an exception decision with strong justification, and `contrarian` rejects by default.
...
- **Astroturfed community presence.** Fake reviews, paid HN comments, ghost-written "user testimonials," persona-actor accounts impersonating real users with fabricated credentials — all in the same prohibition zone as the [`docs/marketing/strategies/ai-ugc-personas.md`](../marketing/strategies/ai-ugc-personas.md) AI-UGC stance.
