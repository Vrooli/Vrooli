...
| Commissioning — `vrooli setup`, host tools, host safeguards | once per host change | Provisioning: tool installs, opt-in host-state changes (kernel params, DNS, firewall), log access, permissions. The only layer where sudo exists. | `path:docs/configuration/host/tools.md`, `path:docs/configuration/host/safeguards.md`, `~/.vrooli/state/vrooli/vrooli-onboarding/operator-state.json` |
