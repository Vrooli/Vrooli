...
This file covers **framework health only** — contract conformance, declaration integrity, coupling visibility, canon coherence, skill conditioning quality, and team orientation cost. It deliberately does **not** state what Vrooli is working toward. What the system is for lives in `path:docs/director-swarm/strategy/OBJECTIVES.md`; how progress toward it is observed lives in `path:docs/director-swarm/evidence/OUTCOMES_CHARTER.md`; per-team reliability targets live in `path:scenarios/infrastructure-manager/setpoint/reliability-setpoint.json`. A target that answers "is the work worth doing" belongs there, not here. A target that answers "can the system still be reasoned about" belongs here.
...
Honesty flags use the same vocabulary as `path:scenarios/infrastructure-manager/docs/concepts/SETPOINT-MODEL.md` — `measured`, `estimate`, `aspirational`, `pending-baseline`, `pending-telemetry`. Read the definitions there; they are not restated here. Note that the instrument **derives** the flag from coverage status rather than accepting a hand-set one: a `MISSING` cell is `pending-telemetry` by definition, becomes `pending-baseline` when a sensor ships, and `measured` once a baseline is recorded. A hand-set flag is an integrity finding.
...
| Instrument coverage | `prompt-manager graph instruments` — per-team `team.json::instrument` declarations | every team declares an instrument or carries a dated gap marker, and each declaration is internally coherent (a `live` status names a scenario; a `none`/`partial` status carries a dated marker). **The band is "declared or dated", never "present"** — a team with no instrument is in band as long as it says so with a date | `team-capability-consolidation` | `measured` 2026-08-13, first baseline. See `path:docs/agent-system/TARGET_MODEL.md` §"The instrument: six invariants" for what is being declared and why presence is deliberately not banded |
...
| Team orientation cost | `prompt-manager graph orientation-cost` — `members×10 + topics×2 + canonLines÷50`, read against declared scenario coverage. `canonLines` is charged by **consumer share** (a plan-of-record document declared for N consuming teams contributes `lines÷N`, with the unsplit total reported beside it as `sharedCanonLines`); `scenarioCoverage` reads `team.json::instrument.coversScenarios`; `domainAddresses` is reported beside both | no team's orientation cost rises across an audit cycle in which its scenario coverage grew | `team-capability-consolidation` | `measured` 2026-08-17, method `orientation-consumer-share-v2` (`director-swarm` 57, `infra-health` 43, `marketing-crew` 81, `meta-optimization` 127, `monetization` 73, `scenario-qa` 40). **Every earlier reading is not comparable and none may be used as the band's baseline.** The 2026-07-30 set was taken while the sensor description claimed a fourth "declared work types" term that `orientationComposite()` never computed. The 2026-08-09 set (`meta-optimization` 206, `marketing-crew` 113, `monetization` 135) was taken under `v1` billing, which charged shared canon wholly to its custodian; the 2026-08-13 consumer-share fix superseded it, and the 08-09 numbers were left in this cell reading as current for four days — a reader acting on them would have concluded `meta-optimization` was 60% heavier than it is and that canon length was the dominant term, when canon is 17 of its 127. 2026-08-17 is therefore the first `v2` reading and the band still has nothing to diff against |
...
**Two corrections landed on 2026-08-13 and both changed what a green cell means.** `scenarioCoverage` previously counted external nodes in a team's operating graph, on the stated assumption that external nodes are how a team declares the scenarios it composes with. They are not — they resolved to `operator`, `vision-walk`, `swarm-manager-work` and `report-friction`, and the custodian of the fleet's own instrument did not have that instrument in its list. The guard series could not move for the reason the band exists, so the trend never fired. It now reads `team.json::instrument.coversScenarios`, and the external node set is retained under the honest name `externalActors`. Separately, `canonLines` billed shared canon to whichever team is its custodian, which made the team that maintains `path:docs/agent-system/` read as the fleet's heaviest and buried the roster/topic signal that actually distinguished it; the manifest already declares `consumers` per document, so the charge is now split by that count.
...
**A date is not enough; name the method.** A dated observation says when it was read, not which sensor version read it, so changing a sensor silently converts every prior reading in this file into a number that still looks current. Team orientation cost demonstrated the failure: `orientationComposite()` changed on 2026-08-13 and its 08-09 readings sat here unmarked. Two rules follow, and `COVERAGE-MODEL.md` §"Answer readiness is a three-signal join" is the worked precedent — it versions its method (`answer-tiered-corpus-end-to-end-v3`) and requires snapshot invalidation whenever the signals move.

1. **Every Observed cell whose sensor has a versioned method carries that version beside the date.** Changing the computation requires a new version, never an in-place redefinition.
2. **Non-comparability is derived, not remembered.** A reading taken under a superseded method is out of the baseline by rule. Do not maintain a hand-written list of which past readings are comparable — that list is itself the `state-in-prose` defect this framework catches elsewhere, and it is what failed here: the 07-30 readings were marked by hand and the 08-09 readings were missed.

Until `graph audit --baseline` rejects a cross-version diff on its own, a reader comparing cycles must check the method version before trusting a trend.
