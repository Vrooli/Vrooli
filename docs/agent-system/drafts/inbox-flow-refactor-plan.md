...
- `docs/agent-system/TOPICS_SCHEMA.md` (was `drafts/topics-schema.md` at the time this plan was authored; promoted to canon as part of this refactor)
- `scenarios/prompt-manager/api/memberflow/schema.go` (current shape)
- `scenarios/prompt-manager/api/heartbeat/prompt_builder.go` (section ordering)
- `scenarios/prompt-manager/api/teamcontract/contract.go` (the gold-standard reference for "structural-data → generated section")
...
3. **Taxonomy as PoR.** Three new PoR documents (`docs/marketing/SIGNAL_TAXONOMY.md`, `docs/monetization/OPPORTUNITY_TAXONOMY.md`, `docs/monetization/VALIDATION_TAXONOMY.md`) hold each domain's signal types, dispatch table, evidence rules, and destination schemas. Each is paired with a parseable JSON sidecar the heartbeat builder consumes.
...
| Domain signal taxonomy (vocabulary) | Embedded in router SKILL.md | `docs/<domain>/<TAXONOMY>.md` + `docs/<domain>/<taxonomy>.json` sidecar |
...
| Team        | marketing-crew |
| Prefix      | `research-inbox/*` |
| Taxonomy    | `marketing-research` (PoR: `docs/marketing/SIGNAL_TAXONOMY.md`) |
...
- `scenarios/prompt-manager/api/memberflow/loader.go` — no behavior change; struct change is wire-compatible by JSON tag.
- `scenarios/prompt-manager/api/memberflow/taxonomy.go` (new file)
  - Load `docs/<domain>/<taxonomy-id>.json` from a registry rooted at repo root.
...
**Files (new)**:
- `docs/marketing/signal-taxonomy.json` (id: `marketing-research`)
- `docs/marketing/SIGNAL_TAXONOMY.md` (human-readable view; cite the JSON sidecar)
- `docs/monetization/opportunity-taxonomy.json` (id: `monetization-opportunity`)
- `docs/monetization/OPPORTUNITY_TAXONOMY.md`
- `docs/monetization/validation-taxonomy.json` (id: `monetization-validation`)
- `docs/monetization/VALIDATION_TAXONOMY.md`
...
- `scenarios/prompt-manager/api/heartbeat/inbox_flow.go` (new)
...
1. Rename directory:
   - `store/skills/packs/core/marketing-research-router/` → `store/skills/packs/core/marketing-signal-classifier/`
   - `store/skills/packs/core/monetization-opportunity-router/` → `store/skills/packs/core/monetization-signal-classifier/`
   - `store/skills/packs/core/market-validation-router/` → `store/skills/packs/core/market-validation-triage/`
...
- `docs/<domain>/<TAXONOMY>.md` — canonical signal types, definitions, dispatch.
...
- `store/teams/marketing-crew/members/researcher/HEARTBEAT.md`
- `store/teams/marketing-crew/members/researcher/RESPONSIBILITIES.md`
- `store/agents/researcher/TOOLS.md`
- `store/teams/monetization/members/opportunity-scout/HEARTBEAT.md`
- `store/teams/monetization/members/opportunity-scout/RESPONSIBILITIES.md`
- `store/teams/monetization/members/market-validator/HEARTBEAT.md`
- `store/teams/monetization/members/market-validator/RESPONSIBILITIES.md`
- `store/teams/monetization/shared/TEAM.md` — remove the two "Unrouted-set invariant" rows that name router skill IDs; replace with one generic invariant row that names the taxonomy.
...
- `docs/agent-system/INTAKE_PIPELINE.md` — update the "topics.json" example to show the new fields; add a paragraph on taxonomies as the dispatch single source of truth.
- `docs/agent-system/TOPICS_SCHEMA.md` — update schema definition to match new fields. *(Note: as of plan execution this file was promoted from `drafts/topics-schema.md` to canon; the stability gate was crossed.)*
- `docs/agent-system/PRIMITIVES.md` — small update to the "Inbox / synthesis" entry referencing the new `taxonomy`/`classifier_skill` shape.
- `docs/agent-system/README.md` — `taxonomy` enters the canon mental-model paragraph.

### Phase H — Member-panel Inbox tab (UI)

Backend dependency:
- `scenarios/prompt-manager/api/memberflow/handlers.go` — add `GET /teams/{id}/members/{agentId}/inbox-entries?prefix=<prefix>` returning unrouted entries from team knowledge under the prefix, paginated. Backed by the existing `KnowledgeQuery` interface (already used by `inbox_aging.go`); add `ListUnroutedFull` returning enough fields for UI display (id, topic, by, source, content preview, at).
- Routes: register in `routes_topics.go` (or wherever member-flow routes register).

Frontend:
- `ui/src/services/memberFlowService.ts` — add `getMemberInboxEntries(teamId, agentId, prefix, opts)` and `routeInboxEntry(teamId, agentId, id, action)` (action: `promote` with destination, `drop`).
...
- `ui/src/components/editor/MemberInboxTab.tsx` (new)
...
5. **Taxonomy registry location.** Plan places taxonomies at `docs/<domain>/`. An alternative is `scenarios/prompt-manager/store/taxonomies/<id>.json` (centralized). Trade-off: domain-colocated is more discoverable for operators reading the PoR; centralized is easier for the loader. Plan keeps domain-colocated; loader does a glob over `docs/**/*-taxonomy.json` (or similar). Mitigation: implement loader with explicit registry path list, easy to refactor.
