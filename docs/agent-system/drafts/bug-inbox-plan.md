...
- `path:docs/agent-system/INTAKE_PIPELINE.md` — two routing modes; cross-team schema ownership
- `path:docs/agent-system/TOPICS.md` — registry; notebook-vs-typed-inbox; scenario-qa observations
- `path:docs/agent-system/TOPICS_SCHEMA.md` — schema; cross-team flow
- `path:docs/agent-system/PRIMITIVES.md`
- `path:docs/agent-system/LAYERS.md`
- `path:docs/agent-system/TEAM_DOCS_PATTERNS.md` — paired-doc-and-skill discipline; per-entity files vs monolithic
- `path:docs/agent-system/TEAM_MEMBER_ARCHITECTURE.md` — 9-layer member capability model; smells (skillless canon, mega-skill pressure, etc.)
- `path:docs/agent-system/PROMOTION_LADDER.md` — how techniques mature
- `path:docs/marketing/post-techniques/README.md` — gold-standard reference for the technique-registry pattern this plan replicates
- `path:docs/marketing/post-types/README.md` — doc + paired skill rule (mandatory hard rule, not recommendation)
- `path:scenarios/prompt-manager/api/memberflow/schema.go` and `validation.go`
- `path:scenarios/prompt-manager/api/heartbeat/inbox_flow.go` and `prompt_builder.go`
- `path:scenarios/prompt-manager/store/skills/packs/core/scientific-debugging/SKILL.md`
- `path:scenarios/prompt-manager/store/skills/packs/core/{screaming-architecture-audit,boundary-of-responsibility-enforcement,seam-discovery-and-enforcement,invariant-discovery-and-enforcement,cognitive-load-reduction,decision-boundary-extraction,code-cleanup}/SKILL.md`
- `path:scenarios/prompt-manager/store/agents/quality-auditor/` (agent template reference)
- `path:scenarios/prompt-manager/store/agents/marketing-contrarian/` (contrarian template reference)
- `path:scenarios/prompt-manager/store/teams/scenario-qa/team.json` (note `enabled: false`, narrow mission)
- `path:scenarios/prompt-manager/store/teams/scenario-qa/members/{programmatic-qa-runner,quality-auditor}/` (existing member files)
...
1. **scenario-qa plan-of-record.** A new `path:docs/scenario-qa/` folder containing the team's full strategic canon: README, three technique registries (investigation, audit, readiness), bug-report taxonomy. Operator-curated like every other team PoR.
2. **Investigation-techniques registry.** `path:docs/scenario-qa/investigation-techniques/` mirrors `path:docs/marketing/post-techniques/` — one paired doc-and-skill per technique. `scientific-debugging` is the first entry, paired with the existing skill.
3. **Audit-techniques registry.** `path:docs/scenario-qa/audit-techniques/` — seven paired PoR docs for `quality-auditor`'s seven existing audit lenses. Closes the `skillless canon` smell across the entire audit rotation in a single pass.
4. **Readiness-checks registry (skeleton).** `path:docs/scenario-qa/readiness-checks/README.md` — stub now; full population deferred until GCT dimensions stabilize. Documented as future PoR work.
...
- **Renaming the team itself.** Team stays `scenario-qa`. Folder is `path:docs/scenario-qa/`.
...
| Concern | Home |
|---|---|
| Team mission, scope, principles, decision contexts, knowledge topics | `path:docs/scenario-qa/README.md` (new — strategic canon overview) + `path:store/teams/scenario-qa/shared/TEAM.md` (charter) |
...
| Investigation methodology canon | `path:docs/scenario-qa/investigation-techniques/<slug>.md` (one PoR doc per technique) |
| Investigation methodology procedure | `path:scenarios/prompt-manager/store/skills/packs/core/<technique-slug>/SKILL.md` (one skill per technique; doc + paired skill discipline) |
| Audit-technique canon | `path:docs/scenario-qa/audit-techniques/<slug>.md` (one PoR doc per existing audit-lens skill) |
| Audit-technique procedure | existing skill at `path:scenarios/prompt-manager/store/skills/packs/core/<slug>/SKILL.md` (already authored; cross-linked in this plan) |
| Readiness-check canon | `path:docs/scenario-qa/readiness-checks/<slug>.md` (skeleton — stub README only in this plan; entries deferred) |
...
### 6.2 `path:docs/scenario-qa/` folder structure
...
Mirrors `path:docs/marketing/post-techniques/` exactly:
...
`scientific-debugging.md` is the first entry. The existing `scientific-debugging` skill is updated to add `path:docs/scenario-qa/investigation-techniques/scientific-debugging.md` to its required-reading list.
...
`path:docs/scenario-qa/readiness-checks/README.md` is a stub: explains the registry's purpose (paired strategic-canon docs for `programmatic-qa-runner`'s individual readiness dimensions and check categories), declares the doc + paired skill discipline applies here too, lists "Files in this folder: none yet — populated as GCT dimensions stabilize and individual readiness checks become candidate skills."
...
All three registries follow the same mandatory rule from `path:docs/marketing/post-types/README.md`:
...
Enforced by a canon coherence test (§9) that checks every `<registry>/<slug>.md` (excluding README.md) has a matching `path:store/skills/packs/core/<slug>/SKILL.md`, and vice versa for skills tagged with the registry's tag.
...
- `path:store/agents/bug-investigator/SOUL.md` — identity (specialist scientific-debugger; calm, methodical, no speculation; honest when investigation is blocked)
- `path:store/agents/bug-investigator/AGENTS.md` — workflow contract (load technique skill from registry, drain top of inbox, investigate, classify outcome, write `bug-investigation-report/<slug>` log, hand off via swarm-manager / decision / route)
- `path:store/agents/bug-investigator/TOOLS.md` — bindings: every technique skill from the investigation-techniques registry (currently `scientific-debugging`), swarm-manager CLI, prompt-manager team knowledge-list/update/delete (no add — bug-investigator never writes to its own inbox)
- `path:store/agents/bug-investigator/agent.json` — metadata
- `path:store/teams/scenario-qa/members/bug-investigator/topics.json` — above
- `path:store/teams/scenario-qa/members/bug-investigator/HEARTBEAT.md` — heartbeat task: drain top of inbox by severity, pick technique from registry, investigate, write audit-log entry, take action
- `path:store/teams/scenario-qa/members/bug-investigator/RESPONSIBILITIES.md` — drain duty + investigation rigor + escalation rules + Available Skills table listing every technique
- `path:store/teams/scenario-qa/members/bug-investigator/last-handoff.md` — initialized empty
...
- `path:store/agents/qa-contrarian/SOUL.md` — identity (skeptical, first-principles, allergic to consensus, honest when no challenge is warranted)
- `path:store/agents/qa-contrarian/AGENTS.md` — workflow contract (read peer decisions on cadence, surface specific failure modes, write challenge-report/<slug>)
- `path:store/agents/qa-contrarian/TOOLS.md` — bindings: prompt-manager team decision-list/knowledge-list, swarm-manager CLI for backlog-item review
- `path:store/agents/qa-contrarian/agent.json` — metadata
- `path:store/teams/scenario-qa/members/qa-contrarian/topics.json` — above
- `path:store/teams/scenario-qa/members/qa-contrarian/HEARTBEAT.md` — heartbeat task: scan recent bug-investigation entries + peer-team decisions; write at most N challenge-notes per heartbeat
- `path:store/teams/scenario-qa/members/qa-contrarian/RESPONSIBILITIES.md` — challenge discipline; signal-vs-noise rules; what NOT to challenge
- `path:store/teams/scenario-qa/members/qa-contrarian/last-handoff.md` — initialized empty
...
- `path:store/teams/scenario-qa/members/quality-auditor/RESPONSIBILITIES.md`:
  - Add "Available Skills" table with all 7 audit-technique skills, each with a one-line "when to apply" hook.
  - Add cross-link to `path:docs/scenario-qa/audit-techniques/README.md` as the canon hub for the rotation.
  - Add cross-link to `path:docs/scenario-qa/README.md` for team-level context.
- `path:store/teams/scenario-qa/members/quality-auditor/HEARTBEAT.md`:
  - Add cross-link to `path:docs/scenario-qa/audit-techniques/<current-rotation-slug>.md` so the auditor reads the strategic-canon doc before applying the lens.
- `path:store/teams/scenario-qa/members/quality-auditor/topics.json`:
...
- `path:store/teams/scenario-qa/members/programmatic-qa-runner/RESPONSIBILITIES.md`:
  - Add cross-link to `path:docs/scenario-qa/readiness-checks/README.md` (stub — flagged as future PoR work).
  - Add cross-link to `path:docs/scenario-qa/README.md` for team-level context.
- `path:store/teams/scenario-qa/members/programmatic-qa-runner/HEARTBEAT.md`:
  - Cross-link to readiness-checks README (stub).
- `path:store/teams/scenario-qa/members/programmatic-qa-runner/topics.json`:
...
- `path:store/skills/packs/core/report-bug/skill.json` — id `report-bug`, modes `["tools"]`, tags `["observability", "bug-report"]`
- `path:store/skills/packs/core/report-bug/SKILL.md`:
  - Required reading: `path:docs/scenario-qa/BUG_REPORT_TAXONOMY.md`
...
`path:docs/scenario-qa/bug-report-taxonomy.json`:
...
`path:docs/scenario-qa/BUG_REPORT_TAXONOMY.md` — human-readable view; cites the JSON sidecar; explains each signal type with examples; documents the operator's view of bug-investigation outputs.
...
**Files (modified):**
- `path:scenarios/prompt-manager/api/memberflow/schema.go` — `IntakeEntry.SourceTeam` semantics extended; `"*"` literal accepted
...
**Files (new):**
- `path:docs/scenario-qa/README.md` — team PoR overview, structured as the marketing/monetization READMEs are:
  - Mission (matches §6.3 rewrite; cross-link to `path:store/teams/scenario-qa/shared/TEAM.md`)
...
**Files (new):**
- `path:docs/scenario-qa/investigation-techniques/README.md`:
  - Purpose: strategic canon for techniques the bug-investigator applies
  - Doc + paired skill discipline (mandatory hard rule, mirroring `path:docs/marketing/post-types/README.md`)
...
- `path:docs/scenario-qa/investigation-techniques/scientific-debugging.md`:
  ...
  - Cites the existing skill as the executable spec
  - Cites `path:docs/agent-system/SKILL_AUTHORING.md`

**Files (modified):**
- `path:scenarios/prompt-manager/store/skills/packs/core/scientific-debugging/SKILL.md` — add `path:docs/scenario-qa/investigation-techniques/scientific-debugging.md` to required reading; cross-link the strategic canon

### Phase D — Author audit-techniques registry

**Files (new):**
- `path:docs/scenario-qa/audit-techniques/README.md`:
...
- `path:docs/scenario-qa/audit-techniques/screaming-architecture-audit.md`
- `path:docs/scenario-qa/audit-techniques/boundary-of-responsibility-enforcement.md`
- `path:docs/scenario-qa/audit-techniques/seam-discovery-and-enforcement.md`
- `path:docs/scenario-qa/audit-techniques/invariant-discovery-and-enforcement.md`
- `path:docs/scenario-qa/audit-techniques/cognitive-load-reduction.md`
- `path:docs/scenario-qa/audit-techniques/decision-boundary-extraction.md`
- `path:docs/scenario-qa/audit-techniques/code-cleanup.md`
...
- `path:scenarios/prompt-manager/store/skills/packs/core/screaming-architecture-audit/SKILL.md`
- `path:scenarios/prompt-manager/store/skills/packs/core/boundary-of-responsibility-enforcement/SKILL.md`
- `path:scenarios/prompt-manager/store/skills/packs/core/seam-discovery-and-enforcement/SKILL.md`
- `path:scenarios/prompt-manager/store/skills/packs/core/invariant-discovery-and-enforcement/SKILL.md`
- `path:scenarios/prompt-manager/store/skills/packs/core/cognitive-load-reduction/SKILL.md`
- `path:scenarios/prompt-manager/store/skills/packs/core/decision-boundary-extraction/SKILL.md`
- `path:scenarios/prompt-manager/store/skills/packs/core/code-cleanup/SKILL.md`
...
**Files (new):**
- `path:docs/scenario-qa/readiness-checks/README.md` — stub per §6.9: purpose, doc + paired skill discipline applies here too, "Files in this folder: none yet — populated as GCT dimensions stabilize." Future PoR work flagged.

### Phase F — Author bug-report taxonomy

**Files (new):**
- `path:docs/scenario-qa/bug-report-taxonomy.json` (per §6.17)
- `path:docs/scenario-qa/BUG_REPORT_TAXONOMY.md` — human-readable view; cite JSON sidecar; explain signal types with examples

**Files (modified):**
- `path:docs/agent-system/README.md` — add `bug-report` row to § Active taxonomies registry (handled here so the registry is current)
...
**Files (new):**
- `path:scenarios/prompt-manager/store/skills/packs/core/report-bug/skill.json`
- `path:scenarios/prompt-manager/store/skills/packs/core/report-bug/SKILL.md` (per §6.16)
...
**Files (new):**
- `path:scenarios/prompt-manager/store/agents/bug-investigator/agent.json`
- `path:scenarios/prompt-manager/store/agents/bug-investigator/SOUL.md`
- `path:scenarios/prompt-manager/store/agents/bug-investigator/AGENTS.md`
- `path:scenarios/prompt-manager/store/agents/bug-investigator/TOOLS.md` — bindings explicitly include the investigation-techniques registry skills (currently `scientific-debugging`)

### Phase I — Bind bug-investigator to scenario-qa team

**Files (new):**
- `path:store/teams/scenario-qa/members/bug-investigator/topics.json` (per §6.12)
- `path:store/teams/scenario-qa/members/bug-investigator/HEARTBEAT.md`
- `path:store/teams/scenario-qa/members/bug-investigator/RESPONSIBILITIES.md` — includes "Available Skills" table listing every registered investigation technique
- `path:store/teams/scenario-qa/members/bug-investigator/last-handoff.md` (initial empty)

### Phase J — Create qa-contrarian agent identity

**Files (new):**
- `path:scenarios/prompt-manager/store/agents/qa-contrarian/agent.json`
- `path:scenarios/prompt-manager/store/agents/qa-contrarian/SOUL.md`
- `path:scenarios/prompt-manager/store/agents/qa-contrarian/AGENTS.md`
- `path:scenarios/prompt-manager/store/agents/qa-contrarian/TOOLS.md` — bindings: prompt-manager team decision-list/knowledge-list, swarm-manager CLI for backlog-item review
...
**Files (new):**
- `path:store/teams/scenario-qa/members/qa-contrarian/topics.json` (per §6.13)
- `path:store/teams/scenario-qa/members/qa-contrarian/HEARTBEAT.md`
- `path:store/teams/scenario-qa/members/qa-contrarian/RESPONSIBILITIES.md`
- `path:store/teams/scenario-qa/members/qa-contrarian/last-handoff.md` (initial empty)

### Phase L — Update existing members

**Files (modified):**
- `path:store/teams/scenario-qa/members/quality-auditor/RESPONSIBILITIES.md` (per §6.14)
- `path:store/teams/scenario-qa/members/quality-auditor/HEARTBEAT.md` (per §6.14)
- `path:store/teams/scenario-qa/members/programmatic-qa-runner/RESPONSIBILITIES.md` (per §6.15)
- `path:store/teams/scenario-qa/members/programmatic-qa-runner/HEARTBEAT.md` (per §6.15)
...
**Files (modified):**
- `path:scenarios/prompt-manager/store/teams/scenario-qa/team.json`:
...
- `path:scenarios/prompt-manager/store/teams/scenario-qa/roles.json`:
  - Add `bug-investigator` role with description
  - Add `qa-contrarian` role with description
- `path:scenarios/prompt-manager/store/teams/scenario-qa/shared/TEAM.md`:
...
**Files (modified):**
- `path:docs/agent-system/TOPICS.md`:
  ...
  - Cross-reference to `path:docs/scenario-qa/investigation-techniques/`, `audit-techniques/`, `readiness-checks/`
- `path:docs/agent-system/TOPICS_SCHEMA.md`:
...
- `path:docs/agent-system/INTAKE_PIPELINE.md` — note universal-source intakes in § Two routing modes context (they pair with deterministic-prefix routing because the producer set is open and trust-by-construction is needed at the producer side; the investigator's first triage step is to validate the assignment)
- `path:docs/agent-system/README.md`:
...
**Files (modified):**
- `path:scenarios/prompt-manager/store/teams/scenario-qa/team.json` — `enabled: true`
...
| Skill registry | `path:cli/skills_test.go` | `report-bug` discoverable; required-reading cites `BUG_REPORT_TAXONOMY.md` |
| Investigation-techniques pairing | extended `agent_system_canon_test.sh` | every `path:docs/scenario-qa/investigation-techniques/<slug>.md` (excluding README) has matching skill `core/<slug>/`, AND every skill tagged `investigation-technique` has matching PoR doc |
| Audit-techniques pairing | extended `agent_system_canon_test.sh` | every `path:docs/scenario-qa/audit-techniques/<slug>.md` (excluding README) has matching skill `core/<slug>/`, AND every skill tagged `audit-technique` has matching PoR doc |
...
| Check | Asserts |
|---|---|
| Investigation-techniques pairing | every `path:docs/scenario-qa/investigation-techniques/<slug>.md` (excluding `README.md`) has a matching `path:scenarios/prompt-manager/store/skills/packs/core/<slug>/SKILL.md`, AND every skill tagged `tags: ["investigation-technique"]` has a matching PoR doc |
| Audit-techniques pairing | same shape, scoped to `path:docs/scenario-qa/audit-techniques/` and skills tagged `audit-technique` |

The canon coherence test loops over a list of `{registryDir, skillTag}` pairs so future registries (readiness-checks, future technique types) plug in by appending one entry. Implementation lives in `path:scenarios/prompt-manager/test/agent_system_canon_test.sh`.
