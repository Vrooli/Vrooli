...
2. **Monetization catalog.** Is this entity load-bearing for any candidate or active SKU? (Query `offer-desk offers catalog-list` for nodes and `offer-desk offers catalog-edges` for the scenario→SKU `belongs_to` mapping. This check read `scenario-sku-map.json` until that file was retired into Offer Desk on 2026-08-16; a gate that searches a deleted file passes for the wrong reason.)
