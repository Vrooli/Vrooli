...
Three postures follow, and every entry declares exactly one:
...
Only storage-manager-managed entries receive Storage Manager providers. Protected entries cannot become cleanup candidates through a lower-level override, and a declared budget whose path overlaps one is refused by the retention enforcer rather than honoured. Self-managed entries are owned end to end by the component that writes them: losing one is survivable because that component re-asserts it, but nothing is stale there merely for being old. `data-backup-manager` keys its backup-target suggestions on `regenerable == false`.
...
**Cleanup repair boundary.** Storage Manager first removes eligible entries as the invoking user. On a permission failure it may make one typed, audited request to the privilege broker for the approved runtime-home class, then retries the same cleanup request once. The broker validates the caller identity, expected owner, class root, and containment; it never accepts arbitrary paths or commands. Missing broker, unsupported platform, partial repair, and failed retry remain explicit refusal evidence.

**Drift guard.** The `QUALITY_NO_RUNTIME_HOME_LITERALS` check fails if `cmd/`, `internal/`, or `packages/` code joins a home-derived value with `".vrooli"` (or embeds a `"~/.vrooli/…"` literal) instead of going through the authority. It is owned by Quality Health (`scenarios/quality-health/api/internal/contractscan`), declared at error severity with `clean_requirement: required`, and runs in that scenario's `standards` dimension — **not** in `vrooli contract validate` or `make hygiene`, which an earlier revision of this page claimed under the name `no_runtime_home_literals`. No check by that name has ever existed; use the real one above when citing this guard. A line that genuinely means the **repo-project** `.vrooli` can opt out with a trailing `// repo-contract:project-config` comment. `packages/repo-contract-go/**` and `internal/repocontractmeta/**` are exempt (they define the authority).
