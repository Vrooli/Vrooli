...
EVENTS_JSONL="${SCRIPT_DIR}/scenario-improvement-events.ndjson"
SUMMARY_JSON="${SCRIPT_DIR}/scenario-summary.json"
SUMMARY_TXT="${SCRIPT_DIR}/scenario-summary.txt"
...
# Function: JSON summaries via jq
json_summaries() {
	local subcmd="${1:-summary}"
	shift || true
	
	if [[ ! -f "$EVENTS_JSONL" ]]; then
		echo "Events file not found: $EVENTS_JSONL"
		echo "Run the loop at least once to generate events."
		return 1
	fi
	
	if ! command -v jq >/dev/null 2>&1; then
		echo "jq is required for JSON summaries. Please install jq."
		return 1
	fi
	...
	case "$subcmd" in
		summary)
			...
			tail -n 2000 "$EVENTS_JSONL" | jq -s '
			  ...
			  {
			    runs:$runs, ok:$ok, timeout:$to, fail:$fail,
			    success_rate:(if $runs==0 then 0 else ($ok/$runs) end)
			  }'
			;;
		recent)
			local n="${1:-10}"
			tail -n 2000 "$EVENTS_JSONL" | jq -c 'select(.type=="finish") | {ts,iteration,exit_code,duration_sec}' | tail -n "$n"
			;;
		inflight)
			tail -n 2000 "$EVENTS_JSONL" | jq -s '[ group_by(.pid)
			  | map({pid: .[0].pid,
		...
		errors)
			local n="${1:-5}"
			tail -n 2000 "$EVENTS_JSONL" | jq -c 'select(.type=="finish" and .exit_code!=0) | {ts,iteration,exit_code,duration_sec}' | tail -n "$n"
			;;
		hourly)
			tail -n 2000 "$EVENTS_JSONL" | jq -s '
			  [ .[] | select(.type=="finish")
			...
			;;
		*)
			echo "Unknown json subcommand: $subcmd"
			echo "Available: summary | recent [N] | inflight | durations | errors [N] | hourly"
			return 1
			;;
	esac
}
...
Commands:
  ...
  json <cmd>  JSON summaries (requires jq). cmds: summary | recent [N] | inflight | durations | errors [N] | hourly
...
Examples:
  ...
  $0 json summary  # Show high-level health
