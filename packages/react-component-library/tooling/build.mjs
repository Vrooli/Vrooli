...
baseConfig.include = [join(sourceRoot, "**/*.ts"), join(sourceRoot, "**/*.tsx"), cssDeclarationPath];
baseConfig.exclude = [
...
// published file. Replace each side-effect stylesheet import in the emitted
// module with the library's one runtime registration mechanism. The emitted
// package is therefore consumable by plain Node/tsc environments as well as
// bundlers that do not understand CSS modules.
const inlineStyles = async (root, base = root) => {
  for (const entry of await readdir(root, { withFileTypes: true })) {
    ...
    for (const match of imports) {
      const emittedRelative = relativePath(base, path);
      const cssPath = join(sourceRoot, dirname(emittedRelative), match[1]);
      const css = await readFile(cssPath, "utf8");
      const key = relativePath(sourceRoot, cssPath).replaceAll("\\", "/");
      // CSS side-effect imports are compiled into the same idempotent head
      // registration semantics as useLibraryStyleSheet. Keep this helper in
      // the emitted module because the immutable StyleSheet foundation
      // exposes the hook API, not a module-load side-effect API.
      prefix += `const __rclRegisterStyleSheet = (key, css) => { if (typeof document === "undefined" || !css.trim() || document.querySelector(\`style[data-rcl-sheet="\${key.replace(/[^a-zA-Z0-9_-]/g, "\\\\$&")}"]\`)) return; const style = document.createElement("style"); style.dataset.rclSheet = key; style.setAttribute("data-rcl-sheet", key); style.textContent = css; document.head.insertBefore(style, document.head.firstChild); };\n__rclRegisterStyleSheet(${JSON.stringify(key)}, ${JSON.stringify(css)});\n`;
      source = source.replace(match[0], "");
    }
    await writeFile(path, prefix + source);
  }
};
await inlineStyles(stagingRoot);
await rm(cssDeclarationPath, { force: true });
