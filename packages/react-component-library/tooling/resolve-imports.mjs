...
const astMatches = (roots, language, pattern) => {
  const result = spawnSync("ast-grep", ["run", "--lang", language, "--pattern", pattern, "--json=stream", ...roots], {
    encoding: "utf8",
...
export function scanModuleSpecifiers(root) {
  const absoluteRoots = (Array.isArray(root) ? root : [root]).map((entry) => resolve(entry));
  const byFile = new Map();
  for (const language of ["typescript", "tsx"]) {
    for (const pattern of AST_PATTERNS) {
      for (const match of astMatches(absoluteRoots, language, pattern)) {
...
export function analyzeSourceFacts(root) {
  ...
  for (const pattern of JSX_PATTERNS) {
    for (const match of astMatches(absoluteRoots, "tsx", pattern)) {
  ...
  for (const language of ["typescript", "tsx"]) {
    ...
    for (const match of astMatches(absoluteRoots, language, CALL_PATTERN)) {
