  ...
  for (const [name, asset] of [...assets.entries()].sort(([left], [right]) => left.localeCompare(right))) {
	...
	if (check) {
		...
		for (const pending of pendingWrites) {
			if (!existsSync(pending.path) || await readFile(pending.path, "utf8") !== pending.content) stale.push(pending.path);
		}
		return { written: 0, stale };
	}
	for (const pending of pendingWrites) await writeFile(pending.path, pending.content);
	return { written: pendingWrites.length, stale: [] };
}

if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
	const result = await generateLocks({ check: process.argv.includes("--check") });
	if (result.stale.length > 0) {
		console.error(JSON.stringify({ staleLocks: result.stale.map((path) => relative(process.cwd(), path)) }, null, 2));
		process.exitCode = 1;
	} else {
		console.log(`${process.argv.includes("--check") ? "Checked" : "Generated"} ${result.written || "all"} dependency locks from the authored TypeScript graph.`);
	}
}
