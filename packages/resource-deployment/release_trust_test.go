...
func TestReleaseTrustModes(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "vault_linux_amd64"), body, 0o755); err != nil {
		t.Fatal(err)
	}
	...
	os.WriteFile(filepath.Join(root, "pub"), pem.EncodeToMemory(&pem.Block{Type: "PUBLIC KEY", Bytes: pub}), 0o644)
	os.WriteFile(filepath.Join(root, "release-manifest.sig.json"), []byte(`{"schema_version":"v1","key_id":"fixture","algorithm":"rsa-pkcs1v15-sha256","signature":"`+base64.StdEncoding.EncodeToString(sig)+`"}`), 0o644)
	if _, _, err = VerifyReleaseDirectory(root, ArtifactTrustProduction, filepath.Join(root, "pub")); err != nil {
