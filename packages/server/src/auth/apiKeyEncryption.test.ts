...
describe("ApiKeyEncryptionService", () => {
    ...
    it("should correctly validate keys", () => {
        expect(ApiKeyEncryptionService.isValidKey("valid")).toBe(true);
        expect(ApiKeyEncryptionService.isValidKey("")).toBe(false);
        expect(ApiKeyEncryptionService.isValidKey("a".repeat(1))).toBe(true);
        expect(ApiKeyEncryptionService.isValidKey("a".repeat(2049))).toBe(false);
        expect(ApiKeyEncryptionService.isValidKey("a".repeat(2048))).toBe(true);
    });
});
