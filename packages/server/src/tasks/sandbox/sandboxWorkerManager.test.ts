...
type WorkerManager = SandboxChildProcessManager; //SandboxWorkerThreadManager | SandboxChildProcessManager;

// Which manager(s) to test
const managers = ["process"];// ["thread", "process"] as const;
...
// // Define an interface for spied methods
...
// }
// interface SpiedChildProcess extends ChildProcess {
//     on: ChildProcess["on"] & sinon.SinonSpy;
//     send: ChildProcess["send"] & sinon.SinonSpy;
//     kill: ChildProcess["kill"] & sinon.SinonSpy;
//     removeListener: ChildProcess["removeListener"] & sinon.SinonSpy;
// }
...
// Factory function to create either manager
async function createManager(type: "thread" | "process") {
    if (type === "thread") {
        const WorkerSpyClass = class extends originalWorker {
        ...
        const mockedModules = await esmock("./sandboxWorkerManager.js", {
            "worker_threads": { Worker: WorkerSpyClass },
        });
        return new mockedModules.SandboxWorkerThreadManager({ idleTimeoutMs: IDLE_TIMEOUT_MS, jobTimeoutMs: JOB_TIMEOUT_MS });
    } else {
        // Capture the original fork function
        const originalFork = fork;

        // Create a stub that calls the original fork and collects instances
        const ForkStub = sandbox.stub().callsFake((...args: unknown[]) => {
            const cp = originalFork(...args as Parameters<typeof originalFork>);  // Call the real fork
            return cp;                                                            // Return the ChildProcess
        });

        // Mock the child_process module with the stub
        const mockedModules = await esmock("./sandboxWorkerManager.js", {
            "child_process": { fork: ForkStub },
        });

        return new mockedModules.SandboxChildProcessManager({ idleTimeoutMs: IDLE_TIMEOUT_MS, jobTimeoutMs: JOB_TIMEOUT_MS });
    }
}
...
describe("/sandbox/managers", () => {
    ...
    // Run tests for both thread and process managers
    managers.forEach((type) => {
        describe(`SandboxManager with ${type}`, () => {
            beforeEach(async () => {
                manager = await createManager(type as "thread" | "process");
            });
            ...
            describe("handle successful code execution", () => {
                ...
                it("URL - created in code", async () => {
                    const href = "https://example.com/";
                    const input = {
                        code: `function test() { return new URL("${href}"); }`,
                        codeLanguage: CodeLanguage.Javascript,
                    };
                    const result = await manager.runUserCode(input);
                ...
                //     const input = {
                //         code: "function test() { return Buffer.from('hello, world!'); }",
                //         codeLanguage: CodeLanguage.Javascript,
                //     };
                //     const result = await manager.runUserCode(input);
