...
// Had too many problems with crashing the V8 engine. Child processes seem more stable
// export class SandboxWorkerThreadManager extends AbstractSandboxWorkerManager {
//     private worker: Worker | null = null;

//     protected async _createWorker(): Promise<void> {
//         let fileName = WORKER_THREAD_FILE;
//         // The test environment runs in src, but needs to use the compiled JS. 
//         // It's a bit monkey-patchy, but it works for now.
//         if (IN_TEST_MODE) fileName = fileName.replace("/src/", "/dist/");

//         const stats = fs.statSync(fileName);
//         if (stats.size === 0) {
//             throw new Error("Worker thread file is empty. Recompile the project.");
//         }
...
//                 maxOldGenerationSizeMb: (this.memoryLimitBytes / MB) + 4,
//             },
//         });
...
//         return this.worker?.threadId?.toString() ?? null;
//     }
...
//             } else {
//                 resolve(); // Worker is null, resolve immediately
//             }
//         });
//     }
