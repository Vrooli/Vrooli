...
export async function rateLimit({
    ...
    if (byAccount && !context.req.userId) throw new CustomError(CODE.Unauthorized);
    ...
    if (count > max) {
        throw new CustomError(CODE.RateLimitExceeded, `Rate limit exceeded. Please try again in ${window} seconds.`);
    }
