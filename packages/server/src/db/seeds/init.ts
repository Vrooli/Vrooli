 ...
 */
import { AUTH_PROVIDERS, DEFAULT_LANGUAGE, FormElement, FormStructureType, generatePK, generatePublicId, InputType, ResourceUsedFor, RoutineVersionConfig, SEEDED_PUBLIC_IDS, SEEDED_TAGS, TeamConfig } from "@local/shared";
...
export async function init(client: InstanceType<typeof PrismaClient>) {
    ...
    const userData = { id: adminId.toString(), publicId: SEEDED_PUBLIC_IDS.Admin, languages: [DEFAULT_LANGUAGE] };
    const importConfig = {
    ...
    await withRedis({
        process: async (redisClient) => {
            ...
            const resourceVersions = await readManyHelper({ info, input, objectType: "ResourceVersion", req });

            // Cache the version data in Redis
            for (const resourceVersion of resourceVersions) {
                redisClient.set(`seeded:resourceVersion:${resourceVersion.id}`, JSON.stringify(resourceVersion));
            }
        },
        trace: "0705",
    });
