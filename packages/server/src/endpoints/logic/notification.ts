...
export const NotificationEndpoints: EndpointsNotification = {
    Query: {
        ...
        notificationSettings: async (_, __, { prisma, req }) => {
    ...
    Mutation: {
        notificationMarkAsRead: async (_p, { input }, { prisma, req }) => {
        ...
        notificationMarkAllAsRead: async (_, __, { prisma, req }) => {
            const { id: userId } = assertRequestFrom(req, { isUser: true });
            await rateLimit({ maxUser: 1000, req });
        ...
        notificationSettingsUpdate: async (_, { input }, { prisma, req }) => {
            const { id } = assertRequestFrom(req, { isUser: true });
