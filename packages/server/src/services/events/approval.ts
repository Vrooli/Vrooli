...
export class ApprovalSystem {
  ...
  async processApprovalRequest(toolCall: ToolCallRequest): Promise<ApprovalResult> {
    ...
    return new Promise<ApprovalResult>((resolve, reject) => {
      ...
      this.eventBus.publish(approvalEvent)
        .then(result => {
          if (!result.success) {
  ...
  async cancelPendingApproval(pendingId: string, reason: string): Promise<void> {
    ...
    await this.eventBus.publish(cancellationEvent);
  ...
  private handleApprovalTimeout(pendingId: string, startTime: number): void {
    ...
    this.eventBus.publish(timeoutEvent).catch(error => {
 ...
 */
export function createApprovalSystem(eventBus: IEventBus, logger: Logger): ApprovalSystem {
  return new ApprovalSystem(eventBus, logger);
}
