 ...
 * 
 * This service provides sophisticated prompt generation capabilities including
 * template processing, swarm state formatting, and role-specific instructions.
 * 
 * Key Features:
...
import type { BotParticipant, ChatConfigObject, ModelType, SessionUser, SwarmState, TeamConfigObject, TriggerContext } from "@vrooli/shared";
 ...
 * 
 * This service provides advanced template-based prompt generation with
 * sophisticated variable substitution and state formatting capabilities.
 */
export class PromptService {
    ...
    static async buildSystemMessage(
        ...
        logger.debug("[PromptService] Building system message", {
        ...
        if ((scope === "swarm" || scope === "context" || scope === "config")) {
            try {
                ...
                if (context.swarmState) {
                    ...
                    logger.debug(`[PromptService] Built TriggerContext from SwarmState for path ${path}`, {
                ...
                logger.debug("[PromptService] SwarmStateAccessor resolved variable successfully", {
                    path,
                ...
                logger.warn(`[PromptService] SwarmStateAccessor failed for path ${path}`, {
     ...
     * Builds formatted swarm state representation for prompts
     */
     ...
     * Truncates long strings for prompt inclusion with smart boundaries
     */
