...
export class StrategyFactory {
    ...
    selectStrategy(context: ExecutionContext): ExecutionStrategy | null {
        this.logger.debug("[StrategyFactory] Selecting strategy", {
        ...
        if (requestedStrategy) {
            ...
            if (strategyType) {
                ...
                if (strategy) {
                    this.logger.debug("[StrategyFactory] Using requested strategy", {
                        type: strategyType,
                    });
                    return strategy;
                }
        ...
        if (context.constraints?.allowedStrategies) {
            ...
            for (const strategy of availableStrategies) {
                if (strategy.canHandle(context.stepType, context.config)) {
                    this.logger.debug("[StrategyFactory] Selected from allowed strategies", {
                        type: strategy.type,
                    });
                    return strategy;
                }
        ...
        if (bestStrategy.score > 0) {
            ...
            this.logger.info("[StrategyFactory] Selected strategy", {
        ...
        this.logger.warn("[StrategyFactory] No suitable strategy found", {
            stepType: context.stepType,
        });
