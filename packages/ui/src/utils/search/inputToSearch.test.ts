...
describe('arrayToSearch', () => {
    ...
    const convertNumber = (item: unknown) => {
    ...
    const convertString = (item: unknown) => {
...
describe('searchToArray', () => {
    ...
    const convertNumber = (value: UrlPrimitive) => {
        const num = Number(value);
        return Number.isFinite(num) && num > 0 ? Math.round(num) : undefined;
    };

    const convertString = (value: UrlPrimitive) => {
...
describe('Round-Trip Tag Conversion', () => {
    it('should preserve tag data through conversions', () => {
        ...
        const roundTripResults = validTags.map(tag => stringToTagObject(tagObjectToString({ tag: tag })));
