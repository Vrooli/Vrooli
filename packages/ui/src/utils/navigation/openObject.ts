 ...
 */
export function openObject(object: NavigableObject, setLocation: SetLocation) {
 ...
 */
export function getObjectEditUrl(object: NavigableObject) {
    return `${getObjectUrlBase(object)}/edit/${getObjectSlug(object, true)}${getObjectSearchParams(object)}`;
}
 ...
 * @param setLocation Function to set location in history
 */
export function openObjectEdit(object: NavigableObject, setLocation: SetLocation) {
    // Actions don't open to anything
    if (isOfType(object, "Action")) return;
    // Store object in local storage, so we can display it while the full data loads
    setCookiePartialData(object, "list");
 ...
 * @param object Object being navigated to
 */
export function getObjectReportUrl(object: NavigableObject) {
    return `${getObjectUrlBase(object)}/reports/${getObjectSlug(object)}`;
}
 ...
 * @param object Object to open
 */
export function openObjectReport(object: NavigableObject, setLocation: SetLocation) {
    return setLocation(getObjectReportUrl(object));
}
 ...
 */
export function getResourceType(link: string): ResourceType | null {
 ...
 */
export function getResourceUrl(link: string): string | undefined {
