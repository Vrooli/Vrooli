...
import { Headers, displayDate, firstString, fontSizeToPixels, generateContext, getLineAtIndex, getLineEnd, getLineStart, getLinesAtRange, getTextSelection, insertBulletList, insertCheckboxList, insertCode, insertHeader, insertLink, insertNumberList, insertQuote, insertTable, padSelection, replaceText } from "./stringTools.js";
...
describe("getLineStart", () => {
    it("returns 0 for single-line text regardless of start index", () => {
        ...
        expect(getLineStart(text, 10)).to.equal(0);
        expect(getLineStart(text, text.length)).to.equal(0); // End of line
    });

    it("returns the correct index at the start of the text", () => {
        const text = "First line\nSecond line\nThird line";
        expect(getLineStart(text, 0)).to.equal(0); // At the very beginning
    });

    it("returns the correct index for multi-line text when cursor is on the first line", () => {
        const text = "First line\nSecond line\nThird line";
        expect(getLineStart(text, 5)).to.equal(0); // In the middle of the first line
    });
    ...
    it("returns the correct index for multi-line text when cursor is on the middle line", () => {
        ...
        expect(getLineStart(text, startSecondLine + 5)).to.equal(startSecondLine);
    });
    ...
    it("returns the correct index for multi-line text when cursor is on the last line", () => {
        ...
        expect(getLineStart(text, startLastLine + 5)).to.equal(startLastLine);
    });

    it("returns the correct index when cursor is at the end of the text", () => {
        const text = "First line\nSecond line\nThird line";
        expect(getLineStart(text, text.length)).to.equal(text.lastIndexOf("\n") + 1);
    });

    it("handles the start index being out of bounds", () => {
        const text = "Out of bounds test";
        expect(getLineStart(text, -1)).to.equal(0); // Less than 0
        expect(getLineStart(text, text.length + 1)).to.equal(0); // Greater than text length
    });

    it("handles empty string correctly", () => {
        const text = "";
        expect(getLineStart(text, 0)).to.equal(0);
    });

    it("returns the start index when the cursor is immediately after a newline character", () => {
        const text = "First line\nSecond line\nThird line";
        const startSecondLine = text.indexOf("\n") + 1; // Start of second line
        expect(getLineStart(text, startSecondLine)).to.equal(startSecondLine);
    });
...
describe("getLineEnd", () => {
    it("returns the length of text for single-line text regardless of start index", () => {
        const text = "This is a single line of text.";
        expect(getLineEnd(text, 10)).to.equal(text.length);
        expect(getLineEnd(text, text.length)).to.equal(text.length); // Cursor at the end of the line
    });

    it("returns the length of text when cursor is at the end of the text", () => {
        const text = "End of text\nSecond line\nThird line";
        expect(getLineEnd(text, text.length)).to.equal(text.length);
    });

    it("returns the correct index for multi-line text when cursor is on the first line", () => {
        const text = "First line\nSecond line\nThird line";
        const endFirstLine = text.indexOf("\n"); // End of first line
        expect(getLineEnd(text, 5)).to.equal(endFirstLine); // Cursor in the middle of the first line
    });
    ...
    it("returns the correct index for multi-line text when cursor is on the middle line", () => {
        ...
        expect(getLineEnd(text, startSecondLine + 5)).to.equal(endSecondLine);
    });

    it("returns text length for multi-line text when cursor is on the last line", () => {
        const text = "First line\nSecond line\nThird line";
        const startLastLine = text.lastIndexOf("\n") + 1; // Start of last line
        expect(getLineEnd(text, startLastLine + 5)).to.equal(text.length);
    });

    it("handles the start index being out of bounds", () => {
        const text = "Out of bounds test";
        expect(getLineEnd(text, -1)).to.equal(text.length); // Less than 0
        expect(getLineEnd(text, text.length + 1)).to.equal(text.length); // Greater than text length
    });

    it("handles empty string correctly", () => {
        const text = "";
        expect(getLineEnd(text, 0)).to.equal(0);
    });
...
describe("getLineAtIndex", () => {
    ...
    it("returns the middle line with correct start and end indices for multi-line input", () => {
        ...
        expect(getLineAtIndex(text, startOfSecondLine + 1)).to.deep.equal([secondLine, startOfSecondLine, startOfSecondLine + secondLine.length]);
    });
    ...
    it("returns the last line with correct start and end indices for multi-line input", () => {
        ...
        expect(getLineAtIndex(text, startOfLastLine + 1)).to.deep.equal([lastLine, startOfLastLine, text.length]);
    });

    it("handles index at the end of the text correctly", () => {
        const text = "Text ending without newline";
        expect(getLineAtIndex(text, text.length - 1)).to.deep.equal([text, 0, text.length]);
    });
    ...
    it("returns the correct line and indices when index is on a newline character", () => {
        ...
        expect(getLineAtIndex(text, startOfLineTwo)).to.deep.equal([lineTwo, startOfLineTwo, startOfLineTwo + lineTwo.length]);
    });
    ...
    it("returns the correct line for empty lines", () => {
        ...
        expect(getLineAtIndex(text, startOfEmptyLine)).to.deep.equal(["", startOfEmptyLine, startOfEmptyLine]);
    });
});
...
Fourth line`;
    ...
    it("returns a single line when start and end are within the same line", () => {
        ...
        expect(getLinesAtRange(multiLineText, start, end)).to.deep.equal([["First line"], start, multiLineText.indexOf("\n")]);
    });
    ...
    it("returns the correct start and end indices for a single line", () => {
        ...
        expect(getLinesAtRange(multiLineText, start, end)).to.deep.equal([["Second line"], lineStart, lineEnd]);
    });
    ...
    it("returns multiple lines when the range spans across lines", () => {
        ...
        expect(getLinesAtRange(multiLineText, start, end)).to.deep.equal([
    ...
    it("returns an empty array and 0 for start and end indices for an empty string", () => {
        expect(getLinesAtRange("", 0, 0)).to.deep.equal([[], 0, 0]);
    });

    it("returns an empty array and 0 for start and end indices if start and end are out of bounds", () => {
        expect(getLinesAtRange(multiLineText, -1, multiLineText.length + 1)).to.deep.equal([[], 0, 0]);
    });
    ...
    it("returns the last line correctly when the range starts in the last line", () => {
        ...
        expect(getLinesAtRange(multiLineText, start, end)).to.deep.equal([["Fourth line"], start, end]);
    });
