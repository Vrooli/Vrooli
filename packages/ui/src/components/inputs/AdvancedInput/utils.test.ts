...
describe("MarkdownUtils", () => {
    describe("replaceText", () => {
        it("replaces text in the middle of the string", () => {
            ...
            expect(result).to.equal("Hello, beautiful!");
        });
    ...
    describe("getLineStart", () => {
        it("returns 0 for single-line text regardless of start index", () => {
            ...
            expect(MarkdownUtils.getLineStart(text, 10)).to.equal(0);
            expect(MarkdownUtils.getLineStart(text, text.length)).to.equal(0); // End of line
        });

        it("returns the correct index at the start of the text", () => {
            const text = "First line\nSecond line\nThird line";
            expect(MarkdownUtils.getLineStart(text, 0)).to.equal(0); // At the very beginning
        });

        it("returns the correct index for multi-line text when cursor is on the first line", () => {
            const text = "First line\nSecond line\nThird line";
            expect(MarkdownUtils.getLineStart(text, 5)).to.equal(0); // In the middle of the first line
        });
        ...
        it("returns the correct index for multi-line text when cursor is on the middle line", () => {
            ...
            expect(MarkdownUtils.getLineStart(text, startSecondLine + 5)).to.equal(startSecondLine);
        });
        ...
        it("returns the correct index for multi-line text when cursor is on the last line", () => {
            ...
            expect(MarkdownUtils.getLineStart(text, startLastLine + 5)).to.equal(startLastLine);
        });

        it("returns the correct index when cursor is at the end of the text", () => {
            const text = "First line\nSecond line\nThird line";
            expect(MarkdownUtils.getLineStart(text, text.length)).to.equal(text.lastIndexOf("\n") + 1);
        });

        it("handles the start index being out of bounds", () => {
            const text = "Out of bounds test";
            expect(MarkdownUtils.getLineStart(text, -1)).to.equal(0); // Less than 0
            expect(MarkdownUtils.getLineStart(text, text.length + 1)).to.equal(0); // Greater than text length
        });

        it("handles empty string correctly", () => {
            const text = "";
            expect(MarkdownUtils.getLineStart(text, 0)).to.equal(0);
        });

        it("returns the start index when the cursor is immediately after a newline character", () => {
            const text = "First line\nSecond line\nThird line";
            const startSecondLine = text.indexOf("\n") + 1; // Start of second line
            expect(MarkdownUtils.getLineStart(text, startSecondLine)).to.equal(startSecondLine);
        });
    ...
    describe("getLineEnd", () => {
        it("returns the length of text for single-line text regardless of start index", () => {
            const text = "This is a single line of text.";
            expect(MarkdownUtils.getLineEnd(text, 10)).to.equal(text.length);
            expect(MarkdownUtils.getLineEnd(text, text.length)).to.equal(text.length); // Cursor at the end of the line
        });

        it("returns the length of text when cursor is at the end of the text", () => {
            const text = "End of text\nSecond line\nThird line";
            expect(MarkdownUtils.getLineEnd(text, text.length)).to.equal(text.length);
        });

        it("returns the correct index for multi-line text when cursor is on the first line", () => {
            const text = "First line\nSecond line\nThird line";
            const endFirstLine = text.indexOf("\n"); // End of first line
            expect(MarkdownUtils.getLineEnd(text, 5)).to.equal(endFirstLine); // Cursor in the middle of the first line
        });
        ...
        it("returns the correct index for multi-line text when cursor is on the middle line", () => {
            ...
            expect(MarkdownUtils.getLineEnd(text, startSecondLine + 5)).to.equal(endSecondLine);
        });

        it("returns text length for multi-line text when cursor is on the last line", () => {
            const text = "First line\nSecond line\nThird line";
            const startLastLine = text.lastIndexOf("\n") + 1; // Start of last line
            expect(MarkdownUtils.getLineEnd(text, startLastLine + 5)).to.equal(text.length);
        });

        it("handles the start index being out of bounds", () => {
            const text = "Out of bounds test";
            expect(MarkdownUtils.getLineEnd(text, -1)).to.equal(text.length); // Less than 0
            expect(MarkdownUtils.getLineEnd(text, text.length + 1)).to.equal(text.length); // Greater than text length
        });

        it("handles empty string correctly", () => {
            const text = "";
            expect(MarkdownUtils.getLineEnd(text, 0)).to.equal(0);
        });
...
Fourth line`;
        ...
        it("returns a single line when start and end are within the same line", () => {
            ...
            expect(MarkdownUtils.getLinesAtRange(multiLineText, start, end)).to.deep.equal([["First line"], start, multiLineText.indexOf("\n")]);
        });
        ...
        it("returns the correct start and end indices for a single line", () => {
            ...
            expect(MarkdownUtils.getLinesAtRange(multiLineText, start, end)).to.deep.equal([["Second line"], lineStart, lineEnd]);
        });
        ...
        it("returns multiple lines when the range spans across lines", () => {
            ...
            expect(MarkdownUtils.getLinesAtRange(multiLineText, start, end)).to.deep.equal([
        ...
        it("returns an empty array and 0 for start and end indices for an empty string", () => {
            expect(MarkdownUtils.getLinesAtRange("", 0, 0)).to.deep.equal([[], 0, 0]);
        });

        it("returns an empty array and 0 for start and end indices if start and end are out of bounds", () => {
            expect(MarkdownUtils.getLinesAtRange(multiLineText, -1, multiLineText.length + 1)).to.deep.equal([[], 0, 0]);
        });
        ...
        it("returns the last line correctly when the range starts in the last line", () => {
            ...
            expect(MarkdownUtils.getLinesAtRange(multiLineText, start, end)).to.deep.equal([["Fourth line"], start, end]);
        });
    ...
    describe("getLineAtIndex", () => {
        ...
        it("returns the middle line with correct start and end indices for multi-line input", () => {
            ...
            expect(MarkdownUtils.getLineAtIndex(text, startOfSecondLine + 1)).to.deep.equal([secondLine, startOfSecondLine, startOfSecondLine + secondLine.length]);
        });
        ...
        it("returns the last line with correct start and end indices for multi-line input", () => {
            ...
            expect(MarkdownUtils.getLineAtIndex(text, startOfLastLine + 1)).to.deep.equal([lastLine, startOfLastLine, text.length]);
        });

        it("handles index at the end of the text correctly", () => {
            const text = "Text ending without newline";
            expect(MarkdownUtils.getLineAtIndex(text, text.length - 1)).to.deep.equal([text, 0, text.length]);
        });
        ...
        it("returns the correct line and indices when index is on a newline character", () => {
            ...
            expect(MarkdownUtils.getLineAtIndex(text, startOfLineTwo)).to.deep.equal([lineTwo, startOfLineTwo, startOfLineTwo + lineTwo.length]);
        });
        ...
        it("returns the correct line for empty lines", () => {
            ...
            expect(MarkdownUtils.getLineAtIndex(text, startOfEmptyLine)).to.deep.equal(["", startOfEmptyLine, startOfEmptyLine]);
        });
    });
    ...
    describe("padSelection function", () => {
        ...
        it("should add underline to unstyled text", () => {
            ...
            expect(text).to.equal("<u>This is a test</u>");
            expect(start).to.equal(3);
            expect(end).to.equal(17);
        });
        ...
        it("should toggle strikethrough when already applied", () => {
            ...
            expect(text).to.equal("Normal Strikethrough Normal");
        });
        ...
        it("should toggle underline when already applied", () => {
            ...
            expect(text).to.equal("Normal Underline Normal");
        });
        ...
        it("should toggle spoiler when already applied", () => {
            ...
            expect(text).to.equal("Normal Spoiler Normal");
        });
    });
    ...
    describe("insertLink function", () => {
        ...
        it("should insert a placeholder link at the cursor position when no text is selected", () => {
            ...
            expect(start).to.equal(expectedText.indexOf("url"));
            expect(end).to.equal(expectedText.indexOf("url") + 3);
        });
        ...
        it("should recognize a valid URL in the selected text and use it as the link target", () => {
            ...
            expect(start).to.equal(expectedText.indexOf("label"));
            expect(end).to.equal(expectedText.indexOf("label") + 5);
        });
    ...
    describe("insertBulletList", () => {
        it("should add bullets to unbulleted lines", () => {
            ...
            expect(start).to.equal(2);
            expect(end).to.equal(expectedText.length - 1); // Still one character before the end
        });
        ...
        it("should remove bullets from bulleted lines", () => {
            ...
            expect(start).to.equal(3); // In the same relative position as before
            expect(end).to.equal(expectedText.length);
        });
        ...
        it("should add a bullet to a single line without affecting the other lines", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            // Check if the selection has been correctly adjusted
            expect(start).to.equal(9); // 7 + "* ".length
            expect(end).to.equal(15); // 13 + "* ".length
        });
        ...
        it("should handle selection that starts and ends mid-line", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(16); // 14 + "* ".length
            expect(end).to.equal(34); // 30 + "* ".length * 2, since two bullets were added
        });
        ...
        it("should handle empty lines correctly", () => {
            ...
            const expectedText = "* ";
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(2); // After the bullet
            expect(end).to.equal(2); // At the end of the text
        });
    });
    ...
    describe("insertNumberList", () => {
        it("should add numbers to unnumbered lines", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(3); // After "1. "
            expect(end).to.equal(expectedText.length);
        });
        ...
        it("should remove numbers from numbered lines", () => {
            ...
            expect(start).to.equal(0);
            expect(end).to.equal(expectedText.length - 2);
        });
        ...
        it("should add numbers only to selected lines", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            // Check if the selection has been correctly adjusted
            expect(start).to.equal(10); // After "1. "
            expect(end).to.equal(expectedText.length - 7); // At the end of "Line 3"
        });
        ...
        it("should handle empty lines correctly", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(3); // After the bullet
            expect(end).to.equal(3); // At the end of the text
        });
    });
    ...
    describe("insertCheckboxList", () => {
        ...
        it("should handle partial line selection correctly and mixed checkboxed and uncheckboxed lines", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            // Check if the selection has been correctly adjusted
            expect(start).to.equal(7 + "- [ ] ".length);
            expect(end).to.equal(expectedText.length - 5); // Same relative position in "Line 3"
        });
        ...
        it("should handle empty lines correctly", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(6); // After "- [ ] "
            expect(end).to.equal(6); // At the end of the text
        });
        ...
        it("should toggle checkboxes for a single line selection", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            // Check if the selection has been correctly adjusted
            expect(start).to.equal(7); // Beginning of Line 2
            expect(end).to.equal(18 - "- [ ] ".length); // Same relative position in "Line 2"
        });
    ...
    describe("insertCode", () => {
        it("should wrap a single line in inline code markers if not already wrapped", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(1); // After "`"
            expect(end).to.equal(expectedText.length - 1); // Before the closing "`"
        });
        ...
        it("should wrap multiple lines in code block markers if not already wrapped", () => {
            ...
            const expectedText = "```\nLine 1\nLine 2\nLine 3\n```";
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(4); // After "```\n"
            expect(end).to.equal(expectedText.length - 4); // Before the closing "\n```"
        });
        ...
        it("should correctly handle single line selection surrounded by other text", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            expect(updatedStart).to.equal(start);
            expect(updatedEnd).to.equal(end - 2); // 2 code markers removed
        });
        ...
        it("should add inline code markers for single line selection surrounded by other text if not present", () => {
            ...
            const expectedText = "Some text `inline code` some more text.";
            expect(updatedText).to.equal(expectedText);
            expect(updatedStart).to.equal(11);
            expect(updatedEnd).to.equal(22);
        });
        ...
        it("should handle an empty selection by inserting inline code markers", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(expectedText.length - 1);
            expect(end).to.equal(expectedText.length - 1);
        });
        ...
        it("should correctly handle multiline selection with mixed content (code and text)", () => {
            ...
            const expectedText = "```\nSome text\n```\nCode block\n```\n```";
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(4); // After the first "```\n"
            expect(end).to.equal(expectedText.length - 4); // Before the last "\n```"
        });
    });
    ...
    describe("insertQuote function", () => {
        it("should add a quote marker to unquoted text", () => {
            ...
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(2); // Adjusted for "> "
            expect(end).to.equal(expectedText.length);
        });
        ...
        it("should remove a quote marker from quoted text", () => {
            ...
            const expectedText = "This is a test.";
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(0); // Adjusted for removal of "> "
            expect(end).to.equal(expectedText.length);
        });
        ...
        it("should add a quote marker to each line in a multi-line selection", () => {
            ...
            const expectedText = "> Line 1\n> Line 2\n> Line 3";
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(2); // Adjusted for first "> "
            expect(end).to.equal(expectedText.length);
        });
        ...
        it("should handle an empty selection by adding a quote marker", () => {
            ...
            const expectedText = "> ";
            expect(updatedText).to.equal(expectedText);
            expect(start).to.equal(expectedText.length); // At the end of the new quote
            expect(end).to.equal(expectedText.length);
        });
    });
...
`;
            ...
            expect(start).to.equal(updatedText.indexOf("Header"));
            expect(end).to.equal(updatedText.indexOf("Header") + "Header".length);
        });
...
`;
        ...
        it("should handle zero rows correctly by doing nothing", () => {
            ...
            expect(start).to.equal(text.length);
...
|   |`;
    ...
    describe("insertStyle function", () => {
        it("should apply bold style correctly", () => {
            ...
            expect(updatedText).to.equal("**This is a test**");
            expect(start).to.equal(2);
            expect(end).to.equal(updatedText.length - 2);
        });
        ...
        it("should apply italic style correctly", () => {
            ...
            expect(updatedText).to.equal("*This is a test*");
            expect(start).to.equal(1);
            expect(end).to.equal(updatedText.length - 1);
        });
        ...
        it("should apply header styles correctly", () => {
            ...
            expect(updatedText).to.equal("## This is a test");
            expect(start).to.equal(3);
            expect(end).to.equal(updatedText.length);
        });
        ...
        it("should apply bullet list style correctly", () => {
            ...
            expect(updatedText).to.equal("* Line 1\n* Line 2");
            expect(start).to.equal(2);
            expect(end).to.equal(updatedText.length);
        });
        ...
        it("should apply number list style correctly", () => {
            ...
            expect(updatedText).to.equal("1. Line 1\n2. Line 2");
            expect(start).to.equal(3);
            expect(end).to.equal(updatedText.length);
        });
        ...
        it("should apply link style correctly", () => {
            ...
            expect(updatedText).to.equal("[label](https://example.com)");
            expect(start).to.equal(1);
            expect(end).to.equal(6);
        });
        ...
        it("should apply spoiler style correctly", () => {
            ...
            expect(updatedText).to.equal("||This is a spoiler||");
            expect(start).to.equal(2);
            expect(end).to.equal(updatedText.length - 2);
        });
        ...
        it("should apply strikethrough style correctly", () => {
            ...
            expect(updatedText).to.equal("~~This is struck through~~");
            expect(start).to.equal(2);
            expect(end).to.equal(updatedText.length - 2);
        });
        ...
        it("should apply underline style correctly", () => {
            ...
            expect(updatedText).to.equal("<u>This is underlined</u>");
            expect(start).to.equal(3);
            expect(end).to.equal(updatedText.length - 4);
        });
        ...
        it("should handle invalid style gracefully", () => {
            ...
            expect(updatedText).to.equal(text);
            expect(start).to.equal(0);
            expect(end).to.equal(text.length);
        });
        ...
        it("should handle partial text selection correctly", () => {
            ...
            expect(updatedText).to.equal("This **is a** test sentence");
        ...
        it("should toggle styles when applied twice", () => {
            ...
            expect(secondApply.text).to.equal(text);
            expect(secondApply.start).to.equal(0);
            expect(secondApply.end).to.equal(text.length);
        });
    });
