...
import { RelationshipItemTeam } from "components/lists/types";
...
export const TeamButton = ({
    ...
    const [field, , helpers] = useField("team");
    ...
    const handleSelect = useCallback((relation: RelationshipItemTeam) => {
    ...
    const tooltip = useMemo(() => {
        ...
        if (!relation) return t(`TeamNoneTogglePress${isEditing ? "Editable" : ""}`);
        return t(`TeamTogglePress${isEditing ? "Editable" : ""}`, { team: getDisplay(relation).title ?? "" });
                ...
                <Tooltip title={tooltip}>
                    ...
                    >
                        {field?.value ? (
                            ...
                            >
                                <TeamIcon width="75%" height="75%" fill="white" />
                            </Avatar>
                        ) : <AddIcon width={"48px"} height={"48px"} fill="white" />}
                        <Typography variant="body1" sx={{ color: "white" }}>
                            {field?.value ? getDisplay(field?.value).title : t("Team")}
                        </Typography>
                    </Stack>
                </Tooltip>
            </Stack>
