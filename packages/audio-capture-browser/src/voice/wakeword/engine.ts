...
/** Cepstral mean subtraction used by the original enrollment fixtures. */
export function applyCms(mfccs: number[][]): number[][] {
  const first = mfccs[0];
  if (!first) return mfccs;
  const means = new Array<number>(first.length).fill(0);
  for (const frame of mfccs) {
    for (let index = 0; index < first.length; index += 1) means[index] = (means[index] ?? 0) + (frame[index] ?? 0);
  }
  return mfccs.map((frame) => frame.map((value, index) => value - (means[index] ?? 0) / mfccs.length));
}
