...
func TestBuildMeterInventoryAggregatesScenarioDeclarations(t *testing.T) {
	...
	if inventory.Meters[0].LimitKey != "ai_credits" || len(inventory.Meters[0].DeclaredBy) != 6 || !inventory.Meters[0].Byok {
		t.Fatalf("ai_credits summary = %+v", inventory.Meters[0])
	}
	if inventory.Meters[1].LimitKey != "voice_minutes" || len(inventory.Meters[1].DeclaredBy) != 2 {
