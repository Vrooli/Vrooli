...
// GateDecision is the permission-command gate verdict. The runtime's
// provider evaluation uses the distinct Decision type in policy.go.
...
func Decide(kind cliutil.CallerKind, cmd CommandSpec, flags CallerOverrideFlags, policy Policy) GateDecision {
	if !cmd.Mutating {
