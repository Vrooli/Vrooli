...
export const VoteButton = ({ direction = "column", disabled = false, emoji, score, objectId, voteFor, onChange, }) => {
    const session = useContext(SessionContext);
    const { palette } = useTheme();
    const { id: userId } = useMemo(() => getCurrentUser(session), [session]);
    const [internalEmoji, setInternalEmoji] = useState(emoji);
    ...
    const onVoteComplete = useCallback((action, data) => {
    ...
    const handleVote = useCallback((event, emoji) => {
        event.stopPropagation();
        event.preventDefault();
        mutate(emoji);
    }, [mutate]);
    const handleUpvoteClick = useCallback((event) => {
        if (!userId || disabled)
            return;
    ...
    const handleDownvoteClick = useCallback((event) => {
        if (!userId || disabled)
            return;
        const vote = getReactionScore(internalEmoji) < 0 ? null : "👎";
        setInternalEmoji(vote);
    ...
    return (_jsxs(Stack, { direction: direction, sx: { pointerEvents: "none" }, children: [_jsx(Box, { display: "inline-block", onClick: handleUpvoteClick, role: "button", "aria-pressed": getReactionScore(internalEmoji) > 0, sx: {
                ...
                }, children: _jsx(UpvoteIcon, { width: "36px", height: "36px", fill: upvoteColor }) }), _jsx(Typography, { variant: "body1", textAlign: "center", sx: { margin: "auto", pointerEvents: "none" }, children: internalScore }), _jsx(Box, { display: "inline-block", onClick: handleDownvoteClick, role: "button", "aria-pressed": getReactionScore(internalEmoji) < 0, sx: {
                    cursor: (userId || disabled) ? "pointer" : "default",
                    pointerEvents: "all",
                    display: "flex",
                    "&:hover": {
                ...
                }, children: _jsx(DownvoteIcon, { width: "36px", height: "36px", fill: downvoteColor }) })] }));
};
