...
const withFontSize = (theme, fontSize) => createTheme({
    ...theme,
    typography: {
        fontSize,
    },
});
const withIsLeftHanded = (theme, isLeftHanded) => createTheme({
    ...theme,
    isLeftHanded,
});
const findThemeWithoutSession = () => {
...
export function App() {
    ...
    const [theme, setTheme] = useState(findThemeWithoutSession());
    ...
    const [validateSession] = useCustomMutation(authValidateSession);
    const [setActiveFocusMode] = useCustomMutation(focusModeSetActive);
    const isSettingActiveFocusMode = useRef(false);
    useEffect(() => {
    ...
    const setThemeAndMeta = useCallback((theme) => {
...
@@       @@       @@            @@                                  
 @@     @@        @             @@@@@@@@@@                Consider developing with us!  
   ...
   @@       @@      @@        @@             @@     @@ 
    ...
    useEffect(() => {
        let theme;
        if (Array.isArray(session?.users) && session?.users[0]?.theme)
            theme = themes[session?.users[0]?.theme];
        if (!theme)
            theme = findThemeWithoutSession();
    ...
    const checkSession = useCallback((data) => {
        if (data) {
            setSession(data);
            return;
        }
    ...
    useEffect(() => {
        ...
        const loadingSub = PubSub.get().subscribeLoading((data) => {
            ...
            if (Number.isInteger(data)) {
                timeoutRef.current = setTimeout(() => setIsLoading(true), Math.abs(data));
            }
            else {
    ...
    return (_jsxs(StyledEngineProvider, { injectFirst: true, children: [_jsx(CssBaseline, {}), _jsx(ThemeProvider, { theme: theme, children: _jsx(SessionContext.Provider, { value: session, children: _jsxs(Box, { id: "App", component: "main", sx: {
                            background: theme.palette.background.default,
                            color: theme.palette.background.textPrimary,
                        }, children: [_jsx(PullToRefresh, {}), _jsx(CommandPalette, {}), _jsx(FindInPage, {}), _jsx(WelcomeDialog, { isOpen: isWelcomeDialogOpen, onClose: () => setIsWelcomeDialogOpen(false) }), isCelebrating && _jsx(AsyncConfetti, {}), _jsx(AlertDialog, {}), _jsx(SnackStack, {}), _jsxs(Box, { id: "content-wrap", sx: {
                                    ...
                                    minHeight: { xs: "calc(100vh - 56px - env(safe-area-inset-bottom))", md: "100vh" },
                                            ...
                                            zIndex: 100000,
                                        }, children: _jsx(DiagonalWaveLoader, { size: 100 }) }), _jsx(Routes, { sessionChecked: session !== undefined })] }), _jsx(BottomNav, {}), _jsx(BannerChicken, {}), _jsx(Footer, {})] }) }) })] }));
}
