...
export async function init(prisma) {
    ...
    const admin = await prisma.user.upsert({
        ...
        create: {
            ...
            wallets: {
                create: [
                    { stakingAddress: process.env.ADMIN_WALLET ?? "", verified: true },
                ],
            },
            languages: {
                create: [{ language: EN }],
    ...
    let mintToken = await prisma.routine.findFirst({
    ...
    const mintNftId = "4e038f3b-f8f9-4f9b-8f9b-f8f9b8f9b8f9";
    let mintNft = await prisma.routine.findFirst({
        where: { id: mintNftId },
    });
    if (!mintNft) {
