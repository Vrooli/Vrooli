import { ProjectOrOrganizationSortBy, ProjectOrRoutineSortBy, RunProjectOrRunRoutineSortBy } from "@local/consts";
...
export const resolvers = {
    ...
    ProjectOrRoutine: { __resolveType(obj) { return resolveUnion(obj); } },
    ProjectOrOrganization: { __resolveType(obj) { return resolveUnion(obj); } },
    Query: {
        projectOrRoutines: async (_, { input }, { prisma, req }, info) => {
            ...
            const withSupplemental = await addSupplementalFieldsMultiTypes({
                ...
                projects: { type: "Project", ...partial.Project },
                routines: { type: "Routine", ...partial.Routine },
            }, prisma, getUser(req));
            const nodes = [];
            ...
            const combined = {
                ...
                edges: nodes.map((node) => ({ __typename: "ProjectOrRoutineEdge", cursor: node.id, node })),
            };
            return combined;
        },
        projectOrOrganizations: async (_, { input }, { prisma, req }, info) => {
            ...
            const withSupplemental = await addSupplementalFieldsMultiTypes({
                ...
                projects: { type: "Project", ...partial.Project },
                organizations: { type: "Organization", ...partial.Organization },
            }, prisma, getUser(req));
            const nodes = [];
            ...
            const combined = {
                ...
                edges: nodes.map((node) => ({ __typename: "ProjectOrOrganizationEdge", cursor: node.id, node })),
            };
            return combined;
        },
        runProjectOrRunRoutines: async (_, { input }, { prisma, req }, info) => {
            ...
            const withSupplemental = await addSupplementalFieldsMultiTypes({
                ...
                runProjects: { type: "RunProject", ...partial.RunProject },
                runRoutines: { type: "RunRoutine", ...partial.RunRoutine },
            }, prisma, getUser(req));
            const nodes = [];
            ...
            const combined = {
                ...
                edges: nodes.map((node) => ({ __typename: "RunProjectOrRunRoutineEdge", cursor: node.id, node })),
            };
            return combined;
        },
    },
