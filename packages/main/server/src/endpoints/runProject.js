import { RunProjectSortBy, RunStatus } from "@local/consts";
...
export const resolvers = {
    ...
    Mutation: {
        ...
        runProjectDeleteAll: async (_p, _d, { prisma, req }, info) => {
            ...
            return RunProjectModel.danger.deleteAll(prisma, { __typename: "User", id: userData.id });
        },
        runProjectComplete: async (_, { input }, { prisma, req }, info) => {
            const userData = assertRequestFrom(req, { isUser: true });
            await rateLimit({ info, maxUser: 1000, req });
            return RunProjectModel.run.complete(prisma, userData, input, info);
        },
        runProjectCancel: async (_, { input }, { prisma, req }, info) => {
            const userData = assertRequestFrom(req, { isUser: true });
            await rateLimit({ info, maxUser: 1000, req });
            return RunProjectModel.run.cancel(prisma, userData, input, info);
        },
    },
};
