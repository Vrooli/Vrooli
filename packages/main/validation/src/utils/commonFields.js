...
export const handle = yup.string().transform(blankToUndefined).min(3, minStrErr).max(16, maxStrErr).test("handle", "Must start with a '$', be from 3-16 characters, and only contain letters and numbers", (value) => {
...
export const url = yup.string().transform(blankToUndefined).max(1024, maxStrErr).test("link", "Must be a URL", (value) => {
...
export const language = yup.string().transform(blankToUndefined).min(2, minStrErr).max(3, maxStrErr);
