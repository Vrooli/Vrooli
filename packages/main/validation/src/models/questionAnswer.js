...
import { blankToUndefined, id, maxStrErr, minStrErr, opt, req, transRel, yupObj } from "../utils";
const text = yup.string().transform(blankToUndefined).min(1, minStrErr).max(32768, maxStrErr);
export const questionAnswerTranslationValidation = transRel({
    create: {
        text: req(text),
    },
    update: {
...
export const questionAnswerValidation = {
    create: ({ o }) => yupObj({
        id: req(id),
    }, [
