...
// not a mandate.
//
// The CollectionSpec uses cfg.EmbedDimensions when the caller supplied resolved
// policy metadata. Otherwise it falls back to DefaultVectorSize for tests and
// non-Ollama adopters. Ollama-backed consumers should resolve policy metadata
...
func NewDenseEngine(cfg Config, collection string) DenseEngine {
	return DenseEngine{
		...
		Spec: CollectionSpec{
			Name:                collection,
			DenseSize:           denseSizeForConfig(cfg),
			DenseDistance:       DefaultDenseDistance,
			Model:               cfg.EmbedModel,
			Role:                cfg.EmbedRole,
			PolicySchemaVersion: cfg.EmbedPolicySchemaVersion,
		},
	}
}
...
// collection name. Ollama-backed consumers should set cfg.EmbedDimensions from
// resolved policy metadata before assembling the engine.
...
func denseSizeForConfig(cfg Config) int {
	if cfg.EmbedDimensions > 0 {
		return cfg.EmbedDimensions
	}
