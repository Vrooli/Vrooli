...
const (
	// DefaultEmbedModel is a fallback model label for callers that have not
	// resolved Ollama policy metadata yet. Ollama-backed consumers should use
	// resource-ollama policy metadata as the source of truth.
	DefaultEmbedModel = "nomic-embed-text"
	// DefaultEmbedRole is the shared Ollama policy role used for runtime
	// embedding calls.
	DefaultEmbedRole = "embedding.default"
	// DefaultVectorSize is a compatibility fallback for tests and non-Ollama
	// adopters. Ollama-backed collection specs should use resolved policy
