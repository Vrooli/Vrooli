...
const fixtureNomicEmbeddingModel = "fixture-nomic-embed-model:latest"
...
func TestTaskPrefixesFor(t *testing.T) {
	cases := []struct {
		...
		{fixtureNomicEmbeddingModel, "search_query: ", "search_document: "},
		{"FIXTURE-NOMIC-EMBED-MODEL:latest", "search_query: ", "search_document: "},
		{"mxbai-embed-large", "", ""},
		{"", "", ""},
	}
	for _, c := range cases {
...
func TestCLIEmbedderAppliesTaskPrefixes(t *testing.T) {
	var seen []string
	e := NewEmbedderWithRunnerPrefixed(fixtureNomicEmbeddingModel, capturingRunner(&seen))

	te, ok := e.(TaskEmbedder)
	if !ok {
		t.Fatal("prefixed fixture embedder must implement TaskEmbedder")
	}
...
func TestEmbedderWithEmptyPrefixesIsSymmetric(t *testing.T) {
	...
	e := &cliEmbedder{bin: defaultEmbedderBin, model: fixtureNomicEmbeddingModel, role: DefaultEmbedRole, run: capturingRunner(&seen), queryPrefix: "", docPrefix: ""}
	te := TaskEmbedder(e)
