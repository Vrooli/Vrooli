...
type Config struct {
	...
	EmbedRole       string
	EmbedDimensions int
	// EmbedPolicySchemaVersion records the Ollama policy schema version used to
	// resolve EmbedRole/EmbedModel. Empty means the caller did not resolve an
	// Ollama policy and Qdrant cannot validate role-retarget metadata.
	EmbedPolicySchemaVersion string
