...
- **Module:** `github.com/vrooli/ai-go`
- **Import path:** `github.com/vrooli/ai-go/search`
...
```go
cfg, err := aisearch.ResolveEmbeddingConfig(ctx, aisearch.LoadConfig("MYSCENARIO"))
if err != nil { /* degrade */ }

// The collection name appears once. Embedder, vector store, reranker, and
// CollectionSpec are derived from the resolved embedding role/model/dimensions.
engine := aisearch.NewDenseEngine(cfg, "my-collection")
...
- `EMBED_TASK_PREFIX` (default off) — embed queries with `search_query:` and
  passages with `search_document:` for asymmetric embedding roles.
...
it must not re-implement A/B comparison, run storage, or experiment tagging. The
gate should load the provider from `search.json`, resolve its `scoring` block,
and call `GradeSuite`; that shared primitive owns candidate/stale exclusion,
case-level scope, and the acceptance denominator. When you want to *change* a
knob, run `evals sweep` (or register/compare by hand) in search-hub and let it
write the winning `tuning` back into `search.json`. When you want to *guard* a
build, the local recall gate is enough. The tuning loop is: golden suite in
`search.json` → `evals sweep --apply` → the four overfit guards → winning
`tuning` written back into `search.json` (the SSOT).
