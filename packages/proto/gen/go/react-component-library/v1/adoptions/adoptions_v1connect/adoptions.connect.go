// Code generated by protoc-gen-connect-go. DO NOT EDIT.
//
// Source: react-component-library/v1/adoptions/adoptions.proto

package adoptions_v1connect

import (
	connect "connectrpc.com/connect"
	context "context"
	errors "errors"
	adoptions "github.com/vrooli/vrooli/packages/proto/gen/go/react-component-library/v1/adoptions"
	http "net/http"
	strings "strings"
)

// This is a compile-time assertion to ensure that this generated file and the connect package are
// compatible. If you get a compiler error that this constant is not defined, this code was
// generated with a version of connect newer than the one compiled into your binary. You can fix the
// problem by either regenerating this code with an older version of connect or updating the connect
// version compiled into your binary.
const _ = connect.IsAtLeastVersion1_13_0

const (
	// AdoptionsServiceName is the fully-qualified name of the AdoptionsService service.
	AdoptionsServiceName = "vrooli.react_component_library.v1.adoptions.AdoptionsService"
)

// These constants are the fully-qualified names of the RPCs defined in this package. They're
// exposed at runtime as Spec.Procedure and as the final two segments of the HTTP route.
//
// Note that these are different from the fully-qualified method names used by
// google.golang.org/protobuf/reflect/protoreflect. To convert from these constants to
// reflection-formatted method names, remove the leading slash and convert the remaining slash to a
// period.
const (
	// AdoptionsServiceListAdoptionsProcedure is the fully-qualified name of the AdoptionsService's
	// ListAdoptions RPC.
	AdoptionsServiceListAdoptionsProcedure = "/vrooli.react_component_library.v1.adoptions.AdoptionsService/ListAdoptions"
	// AdoptionsServiceCreateAdoptionProcedure is the fully-qualified name of the AdoptionsService's
	// CreateAdoption RPC.
	AdoptionsServiceCreateAdoptionProcedure = "/vrooli.react_component_library.v1.adoptions.AdoptionsService/CreateAdoption"
	// AdoptionsServiceDeleteAdoptionProcedure is the fully-qualified name of the AdoptionsService's
	// DeleteAdoption RPC.
	AdoptionsServiceDeleteAdoptionProcedure = "/vrooli.react_component_library.v1.adoptions.AdoptionsService/DeleteAdoption"
	// AdoptionsServiceRefreshAdoptionsProcedure is the fully-qualified name of the AdoptionsService's
	// RefreshAdoptions RPC.
	AdoptionsServiceRefreshAdoptionsProcedure = "/vrooli.react_component_library.v1.adoptions.AdoptionsService/RefreshAdoptions"
)

// AdoptionsServiceClient is a client for the
// vrooli.react_component_library.v1.adoptions.AdoptionsService service.
type AdoptionsServiceClient interface {
	ListAdoptions(context.Context, *connect.Request[adoptions.ListAdoptionsRequest]) (*connect.Response[adoptions.ListAdoptionsResponse], error)
	CreateAdoption(context.Context, *connect.Request[adoptions.CreateAdoptionRequest]) (*connect.Response[adoptions.CreateAdoptionResponse], error)
	DeleteAdoption(context.Context, *connect.Request[adoptions.DeleteAdoptionRequest]) (*connect.Response[adoptions.DeleteAdoptionResponse], error)
	RefreshAdoptions(context.Context, *connect.Request[adoptions.RefreshAdoptionsRequest]) (*connect.Response[adoptions.RefreshAdoptionsResponse], error)
}

// NewAdoptionsServiceClient constructs a client for the
// vrooli.react_component_library.v1.adoptions.AdoptionsService service. By default, it uses the
// Connect protocol with the binary Protobuf Codec, asks for gzipped responses, and sends
// uncompressed requests. To use the gRPC or gRPC-Web protocols, supply the connect.WithGRPC() or
// connect.WithGRPCWeb() options.
//
// The URL supplied here should be the base URL for the Connect or gRPC server (for example,
// http://api.acme.com or https://acme.com/grpc).
func NewAdoptionsServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) AdoptionsServiceClient {
	baseURL = strings.TrimRight(baseURL, "/")
	adoptionsServiceMethods := adoptions.File_react_component_library_v1_adoptions_adoptions_proto.Services().ByName("AdoptionsService").Methods()
	return &adoptionsServiceClient{
		listAdoptions: connect.NewClient[adoptions.ListAdoptionsRequest, adoptions.ListAdoptionsResponse](
			httpClient,
			baseURL+AdoptionsServiceListAdoptionsProcedure,
			connect.WithSchema(adoptionsServiceMethods.ByName("ListAdoptions")),
			connect.WithClientOptions(opts...),
		),
		createAdoption: connect.NewClient[adoptions.CreateAdoptionRequest, adoptions.CreateAdoptionResponse](
			httpClient,
			baseURL+AdoptionsServiceCreateAdoptionProcedure,
			connect.WithSchema(adoptionsServiceMethods.ByName("CreateAdoption")),
			connect.WithClientOptions(opts...),
		),
		deleteAdoption: connect.NewClient[adoptions.DeleteAdoptionRequest, adoptions.DeleteAdoptionResponse](
			httpClient,
			baseURL+AdoptionsServiceDeleteAdoptionProcedure,
			connect.WithSchema(adoptionsServiceMethods.ByName("DeleteAdoption")),
			connect.WithClientOptions(opts...),
		),
		refreshAdoptions: connect.NewClient[adoptions.RefreshAdoptionsRequest, adoptions.RefreshAdoptionsResponse](
			httpClient,
			baseURL+AdoptionsServiceRefreshAdoptionsProcedure,
			connect.WithSchema(adoptionsServiceMethods.ByName("RefreshAdoptions")),
			connect.WithClientOptions(opts...),
		),
	}
}

// adoptionsServiceClient implements AdoptionsServiceClient.
type adoptionsServiceClient struct {
	listAdoptions    *connect.Client[adoptions.ListAdoptionsRequest, adoptions.ListAdoptionsResponse]
	createAdoption   *connect.Client[adoptions.CreateAdoptionRequest, adoptions.CreateAdoptionResponse]
	deleteAdoption   *connect.Client[adoptions.DeleteAdoptionRequest, adoptions.DeleteAdoptionResponse]
	refreshAdoptions *connect.Client[adoptions.RefreshAdoptionsRequest, adoptions.RefreshAdoptionsResponse]
}

// ListAdoptions calls vrooli.react_component_library.v1.adoptions.AdoptionsService.ListAdoptions.
func (c *adoptionsServiceClient) ListAdoptions(ctx context.Context, req *connect.Request[adoptions.ListAdoptionsRequest]) (*connect.Response[adoptions.ListAdoptionsResponse], error) {
	return c.listAdoptions.CallUnary(ctx, req)
}

// CreateAdoption calls vrooli.react_component_library.v1.adoptions.AdoptionsService.CreateAdoption.
func (c *adoptionsServiceClient) CreateAdoption(ctx context.Context, req *connect.Request[adoptions.CreateAdoptionRequest]) (*connect.Response[adoptions.CreateAdoptionResponse], error) {
	return c.createAdoption.CallUnary(ctx, req)
}

// DeleteAdoption calls vrooli.react_component_library.v1.adoptions.AdoptionsService.DeleteAdoption.
func (c *adoptionsServiceClient) DeleteAdoption(ctx context.Context, req *connect.Request[adoptions.DeleteAdoptionRequest]) (*connect.Response[adoptions.DeleteAdoptionResponse], error) {
	return c.deleteAdoption.CallUnary(ctx, req)
}

// RefreshAdoptions calls
// vrooli.react_component_library.v1.adoptions.AdoptionsService.RefreshAdoptions.
func (c *adoptionsServiceClient) RefreshAdoptions(ctx context.Context, req *connect.Request[adoptions.RefreshAdoptionsRequest]) (*connect.Response[adoptions.RefreshAdoptionsResponse], error) {
	return c.refreshAdoptions.CallUnary(ctx, req)
}

// AdoptionsServiceHandler is an implementation of the
// vrooli.react_component_library.v1.adoptions.AdoptionsService service.
type AdoptionsServiceHandler interface {
	ListAdoptions(context.Context, *connect.Request[adoptions.ListAdoptionsRequest]) (*connect.Response[adoptions.ListAdoptionsResponse], error)
	CreateAdoption(context.Context, *connect.Request[adoptions.CreateAdoptionRequest]) (*connect.Response[adoptions.CreateAdoptionResponse], error)
	DeleteAdoption(context.Context, *connect.Request[adoptions.DeleteAdoptionRequest]) (*connect.Response[adoptions.DeleteAdoptionResponse], error)
	RefreshAdoptions(context.Context, *connect.Request[adoptions.RefreshAdoptionsRequest]) (*connect.Response[adoptions.RefreshAdoptionsResponse], error)
}

// NewAdoptionsServiceHandler builds an HTTP handler from the service implementation. It returns the
// path on which to mount the handler and the handler itself.
//
// By default, handlers support the Connect, gRPC, and gRPC-Web protocols with the binary Protobuf
// and JSON codecs. They also support gzip compression.
func NewAdoptionsServiceHandler(svc AdoptionsServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	adoptionsServiceMethods := adoptions.File_react_component_library_v1_adoptions_adoptions_proto.Services().ByName("AdoptionsService").Methods()
	adoptionsServiceListAdoptionsHandler := connect.NewUnaryHandler(
		AdoptionsServiceListAdoptionsProcedure,
		svc.ListAdoptions,
		connect.WithSchema(adoptionsServiceMethods.ByName("ListAdoptions")),
		connect.WithHandlerOptions(opts...),
	)
	adoptionsServiceCreateAdoptionHandler := connect.NewUnaryHandler(
		AdoptionsServiceCreateAdoptionProcedure,
		svc.CreateAdoption,
		connect.WithSchema(adoptionsServiceMethods.ByName("CreateAdoption")),
		connect.WithHandlerOptions(opts...),
	)
	adoptionsServiceDeleteAdoptionHandler := connect.NewUnaryHandler(
		AdoptionsServiceDeleteAdoptionProcedure,
		svc.DeleteAdoption,
		connect.WithSchema(adoptionsServiceMethods.ByName("DeleteAdoption")),
		connect.WithHandlerOptions(opts...),
	)
	adoptionsServiceRefreshAdoptionsHandler := connect.NewUnaryHandler(
		AdoptionsServiceRefreshAdoptionsProcedure,
		svc.RefreshAdoptions,
		connect.WithSchema(adoptionsServiceMethods.ByName("RefreshAdoptions")),
		connect.WithHandlerOptions(opts...),
	)
	return "/vrooli.react_component_library.v1.adoptions.AdoptionsService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case AdoptionsServiceListAdoptionsProcedure:
			adoptionsServiceListAdoptionsHandler.ServeHTTP(w, r)
		case AdoptionsServiceCreateAdoptionProcedure:
			adoptionsServiceCreateAdoptionHandler.ServeHTTP(w, r)
		case AdoptionsServiceDeleteAdoptionProcedure:
			adoptionsServiceDeleteAdoptionHandler.ServeHTTP(w, r)
		case AdoptionsServiceRefreshAdoptionsProcedure:
			adoptionsServiceRefreshAdoptionsHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
}

// UnimplementedAdoptionsServiceHandler returns CodeUnimplemented from all methods.
type UnimplementedAdoptionsServiceHandler struct{}

func (UnimplementedAdoptionsServiceHandler) ListAdoptions(context.Context, *connect.Request[adoptions.ListAdoptionsRequest]) (*connect.Response[adoptions.ListAdoptionsResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("vrooli.react_component_library.v1.adoptions.AdoptionsService.ListAdoptions is not implemented"))
}

func (UnimplementedAdoptionsServiceHandler) CreateAdoption(context.Context, *connect.Request[adoptions.CreateAdoptionRequest]) (*connect.Response[adoptions.CreateAdoptionResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("vrooli.react_component_library.v1.adoptions.AdoptionsService.CreateAdoption is not implemented"))
}

func (UnimplementedAdoptionsServiceHandler) DeleteAdoption(context.Context, *connect.Request[adoptions.DeleteAdoptionRequest]) (*connect.Response[adoptions.DeleteAdoptionResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("vrooli.react_component_library.v1.adoptions.AdoptionsService.DeleteAdoption is not implemented"))
}

func (UnimplementedAdoptionsServiceHandler) RefreshAdoptions(context.Context, *connect.Request[adoptions.RefreshAdoptionsRequest]) (*connect.Response[adoptions.RefreshAdoptionsResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("vrooli.react_component_library.v1.adoptions.AdoptionsService.RefreshAdoptions is not implemented"))
}
