...
`persona` and `persona` in both path components
...
The scenario-owned domain proto files are real product contracts. The shared
`health` and `errors` contracts remain conventional infrastructure contracts.
