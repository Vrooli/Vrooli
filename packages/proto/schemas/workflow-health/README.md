...
`workflow-health` and `workflow_health` in both path components
...
The template's example-domain proto files have been removed for this scenario.
The shared `health` and `errors` contracts are conventional infrastructure
contracts, not example-domain scaffold, so they do not carry `@template`.
