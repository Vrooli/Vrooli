...
`music-tools` and `music_tools` in both path components
...
The scenario-owned domains are composition, jobs, models, styles, and capacity.
The shared `health` and `errors` contracts are conventional infrastructure
contracts. The former generated `notes` example domain has been removed.
