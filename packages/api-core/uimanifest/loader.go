...
// Adoption, automation, and UI validation share this file-location contract.
package uimanifest
...
func (l *FSLoader) Load(scenario string) (Manifest, error) {
	...
	if _, err := os.Stat(filepath.Join(l.RepoRoot, "scenarios", scenario, "ui", "manifest.json")); err == nil {
		return LoadAt(filepath.Join(l.RepoRoot, "scenarios", scenario))
	}
	if templateID == "" {
		if _, err := os.Stat(filepath.Join(l.RepoRoot, "scenarios", scenario, ".vrooli", "ui-manifest.json")); err == nil {
			return LoadAt(filepath.Join(l.RepoRoot, "scenarios", scenario))
		}
	...
	return loadOverlay(mf, filepath.Join(l.RepoRoot, "scenarios", scenario, ".vrooli", "ui-manifest.json"))
}

func loadOverlay(mf Manifest, overlayPath string) (Manifest, error) {
	...
	if err := validateFilePaths(mf); err != nil {
		return Manifest{}, err
	}
	return mf, nil
}
