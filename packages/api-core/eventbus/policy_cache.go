...
func (c *Cache) Replace(next PolicySnapshot, now time.Time) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	if next.Version == "" || next.Version == c.snapshot.Version {
		return false
	}
	c.snapshot = next
...
func (c *Cache) ProjectReceipt(_ string, target, operation string, candidate map[string]any) (map[string]any, string, bool) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	for _, p := range c.snapshot.ReceiptCapturePolicies {
		if !p.Enabled || p.Selector.TargetScenario != target || p.Selector.Operation != operation || p.Selector.Protocol != "connect" || p.Selector.EventType != ReceiptEventType {
			continue
		}
