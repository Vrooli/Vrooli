...
func (r *SliceRepo[T]) Get(ctx context.Context, id string) (T, error) {
	...
	if r.NotFound == nil {
		panic("databasetest.SliceRepo.Get: NotFound is required")
	}
...
func (r *SliceRepo[T]) requireExtractors(op string) {
	if r.GetID == nil {
		panic(fmt.Sprintf("databasetest.SliceRepo.%s: GetID is required", op))
	}
	if op == "Create" && r.SetID == nil {
		panic(fmt.Sprintf("databasetest.SliceRepo.%s: SetID is required", op))
	}
}
