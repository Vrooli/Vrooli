...
describe("MessageConfig", () => {
    ...
    describe("constructor", () => {
        it("should create a MessageConfig with all properties", () => {
                ...
                runs: [
                    {
                        ...
                        resourceVersionName: "Test Routine v1",
                        taskId: "task1",
                        runStatus: "completed",
            ...
            const messageConfig = new MessageConfig({ config });

            expect(messageConfig.contextHints).toEqual(["hint1", "hint2"]);
            expect(messageConfig.eventTopic).toBe("test-event");
            expect(messageConfig.respondingBots).toEqual(["bot1", "@all"]);
            expect(messageConfig.role).toBe("assistant");
            expect(messageConfig.turnId).toBe(5);
            expect(messageConfig.toolCalls).toHaveLength(1);
            expect(messageConfig.toolCalls?.[0]?.id).toBe("call1");
            expect(messageConfig.runs).toHaveLength(1);
            expect(messageConfig.runs?.[0]?.runId).toBe("run1");
        });

        it("should create a MessageConfig with minimal properties", () => {
            const config: MessageConfigObject = {
                __version: "1.0",
                resources: [],
            };
    ...
    describe("default", () => {
        it("should create a default MessageConfig", () => {
            const messageConfig = MessageConfig.default();

            expect(messageConfig.__version).toBe("1.0");
            expect(messageConfig.resources).toEqual([]);
            expect(messageConfig.contextHints).toEqual([]);
            expect(messageConfig.respondingBots).toEqual([]);
            expect(messageConfig.role).toBe("user");
            expect(messageConfig.turnId).toBeNull();
    ...
    describe("export", () => {
        it("should export all properties", () => {
            ...
            const messageConfig = new MessageConfig({ config });
            const exported = messageConfig.export();

            expect(exported).toEqual(config);
        });

        it("should export minimal properties", () => {
            const config: MessageConfigObject = {
                __version: "1.0",
                resources: [],
            };
            ...
            expect(exported.contextHints).toBeUndefined();
            expect(exported.eventTopic).toBeUndefined();
            expect(exported.respondingBots).toBeUndefined();
            expect(exported.role).toBeUndefined();
            expect(exported.turnId).toBeUndefined();
            expect(exported.toolCalls).toBeUndefined();
