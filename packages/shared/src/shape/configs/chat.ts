...
/**
 * Input for the respondToToolApproval endpoint.
 */
export interface RespondToToolApprovalInput {
    /** ID of the chat this pending tool call pertains to. */
    conversationId: string;
    /** Unique ID for this pending request, generated by Vrooli server-side. */
    pendingId: string;
...
export class ChatConfig {
    ...
    eventSubscriptions?: ChatConfigObject["eventSubscriptions"];
    ...
    static default(): ChatConfig {
        const config: ChatConfigObject = {
            ...
            eventSubscriptions: {
                // Leader handles all business events by default
                ...
                // Role-based routing (wildcard example for Writer bots)
                "swarm/Writer/events": ["bot_writer_1", "bot_writer_2"],

                // Private channels are implicitly subscribed by the owning bot
                // Control-plane topics intentionally left UNMAPPED
            },
            resources: [],
