...
describe("orDefault", () => {
    it("should return the default array if the existing array is empty", () => {
        ...
        expect(orDefault<any>([], defaultArray)).to.deep.equal(defaultArray);
    });

    it("should return the existing array if it is non-empty", () => {
        const existingArray = [4, 5, 6];
        expect(orDefault<any>(existingArray, [1, 2, 3])).to.deep.equal(existingArray);
    });

    it("should return the default object if the existing object is empty", () => {
        const defaultObj = { a: 1, b: 2 };
        expect(orDefault<any>({}, defaultObj)).to.deep.equal(defaultObj);
    });
    ...
    it("should fill in missing nested properties from the default object", () => {
        ...
        expect(orDefault<any>(existingObj, defaultObj)).to.deep.equal(expectedObj);
    });
    ...
    it("should handle objects within arrays", () => {
        ...
        expect(orDefault<any>(existingArray, defaultArray)).to.deep.equal(expectedArray);
    });
    ...
    it("should return the existing array of primitives if non-empty", () => {
        ...
        expect(orDefault<any>(existingArray, defaultArray)).to.deep.equal(existingArray);
    });
    ...
    it("should return the default array of objects if the existing array is empty", () => {
        ...
        expect(orDefault<any>(existingArray, defaultArray)).to.deep.equal(defaultArray);
    });
});
