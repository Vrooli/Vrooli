 ...
 * 
 * IMPORTANT: This function must generate URLs that match the expectations of the og-worker,
 * which cannot import from other packages. The og-worker is the single source of truth for URL structure.
 * 
 * URL Structure:
 * - Handles are prefixed with @ (e.g., /u/@username, /team/@teamname)
 * - PublicIds and IDs are used without prefix (e.g., /u/pub123, /u/123)
 * - The server's readOneHelper checks if the ID starts with @ to determine if it's a handle
 * 
...
export function getObjectSlug(object: NavigableObject | null | undefined, prefersId = false): string {
    ...
    // Otherwise, use object's handle (with @ prefix) or id
    const id = object.publicId ?? object.id ?? "";
    const handle = object.handle;
    return `/${prefersId ? id : (handle ? `@${handle}` : id)}`;
}
 ...
 * 
 * IMPORTANT: URLs generated by this function must match the og-worker expectations:
 * - User/Team handles: /u/@username, /team/@teamname  
 * - User/Team publicIds: /u/pub123, /team/pub456
 * - Other objects: /{type}/{publicId}
 * 
 * The og-worker routes these URLs to the API, which uses the @ prefix to distinguish
 * handles from publicIds in the readOneHelper function.
 * 
 * @param object Object being navigated to
 * @returns Full URL for viewing the object
 */
