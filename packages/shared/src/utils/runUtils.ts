...
export class RunIOManager {
    ...
    public runInputsUpdate(params: Omit<RunIOUpdateParams<"input">, "ioType">): RunIOUpdateResult<"input"> {
        return this.runIOUpdate({ ...params, ioType: "input" });
    }

    public runOutputsUpdate(params: Omit<RunIOUpdateParams<"output">, "ioType">): RunIOUpdateResult<"output"> {
...
export type MessageWithStatus = { message: string, status: Status };
export type GraphMessageWithStatus = MessageWithStatus & { nodeId?: string, linkId?: string };
type RoutineVersionStatusMultiStep = {
    messages: GraphMessageWithStatus[];
    nodesOffGraph: { id: string }[];
    nodesOnGraph: { id: string }[];
}

export function routineVersionStatusMultiStep(
    routineVersion: {
        nodes: DefinedArrayElement<RoutineVersionShape["nodes"]>[] | null | undefined,
        nodeLinks: DefinedArrayElement<RoutineVersionShape["nodeLinks"]>[] | null | undefined
    },
    rootStep: RootStep | null,
): RoutineVersionStatusMultiStep {
    const messages: GraphMessageWithStatus[] = [];
    const nodesOffGraph: DefinedArrayElement<RoutineVersionShape["nodes"]>[] = [];
    const nodesOnGraph: DefinedArrayElement<RoutineVersionShape["nodes"]>[] = [];
    ...
    for (const node of nodesWithoutIncomingEdges) {
        const name = getTranslation(node, null, true).name;
    ...
    for (const node of nodesWithoutOutgoingEdges) {
        const name = getTranslation(node, null, true).name;
...
/**
 * Converts runnable objects into Step objects, which 
 * can be used to display the run graph or execute the run.
 */
export class RunStepBuilder {
    private logger: PassableLogger;
     ...
     * @param logger Logger to log errors
     */
    constructor(languages: string[], logger: PassableLogger) {
    ...
    public runnableObjectToStep(
        ...
        if (isOfType(runnableObject, "RoutineVersion")) {
            if (runnableObject.routineType === RoutineType.MultiStep) {
                ...
                return this.singleRoutineToStep(runnableObject, [...location]);
            }
        } else if (isOfType(runnableObject, "ProjectVersion")) {
            return this.projectToStep(runnableObject, [...location]);
        }
        this.logger.error(`Invalid runnable object type in runnableObjectToStep: ${runnableObject !== null && typeof runnableObject === "object" ? (runnableObject as { __type: string }).__type : "runnableObject is not defined"}`);
        return null;
    }
     ...
     * @returns RootStep for the given object, or null if invalid
     */
    public multiRoutineToStep(
        routineVersion: MinimalRunnableRoutineVersion,
        location: number[],
    ): MultiRoutineStep | null {
        // Convert existing nodes into steps
        const unsorted = (routineVersion.nodes || []).map(node => {
            const description = getTranslation(node, this.languages, true).description || null;
            const name = getTranslation(node, this.languages, true).name || "Untitled";
                ...
                case NodeType.RoutineList: {
                    ...
                    return {
                        ...
                        steps: sortedRoutineListSteps.map(item => {
                            return {
                                __type: RunStepType.SingleRoutine,
                                description: getTranslation(item.routineVersion, this.languages, true).description || null,
                                location: [...location, 1, 1], // Sort function will correct this
                                name: getTranslation(item.routineVersion, this.languages, true).name || "Untitled",
    ...
    public addSubroutinesToStep(
        ...
        for (const routineVersion of subroutines) {
            ...
            const location = new RunStepNavigator(this.logger).findStep(
                rootStep,
            ...
            if (!location) {
                this.logger.error("Could not find location to insert routine", routineVersion);
                continue;
            }
            const subroutineStep = this.multiRoutineToStep(routineVersion, [...location]);
            if (!subroutineStep) {
                this.logger.error("Could not convert routine to step", routineVersion);
                continue;
            }
            updatedRootStep = this.insertStep(subroutineStep as MultiRoutineStep, updatedRootStep);
        }
    ...
    public projectToStep(
        ...
        for (let i = 0; i < projectVersion.directories.length; i++) {
            ...
            if (!directory) {
                this.logger.error("Could not find directory in project", projectVersion);
                continue;
            }
            const directoryStep = this.directoryToStep(directory, [...location, i + 1], projectVersion.id);
            if (directoryStep) {
        ...
        return {
            __type: RunStepType.Directory,
            description: getTranslation(projectVersion, this.languages, true).description || null,
            directoryId: null,
            hasBeenQueried: true,
            isOrdered: false,
            isRoot: true,
            location: [...location],
            name: getTranslation(projectVersion, this.languages, true).name || "Untitled",
    ...
    public directoryToStep(
        ...
        return {
            __type: RunStepType.Directory,
            description: getTranslation(projectVersionDirectory, this.languages, true).description || null,
            directoryId: projectVersionDirectory.id,
            hasBeenQueried: true,
            isOrdered: false,
            isRoot: false,
            location: [...location],
            name: getTranslation(projectVersionDirectory, this.languages, true).name || "Untitled",
     ...
     */
    public addSubdirectoriesToStep(
        ...
        for (const directory of subdirectories) {
            ...
            if (!location) {
                this.logger.error("Could not find location to insert directory", directory);
                continue;
            }
            const projectStep = this.directoryToStep(directory, [...location]);
            if (!projectStep) {
                this.logger.error("Could not convert directory to step", directory);
                continue;
            }
            updatedRootStep = this.insertStep(projectStep, updatedRootStep);
        }
        return updatedRootStep;
    }
     ...
     */
    public parseChildOrder(childOrder: string): string[] {
...
/**
 * Navigates a step structure using a location array
 */
export class RunStepNavigator {
    private logger: PassableLogger;
        ...
        for (let i = 1; i < location.length; i++) {
            ...
            if (currentStep.__type === RunStepType.Directory || currentStep.__type === RunStepType.RoutineList) {
                if (index > 0 && index <= currentStep.steps.length) {
                    currentStep = currentStep.steps[index - 1] as RunStep;
                } else {
    ...
    public getPreviousLocation(
        ...
        const currentStep = this.stepFromLocation(currentLocation, rootStep);
        ...
        const parentStep = this.stepFromLocation(parentLocation, rootStep);
        ...
        if (parentStep.__type === RunStepType.MultiRoutine) {
            const matchingNextLocations = parentStep.nodes
                .filter(node => Array.isArray((node as NodeStep).nextLocation) && this.locationArraysMatch((node as NodeStep).nextLocation as number[], currentLocation)) as NodeStep[];
            const matchingDecisions = parentStep.nodes
                .filter(node => node.__type === RunStepType.Decision)
                .filter(node => (node as DecisionStep).options.some(option => this.locationArraysMatch(option.step.location, currentLocation))) as DecisionStep[];
        ...
        const prevSiblingStep = this.stepFromLocation(prevSiblingLocation, rootStep);
        if (prevSiblingStep) {
    ...
    public getNextLocation(
        ...
        while (currentLocation.length > 0) {
            ...
            const parentStep = this.stepFromLocation(currentLocation, rootStep);
            ...
            const parentSiblingStep = this.stepFromLocation(parentSiblingLocation, rootStep);
    ...
    public siblingsAtLocation(
        ...
        const parent = this.stepFromLocation(parentLocation, rootStep);
        if (!parent) {
            this.logger.error(`Could not find parent to count siblings. Location: ${location}`);
            return 0;
        }
     ...
     */
    public findStep(step: RunStep, predicate: (step: RunStep) => boolean): RunStep | null {
        ...
        switch (step.__type) {
            ...
            case RunStepType.RoutineList:
                for (const childStep of step.steps) {
                    const result = this.findStep(childStep, predicate);
                    if (result) return result;
                }
                break;
            case RunStepType.MultiRoutine:
                for (const node of step.nodes) {
                    const result = this.findStep(node, predicate);
                    if (result) return result;
                }
                break;
            case RunStepType.Decision:
                for (const option of step.options) {
                    const result = this.findStep(option.step, predicate);
                    if (result) return result;
                }
                break;
    ...
    public detectSubstepLoad(
        ...
        while (currentLocation.length > 0 && !currentStep) {
            currentStep = this.stepFromLocation(currentLocation, rootStep);
            if (!currentStep) {
                currentLocation.pop();
            }
        }
        ...
        const stepNeedsQuerying = this.stepNeedsQuerying;
     ...
     */
    public locationArraysMatch(locationA: number[], locationB: number[]): boolean {
...
export async function saveRunProgress({
    ...
    if (run.__typename === "RunRoutine") {
        const { inputsCreate, inputsDelete, inputsUpdate } = new RunIOManager(logger).runInputsUpdate({
            existingIO: (run as RunRoutine).inputs,
            formData,
            routineIO: (runnableObject as RunnableRoutineVersion).inputs,
            runRoutineId: runData.id,
        });
        const { outputsCreate, outputsDelete, outputsUpdate } = new RunIOManager(logger).runOutputsUpdate({
