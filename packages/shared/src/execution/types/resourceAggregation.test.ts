...
import {
    ResourceAggregator,
    ResourceTracker,
    type DetailedResourceUsage,
    type HierarchicalResourceAllocation,
...
describe("ResourceAggregator", () => {
    const createUsage = (overrides: Partial<ExecutionResourceUsage> = {}): ExecutionResourceUsage => ({
        creditsUsed: "100",
        durationMs: 1000,
        memoryUsedMB: 256,
        stepsExecuted: 5,
    ...
    describe("aggregateUsage", () => {
        it("should aggregate parallel execution correctly", () => {
            ...
            expect(result.tokens).toBe(150); // Sum of all tokens
            expect(result.apiCalls).toBe(6); // Sum of all API calls
        });
