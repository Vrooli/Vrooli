// Package rlimitexec implements the `rlimit-exec` self-exec shim a host
// binary prepends ahead of a target command on macOS, where there is no
// cgroup and no prlimit: workspace-sandbox's Seatbelt backend and
// platform-go's session containment both use it. The shim applies setrlimit
// for the requested limits and then execs the target command, replacing its
// own process image so the limits carry into the workload.
//
...
// MaybeRun routes a host binary into the shim when its argv[1] is the
// Subcommand: call it first in main. It returns false when argv is not a
// shim invocation; on a shim invocation it never returns (exec replaces the
// image, or the error is printed and the process exits 1).
func MaybeRun(args []string) bool {
	if len(args) <= 1 || args[1] != Subcommand {
		return false
	}
	if err := Run(args[2:]); err != nil {
		fmt.Fprintf(os.Stderr, "%s %s: %v\n", filepath.Base(args[0]), Subcommand, err)
		os.Exit(1)
	}
	return true
}

// Apply sets the limits on the calling process without exec; they are
// inherited by every child and survive an exec the caller performs itself.
func Apply(spec Spec) error { return apply(spec) }
