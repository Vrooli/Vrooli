...
func TestLowest_R2WhenProtoHealthError(t *testing.T) {
	sig := allClean()
	sig.ErrorPlusByDimension[dimd("proto-health")] = 1
	r, ok := Lowest(sig, DefaultThresholds(), "")
	if !ok || r.ID != RungR2 {
		t.Fatalf("proto-health error must select R2, got %v ok=%v", r.ID, ok)
	}
	if r.HardGate {
