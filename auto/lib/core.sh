...
# Config (env-overridable) - use constants from constants.sh
INTERVAL_SECONDS=${INTERVAL_SECONDS:-$DEFAULT_INTERVAL_SECONDS}
...
LOOP_TCP_FILTER="${LOOP_TCP_FILTER:-$DEFAULT_LOOP_TCP_FILTER}"  # Narrow default filter; exclude generic 443 to avoid false positives
OLLAMA_SUMMARY_MODEL="${OLLAMA_SUMMARY_MODEL:-$DEFAULT_OLLAMA_SUMMARY_MODEL}"
ULTRA_THINK_PREFIX="${ULTRA_THINK_PREFIX:-$DEFAULT_ULTRA_THINK_PREFIX}"
ALLOWED_TOOLS="${ALLOWED_TOOLS:-$DEFAULT_ALLOWED_TOOLS}"
SKIP_PERMISSIONS="${SKIP_PERMISSIONS:-$DEFAULT_SKIP_PERMISSIONS}"
