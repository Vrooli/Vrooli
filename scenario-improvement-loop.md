...
- Most resoures are not working right now, and you are FORBIDDEN from trying to fix them or adding new ones, except for any specific exceptions I mention in this prompt. If you would like to run a workflow, you may use the browserless workaround like this (example): `N8N_EMAIL="matthalloran8@gmail.com" N8N_PASSWORD="UQv@9r%%UGMmEJZC" vrooli resource browserless execute-workflow <workflow_id> http://localhost:5678 3000 '{"text":"Hello world", "model":"nomic-embed-text"}'`. However, at the time of starting this task (iteration #0), that command does not yet work. Despite this, it is the best attempt we have at doing any sort of n8n validation. If you feel the need to, you may investigate scripts/resources/agents/browserless/ and attempt ONE fix. If that fix does not work, put it back and note this attempt in your results logs. NOTE: Shared workflows must be defined in the project-level .vrooli/service.json file and `vrooli setup` called, if you want the workflow to be upserted AND set to active. For scenario-level workflows, I think you can run the *generated app*'s `./scripts/manage.sh setup` or call the injection engine directly. Ideally every scenario's workflows would also be injected right after the main ones get injected in the `vrooli setup` process, but that hasn't been implemented yet. So stick with the workaround.
...
- competitor-change-monitor to track the websites/githubs/etc. of competitors, and alert on any relevant changes
- picker-whell for fun random selection
...
- data-generator for building or scraping data required for some of these scenarios
- fall-foliage-explorer for visualizing (with accurate data) where fall foliage peaks are forecasted, with a time slider
- mind-maps for building (both manually and with AI) mind maps that can be semantically searched. Should be thought of as an important scenario to be leveraged by others, any time we need data related to a topic/campaign/etc. to be organized and searchable by AI or humans.
- password-manager as a general password manager
- word-games (wordle, connections, 2048, etc.) just for leisure
