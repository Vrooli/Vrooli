...
**Purpose**: SaaS Landing Page Template is the runtime payload generated by Template Manager. It ships the **public landing page + admin portal** with A/B testing, analytics, Stripe payments, and subscription verification APIs. Template Manager copies this template into a new scenario; all runtime behavior lives in the generated scenario, not in the factory.
...
#### Admin Portal Core
- [ ] OT-P0-005 | Agent integration trigger | Generated landing exposes a template-safe customization trigger endpoint for operator or agent use.
- [ ] OT-P0-006 | Structured agent input | Agent receives structured brief (text + assets + goals) and writes changes via template-defined APIs/files only (no factory-side mutation)
...
- **browser-automation-studio**: Screenshot generation for template previews
...
- CXL — "Landing Page Optimization" (search the CXL blog)
- Nielsen Norman Group — "Landing Pages" articles (nngroup.com)
...
- CXL — "Bayesian vs Frequentist A/B Testing" (search the CXL blog)
