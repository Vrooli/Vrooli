...
// LandingHeaderConfig captures per-variant presentation controls for the public
// landing header. It is stored as JSONB on the variant row and normalized on
// every write so the frontend always receives stable defaults and link ids.
...
func defaultLandingHeaderConfig(variantName string) LandingHeaderConfig {
	return LandingHeaderConfig{
		...
		Nav: HeaderNavConfig{Links: []HeaderNavLink{}},
		Ctas: HeaderCTAGroup{
			Primary:   HeaderCTAConfig{Mode: "inherit_hero", Variant: "solid"},
			Secondary: HeaderCTAConfig{Mode: "downloads", Variant: "ghost"},
		},
		Behavior: HeaderBehaviorConfig{Sticky: true, HideOnScroll: false},
	}
}
...
func normalizeLandingHeaderConfig(input *LandingHeaderConfig, variantName string) LandingHeaderConfig {
	...
	cfg := LandingHeaderConfig{
		...
		Nav:      HeaderNavConfig{Links: make([]HeaderNavLink, len(input.Nav.Links))},
		Ctas:     HeaderCTAGroup{Primary: def.Ctas.Primary, Secondary: def.Ctas.Secondary},
...
func normalizeHeaderNavLink(link HeaderNavLink, index int) HeaderNavLink {
	normalized := HeaderNavLink{
		...
		VisibleOn:   HeaderVisibilityConfig{Desktop: true, Mobile: true},
	}
