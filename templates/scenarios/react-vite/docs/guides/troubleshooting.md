# Troubleshooting — {{SCENARIO_DISPLAY_NAME}}
...
```bash
vrooli scenario status {{SCENARIO_ID}}
make stop
vrooli scenario start {{SCENARIO_ID}} --clean-stale
```
...
{{SCENARIO_ID}} status --auto-start

# Or override the API base for this invocation
{{SCENARIO_ID}} status --api-base "http://localhost:$(vrooli scenario port {{SCENARIO_ID}} API_PORT)/api/v1"
```
...
The generated package paths follow `packages/proto/gen/go/{{SCENARIO_ID}}/v1/<domain>/`.
