...
> feature. Copy the structure for your first real domain, then remove
> the example with [`docs/internal/REPLACING-NOTES.md`](docs/internal/REPLACING-NOTES.md).

## What You Get
- **Three thin surfaces**: Go API (`api/`), Go CLI (`cli/`), and
  React/Vite UI (`ui/`) coordinated through generated proto contracts.
- **Lifecycle metadata**: `.vrooli/service.json`, endpoint metadata,
  testing config, health checks, Makefile entrypoints, and CLI install
  wiring.
- **Domain-first API shape**: domain code owns its service,
  repository, schema, mocks, tests, handler module, and endpoint
  metadata. Shared API wiring stays small.
- **Proto-first contracts**: template proto sources live under
  `proto/v1/` and are relocated to
  `packages/proto/schemas/<your-scenario>/` during generation. Add
  new wire shapes there, regenerate, and consume generated Go/TS types.
- **SQLite by default**: local, pure-Go storage with per-domain schemas.
  Add external resources in `.vrooli/service.json` only when a scenario
  actually needs them.
- **UI guardrails**: i18n catalogs, generated typed string keys,
  locale parity tests, accessibility checks, test utilities, and API
  base resolution for embedded/proxied contexts.
- **CLI guardrails**: `cli-core` global flags/config/staleness,
  domain subcommands, declarative argument schemas, generated Connect
  clients, and report-shaped human output.
- **Canonical docs**: start with [`docs/QUICKSTART.md`](docs/QUICKSTART.md),
  then read [`docs/concepts/ARCHITECTURE.md`](docs/concepts/ARCHITECTURE.md),
  [`docs/internal/TESTING.md`](docs/internal/TESTING.md), and
  [`docs/internal/SEAMS.md`](docs/internal/SEAMS.md) when adding code.
...
- Use generated Connect clients for proto-typed operations. Use
  `cliapp.UploadFile` only for documented multipart REST exceptions.
...
- Use `cliapp.RenderProtoList` / `RenderProtoMutation` when a command
  returns one proto response. Those helpers render human reports by
  default and emit the proto JSON shape under `--json`.
- If a command aggregates multiple sources into a report, use
