...
- `internal/` owns resource-specific Go logic that cannot be expressed through the manifest or shared control-plane packages.

Do not turn `cli/main.go` into the primary implementation surface. If the resource needs specialized install/runtime/status/health/env logic, grow the matching package under `internal/` first.
...
- Prefer shared control-plane behavior first; use `internal/install`, `internal/runtime`, `internal/status`, `internal/health`, and `internal/env` only for real specialization.
