...
- `internal/` is the default home for resource-specific Go logic when the manifest and shared control plane are not enough.
...
- `internal/runtime`: runtime config/materialization logic
- `internal/status`: richer status interpretation when generic lifecycle status is insufficient
- `internal/health`: resource-specific probe helpers
- `internal/env`: environment export and derived configuration helpers
...
5. Keep `cli/main.go` focused on bootstrap and delegation; put resource-specific logic in `internal/...`.
