...
- `internal/` is the default home for compose-specific Go logic when the manifest and shared control plane are not enough.
...
- `internal/topology`: service dependency and readiness semantics
- `internal/runtime`: runtime shaping helpers
- `internal/health`: resource-specific readiness helpers
...
3. Keep `cli/main.go` focused on bootstrap and delegation; put compose-specific logic in `internal/...`.
