...
- `cli/internal/` owns compose-specific Go logic that cannot be expressed through the manifest, compose graph, or shared control-plane packages.

Do not turn `cli/main.go` into the primary implementation surface. If the resource needs specialized graph handling, dependency reasoning, readiness logic, or environment shaping, grow the matching package under `cli/internal/` first.
...
- Prefer shared control-plane behavior first; use `cli/internal/compose`, `cli/internal/topology`, `cli/internal/runtime`, `cli/internal/health`, and `cli/internal/env` only for real specialization.
