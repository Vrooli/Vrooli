...
- `internal/` is optional and intentionally small; it is the default home for resource-specific Go validation or environment helpers when docs and manifest data are not enough.
...
- `internal/env`: environment/export helpers when required
...
3. Keep `cli/main.go` focused on bootstrap and delegation; keep any real validation logic under `internal/...`.
