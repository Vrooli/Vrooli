...
- `internal/` owns desktop-app-specific Go logic that cannot be expressed through the manifest or shared control-plane packages.

Do not turn `cli/main.go` into the primary implementation surface. If the resource needs specialized detection, install translation, platform gating, or health verification, grow the matching package under `internal/` first.
...
- Prefer shared control-plane behavior first; use `internal/discovery`, `internal/install`, `internal/platform`, and `internal/health` only for real specialization.
