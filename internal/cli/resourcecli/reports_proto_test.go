...
// TestWriteTemplateListJSONContract pins `resource template list --json`.
func TestWriteTemplateListJSONContract(t *testing.T) {
	items := []resources.ResourceTemplateInfo{
		{
			Name: "docker-service",
			Path: "/t/docker-service",
			Manifest: resources.ResourceTemplateManifest{
				DisplayName: "Docker Service",
				Driver:      "docker-service",
				RequiredVars: map[string]resources.ResourceTemplateVar{
					"PORT": {Flag: "port", Description: "listen port"},
				},
			},
		},
	}
	var buf bytes.Buffer
	if err := WriteTemplateList(&buf, items); err != nil {
		t.Fatalf("WriteTemplateList: %v", err)
	}
	got := decodeJSON(t, buf.Bytes())
	if got["success"] != true {
		t.Errorf("success: %v", got["success"])
	}
	tpls := got["templates"].([]any)
	tpl := tpls[0].(map[string]any)
	man := tpl["manifest"].(map[string]any)
	if man["display_name"] != "Docker Service" {
		t.Errorf("manifest.display_name snake_case: %v", man["display_name"])
	}
	rv := man["required_vars"].(map[string]any)
	port := rv["PORT"].(map[string]any)
	if port["flag"] != "port" {
		t.Errorf("required_vars[PORT].flag: %v", port)
	}
}

// TestWriteTemplateGenerateReportJSONContract pins `resource template generate --json`.
func TestWriteTemplateGenerateReportJSONContract(t *testing.T) {
	report := resources.ResourceTemplateGenerateReport{
		Template:    resources.ResourceTemplateSummary{Name: "docker-service", DisplayName: "Docker"},
		Destination: "/out",
		Values:      map[string]string{"PORT": "6379"},
		Files:       []string{"a", "b"},
		DryRun:      true,
	}
	var buf bytes.Buffer
	if err := WriteTemplateGenerateReport(&buf, report); err != nil {
		t.Fatalf("WriteTemplateGenerateReport: %v", err)
	}
	got := decodeJSON(t, buf.Bytes())
	rep := got["report"].(map[string]any)
	if rep["dry_run"] != true {
		t.Errorf("dry_run snake_case: %v", rep["dry_run"])
	}
	if rep["destination"] != "/out" {
		t.Errorf("destination: %v", rep["destination"])
	}
	vals := rep["values"].(map[string]any)
	if vals["PORT"] != "6379" {
		t.Errorf("values[PORT]: %v", vals["PORT"])
	}
}
