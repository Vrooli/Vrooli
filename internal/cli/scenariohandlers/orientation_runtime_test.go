...
import (
	...
	. "github.com/vrooli/vrooli/internal/cli/scenariocli" //nolint:revive // tests target scenariohandler glue over scenariocli contracts.
)

func TestEvaluateOrientationCheckPrimitives(t *testing.T) {
	root := t.TempDir()
	mustWrite(t, filepath.Join(root, "docs", "START-HERE.md"), "ready\n")
	mustWrite(t, filepath.Join(root, "requirements", "index.json"), `{"imports":["real/module.json"]}`)
