...
func RenderStatus(w io.Writer, format cliout.Format, report authapp.Report) error {
	return cliout.RenderJSONOr(w, format,
		func(w io.Writer) error { return cliout.WriteProtoJSON(w, authStatusResponse(report)) },
		func(w io.Writer) error {
			rows := make([][]string, 0, len(report.Statuses))
			for _, s := range report.Statuses {
				signIn := strings.Join(s.Result.SignInCommand, " ")
				rows = append(rows, []string{s.Name, string(s.Result.State), s.Result.Detail, signIn})
			}
