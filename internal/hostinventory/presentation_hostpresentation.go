// Package hostinventory classifies the presentation capability of the
...
func detectLinuxWithUID(ctx context.Context, p Probe, uid int) Capability {
	...
	if uid == 0 && strings.TrimSpace(p.Env("SUDO_USER")) != "" {
		...
		if err == nil && strings.TrimSpace(string(active)) != "" {
			...
			if infoErr == nil && name == p.Env("SUDO_USER") && (kind == "x11" || kind == "wayland") && remote != presentationYes {
	...
	if display != "" {
		...
		if remoteValue != presentationYes && sshConnection == "" {
...
func isTruthy(value string) bool {
	...
	case "1", "true", presentationYes, "on":
		return true
	default:
		return false
	}
