...
func TestWithEnvDoesNotMutateCallerBackingArray(t *testing.T) {
	input := make([]string, 1, 3)
	input[0] = "PATH=/bin"
	got := withEnv(input, "DBUS_SESSION_BUS_ADDRESS", "unix:path="+testBus)
	if len(input) != 1 || input[0] != "PATH=/bin" {
		t.Fatalf("input was mutated: %v", input)
	}
	input = append(input, "HOME=/root")
	if len(got) != 2 || got[1] != "DBUS_SESSION_BUS_ADDRESS=unix:path="+testBus {
		t.Fatalf("result does not own the appended assignment: %v", got)
	}
}
