...
func TestRunAsInvokingUserWithSessionRestoresSudoUserBus(t *testing.T) {
	origRoot := RunningAsRootFn
	origRun := RunCommandFn
	defer func() {
		RunningAsRootFn = origRoot
	...
	var gotName string
	var gotArgs []string
	RunCommandFn = func(name string, args []string, _ EnsureOptions) error {
		gotName = name
		gotArgs = append([]string(nil), args...)
		return nil
	}

	if err := RunAsInvokingUserWithSession("systemctl", []string{"--user", "enable", "example.service"}, EnsureOptions{}); err != nil {
		t.Fatalf("RunAsInvokingUserWithSession: %v", err)
	}
	if gotName != "sudo" {
		t.Fatalf("name = %q, want sudo", gotName)
	}
	want := "-u alice -H -- env XDG_RUNTIME_DIR=/run/user/1000 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus systemctl --user enable example.service"
	if got := strings.Join(gotArgs, " "); got != want {
		t.Fatalf("args = %q, want %q", got, want)
	}
}

func TestRunAsInvokingUserWithSessionIgnoresStaleRootRuntimeDir(t *testing.T) {
	origRoot := RunningAsRootFn
	origRun := RunCommandFn
	...
	RunningAsRootFn = func() bool { return false }
	currentUserIDFn = func() string { return "1000" }
	t.Setenv("USER", "matthalloran8")
	t.Setenv("XDG_RUNTIME_DIR", "/run/user/0")
	...
	if err := RunAsInvokingUserWithSession("systemctl", []string{"--user", "is-active", "example.service"}, EnsureOptions{}); err != nil {
		t.Fatalf("RunAsInvokingUserWithSession: %v", err)
	}
	if gotName != "env" {
		t.Fatalf("name = %q, want env", gotName)
	}
	want := "XDG_RUNTIME_DIR=/run/user/1000 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus systemctl --user is-active example.service"
	if got := strings.Join(gotArgs, " "); got != want {
		t.Fatalf("args = %q, want %q", got, want)
	}
}
