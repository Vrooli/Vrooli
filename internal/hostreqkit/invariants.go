...
type Invariant = InvariantDeclaration

// DecodeInvariantDeclarations decodes the invariant portion of a safeguard
// manifest without coupling this evaluator to the safeguard package.
func DecodeInvariantDeclarations(data []byte) ([]Invariant, error) {
	var manifest struct {
		Invariants []Invariant `json:"invariants"`
	}
	if err := json.Unmarshal(data, &manifest); err != nil {
		return nil, fmt.Errorf("decode invariant declarations: %w", err)
	}
	return manifest.Invariants, nil
}
...
// IsNvidiaDriverPackage reports whether name is a recognized NVIDIA driver
// metapackage. The identity helpers live with host requirement capability
// evaluation so inventory and safeguard consumers share one spelling.
...
func resolveLinuxInvariant(invariant Invariant, facts Facts) Result {
	...
	if platforms := applicabilityValue(invariant.Applicability, "platforms"); platforms != "" && !slices.Contains(strings.Split(platforms, ","), facts.OS) {
		result.Verdict = NotApplicable
		result.Reason = "the invariant does not apply to this platform"
		return result
	}
	if vendor := applicabilityValue(invariant.Applicability, "vendor_id"); vendor != "" && facts.VendorID != "" && !strings.EqualFold(vendor, facts.VendorID) {
		result.Verdict = NotApplicable
...
func applicabilityValue(values map[string]any, key string) string {
	value := values[key]
