...
var seamExemptions = map[string]string{
	"cliinstall-part1.go:AtomicInstall": "removes only its own staging temp file, which no other process has ever seen",
	"uninstall-part2.go:Apply":          "dispatches to the Remover interface; the production implementation (fileRemover) routes through the seam, and tests inject a recorder that touches no disk",
}
